...
- Dependency-touching work: direct dependency inspection is mandatory when feasible; do not rely on assumptions, wrappers, or memory. Most dependencies are OSS, so read their source/docs/types. Codex-related work has a hard gate: the acting agent must personally inspect sibling `../codex` source for the exact protocol/runtime behavior before any verdict, comment, approval, merge recommendation, code change, or `proof sufficient` claim. If missing, clone `https://github.com/openai/codex.git` there first. Subagent reports, PR text, MarketingClaw wrappers, generated schemas, memory, and prior bot reviews do not satisfy this gate. No direct `../codex` check means no Codex verdict. Cite Codex files/lines checked in final/review/comment.
...
- MarketingClaw-specific review rules live here; generic ClawSweeper prompts stay repo-agnostic.
- ClawSweeper-owned schema, labels, close reasons, protected-label gates, maintainer-item gates, and mutation rules live in `marketingclaw/clawsweeper`.
...
- Public ClawSweeper comments prefer `https://docs.marketingclaw.ai/...` when a public docs page exists; structured evidence still cites repo files, lines, SHAs.
...
- Installers: sibling `../marketingclaw.ai`.
- Scoped guides: `extensions/`, `src/{plugin-sdk,channels,plugins,gateway,agents}/`, `packages/`, `test/helpers*/`, `docs/`, `ui/`, `scripts/`.

## Docs

- Source docs: `docs/**`; publish repo: `marketingclaw/docs`; host: `https://docs.marketingclaw.ai`.
- Flow: source -> `docs-sync-publish.yml` -> mirror build -> R2 -> Worker router.
- Docs AI: `marketingclaw/ask-molty`; see its `AGENTS.md`.

## Architecture

- Core stays plugin-agnostic. No bundled ids/defaults/policy in core when manifest/registry/capability contracts work.
- Plugins cross into core only via `marketingclaw/plugin-sdk/*`, manifest metadata, injected runtime helpers, documented barrels (`api.ts`, `runtime-api.ts`).
...
- State/storage migrations are database-first. Runtime reads/writes the canonical store only. Old file stores, sidecars, aliases, and fallback readers belong in `marketingclaw doctor --fix` migration code only, never steady-state runtime.
- Storage default: SQLite only. Do not add JSON/JSONL/TXT/sidecar files for MarketingClaw-owned runtime state, caches, queues, registries, indexes, cursors, checkpoints, or plugin scratch data.
...
- Use the shared state DB (`state/marketingclaw.sqlite`) for global runtime state and plugin KV data. Use the per-agent DB (`agents/<agentId>/agent/marketingclaw-agent.sqlite`) for agent-scoped state/cache. Use a dedicated SQLite DB only when schema, volume, or lifecycle clearly does not fit those stores.
...
- Checks in a Codex worktree or linked/sparse checkout: avoid direct local `pnpm check*`; use `node scripts/crabbox-wrapper.mjs run ... -- env MARKETINGCLAW_CHECK_CHANGED_REMOTE_CHILD=1 MARKETINGCLAW_CHANGED_LANES_RAW_SYNC=1 corepack pnpm check:changed` so pnpm runs inside the selected remote box, not locally.
...
- Use `$marketingclaw-testing` for test/CI choice and `$crabbox` for remote/full/E2E proof.
...
- Use `$marketingclaw-pr-maintainer` immediately for maintainer-side MarketingClaw issue/PR review, triage, duplicates, labels, comments, close, land, or evidence. Contributor PR creation/refresh follows the requested contributor workflow; linked refs alone do not require maintainer archive tooling.
...
- PR artifacts/screenshots: attach to PR/comment/external artifact store. Never push screenshots, videos, proof images, or proof assets to MarketingClaw or any product repo branch, including temp artifact branches. Use Crabbox artifact publishing plus the manifest URL. Do not commit `.github/pr-assets`.
- CI polling: exact SHA, relevant checks only, minimal fields. Skip routine noise (`Auto response`, `Labeler`, docs agents, performance/stale). Logs only after failure/completion or concrete need.
- Agent PR landing to `main`: use only the repo-native `scripts/pr` wrapper: run `scripts/pr review-init <PR>`, follow its emitted checkout/guard guidance, initialize and complete review artifacts with `scripts/pr review-artifacts-init <PR>`, validate them with `scripts/pr review-validate-artifacts <PR>`, then run `MARKETINGCLAW_TESTBOX=1 scripts/pr prepare-run <PR>` and `scripts/pr merge-run <PR>`. The Testbox flag is mandatory for agents so prepare verifies exact-head hosted CI/Testbox instead of running full gates locally; do not idle on `auto-response` or `check-docs`.
...
- Naming: **MarketingClaw** product/docs; `marketingclaw` CLI/package/path/config.
...
- Prefer injection and narrow `*.runtime.ts` mocks over broad barrels or `marketingclaw/plugin-sdk/*`.
- Do not edit baseline/inventory/ignore/snapshot/expected-failure files to silence checks without explicit approval.
- Do not run independent `pnpm test`/Vitest commands concurrently in one worktree; Vitest cache races with `ENOTEMPTY`. Group one command or use distinct `MARKETINGCLAW_VITEST_FS_MODULE_CACHE_PATH`.
- Test workers max 16. Memory pressure: `MARKETINGCLAW_VITEST_MAX_WORKERS=1 pnpm test`.
- Live: `MARKETINGCLAW_LIVE_TEST=1 pnpm test:live`; verbose `MARKETINGCLAW_LIVE_TEST_QUIET=0`.
...
- Docs final answers: include relevant full `https://docs.marketingclaw.ai/...` URL(s). If issue/PR work too, GitHub URL last.
...
- Release generation: derive `CHANGELOG.md` from merged PRs + all direct `main` commits. Entries: active `### Changes`/`### Fixes`, single-line, thank credited humans; never thank bots/forbidden handles: `@marketingclaw`, `@clawsweeper`, `@codex`, `@steipete`.
...
- Secrets: channel/provider creds in `~/.marketingclaw/credentials/`; model auth profiles in `~/.marketingclaw/agents/<agentId>/agent/auth-profiles.json`.
...
- Releases/publish/version bumps need explicit approval. Use `$release-marketingclaw-maintainer`.
...
- GHSA/advisories: `$marketingclaw-ghsa-maintainer` / `$security-triage`. Secret scanning: `$marketingclaw-secret-scanning-maintainer`.
...
- Mac gateway: dev watch = `pnpm gateway:watch`; managed installs = `marketingclaw gateway restart/status --deep`; logs = `./scripts/clawlog.sh`. No launchd/ad-hoc tmux.
...
- Version bump surfaces live in `$release-marketingclaw-maintainer`.
- Parallels: `$marketingclaw-parallels-smoke`; Discord roundtrip: `$parallels-discord-roundtrip`.
...
- Rebrand/migration/config warnings: run `marketingclaw doctor`.
