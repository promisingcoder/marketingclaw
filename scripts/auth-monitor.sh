...
#
# Suggested cron: */30 * * * * /path/to/marketingclaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send MarketingClaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., marketingclaw-alerts)
...
STATE_FILE="$HOME/.marketingclaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via MarketingClaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use marketingclaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via MarketingClaw to $NOTIFY_PHONE..."
            marketingclaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
    ...
    if [ -n "$NOTIFY_NTFY" ]; then
        ...
        curl -s -o /dev/null \
            -H "Title: MarketingClaw Auth Alert" \
            -H "Priority: $priority" \
            -H "Tags: warning,key" \
            -d "$message" \
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! MarketingClaw is down. Run on the MarketingClaw host: ${SCRIPT_DIR}/mobile-reauth.sh" "urgent"
    exit 1
