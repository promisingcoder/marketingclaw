...
function testLiveUsage() {
  return [
    ...
    "",
    "Runs live Vitest suites with MARKETINGCLAW_LIVE_TEST=1.",
    "",
    "Options:",
...
export function buildTestLiveEnv(args, baseEnv = process.env) {
  return {
    ...
    pnpm_config_verify_deps_before_run: baseEnv.pnpm_config_verify_deps_before_run || "false",
    MARKETINGCLAW_LIVE_TEST: baseEnv.MARKETINGCLAW_LIVE_TEST || "1",
    MARKETINGCLAW_LIVE_TEST_QUIET:
      args.quietOverride ?? baseEnv.MARKETINGCLAW_LIVE_TEST_QUIET ?? "1",
    ...(args.forceCodexHarness ? { MARKETINGCLAW_LIVE_CODEX_HARNESS: "1" } : {}),
  };
}
...
export function resolveTestLiveHeartbeatMs(baseEnv = process.env) {
  const value = baseEnv.MARKETINGCLAW_LIVE_WRAPPER_HEARTBEAT_MS;
  if (value === undefined || value === "") {
    return 20_000;
  }
  const text = value.trim();
  if (!/^\d+$/u.test(text)) {
    throw new Error(`invalid MARKETINGCLAW_LIVE_WRAPPER_HEARTBEAT_MS: ${text}`);
  }
