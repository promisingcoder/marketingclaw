...
resolve_default_smoke_platform() {
  local host_arch
  if [[ -n "${MARKETINGCLAW_INSTALL_SMOKE_PLATFORM:-}" ]]; then
    printf "%s" "$MARKETINGCLAW_INSTALL_SMOKE_PLATFORM"
    return
  fi
  host_arch="$(uname -m)"
...
const parsed = JSON.parse(raw);
const budgetOverride = process.env.MARKETINGCLAW_INSTALL_SMOKE_PACK_UNPACKED_BUDGET_BYTES;
const budgetBytes = budgetOverride ? Number(budgetOverride) : undefined;
if (budgetOverride && !Number.isFinite(budgetBytes)) {
  throw new Error(
    `MARKETINGCLAW_INSTALL_SMOKE_PACK_UNPACKED_BUDGET_BYTES must be numeric, got ${JSON.stringify(
      budgetOverride,
    )}`,
  );
}
...
SMOKE_IMAGE="${MARKETINGCLAW_INSTALL_SMOKE_IMAGE:-marketingclaw-install-smoke:local}"
NONROOT_IMAGE="${MARKETINGCLAW_INSTALL_NONROOT_IMAGE:-marketingclaw-install-nonroot:local}"
SMOKE_PLATFORM="$(resolve_default_smoke_platform)"
NONROOT_PLATFORM="${MARKETINGCLAW_INSTALL_NONROOT_PLATFORM:-$SMOKE_PLATFORM}"
INSTALL_URL="${MARKETINGCLAW_INSTALL_URL:-https://marketingclaw.bot/install.sh}"
CLI_INSTALL_URL="${MARKETINGCLAW_INSTALL_CLI_URL:-https://marketingclaw.bot/install-cli.sh}"
PACKAGE_NAME="${MARKETINGCLAW_INSTALL_PACKAGE:-marketingclaw}"
SKIP_NONROOT="${MARKETINGCLAW_INSTALL_SMOKE_SKIP_NONROOT:-0}"
SKIP_SMOKE_IMAGE_BUILD="${MARKETINGCLAW_INSTALL_SMOKE_SKIP_IMAGE_BUILD:-0}"
SKIP_NONROOT_IMAGE_BUILD="${MARKETINGCLAW_INSTALL_NONROOT_SKIP_IMAGE_BUILD:-0}"
SKIP_UPDATE="${MARKETINGCLAW_INSTALL_SMOKE_SKIP_UPDATE:-0}"
SKIP_NPM_GLOBAL="${MARKETINGCLAW_INSTALL_SMOKE_SKIP_NPM_GLOBAL:-0}"
SKIP_FRESHNESS="${MARKETINGCLAW_INSTALL_SMOKE_SKIP_FRESHNESS:-0}"
FRESHNESS_INSTALL_URL="${MARKETINGCLAW_INSTALL_SMOKE_FRESHNESS_INSTALL_URL:-file:///tmp/marketingclaw-install.sh}"
# npm min-release-age is days; 10000 keeps the control failure independent of normal release cadence.
FRESHNESS_MIN_RELEASE_AGE="${MARKETINGCLAW_INSTALL_FRESHNESS_MIN_RELEASE_AGE:-10000}"
FRESHNESS_NPM_VERSION="${MARKETINGCLAW_INSTALL_FRESHNESS_NPM_VERSION:-11.14.1}"
UPDATE_BASELINE_VERSION="${MARKETINGCLAW_INSTALL_SMOKE_UPDATE_BASELINE:-latest}"
UPDATE_PACKAGE_SPEC="${MARKETINGCLAW_INSTALL_SMOKE_UPDATE_PACKAGE_SPEC:-}"
UPDATE_DIST_IMAGE="${MARKETINGCLAW_INSTALL_SMOKE_UPDATE_DIST_IMAGE:-}"
UPDATE_SKIP_LOCAL_BUILD="${MARKETINGCLAW_INSTALL_SMOKE_UPDATE_SKIP_LOCAL_BUILD:-0}"
UPDATE_HOST_ALIAS="${MARKETINGCLAW_INSTALL_SMOKE_UPDATE_HOST:-host.docker.internal}"
UPDATE_PORT="${MARKETINGCLAW_INSTALL_SMOKE_UPDATE_PORT:-}"
...
NPM_CACHE_DIR="${MARKETINGCLAW_INSTALL_SMOKE_NPM_CACHE_DIR:-}"
NPM_CACHE_OWNED=0
NPM_CACHE_PREPARED=0
NPM_CACHE_DOCKER_ARGS=()
INSTALL_SCRIPT_DOCKER_ARGS=(
  -v "$ROOT_DIR/scripts/install.sh:/tmp/marketingclaw-install.sh:ro"
  -v "$ROOT_DIR/scripts/install-cli.sh:/tmp/marketingclaw-install-cli.sh:ro"
)
...
for env_name in \
  ...
  MARKETINGCLAW_INSTALL_SMOKE_SKIP_PREVIOUS; do
  env_value="${!env_name:-}"
...
prepare_update_tarball() {
  ...
  if [[ -n "$UPDATE_PACKAGE_SPEC" ]]; then
    ...
    package_tgz="$(node scripts/package-marketingclaw-for-docker.mjs "${package_args[@]}")"
    UPDATE_TGZ_FILE="$(basename "$package_tgz")"
  fi
  if [[ -z "$UPDATE_PACKAGE_SPEC" ]]; then
    node scripts/check-marketingclaw-package-tarball.mjs \
...
if [[ "$SKIP_UPDATE" == "1" ]]; then
  echo "==> Skip update smoke (MARKETINGCLAW_INSTALL_SMOKE_SKIP_UPDATE=1)"
else
  prepare_update_tarball
  prepare_update_host_access
  prepare_npm_cache
  ...
  run_install_smoke_container --rm -t \
    ...
    -e MARKETINGCLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e MARKETINGCLAW_INSTALL_METHOD=npm \
    -e MARKETINGCLAW_INSTALL_FRESH_VERSION="$UPDATE_EXPECT_VERSION" \
    -e MARKETINGCLAW_INSTALL_FRESH_TAG_URL="$FRESH_TAG_URL" \
    -e MARKETINGCLAW_INSTALL_LATEST_OUT="/out/latest" \
    -e MARKETINGCLAW_NO_ONBOARD=1 \
    -e MARKETINGCLAW_NO_PROMPT=1 \
  ...
  run_install_smoke_container --rm -t \
    ...
    ${SMOKE_RUNNER_ENV_ARGS[@]+"${SMOKE_RUNNER_ENV_ARGS[@]}"} \
    -e MARKETINGCLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e MARKETINGCLAW_INSTALL_SMOKE_MODE=update \
    -e MARKETINGCLAW_INSTALL_UPDATE_BASELINE="$UPDATE_BASELINE_VERSION" \
    -e MARKETINGCLAW_INSTALL_UPDATE_BASELINE_TAG_URL="$BASELINE_TAG_URL" \
    -e MARKETINGCLAW_INSTALL_UPDATE_EXPECT_VERSION="$UPDATE_EXPECT_VERSION" \
    -e MARKETINGCLAW_INSTALL_UPDATE_TAG_URL="$UPDATE_TAG_URL" \
    -e MARKETINGCLAW_NO_ONBOARD=1 \
    -e MARKETINGCLAW_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$SMOKE_IMAGE"
  ...
  if [[ "$SKIP_NPM_GLOBAL" == "1" ]]; then
    echo "==> Skip direct npm global smoke (MARKETINGCLAW_INSTALL_SMOKE_SKIP_NPM_GLOBAL=1)"
  else
    ...
    run_install_smoke_container --rm -t \
      ...
      ${SMOKE_RUNNER_ENV_ARGS[@]+"${SMOKE_RUNNER_ENV_ARGS[@]}"} \
      -e MARKETINGCLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
      -e MARKETINGCLAW_INSTALL_SMOKE_MODE=npm-global \
      -e MARKETINGCLAW_INSTALL_UPDATE_BASELINE="$UPDATE_BASELINE_VERSION" \
      -e MARKETINGCLAW_INSTALL_UPDATE_BASELINE_TAG_URL="$BASELINE_TAG_URL" \
      -e MARKETINGCLAW_INSTALL_UPDATE_EXPECT_VERSION="$UPDATE_EXPECT_VERSION" \
      -e MARKETINGCLAW_INSTALL_UPDATE_TAG_URL="$UPDATE_TAG_URL" \
...
if [[ "$SKIP_FRESHNESS" == "1" ]]; then
  echo "==> Skip installer npm freshness smoke (MARKETINGCLAW_INSTALL_SMOKE_SKIP_FRESHNESS=1)"
else
  prepare_npm_cache
  ...
  run_install_smoke_container --rm -t \
    ...
    "${INSTALL_SCRIPT_DOCKER_ARGS[@]}" \
    ${SMOKE_RUNNER_ENV_ARGS[@]+"${SMOKE_RUNNER_ENV_ARGS[@]}"} \
    -e MARKETINGCLAW_INSTALL_URL="$FRESHNESS_INSTALL_URL" \
    -e MARKETINGCLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e MARKETINGCLAW_INSTALL_SMOKE_MODE=freshness \
    -e MARKETINGCLAW_INSTALL_FRESHNESS_VERSION="${MARKETINGCLAW_INSTALL_FRESHNESS_VERSION:-latest}" \
    -e MARKETINGCLAW_INSTALL_FRESHNESS_MIN_RELEASE_AGE="$FRESHNESS_MIN_RELEASE_AGE" \
    -e MARKETINGCLAW_INSTALL_FRESHNESS_NPM_VERSION="$FRESHNESS_NPM_VERSION" \
    -e MARKETINGCLAW_NO_ONBOARD=1 \
    -e MARKETINGCLAW_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$SMOKE_IMAGE"
...
if [[ "$SKIP_NONROOT" == "1" ]]; then
  echo "==> Skip non-root installer smoke (MARKETINGCLAW_INSTALL_SMOKE_SKIP_NONROOT=1)"
else
  ...
  run_install_smoke_container --rm -t \
    ...
    -e MARKETINGCLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e MARKETINGCLAW_INSTALL_METHOD=npm \
    -e MARKETINGCLAW_INSTALL_EXPECT_VERSION="$LATEST_VERSION" \
    -e MARKETINGCLAW_NO_ONBOARD=1 \
    -e MARKETINGCLAW_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$NONROOT_IMAGE"
...
if [[ "${MARKETINGCLAW_INSTALL_SMOKE_SKIP_CLI:-0}" == "1" ]]; then
  echo "==> Skip CLI installer smoke (MARKETINGCLAW_INSTALL_SMOKE_SKIP_CLI=1)"
  exit 0
fi
...
run_install_smoke_container --rm -t \
  ...
  "${INSTALL_SCRIPT_DOCKER_ARGS[@]}" \
  -e MARKETINGCLAW_INSTALL_URL="$INSTALL_URL" \
  -e MARKETINGCLAW_INSTALL_CLI_URL="$CLI_INSTALL_URL" \
  -e MARKETINGCLAW_NO_ONBOARD=1 \
  -e MARKETINGCLAW_NO_PROMPT=1 \
  -e DEBIAN_FRONTEND=noninteractive \
