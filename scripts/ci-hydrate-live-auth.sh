...
profile_path="${1:-${RUNNER_TEMP:-/tmp}/marketingclaw-live.profile}"
...
activate_claude_oauth_access_token() {
  local credentials="${MARKETINGCLAW_CLAUDE_CREDENTIALS_JSON:-}"
...
for env_key in \
  ...
  MARKETINGCLAW_LIVE_BROWSER_CDP_URL \
  MARKETINGCLAW_LIVE_SETUP_TOKEN \
  MARKETINGCLAW_LIVE_SETUP_TOKEN_MODEL \
  MARKETINGCLAW_LIVE_SETUP_TOKEN_PROFILE \
  MARKETINGCLAW_LIVE_SETUP_TOKEN_VALUE \
  FACTORY_API_KEY \
  GEMINI_API_KEY \
  GOOGLE_API_KEY \
  OPENROUTER_API_KEY \
...
write_secret_file "$HOME/.codex/auth.json" MARKETINGCLAW_CODEX_AUTH_JSON
write_secret_file "$HOME/.codex/config.toml" MARKETINGCLAW_CODEX_CONFIG_TOML
write_secret_file "$HOME/.claude.json" MARKETINGCLAW_CLAUDE_JSON
write_secret_file "$HOME/.claude/.credentials.json" MARKETINGCLAW_CLAUDE_CREDENTIALS_JSON
write_secret_file "$HOME/.claude/settings.json" MARKETINGCLAW_CLAUDE_SETTINGS_JSON
write_secret_file "$HOME/.claude/settings.local.json" MARKETINGCLAW_CLAUDE_SETTINGS_LOCAL_JSON
write_secret_file "$HOME/.gemini/settings.json" MARKETINGCLAW_GEMINI_SETTINGS_JSON

if [[ -n "${GITHUB_ENV:-}" ]]; then
  {
    echo "MARKETINGCLAW_PROFILE_FILE=$profile_path"
  } >>"$GITHUB_ENV"
fi
