...
process.env.MARKETINGCLAW_DISABLE_BUNDLED_ENTRY_SOURCE_FALLBACK ??= "1";

const { packageRoot } = parsePackageRootArg(
  process.argv.slice(2),
  "MARKETINGCLAW_BUNDLED_CHANNEL_SMOKE_ROOT",
);
...
const installedLayoutEnv = "MARKETINGCLAW_BUNDLED_CHANNEL_SMOKE_INSTALLED_LAYOUT";
...
function packageRootLooksInstalled(root) {
  return root.replaceAll("\\", "/").endsWith("/node_modules/marketingclaw");
}
...
function smokeInInstalledLayoutIfNeeded() {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-channel-entry-smoke-"));
  const nodeModulesRoot = path.join(tempRoot, "node_modules");
  const installedPackageRoot = path.join(nodeModulesRoot, "marketingclaw");
...
function collectBundledChannelEntryFiles() {
  ...
  for (const dirent of fs.readdirSync(distExtensionsRoot, { withFileTypes: true })) {
    ...
    const packageJson = readJson(packageJsonPath);
    if (!packageJson.marketingclaw?.channel) {
      continue;
    }
    ...
    const extensionEntries =
      Array.isArray(packageJson.marketingclaw.extensions) &&
      packageJson.marketingclaw.extensions.length > 0
    ...
    const setupEntry = packageJson.marketingclaw.setupEntry;
