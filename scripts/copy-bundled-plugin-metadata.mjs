...
function shouldCopyBundledPluginMetadata(id, env, buildablePluginDirs) {
  ...
  return env.MARKETINGCLAW_BUILD_PRIVATE_QA === "1";
}
...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const distManifestPath = path.join(distPluginDir, "marketingclaw.plugin.json");
    ...
    if (packageJson.marketingclaw && "extensions" in packageJson.marketingclaw) {
      packageJson.marketingclaw = {
        ...packageJson.marketingclaw,
        extensions: rewritePackageExtensions(packageJson.marketingclaw.extensions),
        ...(typeof packageJson.marketingclaw.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.marketingclaw.setupEntry) }
          : {}),
      };
    }
