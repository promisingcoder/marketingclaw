...
# Checks both Claude Code and MarketingClaw auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
MARKETINGCLAW_AUTH="$HOME/.marketingclaw/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    marketingclaw models status --json 2>/dev/null || true
}
...
check_marketingclaw_auth() {
    ...
    expires=$(jq -r '
        ...
        | max // 0
    ' "$MARKETINGCLAW_AUTH" 2>/dev/null || echo "0")
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    marketingclaw_status=$(check_marketingclaw_auth 2>/dev/null || true)

    claude_expires=0
    marketingclaw_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        marketingclaw_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        marketingclaw_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$MARKETINGCLAW_AUTH" 2>/dev/null || echo "0")
    fi

    jq -n \
        --arg cs "$claude_status" \
        --arg ce "$claude_expires" \
        --arg bs "$marketingclaw_status" \
        --arg be "$marketingclaw_expires" \
        '{
            claude_code: {status: $cs, expires_at_ms: ($ce | tonumber)},
            marketingclaw: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$marketingclaw_status" == EXPIRED* ]] || [[ "$marketingclaw_status" == MISSING* ]]; then
        echo "MARKETINGCLAW_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$marketingclaw_status" == EXPIRING* ]]; then
        echo "MARKETINGCLAW_EXPIRING"
        exit 2
    else
        echo "OK"
        exit 0
...
echo ""
echo "MarketingClaw Auth (~/.marketingclaw/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$MARKETINGCLAW_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$MARKETINGCLAW_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'marketingclaw doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active marketingclaw >/dev/null 2>&1; then
    echo -e "MarketingClaw service: ${GREEN}running${NC}"
else
    echo -e "MarketingClaw service: ${RED}NOT running${NC}"
fi
