...
- `marketingclaw-rtt` Discord main rows appeared to jump from about 5-7s p50 to about
...
- `marketingclaw-rtt` `scripts/import-discord-rtt.mjs` falls back to whole summary
...
- Testbox `tbx_01krvces8y0c99nzra2a90jg13` ran
  `pnpm marketingclaw qa suite --scenario channel-chat-baseline` and emitted gateway
...
- Testbox `tbx_01krwbsg15xvjdgpcz8fxq1htz` ran
  `MARKETINGCLAW_QA_GATEWAY_HEAP_CHECKPOINTS=1 pnpm marketingclaw qa suite --scenario channel-chat-baseline`.
...
- After rebasing onto `b5046968f61`, a fresh Testbox `pnpm check:changed`
  ...
  `marketingclaw/plugin-sdk/llm@0.74.1` under `minimumReleaseAge`.
...
- Follow-up branch `perf/discord-rtt-summary-import` in `marketingclaw-rtt` updates
...
- The branch fixes MarketingClaw artifact quality. `marketingclaw-rtt` has a paired
