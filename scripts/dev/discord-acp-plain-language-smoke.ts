#!/usr/bin/env bun
// Discord Acp Plain Language Smoke script supports MarketingClaw repository automation.
...
type DriverMode = "token" | "webhook" | "marketingclaw";
...
const DEFAULT_MARKETINGCLAW_CLI_TIMEOUT_MS = 60_000;
...
function resolveStateDir(): string {
  const override = process.env.MARKETINGCLAW_STATE_DIR?.trim();
  if (override) {
    if (override === "~") {
  ...
  const home = process.env.MARKETINGCLAW_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".marketingclaw");
}
...
function parseDriverMode(raw: string): DriverMode {
  ...
  if (normalized === "token" || normalized === "webhook" || normalized === "marketingclaw") {
    return normalized;
  }
  throw new Error(
    `Invalid --driver value ${JSON.stringify(raw)}; expected token, webhook, or marketingclaw.`,
  );
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver marketingclaw] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) MarketingClaw spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|marketingclaw> Driver transport mode (default: token)\n" +
    ...
    "  --state-dir <p>              Override MarketingClaw state dir for plugin-state polling\n" +
    "  --marketingclaw-bin <path>        MarketingClaw CLI binary for driver=marketingclaw (default: marketingclaw)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  MARKETINGCLAW_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  MARKETINGCLAW_DISCORD_SMOKE_DRIVER\n" +
    "  MARKETINGCLAW_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  MARKETINGCLAW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  MARKETINGCLAW_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  MARKETINGCLAW_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  MARKETINGCLAW_DISCORD_SMOKE_AGENT\n" +
    "  MARKETINGCLAW_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  MARKETINGCLAW_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  MARKETINGCLAW_DISCORD_SMOKE_POLL_MS\n" +
    "  MARKETINGCLAW_STATE_DIR\n" +
    "  MARKETINGCLAW_DISCORD_SMOKE_MARKETINGCLAW_BIN"
  );
}
...
function parseArgs(argv = process.argv.slice(2)): Args {
  ...
  const channelId =
    resolveArg("--channel", argv) || process.env.MARKETINGCLAW_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver", argv) || process.env.MARKETINGCLAW_DISCORD_SMOKE_DRIVER || "token";
  const driverMode = parseDriverMode(driverModeRaw);
  const driverToken =
    resolveArg("--token", argv) || process.env.MARKETINGCLAW_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix", argv) ||
    process.env.MARKETINGCLAW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX ||
    "Bot";
  const botToken =
    resolveArg("--bot-token", argv) ||
    process.env.MARKETINGCLAW_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix", argv) ||
    process.env.MARKETINGCLAW_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent =
    resolveArg("--agent", argv) || process.env.MARKETINGCLAW_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention", argv) ||
    process.env.MARKETINGCLAW_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction", argv) ||
    process.env.MARKETINGCLAW_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms", argv) || process.env.MARKETINGCLAW_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
    "--timeout-ms",
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms", argv) || process.env.MARKETINGCLAW_DISCORD_SMOKE_POLL_MS,
    1_500,
    "--poll-ms",
  );
  const stateDir = path.resolve(resolveArg("--state-dir", argv) || resolveStateDir());
  const marketingclawBin =
    resolveArg("--marketingclaw-bin", argv) ||
    process.env.MARKETINGCLAW_DISCORD_SMOKE_MARKETINGCLAW_BIN ||
    "marketingclaw";
...
async function marketingclawCliJson<T>(params: {
  marketingclawBin: string;
  args: string[];
  timeoutMs?: number;
}): Promise<T> {
  const result = await execFileAsync(params.marketingclawBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
    timeout: params.timeoutMs ?? DEFAULT_MARKETINGCLAW_CLI_TIMEOUT_MS,
    killSignal: "SIGKILL",
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`marketingclaw ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithMarketingclaw(params: {
  marketingclawBin: string;
  target: string;
  limit: number;
  timeoutMs?: number;
}): Promise<DiscordMessage[]> {
  const response = await marketingclawCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    marketingclawBin: params.marketingclawBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "marketingclaw") {
    return await readMessagesWithMarketingclaw({
      marketingclawBin: params.args.marketingclawBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        timeoutMs: remainingTimeoutMs(deadline),
        body: {
          name: `marketingclaw-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      minBindingBoundAt = Date.now() - 3_000;
      const sent = await marketingclawCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
        ...
        marketingclawBin: args.marketingclawBin,
        args: [
          "message",
          "send",
          "--channel",
      ...
      if (!sentMessageId) {
        throw new Error("marketingclaw message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "marketingclaw"
            ? await readMessagesWithMarketingclaw({
                marketingclawBin: args.marketingclawBin,
                target: threadId,
                limit: 50,
                timeoutMs: remainingTimeoutMs(deadline),
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from MarketingClaw.`,
        diagnostics: {
          bindingCandidates: [
            {
