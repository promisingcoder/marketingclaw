...
const MANUAL_SECTION_START = "<!-- marketingclaw-plugin-reference:manual-start -->";
const MANUAL_SECTION_END = "<!-- marketingclaw-plugin-reference:manual-end -->";
...
function resolveDescription({ manifest, packageJson }) {
  ...
  if (channels.length > 0) {
    ...
    return `Adds the ${channelLabel}${channelNoun} surface for sending and receiving MarketingClaw messages.`;
  }

  const providers = Array.isArray(manifest.providers) ? manifest.providers : [];
  if (providers.length > 0) {
    return `Adds ${displayList(providers)} model provider support to MarketingClaw.`;
  }
  ...
  return packageDescription ? `${packageDescription}.` : "Provides an MarketingClaw plugin.";
}
...
function resolveDocs({ dirName, manifest, packageJson }) {
  ...
  const channelDoc = normalizeDocPath(packageJson.marketingclaw?.channel?.docsPath);
  if (channelDoc) {
...
function resolveInstallRoute(packageJson, status) {
  ...
  if (status === "core") {
    const release = packageJson.marketingclaw?.release;
    if (release?.publishToClawHub === true || release?.publishToNpm === true) {
      return `included in MarketingClaw; ${resolveInstallRoute(packageJson, "external")}`;
    }
    return "included in MarketingClaw";
  }
  const install = packageJson.marketingclaw?.install;
  const release = packageJson.marketingclaw?.release;
  const clawhubSpec =
...
function resolveStatus({ dirName, packageJson, excludedDirs }) {
  ...
  const hasInstallSpec =
    typeof packageJson.marketingclaw?.install?.clawhubSpec === "string" ||
...
function collectPluginSourceEntries() {
  ...
  for (const dirName of fs
    ...
    const manifestPath = path.join(EXTENSIONS_DIR, dirName, "marketingclaw.plugin.json");
