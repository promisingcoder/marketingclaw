...
has_native_swift_changes() {
  if git rev-parse --verify --quiet origin/main >/dev/null; then
    if git diff --name-only --relative origin/main...HEAD -- apps/macos apps/ios apps/shared/MarketingClawKit | rg -q .; then
      return 0
    fi
  fi

  if git rev-parse --verify --quiet HEAD^ >/dev/null; then
    git diff --name-only --relative HEAD^..HEAD -- apps/macos apps/ios apps/shared/MarketingClawKit | rg -q .
    return $?
  fi

  git show --name-only --relative --pretty='' HEAD -- apps/macos apps/ios apps/shared/MarketingClawKit | rg -q .
}
...
run_linux_ci_mirror() {
  ...
  log_step "MARKETINGCLAW_VITEST_MAX_WORKERS=${MARKETINGCLAW_VITEST_MAX_WORKERS:-1} NODE_OPTIONS=${NODE_OPTIONS:---max-old-space-size=6144} pnpm test"
...
run_macos_ci_mirror() {
  if [[ "${MARKETINGCLAW_PREPUSH_SKIP_MACOS:-0}" == "1" ]]; then
    log_step "Skipping macOS mirror because MARKETINGCLAW_PREPUSH_SKIP_MACOS=1"
    return 0
  fi
  ...
  run_step swiftformat --lint apps/macos/Sources --config config/swiftformat --exclude '**/MarketingClawProtocol,**/HostEnvSecurityPolicy.generated.swift'
