...
// Builds shared Docker images, prepares one MarketingClaw npm tarball, assigns lanes
...
const ROOT_DIR = path.resolve(process.env.MARKETINGCLAW_DOCKER_E2E_REPO_ROOT || SCRIPT_ROOT_DIR);
...
const DEFAULT_GITHUB_WORKFLOW = "marketingclaw-live-and-e2e-checks-reusable.yml";
...
function dockerAllUsage() {
  return [
    ...
    "",
    "Lane selection and scheduler settings are configured with MARKETINGCLAW_DOCKER_ALL_* env vars.",
  ].join("\n");
}
...
function resourceLimitEnvName(resource) {
  return `MARKETINGCLAW_DOCKER_ALL_${resource.toUpperCase().replace(/[^A-Z0-9]+/g, "_")}_LIMIT`;
}
...
function parseSchedulerOptions(env, parallelism) {
  const weightLimit = parsePositiveInt(
    env.MARKETINGCLAW_DOCKER_ALL_WEIGHT_LIMIT,
    parallelism,
    "MARKETINGCLAW_DOCKER_ALL_WEIGHT_LIMIT",
  );
...
function appendExtension(env, extension) {
  const current = env.MARKETINGCLAW_DOCKER_BUILD_EXTENSIONS ?? env.MARKETINGCLAW_EXTENSIONS ?? "";
  const tokens = current.split(/\s+/).filter(Boolean);
  if (!tokens.includes(extension)) {
    tokens.push(extension);
  }
  env.MARKETINGCLAW_DOCKER_BUILD_EXTENSIONS = tokens.join(" ");
}
...
function githubWorkflowRef() {
  const explicit = process.env.MARKETINGCLAW_DOCKER_E2E_WORKFLOW_REF;
  if (explicit) {
    return explicit;
  }
...
function githubWorkflowRerunCommand(laneNames, ref) {
  ...
  const releasePath = process.env.MARKETINGCLAW_DOCKER_ALL_PROFILE === RELEASE_PATH_PROFILE;
  const fields = [
    "gh workflow run",
    shellQuote(process.env.MARKETINGCLAW_DOCKER_E2E_WORKFLOW || DEFAULT_GITHUB_WORKFLOW),
  ...
  if (process.env.GITHUB_RUN_ID) {
    ...
    fields.push(
      ...
      `package_artifact_name=${shellQuote(
        process.env.MARKETINGCLAW_DOCKER_E2E_PACKAGE_ARTIFACT_NAME || "docker-e2e-package",
      )}`,
    );
  }
  if (process.env.MARKETINGCLAW_UPGRADE_SURVIVOR_BASELINE_SPEC) {
    fields.push(
      "-f",
      `published_upgrade_survivor_baseline=${shellQuote(process.env.MARKETINGCLAW_UPGRADE_SURVIVOR_BASELINE_SPEC)}`,
    );
  }
  if (process.env.MARKETINGCLAW_UPGRADE_SURVIVOR_BASELINE_SPECS) {
    fields.push(
      "-f",
      `published_upgrade_survivor_baselines=${shellQuote(process.env.MARKETINGCLAW_UPGRADE_SURVIVOR_BASELINE_SPECS)}`,
    );
  }
  if (process.env.MARKETINGCLAW_UPGRADE_SURVIVOR_SCENARIOS) {
    fields.push(
      "-f",
      `published_upgrade_survivor_scenarios=${shellQuote(process.env.MARKETINGCLAW_UPGRADE_SURVIVOR_SCENARIOS)}`,
    );
  }
  if (process.env.MARKETINGCLAW_DOCKER_E2E_BARE_IMAGE) {
    fields.push(
      "-f",
      `docker_e2e_bare_image=${shellQuote(process.env.MARKETINGCLAW_DOCKER_E2E_BARE_IMAGE)}`,
    );
  }
  if (process.env.MARKETINGCLAW_DOCKER_E2E_FUNCTIONAL_IMAGE) {
    fields.push(
      "-f",
      `docker_e2e_functional_image=${shellQuote(process.env.MARKETINGCLAW_DOCKER_E2E_FUNCTIONAL_IMAGE)}`,
    );
  }
  return fields.join(" ");
}
...
function buildLaneRerunCommand(name, baseEnv) {
  ...
  const image = poolLane
    ...
    : baseEnv.MARKETINGCLAW_DOCKER_E2E_IMAGE;
  const env = [
    ["MARKETINGCLAW_DOCKER_ALL_LANES", name],
    ["MARKETINGCLAW_DOCKER_ALL_BUILD", build],
    ["MARKETINGCLAW_DOCKER_ALL_PREFLIGHT", "0"],
    ["MARKETINGCLAW_SKIP_DOCKER_BUILD", "1"],
    ["MARKETINGCLAW_DOCKER_E2E_IMAGE", image || DEFAULT_E2E_IMAGE],
    ["MARKETINGCLAW_DOCKER_E2E_BARE_IMAGE", baseEnv.MARKETINGCLAW_DOCKER_E2E_BARE_IMAGE],
    [
      "MARKETINGCLAW_DOCKER_E2E_FUNCTIONAL_IMAGE",
      baseEnv.MARKETINGCLAW_DOCKER_E2E_FUNCTIONAL_IMAGE,
    ],
    ["MARKETINGCLAW_CURRENT_PACKAGE_TGZ", baseEnv.MARKETINGCLAW_CURRENT_PACKAGE_TGZ],
    [
      "MARKETINGCLAW_UPGRADE_SURVIVOR_BASELINE_SPEC",
      baseEnv.MARKETINGCLAW_UPGRADE_SURVIVOR_BASELINE_SPEC,
    ],
    [
      "MARKETINGCLAW_UPGRADE_SURVIVOR_BASELINE_SPECS",
      baseEnv.MARKETINGCLAW_UPGRADE_SURVIVOR_BASELINE_SPECS,
    ],
    ["MARKETINGCLAW_UPGRADE_SURVIVOR_SCENARIOS", baseEnv.MARKETINGCLAW_UPGRADE_SURVIVOR_SCENARIOS],
  ];
  if (baseEnv.MARKETINGCLAW_DOCKER_ALL_PNPM_COMMAND) {
    env.push([
      "MARKETINGCLAW_DOCKER_ALL_PNPM_COMMAND",
      baseEnv.MARKETINGCLAW_DOCKER_ALL_PNPM_COMMAND,
    ]);
  }
  return `${env
...
function withResolvedPnpmCommand(command, env) {
  const pnpmCommand = env.MARKETINGCLAW_DOCKER_ALL_PNPM_COMMAND?.trim();
  if (!pnpmCommand) {
    return command;
  }
...
function liveDockerHarnessScriptCommand(script) {
  return `bash -c 'harness="\${MARKETINGCLAW_DOCKER_E2E_TRUSTED_HARNESS_DIR:-${LIVE_DOCKER_DEFAULT_HARNESS_DIR}}"; MARKETINGCLAW_LIVE_DOCKER_REPO_ROOT="\${MARKETINGCLAW_DOCKER_E2E_REPO_ROOT:-$PWD}" bash "$harness/scripts/${script}"'`;
}
...
export async function writeRunSummary(logDir, summary) {
  ...
  const payload = {
    ...summary,
    packageArtifactName: process.env.MARKETINGCLAW_DOCKER_E2E_PACKAGE_ARTIFACT_NAME || undefined,
    ...
    github: {
      ...
      runUrl:
        process.env.GITHUB_SERVER_URL && process.env.GITHUB_REPOSITORY && process.env.GITHUB_RUN_ID
          ...
          : undefined,
      selectedSha: process.env.MARKETINGCLAW_DOCKER_E2E_SELECTED_SHA || undefined,
...
async function writeFailureIndex(logDir, summary) {
  const ref =
    ...
    process.env.MARKETINGCLAW_DOCKER_E2E_SELECTED_SHA ||
    summary.github?.sha ||
    summary.github?.ref ||
  ...
  const failureIndex = {
    ...
    version: 1,
    workflow: process.env.MARKETINGCLAW_DOCKER_E2E_WORKFLOW || DEFAULT_GITHUB_WORKFLOW,
  };
...
export function dockerPreflightContainerNames(raw) {
  return raw
    ...
    .filter((name) =>
      /^(?:marketingclaw-[a-z0-9-]+-e2e-\d+|marketingclaw-openwebui(?:-gateway)?-\d+)$/u.test(name),
    );
}
...
async function recordCleanupSmokeFailure(error, baseEnv, logDir, command, startedAtMs) {
  ...
  return {
    ...
    elapsedSeconds: phaseElapsedSeconds(startedAtMs),
    finishedAt: new Date().toISOString(),
    image: baseEnv.MARKETINGCLAW_DOCKER_E2E_IMAGE,
    logFile,
...
async function prepareMarketingClawPackage(baseEnv, logDir) {
  const existing = baseEnv.MARKETINGCLAW_CURRENT_PACKAGE_TGZ;
  if (existing) {
    const packageTgz = path.resolve(existing);
    baseEnv.MARKETINGCLAW_CURRENT_PACKAGE_TGZ = packageTgz;
    baseEnv.MARKETINGCLAW_BUNDLED_CHANNEL_HOST_BUILD = "0";
    baseEnv.MARKETINGCLAW_NPM_ONBOARD_HOST_BUILD = "0";
    console.log(`==> MarketingClaw package: ${packageTgz}`);
    return;
  }

  const packDir = path.join(logDir, "marketingclaw-package");
  await mkdir(packDir, { recursive: true });
  const packageTgz = path.join(packDir, "marketingclaw-current.tgz");
  await runForeground(
    "Prepare MarketingClaw package once",
    `node scripts/package-marketingclaw-for-docker.mjs --output-dir ${shellQuote(packDir)} --output-name marketingclaw-current.tgz`,
    baseEnv,
  );
  ...
  baseEnv.MARKETINGCLAW_BUNDLED_CHANNEL_HOST_BUILD = "0";
  baseEnv.MARKETINGCLAW_NPM_ONBOARD_HOST_BUILD = "0";
  console.log(`==> MarketingClaw package: ${baseEnv.MARKETINGCLAW_CURRENT_PACKAGE_TGZ}`);
}

function e2eImageForLane(poolLane, baseEnv) {
  if (poolLane.e2eImageKind === "bare") {
    return baseEnv.MARKETINGCLAW_DOCKER_E2E_BARE_IMAGE;
  }
  if (poolLane.e2eImageKind === "functional") {
    return baseEnv.MARKETINGCLAW_DOCKER_E2E_FUNCTIONAL_IMAGE;
  }
  return undefined;
}
...
function laneEnv(poolLane, baseEnv, logDir, cacheKey) {
  ...
  env.MARKETINGCLAW_DOCKER_ALL_LANE_NAME = name;
  const image = e2eImageForLane(poolLane, baseEnv);
  if (image) {
    env.MARKETINGCLAW_DOCKER_E2E_IMAGE = image;
  }
  if (poolLane.e2eImageKind) {
    env.MARKETINGCLAW_DOCKER_E2E_IMAGE_KIND = poolLane.e2eImageKind;
  }
  const cacheName = cacheKey || name;
  if (!process.env.MARKETINGCLAW_DOCKER_CLI_TOOLS_DIR) {
    env.MARKETINGCLAW_DOCKER_CLI_TOOLS_DIR = path.join(logDir, `${cacheName}-cli-tools`);
  }
  if (!process.env.MARKETINGCLAW_DOCKER_CACHE_HOME_DIR) {
    env.MARKETINGCLAW_DOCKER_CACHE_HOME_DIR = path.join(logDir, `${cacheName}-cache`);
  }
  return env;
}
...
async function runLane(lane, baseEnv, logDir, fallbackTimeoutMs) {
  ...
  await mkdir(env.MARKETINGCLAW_DOCKER_CLI_TOOLS_DIR, { recursive: true });
  await mkdir(env.MARKETINGCLAW_DOCKER_CACHE_HOME_DIR, { recursive: true });
  await fs.promises.writeFile(
    logFile,
    [
      `==> [${name}] cli tools dir: ${env.MARKETINGCLAW_DOCKER_CLI_TOOLS_DIR}`,
      `==> [${name}] cache dir: ${env.MARKETINGCLAW_DOCKER_CACHE_HOME_DIR}`,
      ...
      `==> [${name}] e2e image: ${env.MARKETINGCLAW_DOCKER_E2E_IMAGE ?? ""}`,
      "",
    ].join("\n"),
  );
  ...
  return {
    ...
    attempts,
    finishedAt: new Date().toISOString(),
    image: env.MARKETINGCLAW_DOCKER_E2E_IMAGE,
      ...
      if (running.size === 0) {
        ...
        throw new Error(
          `No Docker lanes fit scheduler limits (${describeDockerSchedulerLimits(
            parallelism,
            options,
          )}): ${blocked}. Tune MARKETINGCLAW_DOCKER_ALL_PARALLELISM, MARKETINGCLAW_DOCKER_ALL_WEIGHT_LIMIT, or MARKETINGCLAW_DOCKER_ALL_<RESOURCE>_LIMIT.`,
        );
      }
...
async function main() {
  ...
  const parallelism = parsePositiveInt(
    process.env.MARKETINGCLAW_DOCKER_ALL_PARALLELISM,
    DEFAULT_PARALLELISM,
    "MARKETINGCLAW_DOCKER_ALL_PARALLELISM",
  );
  const tailParallelism = parsePositiveInt(
    process.env.MARKETINGCLAW_DOCKER_ALL_TAIL_PARALLELISM,
    Math.min(parallelism, DEFAULT_TAIL_PARALLELISM),
    "MARKETINGCLAW_DOCKER_ALL_TAIL_PARALLELISM",
  );
  const tailLines = parsePositiveInt(
    process.env.MARKETINGCLAW_DOCKER_ALL_FAILURE_TAIL_LINES,
    DEFAULT_FAILURE_TAIL_LINES,
    "MARKETINGCLAW_DOCKER_ALL_FAILURE_TAIL_LINES",
  );
  const laneTimeoutMs = parsePositiveInt(
    process.env.MARKETINGCLAW_DOCKER_ALL_LANE_TIMEOUT_MS,
    DEFAULT_LANE_TIMEOUT_MS,
    "MARKETINGCLAW_DOCKER_ALL_LANE_TIMEOUT_MS",
  );
  const laneStartStaggerMs = parseNonNegativeInt(
    process.env.MARKETINGCLAW_DOCKER_ALL_START_STAGGER_MS,
    DEFAULT_LANE_START_STAGGER_MS,
    "MARKETINGCLAW_DOCKER_ALL_START_STAGGER_MS",
  );
  const statusIntervalMs = parseNonNegativeInt(
    process.env.MARKETINGCLAW_DOCKER_ALL_STATUS_INTERVAL_MS,
    DEFAULT_STATUS_INTERVAL_MS,
    "MARKETINGCLAW_DOCKER_ALL_STATUS_INTERVAL_MS",
  );
  const preflightRunTimeoutMs = parsePositiveInt(
    process.env.MARKETINGCLAW_DOCKER_ALL_PREFLIGHT_RUN_TIMEOUT_MS,
    DEFAULT_PREFLIGHT_RUN_TIMEOUT_MS,
    "MARKETINGCLAW_DOCKER_ALL_PREFLIGHT_RUN_TIMEOUT_MS",
  );
  const failFast = parseBool(process.env.MARKETINGCLAW_DOCKER_ALL_FAIL_FAST, true);
  const dryRun = parseBool(process.env.MARKETINGCLAW_DOCKER_ALL_DRY_RUN, false);
  const preflightEnabled = parseBool(process.env.MARKETINGCLAW_DOCKER_ALL_PREFLIGHT, true);
  const preflightCleanup = parseBool(process.env.MARKETINGCLAW_DOCKER_ALL_PREFLIGHT_CLEANUP, true);
  const timingsEnabled = parseBool(process.env.MARKETINGCLAW_DOCKER_ALL_TIMINGS, true);
  const buildEnabled = parseBool(process.env.MARKETINGCLAW_DOCKER_ALL_BUILD, true);
  const planJson =
    cliOptions.planJson || parseBool(process.env.MARKETINGCLAW_DOCKER_ALL_PLAN_JSON, false);
  const planReleaseAll = parseBool(process.env.MARKETINGCLAW_DOCKER_ALL_PLAN_RELEASE_ALL, false);
  const profile = parseProfile(process.env.MARKETINGCLAW_DOCKER_ALL_PROFILE);
  const releaseProfile = normalizeReleaseProfile(
    process.env.MARKETINGCLAW_DOCKER_ALL_RELEASE_PROFILE ||
      process.env.MARKETINGCLAW_RELEASE_PROFILE,
  );
  const releaseChunk =
    process.env.MARKETINGCLAW_DOCKER_ALL_CHUNK || process.env.DOCKER_E2E_CHUNK || "";
  const includeOpenWebUI = parseBool(
    process.env.MARKETINGCLAW_DOCKER_ALL_INCLUDE_OPENWEBUI ?? process.env.INCLUDE_OPENWEBUI,
    true,
  );
  const selectedLaneNamesRaw =
    process.env.MARKETINGCLAW_DOCKER_ALL_LANES || process.env.DOCKER_E2E_LANES || "";
  const selectedLaneNames = parseLaneSelection(selectedLaneNamesRaw);
  if (selectedLaneNamesRaw && selectedLaneNames.length === 0) {
    throw new Error("MARKETINGCLAW_DOCKER_ALL_LANES must include at least one lane name");
  }
  const liveMode = parseLiveMode(process.env.MARKETINGCLAW_DOCKER_ALL_LIVE_MODE);
  const liveRetries = parseNonNegativeInt(
    process.env.MARKETINGCLAW_DOCKER_ALL_LIVE_RETRIES,
    DEFAULT_LIVE_RETRIES,
    "MARKETINGCLAW_DOCKER_ALL_LIVE_RETRIES",
  );
  const timingsFile = path.resolve(
    process.env.MARKETINGCLAW_DOCKER_ALL_TIMINGS_FILE || DEFAULT_TIMINGS_FILE,
  );
  const runId = process.env.MARKETINGCLAW_DOCKER_ALL_RUN_ID || utcStampForPath();
  const logDir = path.resolve(
    process.env.MARKETINGCLAW_DOCKER_ALL_LOG_DIR ||
      path.join(ROOT_DIR, ".artifacts/docker-tests", runId),
  );

  const baseEnv = commandEnv({
    MARKETINGCLAW_DOCKER_E2E_BARE_IMAGE:
      process.env.MARKETINGCLAW_DOCKER_E2E_BARE_IMAGE ||
      process.env.MARKETINGCLAW_DOCKER_E2E_IMAGE ||
      DEFAULT_E2E_BARE_IMAGE,
    MARKETINGCLAW_DOCKER_E2E_FUNCTIONAL_IMAGE:
      process.env.MARKETINGCLAW_DOCKER_E2E_FUNCTIONAL_IMAGE ||
      process.env.MARKETINGCLAW_DOCKER_E2E_IMAGE ||
      DEFAULT_E2E_FUNCTIONAL_IMAGE,
  });
  baseEnv.MARKETINGCLAW_DOCKER_E2E_IMAGE =
    process.env.MARKETINGCLAW_DOCKER_E2E_IMAGE || baseEnv.MARKETINGCLAW_DOCKER_E2E_FUNCTIONAL_IMAGE;
  ...
  const { orderedLanes, orderedTailLanes, plan, scheduledLanes } = resolveDockerE2ePlan({
    ...
    selectedLaneNames,
    timingStore,
    upgradeSurvivorBaselines: process.env.MARKETINGCLAW_UPGRADE_SURVIVOR_BASELINE_SPECS,
    upgradeSurvivorScenarios: process.env.MARKETINGCLAW_UPGRADE_SURVIVOR_SCENARIOS,
  });
  ...
  console.log(`==> Docker E2E bare image: ${baseEnv.MARKETINGCLAW_DOCKER_E2E_BARE_IMAGE}`);
  console.log(
    `==> Docker E2E functional image: ${baseEnv.MARKETINGCLAW_DOCKER_E2E_FUNCTIONAL_IMAGE}`,
  );
  if (profile === RELEASE_PATH_PROFILE) {
  ...
  console.log(`==> Live-test bundled plugins: ${baseEnv.MARKETINGCLAW_DOCKER_BUILD_EXTENSIONS}`);
  ...
  if (lanesNeedMarketingClawPackage(scheduledLanes)) {
    await runPhase(phases, "prepare-marketingclaw-package", {}, async () => {
      await prepareMarketingClawPackage(baseEnv, logDir);
    });
  } else {
    console.log("==> MarketingClaw package: not needed for selected lanes");
  }
  ...
  if (buildEnabled) {
    ...
    if (lanesNeedE2eImageKind(scheduledLanes, "bare")) {
      buildEntries.push({
        ...
        env: {
          MARKETINGCLAW_DOCKER_E2E_IMAGE: baseEnv.MARKETINGCLAW_DOCKER_E2E_BARE_IMAGE,
          MARKETINGCLAW_DOCKER_E2E_TARGET: "bare",
        },
        label: `shared bare Docker E2E image once: ${baseEnv.MARKETINGCLAW_DOCKER_E2E_BARE_IMAGE}`,
        phaseDetails: { image: baseEnv.MARKETINGCLAW_DOCKER_E2E_BARE_IMAGE, imageKind: "bare" },
        phases,
      });
    }
    if (lanesNeedE2eImageKind(scheduledLanes, "functional")) {
      buildEntries.push({
        command: "pnpm test:docker:e2e-build",
        env: {
          MARKETINGCLAW_DOCKER_E2E_IMAGE: baseEnv.MARKETINGCLAW_DOCKER_E2E_FUNCTIONAL_IMAGE,
          MARKETINGCLAW_DOCKER_E2E_TARGET: "functional",
        },
        label: `shared functional Docker E2E image once: ${baseEnv.MARKETINGCLAW_DOCKER_E2E_FUNCTIONAL_IMAGE}`,
        phaseDetails: {
          image: baseEnv.MARKETINGCLAW_DOCKER_E2E_FUNCTIONAL_IMAGE,
  ...
  if (failFast && failures.length > 0) {
    await writeRunSummary(logDir, {
      ...
      image: baseEnv.MARKETINGCLAW_DOCKER_E2E_IMAGE,
      images: {
        bare: baseEnv.MARKETINGCLAW_DOCKER_E2E_BARE_IMAGE,
        functional: baseEnv.MARKETINGCLAW_DOCKER_E2E_FUNCTIONAL_IMAGE,
      },
      lanes: allResults,
      phases,
      profile,
