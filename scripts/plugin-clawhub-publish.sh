...
manual_override_reason="${MARKETINGCLAW_CLAWHUB_MANUAL_OVERRIDE_REASON:-}"

pack_dir="$(mktemp -d "${RUNNER_TEMP:-/tmp}/marketingclaw-clawhub-pack.XXXXXX")"
cleanup() {
  rm -rf "${pack_dir}"
}
trap cleanup EXIT
...
build_package_runtime() {
  if [[ "${MARKETINGCLAW_PLUGIN_NPM_RUNTIME_BUILD:-1}" == "0" || "${MARKETINGCLAW_PLUGIN_NPM_RUNTIME_BUILD:-1}" == "false" ]]; then
...
if [[ "${mode}" == "--pack" ]]; then
  output_dir="${MARKETINGCLAW_CLAWHUB_PACK_OUTPUT_DIR:-}"
  if [[ -z "${output_dir}" ]]; then
    echo "MARKETINGCLAW_CLAWHUB_PACK_OUTPUT_DIR is required for --pack" >&2
    exit 2
  fi
  mkdir -p "${output_dir}"
...
for attempt in $(seq 1 "${MARKETINGCLAW_CLAWHUB_PUBLISH_ATTEMPTS:-8}"); do
  ...
  echo "ClawHub publish hit a rate limit; retrying (${attempt}/${MARKETINGCLAW_CLAWHUB_PUBLISH_ATTEMPTS:-8})." >&2
  sleep "${MARKETINGCLAW_CLAWHUB_PUBLISH_RETRY_DELAY_SECONDS:-60}"
done

echo "ClawHub publish failed after ${MARKETINGCLAW_CLAWHUB_PUBLISH_ATTEMPTS:-8} attempts." >&2
exit 1
