...
const pierreDiffsEmptySideEffectNamespace = "marketingclaw-diffs-empty-side-effect";
...
export function createPierreDiffsSideEffectImportPlugin() {
  return {
    name: "marketingclaw-diffs-pierre-side-effect-imports",
    setup(buildContext) {
...
async function buildDiffsViewerRuntime(targetName) {
  ...
  const result = await build({
    ...
    minify: true,
    define: {
      __MARKETINGCLAW_DIFFS_LANGUAGE_PACK__: String(target.languagePackAvailable),
      NaN: "Number.NaN",
    },
    legalComments: "none",
    outfile: outputPath,
    ...
    plugins: [
      ...
      ...(target.shikiAlias
        ? [
            {
              name: "marketingclaw-diffs-curated-shiki",
              setup(buildContext) {
