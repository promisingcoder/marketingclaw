...
ZIP=${1:?"Usage: $0 MarketingClaw-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/promisingcoder/marketingclaw/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  ...
  if [[ "$ZIP_NAME" =~ ^MarketingClaw-([0-9]+(\.[0-9]+){1,2}([-.][0-9A-Za-z]+([.-][0-9A-Za-z]+)*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/promisingcoder/marketingclaw/releases/download/v${VERSION}/"}
