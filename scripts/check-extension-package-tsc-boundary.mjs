...
export function resolveCompileConcurrency(
  ...
  const raw = env.MARKETINGCLAW_EXTENSION_BOUNDARY_CONCURRENCY?.trim();
  if (raw) {
    return parsePositiveInt(raw, "MARKETINGCLAW_EXTENSION_BOUNDARY_CONCURRENCY");
  }
...
async function runCompileCheck(extensionIds) {
  ...
  const verboseFreshLogs = process.env.MARKETINGCLAW_EXTENSION_BOUNDARY_VERBOSE_FRESH === "1";
