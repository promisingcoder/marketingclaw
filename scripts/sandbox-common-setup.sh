...
BASE_IMAGE="${BASE_IMAGE:-marketingclaw-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-marketingclaw-sandbox-common:bookworm-slim}"
...
MARKETINGCLAW_DOCKER_BUILD_USE_BUILDX="${MARKETINGCLAW_DOCKER_BUILD_USE_BUILDX:-0}"
MARKETINGCLAW_DOCKER_BUILD_CACHE_FROM="${MARKETINGCLAW_DOCKER_BUILD_CACHE_FROM:-}"
MARKETINGCLAW_DOCKER_BUILD_CACHE_TO="${MARKETINGCLAW_DOCKER_BUILD_CACHE_TO:-}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=marketingclaw.sandbox=1)
NOTE
