#!/usr/bin/env node
// Plugin Boundary Report script supports MarketingClaw repository automation.
...
const PLUGIN_SDK_SPECIFIER_PATTERN =
  /\b(?:from\s*["']|import\s*\(\s*["']|require\s*\(\s*["']|vi\.(?:mock|doMock)\s*\(\s*["'])(marketingclaw\/plugin-sdk\/([a-z0-9][a-z0-9-]*))["']/g;
...
function collectSummaryWorkspaceTextFileSources(): WorkspaceTextFile[] {
  const pluginSdkFiles = collectWorkspaceTextFilesMatchingGit(
    String.raw`marketingclaw/plugin-sdk/[a-z0-9][a-z0-9-]*`,
  );
  if (!pluginSdkFiles) {
...
function extractCompatTokens(record: PluginCompatRecord): string[] {
  ...
  for (const value of values) {
    ...
    for (const match of value.matchAll(/\bmarketingclaw\/[a-z0-9/-]+\b/g)) {
      tokens.add(match[0]);
    }
    for (const match of value.matchAll(/\bMARKETINGCLAW_[A-Z0-9_]+\b/g)) {
      tokens.add(match[0]);
    }
