...
push_prep_head_once() {
  ...
  if [ -n "${PR_HEAD_OWNER:-}" ] && [ -n "${PR_HEAD_REPO_NAME:-}" ] && [ "${MARKETINGCLAW_PR_PUSH_MODE:-graphql}" != "git" ]; then
  ...
  if [ "${MARKETINGCLAW_ALLOW_UNSIGNED_GIT_PUSH:-}" != "1" ]; then
    echo "Refusing git-protocol PR branch push because it can publish unsigned commits." >&2
    echo "Use the default GitHub createCommitOnBranch path, or set MARKETINGCLAW_ALLOW_UNSIGNED_GIT_PUSH=1 for an explicit manual override." >&2
    return 2
  fi
