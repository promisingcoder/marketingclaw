...
merge_verify() {
  ...
  if ! git merge-base --is-ancestor origin/main "pr-$pr"; then
    ...
    then
      ...
      # traffic. Set MARKETINGCLAW_PR_STRICT_DRIFT=1 to restore the hard gate.
      if [ "${MARKETINGCLAW_PR_STRICT_DRIFT:-}" = "1" ]; then
        echo "Merge verify failed: mainline drift is relevant to this PR; run scripts/pr prepare-sync-head $pr before merge."
        exit 1
      fi
      echo "Merge verify: WARNING — mainline drift is relevant to this PR; proceeding (MARKETINGCLAW_PR_STRICT_DRIFT=1 restores the hard gate)."
    else
