...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.marketingclaw) ? packageJson.marketingclaw : null;
...
function getCatalogChannelId(entry) {
  return trimString(entry?.marketingclaw?.channel?.id) || trimString(entry?.name);
}
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.marketingclaw?.channel?.id) || left.name;
    const rightId = trimString(right.marketingclaw?.channel?.id) || right.name;
