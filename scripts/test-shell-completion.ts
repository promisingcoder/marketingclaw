 ...
 * `marketingclaw update`. Use it to verify the completion installation flow
...
\`marketingclaw update\`, \`marketingclaw doctor\`, and \`marketingclaw onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# MarketingClaw Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(
      theme.muted(`Skipped. Run \`marketingclaw completion --install\` later to enable.`),
    );
    return;
  }
