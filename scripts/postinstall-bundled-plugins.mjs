...
const DISABLE_POSTINSTALL_ENV = "MARKETINGCLAW_DISABLE_BUNDLED_PLUGIN_POSTINSTALL";
const DISABLE_PLUGIN_REGISTRY_MIGRATION_ENV = "MARKETINGCLAW_DISABLE_PLUGIN_REGISTRY_MIGRATION";
...
function resolvePostinstallMarketingClawHomeDir(env, getHomedir = homedir) {
  const osHome = resolvePostinstallOsHomeDir(env, getHomedir);
  const override = env?.MARKETINGCLAW_HOME?.trim();
  return override ? pathResolve(resolvePostinstallTildePath(override, osHome)) : osHome;
}

function resolvePostinstallUserPath(input, marketingClawHome) {
  return pathResolve(resolvePostinstallTildePath(input, marketingClawHome));
}
...
function isLegacyInstalledPluginDependencyDirName(name) {
  return name === "node_modules" || /^\.marketingclaw-install-stage(?:-[^/]+)?$/iu.test(name);
}
...
export function collectLegacyPluginRuntimeDepsStateRoots(params = {}) {
  ...
  const marketingClawHome = resolvePostinstallMarketingClawHomeDir(env, getHomedir);
  const stateRoots = [];
  ...
  if (stateOverride) {
    addStateRoot(resolvePostinstallUserPath(stateOverride, marketingClawHome));
  }
  const configPath = env?.MARKETINGCLAW_CONFIG_PATH?.trim();
  if (configPath) {
    addStateRoot(dirname(resolvePostinstallUserPath(configPath, marketingClawHome)));
  }
  addStateRoot(join(marketingClawHome, ".marketingclaw"));
  addStateRoot(join(marketingClawHome, ".clawdbot"));

  for (const entry of splitPostinstallPathList(env?.STATE_DIRECTORY)) {
    addStateRoot(resolvePostinstallUserPath(entry, marketingClawHome));
  }
  ...
  const createTempPath =
    ...
    ((unsafeTargetPath) =>
      join(
        ...
        `.${basename(unsafeTargetPath)}.marketingclaw-hotfix-${randomUUID()}`,
      ));
  const writeFile =
...
export function pruneMarketingClawCompileCache(params = {}) {
  const env = params.env ?? process.env;
  const pathExists = params.existsSync ?? existsSync;
  const readDir = params.readdirSync ?? readdirSync;
    ...
    try {
      for (const entry of readDir(baseDir, { withFileTypes: true })) {
        ...
        try {
          ...
          log.warn?.(`[postinstall] could not prune MarketingClaw compile cache: ${String(error)}`);
        }
      }
    } catch (error) {
      if (isCompileCachePrunePermissionDenied(error)) {
