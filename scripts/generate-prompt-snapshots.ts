// Generate Prompt Snapshots script supports MarketingClaw repository automation.
...
export async function createFormattedPromptSnapshotFiles(): Promise<PromptSnapshotFile[]> {
  ...
  const tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-prompt-snapshots-"));
  try {
