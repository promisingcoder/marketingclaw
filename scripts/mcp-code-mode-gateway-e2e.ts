// Mcp Code Mode Gateway E2E script supports MarketingClaw repository automation.
...
export async function main() {
  const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-mcp-code-mode-"));
  const keep = process.env.MARKETINGCLAW_MCP_CODE_MODE_GATEWAY_E2E_KEEP === "1";
  const previousEnv = {
  ...
  try {
    ...
    const response = await fetchJson(`http://127.0.0.1:${gatewayPort}/v1/responses`, {
      ...
      headers: {
        ...
        "x-marketingclaw-agent": "qa",
      },
      body: JSON.stringify({
        model: "marketingclaw/qa",
        input: [
          {
            type: "message",
            role: "user",
    ...
    await provider?.stop();
    resetConfigRuntimeState();
    restoreEnvValue("MARKETINGCLAW_STATE_DIR", previousEnv.stateDir);
    restoreEnvValue("MARKETINGCLAW_CONFIG_PATH", previousEnv.configPath);
    restoreEnvValue("MARKETINGCLAW_TEST_FAST", previousEnv.testFast);
    if (!keep) {
