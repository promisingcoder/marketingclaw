...
function applyDefaultParallelVitestWorkerBudget(specs, env) {
  if (env.MARKETINGCLAW_VITEST_MAX_WORKERS || env.MARKETINGCLAW_TEST_WORKERS || isCiLikeEnv(env)) {
    return specs;
  }
  ...
  return specs.map((spec) => ({
    ...
    env: {
      ...spec.env,
      MARKETINGCLAW_VITEST_MAX_WORKERS: String(vitestMaxWorkers),
    },
  }));
}
...
async function main() {
  ...
  const isExplicitParallelMultiConfigRun =
    Boolean(baseEnv.MARKETINGCLAW_TEST_PROJECTS_PARALLEL) &&
    runSpecs.length > 1 &&
    !runSpecs.some((spec) => spec.watchMode);
