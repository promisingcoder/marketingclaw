#!/usr/bin/env bash
# Deploy MarketingClaw to Kubernetes.
#
...
#
# Environment:
#   MARKETINGCLAW_NAMESPACE   Kubernetes namespace (default: marketingclaw)
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
MANIFESTS="$SCRIPT_DIR/manifests"
NS="${MARKETINGCLAW_NAMESPACE:-marketingclaw}"
...
Usage: ./scripts/k8s/deploy.sh [OPTION]
  ...
  (no args)        Deploy MarketingClaw (creates secret from env if needed)
...
_apply_secret() {
  ...
  if kubectl get secret marketingclaw-secrets -n "$NS" &>/dev/null; then
    EXISTING_SECRET=true
    EXISTING_TOKEN="$(kubectl get secret marketingclaw-secrets -n "$NS" -o jsonpath='{.data.MARKETINGCLAW_GATEWAY_TOKEN}' | base64 -d)"
    ANTHROPIC_VALUE="$(kubectl get secret marketingclaw-secrets -n "$NS" -o jsonpath='{.data.ANTHROPIC_API_KEY}' 2>/dev/null | base64 -d)"
    OPENAI_VALUE="$(kubectl get secret marketingclaw-secrets -n "$NS" -o jsonpath='{.data.OPENAI_API_KEY}' 2>/dev/null | base64 -d)"
    GEMINI_VALUE="$(kubectl get secret marketingclaw-secrets -n "$NS" -o jsonpath='{.data.GEMINI_API_KEY}' 2>/dev/null | base64 -d)"
    OPENROUTER_VALUE="$(kubectl get secret marketingclaw-secrets -n "$NS" -o jsonpath='{.data.OPENROUTER_API_KEY}' 2>/dev/null | base64 -d)"
  fi
  ...
  printf '%s' "$TOKEN" > "$TMP_DIR/MARKETINGCLAW_GATEWAY_TOKEN"
  ...
  chmod 600 \
    "$TMP_DIR/MARKETINGCLAW_GATEWAY_TOKEN" \
    "$TMP_DIR/ANTHROPIC_API_KEY" \
    "$TMP_DIR/OPENAI_API_KEY" \
    "$TMP_DIR/GEMINI_API_KEY" \
    "$TMP_DIR/OPENROUTER_API_KEY"

  kubectl create secret generic marketingclaw-secrets \
    -n "$NS" \
    --from-file=MARKETINGCLAW_GATEWAY_TOKEN="$TMP_DIR/MARKETINGCLAW_GATEWAY_TOKEN" \
  ...
  kubectl apply --server-side --field-manager=marketingclaw -f "$SECRET_MANIFEST" >/dev/null
  # Clean up any annotation left by older client-side apply runs.
  kubectl annotate secret marketingclaw-secrets -n "$NS" kubectl.kubernetes.io/last-applied-configuration- >/dev/null 2>&1 || true
  rm -rf "$TMP_DIR"
  trap - EXIT
  ...
  if $SHOW_TOKEN; then
    ...
    echo "  kubectl get secret marketingclaw-secrets -n $NS -o jsonpath='{.data.MARKETINGCLAW_GATEWAY_TOKEN}' | base64 -d && echo"
  fi
}
...
kubectl rollout restart deployment/marketingclaw -n "$NS" 2>/dev/null || true
echo ""
echo "Waiting for rollout..."
kubectl rollout status deployment/marketingclaw -n "$NS" --timeout=300s
echo ""
echo "Done. Access the gateway:"
echo "  kubectl port-forward svc/marketingclaw 18789:18789 -n $NS"
echo "  open http://localhost:18789"
echo ""
if $SHOW_TOKEN; then
  echo "Gateway token (paste into Control UI):"
  echo "  $(kubectl get secret marketingclaw-secrets -n "$NS" -o jsonpath='{.data.MARKETINGCLAW_GATEWAY_TOKEN}' | base64 -d)"
echo ""
fi
