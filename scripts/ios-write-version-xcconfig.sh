...
- MARKETINGCLAW_IOS_VERSION = exact canonical iOS version
- MARKETINGCLAW_MARKETING_VERSION = short iOS/App Store version
- MARKETINGCLAW_BUILD_VERSION = explicit build number or local numeric fallback
EOF
}
...
while IFS='=' read -r key value; do
  case "${key}" in
    MARKETINGCLAW_IOS_VERSION)
      IOS_VERSION="${value}"
      ;;
    MARKETINGCLAW_MARKETING_VERSION)
...
MARKETINGCLAW_IOS_VERSION = ${IOS_VERSION}
MARKETINGCLAW_MARKETING_VERSION = ${MARKETING_VERSION}
MARKETINGCLAW_BUILD_VERSION = ${BUILD_NUMBER}
EOF
