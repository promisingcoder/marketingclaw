// Label Open Issues script supports MarketingClaw repository automation.
...
const STATE_FILE_PATH = join(CONFIG_BASE_DIR, "marketingclaw", STATE_FILE_NAME);
...
function resolveOpenAITimeoutMs(
  raw = process.env.MARKETINGCLAW_LABEL_OPEN_ISSUES_OPENAI_TIMEOUT_MS,
) {
  return parseStrictIntegerOption({
    fallback: DEFAULT_OPENAI_TIMEOUT_MS,
    label: "MARKETINGCLAW_LABEL_OPEN_ISSUES_OPENAI_TIMEOUT_MS",
    min: 1,
    raw,
  });
}
          ...
          input: [
            {
              role: "system",
              content:
                "You classify GitHub issues and pull requests for MarketingClaw. Respond with JSON only, no extra text.",
            },
            {
              role: "user",
              content: [
...
async function main() {
  ...
  logHeader("MarketingClaw Issue Label Audit");
