// Npm Update Scripts script supports MarketingClaw repository automation.
...
const windowsStalePostSwapImportRegex = String.raw`node_modules\\marketingclaw\\dist\\[^\\]+-[A-Za-z0-9_-]+\.js`;
const macosGuestPath =
  "/opt/homebrew/bin:/opt/homebrew/opt/node/bin:/usr/local/bin:/usr/local/sbin:/opt/homebrew/sbin:/usr/bin:/bin:/usr/sbin:/sbin";
const macosMarketingClawCommand = '"$MARKETINGCLAW_BIN"';
...
function posixPrintLogTailFunction(): string {
  ...
  log_file="$1"
  max_bytes="\${MARKETINGCLAW_PARALLELS_NPM_UPDATE_LOG_TAIL_BYTES:-262144}"
  case "$max_bytes" in
...
for attempt in 1 2; do
  ...
  rm -f "$HOME/.marketingclaw/agents/main/sessions/$session_id.jsonl"
  output_file="$(mktemp)"
  set +e
  MARKETINGCLAW_ALLOW_ROOT="\${MARKETINGCLAW_ALLOW_ROOT:-}" ${input.auth.apiKeyEnv}=${shellQuote(input.auth.apiKeyValue)} ${command} agent --local --agent main --session-id "$session_id" --message 'Reply with exact ASCII text OK only.' --thinking off --timeout ${resolveParallelsModelTimeoutSeconds(platform)} --json >"$output_file" 2>&1
  rc=$?
  set -e
...
if [ "$agent_ok" != true ]; then
  echo "marketingclaw agent finished without OK response" >&2
  exit 1
fi`;
}
...
function windowsUpdateWithBundledPluginsDisabled(input: NpmUpdateScriptInput): string {
  const registryEntry = input.npmRegistry
    ...
    : "";
  return `$script:MarketingClawUpdateExit = 0
$updateOutput = Invoke-WithScopedEnv @{ MARKETINGCLAW_DISABLE_BUNDLED_PLUGINS = '1'; MARKETINGCLAW_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS = '1'${registryEntry} } {
  Invoke-MarketingClaw update --tag ${psSingleQuote(input.updateTarget)} --yes --json --no-restart 2>&1
  $script:MarketingClawUpdateExit = $LASTEXITCODE
}
$updateExit = $script:MarketingClawUpdateExit
$updateOutput`;
}

function windowsGatewayReadyScript(): string {
  return `function Wait-MarketingClawGateway {
  $deadline = (Get-Date).AddSeconds(180)
  $attempt = 0
  while ((Get-Date) -lt $deadline) {
    Invoke-MarketingClaw gateway status --deep --require-rpc --timeout 15000
    if ($LASTEXITCODE -eq 0) { return }
    $attempt += 1
    if ($attempt -eq 4) {
      Invoke-MarketingClaw gateway start *>&1 | Out-Host
    }
    Start-Sleep -Seconds 5
  }
  throw "gateway did not become ready after update"
}
Invoke-MarketingClaw gateway restart *>&1 | Out-Host
if ($LASTEXITCODE -ne 0) {
  "gateway restart exited with code $LASTEXITCODE; probing readiness before failing" | Out-Host
}
Wait-MarketingClawGateway`;
}
...
${windowsCodexPlatformPackageRepairFunction()}
$sessionPath = Join-Path $env:USERPROFILE '.marketingclaw\\agents\\main\\sessions\\parallels-npm-update-windows.jsonl'
...
for ($attempt = 1; $attempt -le 2; $attempt++) {
  ...
  $sessionsDir = Join-Path $env:USERPROFILE '.marketingclaw\\agents\\main\\sessions'
  $sessionPath = Join-Path $sessionsDir "$sessionId.jsonl"
  Remove-Item $sessionPath -Force -ErrorAction SilentlyContinue
  $output = Invoke-MarketingClaw agent --local --agent main --session-id $sessionId --model ${psSingleQuote(input.auth.modelId)} --message 'Reply with exact ASCII text OK only.' --thinking off --timeout ${resolveParallelsModelTimeoutSeconds("windows")} --json 2>&1
...
if (-not $agentOk) { throw 'marketingclaw agent finished without OK response' }`;
}
...
MARKETINGCLAW_BIN="$(resolve_required_command marketingclaw)"
scrub_future_plugin_entries() {
  python3 - <<'PY'
import json
from pathlib import Path
path = Path.home() / ".marketingclaw" / "marketingclaw.json"
if not path.exists():
    raise SystemExit(0)
try:
...
stop_marketingclaw_gateway_processes() {
  MARKETINGCLAW_DISABLE_BUNDLED_PLUGINS=1 "$MARKETINGCLAW_BIN" gateway stop || true
  pkill -f 'marketingclaw.*gateway' >/dev/null 2>&1 || true
...
start_marketingclaw_gateway() {
  stop_marketingclaw_gateway_processes
  rm -f /tmp/marketingclaw-parallels-macos-gateway.log
  trap '' HUP
  /usr/bin/env MARKETINGCLAW_HOME="$HOME" MARKETINGCLAW_STATE_DIR="$HOME/.marketingclaw" MARKETINGCLAW_CONFIG_PATH="$HOME/.marketingclaw/marketingclaw.json" ${input.auth.apiKeyEnv}=${shellQuote(
    input.auth.apiKeyValue,
  )} "$MARKETINGCLAW_BIN" gateway run --bind loopback --port 18789 --force >/tmp/marketingclaw-parallels-macos-gateway.log 2>&1 </dev/null &
  sleep 1
}
wait_for_gateway() {
  deadline=$((SECONDS + 240))
  while [ "$SECONDS" -lt "$deadline" ]; do
    if "$MARKETINGCLAW_BIN" gateway status --deep --require-rpc --timeout 15000; then
      return
    fi
    sleep 2
  done
  print_log_tail /tmp/marketingclaw-parallels-macos-gateway.log >&2
  echo "gateway did not become ready after update" >&2
  exit 1
}
scrub_future_plugin_entries
stop_marketingclaw_gateway_processes
${posixNpmRegistryEnv(input.npmRegistry)}MARKETINGCLAW_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1 MARKETINGCLAW_DISABLE_BUNDLED_PLUGINS=1 "$MARKETINGCLAW_BIN" update --tag ${shellQuote(input.updateTarget)} --yes --json --no-restart
${posixVersionCheck(macosMarketingClawCommand, input.expectedNeedle)}
start_marketingclaw_gateway
wait_for_gateway
"$MARKETINGCLAW_BIN" models set ${shellQuote(input.auth.modelId)}
${posixModelProviderConfigCommands(macosMarketingClawCommand, input.auth.modelId, "macos")}
"$MARKETINGCLAW_BIN" config set agents.defaults.skipBootstrap true --strict-json
"$MARKETINGCLAW_BIN" config set tools.profile minimal
${posixAgentWorkspaceScript("Parallels npm update smoke test assistant.")}
${posixAssertAgentOkScript(macosMarketingClawCommand, input, "macos", "parallels-npm-update-macos")}`;
}
...
function Remove-FuturePluginEntries {
  $configPath = Join-Path $env:USERPROFILE '.marketingclaw\\marketingclaw.json'
...
'@
  $nodeScriptPath = Join-Path ([System.IO.Path]::GetTempPath()) ('marketingclaw-future-plugin-scrub-' + [guid]::NewGuid().ToString('N') + '.cjs')
  try {
...
function Stop-MarketingClawGatewayProcesses {
  Invoke-MarketingClaw gateway stop *>&1 | Out-Host
  Get-CimInstance Win32_Process -ErrorAction SilentlyContinue |
    Where-Object { $_.CommandLine -match 'marketingclaw.*gateway' } |
...
if ($updateExit -ne 0) {
  ...
  if (-not $stalePostSwapImport) { throw "marketingclaw update failed with exit code $updateExit" }
  Write-Host "marketingclaw update returned a stale post-swap module import; continuing to post-update health checks"
}
...
export MARKETINGCLAW_ALLOW_ROOT=1
${posixPrintLogTailFunction()}
scrub_future_plugin_entries() {
  node - <<'JS'
const fs = require("node:fs");
const path = require("node:path");
const configPath = path.join(process.env.HOME || "/root", ".marketingclaw", "marketingclaw.json");
...
stop_marketingclaw_gateway_processes() {
  MARKETINGCLAW_DISABLE_BUNDLED_PLUGINS=1 MARKETINGCLAW_ALLOW_ROOT=1 marketingclaw gateway stop || true
  pkill -f 'marketingclaw.*gateway' >/dev/null 2>&1 || true
}
start_marketingclaw_gateway() {
  pkill -f "marketingclaw gateway run" >/dev/null 2>&1 || true
  rm -f /tmp/marketingclaw-parallels-linux-gateway.log
  setsid sh -lc ${shellQuote(
    `exec env MARKETINGCLAW_HOME=/root MARKETINGCLAW_STATE_DIR=/root/.marketingclaw MARKETINGCLAW_CONFIG_PATH=/root/.marketingclaw/marketingclaw.json MARKETINGCLAW_DISABLE_BONJOUR=1 MARKETINGCLAW_ALLOW_ROOT=1 ${input.auth.apiKeyEnv}=${shellQuote(
      input.auth.apiKeyValue,
    )} marketingclaw gateway run --bind loopback --port 18789 --force >/tmp/marketingclaw-parallels-linux-gateway.log 2>&1`,
  )} >/dev/null 2>&1 < /dev/null &
}
wait_for_gateway() {
  deadline=$((SECONDS + 240))
  while [ "$SECONDS" -lt "$deadline" ]; do
    if marketingclaw gateway status --deep --require-rpc --timeout 15000; then
      return
    fi
    sleep 2
  done
  print_log_tail /tmp/marketingclaw-parallels-linux-gateway.log >&2
  echo "gateway did not become ready after update" >&2
  exit 1
}
scrub_future_plugin_entries
stop_marketingclaw_gateway_processes
${posixNpmRegistryEnv(input.npmRegistry)}MARKETINGCLAW_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1 MARKETINGCLAW_DISABLE_BUNDLED_PLUGINS=1 marketingclaw update --tag ${shellQuote(input.updateTarget)} --yes --json --no-restart
${posixVersionCheck("marketingclaw", input.expectedNeedle)}
start_marketingclaw_gateway
wait_for_gateway
marketingclaw models set ${shellQuote(input.auth.modelId)}
${posixModelProviderConfigCommands("marketingclaw", input.auth.modelId, "linux")}
marketingclaw config set agents.defaults.skipBootstrap true --strict-json
marketingclaw config set tools.profile minimal
${posixAgentWorkspaceScript("Parallels npm update smoke test assistant.")}
${posixAssertAgentOkScript("marketingclaw", input, "linux", "parallels-npm-update-linux")}`;
}
...
while ($true) {
  $version = Invoke-MarketingClaw --version
  $version
  if ($LASTEXITCODE -eq 0) { break }
  if ((Get-Date) -ge $versionDeadline) { throw "marketingclaw --version failed with exit code $LASTEXITCODE" }
  Start-Sleep -Seconds 2
}`;
  }
...
while ($true) {
  ...
  if ((Get-Date) -ge $versionDeadline) {
    if ($LASTEXITCODE -ne 0) { throw "marketingclaw --version failed with exit code $LASTEXITCODE" }
    throw ${mismatch}
  }
  Start-Sleep -Seconds 2
}`;
