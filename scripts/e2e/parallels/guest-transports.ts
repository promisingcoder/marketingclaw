// Guest Transports script supports MarketingClaw repository automation.
...
function guestScriptName(extension: string): string {
  return `marketingclaw-parallels-${randomUUID()}.${extension}`;
}
...
export async function runWindowsBackgroundPowerShell(
  ...
  const guestRunDir = `marketingclaw-parallels\\${nonce}`;
  const windowsDonePath = `%WINDIR%\\Temp\\${guestRunDir}\\done`;
  const windowsLogPath = `%WINDIR%\\Temp\\${guestRunDir}\\run.log`;
  const backgroundExitPrefix = `__MARKETINGCLAW_BACKGROUND_EXIT__:${nonce}:`;
  const backgroundDoneMarker = `__MARKETINGCLAW_BACKGROUND_DONE__:${nonce}`;
  const deadline = Date.now() + options.timeoutMs;
  const pathsScript = `$runDir = Join-Path (Join-Path $env:WINDIR 'Temp\\marketingclaw-parallels') ${psSingleQuote(nonce)}
...
function Write-MarketingClawUtf8File([string]$Path, [string]$Value) {
  [System.IO.File]::WriteAllText($Path, $Value, [System.Text.UTF8Encoding]::new($false))
}`;
  const payload = `$ErrorActionPreference = 'Stop'
$PSNativeCommandUseErrorActionPreference = $false
${pathsScript}
Write-MarketingClawUtf8File $pidPath ([string]$PID)
$script:MarketingClawBackgroundLogBytes = 0
function Add-MarketingClawBackgroundLog {
  param([Parameter(ValueFromPipeline=$true)]$InputObject)
  process {
    $text = $InputObject | Out-String
    $bytes = [System.Text.Encoding]::UTF8.GetBytes($text)
    $remaining = [int64]${WINDOWS_BACKGROUND_LOG_MAX_BYTES} - $script:MarketingClawBackgroundLogBytes
    if ($remaining -le 0) {
      return
    }
    ...
    try {
      if ($count -gt 0) {
        ...
        $script:MarketingClawBackgroundLogBytes += $count
      }
      if ($needsBoundaryNewline) {
        $stream.WriteByte(10)
        $script:MarketingClawBackgroundLogBytes++
      }
    } finally {
      $stream.Dispose()
    }
...
try {
  & {
${options.script}
  } *>&1 | Add-MarketingClawBackgroundLog
  Write-MarketingClawUtf8File $exitPath '0'
} catch {
  $_ | Add-MarketingClawBackgroundLog
  Write-MarketingClawUtf8File $exitPath '1'
} finally {
  Write-MarketingClawUtf8File $donePath 'done'
}`;
  const writeArgs = [
    "exec",
    options.vmName,
...
function cleanupWindowsBackground(
  ...
  const stopProcessTree = options.stopProcessTree
    ? `function Stop-MarketingClawBackgroundProcessTree([int]$ProcessId) {
  Get-CimInstance Win32_Process -Filter "ParentProcessId=$ProcessId" -ErrorAction SilentlyContinue | ForEach-Object {
    Stop-MarketingClawBackgroundProcessTree ([int]$_.ProcessId)
  }
