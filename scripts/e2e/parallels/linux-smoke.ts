...
// Linux Smoke script supports MarketingClaw repository automation.
...
function parseMarketingClawPackageVersion(value: string): string | null {
  return value.match(/\b(\d{4}\.\d{1,2}\.\d{1,2}(?:-[A-Za-z0-9.]+)?)\b/u)?.[1] ?? null;
}

function compareMarketingClawPackageVersions(left: string, right: string): number {
  const parse = (value: string): [number, number, number] => {
    const match = parseMarketingClawPackageVersion(value)?.match(/^(\d{4})\.(\d+)\.(\d+)/u);
    if (!match) {
      return [0, 0, 0];
    }
...
class LinuxSmoke extends SmokeRunController<LinuxOptions> {
  ...
  private disableBonjour = parseBoolEnv(process.env.MARKETINGCLAW_PARALLELS_LINUX_DISABLE_BONJOUR);
  ...
  async run(): Promise<void> {
    this.runDir = await makeTempDir("marketingclaw-parallels-linux.");
    this.phases = new PhaseRunner(this.runDir);
    this.tgzDir = await makeTempDir("marketingclaw-parallels-linux-tgz.");
    try {
  ...
  protected async runFreshLane(): Promise<void> {
    ...
    await this.phase("fresh.install-main", 420, () =>
      this.installMainTgz("marketingclaw-main-fresh.tgz"),
    );
  ...
  protected async runUpgradeLane(): Promise<void> {
    ...
    await this.phase("upgrade.install-main", 420, () =>
      this.installMainTgz("marketingclaw-main-upgrade.tgz"),
    );
...
run_apt_with_lock_retry apt-get -o DPkg::Lock::Timeout=30 install -y curl ca-certificates`);
  ...
  private installLatestRelease(): void {
    this.downloadGuestFile(this.options.installUrl, "/tmp/marketingclaw-install.sh");
    if (this.options.installVersion) {
      this.guestExec([
        "/usr/bin/env",
        "MARKETINGCLAW_NO_ONBOARD=1",
        "bash",
        "/tmp/marketingclaw-install.sh",
        "--version",
    ...
    this.guestExec(["marketingclaw", "--version"]);
  }
...
if command -v curl >/dev/null 2>&1; then
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestExec(["marketingclaw", "--version"]);
  ...
  private injectBadPluginFixture(): void {
    this.guestBash(String.raw`set -euo pipefail
plugin_dir=/root/.marketingclaw/test-bad-plugin
mkdir -p "$plugin_dir"
cat >"$plugin_dir/package.json" <<'JSON'
{"name":"@openclaw/test-bad-plugin","version":"1.0.0","marketingclaw":{"extensions":["./index.cjs"],"setupEntry":"./setup-entry.cjs"}}
JSON
cat >"$plugin_dir/marketingclaw.plugin.json" <<'JSON'
...
config_path = Path("/root/.marketingclaw/marketingclaw.json")
...
PY`);
  ...
  private shouldExpectBadPluginDiagnostic(lane: "fresh" | "upgrade"): boolean {
    const version = parseMarketingClawPackageVersion(this.versionForLane(lane));
    if (!version) {
      return true;
    }
    return compareMarketingClawPackageVersions(version, BAD_PLUGIN_DIAGNOSTIC_MIN_VERSION) >= 0;
  }
  ...
  private startGatewayBackground(): void {
    const bonjourEnv = this.disableBonjour ? " MARKETINGCLAW_DISABLE_BONJOUR=1" : "";
    this.guestBash(
      String.raw`pkill -f "marketingclaw gateway run" >/dev/null 2>&1 || true
rm -f /tmp/marketingclaw-parallels-linux-gateway.log
setsid sh -lc ` +
        shellQuote(
          `exec env MARKETINGCLAW_HOME=/root MARKETINGCLAW_STATE_DIR=/root/.marketingclaw MARKETINGCLAW_CONFIG_PATH=/root/.marketingclaw/marketingclaw.json MARKETINGCLAW_ALLOW_ROOT=1${bonjourEnv} ${this.auth.apiKeyEnv}=${shellQuote(
  ...
  private showGatewayStatusCompat(check = true): boolean {
    const help = this.guestExec(["marketingclaw", "gateway", "status", "--help"], { check: false });
...
if isinstance(paths, list):
    load["paths"] = [path for path in paths if path != "/root/.marketingclaw/test-bad-plugin"]
allow = plugins.get("allow")
if isinstance(allow, list):
    plugins["allow"] = [plugin_id for plugin_id in allow if plugin_id != "test-bad-plugin"]
config_path.write_text(json.dumps(config, indent=2) + "\n")
PY
rm -rf /root/.marketingclaw/test-bad-plugin`);
  }
  ...
  private verifyLocalTurn(): void {
    this.guestExec(["marketingclaw", "models", "set", this.auth.modelId]);
...
JSON
marketingclaw config set --batch-file "$provider_config_batch" --strict-json
rm -f "$provider_config_batch"`);
    ...
    this.guestExec(["marketingclaw", "config", "set", "tools.profile", "minimal"]);
...
for attempt in 1 2; do
  ...
  output_file="$(mktemp)"
  set +e
  /usr/bin/env MARKETINGCLAW_ALLOW_ROOT=1 ${shellQuote(`${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`)} marketingclaw agent --local --agent main --session-id "$session_id" --message ${shellQuote(
