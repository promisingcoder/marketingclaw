#!/usr/bin/env bash
# Verifies `marketingclaw plugins update` is a no-op for an already-current plugin.
...
IMAGE_NAME="$(docker_e2e_resolve_image "marketingclaw-plugin-update-e2e" MARKETINGCLAW_PLUGIN_UPDATE_E2E_IMAGE)"
SKIP_BUILD="${MARKETINGCLAW_PLUGIN_UPDATE_E2E_SKIP_BUILD:-0}"
cleanup() {
  docker_e2e_cleanup_package_tgz "${PACKAGE_TGZ:-}"
}
trap cleanup EXIT

PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz plugin-update "${MARKETINGCLAW_CURRENT_PACKAGE_TGZ:-}")"
...
MARKETINGCLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 plugin-update empty)"
