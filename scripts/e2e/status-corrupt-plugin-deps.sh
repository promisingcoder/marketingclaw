...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/marketingclaw-status-corrupt-plugin-deps.XXXXXX")"
cleanup() {
  rm -rf "$TMP_DIR"
}
trap cleanup EXIT
...
CONFIG_PATH="$TMP_DIR/marketingclaw.json"
PLUGIN_DIR="$TMP_DIR/plugin"
STAGE_DIR="$TMP_DIR/stage"
mkdir -p "$HOME_DIR" "$STATE_DIR" "$PLUGIN_DIR" "$STAGE_DIR/node_modules/ansi-escapes"
printf "corrupt rename residue\n" > "$STAGE_DIR/node_modules/ansi-escapes/.marketingclaw-rename-tmp"

cat > "$PLUGIN_DIR/package.json" <<'JSON'
{
  "name": "@example/marketingclaw-e2e-corrupt-chat",
  "version": "1.0.0",
  "marketingclaw": {
...
const stageDir = process.env.MARKETINGCLAW_PLUGIN_STAGE_DIR || "";
const renameResidue = path.join(stageDir, "node_modules", "ansi-escapes", ".marketingclaw-rename-tmp");
...
run_marketingclaw() {
  HOME="$HOME_DIR" \
  MARKETINGCLAW_HOME="$STATE_DIR" \
  MARKETINGCLAW_STATE_DIR="$STATE_DIR" \
  MARKETINGCLAW_CONFIG_PATH="$CONFIG_PATH" \
  MARKETINGCLAW_PLUGIN_STAGE_DIR="$STAGE_DIR" \
...
run_marketingclaw status --all --timeout 1 > "$BEFORE"
grep -F "e2e-corrupt-chat" "$BEFORE" >/dev/null
grep -F "plugin load failed: dependency tree corrupted; run marketingclaw doctor --fix" "$BEFORE" >/dev/null

run_marketingclaw doctor --fix --non-interactive --yes > "$DOCTOR"
if [[ -e "$STAGE_DIR" ]]; then
  echo "doctor --fix did not remove corrupt plugin stage dir: $STAGE_DIR" >&2
  exit 1
fi

run_marketingclaw status --all --timeout 1 > "$AFTER"
