...
// Telegram User Credential script supports MarketingClaw repository automation.
...
const COMMAND_TIMEOUT_MS = optionalPositiveInteger(
  process.env.MARKETINGCLAW_TELEGRAM_USER_CREDENTIAL_COMMAND_TIMEOUT_MS?.trim(),
  120_000,
  "MARKETINGCLAW_TELEGRAM_USER_CREDENTIAL_COMMAND_TIMEOUT_MS",
);
const BROKER_TIMEOUT_MS = optionalPositiveInteger(
  process.env.MARKETINGCLAW_TELEGRAM_USER_CREDENTIAL_BROKER_TIMEOUT_MS?.trim(),
  30_000,
  "MARKETINGCLAW_TELEGRAM_USER_CREDENTIAL_BROKER_TIMEOUT_MS",
);
const CHUNKED_PAYLOAD_MAX_BYTES = optionalPositiveInteger(
  process.env.MARKETINGCLAW_QA_CREDENTIAL_PAYLOAD_MAX_BYTES?.trim(),
  DEFAULT_CHUNKED_PAYLOAD_MAX_BYTES,
  "MARKETINGCLAW_QA_CREDENTIAL_PAYLOAD_MAX_BYTES",
);
const CHUNKED_PAYLOAD_MAX_CHUNKS = optionalPositiveInteger(
  process.env.MARKETINGCLAW_QA_CREDENTIAL_PAYLOAD_MAX_CHUNKS?.trim(),
  DEFAULT_CHUNKED_PAYLOAD_MAX_CHUNKS,
  "MARKETINGCLAW_QA_CREDENTIAL_PAYLOAD_MAX_CHUNKS",
);
...
async function resolveConvexLeaseConfig(opts: Map<string, string>) {
  ...
  const siteUrl =
    opts.get("site-url") ||
    process.env.MARKETINGCLAW_QA_CONVEX_SITE_URL?.trim() ||
    fileEnv.MARKETINGCLAW_QA_CONVEX_SITE_URL;
  const token =
    opts.get("ci-secret") ||
    process.env.MARKETINGCLAW_QA_CONVEX_SECRET_CI?.trim() ||
    fileEnv.MARKETINGCLAW_QA_CONVEX_SECRET_CI;
  if (!siteUrl) {
    throw new Error("Missing MARKETINGCLAW_QA_CONVEX_SITE_URL.");
  }
  if (!token) {
    throw new Error("Missing MARKETINGCLAW_QA_CONVEX_SECRET_CI.");
  }
  return {
    siteUrl,
    token,
    leaseTtlMs: optionalPositiveInteger(
      opts.get("lease-ttl-ms") ||
        process.env.MARKETINGCLAW_QA_CREDENTIAL_LEASE_TTL_MS?.trim() ||
        fileEnv.MARKETINGCLAW_QA_CREDENTIAL_LEASE_TTL_MS,
      20 * 60 * 1_000,
      "MARKETINGCLAW_QA_CREDENTIAL_LEASE_TTL_MS",
    ),
    heartbeatIntervalMs: optionalPositiveInteger(
      opts.get("heartbeat-interval-ms") ||
        process.env.MARKETINGCLAW_QA_CREDENTIAL_HEARTBEAT_INTERVAL_MS?.trim() ||
        fileEnv.MARKETINGCLAW_QA_CREDENTIAL_HEARTBEAT_INTERVAL_MS,
      30_000,
      "MARKETINGCLAW_QA_CREDENTIAL_HEARTBEAT_INTERVAL_MS",
    ),
    ownerId:
      opts.get("owner-id") ||
      process.env.MARKETINGCLAW_QA_CREDENTIAL_OWNER_ID?.trim() ||
...
async function createTelegramUserPayload(opts: Map<string, string>) {
  ...
  const sutToken =
    process.env.MARKETINGCLAW_QA_TELEGRAM_SUT_BOT_TOKEN?.trim() ||
  ...
  const groupId =
    process.env.MARKETINGCLAW_QA_TELEGRAM_GROUP_ID?.trim() ||
  ...
  const tempRoot = await mkdtemp(path.join(tmpdir(), "marketingclaw-telegram-user-credential-"));
...
async function restoreTelegramUserPayload(params: {
  ...
  const tempRoot = await mkdtemp(path.join(tmpdir(), "marketingclaw-telegram-user-restore-"));
  const tdlibArchive = path.join(tempRoot, "tdlib.tgz");
  const desktopArchive = path.join(tempRoot, "desktop-tdata.tgz");
