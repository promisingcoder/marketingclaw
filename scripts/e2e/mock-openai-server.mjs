...
const port =
  process.env.MOCK_PORT != null
    ...
    : readTcpPortEnv("MARKETINGCLAW_MOCK_OPENAI_PORT");
const successMarker = process.env.SUCCESS_MARKER ?? "MARKETINGCLAW_E2E_OK";
    ...
    data: [
      {
        ...
        mime_type: "image/png",
        revised_prompt: "marketingclaw mock image",
      },
    ],
  });
}
...
function resolveResponseText(bodyText) {
  const matches = Array.from(bodyText.matchAll(/\bMARKETINGCLAW_E2E_OK(?:_\d+)?\b/gu));
...
const server = http.createServer((req, res) => {
  void (async () => {
    ...
    if (req.method === "GET" && url.pathname === "/v1/models") {
      writeJson(res, 200, {
        object: "list",
        data: [{ id: "gpt-5.5", object: "model", owned_by: "marketingclaw-e2e" }],
      });
      return;
    }
