...
IMAGE_NAME="$(docker_e2e_resolve_image "marketingclaw-update-channel-switch-e2e" MARKETINGCLAW_UPDATE_CHANNEL_SWITCH_E2E_IMAGE)"
SKIP_BUILD="${MARKETINGCLAW_UPDATE_CHANNEL_SWITCH_E2E_SKIP_BUILD:-0}"
cleanup() {
  docker_e2e_cleanup_package_tgz "${PACKAGE_TGZ:-}"
}
trap cleanup EXIT

PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz update-channel-switch "${MARKETINGCLAW_CURRENT_PACKAGE_TGZ:-}")"
...
(
  cd "$git_root"
  if ! marketingclaw_e2e_maybe_timeout "${MARKETINGCLAW_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install --omit=optional --no-fund --no-audit >/tmp/marketingclaw-git-install.log 2>&1; then
    marketingclaw_e2e_print_log /tmp/marketingclaw-git-install.log >&2
    exit 1
  fi
)
node scripts/e2e/lib/update-channel-switch/assertions.mjs write-control-ui "$git_root"

git config --global user.email "docker-e2e@marketingclaw.local"
git config --global user.name "MarketingClaw Docker E2E"
...
package_install_log="/tmp/marketingclaw-update-channel-switch-package-install.log"
if ! marketingclaw_e2e_maybe_timeout "${MARKETINGCLAW_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g --prefix /tmp/npm-prefix --omit=optional "$pkg_tgz_path" >"$package_install_log" 2>&1; then
  marketingclaw_e2e_print_log "$package_install_log" >&2
  exit 1
fi
package_version="$(node -p "JSON.parse(require(\"node:fs\").readFileSync(\"/tmp/npm-prefix/lib/node_modules/marketingclaw/package.json\", \"utf8\")).version")"
...
export MARKETINGCLAW_GIT_DIR="$git_root"
export MARKETINGCLAW_UPDATE_DEV_TARGET_REF="$fixture_sha"

echo "==> package -> git dev channel"
set +e
dev_json="$(marketingclaw update --channel dev --yes --json --no-restart)"
dev_status=$?
set -e
printf "%s\n" "$dev_json"
...
status_json="$(marketingclaw update status --json)"
printf "%s\n" "$status_json"
...
set +e
stable_json="$(marketingclaw update --channel stable --tag "$pkg_tgz_path" --yes --json --no-restart)"
stable_status=$?
set -e
printf "%s\n" "$stable_json"
