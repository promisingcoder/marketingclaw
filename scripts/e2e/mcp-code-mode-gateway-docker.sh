...
IMAGE_NAME="$(docker_e2e_resolve_image "marketingclaw-mcp-code-mode-gateway-e2e" MARKETINGCLAW_IMAGE)"
PORT="$(docker_e2e_read_tcp_port_env MARKETINGCLAW_MCP_CODE_MODE_GATEWAY_PORT 18789)"
MOCK_PORT="$(docker_e2e_read_tcp_port_env MARKETINGCLAW_MCP_CODE_MODE_MOCK_PORT 44082)"
...
CONTAINER_NAME="marketingclaw-mcp-code-mode-e2e-$$"

CLIENT_LOG="$(mktemp -t marketingclaw-mcp-code-mode-client-log.XXXXXX)"
...
MARKETINGCLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 mcp-code-mode-gateway empty)"
...
docker_e2e_run_with_harness \
  ...
  bash -lc "set -euo pipefail
    ...
    mock_pid=\"\$(marketingclaw_e2e_start_mock_openai \"$MOCK_PORT\" /tmp/mcp-code-mode-mock-openai.log)\"
    gateway_pid=
    cleanup_inner() {
    ...
    gateway_pid=\"\$(marketingclaw_e2e_start_gateway \"\$entry\" $PORT /tmp/mcp-code-mode-gateway.log)\"
    marketingclaw_e2e_wait_gateway_ready \"\$gateway_pid\" /tmp/mcp-code-mode-gateway.log 480 $PORT
