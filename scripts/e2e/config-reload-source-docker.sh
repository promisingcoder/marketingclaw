...
IMAGE_NAME="$(docker_e2e_resolve_image "marketingclaw-config-reload-e2e" MARKETINGCLAW_CONFIG_RELOAD_E2E_IMAGE)"
SKIP_BUILD="${MARKETINGCLAW_CONFIG_RELOAD_E2E_SKIP_BUILD:-0}"
PORT="18789"
TOKEN="reload-e2e-token"
CONTAINER_NAME="marketingclaw-config-reload-e2e-$$"
...
MARKETINGCLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 config-reload empty)"
...
marketingclaw_e2e_exec_gateway \"\$entry\" $PORT loopback /tmp/config-reload-e2e.log" >/dev/null

echo "Waiting for gateway..."
if ! docker_e2e_wait_container_bash "$CONTAINER_NAME" 180 0.5 "source scripts/lib/marketingclaw-e2e-instance.sh; marketingclaw_e2e_probe_tcp 127.0.0.1 $PORT"; then
...
docker_e2e_docker_cmd exec "$CONTAINER_NAME" bash -lc "source /tmp/marketingclaw-test-state-env
