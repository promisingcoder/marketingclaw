...
IMAGE_NAME="${MARKETINGCLAW_QR_SMOKE_IMAGE:-marketingclaw-qr-smoke}"
DOCKER_BUILD_ARGS=()

if [[ "${MARKETINGCLAW_QR_SMOKE_FORCE_INSTALL:-0}" == "1" ]]; then
  INSTALL_CACHE_BUSTER="${GITHUB_SHA:-manual}-${GITHUB_RUN_ID:-$(date +%s)}-${GITHUB_RUN_ATTEMPT:-0}"
  DOCKER_BUILD_ARGS+=(
    --build-arg
    "MARKETINGCLAW_QR_INSTALL_CACHE_BUSTER=${INSTALL_CACHE_BUSTER}"
  )
fi
