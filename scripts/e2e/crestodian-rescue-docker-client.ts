...
import type { MarketingClawConfig } from "../../dist/config/types.marketingclaw.js";
...
function makeParams(commandBody: string, cfg: MarketingClawConfig, isGroup = false) {
  return {
    cfg,
    command: {
      surface: "whatsapp",
...
async function main() {
  const tempState = await createE2eStateDir("marketingclaw-crestodian-");
  tempState.registerExitCleanup();
  const stateDir = tempState.stateDir;
  const configPath =
    process.env.MARKETINGCLAW_CONFIG_PATH ?? path.join(stateDir, "marketingclaw.json");
  ...
  const refPlan = await invoke(
    "/crestodian config set-ref gateway.auth.token env MARKETINGCLAW_GATEWAY_TOKEN",
    cfg,
  );
  assert(
  ...
  const agentPlan = await invoke(
    "/crestodian create agent work workspace /tmp/marketingclaw-work",
    cfg,
  );
  assert(
  ...
  const setupPlan = await invoke(
    "/crestodian setup workspace /tmp/marketingclaw-setup model openai/gpt-5.2",
    cfg,
  );
  ...
  const updatedConfig = JSON.parse(await fs.readFile(configPath, "utf8")) as MarketingClawConfig;
  assert(
  ...
  assert(
    updatedConfig.gateway?.auth?.token &&
      ...
      updatedConfig.gateway.auth.token.id === "MARKETINGCLAW_GATEWAY_TOKEN",
    "SecretRef set did not update gateway.auth.token",
  );
  assert(
    updatedConfig.agents?.defaults?.workspace === "/tmp/marketingclaw-setup",
    "setup did not update default workspace",
  );
  assert(
    updatedConfig.agents?.list?.some(
      (agent) => agent.id === "work" && agent.workspace === "/tmp/marketingclaw-work",
    ),
