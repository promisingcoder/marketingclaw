// Mcp Code Mode Gateway Client script supports MarketingClaw repository automation.
...
export function readMcpCodeModeClientFetchLimits(
  ...
  return {
    ...
    timeoutMs: readPositiveIntEnv("MARKETINGCLAW_MCP_CODE_MODE_CLIENT_TIMEOUT_MS", 300_000, env),
  };
}
...
async function main() {
  ...
  const model = process.env.MARKETINGCLAW_MCP_CODE_MODE_MODEL?.trim() || "marketingclaw/main";
  assert(gatewayUrl, "missing GW_URL");
  assert(gatewayToken, "missing GW_TOKEN");
  assert(stateDir, "missing MARKETINGCLAW_STATE_DIR");
