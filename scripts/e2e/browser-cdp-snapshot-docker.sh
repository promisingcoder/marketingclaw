...
BASE_IMAGE="$(docker_e2e_resolve_image "marketingclaw-browser-cdp-base-e2e" MARKETINGCLAW_BROWSER_CDP_BASE_E2E_IMAGE)"
if [ -n "${MARKETINGCLAW_BROWSER_CDP_SNAPSHOT_E2E_IMAGE:-}" ]; then
  IMAGE_NAME="$MARKETINGCLAW_BROWSER_CDP_SNAPSHOT_E2E_IMAGE"
  DERIVED_SHARED_IMAGE="0"
elif [ -n "${MARKETINGCLAW_DOCKER_E2E_IMAGE:-}" ]; then
  IMAGE_NAME="marketingclaw-browser-cdp-snapshot-e2e:${MARKETINGCLAW_DOCKER_ALL_LANE_NAME:-shared}"
  DERIVED_SHARED_IMAGE="1"
else
  IMAGE_NAME="marketingclaw-browser-cdp-snapshot-e2e"
  DERIVED_SHARED_IMAGE="0"
fi
SKIP_BUILD="${MARKETINGCLAW_BROWSER_CDP_SNAPSHOT_E2E_SKIP_BUILD:-0}"
PORT="18789"
CDP_PORT="19222"
FIXTURE_PORT="18080"
TOKEN="browser-cdp-e2e-token"
CONTAINER_NAME="marketingclaw-browser-cdp-e2e-$$"
DOCKER_COMMAND_TIMEOUT="${MARKETINGCLAW_BROWSER_CDP_SNAPSHOT_DOCKER_COMMAND_TIMEOUT:-900s}"
SNAPSHOT_MAX_BYTES="$(docker_e2e_read_positive_int_env MARKETINGCLAW_BROWSER_CDP_SNAPSHOT_MAX_BYTES 524288)"
...
if [ "$SKIP_BUILD" = "1" ] || { [ "$DERIVED_SHARED_IMAGE" = "0" ] && [ "${MARKETINGCLAW_SKIP_DOCKER_BUILD:-0}" = "1" ]; }; then
  ...
  build_dir="$(mktemp -d "${TMPDIR:-/tmp}/marketingclaw-browser-cdp-build.XXXXXX")"
...
MARKETINGCLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 browser-cdp-snapshot empty)"
...
mkdir -p /tmp/marketingclaw-browser-cdp
find dist -maxdepth 1 -type f -name 'pw-ai-*.js' ! -name 'pw-ai-state-*' -exec mv {} /tmp/marketingclaw-browser-cdp/ \;
...
marketingclaw_e2e_exec_gateway \"\$entry\" $PORT loopback /tmp/browser-cdp-gateway.log" >/dev/null

echo "Waiting for Gateway and fixture server..."
if ! docker_e2e_wait_container_bash "$CONTAINER_NAME" 180 0.5 "
    source scripts/lib/marketingclaw-e2e-instance.sh
    marketingclaw_e2e_probe_http_status http://127.0.0.1:$FIXTURE_PORT/
    marketingclaw_e2e_probe_tcp 127.0.0.1 $PORT
"; then
...
set -euo pipefail
source /tmp/marketingclaw-test-state-env
source scripts/lib/marketingclaw-e2e-instance.sh
entry=\"\$(marketingclaw_e2e_resolve_entrypoint)\"
