...
dump_debug_logs() {
  ...
  for file in \
    ...
    "$CLIENT_REJECT_LOG" \
    "$MOCK_REQUEST_LOG" \
    "$MARKETINGCLAW_STATE_DIR/marketingclaw.json"; do
    if [ -f "$file" ]; then
      echo "--- $file ---" >&2
...
PORT="$PORT" MARKETINGCLAW_GATEWAY_TOKEN="$TOKEN" node scripts/e2e/lib/openai-web-search-minimal/client.mjs success >"$CLIENT_SUCCESS_LOG" 2>&1

node scripts/e2e/lib/openai-web-search-minimal/assertions.mjs assert-success-request "$MOCK_REQUEST_LOG"

PORT="$PORT" MARKETINGCLAW_GATEWAY_TOKEN="$TOKEN" node scripts/e2e/lib/openai-web-search-minimal/client.mjs reject >"$CLIENT_REJECT_LOG" 2>&1
