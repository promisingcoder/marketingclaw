...
export OPENAI_API_KEY="sk-marketingclaw-release-media-memory"
export MARKETINGCLAW_QA_ALLOW_LOCAL_IMAGE_PROVIDER=1

PORT="18789"
MOCK_PORT="44200"
SUCCESS_MARKER="MARKETINGCLAW_E2E_OK_MEDIA_MEMORY"
MEMORY_MARKER="release-media-memory-saffron-$(date +%s)"
media_root="$(mktemp -d /tmp/marketingclaw-release-media-memory.XXXXXX)"
...
entry="$(marketingclaw_e2e_package_entrypoint "$package_root")"
{
  ...
  printf 'entry=%s\n' "$entry"
  printf 'HOME=%s\n' "$HOME"
  printf 'MARKETINGCLAW_HOME=%s\n' "$MARKETINGCLAW_HOME"
  printf 'MARKETINGCLAW_STATE_DIR=%s\n' "$MARKETINGCLAW_STATE_DIR"
  printf 'MARKETINGCLAW_CONFIG_PATH=%s\n' "$MARKETINGCLAW_CONFIG_PATH"
} >"$ENV_LOG"
...
mock_pid="$(marketingclaw_e2e_start_mock_openai "$MOCK_PORT" "$MOCK_OPENAI_LOG")"
...
cp "$MARKETINGCLAW_CONFIG_PATH" "$CONFIG_JSON"
marketingclaw plugins list --json >"$PLUGINS_JSON" 2>"$PLUGINS_STDERR_LOG"
...
mkdir -p "$MARKETINGCLAW_STATE_DIR/workspace/memory"
...
cat >"$MARKETINGCLAW_STATE_DIR/workspace/MEMORY.md" <<EOF
# Long-term memory

- The release media memory marker is $MEMORY_MARKER.
EOF

marketingclaw memory index --force >"$INDEX_LOG" 2>&1
marketingclaw memory search "$MEMORY_MARKER" --json >"$SEARCH_BEFORE_JSON" 2>"$SEARCH_BEFORE_STDERR_LOG"
...
marketingclaw memory search "$MEMORY_MARKER" --json >"$SEARCH_AFTER_JSON" 2>"$SEARCH_AFTER_STDERR_LOG"
