MARKETINGCLAW_PLUGINS_FIXTURE_PID_FILES=()
MARKETINGCLAW_PLUGINS_FIXTURE_EXIT_TRAP_INSTALLED=0
MARKETINGCLAW_PLUGINS_FIXTURE_PREVIOUS_EXIT_ACTION=""

marketingclaw_plugins_read_positive_int_env() {
...
marketingclaw_plugins_read_nonnegative_decimal_env() {
  local name="${1:?missing environment variable name}"
  local fallback="${2:?missing fallback value}"
  local value="${!name-}"
  if [[ -z "${!name+x}" ]]; then
...
marketingclaw_plugins_cleanup_fixture_servers() {
  local pid_file
  local pid
  for pid_file in "${MARKETINGCLAW_PLUGINS_FIXTURE_PID_FILES[@]:-}"; do
    [[ -f "$pid_file" ]] || continue
    pid="$(cat "$pid_file" 2>/dev/null || true)"
    if [[ "$pid" =~ ^[0-9]+$ ]]; then
      marketingclaw_plugins_stop_fixture_process "$pid"
    fi
    rm -f "$pid_file"
  done
}

marketingclaw_plugins_signal_fixture_process() {
  local pid="$1"
  local signal="$2"
...
marketingclaw_plugins_fixture_process_alive() {
  local pid="$1"
  kill -0 "$pid" >/dev/null 2>&1 || kill -0 -- "-$pid" >/dev/null 2>&1
}

marketingclaw_plugins_stop_fixture_process() {
  local pid="$1"
  local _
  local attempts interval
  attempts="$(marketingclaw_plugins_read_positive_int_env MARKETINGCLAW_PLUGINS_FIXTURE_STOP_ATTEMPTS 40)" || return $?
  interval="$(marketingclaw_plugins_read_nonnegative_decimal_env MARKETINGCLAW_PLUGINS_FIXTURE_STOP_INTERVAL_SECONDS 0.25)" || return $?
  if declare -F marketingclaw_e2e_stop_process >/dev/null 2>&1; then
    marketingclaw_e2e_stop_process "$pid"
    return
  fi
  marketingclaw_plugins_signal_fixture_process "$pid" TERM
  for _ in $(seq 1 "$attempts"); do
    ! marketingclaw_plugins_fixture_process_alive "$pid" && { wait "$pid" >/dev/null 2>&1 || true; return; }
    sleep "$interval"
  done
  marketingclaw_plugins_signal_fixture_process "$pid" KILL
  wait "$pid" >/dev/null 2>&1 || true
}

marketingclaw_plugins_print_fixture_log() {
  local log_file="$1"
  ...
  local max_bytes
  max_bytes="$(marketingclaw_plugins_read_positive_int_env MARKETINGCLAW_DOCKER_E2E_LOG_PRINT_BYTES 65536)" || return $?
...
marketingclaw_plugins_validate_fixture_log_print_bytes() {
  marketingclaw_plugins_read_positive_int_env MARKETINGCLAW_DOCKER_E2E_LOG_PRINT_BYTES 65536 >/dev/null
}

marketingclaw_plugins_register_fixture_pid_file() {
  local pid_file="$1"
  MARKETINGCLAW_PLUGINS_FIXTURE_PID_FILES+=("$pid_file")
  marketingclaw_plugins_install_fixture_cleanup_trap
}

marketingclaw_plugins_install_fixture_cleanup_trap() {
  if [[ "${MARKETINGCLAW_PLUGINS_FIXTURE_EXIT_TRAP_INSTALLED:-0}" = "1" ]]; then
    return
  fi

  local existing_trap
  existing_trap="$(trap -p EXIT || true)"
  if [[ -n "$existing_trap" && "$existing_trap" != *marketingclaw_plugins_fixture_exit_trap* ]]; then
    local existing_action="${existing_trap#trap -- }"
    existing_action="${existing_action% EXIT}"
    eval "MARKETINGCLAW_PLUGINS_FIXTURE_PREVIOUS_EXIT_ACTION=$existing_action"
  fi

  MARKETINGCLAW_PLUGINS_FIXTURE_EXIT_TRAP_INSTALLED=1
  trap marketingclaw_plugins_fixture_exit_trap EXIT
}

marketingclaw_plugins_fixture_exit_trap() {
  local status="$?"
  marketingclaw_plugins_cleanup_fixture_servers
  if [[ -n "${MARKETINGCLAW_PLUGINS_FIXTURE_PREVIOUS_EXIT_ACTION:-}" ]]; then
    eval "$MARKETINGCLAW_PLUGINS_FIXTURE_PREVIOUS_EXIT_ACTION"
  fi
  exit "$status"
}
...
packageJson.marketingclaw.extensions = ["./index.js", " "];
