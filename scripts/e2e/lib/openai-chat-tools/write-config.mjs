...
const configPath = requireEnv("MARKETINGCLAW_CONFIG_PATH");
const stateDir = requireEnv("MARKETINGCLAW_STATE_DIR");
const workspaceDir = requireEnv("MARKETINGCLAW_TEST_WORKSPACE_DIR");
const modelRef = requireEnv("MARKETINGCLAW_OPENAI_CHAT_TOOLS_MODEL");
const token = requireEnv("MARKETINGCLAW_GATEWAY_TOKEN");
...
if (providerId !== "openai" || !modelId) {
  throw new Error(`MARKETINGCLAW_OPENAI_CHAT_TOOLS_MODEL must be openai/*, got ${modelRef}`);
}
