...
const AGENTS_DELETE_OUTPUT_MAX_BYTES = readPositiveIntEnv(
  "MARKETINGCLAW_FIXTURE_AGENTS_DELETE_OUTPUT_MAX_BYTES",
  1024 * 1024,
);
...
function writeOpenWebUiWorkspace() {
  const workspace =
    process.env.MARKETINGCLAW_WORKSPACE_DIR ||
    path.join(process.env.HOME, ".marketingclaw", "workspace");
  write(
...
function writeAgentsDeleteConfig() {
  const stateDir = requireArg(process.env.MARKETINGCLAW_STATE_DIR, "MARKETINGCLAW_STATE_DIR");
  const sharedWorkspace = requireArg(process.env.SHARED_WORKSPACE, "SHARED_WORKSPACE");
  const gatewayToken = process.env.MARKETINGCLAW_GATEWAY_TOKEN?.trim();
...
function assertAgentsDeleteResult([outputPath]) {
  ...
  const remaining =
    readJson(path.join(process.env.MARKETINGCLAW_STATE_DIR, "marketingclaw.json"))?.agents?.list ??
    [];
