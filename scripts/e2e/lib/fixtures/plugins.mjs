...
function writePluginDemo([dir]) {
  ...
  writePluginManifest(path.join(dir, "marketingclaw.plugin.json"), "demo-plugin", {
...
function writePlugin([dir, id, version, method, name]) {
  ...
  writePluginManifest(path.join(dir, "marketingclaw.plugin.json"), id);
}
...
function writePluginMarketplace(args) {
  ...
  writeJson(path.join(process.env.HOME, ".claude", "plugins", "known_marketplaces.json"), {
    "claude-fixtures": {
      ...
      source: { type: "github", repo: "marketingclaw/fixture-marketplace" },
    },
  });
}
