...
function resolveStateDir() {
  if (process.env.MARKETINGCLAW_STATE_DIR) {
    return process.env.MARKETINGCLAW_STATE_DIR;
  }
  return path.join(process.env.HOME || os.homedir(), ".marketingclaw");
}
...
function resolveMarketingClawEntry() {
  if (process.env.MARKETINGCLAW_ENTRY) {
    return process.env.MARKETINGCLAW_ENTRY;
  }
  ...
  throw new Error("Missing MARKETINGCLAW_ENTRY and dist/index.(m)js");
}

function readPluginsList() {
  const entry = resolveMarketingClawEntry();
...
async function loadManifestEntries() {
  const explicit = (process.env.MARKETINGCLAW_BUNDLED_PLUGIN_SWEEP_IDS || "")
    .split(/[,\s]+/u)
  ...
  return explicit.map((lookup) => {
    ...
    if (!found) {
      throw new Error(
        `MARKETINGCLAW_BUNDLED_PLUGIN_SWEEP_IDS entry is not an installable bundled plugin in this package: ${lookup}. Available: ${available}`,
      );
    }
    return found;
  });
...
async function selectedManifestEntries() {
  ...
  const total = readPositiveIntEnv("MARKETINGCLAW_BUNDLED_PLUGIN_SWEEP_TOTAL", 1);
  const index = readNonNegativeIntEnv("MARKETINGCLAW_BUNDLED_PLUGIN_SWEEP_INDEX", 0);
  if (index >= total) {
    throw new Error(
      `MARKETINGCLAW_BUNDLED_PLUGIN_SWEEP_INDEX must be in [0, ${total - 1}], got ${process.env.MARKETINGCLAW_BUNDLED_PLUGIN_SWEEP_INDEX}`,
    );
  }
