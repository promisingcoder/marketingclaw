...
export OPENAI_API_KEY="sk-marketingclaw-release-typed-onboarding"

PORT="18789"
MOCK_PORT="44190"
SUCCESS_MARKER="MARKETINGCLAW_E2E_OK_TYPED_ONBOARDING"
...
dump_debug_logs() {
  ...
  marketingclaw_e2e_dump_logs \
    ...
    "$HOME/.marketingclaw/agents/main/agent/auth-profiles.json"
}
...
mkfifo "$input_fifo"
marketingclaw_e2e_run_script_with_pty "node \"$entry\" onboard --flow quickstart --mode local --auth-choice skip --gateway-port \"$PORT\" --gateway-bind loopback --skip-daemon --skip-ui --skip-channels --skip-skills --skip-health" "$ONBOARD_LOG" <"$input_fifo" >/dev/null 2>&1 &
wizard_pid="$!"
exec 3>"$input_fifo"
