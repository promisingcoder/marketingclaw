...
async function buildNpmPackArtifact(fixture) {
  const packRoot = await fs.promises.mkdtemp(
    path.join(os.tmpdir(), "marketingclaw-clawhub-fixture-"),
  );
  try {
...
export default definePluginEntry({
  id: "${pluginId}",
  name: "MarketingClaw Kitchen Sink",
  register(api) {
    if (!isNumber(42)) {
...
`,
    ...
    packageDetail(artifact) {
      ...
      const packageDetail = {
        package: {
          name: packageName,
          displayName: "MarketingClaw Kitchen Sink",
          family: "code-plugin",
          runtimeId: pluginId,
          channel: "official",
          isOfficial: true,
...
async function main() {
  ...
  zip.file("package/marketingclaw.plugin.json", manifestJson, { date: new Date(0) });
  ...
  const artifactResolverDetail = {
    package: versionDetail.package ?? {
      name: packageName,
      displayName: packageDetail.package?.displayName ?? "MarketingClaw Kitchen Sink",
