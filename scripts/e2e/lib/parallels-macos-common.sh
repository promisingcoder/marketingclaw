...
parallels_macos_desktop_user_exec_with_secret_file() {
  ...
  local secret_path
  secret_path="/tmp/marketingclaw-secret-${api_key_env:-env}-$RANDOM-$RANDOM"
  ...
  local wrapper
  local wrapper_path
  wrapper_path="/tmp/marketingclaw-secret-env-wrapper-$RANDOM-$RANDOM.sh"
  wrapper='#!/bin/bash
set -e
cleanup() {
  rm -f "${MARKETINGCLAW_WRAPPER_FILE:-}"
}
trap cleanup EXIT
if [ -n "${MARKETINGCLAW_SECRET_ENV_NAME:-}" ] && [ -n "${MARKETINGCLAW_SECRET_FILE:-}" ] && [ -f "$MARKETINGCLAW_SECRET_FILE" ]; then
  secret_value="$(cat "$MARKETINGCLAW_SECRET_FILE")"
  rm -f "$MARKETINGCLAW_SECRET_FILE"
  export "${MARKETINGCLAW_SECRET_ENV_NAME}=${secret_value}"
fi
"$@"
'
  ...
  if [[ "$user_flag" == "current-user" ]]; then
    prlctl exec "$vm_name" --current-user /usr/bin/env \
      "PATH=$path_value" \
      "MARKETINGCLAW_SECRET_ENV_NAME=$api_key_env" \
      "MARKETINGCLAW_SECRET_FILE=$secret_path" \
      "MARKETINGCLAW_WRAPPER_FILE=$wrapper_path" \
