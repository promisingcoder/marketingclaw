...
DOCKER_BUILD_EXTENSIONS="${MARKETINGCLAW_DOCKER_BUILD_EXTENSIONS:-${MARKETINGCLAW_EXTENSIONS:-}}"
LIVE_IMAGE_PULL_ATTEMPTS="${MARKETINGCLAW_LIVE_DOCKER_PULL_ATTEMPTS:-3}"
LIVE_IMAGE_PULL_RETRY_DELAY_SECONDS="${MARKETINGCLAW_LIVE_DOCKER_PULL_RETRY_DELAY_SECONDS:-5}"

if ! [[ "$LIVE_IMAGE_PULL_ATTEMPTS" =~ ^[1-9][0-9]*$ ]]; then
  echo "MARKETINGCLAW_LIVE_DOCKER_PULL_ATTEMPTS must be a positive integer, got: $LIVE_IMAGE_PULL_ATTEMPTS" >&2
  exit 2
fi
if ! [[ "$LIVE_IMAGE_PULL_RETRY_DELAY_SECONDS" =~ ^[0-9]+$ ]]; then
  echo "MARKETINGCLAW_LIVE_DOCKER_PULL_RETRY_DELAY_SECONDS must be a non-negative integer, got: $LIVE_IMAGE_PULL_RETRY_DELAY_SECONDS" >&2
  exit 2
fi
...
if [[ -n "${DOCKER_BUILD_EXTENSIONS}" ]]; then
  DOCKER_BUILD_ARGS+=(--build-arg "MARKETINGCLAW_EXTENSIONS=${DOCKER_BUILD_EXTENSIONS}")
fi
...
if [[ "${MARKETINGCLAW_SKIP_DOCKER_BUILD:-}" == "1" ]]; then
  ...
  echo "==> Live-test image not available; building because MARKETINGCLAW_DOCKER_BUILD_ON_MISSING/MARKETINGCLAW_TESTBOX allows fallback."
fi
