// Gh Read script supports MarketingClaw repository automation.
...
const APP_ID_ENV = "MARKETINGCLAW_GH_READ_APP_ID";
const KEY_FILE_ENV = "MARKETINGCLAW_GH_READ_PRIVATE_KEY_FILE";
const INSTALLATION_ID_ENV = "MARKETINGCLAW_GH_READ_INSTALLATION_ID";
const PERMISSIONS_ENV = "MARKETINGCLAW_GH_READ_PERMISSIONS";
...
export function resolveGitHubFetchTimeoutMs(
  raw = process.env.MARKETINGCLAW_GH_READ_FETCH_TIMEOUT_MS,
) {
  return parseStrictIntegerOption({
    fallback: DEFAULT_GITHUB_FETCH_TIMEOUT_MS,
    label: "MARKETINGCLAW_GH_READ_FETCH_TIMEOUT_MS",
    min: 1,
    raw,
  });
}
    ...
    async (signal, timeoutPromise) => {
      const response = await fetchImpl(`https://api.github.com${path}`, {
        ...
        headers: {
          ...
          "User-Agent": "marketingclaw-gh-read",
...
async function main() {
  if (process.argv.length <= 2) {
    fail(
      "usage: scripts/gh-read <gh args...>\nset MARKETINGCLAW_GH_READ_APP_ID and MARKETINGCLAW_GH_READ_PRIVATE_KEY_FILE first",
    );
  }
