...
- regenerates apps/ios/MarketingClaw.xcodeproj via xcodegen
EOF
}
...
if [[ "${TEAM_ID}" != "${CANONICAL_TEAM_ID}" ]]; then
  echo "iOS App Store release must use canonical MarketingClaw Team ID ${CANONICAL_TEAM_ID}; got ${TEAM_ID}." >&2
  exit 1
fi

if [[ -n "${MARKETINGCLAW_PUSH_RELAY_BASE_URL:-}" || -n "${IOS_PUSH_RELAY_BASE_URL:-}" ]]; then
...
MARKETINGCLAW_APP_BUNDLE_ID = ai.marketingclaw.app
MARKETINGCLAW_SHARE_BUNDLE_ID = ai.marketingclaw.app.share
MARKETINGCLAW_ACTIVITY_WIDGET_BUNDLE_ID = ai.marketingclaw.app.activitywidget
MARKETINGCLAW_WATCH_APP_BUNDLE_ID = ai.marketingclaw.app.watchkitapp
MARKETINGCLAW_CODE_SIGN_ENTITLEMENTS = Sources/MarketingClawAppAttest.entitlements
MARKETINGCLAW_APNS_ENTITLEMENT_ENVIRONMENT = production
MARKETINGCLAW_APP_ATTEST_ENVIRONMENT = production
MARKETINGCLAW_PUSH_MODE = appStore
MARKETINGCLAW_PUSH_RELAY_BASE_URL =
EOF
