...
IMAGE_NAME="$(docker_e2e_resolve_image "marketingclaw-code-mode-namespace-live-e2e" MARKETINGCLAW_CODE_MODE_NAMESPACE_LIVE_E2E_IMAGE)"
SKIP_BUILD="${MARKETINGCLAW_CODE_MODE_NAMESPACE_LIVE_E2E_SKIP_BUILD:-0}"
PROFILE_FILE="${MARKETINGCLAW_CODE_MODE_NAMESPACE_LIVE_PROFILE_FILE:-${MARKETINGCLAW_TESTBOX_PROFILE_FILE:-$HOME/.marketingclaw-testbox-live.profile}}"
run_log=""
...
if ! docker_e2e_run_with_harness \
  ...
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
  -e "MARKETINGCLAW_CODE_MODE_LIVE_MODEL=${MARKETINGCLAW_CODE_MODE_LIVE_MODEL:-gpt-5.4-mini}" \
  -e "MARKETINGCLAW_CODE_MODE_LIVE_TASKS=${MARKETINGCLAW_CODE_MODE_LIVE_TASKS:-3}" \
  -v "$ROOT_DIR":/src:ro \
  "${PROFILE_MOUNT[@]}" \
  "$IMAGE_NAME" \
