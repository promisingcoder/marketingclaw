...
func (docFrontmatterTranslator) Translate(_ context.Context, text, _, _ string) (string, error) {
	replacer := strings.NewReplacer(
		"Step-by-step Fly.io deployment for MarketingClaw with persistent storage and HTTPS", "在 Fly.io 上逐步部署 MarketingClaw，包含持久化存储和 HTTPS",
		"Deploying MarketingClaw on Fly.io", "在 Fly.io 上部署 MarketingClaw",
...
func (docFrontmatterFallbackTranslator) Translate(_ context.Context, text, _, _ string) (string, error) {
	switch text {
	case "Step-by-step Fly.io deployment for MarketingClaw with persistent storage and HTTPS":
		return strings.Join([]string{
			"<frontmatter>",
			"title: Fly.io",
			"summary: \"在 Fly.io 上部署 MarketingClaw 的逐步指南，包含持久化存储和 HTTPS 设置\"",
			"read_when:",
			"  - 在 Fly.io 上部署 MarketingClaw",
			"  - 设置 Fly 卷、机密和初始运行配置",
			"</frontmatter>",
			"",
			"<body>",
	...
	case "Deploying MarketingClaw on Fly.io":
		return "在 Fly.io 上部署 MarketingClaw", nil
...
func TestTranslateDocBodyChunkedFallsBackToSmallerChunks(t *testing.T) {
	...
	t.Setenv("MARKETINGCLAW_DOCS_I18N_DOC_CHUNK_MAX_BYTES", "4096")
...
func TestTranslateDocBodyChunkedPreSplitsOversizedPromptBudget(t *testing.T) {
	...
	t.Setenv("MARKETINGCLAW_DOCS_I18N_DOC_CHUNK_MAX_BYTES", "4096")
	t.Setenv("MARKETINGCLAW_DOCS_I18N_DOC_CHUNK_PROMPT_BUDGET", "60")
...
func TestTranslateDocBodyChunkedSplitsOversizedSingletonBlock(t *testing.T) {
	...
	t.Setenv("MARKETINGCLAW_DOCS_I18N_DOC_CHUNK_MAX_BYTES", "24")
...
func TestTranslateDocBodyChunkedSplitsSingletonBlockWhenPromptBudgetExceeded(t *testing.T) {
	...
	t.Setenv("MARKETINGCLAW_DOCS_I18N_DOC_CHUNK_MAX_BYTES", "4096")
	t.Setenv("MARKETINGCLAW_DOCS_I18N_DOC_CHUNK_PROMPT_BUDGET", strconv.Itoa(budget))
	translator := &oversizedBlockTranslator{}
	translated, err := translateDocBodyChunked(context.Background(), translator, "gateway/configuration-reference.md", body, "en", "zh-CN")
	if err != nil {
		t.Fatalf("translateDocBodyChunked returned error: %v", err)
...
func TestTranslateDocBodyChunkedRetriesSingletonFenceAfterValidationFailure(t *testing.T) {
	...
	t.Setenv("MARKETINGCLAW_DOCS_I18N_DOC_CHUNK_MAX_BYTES", "4096")
	t.Setenv("MARKETINGCLAW_DOCS_I18N_DOC_CHUNK_PROMPT_BUDGET", "4096")
...
func TestProcessFileDocUsesFieldLevelFrontmatterTranslation(t *testing.T) {
	...
	text := string(output)
	if !strings.Contains(text, "在 Fly.io 上逐步部署 MarketingClaw，包含持久化存储和 HTTPS") {
		t.Fatalf("expected translated summary in output:\n%s", text)
	}
	if !strings.Contains(text, "在 Fly.io 上部署 MarketingClaw") {
...
func TestProcessFileDocRejectsSuspiciousFrontmatterScalarExpansion(t *testing.T) {
	...
	if !strings.Contains(text, "summary: Step-by-step Fly.io deployment for MarketingClaw with persistent storage and HTTPS") {
