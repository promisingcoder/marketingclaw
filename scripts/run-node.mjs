#!/usr/bin/env node
// Development runner that rebuilds MarketingClaw, runs runtime postbuild steps, and
...
const listRequiredBundledPluginMetadataOutputs = (pluginEntries, deps) =>
  pluginEntries.flatMap(({ id, hasManifest, hasPackageJson }) => {
    ...
    if (hasManifest) {
      requiredPaths.push(path.join(builtPluginDir, "marketingclaw.plugin.json"));
    }
...
const listRequiredMarketingClawExtensionAliasOutputs = (deps) => {
  const distRoot = resolveRuntimePostBuildDistRoot(deps);
  ...
  const aliasDir = path.join(distRoot, "extensions", "node_modules", "marketingclaw");
  return [
...
const listRequiredStaticExtensionAssetOutputs = (deps) => {
  if (deps.env.MARKETINGCLAW_RUNTIME_POSTBUILD_STATIC_ASSETS === "0") {
    return [];
  }
  const distRoot = resolveRuntimePostBuildDistRoot(deps);
  const runtimeRoot = resolveRuntimePostBuildRuntimeRoot(deps);
...
export const listRequiredRuntimePostBuildOutputs = (deps) => {
  ...
  return [
    ...
    ...listRequiredMarketingClawExtensionAliasOutputs(deps),
...
export const resolveBuildRequirement = (deps) => {
  if (deps.env.MARKETINGCLAW_FORCE_BUILD === "1") {
    return { shouldBuild: true, reason: "force_build" };
  }
  if (
    deps.env.MARKETINGCLAW_BUILD_PRIVATE_QA === "1" &&
...
export const resolveRuntimePostBuildRequirement = (deps) => {
  if (deps.env.MARKETINGCLAW_FORCE_RUNTIME_POSTBUILD === "1") {
...
const BUILD_REASON_LABELS = {
  force_build: "forced by MARKETINGCLAW_FORCE_BUILD",
...
const RUNTIME_POSTBUILD_REASON_LABELS = {
  force_runtime_postbuild: "forced by MARKETINGCLAW_FORCE_RUNTIME_POSTBUILD",
...
const RUN_NODE_OUTPUT_LOG_ENV = "MARKETINGCLAW_RUN_NODE_OUTPUT_LOG";
const RUN_NODE_CPU_PROF_DIR_ENV = "MARKETINGCLAW_RUN_NODE_CPU_PROF_DIR";
const RUN_NODE_CPU_PROF_MAX_FILES_ENV = "MARKETINGCLAW_RUN_NODE_CPU_PROF_MAX_FILES";
const RUN_NODE_FILTER_SYNC_IO_STDERR_ENV = "MARKETINGCLAW_RUN_NODE_FILTER_SYNC_IO_STDERR";
const RUN_NODE_BUILD_LOCK_TIMEOUT_ENV = "MARKETINGCLAW_RUN_NODE_BUILD_LOCK_TIMEOUT_MS";
const RUN_NODE_BUILD_LOCK_POLL_ENV = "MARKETINGCLAW_RUN_NODE_BUILD_LOCK_POLL_MS";
const RUN_NODE_BUILD_LOCK_STALE_ENV = "MARKETINGCLAW_RUN_NODE_BUILD_LOCK_STALE_MS";
...
const logRunner = (message, deps) => {
  if (deps.env.MARKETINGCLAW_RUNNER_LOG === "0") {
    return;
  }
  const line = `[marketingclaw] ${message}\n`;
...
const shouldUseRunNodeProgress = (deps) =>
  ...
  deps.env.MARKETINGCLAW_RUNNER_PROGRESS !== "0" &&
  deps.env.CI !== "true" &&
  !deps.outputTee;
...
const createRunNodeProgress = (label, deps) => {
  ...
  const render = () => {
    ...
    frameIndex += 1;
    deps.stderr.write(`\r[marketingclaw] ${frame} ${label} (${elapsedSeconds}s)`);
    visible = true;
  };
...
const listRunNodeCpuProfiles = (deps, absoluteProfileDir, commandName) => {
  ...
  const prefix = `marketingclaw-${commandName}-`;
  return entries
    .filter(
      (entry) =>
...
const resolveRunNodeCpuProfileArgs = (deps) => {
  ...
  const profileName = `marketingclaw-${commandName}-${pid}-${timestamp}.cpuprofile`;
...
const resolveRunNodeDiagnosticArgs = (deps) => {
  ...
  if (deps.env.MARKETINGCLAW_TRACE_SYNC_IO === "1") {
...
const closeRunNodeOutputTee = async (deps, exitCode) => {
  ...
  try {
    ...
    deps.stderr.write(
      `[marketingclaw] Failed to write output log: ${error?.message ?? "unknown error"}\n`,
    );
...
const shouldSkipWatchRuntimeSync = (deps, requirement) =>
  deps.env.MARKETINGCLAW_WATCH_MODE === "1" &&
...
const shouldFastPathExistingDistForGatewayClient = (deps) =>
  ...
  deps.env.MARKETINGCLAW_FORCE_BUILD !== "1" &&
...
const canUseStampedGatewayClientDist = (deps) => {
  ...
  if (
    ...
    deps.env.MARKETINGCLAW_FORCE_RUNTIME_POSTBUILD === "1"
  ) {
    return false;
  }
...
export async function runNodeMain(params = {}) {
  ...
  if (deps.args[0] === "qa") {
    deps.env.MARKETINGCLAW_BUILD_PRIVATE_QA = "1";
    deps.env.MARKETINGCLAW_ENABLE_PRIVATE_QA_CLI = "1";
    deps.env.MARKETINGCLAW_DISABLE_BUNDLED_PLUGINS ??= "0";
  }
  ...
  try {
    ...
    if (shouldFastPathExistingDistForGatewayClient(deps)) {
      exitCode = await runMarketingClaw(deps);
