...
function packageOptsOutOfBundledRuntimeDependencies(packageJson) {
  return packageJson?.marketingclaw?.release?.bundleRuntimeDependencies === false;
}
...
export function resolveAugmentedPluginNpmPackageJson(params) {
  ...
  const packageJson = {
    ...
    marketingclaw: {
      ...plan.packageJson.marketingclaw,
...
export function resolveAugmentedPluginNpmManifest(params) {
  const repoRoot = path.resolve(params.repoRoot ?? ".");
  const packageDir = resolvePackageDir(repoRoot, params.packageDir);
  const manifestPath = path.join(packageDir, "marketingclaw.plugin.json");
...
function main(argv = process.argv.slice(2)) {
  ...
  return withAugmentedPluginNpmManifestForPackage(
    {
      packageDir,
      bundleDependencies: process.env.MARKETINGCLAW_PLUGIN_NPM_BUNDLE_DEPENDENCIES,
    },
    ({ packageDir: cwd }) => {
