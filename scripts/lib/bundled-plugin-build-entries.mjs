...
const BUNDLED_PLUGIN_BUILD_IDS_ENV = "MARKETINGCLAW_BUNDLED_PLUGIN_BUILD_IDS";
...
function isManifestlessBundledRuntimeSupportPackage(params) {
  if (params.packageJson?.marketingclaw?.release?.publishToNpm === true) {
    return false;
  }
  const packageName = typeof params.packageJson?.name === "string" ? params.packageJson.name : "";
  if (packageName !== `@marketingclaw/${params.dirName}`) {
    return false;
  }
...
function shouldBuildBundledDistEntry(packageJson) {
  return packageJson?.marketingclaw?.build?.bundledDist !== false;
}
...
export function collectPluginSourceEntries(packageJson) {
  let packageEntries = Array.isArray(packageJson?.marketingclaw?.extensions)
    ? packageJson.marketingclaw.extensions.filter(
        (entry) => typeof entry === "string" && entry.trim().length > 0,
      )
    : [];
  const setupEntry =
    typeof packageJson?.marketingclaw?.setupEntry === "string" &&
    packageJson.marketingclaw.setupEntry.trim().length > 0
...
export function collectBundledPluginBuildEntries(params = {}) {
  ...
  for (const candidate of collectBundledPluginCandidates(cwd, extensionsRoot)) {
    ...
    const hasManifest =
      relativeFiles?.includes("marketingclaw.plugin.json") ?? fs.existsSync(manifestPath);
...
export function listBundledPluginPackArtifacts(params = {}) {
  ...
  for (const { id, hasManifest, hasPackageJson, sourceEntries } of entries) {
    if (hasManifest) {
      artifacts.add(bundledDistPluginFile(id, "marketingclaw.plugin.json"));
    }
    if (hasPackageJson) {
