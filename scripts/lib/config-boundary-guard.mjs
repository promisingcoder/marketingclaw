...
function pushBroadConfigRuntimeBarrelViolations(violations, files) {
  const staticImportPattern =
    /\b(?:import|export)\s+(?:type\s+)?\{[\s\S]*?\}\s+from\s+["']marketingclaw\/plugin-sdk\/config-runtime["']/g;
  const dynamicImportPattern =
    /\b(?:const|let|var)\s+\{[\s\S]*?\}\s*=\s*(?:await\s+)?import\(["']marketingclaw\/plugin-sdk\/config-runtime["']\)/g;
  const typeQueryPattern =
    /\b(?:typeof\s+)?import\(["']marketingclaw\/plugin-sdk\/config-runtime["']\)\.[A-Za-z_$][\w$]*/g;
  ...
  for (const { filePath, relPath } of files) {
    ...
    for (const pattern of [staticImportPattern, dynamicImportPattern, typeQueryPattern]) {
      for (const line of findMatchLineNumbers(source, pattern)) {
        violations.push(
          `${relPath}:${line} use narrow plugin-sdk config subpaths instead of marketingclaw/plugin-sdk/config-runtime`,
        );
      }
    }
  }
...
function pushBroadConfigRuntimeSpecifierViolations(violations, files) {
  const moduleSpecifierPattern = /["']marketingclaw\/plugin-sdk\/config-runtime["']/g;
...
export function collectDeprecatedInternalConfigApiViolations({
  ...
  for (const { filePath, relPath } of productionExtensionFiles) {
    ...
    const guards = [
      {
        pattern:
          /\b(?:import|export)\s+(?:type\s+)?\{[^}]*\bloadConfig\b[^}]*\}\s+from\s+["']marketingclaw\/plugin-sdk\/(?:config-runtime|memory-core-host-runtime-core)["']/,
        replacement:
  ...
  for (const { filePath, relPath } of repoFiles.filter(
    ...
    const guards = [
      {
        pattern:
          /\b(?:import|export)\s+(?:type\s+)?\{[\s\S]*?\b(?:loadConfig|writeConfigFile)\b[\s\S]*?\}\s+from\s+["']marketingclaw\/plugin-sdk\/(?:config-runtime|memory-core-host-runtime-core)["']/,
        replacement:
          "use getRuntimeConfig(), runtime.config.current(), or mutation helpers with afterWrite",
      },
      {
        pattern:
          /ReturnType<typeof import\(["']marketingclaw\/plugin-sdk\/(?:config-runtime|memory-core-host-runtime-core)["']\)\.(?:loadConfig|writeConfigFile)>/,
        replacement: "use MarketingClawConfig or the explicit mutation helper type",
      },
    ];
    for (const guard of guards) {
      for (const line of findMatchLineNumbers(source, guard.pattern)) {
