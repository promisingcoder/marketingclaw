...
marketingclaw_live_stage_source_tree() {
  local dest_dir="${1:?destination directory required}"
  local stage_mode="${MARKETINGCLAW_LIVE_DOCKER_SOURCE_STAGE_MODE:-copy}"

  if [ "$stage_mode" = "symlink" ]; then
    echo "MARKETINGCLAW_LIVE_DOCKER_SOURCE_STAGE_MODE=symlink is disabled; using copy staging." >&2
  fi
  ...
  tar -C /src \
    ...
    --exclude=.worktrees \
    --exclude=__marketingclaw_vitest__ \
    --exclude=relay.sock \
    --exclude='*.sock' \
    --exclude='*/*.sock' \
    --exclude='apps/*/.build' \
...
marketingclaw_live_link_runtime_tree() {
  local dest_dir="${1:?destination directory required}"
...
marketingclaw_live_stage_node_modules() {
  local dest_dir="${1:?destination directory required}"
...
marketingclaw_live_scrub_staged_plugin_index() {
  local dest_dir="${1:?destination directory required}"
  local db_path="$dest_dir/state/marketingclaw.sqlite"
...
marketingclaw_live_stage_state_dir() {
  local dest_dir="${1:?destination directory required}"
  local source_dir="${HOME}/.marketingclaw"
  ...
  if [ -d "$source_dir" ]; then
    ...
    marketingclaw_live_scrub_staged_plugin_index "$dest_dir"
    if [ -d "$source_dir/workspace" ] && [ ! -e "$dest_dir/workspace" ]; then
      ln -s "$source_dir/workspace" "$dest_dir/workspace"
    fi
  fi

  export MARKETINGCLAW_STATE_DIR="$dest_dir"
  export MARKETINGCLAW_CONFIG_PATH="$dest_dir/marketingclaw.json"
}

marketingclaw_live_prepare_staged_config() {
  if [ ! -f "${MARKETINGCLAW_CONFIG_PATH:-}" ]; then
    return 0
  fi

  local scripts_dir="${MARKETINGCLAW_LIVE_DOCKER_SCRIPTS_DIR:-/src/scripts}"
  (
    cd /app
