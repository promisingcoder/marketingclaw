...
export const DEFAULT_E2E_BARE_IMAGE = "marketingclaw-docker-e2e-bare:local";
export const DEFAULT_E2E_FUNCTIONAL_IMAGE = "marketingclaw-docker-e2e-functional:local";
      ...
      matrixScenarios
        .filter(
          (scenario) =>
            ...
            (scenario !== "acpx-marketingclaw-tools-bridge" ||
        ...
        .map((scenario) => {
          ...
          const commandPrefix = [
            `MARKETINGCLAW_UPGRADE_SURVIVOR_ARTIFACT_DIR="$PWD/.artifacts/upgrade-survivor/${name}"`,
            baselineSpec
              ? `MARKETINGCLAW_UPGRADE_SURVIVOR_BASELINE_SPEC=${shellQuote(baselineSpec)}`
              : "",
            scenario ? `MARKETINGCLAW_UPGRADE_SURVIVOR_SCENARIO=${shellQuote(scenario)}` : "",
          ]
            .filter(Boolean)
            .join(" ");
...
export function parseLiveMode(raw) {
  ...
  throw new Error(
    `MARKETINGCLAW_DOCKER_ALL_LIVE_MODE must be one of: all, skip, only. Got: ${JSON.stringify(raw)}`,
  );
}
...
export function parseProfile(raw) {
  ...
  throw new Error(
    `MARKETINGCLAW_DOCKER_ALL_PROFILE must be one of: ${DEFAULT_PROFILE}, ${RELEASE_PATH_PROFILE}. Got: ${JSON.stringify(raw)}`,
  );
}
...
export function lanesNeedMarketingClawPackage(poolLanes) {
...
export function findLaneByName(name) {
  return dedupeLanes(
    expandUpgradeSurvivorBaselineLanes(
      ...
      process.env.MARKETINGCLAW_UPGRADE_SURVIVOR_BASELINE_SPECS,
      process.env.MARKETINGCLAW_UPGRADE_SURVIVOR_SCENARIOS,
    ),
...
function buildPlanJson(params) {
  ...
  return {
    ...
    needs: {
      ...
      package: lanesNeedMarketingClawPackage(scheduledLanes),
    },
  ...
  const selectedLanes =
    options.selectedLaneNames.length > 0
      ? options.selectedLaneNames.flatMap((selectedName) => {
          ...
          selectNamedLanes(selectableLanes, [selectedName], "MARKETINGCLAW_DOCKER_ALL_LANES");
          return [];
        })
      : undefined;
