...
function collectTrackedBundledPluginSourceCandidates(repoRoot) {
  const result = spawnSync(
    ...
    [
      "ls-files",
      "--",
      ":(glob)extensions/*/marketingclaw.plugin.json",
  ...
  for (const rawLine of result.stdout.split("\n")) {
    ...
    const match = /^extensions\/([^/]+)\/(marketingclaw\.plugin\.json|package\.json)$/u.exec(line);
    if (!match?.[1] || !match[2]) {
      continue;
    }
    ...
    if (match[2] === "marketingclaw.plugin.json") {
