...
const TMUX_CWD_ENV_KEY = "MARKETINGCLAW_GATEWAY_WATCH_CWD";
const TMUX_CWD_OPTION_KEY = "@marketingclaw.gateway_watch.cwd";
const TMUX_CHILD_ENV_KEYS = [
  "NODE_OPTIONS",
  "MARKETINGCLAW_CONFIG_PATH",
  "MARKETINGCLAW_DIAGNOSTICS",
  "MARKETINGCLAW_DIAGNOSTICS_EVENT_LOOP",
  "MARKETINGCLAW_DIAGNOSTICS_TIMELINE_PATH",
  "MARKETINGCLAW_GATEWAY_PORT",
  "MARKETINGCLAW_GATEWAY_RESTART_TRACE",
  "MARKETINGCLAW_GATEWAY_STARTUP_TRACE",
  "MARKETINGCLAW_HOME",
...
const resolveGatewayWatchBenchmarkArgs = ({ args = [], env = process.env } = {}) => {
  ...
  if (benchmarkFlagSeen) {
    ...
    nextEnv.MARKETINGCLAW_TRACE_SYNC_IO ??= "0";
    if (nextEnv.MARKETINGCLAW_TRACE_SYNC_IO === "1") {
...
export const resolveGatewayWatchTmuxSessionName = ({ args = [], env = process.env } = {}) => {
  const profile =
    env.MARKETINGCLAW_PROFILE ||
    readArgValue(args, "--profile") ||
    (args.includes("--dev") ? "dev" : null);
  const port = env.MARKETINGCLAW_GATEWAY_PORT || readArgValue(args, "--port");
  const parts = [
    "marketingclaw",
    "gateway",
    "watch",
...
export const buildGatewayWatchTmuxCommand = ({
  ...
  const childEnv = [
    "env",
    ...colorEnv.options,
    `MARKETINGCLAW_GATEWAY_WATCH_TMUX_CHILD=1`,
    `MARKETINGCLAW_GATEWAY_WATCH_SESSION=${sessionName}`,
...
const log = (stderr, message) => {
  stderr.write(`[marketingclaw] ${message}\n`);
};
...
const shouldAttachTmux = ({ env, stdinIsTTY, stdoutIsTTY }) => {
  const raw = String(env.MARKETINGCLAW_GATEWAY_WATCH_ATTACH ?? "").toLowerCase();
...
export const runGatewayWatchTmuxMain = (params = {}) => {
  ...
  if (TMUX_DISABLE_VALUES.has((deps.env.MARKETINGCLAW_GATEWAY_WATCH_TMUX ?? "").toLowerCase())) {
  ...
  if (deps.env.MARKETINGCLAW_GATEWAY_WATCH_TMUX_CHILD === "1") {
    return runForegroundWatcher({
      args: deps.args,
      cwd: deps.cwd,
      env: deps.env,
