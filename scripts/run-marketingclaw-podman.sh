#!/usr/bin/env bash
# Rootless MarketingClaw in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-marketingclaw-podman.sh launch        # Start gateway
#   ./scripts/run-marketingclaw-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   marketingclaw --container marketingclaw dashboard --no-open
#   marketingclaw --container marketingclaw channels login
#
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      MARKETINGCLAW_GATEWAY_TOKEN|MARKETINGCLAW_PODMAN_CONTAINER|MARKETINGCLAW_PODMAN_IMAGE|MARKETINGCLAW_IMAGE|MARKETINGCLAW_PODMAN_PULL|MARKETINGCLAW_PODMAN_RUN_TIMEOUT|MARKETINGCLAW_PODMAN_GATEWAY_HOST_PORT|MARKETINGCLAW_GATEWAY_PORT|MARKETINGCLAW_PODMAN_BRIDGE_HOST_PORT|MARKETINGCLAW_BRIDGE_PORT|MARKETINGCLAW_GATEWAY_BIND|MARKETINGCLAW_PODMAN_USERNS|MARKETINGCLAW_BIND_MOUNT_OPTIONS|MARKETINGCLAW_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-marketingclaw-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${MARKETINGCLAW_CONFIG_DIR:-$EFFECTIVE_HOME/.marketingclaw}"
ENV_FILE="${MARKETINGCLAW_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${MARKETINGCLAW_CONFIG_DIR:-$EFFECTIVE_HOME/.marketingclaw}"
ENV_FILE="${MARKETINGCLAW_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${MARKETINGCLAW_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${MARKETINGCLAW_PODMAN_CONTAINER:-marketingclaw}"
MARKETINGCLAW_IMAGE="${MARKETINGCLAW_PODMAN_IMAGE:-${MARKETINGCLAW_IMAGE:-marketingclaw:local}}"
PODMAN_PULL="${MARKETINGCLAW_PODMAN_PULL:-never}"
PODMAN_RUN_TIMEOUT="${MARKETINGCLAW_PODMAN_RUN_TIMEOUT:-600s}"
HOST_GATEWAY_PORT="${MARKETINGCLAW_PODMAN_GATEWAY_HOST_PORT:-${MARKETINGCLAW_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${MARKETINGCLAW_PODMAN_BRIDGE_HOST_PORT:-${MARKETINGCLAW_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${MARKETINGCLAW_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$MARKETINGCLAW_IMAGE"
...
resolve_config_gateway_bind() {
  ...
  MARKETINGCLAW_CONTAINER="" MARKETINGCLAW_CONFIG_DIR="$config_dir" \
    marketingclaw config get gateway.bind 2>/dev/null || true
}
...
# MARKETINGCLAW_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${MARKETINGCLAW_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
create_token_env_file() {
  ...
  chmod 600 "$tmp"
  printf 'MARKETINGCLAW_GATEWAY_TOKEN=%s\n' "$token" >"$tmp"
  printf '%s' "$tmp"
}
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v marketingclaw >/dev/null 2>&1; then
    echo "Warning: marketingclaw not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    MARKETINGCLAW_CONTAINER="" MARKETINGCLAW_CONFIG_DIR="$config_dir" \
      marketingclaw config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    MARKETINGCLAW_CONTAINER="" MARKETINGCLAW_CONFIG_DIR="$config_dir" \
      marketingclaw config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  MARKETINGCLAW_CONTAINER="" MARKETINGCLAW_CONFIG_DIR="$config_dir" \
    marketingclaw config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${MARKETINGCLAW_GATEWAY_TOKEN:-}" ]]; then
  export MARKETINGCLAW_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "MARKETINGCLAW_GATEWAY_TOKEN" "$MARKETINGCLAW_GATEWAY_TOKEN"
  echo "Generated MARKETINGCLAW_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/marketingclaw.json"
...
PODMAN_USERNS="${MARKETINGCLAW_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported MARKETINGCLAW_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (MARKETINGCLAW_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${MARKETINGCLAW_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${MARKETINGCLAW_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$MARKETINGCLAW_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.marketingclaw/.npm \
    -e MARKETINGCLAW_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.marketingclaw:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.marketingclaw/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$MARKETINGCLAW_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME marketingclaw dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME marketingclaw devices approve --latest  # if pairing required"
