...
# Build and bundle MarketingClaw into a minimal .app we can open.
# Outputs to dist/MarketingClaw.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
source "$ROOT_DIR/scripts/lib/plistbuddy.sh"
source "$ROOT_DIR/scripts/lib/swift-toolchain.sh"
APP_ROOT="$ROOT_DIR/dist/MarketingClaw.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="MarketingClaw"
MLX_TTS_HELPER_PRODUCT="marketingclaw-mlx-tts"
MLX_TTS_HELPER_ROOT="$ROOT_DIR/apps/macos-mlx-tts"
MLX_TTS_HELPER_BUILD_ROOT="$MLX_TTS_HELPER_ROOT/.build"
BUNDLE_ID="${BUNDLE_ID:-ai.marketingclaw.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/promisingcoder/marketingclaw/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/MarketingClaw/Resources/Info.plist"
...
plist_set_string_required "$APP_ROOT/Contents/Info.plist" MarketingClawBuildTimestamp "$BUILD_TS"
plist_set_string_required "$APP_ROOT/Contents/Info.plist" MarketingClawGitCommit "$GIT_COMMIT"
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/MarketingClaw"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/MarketingClaw"
fi
chmod +x "$APP_ROOT/Contents/MacOS/MarketingClaw"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/MarketingClaw" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/MarketingClaw/Resources/MarketingClaw.icns" "$APP_ROOT/Contents/Resources/MarketingClaw.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/MarketingClaw/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying MarketingClawKit resources"
MARKETINGCLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/MarketingClawKit_MarketingClawKit.bundle"
if [ -d "$MARKETINGCLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/MarketingClawKit_MarketingClawKit.bundle"
  cp -R "$MARKETINGCLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/MarketingClawKit_MarketingClawKit.bundle"
else
  echo "ERROR: MarketingClawKit resource bundle not found at $MARKETINGCLAWKIT_BUNDLE" >&2
  exit 1
fi

running_packaged_app_pids() {
  command -v pgrep >/dev/null 2>&1 || return 0
  local app_binary="$APP_ROOT/Contents/MacOS/MarketingClaw"
  local pid
...
stop_packaged_app_if_running() {
  ...
  echo "⏹  Stopping packaged MarketingClaw bundle (${pids[*]})"
  ...
  echo "ERROR: Packaged MarketingClaw bundle did not exit: ${pids[*]}" >&2
  return 1
}
