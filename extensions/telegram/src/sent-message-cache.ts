...
const TELEGRAM_SENT_MESSAGES_STATE_KEY = Symbol.for("marketingclaw.telegramSentMessagesState");
const TELEGRAM_SENT_MESSAGES_STORE_FOR_TEST_KEY = Symbol.for(
  "marketingclaw.telegramSentMessagesStoreForTest",
);
...
function resolveSentMessageStorePath(cfg?: Pick<MarketingClawConfig, "session">): string {
  return `${resolveStorePath(cfg?.session?.store)}.telegram-sent-messages.json`;
}

function resolveSentMessageScopeKey(cfg?: Pick<MarketingClawConfig, "session">): string {
...
function getSentMessageBucket(cfg?: Pick<MarketingClawConfig, "session">): SentMessageBucket {
...
function getSentMessages(cfg?: Pick<MarketingClawConfig, "session">): SentMessageStore {
...
export function recordSentMessage(
  ...
  messageId: number,
  cfg?: Pick<MarketingClawConfig, "session">,
): void {
