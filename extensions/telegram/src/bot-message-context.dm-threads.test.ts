...
const { inboundBodyResult, recordInboundSessionMock, resolveStorePathMock } = vi.hoisted(() => {
  ...
  return {
    ...
    resolveStorePathMock: vi.fn<ResolveStorePathFn>(() => "/tmp/marketingclaw-session-store.json"),
  };
});
...
afterEach(() => {
  ...
  resolveStorePathMock.mockReturnValue("/tmp/marketingclaw-session-store.json");
});
...
describe("buildTelegramMessageContext group sessions without forum", () => {
  ...
  it("does not add a topic-cache store lookup for non-forum group reply threads", async () => {
    const resolveStorePath = vi.fn(() => "/tmp/marketingclaw/session-store.json");
  ...
  it("reloads topic name from disk after cache reset", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-telegram-topic-name-"));
...
describe("buildTelegramMessageContext direct peer routing", () => {
  it("isolates dm sessions by sender id when chat id differs", async () => {
    const runtimeCfg = {
      agents: { defaults: { model: "anthropic/claude-opus-4-5", workspace: "/tmp/marketingclaw" } },
      channels: { telegram: {} },
