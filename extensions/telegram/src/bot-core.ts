...
import { formatErrorMessage, formatUncaughtError } from "marketingclaw/plugin-sdk/error-runtime";
import {
...
export function createTelegramBotCore(
  ...
  const botHistorySender = buildTelegramGroupHistorySelfSender(
    account.name ?? opts.botInfo?.first_name ?? opts.botInfo?.username ?? "MarketingClaw",
  );
