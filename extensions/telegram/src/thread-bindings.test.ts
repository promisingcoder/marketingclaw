...
import { getSessionBindingService } from "marketingclaw/plugin-sdk/conversation-runtime";
...
describe("telegram thread bindings", () => {
  ...
  beforeEach(async () => {
    ...
    const acpRuntime = await vi.importActual<typeof import("marketingclaw/plugin-sdk/acp-runtime")>(
      "marketingclaw/plugin-sdk/acp-runtime",
    );
  ...
  it("does not persist lifecycle updates when manager persistence is disabled", async () => {
    stateDirOverride = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-telegram-bindings-"));
    process.env.MARKETINGCLAW_STATE_DIR = stateDirOverride;
    vi.useFakeTimers();
    vi.setSystemTime(new Date("2026-03-06T10:00:00.000Z"));
  ...
  it("persists unbinds before restart so removed bindings do not come back", async () => {
    ...
    const bound = await getSessionBindingService().bind({
      targetSessionKey: "plugin-binding:marketingclaw-codex-app-server:abc123",
      targetKind: "session",
      conversation: {
        channel: "telegram",
        accountId: "default",
  ...
  it("keeps plugin-owned bindings when ACP cleanup runs on startup", async () => {
    ...
    await getSessionBindingService().bind({
      targetSessionKey: "plugin-binding:marketingclaw-codex-app-server:still-valid",
      targetKind: "session",
      conversation: {
        channel: "telegram",
        accountId: "default",
    ...
    expect(reloaded.getByConversationId("plugin-binding-convo")?.targetSessionKey).toBe(
      "plugin-binding:marketingclaw-codex-app-server:still-valid",
    );
