...
import { resolvePersistentDedupePluginStateNamespace } from "marketingclaw/plugin-sdk/persistent-dedupe";
import {
...
describe("telegram state migrations", () => {
  it("detects legacy bot-info cache import", async () => {
    const dir = await mkdtemp(path.join(os.tmpdir(), "marketingclaw-telegram-state-migration-"));
