...
describe("createTelegramSendChatActionHandler", () => {
  ...
  it("suspends after maxConsecutive401 failures", async () => {
    ...
    expect(logger.mock.calls.at(-1)).toEqual([
      "CRITICAL: sendChatAction suspended after 3 consecutive 401 errors. Bot token is likely invalid. Telegram may DELETE the bot if requests continue. Replace the token and restart: marketingclaw channels restart telegram",
    ]);
  ...
  it("detects a structured Telegram 401 error by error_code, not by message substring", async () => {
    ...
    expect(handler.isSuspended()).toBe(true);
    expect(logger.mock.calls.at(-1)).toEqual([
      "CRITICAL: sendChatAction suspended after 2 consecutive 401 errors. Bot token is likely invalid. Telegram may DELETE the bot if requests continue. Replace the token and restart: marketingclaw channels restart telegram",
    ]);
  });
