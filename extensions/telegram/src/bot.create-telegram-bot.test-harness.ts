...
type GetRuntimeConfigFn =
  typeof import("marketingclaw/plugin-sdk/runtime-config-snapshot").getRuntimeConfig;
type GetSessionEntryFn =
  typeof import("marketingclaw/plugin-sdk/session-store-runtime").getSessionEntry;
type ListSessionEntriesFn =
  typeof import("marketingclaw/plugin-sdk/session-store-runtime").listSessionEntries;
type LoadSessionStoreFn =
  typeof import("marketingclaw/plugin-sdk/session-store-runtime").loadSessionStore;
type ResolveStorePathFn =
  typeof import("marketingclaw/plugin-sdk/session-store-runtime").resolveStorePath;
type ReadSessionUpdatedAtFn =
  typeof import("marketingclaw/plugin-sdk/session-store-runtime").readSessionUpdatedAt;
...
const { sessionStorePath } = vi.hoisted(() => {
  ...
  return {
    sessionStorePath: `${tempRoot.replace(/[\\/]+$/u, "")}${separator}marketingclaw-telegram-${
      process.pid
...
vi.mock("marketingclaw/plugin-sdk/web-media", () => ({
  loadWebMedia,
}));
...
function resolveDefaultModelForAgentForTest(params: { cfg: MarketingClawConfig }): {
  provider: string;
  model: string;
} {
...
function createModelsProviderDataFromConfig(cfg: MarketingClawConfig): {
...
const DEFAULT_TELEGRAM_TEST_CONFIG: MarketingClawConfig = {
  agents: {
    defaults: {
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: MarketingClawConfig) => {
