...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-telegram-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}
...
function buildConfig(
  ...
  >["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<MarketingClawConfig["channels"]>["telegram"]>>,
): MarketingClawConfig {
  return {
    channels: {
      telegram: {
