...
const pluginStateTestRuntime = await import("marketingclaw/plugin-sdk/plugin-state-test-runtime");
const pluginRuntime = await import("marketingclaw/plugin-sdk/plugin-runtime");
const conversationRuntime = await import("marketingclaw/plugin-sdk/conversation-runtime");
const configMutation = await import("marketingclaw/plugin-sdk/config-mutation");
const sessionStoreRuntime = await import("marketingclaw/plugin-sdk/session-store-runtime");
...
function createTelegramBotTestStateDir(): string {
  const dir = mkdtempSync(path.join(tmpdir(), "marketingclaw-telegram-bot-"));
...
describe("createTelegramBot", () => {
  ...
  beforeEach(async () => {
    ...
    process.env.MARKETINGCLAW_STATE_DIR = createTelegramBotTestStateDir();
    resetTelegramForumFlagCacheForTest();
    pluginRuntime.clearPluginInteractiveHandlers();
  ...
  it.each(["stop", "/stop@marketingclaw_bot"] as const)(
    ...
    try {
      ...
      await runTelegramMiddlewareChain({
        ctx: {
          ...
          message: {
            chat: { id: -1007, type: "supergroup", title: "MarketingClaw Ops" },
            text: "first",
            date: 1736380804,
            message_id: 104,
            from: { id: 42, first_name: "Ada" },
  ...
  it("routes plugin callback_query payloads to plugin handlers without fallback callback_data text", async () => {
    ...
    expect(
      pluginRuntime.registerPluginInteractiveHandler("marketingclaw-code-agent", {
        channel: "telegram",
  ...
  it("handles pairing DM flows for new and already-pending requests", async () => {
    ...
    for (const [index, testCase] of cases.entries()) {
      ...
      expect(pairingText, testCase.name).toContain("marketingclaw pairing approve telegram");
      expectRecordFields(
  ...
  it("ignores private self-authored message updates instead of issuing a pairing challenge", async () => {
    ...
    await handler({
      message: {
        ...
        message_id: 1884,
        date: 1736380800,
        from: { id: 7, is_bot: true, first_name: "MarketingClaw", username: "marketingclaw_bot" },
        pinned_message: {
          message_id: 1883,
          date: 1736380799,
          chat: { id: 1234, type: "private", first_name: "Harold" },
          from: { id: 7, is_bot: true, first_name: "MarketingClaw", username: "marketingclaw_bot" },
          text: "Binding: Review pull request 54118 (marketingclaw)",
        },
      },
      me: { id: 7, is_bot: true, first_name: "MarketingClaw", username: "marketingclaw_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
  ...
  it("ignores group self-authored message updates instead of re-processing bot output", async () => {
    ...
    await handler({
      message: {
        chat: { id: -1001234, type: "supergroup", title: "MarketingClaw Ops" },
        message_id: 1884,
        date: 1736380800,
        from: { id: 7, is_bot: true, first_name: "MarketingClaw", username: "marketingclaw_bot" },
  ...
  it("reloads topic agent overrides between messages without recreating the bot", async () => {
    ...
    const sendTopicMessage = async (chatId: number, messageId: number, text: string) => {
      await handler({
        ...
        me: { username: "marketingclaw_bot", has_topics_enabled: true },
        getFile: async () => ({ download: async () => new Uint8Array() }),
      });
    };
  ...
  async function dispatchMessage(params: {
    ...
    await handler({
      ...
      me: params.me ?? { username: "marketingclaw_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
  }
  ...
  it("marks explicit Telegram bot-handle mentions in the inbound context", async () => {
    ...
    await dispatchMessage({
      message: {
        chat: { id: 7, type: "group", title: "Test Group" },
        text: "@marketingclaw_bot status",
        entities: [{ type: "mention", offset: 0, length: "@marketingclaw_bot".length }],
        date: 1736380800,
        message_id: 4,
        from: { id: 9, first_name: "Ada" },
      },
    ...
    expect(payload.BotUsername).toBe("marketingclaw_bot");
  });
  ...
  it("honors routed group activation from session store", async () => {
    const storePath = "/tmp/marketingclaw-telegram-group-activation.json";
