...
async function loadEnvelopeTimestampHelpers() {
  return await import("marketingclaw/plugin-sdk/channel-test-helpers");
}
...
describe("createTelegramBot", () => {
  ...
  it("records outbound prompt-context sends into ambient group history", async () => {
    ...
    await recordOutboundMessageForPromptContext({
      cfg,
      account: { accountId: "default", name: "MarketingClaw" },
      chatId: -42,
      message: {
    ...
    await handler({
      me: { id: 999, username: "marketingclaw_bot" },
    ...
    expect(payload.InboundHistory).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ body: "Bot just replied", sender: "MarketingClaw (you)" }),
      ]),
    );
  });
  ...
  it("blocks DM model-selection callbacks for unpaired users when inline buttons are DM-scoped", async () => {
    ...
    const storePath = `/tmp/marketingclaw-telegram-callback-authz-${process.pid}-${Date.now()}.json`;
  ...
  it("blocks group model-selection callbacks for senders who are not authorized for /models", async () => {
    ...
    const storePath = `/tmp/marketingclaw-telegram-group-model-authz-${process.pid}-${Date.now()}.json`;
  ...
  it("recomputes group model-selection callback auth from runtime command config", async () => {
    ...
    const storePath = `/tmp/marketingclaw-telegram-group-model-authz-runtime-${process.pid}-${Date.now()}.json`;
  ...
  it("routes compact model callbacks against the configured provider", async () => {
    ...
    const storePath = `/tmp/marketingclaw-telegram-model-compact-${process.pid}-${Date.now()}.json`;
  ...
  it("renders model callback lists with configured display names", async () => {
    ...
    const storePath = `/tmp/marketingclaw-telegram-model-display-names-${process.pid}-${Date.now()}.json`;
  ...
  it("resets overrides when selecting the configured default model", async () => {
    ...
    const storePath = `/tmp/marketingclaw-telegram-model-default-${process.pid}-${Date.now()}.json`;
    const config: MarketingClawConfig = {
      agents: {
        defaults: {
  ...
  it("formats non-default model selection confirmations with Telegram HTML parse mode", async () => {
    ...
    const storePath = `/tmp/marketingclaw-telegram-model-html-${process.pid}-${Date.now()}.json`;
    ...
    try {
      ...
      expect(editCall[2]).toBe(
        `${CHECK_MARK_EMOJI} Model changed to <b>openai/gpt-5.4</b>\n\nSession-only model selection. Runtime unchanged. Use /model openai/gpt-5.4 --runtime &lt;runtime&gt; to switch harnesses. The agent default in marketingclaw.json is unchanged; /reset or a new session may return to that default.`,
      );
  ...
  it("persists non-default model override using fresh config, not stale startup snapshot", async () => {
    ...
    const storePath = `/tmp/marketingclaw-telegram-model-fresh-cfg-${process.pid}-${Date.now()}.json`;
  ...
  it("adds live chat and reply-target windows for stale group replies", async () => {
    ...
    await handler({
      ...
      message: {
        chat: { id: 42, type: "group", title: "Ops" },
        text: "@marketingclaw_bot thoughts?",
        date: 1736380920,
        message_id: 202,
  ...
  it("excludes ambient transcript rows from live group conversation context", async () => {
    ...
    try {
      ...
      await handler({
        ...
        message: {
          chat: { id: 42, type: "group", title: "Ops" },
          text: "@marketingclaw_bot what changed?",
          date: 1_736_380_980,
          message_id: 504,
  ...
  it("keeps direct Telegram media context when transcript context exists", async () => {
    ...
    const storePath = `/tmp/marketingclaw-telegram-dm-media-context-${process.pid}-${Date.now()}.json`;
    const config = {
      channels: {
        telegram: {
          dmPolicy: "open",
  ...
  it("dedupes direct assistant transcript context against cached Telegram replies after stripping directives", async () => {
    ...
    const storePath = `/tmp/marketingclaw-telegram-dm-visible-dedupe-${process.pid}-${Date.now()}.json`;
    const config = {
      channels: {
        telegram: {
          dmPolicy: "open",
      ...
      await handler({
        ...
        message: {
          ...
          reply_to_message: {
            ...
            from: { id: 999, is_bot: true, first_name: "MarketingClaw" },
            message_id: 736,
            marketingclaw_prompt_context_timestamp_ms: replyTimestampMs,
            text: visibleReply,
          },
        },
      });
  ...
  it("skips direct transcript context for hard reset messages", async () => {
    ...
    const storePath = `/tmp/marketingclaw-telegram-dm-reset-context-${process.pid}-${Date.now()}.json`;
    const config = {
      channels: {
        telegram: {
          dmPolicy: "open",
    ...
    try {
      ...
      await handler({
        ...
        message: {
          chat,
          message_id: 103,
          text: "@marketingclaw_bot explain what went wrong",
          date: 1736380800,
      ...
      try {
        ...
        await handler({
          ...
          message: {
            chat,
            message_id: 103,
            text: "@marketingclaw_bot explain this",
            date: 1736380800,
            from: { id: 1, is_bot: false, first_name: "Allowed" },
            reply_to_message: {
              chat,
    ...
    await handler({
      message: {
        ...
        reply_to_message: {
          message_id: 42,
          text: "original reply",
          from: { id: 999, first_name: "MarketingClaw" },
        },
      },
      me: { id: 999, username: "marketingclaw_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
  ...
  it("submits plugin-owned callback text through the Telegram inbound path", async () => {
    ...
    registerPluginInteractiveHandler("smart-replies-plugin", {
      channel: "telegram",
      namespace: "marketingclaw-smart-replies",
    ...
    try {
      ...
      await callbackHandler({
        callbackQuery: {
          ...
          data: "marketingclaw-smart-replies:v1:Rm14IGEgYnJva2VuIHRvb2w",
          from: { id: 9, first_name: "Ada", username: "ada_bot" },
          message: {
  ...
  it("does not submit plugin-owned callback text when the handler declines the callback", async () => {
    ...
    try {
      ...
      await callbackHandler({
        callbackQuery: {
          ...
          data: "marketingclaw-smart-replies:v1:SWdub3JlIHRoaXM",
          from: { id: 9, first_name: "Ada", username: "ada_bot" },
          message: {
            chat: { id: 9, type: "private" },
            date: 1736380800,
    ...
    expect(replySpy).toHaveBeenCalledTimes(1);
    const payload = mockMsgContextArg(replySpy as unknown as MockCallSource, 0, 0, "replySpy call");
    expect(payload.Body).toContain("callback_data: marketingclaw-smart-replies");
    ...
    try {
      ...
      const callbackContext = {
        callbackQuery: {
          ...
          data: "marketingclaw-smart-replies:v1:RG8gbm90IHN1Ym1pdCB0aGlz",
          from: { id: 9, first_name: "Ada", username: "ada_bot" },
          message: {
            chat: { id: 9, type: "private" },
            date: 1736380800,
    ...
    try {
      ...
      await callbackHandler({
        callbackQuery: {
          ...
          data: "marketingclaw-smart-replies:v1:SW52ZXN0aWdhdGUgdG9waWMgY2FsbGJhY2s",
          from: { id: 9, first_name: "Ada", username: "ada_bot" },
          message: {
    ...
    try {
      ...
      await callbackHandler({
        callbackQuery: {
          ...
          data: "marketingclaw-smart-replies:v1:TWFrZSBBbGljZSBmdW5uaWVy",
          from: { id: 9, first_name: "Ada", username: "ada_bot" },
          message: {
            chat: { id: 9, type: "private" },
            date: 1736380800,
    ...
    try {
      ...
      const createCallbackUpdate = (updateId: number) => ({
        ...
        callbackQuery: {
          ...
          data: "marketingclaw-smart-replies:v1:VHJ5IHRoaXMgbGF0ZXI",
          from: { id: 9, first_name: "Ada", username: "ada_bot" },
          message: {
            chat: { id: 9, type: "private" },
            date: 1736380800,
