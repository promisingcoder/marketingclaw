...
import { resolveRequestUrl } from "marketingclaw/plugin-sdk/request-url";
...
// fix for the leak observed in marketingclaw#68128 is the transport lifecycle that
...
function resolveMarketingClawProxyUrlForTelegram(
  env: NodeJS.ProcessEnv = process.env,
): string | undefined {
  const proxyUrl = env.MARKETINGCLAW_PROXY_URL?.trim();
...
export function resolveTelegramTransport(
  ...
  const managedProxyUrl =
    !effectiveProxyFetch && !hasEnvProxy ? resolveMarketingClawProxyUrlForTelegram() : undefined;
