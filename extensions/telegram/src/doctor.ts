...
function formatTelegramAccountConfigPath(cfg: MarketingClawConfig, accountId: string): string {
  const telegram = asObjectRecord((cfg.channels as Record<string, unknown> | undefined)?.telegram);
  const accounts = asObjectRecord(telegram?.accounts);
...
export function maybeRepairTelegramApiRoots(cfg: MarketingClawConfig): {
...
async function repairTelegramConfig(params: { cfg: MarketingClawConfig }): Promise<{
  config: MarketingClawConfig;
  changes: string[];
}> {
...
export async function maybeRepairTelegramAllowFromUsernames(cfg: MarketingClawConfig): Promise<{
  config: MarketingClawConfig;
  changes: string[];
}> {
  ...
  const { getChannelsCommandSecretTargetIds, resolveCommandSecretRefsViaGateway } =
    await import("marketingclaw/plugin-sdk/runtime");
