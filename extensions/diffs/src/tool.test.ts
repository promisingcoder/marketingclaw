...
describe("diffs tool", () => {
  ...
  beforeEach(async () => {
    ...
    } = await createDiffStoreHarness("marketingclaw-diffs-tool-"));
  });
  ...
  it("uses configured viewerBaseUrl when tool input omits baseUrl", async () => {
    ...
    expect(readTextContent(result, 0)).toContain(
      "https://example.com/marketingclaw/plugins/diffs/view/",
    );
...
function createApi(pluginConfig?: Record<string, unknown>): MarketingClawPluginApi {
