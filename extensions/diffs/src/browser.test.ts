...
import { createMockServerResponse } from "marketingclaw/plugin-sdk/test-env";
import { afterAll, afterEach, beforeAll, beforeEach, describe, expect, it, vi } from "vitest";
import type { MarketingClawConfig } from "../api.js";
import type { MarketingClawPluginApi, MarketingClawPluginToolContext } from "../api.js";
...
describe("diffs plugin registration", () => {
  it("uses live runtime tool config through the registered tool factory", async () => {
    ...
    type RegisteredHttpRouteParams = Parameters<MarketingClawPluginApi["registerHttpRoute"]>[0];

    let registeredToolFactory:
      | ((
          ctx: MarketingClawPluginToolContext,
        ) => RegisteredTool | RegisteredTool[] | null | undefined)
      | undefined;
    let registeredHttpRouteHandler: HttpRouteHandler | undefined;
    let configFile: MarketingClawConfig = {
      gateway: {
        port: 18789,
        bind: "loopback",
      },
      plugins: {
        entries: {
          diffs: {
            config: {
              viewerBaseUrl: "https://startup.example.com/marketingclaw",
              defaults: {
                mode: "view",
                theme: "light",
                background: false,
    ...
    const api = createTestPluginApi({
      ...
      registerTool(tool: Parameters<MarketingClawPluginApi["registerTool"]>[0]) {
    ...
    registerDiffsPlugin(api as unknown as MarketingClawPluginApi);
