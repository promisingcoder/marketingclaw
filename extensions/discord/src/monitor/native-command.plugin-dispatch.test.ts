...
import type { NativeCommandSpec } from "marketingclaw/plugin-sdk/command-auth-native";
import { resolveDirectStatusReplyForSession } from "marketingclaw/plugin-sdk/command-status-runtime";
...
import { dispatchReplyWithDispatcher } from "marketingclaw/plugin-sdk/reply-dispatch-runtime";
import {
  clearRuntimeConfigSnapshot,
  setRuntimeConfigSnapshot,
} from "marketingclaw/plugin-sdk/runtime-config-snapshot";
import { getSessionEntry } from "marketingclaw/plugin-sdk/session-store-runtime";
...
async function createNativeCommand(cfg: MarketingClawConfig, commandSpec: NativeCommandSpec) {
...
async function createPluginCommand(params: { cfg: MarketingClawConfig; name: string }) {
  return createDiscordNativeCommand({
    command: {
      name: params.name,
      description: "Pair",
...
async function createStatusCommand(cfg: MarketingClawConfig) {
...
describe("Discord native plugin command dispatch", () => {
  ...
  beforeEach(() => {
    ...
    discordNativeCommandTesting.setMatchPluginCommand(
      runtimeModuleMocks.matchPluginCommand as typeof import("marketingclaw/plugin-sdk/plugin-runtime").matchPluginCommand,
    );
    discordNativeCommandTesting.setExecutePluginCommand(
      runtimeModuleMocks.executePluginCommand as typeof import("marketingclaw/plugin-sdk/plugin-runtime").executePluginCommand,
    );
    ...
    discordNativeCommandTesting.setGetSessionEntry(
      runtimeModuleMocks.getSessionEntry as typeof import("marketingclaw/plugin-sdk/session-store-runtime").getSessionEntry,
    );
  });
  ...
  it("refreshes native command routing config between invocations", async () => {
    ...
    const resolveRouteState = vi.fn(async (params: { cfg: MarketingClawConfig }) =>
      createUnboundRouteState({
        sessionKey:
  ...
  it("passes the configured binding agent to plugin-owned Discord command sessions", async () => {
    const cfg = createConfig();
    const interaction = createInteraction();
    const pluginSessionKey = "plugin-binding:marketingclaw-codex-app-server:dm";
