...
import { typedCases } from "marketingclaw/plugin-sdk/test-fixtures";
...
describe("DiscordMessageListener", () => {
  ...
  it("logs handler failures", async () => {
    const logger = {
      ...
      typeof import("marketingclaw/plugin-sdk/logging-core").createSubsystemLogger
    >;
    const handler = vi.fn(async () => {
...
describe("discord allowlist helpers", () => {
  it("normalizes slugs", () => {
    expect(normalizeDiscordSlug("Friends of MarketingClaw")).toBe("friends-of-marketingclaw");
  ...
  it("matches ids by default and names only when enabled", () => {
    const allow = expectNormalizedAllowList(
      ["123", "steipete", "Friends of MarketingClaw"],
      ["discord:", "user:", "guild:", "channel:"],
    );
    expect(allowListMatches(allow, { id: "123" })).toBe(true);
    expect(allowListMatches(allow, { name: "steipete" })).toBe(false);
    expect(allowListMatches(allow, { name: "friends-of-marketingclaw" })).toBe(false);
    expect(allowListMatches(allow, { name: "steipete" }, { allowNameMatching: true })).toBe(true);
    expect(
      allowListMatches(allow, { name: "friends-of-marketingclaw" }, { allowNameMatching: true }),
    ).toBe(true);
...
describe("discord guild/channel resolution", () => {
  it("resolves guild entry by id", () => {
    const guildEntries = makeEntries({
      "123": { slug: "friends-of-marketingclaw" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of MarketingClaw"),
      guildEntries,
    });
    expect(resolved?.id).toBe("123");
    expect(resolved?.slug).toBe("friends-of-marketingclaw");
  });
  ...
  it("resolves guild entry by slug key", () => {
    const guildEntries = makeEntries({
      "friends-of-marketingclaw": { slug: "friends-of-marketingclaw" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of MarketingClaw"),
      guildEntries,
...
describe("discord group DM gating", () => {
  ...
  it("matches group DM allowlist", () => {
    expect(
      resolveGroupDmAllow({
        channels: ["marketingclaw-dm"],
        channelId: "1",
        channelName: "MarketingClaw DM",
        channelSlug: "marketingclaw-dm",
      }),
    ).toBe(true);
    expect(
      resolveGroupDmAllow({
...
const channelRuntimeModule = await import("marketingclaw/plugin-sdk/system-event-runtime");
vi.spyOn(channelRuntimeModule, "enqueueSystemEvent").mockImplementation(enqueueSystemEventSpy);

const routingModule = await import("marketingclaw/plugin-sdk/routing");
...
function makeReactionListenerParams(overrides?: {
  ...
  return {
    ...
    accountId: "acc-1",
    runtime: {} as import("marketingclaw/plugin-sdk/runtime-env").RuntimeEnv,
