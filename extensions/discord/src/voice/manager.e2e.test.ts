...
import type { RealtimeVoiceAgentControlResult } from "marketingclaw/plugin-sdk/realtime-voice";
  ...
  return {
    ...
    controlRealtimeVoiceAgentRunMock: vi.fn<() => Promise<RealtimeVoiceAgentControlResult>>(
      async () => ({
        ...
        message: "There is no active MarketingClaw run to steer.",
        speak: true,
        show: true,
        suppress: false,
      }),
...
vi.mock("marketingclaw/plugin-sdk/routing", async () => {
  const actual = await vi.importActual<typeof import("marketingclaw/plugin-sdk/routing")>(
    "marketingclaw/plugin-sdk/routing",
  );
  return {
    ...actual,
...
vi.mock("marketingclaw/plugin-sdk/agent-runtime", async () => {
  const actual = await vi.importActual<typeof import("marketingclaw/plugin-sdk/agent-runtime")>(
    "marketingclaw/plugin-sdk/agent-runtime",
  );
  return {
    ...actual,
    agentCommandFromIngress: agentCommandMock,
    getTtsProvider: vi.fn(() => "openai"),
    resolveAgentDir: vi.fn(() => "/tmp/marketingclaw-agent"),
    resolveTtsConfig: vi.fn(() => ({
      modelOverrides: {},
      providerConfigs: {},
    })),
    resolveTtsPrefsPath: vi.fn(() => "/tmp/marketingclaw-tts.json"),
  };
});

vi.mock("marketingclaw/plugin-sdk/realtime-bootstrap-context", async () => {
  const actual = await vi.importActual<
    typeof import("marketingclaw/plugin-sdk/realtime-bootstrap-context")
  >("marketingclaw/plugin-sdk/realtime-bootstrap-context");
  return {
    ...actual,
...
vi.mock("marketingclaw/plugin-sdk/realtime-voice", async () => {
  const actual = await vi.importActual<typeof import("marketingclaw/plugin-sdk/realtime-voice")>(
    "marketingclaw/plugin-sdk/realtime-voice",
  );
  return {
    ...actual,
...
describe("DiscordVoiceManager", () => {
  ...
  it("destroys stale tracked voice connections before joining", async () => {
    ...
    expect(getVoiceConnectionMock).toHaveBeenCalledWith("g1", "marketingclaw:default");
  ...
  it("isolates voice connections by Discord account", async () => {
    ...
    expect(getVoiceConnectionMock).toHaveBeenNthCalledWith(1, "g1", "marketingclaw:first");
    expect(getVoiceConnectionMock).toHaveBeenNthCalledWith(2, "g1", "marketingclaw:second");
    expect(joinVoiceChannelMock).toHaveBeenNthCalledWith(
      1,
      expect.objectContaining({ group: "marketingclaw:first" }),
    );
    expect(joinVoiceChannelMock).toHaveBeenNthCalledWith(
      2,
      expect.objectContaining({ group: "marketingclaw:second" }),
    );
  });
  ...
  it("uses agent-proxy realtime voice by default", async () => {
    ...
    expect(bridgeParams?.instructions).toContain("same MarketingClaw agent");
    expect(bridgeParams?.instructions).toContain("short natural backchannel");
    expect(bridgeParams?.tools?.map((tool) => tool.name)).toContain("marketingclaw_agent_consult");
    expect(bridgeParams?.tools?.map((tool) => tool.name)).toContain("marketingclaw_agent_control");
    const player = getLastAudioPlayer();
    bridgeParams?.audioSink?.sendAudio(Buffer.alloc(24_000));
    ...
    bridgeParams?.onToolCall?.(
      {
        ...
        args: {
          question: [
            "Speak this exact MarketingClaw answer to the Discord voice channel, without adding, removing, or rephrasing words.",
  ...
  it("routes active-run realtime transcripts to voice control before forced consults", async () => {
    ...
    await vi.waitFor(() => expectUserMessageIncludes("Cancelled the active MarketingClaw run."));
    expect(textToSpeechMock).not.toHaveBeenCalledWith(
      expect.objectContaining({ text: "Cancelled the active MarketingClaw run." }),
    );

    const stopCallsAfterControl = player.stop.mock.calls.length;
    bridgeParams?.onTranscript?.("assistant", "Cancelled the active MarketingClaw run.", true);
  ...
  it("accepts MarketingClaw as a default wake name before realtime agent-proxy consults", async () => {
    agentCommandMock.mockResolvedValueOnce({ payloads: [{ text: "marketingclaw wake answer" }] });
    const manager = createManager(
      {
        groupPolicy: "open",
        voice: {
    ...
    ownerTurn?.sendInputAudio(Buffer.alloc(8));
    await emitFinalRealtimeUserTranscript(bridgeParams, "MarketingClaw, how is it going");
    ...
    expect(lastAgentCommandArgs().message).toContain("how is it going");
    expect(lastAgentCommandArgs().message).not.toContain("MarketingClaw");
    expectUserMessageIncludes("marketingclaw wake answer");
  });
  ...
  it("ignores default agent wake names longer than two words", async () => {
    ...
    await emitFinalRealtimeUserTranscript(bridgeParams, "MarketingClaw, fallback still wakes");
  ...
  it("accepts leading fuzzy wake names before realtime agent-proxy consults", async () => {
    ...
    const marketingClawTurn = entry.realtime?.beginSpeakerTurn(
      { extraSystemPrompt: undefined, senderIsOwner: true, speakerLabel: "Owner" },
      "u-owner",
    );
    marketingClawTurn?.sendInputAudio(Buffer.alloc(8));
  ...
  it("does not accept configured realtime wake names longer than two words", async () => {
    ...
    fallbackTurn?.sendInputAudio(Buffer.alloc(8));
    await emitFinalRealtimeUserTranscript(bridgeParams, "MarketingClaw, ship it");
  ...
  it("keeps forced agent-proxy fallback diagnostics out of agent prompts", async () => {
    ...
    expect(lastAgentCommandArgs().message).not.toContain("marketingclaw_agent_consult");
  ...
  it("reuses forced agent-proxy answers for late matching consult tool calls", async () => {
    ...
    expect(realtimeSessionMock.submitToolResult).toHaveBeenCalledWith(
      "call-late",
      {
        ...
        message: "MarketingClaw already delivered this answer to Discord voice.",
      },
  ...
  it("starts Discord realtime voice in bidi mode with the consult tool", async () => {
    ...
    expect(bridgeParams?.autoRespondToAudio).toBe(true);
    expect(bridgeParams?.interruptResponseOnInputAudio).toBe(false);
    expect(bridgeParams?.instructions).toContain("Call marketingclaw_agent_consult");
    expect(bridgeParams?.tools?.map((tool) => tool.name)).toContain("marketingclaw_agent_consult");
  ...
  it("adds default bootstrap profile context to realtime voice instructions", async () => {
    ...
    resolveRealtimeBootstrapContextInstructionsMock.mockResolvedValue(
      "MarketingClaw realtime voice profile context:\n\n### IDENTITY.md\nName: Wilfred",
    );
    const manager = createManager({
      groupPolicy: "open",
      voice: {
    ...
    const bridgeParams = lastRealtimeBridgeParams() as
      ...
      | undefined;
    expect(bridgeParams?.instructions).toContain("MarketingClaw realtime voice profile context");
