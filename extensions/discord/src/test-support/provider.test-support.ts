...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: MarketingClawConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): MarketingClawConfig =>
  ({
    channels: {
      discord: {
        accounts: {
...
vi.mock("marketingclaw/plugin-sdk/acp-runtime", async () => {
  const actual = await vi.importActual<typeof import("marketingclaw/plugin-sdk/acp-runtime")>(
    "marketingclaw/plugin-sdk/acp-runtime",
  );
  return {
    ...actual,
...
vi.mock("marketingclaw/plugin-sdk/native-command-config-runtime", async () => {
  const actual = await vi.importActual<
    typeof import("marketingclaw/plugin-sdk/native-command-config-runtime")
  >("marketingclaw/plugin-sdk/native-command-config-runtime");
  return {
    ...actual,
...
vi.mock("marketingclaw/plugin-sdk/error-runtime", async () => {
  const actual = await vi.importActual<typeof import("marketingclaw/plugin-sdk/error-runtime")>(
    "marketingclaw/plugin-sdk/error-runtime",
  );
  return {
    ...actual,
