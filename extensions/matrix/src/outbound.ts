...
import { createReplyToFanout } from "marketingclaw/plugin-sdk/channel-outbound";
import {
  renderMessagePresentationFallbackText,
  type MessagePresentation,
} from "marketingclaw/plugin-sdk/interactive-runtime";
import { resolvePayloadMediaUrls } from "marketingclaw/plugin-sdk/reply-payload";
...
const MATRIX_MARKETINGCLAW_PRESENTATION_KEY = "com.marketingclaw.presentation" as const;
const MATRIX_MARKETINGCLAW_PRESENTATION_TYPE = "message.presentation" as const;
...
function buildMatrixPresentationContent(presentation: MessagePresentation) {
  return {
    ...presentation,
    version: 1,
    type: MATRIX_MARKETINGCLAW_PRESENTATION_TYPE,
  };
}
...
function resolveMatrixPresentationContent(
  ...
  const presentation = toRecord(extraContent?.[MATRIX_MARKETINGCLAW_PRESENTATION_KEY]);
  if (
    !presentation ||
    presentation.version !== 1 ||
    presentation.type !== MATRIX_MARKETINGCLAW_PRESENTATION_TYPE
  ) {
    return undefined;
  }
  return presentation;
  ...
  return {
    ...
    channelData: {
      ...
      matrix: {
        ...matrixData,
        extraContent: {
          [MATRIX_MARKETINGCLAW_PRESENTATION_KEY]: buildMatrixPresentationContent(
            params.presentation,
          ),
        },
      },
...
function resolveMatrixExtraContent(payload: ReplyPayload): MatrixExtraContentFields | undefined {
  ...
  return presentation ? { [MATRIX_MARKETINGCLAW_PRESENTATION_KEY]: presentation } : undefined;
}
