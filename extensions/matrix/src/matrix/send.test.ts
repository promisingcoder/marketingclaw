...
import { MATRIX_MARKETINGCLAW_FINALIZED_PREVIEW_KEY } from "./send/types.js";
...
vi.mock("marketingclaw/plugin-sdk/plugin-config-runtime", async () => {
  const actual = await vi.importActual<
    typeof import("marketingclaw/plugin-sdk/plugin-config-runtime")
  >("marketingclaw/plugin-sdk/plugin-config-runtime");
  return {
    ...actual,
...
describe("sendMessageMatrix media", () => {
  ...
  it("passes caller mediaLocalRoots to media loading", async () => {
    ...
    expect(mediaOptions.localRoots).toEqual(["/tmp/marketingclaw-matrix-test"]);
  });
});
...
describe("sendMessageMatrix threads", () => {
  ...
  it("merges extra content into only the first chunked text event", async () => {
    ...
    await sendMessageMatrix("room:!room:example", "ignored", {
      client,
      cfg: {} as never,
      extraContent: { "com.marketingclaw.approval": { id: "req-1" } },
    });

    expect(sendMessage).toHaveBeenCalledTimes(3);
    expect(sentContent(sendMessage, 0).body).toBe("first");
    expect(sentContent(sendMessage, 0)["com.marketingclaw.approval"]).toEqual({ id: "req-1" });
    expect(sentContent(sendMessage, 1).body).toBe("second");
    expect(sentContent(sendMessage, 1)).not.toHaveProperty("com.marketingclaw.approval");
    expect(sentContent(sendMessage, 2).body).toBe("third");
    expect(sentContent(sendMessage, 2)).not.toHaveProperty("com.marketingclaw.approval");
  });
});
...
describe("sendSingleTextMessageMatrix", () => {
  ...
  it("merges extra content fields into single-event sends", async () => {
    ...
    const result = await sendSingleTextMessageMatrix("room:!room:example", "done", {
      client,
      cfg: {} as never,
      extraContent: { [MATRIX_MARKETINGCLAW_FINALIZED_PREVIEW_KEY]: true },
    });

    expect(sentContent(sendMessage).body).toBe("done");
    expect(sentContent(sendMessage)[MATRIX_MARKETINGCLAW_FINALIZED_PREVIEW_KEY]).toBe(true);
...
describe("editMessageMatrix mentions", () => {
  ...
  it("merges extra content fields into edit payloads and m.new_content", async () => {
    ...
    const content = sentContent(sendMessage);
    expect(content[MATRIX_MARKETINGCLAW_FINALIZED_PREVIEW_KEY]).toBe(true);
    expect(newContent(content)[MATRIX_MARKETINGCLAW_FINALIZED_PREVIEW_KEY]).toBe(true);
  });
