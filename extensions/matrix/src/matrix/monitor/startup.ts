...
export async function runMatrixStartupMaintenance(
  ...
  try {
    ...
    if (deviceHealth.staleMarketingClawDevices.length > 0) {
      params.logger.warn(
        `matrix: stale MarketingClaw devices detected for ${params.auth.userId}: ${deviceHealth.staleMarketingClawDevices.map((device) => device.deviceId).join(", ")}. Run 'marketingclaw matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
    if (isMatrixStartupAbortError(err)) {
