...
function expectFinalizedPreviewEdit(eventId: string, text: string) {
  ...
  expect(options.extraContent).toEqual({ [MATRIX_MARKETINGCLAW_FINALIZED_PREVIEW_KEY]: true });
}
...
describe("matrix monitor handler draft streaming", () => {
  ...
  it("replaces recovered Matrix command progress instead of leaving stale failed text", async () => {
    ...
    await opts.onItemEvent?.({
      ...
      phase: "end",
      status: "failed",
      progressText: "run marketingclaw cron -> run jq (agent) failed",
    });
    await opts.onItemEvent?.({
      itemId: "command-1",
      kind: "command",
    ...
    expect(recoveredEdit?.[2]).not.toContain("run marketingclaw cron -> run jq");
  });
