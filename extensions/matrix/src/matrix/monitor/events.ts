...
function formatMatrixPostHealthySyncDecryptionHint(accountId: string): string {
  return (
    ...
    `Check 'marketingclaw matrix verify status --verbose --account ${accountId}' and 'marketingclaw matrix devices list --account ${accountId}'.`
  );
}
...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another MarketingClaw runtime is using the same account. " +
    `Check 'marketingclaw matrix verify status --verbose --account ${accountId}' and 'marketingclaw matrix devices list --account ${accountId}'.`
  );
}
