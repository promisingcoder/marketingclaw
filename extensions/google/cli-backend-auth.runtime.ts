...
import type { CliBackendPreparedExecution } from "marketingclaw/plugin-sdk/cli-backend";
...
function throwUnsupportedGeminiCredential(credential: GeminiAuthProfileCredential): never {
  if (credential.provider === VERCEL_AI_GATEWAY_PROVIDER_ID) {
    throw new Error(
      "Gemini CLI execution cannot use a vercel-ai-gateway auth profile. Use the MarketingClaw vercel-ai-gateway provider instead.",
    );
  }
...
function throwUnstageableSelectedGeminiProfile(
  ...
  if (!credential) {
    throw new Error(
      "Gemini CLI auth profile was selected but no credential material was found. Re-authenticate with `marketingclaw models auth login --provider google-gemini-cli --force`.",
    );
  }
  ...
  throw new Error(
    "Gemini CLI execution supports google-gemini-cli OAuth or API-key auth profiles. Re-authenticate with `marketingclaw models auth login --provider google-gemini-cli --force`.",
  );
}
...
function requireGeminiOAuthCredential(
  ...
  if (
    ...
    throw new Error(
      "Gemini CLI OAuth profile is missing usable token material. Re-authenticate with `marketingclaw models auth login --provider google-gemini-cli --force`.",
    );
  }
...
async function buildGeminiCliSystemSettings(
  ...
  if (enforcedType && enforcedType !== selectedType) {
    throw new Error(
      `Gemini CLI system settings enforce ${enforcedType} auth, but the selected MarketingClaw profile requires ${selectedType}.`,
    );
  }
...
async function prepareGeminiCliProfileHome(
  ...
  const systemSettingsDir = await fs.mkdtemp(
    path.join(resolvePreferredMarketingClawTmpDir(), "marketingclaw-gemini-cli-"),
  );
...
async function clearGeminiCliCachedCredentials(geminiDir: string): Promise<void> {
  ...
  // from the selected MarketingClaw profile each run so stale CLI auth cannot win.
