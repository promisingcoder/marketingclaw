...
describe("loginGeminiCliOAuth", () => {
  ...
  function userInfoResponse(): Response {
    return responseJson({ email: "lobster@marketingclaw.ai" });
  }
  ...
  beforeEach(() => {
    envSnapshot = Object.fromEntries(ENV_KEYS.map((key) => [key, process.env[key]]));
    process.env.MARKETINGCLAW_GEMINI_OAUTH_CLIENT_ID = "test-client-id.apps.googleusercontent.com";
    process.env.MARKETINGCLAW_GEMINI_OAUTH_CLIENT_SECRET = "GOCSPX-test-client-secret"; // pragma: allowlist secret
  ...
  it("refreshes Gemini CLI OAuth tokens without loadCodeAssist in personal OAuth mode", async () => {
    ...
    const result = await refreshTokensForGeminiCli({
      ...
      email: "lobster@marketingclaw.ai",
    });
