...
import { asObject, trimToUndefined } from "marketingclaw/plugin-sdk/speech-core";
...
function resolveGoogleTtsModelProviderApiKey(cfg?: MarketingClawConfig): string | undefined {
...
function isMarketingClawGoogleAudioProfilePrompt(text: string): boolean {
  return (
...
export function buildGoogleSpeechProvider(): SpeechProviderPlugin {
  return {
    ...
    prepareSynthesis: (ctx) => {
      ...
      if (!shouldWrap || isMarketingClawGoogleAudioProfilePrompt(ctx.text)) {
        return undefined;
      }
      return {
