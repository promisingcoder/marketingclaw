...
describe("kimi web search provider", () => {
  ...
  it("points missing-key users to fetch/browser alternatives", async () => {
    await withEnvAsync({ KIMI_API_KEY: undefined, MOONSHOT_API_KEY: undefined }, async () => {
      ...
      const result = await tool.execute({ query: "MarketingClaw docs" });
  ...
  it("accepts final responses backed by Kimi web search tool replay", async () => {
    const toolArguments = JSON.stringify({
      query: "MarketingClaw GitHub repository",
      ...
      .mockResolvedValueOnce(
        jsonResponse({
          choices: [
            {
              finish_reason: "stop",
              message: { content: "MarketingClaw is available on GitHub." },
            },
          ],
        }),
      );
    ...
    await withEnvAsync({ KIMI_API_KEY: "kimi-test-key" }, async () => {
      ...
      expect(result.provider).toBe("kimi");
      expectStringFieldContains(result, "content", "MarketingClaw is available on GitHub.");
  ...
  it("accepts final responses with search result citations", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      jsonResponse({
        search_results: [
          { title: "MarketingClaw", url: "https://github.com/promisingcoder/marketingclaw" },
        ],
        choices: [
          {
            finish_reason: "stop",
            message: { content: "MarketingClaw is on GitHub." },
          },
        ],
      }),
    );
    ...
    await withEnvAsync({ KIMI_API_KEY: "kimi-test-key" }, async () => {
      ...
      expect(result.provider).toBe("kimi");
      expectStringFieldContains(result, "content", "MarketingClaw is on GitHub.");
      expect(result.citations).toEqual(["https://github.com/promisingcoder/marketingclaw"]);
      expect(result).not.toHaveProperty("error");
    });
  });
