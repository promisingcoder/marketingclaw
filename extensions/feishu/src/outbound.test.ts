...
import type { MessagePresentation } from "marketingclaw/plugin-sdk/interactive-runtime";
...
describe("feishuOutbound.sendText local-image auto-convert", () => {
  ...
  async function createTmpImage(ext = ".png"): Promise<{ dir: string; file: string }> {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-feishu-outbound-"));
...
describe("feishuOutbound.sendPayload native cards", () => {
  ...
  async function createTmpImage(ext = ".png"): Promise<{ dir: string; file: string }> {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-feishu-payload-"));
    const file = path.join(dir, `sample${ext}`);
    await fs.writeFile(file, "image-data");
    return { dir, file };
  }
