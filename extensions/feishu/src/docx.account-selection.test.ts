...
describe("feishu_doc account selection", () => {
  ...
  function createDocEnabledConfig(): MarketingClawPluginApi["config"] {
    return {
      channels: {
        feishu: {
          enabled: true,
    ...
    } as MarketingClawPluginApi["config"];
  }

  function createMixedToolConfig(): MarketingClawPluginApi["config"] {
    return {
      channels: {
        feishu: {
          enabled: true,
