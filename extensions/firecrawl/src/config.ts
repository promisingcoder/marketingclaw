...
import { canResolveEnvSecretRefInReadOnlyPath } from "marketingclaw/plugin-sdk/extension-shared";
import { resolvePositiveTimeoutSeconds } from "marketingclaw/plugin-sdk/provider-web-fetch";
import {
...
type WebFetchConfig = NonNullable<MarketingClawConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: MarketingClawConfig): WebSearchConfig {
...
function resolveFetchConfig(cfg?: MarketingClawConfig): WebFetchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: MarketingClawConfig): FirecrawlSearchConfig {
...
function resolveFirecrawlFetchConfig(cfg?: MarketingClawConfig): FirecrawlFetchConfig {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlApiKey(cfg?: MarketingClawConfig): string | undefined {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlBaseUrl(cfg?: MarketingClawConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlMaxAgeMs(cfg?: MarketingClawConfig, override?: number): number {
