...
import { isUsageCountedSessionTranscriptFileName } from "marketingclaw/plugin-sdk/memory-core-host-engine-qmd";
import type { MemoryEmbeddingProbeResult } from "marketingclaw/plugin-sdk/memory-core-host-engine-storage";
import {
...
function formatMemoryIndexIdentityWarning(
  ...
  return {
    reason,
    fix: `Run: marketingclaw memory status --index --agent ${agentId}`,
  };
}
...
function resolveMemoryPluginConfig(cfg: MarketingClawConfig): Record<string, unknown> {
...
async function createHistoricalRemHarnessWorkspace(params: {
  ...
  const workspaceDir = await fs.mkdtemp(
    path.join(resolvePreferredMarketingClawTmpDir(), "marketingclaw-rem-harness-"),
  );
...
function formatDreamingSummary(cfg: MarketingClawConfig): string {
...
function resolveAgent(cfg: MarketingClawConfig, agent?: string) {
...
function resolveAgentIds(cfg: MarketingClawConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (trimmed) {
    return [trimmed];
  }
...
export async function runMemoryStatus(opts: MemoryCommandOptions) {
  ...
  for (const result of allResults) {
    ...
    if (audit?.issues.length) {
      ...
      if (!opts.fix) {
        lines.push(`  ${muted(`Fix: marketingclaw memory status --fix --agent ${agentId}`)}`);
      }
    }
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      const scratchDir = await fs.mkdtemp(
        path.join(resolvePreferredMarketingClawTmpDir(), "marketingclaw-rem-backfill-"),
      );
      try {
