...
const {
  ...
  const chokidarKey = Symbol.for("marketingclaw.test.memoryWatchFactory");
  const nativeKey = Symbol.for("marketingclaw.test.memoryNativeWatchFactory");
...
const CHOKIDAR_FACTORY_KEY = Symbol.for("marketingclaw.test.memoryWatchFactory");
const NATIVE_FACTORY_KEY = Symbol.for("marketingclaw.test.memoryNativeWatchFactory");
const originalWatcherStateDir = process.env.MARKETINGCLAW_STATE_DIR;
...
function restoreWatcherStateDir(): void {
  if (originalWatcherStateDir === undefined) {
    ...
    Reflect.set(process.env, "MARKETINGCLAW_STATE_DIR", originalWatcherStateDir);
  }
}

vi.mock("marketingclaw/plugin-sdk/memory-core-host-engine-foundation", async (importOriginal) => {
  const actual =
...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): MarketingClawConfig {
    const defaults: NonNullable<NonNullable<MarketingClawConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: MarketingClawConfig) {
  ...
  it("routes Linux directories through directory-only native watchers", async () => {
    ...
    // internal/fs/recursive_watch. MarketingClaw watches directories only so
