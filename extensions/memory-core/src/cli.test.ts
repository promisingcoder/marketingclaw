...
vi.mock("./cli.host.runtime.js", async () => {
  const [runtimeCli, runtimeCore, runtimeFiles] = await Promise.all([
    import("marketingclaw/plugin-sdk/memory-core-host-runtime-cli"),
    import("marketingclaw/plugin-sdk/memory-core-host-runtime-core"),
    import("marketingclaw/plugin-sdk/memory-core-host-runtime-files"),
  ]);
  return {
...
let defaultRuntime: typeof import("marketingclaw/plugin-sdk/memory-core-host-runtime-cli").defaultRuntime;
let isVerbose: typeof import("marketingclaw/plugin-sdk/memory-core-host-runtime-cli").isVerbose;
let setVerbose: typeof import("marketingclaw/plugin-sdk/memory-core-host-runtime-cli").setVerbose;
let fixtureRoot = "";
...
beforeAll(async () => {
  ...
  ({ defaultRuntime, isVerbose, setVerbose } =
    await import("marketingclaw/plugin-sdk/memory-core-host-runtime-cli"));
...
describe("memory cli", () => {
  ...
  it("prints index identity mismatch reasons", async () => {
    ...
    expectLogged(log, "Fix: Run: marketingclaw memory status --index --agent main");
    expect(close).toHaveBeenCalled();
  });
  ...
  it("documents memory help examples", () => {
    ...
    expect(helpText).toContain("marketingclaw memory status --fix");
    expect(helpText).toContain("Repair stale recall locks and normalize promotion metadata.");
    expect(helpText).toContain("marketingclaw memory status --deep");
    expect(helpText).toContain("Probe embedding provider readiness.");
    expect(helpText).toContain('marketingclaw memory search "meeting notes"');
    expect(helpText).toContain("Quick search using positional query.");
    expect(helpText).toContain('marketingclaw memory search --query "deployment" --max-results 20');
    expect(helpText).toContain("Limit results for focused troubleshooting.");
    expect(helpText).toContain("marketingclaw memory promote --apply");
    expect(helpText).toContain("Append top-ranked short-term candidates into MEMORY.md.");
    expect(helpText).toContain('marketingclaw memory promote-explain "router vlan"');
    expect(helpText).toContain("Explain why a specific candidate would or would not promote.");
    expect(helpText).toContain("marketingclaw memory rem-harness --json");
  ...
  it("shows the fix hint only before --fix has been run", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      ...
      const log = spyRuntimeLogs(defaultRuntime);
      await runMemoryCli(["status"]);
      expectLogged(log, "Fix: marketingclaw memory status --fix --agent main");
      ...
      await runMemoryCli(["status", "--fix"]);
      expectNotLogged(log, "Fix: marketingclaw memory status --fix --agent main");
    });
  });
  ...
  it("writes grounded rem backfill entries into DREAMS.md", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      ...
      expect(dreams).toContain("marketingclaw:dreaming:backfill-entry");
  ...
  it("prefers persistence-relevant evidence over narrated operational logs in grounded what happened", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      ...
      await fs.writeFile(
        historyPath,
        [
          "## MarketingClaw / runtime / workflow preferences and corrections",
    ...
    await withTempWorkspace(async (workspaceDir) => {
      ...
      await fs.writeFile(
        ...
        [
          ...
          "*January 1, 2025*",
          "",
          "<!-- marketingclaw:dreaming:backfill-entry day=2025-01-01 source=memory/2025-01-01.md -->",
          "",
          "What Happened",
          "1. Remove this entry.",
          "",
  ...
  it("applies top promote candidates into MEMORY.md", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      ...
      expect(memoryText).toContain("marketingclaw-memory-promotion:");
