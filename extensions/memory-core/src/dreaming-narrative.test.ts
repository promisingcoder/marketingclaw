...
import * as memoryCoreHostRuntimeCoreModule from "marketingclaw/plugin-sdk/memory-core-host-runtime-core";
import * as runtimeConfigSnapshotModule from "marketingclaw/plugin-sdk/runtime-config-snapshot";
import * as sessionStoreRuntimeModule from "marketingclaw/plugin-sdk/session-store-runtime";
...
const originalNarrativeStateDir = process.env.MARKETINGCLAW_STATE_DIR;
...
function restoreNarrativeTestEnv(): void {
  if (originalNarrativeStateDir === undefined) {
    ...
    Reflect.set(process.env, "MARKETINGCLAW_STATE_DIR", originalNarrativeStateDir);
  }
}
...
describe("backfill diary entries", () => {
  ...
  it("builds a marked backfill diary entry", () => {
    ...
    expect(entry).toContain("marketingclaw:dreaming:backfill-entry");
    expect(entry).toContain("What Happened");
  });

  it("writes and replaces backfill diary entries", async () => {
    const workspaceDir = await createTempWorkspace("marketingclaw-dreaming-backfill-");
    ...
    expect(content.match(/marketingclaw:dreaming:backfill-entry/g)?.length).toBe(1);
  });
...
describe("appendNarrativeEntry", () => {
  it("creates DREAMS.md with diary header on fresh workspace", async () => {
    const workspaceDir = await createTempWorkspace("marketingclaw-dreaming-narrative-");
    ...
    expect(content).toContain("<!-- marketingclaw:dreaming:diary:start -->");
    expect(content).toContain("<!-- marketingclaw:dreaming:diary:end -->");
  });
  ...
  it("appends a second entry within the diary markers", async () => {
    ...
    const start = content.indexOf("<!-- marketingclaw:dreaming:diary:start -->");
    const end = content.indexOf("<!-- marketingclaw:dreaming:diary:end -->");
  ...
  it("prepends diary before existing managed blocks", async () => {
    ...
    await fs.writeFile(
      dreamsPath,
      "## Light Sleep\n<!-- marketingclaw:dreaming:light:start -->\n- Candidate: test\n<!-- marketingclaw:dreaming:light:end -->\n",
      "utf-8",
    );
    await appendNarrativeEntry({
      workspaceDir,
  ...
  it("dedupes only exact diary duplicates while keeping distinct timestamps", async () => {
    const workspaceDir = await createTempWorkspace("marketingclaw-dreaming-dedupe-");
    const dreamsPath = path.join(workspaceDir, "DREAMS.md");
    await fs.writeFile(
      dreamsPath,
      [
...
describe("generateAndAppendDreamNarrative", () => {
  ...
  it("scrubs stale dreaming entries and orphan transcripts after cleanup", async () => {
    const workspaceDir = await createTempWorkspace("marketingclaw-dreaming-narrative-");
    const stateDir = await createTempWorkspace("marketingclaw-dreaming-state-");
  ...
  it("isolates narrative sessions across workspaces even at the same timestamp", async () => {
    const firstWorkspaceDir = await createTempWorkspace("marketingclaw-dreaming-narrative-");
    const secondWorkspaceDir = await createTempWorkspace("marketingclaw-dreaming-narrative-");
...
describe("runDetachedDreamNarrative", () => {
  ...
  it("caps the number of in-flight detached narratives at 3", async () => {
    ...
    const workspaceDirs = await Promise.all(
      Array.from({ length: 5 }, () => createTempWorkspace("marketingclaw-dreaming-detach-")),
    );
    const logger = createMockLogger();
  ...
  it("serializes detached narratives that reuse a workspace and phase session", async () => {
    ...
    const workspaceDir = await createTempWorkspace("marketingclaw-dreaming-detach-");
    const logger = createMockLogger();
