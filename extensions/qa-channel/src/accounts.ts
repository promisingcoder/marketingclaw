...
import { createAccountListHelpers } from "marketingclaw/plugin-sdk/account-helpers";
import { DEFAULT_ACCOUNT_ID, normalizeAccountId } from "marketingclaw/plugin-sdk/account-id";
import { resolveMergedAccountConfig } from "marketingclaw/plugin-sdk/account-resolution-runtime";
...
export function resolveQaChannelAccount(params: {
  ...
  const botUserId = merged.botUserId?.trim() || "marketingclaw";
  const botDisplayName = merged.botDisplayName?.trim() || "MarketingClaw QA";
  return {
    accountId,
    enabled,
