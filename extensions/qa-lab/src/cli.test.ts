...
vi.mock("marketingclaw/plugin-sdk/qa-runner-runtime", () => ({
...
describe("qa cli registration", () => {
  ...
  it.each([
    ...
    await expect(
      program.parseAsync(["node", "marketingclaw", "qa", "run", flag, ...values]),
  ...
  it("rejects an empty qa run --qa-profile instead of falling back to self-check", async () => {
    await expect(
      program.parseAsync(["node", "marketingclaw", "qa", "run", "--qa-profile", ""]),
  ...
  it("routes mantis desktop browser smoke flags into the mantis runtime command", async () => {
    ...
    expect(runMantisDesktopBrowserSmokeCommand).toHaveBeenCalledWith({
      browserUrl: "https://marketingclaw.ai/docs",
  ...
  it("routes mantis Slack desktop smoke flags into the mantis runtime command", async () => {
    await program.parseAsync([
      ...
      "cbx_123abc",
      "--fresh-pr",
      "promisingcoder/marketingclaw#85141",
      "--idle-timeout",
      "45m",
      "--ttl",
      "120m",
  ...
  it("registers standalone provider server commands from the provider registry", async () => {
    ...
    await program.parseAsync(["node", "marketingclaw", "qa", "aimock", "--port", "44080"]);
  ...
  it("normalizes signed decimal QA numeric option values through the shared parser", async () => {
    await program.parseAsync(["node", "marketingclaw", "qa", "aimock", "--port", "+044080"]);
  ...
  it.each([
    ...
    await expect(invalidProgram.parseAsync(["node", "marketingclaw", ...args])).rejects.toThrow(
      message,
    );
  });
  ...
  it("shows an enable hint when a discovered runner plugin is installed but blocked", async () => {
    ...
    await expect(
      blockedProgram.parseAsync(["node", "marketingclaw", "qa", TEST_QA_RUNNER.commandName]),
  ...
  it("routes telegram CLI defaults into the lane runtime", async () => {
    await program.parseAsync(["node", "marketingclaw", "qa", "telegram"]);
  ...
  it("forwards --list-scenarios for telegram runs", async () => {
    await program.parseAsync(["node", "marketingclaw", "qa", "telegram", "--list-scenarios"]);
  ...
  it("forwards --allow-failures for telegram runs", async () => {
    await program.parseAsync(["node", "marketingclaw", "qa", "telegram", "--allow-failures"]);
  ...
  it("forwards --allow-failures for suite runs", async () => {
    await program.parseAsync(["node", "marketingclaw", "qa", "suite", "--allow-failures"]);
  ...
  it("forwards --pack for suite runs", async () => {
    await program.parseAsync(["node", "marketingclaw", "qa", "suite", "--pack", "personal-agent"]);
