...
describe("qa suite planning helpers", () => {
  it("normalizes suite concurrency to a bounded integer", () => {
    const previous = process.env.MARKETINGCLAW_QA_SUITE_CONCURRENCY;
    delete process.env.MARKETINGCLAW_QA_SUITE_CONCURRENCY;
    try {
      ...
      process.env.MARKETINGCLAW_QA_SUITE_CONCURRENCY = "3";
      expect(normalizeQaSuiteConcurrency(undefined, 10)).toBe(3);

      process.env.MARKETINGCLAW_QA_SUITE_CONCURRENCY = "0";
      expect(normalizeQaSuiteConcurrency(undefined, 10)).toBe(1);

      for (const value of ["0x10", "1e2", "2.5"]) {
        process.env.MARKETINGCLAW_QA_SUITE_CONCURRENCY = value;
        expect(normalizeQaSuiteConcurrency(undefined, 10)).toBe(10);
      }
    } finally {
      if (previous === undefined) {
        ...
        process.env.MARKETINGCLAW_QA_SUITE_CONCURRENCY = previous;
      }
    }
  });
  ...
  it("resolves a default worker startup stagger for concurrent suite workers", () => {
    ...
    expect(
      resolveQaSuiteWorkerStartStaggerMs(4, {
        MARKETINGCLAW_QA_SUITE_WORKER_START_STAGGER_MS: "0",
      }),
    ).toBe(0);
    expect(
      resolveQaSuiteWorkerStartStaggerMs(4, {
        MARKETINGCLAW_QA_SUITE_WORKER_START_STAGGER_MS: "25",
      }),
    ).toBe(25);
    for (const value of ["0x10", "1e3", "10.5"]) {
      expect(
        resolveQaSuiteWorkerStartStaggerMs(4, {
          MARKETINGCLAW_QA_SUITE_WORKER_START_STAGGER_MS: value,
        }),
      ).toBe(1500);
    }
