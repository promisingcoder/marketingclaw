...
const QA_LIVE_ENV_ALIASES = Object.freeze([
  {
    liveVar: "MARKETINGCLAW_LIVE_OPENAI_KEY",
    providerVar: "OPENAI_API_KEY",
  },
  {
    liveVar: "MARKETINGCLAW_LIVE_ANTHROPIC_KEY",
    providerVar: "ANTHROPIC_API_KEY",
  },
  {
    liveVar: "MARKETINGCLAW_LIVE_GEMINI_KEY",
    providerVar: "GEMINI_API_KEY",
  },
]);

export const QA_LIVE_PROVIDER_CONFIG_PATH_ENV = "MARKETINGCLAW_QA_LIVE_PROVIDER_CONFIG_PATH";
const QA_LIVE_CLI_BACKEND_PRESERVE_ENV = "MARKETINGCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV";
const QA_LIVE_CLI_BACKEND_AUTH_MODE_ENV = "MARKETINGCLAW_LIVE_CLI_BACKEND_AUTH_MODE";
...
export const QA_PROVIDER_SECRET_ENV_KEY_PATTERNS = Object.freeze([
  /^MARKETINGCLAW_LIVE_[A-Z0-9_]+_KEYS?$/u,
]);
...
const QA_LIVE_ALLOWED_ENV_PATTERNS = Object.freeze([
  ...
  /^MARKETINGCLAW_LIVE_[A-Z0-9_]+_KEY$/u,
  /^MARKETINGCLAW_LIVE_[A-Z0-9_]+_KEYS$/u,
]);
...
export function resolveQaLiveCliAuthEnv(
  ...
  const hasAnthropicKey = Boolean(
    baseEnv.ANTHROPIC_API_KEY?.trim() || baseEnv.MARKETINGCLAW_LIVE_ANTHROPIC_KEY?.trim(),
  );
  if (opts?.forwardHostHomeForClaudeCli && authMode === "api-key" && !hasAnthropicKey) {
    throw new Error(
      "Claude CLI API-key QA mode requires ANTHROPIC_API_KEY or MARKETINGCLAW_LIVE_ANTHROPIC_KEY",
    );
  }
...
export function resolveQaLiveProviderConfigPath(env: NodeJS.ProcessEnv = process.env) {
  const explicit =
    env[QA_LIVE_PROVIDER_CONFIG_PATH_ENV]?.trim() || env.MARKETINGCLAW_CONFIG_PATH?.trim();
  return explicit
    ? { path: resolveUserPath(explicit, env), explicit: true }
    : { path: path.join(os.homedir(), ".marketingclaw", "marketingclaw.json"), explicit: false };
}
