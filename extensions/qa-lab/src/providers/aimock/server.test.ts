    ...
    try {
      const response = await fetch(`${server.baseUrl}/v1/responses`, {
        ...
        body: JSON.stringify({
          model: "aimock/gpt-5.5",
          stream: false,
          input: [makeResponsesInput("@marketingclaw explain the QA lab")],
        }),
      });
      expect(response.status).toBe(200);
      ...
      const expectedBody = {
        model: "aimock/gpt-5.5",
        messages: [{ role: "user", content: "@marketingclaw explain the QA lab" }],
        stream: false,
        _endpointType: "chat",
      };
      expect(await debug.json()).toEqual([
        {
          raw: JSON.stringify(expectedBody),
          body: expectedBody,
          prompt: "@marketingclaw explain the QA lab",
          allInputText: "@marketingclaw explain the QA lab",
          toolOutput: "",
          model: "aimock/gpt-5.5",
          providerVariant: "openai",
          imageInputCount: 0,
