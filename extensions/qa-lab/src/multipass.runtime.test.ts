...
describe("qa multipass runtime", () => {
  ...
  it("rejects repo-local symlink output directories that escape the repo root", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-multipass-"));
  ...
  it("reuses suite scenario semantics and resolves mounted artifact paths", () => {
    ...
    expect(plan.guestOutputDir).toBe(
      "/workspace/marketingclaw-host/.artifacts/qa-e2e/multipass-test",
    );
  ...
  it("renders a guest script that runs the live qa suite by default", () => {
    ...
    expect(script).toContain("'pnpm' 'marketingclaw' 'qa' 'suite' '--transport' 'qa-channel'");
    ...
    expect(script).toContain("/workspace/marketingclaw-host/.artifacts/qa-e2e/multipass-test");
  });
  ...
  it("forwards --runtime-pair into the guest qa suite command when requested", () => {
    ...
    expect(plan.qaCommand).toEqual(
      expect.arrayContaining(["--runtime-pair", "marketingclaw,codex"]),
    );
  });
  ...
  it("forwards live key list and numbered key env shapes", () => {
    vi.stubEnv("MARKETINGCLAW_LIVE_ANTHROPIC_KEYS", "anthropic-a anthropic-b");
    vi.stubEnv("MARKETINGCLAW_LIVE_CODEX_API_KEY", "codex-live");
    ...
    expect(plan.forwardedEnv.MARKETINGCLAW_LIVE_ANTHROPIC_KEYS).toBe("anthropic-a anthropic-b");
    expect(plan.forwardedEnv.MARKETINGCLAW_LIVE_CODEX_API_KEY).toBe("codex-live");
  ...
  it("skips stale CODEX_HOME values that do not exist on the host", () => {
    vi.stubEnv("CODEX_HOME", "/tmp/does-not-exist-marketingclaw-codex-home");
    const plan = createQaMultipassPlan({
      repoRoot: process.cwd(),
      outputDir: path.join(process.cwd(), ".artifacts", "qa-e2e", "multipass-live-test"),
      providerMode: "live-frontier",
  ...
  it("falls back to os.homedir() when HOME is unset for CODEX_HOME discovery", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-multipass-home-"));
    ...
    try {
      ...
      expect(plan.guestCodexHomePath).toBe("/workspace/marketingclaw-codex-home");
    } finally {
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("does not leave a temp guest transfer script behind when multipass is missing", async () => {
    ...
    const tempEntries = fs
      .readdirSync(resolvePreferredMarketingClawTmpDir())
