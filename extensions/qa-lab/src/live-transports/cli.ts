...
import { listQaRunnerCliContributions } from "marketingclaw/plugin-sdk/qa-runner-runtime";
  ...
  return {
    ...
    register(qa) {
      qa.command(params.commandName)
        ...
        .action(() => {
          throw new Error(
            `QA runner "${params.commandName}" is installed but not active. Enable or allow plugin "${params.pluginId}" in your MarketingClaw config, then try again.`,
          );
        });
    },
  };
