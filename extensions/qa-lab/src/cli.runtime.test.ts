...
describe("qa cli runtime", () => {
  ...
  beforeEach(async () => {
    ...
    writeQaDockerHarnessFiles.mockResolvedValue({
      outputDir: "/tmp/marketingclaw-repo/.artifacts/qa-docker",
    });
    buildQaDockerHarnessImage.mockResolvedValue({
      imageName: "marketingclaw:qa-local-prebaked",
    });
  ...
  it("dispatches a taxonomy-backed profile category through the suite runner", async () => {
    const previousProfile = process.env.MARKETINGCLAW_QA_PROFILE;
    process.env.MARKETINGCLAW_QA_PROFILE = "release";
    try {
      runQaSuite.mockImplementationOnce(async () => {
        expect(process.env.MARKETINGCLAW_QA_PROFILE).toBe("smoke-ci");
        await fs.writeFile(
          suiteEvidencePath,
          JSON.stringify(
            makeQaEvidence([
      ...
      expectFields(suiteArgs, {
        repoRoot: path.resolve("/tmp/marketingclaw-repo"),
        outputDir: path.resolve("/tmp/marketingclaw-repo", ".artifacts/qa-e2e/smoke-ci"),
        transportId: "qa-channel",
      ...
      expect(process.env.MARKETINGCLAW_QA_PROFILE).toBe("release");
      ...
      if (previousProfile === undefined) {
        delete process.env.MARKETINGCLAW_QA_PROFILE;
      } else {
        process.env.MARKETINGCLAW_QA_PROFILE = previousProfile;
      }
    }
  });
  ...
  it("resolves suite repo-root-relative paths before dispatching", async () => {
    ...
    expect(runQaSuite).toHaveBeenCalledWith({
      repoRoot: path.resolve("/tmp/marketingclaw-repo"),
      outputDir: path.resolve("/tmp/marketingclaw-repo", ".artifacts/qa/frontier"),
      transportId: "qa-channel",
      channelDriver: undefined,
      channelDriverSelection: undefined,
      providerMode: "live-frontier",
  ...
  it("uses the Crabline default channel when selected scenarios do not request one", async () => {
    ...
    expect(runQaSuite).toHaveBeenCalledWith({
      repoRoot: path.resolve("/tmp/marketingclaw-repo"),
      outputDir: path.resolve("/tmp/marketingclaw-repo", ".artifacts/qa/multipass-telegram"),
      transportId: "qa-channel",
      channelDriver: "crabline",
  ...
  it("rejects unknown runtime-pair ids at the CLI boundary", async () => {
    ...
    ).rejects.toThrow('--runtime-pair only supports "marketingclaw" and "codex".');
    expect(runQaSuite).not.toHaveBeenCalled();
  });
    ...
    expect(runCanonicalLiveScenarios).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        options: expect.objectContaining({
          repoRoot: path.resolve("/tmp/marketingclaw-repo"),
          outputDir: path.resolve("/tmp/marketingclaw-repo", ".artifacts/qa/telegram"),
          providerMode: "live-frontier",
          primaryModel: "openai/gpt-5.5",
          alternateModel: "openai/gpt-5.5",
          fastMode: true,
  ...
  it("rejects output dirs that escape the repo root", () => {
    expect(() => resolveRepoRelativeOutputDir("/tmp/marketingclaw-repo", "../outside")).toThrow(
      "--output-dir must stay within the repo root.",
    );
    expect(() => resolveRepoRelativeOutputDir("/tmp/marketingclaw-repo", "/tmp/outside")).toThrow(
  ...
  it("runs a host-only parity preflight against the sentinel scenario", async () => {
    const repoRoot = path.resolve("/tmp/marketingclaw-repo");
    await runQaSuiteCommand({
      repoRoot: "/tmp/marketingclaw-repo",
      providerMode: "mock-openai",
      primaryModel: "openai/gpt-5.5",
  ...
  it("prints a markdown tool coverage report from runtime tool fixtures", async () => {
    ...
    expectWriteContains(stdoutWrite, "# MarketingClaw Runtime Tool Coverage");
  ...
  it("writes a curated mock JSONL replay report and summary", async () => {
    ...
    try {
      ...
      expect(report).toContain("# MarketingClaw JSONL Replay Report - marketingclaw vs codex");
    ...
    try {
      await fs.writeFile(
        ...
        JSON.stringify({
          ...
          run: { runtimePair: ["marketingclaw", "codex"] },
        }),
        "utf8",
      );
  ...
  it("resolves character eval paths and passes model refs through", async () => {
    ...
    expectFields(characterEvalArgs, {
      repoRoot: path.resolve("/tmp/marketingclaw-repo"),
      outputDir: path.resolve("/tmp/marketingclaw-repo", ".artifacts/qa/character"),
  ...
  it("routes suite runs through multipass when the runner is selected", async () => {
    ...
    expect(runQaMultipass).toHaveBeenCalledWith({
      repoRoot: path.resolve("/tmp/marketingclaw-repo"),
      outputDir: path.resolve("/tmp/marketingclaw-repo", ".artifacts/qa-multipass"),
      transportId: "qa-channel",
      providerMode: "mock-openai",
      primaryModel: undefined,
      alternateModel: undefined,
  ...
  it("passes runtime-pair suite selection through to the multipass runner", async () => {
    await runQaSuiteCommand({
      ...
      providerMode: "mock-openai",
      scenarioIds: ["approval-turn-tool-followthrough"],
      runtimePair: "codex,marketingclaw",
      allowFailures: true,
    });
  ...
  it("resolves self-check repo-root-relative paths before starting the lab server", async () => {
    ...
    expect(startQaLabServer).toHaveBeenCalledWith({
      repoRoot: path.resolve("/tmp/marketingclaw-repo"),
      outputPath: path.resolve("/tmp/marketingclaw-repo", ".artifacts/qa/self-check.md"),
    });
  });
  ...
  it("rejects oversized credential payload files before broker setup", async () => {
    const previousMaxBytes = process.env.MARKETINGCLAW_QA_CREDENTIAL_PAYLOAD_MAX_BYTES;
    ...
    process.env.MARKETINGCLAW_QA_CREDENTIAL_PAYLOAD_MAX_BYTES = "32";
    ...
    try {
      await expect(
        ...
        "Payload file exceeds MARKETINGCLAW_QA_CREDENTIAL_PAYLOAD_MAX_BYTES (32 bytes).",
      );
    } finally {
      if (previousMaxBytes === undefined) {
        delete process.env.MARKETINGCLAW_QA_CREDENTIAL_PAYLOAD_MAX_BYTES;
      } else {
        process.env.MARKETINGCLAW_QA_CREDENTIAL_PAYLOAD_MAX_BYTES = previousMaxBytes;
      }
    }
  });
  ...
  it("resolves docker scaffold paths relative to the explicit repo root", async () => {
    ...
    expect(writeQaDockerHarnessFiles).toHaveBeenCalledWith({
      outputDir: path.resolve("/tmp/marketingclaw-repo", ".artifacts/qa-docker"),
      repoRoot: path.resolve("/tmp/marketingclaw-repo"),
  ...
  it("passes the explicit repo root into docker image builds", async () => {
    await runQaDockerBuildImageCommand({
      repoRoot: "/tmp/marketingclaw-repo",
      image: "marketingclaw:qa-local-prebaked",
    });
  ...
  it("resolves docker up paths relative to the explicit repo root", async () => {
    ...
    expect(runQaDockerUp).toHaveBeenCalledWith({
      repoRoot: path.resolve("/tmp/marketingclaw-repo"),
      outputDir: path.resolve("/tmp/marketingclaw-repo", ".artifacts/qa-up"),
      gatewayPort: undefined,
      qaLabPort: undefined,
