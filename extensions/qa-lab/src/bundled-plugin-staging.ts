...
function findQaBundledPluginDirsByManifestId(params: {
  ...
  for (const sourceRoot of resolveQaBundledPluginScanRoots(params.repoRoot)) {
    for (const entry of readdirSync(sourceRoot, { withFileTypes: true }).toSorted((left, right) =>
      ...
      const manifestId = readQaBundledManifestId(path.join(candidate, "marketingclaw.plugin.json"));
...
export async function resolveQaOwnerPluginIdsForProviderIds(params: {
  ...
  for (const sourceRoot of resolveQaBundledPluginScanRoots(params.repoRoot)) {
    for (const entry of await fs.readdir(sourceRoot, { withFileTypes: true })) {
      ...
      const manifestPath = path.join(sourceRoot, entry.name, "marketingclaw.plugin.json");
...
export async function resolveQaRuntimeHostVersion(params: {
  ...
  for (const pluginId of stagedPluginIds) {
    ...
    const candidate = parseStableSemverFloor(packageJson.marketingclaw?.install?.minHostVersion);
...
export async function createQaBundledPluginsDir(params: {
  ...
  const stagedMarketingClawPackageDir = path.join(stagedRoot, "node_modules", "marketingclaw");
  await fs.mkdir(stagedMarketingClawPackageDir, { recursive: true });
  await fs.copyFile(
    path.join(params.repoRoot, "package.json"),
    path.join(stagedMarketingClawPackageDir, "package.json"),
  );
  ...
  await symlinkQaStagedDirEntry({
    ...
    targetPath: path.join(stagedMarketingClawPackageDir, "dist"),
    directory: true,
  });
  return {
    bundledPluginsDir,
