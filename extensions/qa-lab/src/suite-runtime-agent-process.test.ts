...
describe("qa suite runtime agent process helpers", () => {
  ...
  it.runIf(process.platform !== "win32")("kills timed-out qa cli process groups", async () => {
    ...
    try {
      ...
      const timeoutAssertion = expect(pending).rejects.toThrow(
        "qa cli timed out: marketingclaw qa suite",
      );
  ...
  it("merges isolated env overrides into qa cli runs", async () => {
    ...
    const pending = runQaCli(
      {
        ...
        gateway: {
          tempRoot: "/tmp/runtime",
          runtimeEnv: { PATH: "/usr/bin", MARKETINGCLAW_STATE_DIR: "/tmp/default-state" },
        },
        primaryModel: "openai/gpt-5.5",
        alternateModel: "openai/gpt-5.5-mini",
        providerMode: "mock-openai",
      ...
      {
        env: {
          MARKETINGCLAW_STATE_DIR: "/tmp/isolated-state",
          MARKETINGCLAW_CONFIG_PATH: "/tmp/isolated-state/marketingclaw.json",
        },
      },
    );
    ...
    expect(spawnEnv?.MARKETINGCLAW_STATE_DIR).toBe("/tmp/isolated-state");
    expect(spawnEnv?.MARKETINGCLAW_CONFIG_PATH).toBe("/tmp/isolated-state/marketingclaw.json");
  });
  ...
  it("finds managed dreaming cron jobs across legacy and current payload contracts", () => {
    ...
    const current = {
      ...
      payload: {
        kind: "agentTurn",
        message: "__marketingclaw_memory_core_short_term_promotion_dream__",
        lightContext: true,
      },
