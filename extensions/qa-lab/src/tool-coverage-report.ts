...
export type QaToolCoverageRow = {
  ...
  scenarios: string[];
  sourcePaths: string[];
  marketingclaw: QaToolCoverageStatus;
  codex: QaToolCoverageStatus;
  drift: QaToolCoverageDrift;
  marketingclawToolCalls: number;
...
function countRuntimeToolCalls(
  ...
  const cell = runtime === "marketingclaw" ? result.cells.marketingclaw : result.cells.codex;
...
function buildRow(params: {
  ...
  return {
    ...
    marketingclaw: result ? cellStatus(result.cells.marketingclaw) : "not-run",
    codex: result ? cellStatus(result.cells.codex) : "not-run",
    drift: result?.drift ?? "not-run",
    marketingclawToolCalls: countRuntimeToolCalls(result, "marketingclaw", runtimeToolName),
...
function coverageFailureForRow(row: QaToolCoverageRow): string | undefined {
  ...
  if (row.marketingclaw !== "pass" || row.codex !== "pass") {
    return `${row.tool} status marketingclaw=${row.marketingclaw} codex=${row.codex}`;
  }
  if (row.drift === "failure-mode") {
    return `${row.tool} drift=failure-mode${row.details ? ` (${row.details})` : ""}`;
  }
  if (row.runtimeToolName && row.marketingclawToolCalls === 0) {
    return `${row.tool} missing marketingclaw tool call ${row.runtimeToolName}`;
  }
...
export function buildQaToolCoverageReport(params: {
  ...
  return {
    ...
    dynamicIntegrationTools: rows.filter(
      (row) => row.bucket === "marketingclaw-dynamic-integration",
    ).length,
    searchableDynamicTools: rows.filter(
      (row) => row.capabilityLayer === "marketingclaw-dynamic-searchable",
    ).length,
...
export function renderQaToolCoverageMarkdownReport(report: QaToolCoverageReport): string {
  const lines = [
    `# MarketingClaw Runtime Tool Coverage — ${report.runtimePair[0]} vs ${report.runtimePair[1]}`,
    "",
    ...
    `- MarketingClaw dynamic integration tools: ${report.dynamicIntegrationTools}`,
    ...
    "",
    "| Tool | Bucket | Expected layer | Capability layer | Required | Fixtures | MarketingClaw | Codex | Drift | Codex default impact | QA impact | Action | Tracking |",
    "| --- | --- | --- | --- | --- | ---: | --- | --- | --- | --- | --- | --- | --- |",
  ];
