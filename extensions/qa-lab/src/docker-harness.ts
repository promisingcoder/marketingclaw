...
const QA_LAB_UI_OVERLAY_DIR = "/opt/marketingclaw-qa-lab-ui";
...
function renderImageBlock(params: {
  ...
  return `    build:\n      context: ${yamlDoubleQuoted(context)}\n      dockerfile: Dockerfile\n      args:\n        MARKETINGCLAW_EXTENSIONS: "qa-channel qa-lab"\n`;
}
...
${imageBlock}    pull_policy: never
    ...
    volumes:
      - ./state:/opt/marketingclaw-scaffold:ro
${params.bindUiDist ? `      - ${yamlDoubleQuoted(`${qaLabUiMount}:${QA_LAB_UI_OVERLAY_DIR}:ro`)}\n` : ""}    healthcheck:
    ...
    command:
      - sh
      - -lc
      - MARKETINGCLAW_QA_CONTROL_UI_PROXY_TOKEN="$(node -e 'const fs=require("node:fs");const cfg=JSON.parse(fs.readFileSync("/opt/marketingclaw-scaffold/marketingclaw.json","utf8"));process.stdout.write(cfg.gateway?.auth?.token ?? "")')" exec node dist/index.js qa ui --host 0.0.0.0 --port ${QA_LAB_INTERNAL_PORT} --advertise-host 127.0.0.1 --advertise-port ${params.qaLabPort} --control-ui-url http://127.0.0.1:${params.gatewayPort}/ --control-ui-proxy-target http://marketingclaw-qa-gateway:18789/${params.bindUiDist ? ` --ui-dist-dir ${QA_LAB_UI_OVERLAY_DIR}` : ""} --auto-kickoff-target direct --send-kickoff-on-start --embedded-gateway disabled
    depends_on:
      qa-mock-openai:
        condition: service_healthy
`
    : ""
}  marketingclaw-qa-gateway:
${imageBlock}    pull_policy: never
    extra_hosts:
    ...
    environment:
      MARKETINGCLAW_CONFIG_PATH: /tmp/marketingclaw/marketingclaw.json
      MARKETINGCLAW_STATE_DIR: /tmp/marketingclaw/state
    ...
    volumes:
      - ./state:/opt/marketingclaw-scaffold:ro
      - ${yamlDoubleQuoted(`${repoMount}:/opt/marketingclaw-repo:ro`)}
    healthcheck:
      test:
        - CMD
        - node
...
`
    ...
    command:
      - sh
      - -lc
      - mkdir -p /tmp/marketingclaw/workspace /tmp/marketingclaw/state && cp /opt/marketingclaw-scaffold/marketingclaw.json /tmp/marketingclaw/marketingclaw.json && cp -R /opt/marketingclaw-scaffold/seed-workspace/. /tmp/marketingclaw/workspace/ && rm -rf /tmp/marketingclaw/workspace/repo && ln -s /opt/marketingclaw-repo /tmp/marketingclaw/workspace/repo && exec node dist/index.js gateway run --port 18789 --bind lan --allow-unconfigured
`;
}
...
- \`.env.example\`
- \`state/marketingclaw.json\`

Suggested flow:

1. Build the prebaked image once:
   - \`docker build -t marketingclaw:qa-local-prebaked --build-arg MARKETINGCLAW_EXTENSIONS="qa-channel qa-lab" -f Dockerfile .\`
2. Start the stack:
...
export async function writeQaDockerHarnessFiles(params: {
  ...
  const imageName = params.imageName ?? "marketingclaw:qa-local-prebaked";
  ...
  const files = [
    ...
    path.join(params.outputDir, "state", "marketingclaw.json"),
  ];
