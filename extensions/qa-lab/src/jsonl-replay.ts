...
export type JsonlReplayResult = {
  transcripts: Array<{
    ...
    cells: { marketingclaw: RuntimeParityCell[]; codex: RuntimeParityCell[] };
...
export async function runJsonlReplay(
  ...
  for (const transcriptPath of transcriptPaths) {
    ...
    const cells: { marketingclaw: RuntimeParityCell[]; codex: RuntimeParityCell[] } = {
      marketingclaw: [],
      codex: [],
    };
    ...
    for (const turn of turns) {
      ...
      cells.marketingclaw.push(parity.cells.marketingclaw);
...
export function renderJsonlReplayMarkdownReport(report: JsonlReplayMarkdownReport): string {
  ...
  const lines = [
    `# MarketingClaw JSONL Replay Report - ${report.runtimePair[0]} vs ${report.runtimePair[1]}`,
    "",
