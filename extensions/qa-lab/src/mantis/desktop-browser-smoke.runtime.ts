...
const DEFAULT_BROWSER_URL = "https://marketingclaw.ai";
...
const BROWSER_PROFILE_ARCHIVE_ENV = "MARKETINGCLAW_MANTIS_BROWSER_PROFILE_TGZ_B64";
const BROWSER_PROFILE_DIR_ENV = "MARKETINGCLAW_MANTIS_BROWSER_PROFILE_DIR";
...
if [ -n "$profile_archive_b64" ]; then
  profile_archive="$profile/marketingclaw-mantis-browser-profile.tgz"
...
export async function runMantisDesktopBrowserSmoke(
  ...
  const browserProfileArchiveEnv =
    ...
    trimToValue(env.MARKETINGCLAW_MANTIS_BROWSER_PROFILE_ARCHIVE_ENV) ??
  ...
  const remoteOutputDir = `/tmp/marketingclaw-mantis-desktop-${startedAt
    .toISOString()
