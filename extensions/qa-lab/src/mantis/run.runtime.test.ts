...
describe("mantis before/after runtime", () => {
  ...
  it("runs baseline and candidate worktrees and writes stable comparison artifacts", async () => {
    ...
    const runner = vi.fn(async (command: string, args: readonly string[]) => {
      ...
      if (command !== "pnpm" || !args.includes("marketingclaw")) {
        return;
      }
    ...
    expect(commands[1]?.args.slice(2, 4)).toEqual(["marketingclaw", "qa"]);
    ...
    expect(commands[3]?.args.slice(2, 4)).toEqual(["marketingclaw", "qa"]);
