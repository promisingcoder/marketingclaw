...
const TELEGRAM_PROFILE_ARCHIVE_ENV = "MARKETINGCLAW_MANTIS_TELEGRAM_DESKTOP_PROFILE_TGZ_B64";
const TELEGRAM_PROFILE_ARCHIVE_ENV_NAME_ENV =
  "MARKETINGCLAW_MANTIS_TELEGRAM_DESKTOP_PROFILE_ARCHIVE_ENV";
const TELEGRAM_PROFILE_DIR_ENV = "MARKETINGCLAW_MANTIS_TELEGRAM_DESKTOP_PROFILE_DIR";
...
function buildCrabboxEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  if (!trimToValue(next.MARKETINGCLAW_MANTIS_TELEGRAM_GROUP_ID)) {
    next.MARKETINGCLAW_MANTIS_TELEGRAM_GROUP_ID = trimToValue(next.MARKETINGCLAW_QA_TELEGRAM_GROUP_ID);
  }
  if (!trimToValue(next.MARKETINGCLAW_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN)) {
    next.MARKETINGCLAW_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN = trimToValue(
      next.MARKETINGCLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN,
    );
  }
  if (!trimToValue(next.MARKETINGCLAW_MANTIS_TELEGRAM_SUT_BOT_TOKEN)) {
    next.MARKETINGCLAW_MANTIS_TELEGRAM_SUT_BOT_TOKEN = trimToValue(
      next.MARKETINGCLAW_QA_TELEGRAM_SUT_BOT_TOKEN,
    );
  }
  return next;
}
...
function resolveTelegramGatewayEnvPayload(
  ...
  const groupId = trimToValue(env.MARKETINGCLAW_QA_TELEGRAM_GROUP_ID);
  const driverToken = trimToValue(env.MARKETINGCLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN);
  const sutToken = trimToValue(env.MARKETINGCLAW_QA_TELEGRAM_SUT_BOT_TOKEN);
  if (!groupId || !driverToken || !sutToken) {
    throw new Error(
      "Telegram desktop builder requires MARKETINGCLAW_QA_TELEGRAM_GROUP_ID, MARKETINGCLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN, and MARKETINGCLAW_QA_TELEGRAM_SUT_BOT_TOKEN when using --credential-source env.",
    );
  }
...
async function prepareGatewayCredentialEnv(params: {
  ...
  if (
    trimToValue(params.env.MARKETINGCLAW_MANTIS_TELEGRAM_GROUP_ID) &&
    trimToValue(params.env.MARKETINGCLAW_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN) &&
    trimToValue(params.env.MARKETINGCLAW_MANTIS_TELEGRAM_SUT_BOT_TOKEN)
  ) {
    return {};
  }
  ...
  params.env.MARKETINGCLAW_MANTIS_TELEGRAM_GROUP_ID = payload.groupId;
  params.env.MARKETINGCLAW_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN = payload.driverToken;
  params.env.MARKETINGCLAW_MANTIS_TELEGRAM_SUT_BOT_TOKEN = payload.sutToken;
  params.env.MARKETINGCLAW_QA_TELEGRAM_GROUP_ID =
    trimToValue(params.env.MARKETINGCLAW_QA_TELEGRAM_GROUP_ID) ?? payload.groupId;
  params.env.MARKETINGCLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN =
    trimToValue(params.env.MARKETINGCLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN) ?? payload.driverToken;
  params.env.MARKETINGCLAW_QA_TELEGRAM_SUT_BOT_TOKEN =
    trimToValue(params.env.MARKETINGCLAW_QA_TELEGRAM_SUT_BOT_TOKEN) ?? payload.sutToken;
  return {
    credentialLease,
    leaseHeartbeat,
  };
...
telegram_root="$HOME/.local/share/marketingclaw-mantis/telegram-desktop-bin"
...
if [ -n "\${MARKETINGCLAW_MANTIS_TELEGRAM_DESKTOP_PROFILE_TGZ_B64:-}" ]; then
  printf '%s' "$MARKETINGCLAW_MANTIS_TELEGRAM_DESKTOP_PROFILE_TGZ_B64" | base64 -d >"$out/telegram-profile.tgz"
...
qa_status=0
{
  ...
  if [ "$setup_gateway" = "1" ]; then
    export TELEGRAM_BOT_TOKEN="\${MARKETINGCLAW_MANTIS_TELEGRAM_SUT_BOT_TOKEN:-\${TELEGRAM_BOT_TOKEN:-}}"
    telegram_group_id="\${MARKETINGCLAW_MANTIS_TELEGRAM_GROUP_ID:-}"
    driver_token="\${MARKETINGCLAW_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN:-}"
    if [ -z "$TELEGRAM_BOT_TOKEN" ] || [ -z "$telegram_group_id" ] || [ -z "$driver_token" ]; then
      echo "Gateway setup requires MARKETINGCLAW_MANTIS_TELEGRAM_GROUP_ID, MARKETINGCLAW_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN, and MARKETINGCLAW_MANTIS_TELEGRAM_SUT_BOT_TOKEN." >&2
      exit 2
    fi
...
const token = process.env.MARKETINGCLAW_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN;
...
node --input-type=module -e 'import fs from "node:fs"; const value = JSON.parse(fs.readFileSync(process.argv[1], "utf8")); process.stdout.write(String(value.id || ""));' "$out/telegram-driver-getme.json")"
    export MARKETINGCLAW_HOME="$HOME/.marketingclaw-mantis/telegram-marketingclaw"
...
MANTIS_TELEGRAM_PATCH
    pnpm marketingclaw config patch --file "$out/telegram.patch.json5" --dry-run
    pnpm marketingclaw config patch --file "$out/telegram.patch.json5"
    node --input-type=module >"$out/telegram-ready-message.json" 2>"$out/telegram-ready-message.err" <<'MANTIS_TELEGRAM_READY'
const token = process.env.MARKETINGCLAW_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN;
const chatId = process.env.MARKETINGCLAW_MANTIS_TELEGRAM_GROUP_ID;
...
MANTIS_TELEGRAM_READY
    nohup pnpm marketingclaw gateway run --dev --allow-unconfigured --port 38974 --cli-backend-logs </dev/null >"$out/marketingclaw-gateway.log" 2>&1 &
    gateway_pid="$!"
...
function renderReport(summary: MantisTelegramDesktopBuilderSummary) {
  const lines = [
    ...
    "- MarketingClaw gateway log: `marketingclaw-gateway.log`",
...
export async function runMantisTelegramDesktopBuilder(
  ...
  const remoteOutputDir = `/tmp/marketingclaw-mantis-telegram-desktop-${startedAt
    .toISOString()
  ...
  try {
    ...
    if (gatewaySetup && !gatewaySetupCompleted) {
      throw new Error("Telegram desktop builder did not report a live MarketingClaw gateway.");
    }
    summary = {
      artifacts: {
        reportPath,
