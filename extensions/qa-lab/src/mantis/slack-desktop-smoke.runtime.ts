...
const CRABBOX_BIN_ENV = "MARKETINGCLAW_MANTIS_CRABBOX_BIN";
const CRABBOX_PROVIDER_ENV = "MARKETINGCLAW_MANTIS_CRABBOX_PROVIDER";
const CRABBOX_CLASS_ENV = "MARKETINGCLAW_MANTIS_CRABBOX_CLASS";
const CRABBOX_MARKET_ENV = "MARKETINGCLAW_MANTIS_CRABBOX_MARKET";
const CRABBOX_LEASE_ID_ENV = "MARKETINGCLAW_MANTIS_CRABBOX_LEASE_ID";
const CRABBOX_KEEP_ENV = "MARKETINGCLAW_MANTIS_KEEP_VM";
const CRABBOX_IDLE_TIMEOUT_ENV = "MARKETINGCLAW_MANTIS_CRABBOX_IDLE_TIMEOUT";
const CRABBOX_TTL_ENV = "MARKETINGCLAW_MANTIS_CRABBOX_TTL";
const HYDRATE_MODE_ENV = "MARKETINGCLAW_MANTIS_HYDRATE_MODE";
const SLACK_URL_ENV = "MARKETINGCLAW_MANTIS_SLACK_URL";
const SLACK_CHANNEL_ID_ENV = "MARKETINGCLAW_MANTIS_SLACK_CHANNEL_ID";
...
function buildCrabboxEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  if (!trimToValue(next.MARKETINGCLAW_LIVE_OPENAI_KEY) && trimToValue(next.OPENAI_API_KEY)) {
    next.MARKETINGCLAW_LIVE_OPENAI_KEY = next.OPENAI_API_KEY;
  }
  if (!trimToValue(next.MARKETINGCLAW_MANTIS_SLACK_BOT_TOKEN) && trimToValue(next.SLACK_BOT_TOKEN)) {
    next.MARKETINGCLAW_MANTIS_SLACK_BOT_TOKEN = next.SLACK_BOT_TOKEN;
  }
  if (
    !trimToValue(next.MARKETINGCLAW_MANTIS_SLACK_BOT_TOKEN) &&
    trimToValue(next.MARKETINGCLAW_QA_SLACK_SUT_BOT_TOKEN)
  ) {
    next.MARKETINGCLAW_MANTIS_SLACK_BOT_TOKEN = next.MARKETINGCLAW_QA_SLACK_SUT_BOT_TOKEN;
  }
  if (!trimToValue(next.MARKETINGCLAW_MANTIS_SLACK_APP_TOKEN) && trimToValue(next.SLACK_APP_TOKEN)) {
    next.MARKETINGCLAW_MANTIS_SLACK_APP_TOKEN = next.SLACK_APP_TOKEN;
  }
  if (
    !trimToValue(next.MARKETINGCLAW_MANTIS_SLACK_APP_TOKEN) &&
    trimToValue(next.MARKETINGCLAW_QA_SLACK_SUT_APP_TOKEN)
  ) {
    next.MARKETINGCLAW_MANTIS_SLACK_APP_TOKEN = next.MARKETINGCLAW_QA_SLACK_SUT_APP_TOKEN;
  }
  if (
    !trimToValue(next.MARKETINGCLAW_MANTIS_SLACK_CHANNEL_ID) &&
    trimToValue(next.MARKETINGCLAW_QA_SLACK_CHANNEL_ID)
  ) {
    next.MARKETINGCLAW_MANTIS_SLACK_CHANNEL_ID = next.MARKETINGCLAW_QA_SLACK_CHANNEL_ID;
  }
  return next;
}

function resolveSlackGatewayEnvPayload(env: NodeJS.ProcessEnv): SlackGatewayCredentialPayload {
  const channelId = trimToValue(env.MARKETINGCLAW_QA_SLACK_CHANNEL_ID);
  const sutBotToken = trimToValue(env.MARKETINGCLAW_QA_SLACK_SUT_BOT_TOKEN);
  const sutAppToken = trimToValue(env.MARKETINGCLAW_QA_SLACK_SUT_APP_TOKEN);
  if (!channelId || !sutBotToken || !sutAppToken) {
    throw new Error(
      "Gateway setup requires MARKETINGCLAW_QA_SLACK_CHANNEL_ID, MARKETINGCLAW_QA_SLACK_SUT_BOT_TOKEN, and MARKETINGCLAW_QA_SLACK_SUT_APP_TOKEN when using --credential-source env.",
    );
  }
  return {
    channelId,
...
async function prepareGatewayCredentialEnv(params: {
  ...
  if (
    trimToValue(params.env.MARKETINGCLAW_MANTIS_SLACK_BOT_TOKEN) &&
    trimToValue(params.env.MARKETINGCLAW_MANTIS_SLACK_APP_TOKEN)
  ) {
    return {};
  }
  ...
  params.env.MARKETINGCLAW_MANTIS_SLACK_BOT_TOKEN = payload.sutBotToken;
  params.env.MARKETINGCLAW_MANTIS_SLACK_APP_TOKEN = payload.sutAppToken;
  params.env.MARKETINGCLAW_MANTIS_SLACK_CHANNEL_ID =
    trimToValue(params.env.MARKETINGCLAW_MANTIS_SLACK_CHANNEL_ID) ?? payload.channelId;
  params.env.MARKETINGCLAW_QA_SLACK_CHANNEL_ID =
    trimToValue(params.env.MARKETINGCLAW_QA_SLACK_CHANNEL_ID) ?? payload.channelId;
  params.env.MARKETINGCLAW_QA_SLACK_SUT_BOT_TOKEN =
    trimToValue(params.env.MARKETINGCLAW_QA_SLACK_SUT_BOT_TOKEN) ?? payload.sutBotToken;
  params.env.MARKETINGCLAW_QA_SLACK_SUT_APP_TOKEN =
    trimToValue(params.env.MARKETINGCLAW_QA_SLACK_SUT_APP_TOKEN) ?? payload.sutAppToken;
  return {
    credentialLease,
    leaseHeartbeat,
  };
...
remote_command_timeout_seconds="\${MARKETINGCLAW_MANTIS_REMOTE_COMMAND_TIMEOUT_SECONDS:-${remoteCommandTimeoutSeconds}}"
if [ -z "\${MARKETINGCLAW_QA_SLACK_CHANNEL_ID:-}" ] && [ -n "$slack_channel_id" ]; then
  export MARKETINGCLAW_QA_SLACK_CHANNEL_ID="$slack_channel_id"
fi
case "$remote_command_timeout_seconds" in
  ''|*[!0-9]*)
    echo "MARKETINGCLAW_MANTIS_REMOTE_COMMAND_TIMEOUT_SECONDS must be an integer number of seconds." >&2
    exit 2
    ;;
esac
if [ "$remote_command_timeout_seconds" -le 0 ]; then
  echo "MARKETINGCLAW_MANTIS_REMOTE_COMMAND_TIMEOUT_SECONDS must be greater than zero." >&2
  exit 2
fi
rm -rf "$out"
mkdir -p "$out"
export DISPLAY="\${DISPLAY:-:99}"
if [ -n "\${MARKETINGCLAW_LIVE_OPENAI_KEY:-}" ] && [ -z "\${OPENAI_API_KEY:-}" ]; then
  export OPENAI_API_KEY="$MARKETINGCLAW_LIVE_OPENAI_KEY"
fi
...
team_id="\${MARKETINGCLAW_QA_SLACK_TEAM_ID:-}"
auth_test_token="\${MARKETINGCLAW_QA_SLACK_SUT_BOT_TOKEN:-\${MARKETINGCLAW_MANTIS_SLACK_BOT_TOKEN:-}}"
...
const token = process.env.MARKETINGCLAW_QA_SLACK_SUT_BOT_TOKEN || process.env.MARKETINGCLAW_MANTIS_SLACK_BOT_TOKEN;
...
if [ -z "$slack_url" ] && [ -n "$team_id" ] && [ -n "\${MARKETINGCLAW_QA_SLACK_CHANNEL_ID:-}" ]; then
  slack_url="https://app.slack.com/client/$team_id/$MARKETINGCLAW_QA_SLACK_CHANNEL_ID"
fi
profile="\${MARKETINGCLAW_MANTIS_SLACK_BROWSER_PROFILE_DIR:-$HOME/.config/marketingclaw-mantis/slack-chrome-profile}"
mkdir -p "$profile"
if [ "$setup_gateway" = "1" ]; then
  export SLACK_BOT_TOKEN="\${MARKETINGCLAW_MANTIS_SLACK_BOT_TOKEN:-\${SLACK_BOT_TOKEN:-}}"
  export SLACK_APP_TOKEN="\${MARKETINGCLAW_MANTIS_SLACK_APP_TOKEN:-\${SLACK_APP_TOKEN:-}}"
  if [ -z "$SLACK_BOT_TOKEN" ] || [ -z "$SLACK_APP_TOKEN" ]; then
    echo "Gateway setup requires MARKETINGCLAW_MANTIS_SLACK_BOT_TOKEN and MARKETINGCLAW_MANTIS_SLACK_APP_TOKEN." >&2
    exit 2
  fi
...
run_mantis_remote_body() {
  ...
  if ! node_supports_type_stripping; then
    ...
    node_root="$HOME/.cache/marketingclaw-mantis/node-v$node_version-linux-$node_arch"
...
')
  ...
  if [ "$setup_gateway" = "1" ]; then
    export MARKETINGCLAW_HOME="$HOME/.marketingclaw-mantis/slack-marketingclaw"
    mkdir -p "$MARKETINGCLAW_HOME"
...
MANTIS_SLACK_PATCH
    pnpm marketingclaw config patch --file "$out/slack.socket.patch.json5" --dry-run
    pnpm marketingclaw config patch --file "$out/slack.socket.patch.json5"
    nohup pnpm marketingclaw gateway run --dev --allow-unconfigured --port 38973 --cli-backend-logs </dev/null >"$out/marketingclaw-gateway.log" 2>&1 &
    gateway_pid="$!"
    echo "$gateway_pid" >"$out/marketingclaw-gateway.pid"
    sleep 12
    if ! kill -0 "$gateway_pid" >/dev/null 2>&1; then
      echo "MarketingClaw gateway exited during startup." >&2
    ...
    qa_args=(marketingclaw qa slack --repo-root . --output-dir "$slack_qa_output_dir" --provider-mode "$provider_mode" --model "$primary_model" --alt-model "$alternate_model" --credential-source "$credential_source" --credential-role "$credential_role")
    if [ "$fast_mode" = "1" ]; then
      qa_args+=(--fast)
    fi
    if [ "$approval_checkpoints" = "1" ]; then
      ...
      export MARKETINGCLAW_QA_SLACK_APPROVAL_CHECKPOINT_DIR="$checkpoint_dir"
      export MARKETINGCLAW_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS="\${MARKETINGCLAW_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS:-${approvalCheckpointTimeoutMs}}"
      export MARKETINGCLAW_MANTIS_APPROVAL_CHECKPOINT_SCENARIOS_JSON="$approval_checkpoint_scenarios_json"
      export MARKETINGCLAW_MANTIS_APPROVAL_BROWSER_BIN="$browser_bin"
      cat >"$out/approval-checkpoint-watcher.mjs" <<'MANTIS_APPROVAL_WATCHER'
	import { spawn } from "node:child_process";
	import fs from "node:fs/promises";
	import path from "node:path";

const checkpointDir = process.env.MARKETINGCLAW_QA_SLACK_APPROVAL_CHECKPOINT_DIR;
const timeoutMs = Number.parseInt(
  process.env.MARKETINGCLAW_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS || "${approvalCheckpointTimeoutMs}",
  10,
);
	const scenarioIds = JSON.parse(
	  process.env.MARKETINGCLAW_MANTIS_APPROVAL_CHECKPOINT_SCENARIOS_JSON || "[]",
	);
	const browserBin = process.env.MARKETINGCLAW_MANTIS_APPROVAL_BROWSER_BIN;

if (!checkpointDir) {
  throw new Error("MARKETINGCLAW_QA_SLACK_APPROVAL_CHECKPOINT_DIR is required.");
}
if (!Number.isFinite(timeoutMs) || timeoutMs <= 0) {
  throw new Error("MARKETINGCLAW_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS must be a positive integer.");
}
  ...
  throw new Error(\`Timed out waiting for approval checkpoint: \${filePath}\`);
	...
	function renderCheckpointHtml(checkpoint) {
	  ...
	  return '<!doctype html><html><head><meta charset="utf-8">' +
	    ...
	    '<div class="meta"><span class="name">marketingclaw</span><span class="app">APP</span><span class="state">' +
	...
	async function captureScreenshot(screenshotPath, checkpoint) {
	  if (!browserBin) {
	    throw new Error("MARKETINGCLAW_MANTIS_APPROVAL_BROWSER_BIN is required to render approval checkpoint screenshots.");
	  }
...
cat >"$out/remote-metadata.json" <<MANTIS_REMOTE_METADATA
{
  ...
  "gatewayAlive": $(if [ "$setup_gateway" = "1" ] && [ -f "$out/marketingclaw-gateway.pid" ] && kill -0 "$(cat "$out/marketingclaw-gateway.pid")" >/dev/null 2>&1; then echo true; else echo false; fi),
  "gatewayPid": "$(if [ -f "$out/marketingclaw-gateway.pid" ]; then cat "$out/marketingclaw-gateway.pid"; fi)",
  "gatewayPort": 38973,
...
export async function runMantisSlackDesktopSmoke(
  ...
  const slackChannelId =
    ...
    trimToValue(env.MARKETINGCLAW_QA_SLACK_CHANNEL_ID) ??
  ...
  const remoteOutputDir = `/tmp/marketingclaw-mantis-slack-desktop-${startedAt
    .toISOString()
  ...
  try {
    ...
    if (gatewaySetup && !gatewaySetupCompleted) {
      throw new Error("Slack desktop gateway setup did not report a live MarketingClaw gateway.");
    }
