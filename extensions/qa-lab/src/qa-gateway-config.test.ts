  ...
  return {
    ...
    transportConfig: {
      ...
      messages: {
        ...
        groupChat: {
          mentionPatterns: ["\\b@?marketingclaw\\b"],
          visibleReplies: "automatic",
        },
      },
...
describe("buildQaGatewayConfig", () => {
  it("keeps mock-openai as the default provider lane", () => {
    ...
    expect(cfg.messages?.groupChat?.mentionPatterns).toEqual(["\\b@?marketingclaw\\b"]);
  ...
  it("pins control ui to a provided built root when available", () => {
    const cfg = buildQaGatewayConfig({
      ...
      gatewayToken: "token",
      workspaceDir: "/tmp/qa-workspace",
      controlUiRoot: "/tmp/marketingclaw/dist/control-ui",
      ...createQaChannelTransportParams(),
    });

    expect(cfg.gateway?.controlUi?.enabled).toBe(true);
    expect(cfg.gateway?.controlUi?.root).toBe("/tmp/marketingclaw/dist/control-ui");
  });
