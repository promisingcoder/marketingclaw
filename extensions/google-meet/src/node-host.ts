...
function startChrome(params: Record<string, unknown>) {
  ...
  if (mode === "agent" || mode === "bidi" || mode === "realtime") {
    ...
    if (bridgeCommand) {
      if (mode === "agent") {
        throw new Error(
          "Chrome agent mode requires audioInputCommand and audioOutputCommand so MarketingClaw can run STT and regular TTS directly.",
        );
      }
    ...
    browser:
      params.launch !== false
        ? {
            ...
            browserUrl: url,
            notes: [
              "Browser page control is handled by MarketingClaw browser automation when using chrome-node.",
            ],
          }
        : undefined,
  };
