...
export const CREATE_MEET_FROM_BROWSER_SCRIPT = `async () => {
  ...
  if (!current().startsWith("https://meet.google.com/")) {
    return {
      ...
      manualAction: "Sign in to Google in the MarketingClaw browser profile, then retry meeting creation.",
      browserUrl: current(),
  ...
  if (/do you want people to hear you in the meeting/i.test(pageText)) {
    return {
      ...
      manualAction: "Meet is showing the microphone choice. Click Use microphone in the MarketingClaw browser profile, then retry meeting creation.",
      browserUrl: href,
      browserTitle: document.title,
      notes,
    };
  ...
  if (/allow.*(microphone|camera)|blocked.*(microphone|camera)|permission.*(microphone|camera)/i.test(pageText)) {
    return {
      ...
      manualAction: "Allow microphone/camera permissions for Meet in the MarketingClaw browser profile, then retry meeting creation.",
      browserUrl: href,
      browserTitle: document.title,
      notes,
    };
  ...
  if (/couldn't create|unable to create/i.test(pageText)) {
    return {
      manualAction: "Resolve the Google Meet page prompt in the MarketingClaw browser profile, then retry meeting creation.",
      browserUrl: href,
      browserTitle: document.title,
      notes,
    };
