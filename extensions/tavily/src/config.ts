...
import { resolvePositiveTimeoutSeconds } from "marketingclaw/plugin-sdk/provider-web-search";
import {
...
export function resolveTavilySearchConfig(cfg?: MarketingClawConfig): TavilySearchConfig {
...
export function resolveTavilyApiKey(cfg?: MarketingClawConfig): string | undefined {
...
export function resolveTavilyBaseUrl(cfg?: MarketingClawConfig): string {
  const search = resolveTavilySearchConfig(cfg);
  const configured =
