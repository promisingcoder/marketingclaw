...
import type { MarketingClawConfig, PluginRuntime } from "../api.js";
...
function createRuntime(): { runtime: PluginRuntime; mocks: LineRuntimeMocks } {
  ...
  const resolveLineAccount = vi.fn(
    ({ cfg, accountId }: { cfg: MarketingClawConfig; accountId?: string }) => {
...
describe("line outbound sendPayload", () => {
  ...
  it("uses configured text chunk limit for payloads", async () => {
    const { runtime, mocks } = createRuntime();
    setLineRuntime(runtime);
    const cfg = { channels: { line: { textChunkLimit: 123 } } } as MarketingClawConfig;
