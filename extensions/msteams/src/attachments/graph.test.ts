...
describe("downloadMSTeamsGraphMedia hosted content $value fallback", () => {
  ...
  it("adds the MarketingClaw User-Agent to guarded Graph attachment fetches", async () => {
    ...
    for (const [call] of guardCalls) {
      ...
      expect((headers as Headers).get("User-Agent")).toMatch(
        /^teams\.ts\[apps\]\/.+ MarketingClaw\/.+$/,
      );
    }
  });

  it("adds the MarketingClaw User-Agent to Graph shares downloads for reference attachments", async () => {
    mockGraphMediaFetch({
      messageId: "msg-share",
      messageResponse: {
        body: {},
    ...
    expect(headers.get("User-Agent")).toMatch(/^teams\.ts\[apps\]\/.+ MarketingClaw\/.+$/);
  });
});
