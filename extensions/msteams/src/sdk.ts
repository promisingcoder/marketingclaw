...
import { buildMarketingClawUserAgentFragment } from "./user-agent.js";
...
type MSTeamsAppOn = {
  ...
  // delegates back into the SDK handlers after MarketingClaw sender policy passes.
...
export async function createMSTeamsApp(
  ...
  // backend can identify MarketingClaw traffic for usage telemetry. Teams SDK
  ...
  const appOptions: Record<string, unknown> = {
    client: options?.httpClient ?? {
      headers: { "User-Agent": buildMarketingClawUserAgentFragment() },
    },
