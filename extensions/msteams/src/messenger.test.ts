...
import { SILENT_REPLY_TOKEN } from "marketingclaw/plugin-sdk/reply-chunking";
  ...
  describe("sendMSTeamsMessages", () => {
    ...
    it("preserves parsed mentions when appending OneDrive fallback file links", async () => {
      const tmpDir = await mkdtemp(
        path.join(resolvePreferredMarketingClawTmpDir(), "msteams-mention-"),
      );
    ...
    it("retries full activity preparation when media upload fails transiently", async () => {
      const tmpDir = await mkdtemp(
        path.join(resolvePreferredMarketingClawTmpDir(), "msteams-retry-"),
      );
