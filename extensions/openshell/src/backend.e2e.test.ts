...
import { createSandboxTestContext } from "marketingclaw/plugin-sdk/test-fixtures";
import {
...
const MARKETINGCLAW_OPENSHELL_E2E = process.env.MARKETINGCLAW_E2E_OPENSHELL === "1";
const MARKETINGCLAW_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const MARKETINGCLAW_OPENSHELL_COMMAND =
  process.env.MARKETINGCLAW_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
const MARKETINGCLAW_OPENSHELL_CONFIG_HOME =
  process.env.MARKETINGCLAW_E2E_OPENSHELL_CONFIG_HOME?.trim() || null;
const MARKETINGCLAW_OPENSHELL_HOST_IP =
  process.env.MARKETINGCLAW_E2E_OPENSHELL_HOST_IP?.trim() || null;
...
RUN echo "marketingclaw-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
async function resolveOpenShellHostIp(): Promise<string> {
  if (MARKETINGCLAW_OPENSHELL_HOST_IP) {
    return MARKETINGCLAW_OPENSHELL_HOST_IP;
  }
  ...
  throw new Error(
    "OpenShell E2E could not resolve the OpenShell Docker network gateway; set MARKETINGCLAW_E2E_OPENSHELL_HOST_IP",
  );
}
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && MARKETINGCLAW_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: MARKETINGCLAW_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        throw new Error("OpenShell E2E requires a working Docker daemon");
      }
      if (!(await commandAvailable(MARKETINGCLAW_OPENSHELL_COMMAND))) {
        throw new Error(`OpenShell CLI is unavailable: ${MARKETINGCLAW_OPENSHELL_COMMAND}`);
      }
      if (!MARKETINGCLAW_OPENSHELL_CONFIG_HOME) {
        throw new Error(
          "OpenShell E2E requires MARKETINGCLAW_E2E_OPENSHELL_CONFIG_HOME because tests isolate HOME and XDG_CONFIG_HOME",
        );
      }
      const openshellConfigHome = MARKETINGCLAW_OPENSHELL_CONFIG_HOME;
      const hostIp = await resolveOpenShellHostIp();
      const gatewayName = await activeOpenShellGateway(MARKETINGCLAW_OPENSHELL_COMMAND, {
        ...process.env,
      ...
      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-openshell-e2e-"));
      ...
      const allowSandboxName = `marketingclaw-policy-allow-${scopeSuffix}`;
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: MARKETINGCLAW_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("marketingclaw-openshell-e2e");
