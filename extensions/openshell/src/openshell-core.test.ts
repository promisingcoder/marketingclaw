...
import type { CreateSandboxBackendParams } from "marketingclaw/plugin-sdk/sandbox";
import {
...
describe("openshell backend manager", () => {
  ...
  it("uploads staged directory snapshots to the managed remote directory itself", () => {
    expect(
      buildOpenShellDirectoryUploadArgs({
        sandboxName: "marketingclaw-session",
        localPath: "/tmp/marketingclaw-upload/sandbox/seed.txt",
        remotePath: "/sandbox",
      }),
    ).toEqual([
      "sandbox",
      ...
      "/tmp/marketingclaw-upload/sandbox/seed.txt",
      "/sandbox",
    ]);
    expect(
      buildOpenShellDirectoryUploadArgs({
        sandboxName: "marketingclaw-session",
        localPath: "/tmp/marketingclaw-upload/project",
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const realParent = await makeTempDir("marketingclaw-openshell-real-");
      const root = path.join(realParent, "sandbox");
      const target = path.join(root, ".marketingclaw", "sandbox-skills");
  ...
  it("checks runtime status with config override from MarketingClaw config", async () => {
    ...
    const result = await manager.describeRuntime({
      entry: {
        containerName: "marketingclaw-session-1234",
        backendId: "openshell",
        runtimeLabel: "marketingclaw-session-1234",
    ...
    expect(cliMocks.runOpenShellCli).toHaveBeenCalledWith({
      context: {
        sandboxName: "marketingclaw-session-1234",
        config: expectedConfig,
      },
      args: ["sandbox", "get", "marketingclaw-session-1234"],
    });
  });
  ...
  it("removes runtimes via openshell sandbox delete", async () => {
    ...
    await manager.removeRuntime({
      entry: {
        containerName: "marketingclaw-session-5678",
        backendId: "openshell",
        runtimeLabel: "marketingclaw-session-5678",
        sessionKey: "agent:main",
        createdAtMs: 1,
        lastUsedAtMs: 1,
        image: "marketingclaw",
    ...
    expect(cliMocks.runOpenShellCli).toHaveBeenCalledWith({
      context: {
        sandboxName: "marketingclaw-session-5678",
        config: expectedConfig,
      },
      args: ["sandbox", "delete", "marketingclaw-session-5678"],
    });
  });
  ...
  it("preserves a local sandbox skills shadow when mirror sync crosses filesystems", async () => {
    const workspaceDir = await makeTempDir("marketingclaw-openshell-workspace-");
    const shadowFile = path.join(workspaceDir, ".marketingclaw", "sandbox-skills", "user-note.txt");
    ...
    cliMocks.runOpenShellCli.mockImplementation(async ({ args }: { args: string[] }) => {
      if (args[0] === "sandbox" && args[1] === "download") {
        ...
        await fs.mkdir(path.join(tmpDir, ".marketingclaw", "sandbox-skills", "skills"), {
          recursive: true,
        });
        await fs.writeFile(
          path.join(tmpDir, ".marketingclaw", "sandbox-skills", "skills", "generated.txt"),
          "generated",
          "utf8",
        );
      }
    ...
    try {
      ...
      await expectPathMissing(
        path.join(workspaceDir, ".marketingclaw", "sandbox-skills", "skills", "generated.txt"),
      );
    } finally {
  ...
  it("drops non-directory materialized sandbox skills from mirror downloads", async () => {
    ...
    cliMocks.runOpenShellCli.mockImplementation(async ({ args }: { args: string[] }) => {
      if (args[0] === "sandbox" && args[1] === "download") {
        const tmpDir = args[4];
        await fs.writeFile(path.join(tmpDir, "from-remote.txt"), "remote", "utf8");
        await fs.mkdir(path.join(tmpDir, ".marketingclaw"), { recursive: true });
        await fs.writeFile(path.join(tmpDir, ".marketingclaw", "sandbox-skills"), "poison", "utf8");
      }
      return { code: 0, stdout: "", stderr: "" };
    });
    ...
    await expectPathMissing(path.join(workspaceDir, ".marketingclaw", "sandbox-skills"));
  });
  ...
  it("restores a local sandbox skills shadow when mirror download has a file parent", async () => {
    ...
    cliMocks.runOpenShellCli.mockImplementation(async ({ args }: { args: string[] }) => {
      if (args[0] === "sandbox" && args[1] === "download") {
        const tmpDir = args[4];
        await fs.writeFile(path.join(tmpDir, "from-remote.txt"), "remote", "utf8");
        await fs.writeFile(path.join(tmpDir, ".marketingclaw"), "poison", "utf8");
      }
      return { code: 0, stdout: "", stderr: "" };
    });
    ...
    await expect(fs.readFile(shadowFile, "utf8")).resolves.toBe("local shadow");
    expect((await fs.stat(path.join(workspaceDir, ".marketingclaw"))).isDirectory()).toBe(true);
  });
});
...
function createOpenShellBackendSandboxConfig(): CreateSandboxBackendParams["cfg"] {
  return {
    ...
    ssh: createSandboxSshConfig("/tmp/marketingclaw-sandboxes"),
...
async function makeExecutable(params: { name: string; script: string }): Promise<string> {
  const dir = await makeTempDir("marketingclaw-openshell-bin-");
...
describe("openshell fs bridges", () => {
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const stateDir = await makeTempDir("marketingclaw-openshell-remote-pin-");
  ...
  it("writes locally and syncs the file to the remote workspace", async () => {
    const workspaceDir = await makeTempDir("marketingclaw-openshell-fs-");
  ...
  it("rejects cross-root mirror renames before the remote backend commit", async () => {
    const workspaceDir = await makeTempDir("marketingclaw-openshell-fs-");
    const agentWorkspaceDir = await makeTempDir("marketingclaw-openshell-agent-fs-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const workspaceDir = await makeTempDir("marketingclaw-openshell-fs-");
      const outsideDir = await makeTempDir("marketingclaw-openshell-outside-");
  ...
  it("reads materialized sandbox skills from the protected skills workspace", async () => {
    const workspaceDir = await makeTempDir("marketingclaw-openshell-fs-");
    const skillsWorkspaceDir = await makeTempDir("marketingclaw-openshell-skills-");
    ...
    await expect(
      bridge.readFile({
        filePath: "/sandbox/.marketingclaw/sandbox-skills/skills/demo/SKILL.md",
      }),
    ).resolves.toEqual(Buffer.from("# Demo\nmaterialized\n"));
    await expect(
      bridge.readFile({
        filePath: ".marketingclaw/sandbox-skills/skills/demo/SKILL.md",
      }),
    ).resolves.toEqual(Buffer.from("# Demo\nmaterialized\n"));
    await expect(
      bridge.writeFile({
  ...
  it("maps agent mount paths when the sandbox workspace is read-only", async () => {
    const workspaceDir = await makeTempDir("marketingclaw-openshell-fs-");
    const agentWorkspaceDir = await makeTempDir("marketingclaw-openshell-agent-");
