...
describe("voice-call config compatibility", () => {
  ...
  it("reports doctor-oriented legacy issues and warnings", () => {
    ...
    expect(
      ...
      `[voice-call] legacy config keys detected under plugins.entries.voice-call.config; runtime loading will not rewrite them, and support for the legacy shape will be removed in ${VOICE_CALL_LEGACY_CONFIG_REMOVAL_VERSION}. Run "marketingclaw doctor --fix".`,
