...
import { createPluginStateSyncKeyedStoreForTests } from "marketingclaw/plugin-sdk/plugin-state-test-runtime";
...
export function createTestStorePath(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-voice-call-test-"));
}
