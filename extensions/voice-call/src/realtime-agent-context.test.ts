...
async function createWorkspace(): Promise<string> {
  const workspaceDir = await mkdtemp(path.join(tmpdir(), "marketingclaw-voice-context-"));
...
describe("buildRealtimeVoiceInstructions", () => {
  it("injects bounded identity and workspace context", async () => {
    ...
    const coreConfig = { agents: { list: [{ id: "voice" }] } } as MarketingClawConfig;
    ...
    expect(instructions).toContain("MarketingClaw agent voice context:");
    expect(instructions).toContain("Consult behavior:");
    expect(instructions).toContain("Call marketingclaw_agent_consult before answering requests");
  ...
  it("truncates injected context without splitting UTF-16 surrogate pairs", async () => {
    ...
    const expectedContext = "MarketingClaw agent voice context:\n\n- Agent id: abc";
    ...
    const instructions = await buildRealtimeVoiceInstructions({
      baseInstructions: "Base voice instructions.",
      config,
      coreConfig: { agents: { list: [{ id: agentId }] } } as MarketingClawConfig,
