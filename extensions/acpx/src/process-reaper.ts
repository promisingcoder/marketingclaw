 ...
 * MarketingClaw-owned wrapper trees after validating paths, packages, and lease ids.
 */
...
const MARKETINGCLAW_PLUGIN_DEPS_MARKER = "/plugin-runtime-deps/";
...
type AcpxProcessCleanupResult = {
  ...
  skippedReason?: "missing-root" | "not-marketingclaw-owned" | "unverified-root";
};
...
function resolveMarketingClawInstallRoot(pluginRoot: string): string {
  if (
...
function resolveOwnedAcpPackageRootCandidates(packageName: string): string[] {
  ...
  const marketingClawRoot = resolveMarketingClawInstallRoot(pluginRoot);
  return [
    resolvePackageRoot(packageName),
    path.join(pluginRoot, "node_modules", packageName),
    path.join(marketingClawRoot, "node_modules", packageName),
...
/** Check whether a command references an MarketingClaw-generated ACPX wrapper path. */
export function isMarketingClawLeaseAwareAcpxProcessCommand(params: {
...
function liveCommandMatchesLeaseIdentity(params: {
  ...
  return (
    commandOptionEquals(parts, MARKETINGCLAW_ACPX_LEASE_ID_ARG, params.expectedLeaseId) &&
    commandOptionEquals(
      parts,
      MARKETINGCLAW_GATEWAY_INSTANCE_ID_ARG,
...
/** Check whether a command is owned by MarketingClaw ACPX runtime packages or wrappers. */
export function isMarketingClawOwnedAcpxProcessCommand(params: {
  command: string | undefined;
  wrapperRoot?: string;
}): boolean {
  const command = params.command?.trim();
  ...
  if (!normalized.includes(MARKETINGCLAW_PLUGIN_DEPS_MARKER)) {
    return false;
  }
...
/** Terminate one validated MarketingClaw-owned ACPX wrapper process tree. */
export async function cleanupMarketingClawOwnedAcpxProcessTree(params: {
  rootPid?: number;
  rootCommand?: string;
  expectedLeaseId?: string;
  expectedGatewayInstanceId?: string;
  ...
  // that this PID still belongs to an MarketingClaw-owned wrapper, fail closed to
...
/** Reap orphaned MarketingClaw-owned ACPX wrapper trees during runtime startup. */
export async function reapStaleMarketingClawOwnedAcpxOrphans(params: {
  wrapperRoot: string;
  deps?: AcpxProcessCleanupDeps;
