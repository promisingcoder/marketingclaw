...
import {
  MARKETINGCLAW_ACPX_LEASE_ID_ARG,
  MARKETINGCLAW_GATEWAY_INSTANCE_ID_ARG,
...
const DOCUMENTED_MARKETINGCLAW_BRIDGE_COMMAND =
  "env MARKETINGCLAW_HIDE_BANNER=1 MARKETINGCLAW_SUPPRESS_NOTES=1 marketingclaw acp --url ws://127.0.0.1:18789 --token-file ~/.marketingclaw/gateway.token --session agent:main:main";
const CODEX_ACP_COMMAND = "npx @zed-industries/codex-acp@0.13.0";
const CODEX_ACP_WRAPPER_COMMAND = `node "/tmp/marketingclaw/acpx/codex-acp-wrapper.mjs"`;
const CODEX_ACP_WRAPPER_COMMAND_WITH_LEASE = `${CODEX_ACP_WRAPPER_COMMAND} ${MARKETINGCLAW_ACPX_LEASE_ID_ARG} lease-close ${MARKETINGCLAW_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
  ...
  const runtime = new AcpxRuntime(
    {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "marketingclaw" ? "marketingclaw acp" : agentName,
        list: () => ["codex", "marketingclaw"],
      },
...
describe("AcpxRuntime fresh reset wrapper", () => {
  ...
  it("adds the MarketingClaw session key to the managed MarketingClaw tools MCP bridge", () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    const { runtime } = makeRuntime(baseStore, {
      marketingclawToolsMcpBridgeEnabled: true,
      mcpServers: [
        {
          name: "marketingclaw-tools",
          command: "node",
          args: ["dist/mcp/marketingclaw-tools-serve.js"],
          env: [],
        },
      ],
    });
    ...
    const readScopedMcpEnv = (sessionKey: string) => {
      const delegate = (
        runtime as unknown as {
          resolveMarketingClawToolsDelegateForSession(sessionKey: string): unknown;
        }
      ).resolveMarketingClawToolsDelegateForSession(sessionKey) as {
        options: {
          mcpServers?: Array<{
      ...
      return delegate.options.mcpServers?.find((server) => server.name === "marketingclaw-tools")
        ?.env;
    };

    expect(readScopedMcpEnv("agent:worker:main")).toContainEqual({
      name: "MARKETINGCLAW_TOOLS_MCP_AGENT_SESSION_KEY",
  ...
  it("keeps managed MarketingClaw tools MCP delegates reachable for fresh sessions", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const exposedRuntime = runtime as unknown as {
      marketingclawToolsSessionDelegates: Map<string, unknown>;
      resolveMarketingClawToolsDelegateForSession(sessionKey: string): unknown;
    };

    const firstDelegate =
      exposedRuntime.resolveMarketingClawToolsDelegateForSession("agent:worker:main");
    expect(exposedRuntime.marketingclawToolsSessionDelegates.has("agent:worker:main")).toBe(true);

    await runtime.prepareFreshSession({ sessionKey: "agent:worker:main" });

    expect(exposedRuntime.marketingclawToolsSessionDelegates.has("agent:worker:main")).toBe(true);
    expect(exposedRuntime.resolveMarketingClawToolsDelegateForSession("agent:worker:main")).toBe(
      firstDelegate,
    );
  });

  it("uses the no-MCP delegate for startup probes when the MarketingClaw tools bridge is enabled", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("normalizes MarketingClaw Codex model ids for ACP startup", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("strips the MarketingClaw Anthropic provider prefix for Claude ACP startup", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("keeps Claude ACP model ids intact after stripping the MarketingClaw provider prefix", () => {
  ...
  it("adds Codex wrapper stderr tail to generic session initialization failures", async () => {
    const wrapperRoot = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-acpx-runtime-"));
    ...
    const { runtime, delegate } = makeRuntime(baseStore, {
      marketingclawGatewayInstanceId: "gateway-test",
      marketingclawProcessLeaseStore: leaseStore.store,
      marketingclawWrapperRoot: wrapperRoot,
      agentRegistry: {
  ...
  it("adds Codex wrapper stderr tail to generic first-turn failures", async () => {
    ...
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        ...
        marketingclawLeaseId: "lease-turn",
      })),
      save: vi.fn(async () => {}),
    };
    const { runtime, delegate } = makeRuntime(baseStore, {
  ...
  it("adds Codex wrapper stderr tail to generic terminal turn error events", async () => {
    ...
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:test",
        agentCommand: CODEX_ACP_WRAPPER_COMMAND,
        marketingclawLeaseId: "lease-turn-event",
      })),
      save: vi.fn(async () => {}),
    };
    const { runtime, delegate } = makeRuntime(baseStore, {
  ...
  it("adds Codex wrapper stderr tail to generic startTurn failure results", async () => {
    ...
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:test",
        agentCommand: CODEX_ACP_WRAPPER_COMMAND,
        marketingclawLeaseId: "lease-start-turn",
      })),
      save: vi.fn(async () => {}),
    };
    const { runtime, delegate } = makeRuntime(baseStore, {
  ...
  it("adds Codex wrapper stderr tail when startTurn creation throws", async () => {
    ...
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:test",
        agentCommand: CODEX_ACP_WRAPPER_COMMAND,
        marketingclawLeaseId: "lease-start-turn-create",
      })),
      save: vi.fn(async () => {}),
    };
    const { runtime, delegate } = makeRuntime(baseStore, {
  ...
  it("disables delegate prompt timeout for MarketingClaw-managed turns", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:test",
  ...
  it("injects Codex ACP startup config into the scoped registry", () => {
    ...
    expect(testing.isCodexAcpCommand("marketingclaw acp")).toBe(false);
  });
  ...
  it("recognizes claude-agent-acp commands", () => {
    ...
    expect(
      testing.isClaudeAcpCommand(`node "/tmp/marketingclaw/acpx/claude-agent-acp-wrapper.mjs"`),
    ).toBe(true);
    expect(
      testing.isClaudeAcpCommand(
        `node.exe "C:/Users/runner/AppData/Local/Temp/marketingclaw/acpx/claude-agent-acp-wrapper.mjs"`,
      ),
    ).toBe(true);
    expect(
      testing.isClaudeAcpCommand(
    ...
    expect(testing.isClaudeAcpCommand("marketingclaw acp")).toBe(false);
  ...
  it("releases managed MarketingClaw tools MCP delegates after close", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const exposedRuntime = runtime as unknown as {
      marketingclawToolsSessionDelegates: Map<string, { close: AcpRuntime["close"] }>;
      resolveMarketingClawToolsDelegateForSession(sessionKey: string): {
        close: AcpRuntime["close"];
      };
    };
    const scopedDelegate =
      exposedRuntime.resolveMarketingClawToolsDelegateForSession("agent:codex:main");
    ...
    expect(exposedRuntime.marketingclawToolsSessionDelegates.has("agent:codex:main")).toBe(false);
  });

  it("cleans up MarketingClaw-owned ACPX process trees after close", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:binding:test",
        agentCommand: 'node "/tmp/marketingclaw/acpx/codex-acp-wrapper.mjs"',
        pid: 900,
      })),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate } = makeRuntime(
      baseStore,
      {
        marketingclawWrapperRoot: "/tmp/marketingclaw/acpx",
      },
      {
        marketingclawProcessCleanup: {
          listProcesses: vi.fn(async () => [
            {
              pid: 900,
              ppid: 1,
              command: 'node "/tmp/marketingclaw/acpx/codex-acp-wrapper.mjs"',
            },
            {
              pid: 901,
              ppid: 900,
              command:
                "node /tmp/marketingclaw/plugin-runtime-deps/node_modules/@zed-industries/codex-acp/bin/codex-acp.js",
            },
          ]),
  ...
  it("records ACPX process leases without persisting lease-specific agent commands", async () => {
    ...
    expect(lease?.wrapperPath).toBe("/tmp/marketingclaw/acpx/codex-acp-wrapper.mjs");
    expect(launchCommands[0]).toContain("MARKETINGCLAW_ACPX_LEASE_ID=");
    expect(launchCommands[0]).toContain("MARKETINGCLAW_GATEWAY_INSTANCE_ID=gateway-test");
    expect(savedRecords[0]?.agentCommand).toBe(CODEX_ACP_WRAPPER_COMMAND);
    expect(savedRecords[0]?.marketingclawGatewayInstanceId).toBe("gateway-test");
    expect(savedRecords[0]?.marketingclawLeaseId).toBe(lease?.leaseId);
  });
  ...
  it("merges sidecar lease ids into loaded ACPX session records", async () => {
    ...
    leaseStore.leases.set("lease-loaded", {
      ...
      sessionKey: "agent:codex:acp:binding:test",
      wrapperRoot: "/tmp/marketingclaw/acpx",
      wrapperPath: "/tmp/marketingclaw/acpx/codex-acp-wrapper.mjs",
      rootPid: 777,
      commandHash: "hash",
      startedAt: 1,
      state: "open",
    ...
    expect(loadedRecord?.marketingclawGatewayInstanceId).toBe("gateway-test");
    expect(loadedRecord?.marketingclawLeaseId).toBe("lease-loaded");
  });
  ...
  it("merges the lease for the current ACPX session process when old leases exist", async () => {
    ...
    const loadedRecord = await wrappedStore.load("agent:codex:acp:binding:test");
    expect(loadedRecord?.marketingclawGatewayInstanceId).toBe("gateway-test");
    expect(loadedRecord?.marketingclawLeaseId).toBe("lease-current");
  });
  ...
  it("uses matching leases before legacy pid cleanup on close", async () => {
    ...
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:binding:test",
        agentCommand: 'node "/tmp/marketingclaw/acpx/codex-acp-wrapper.mjs"',
        marketingclawLeaseId: "lease-close",
        pid: 930,
      })),
      save: vi.fn(async () => {}),
    };
  ...
  it("closes the current process lease when the saved lease id is stale", async () => {
    ...
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:binding:test",
        agentCommand: 'node "/tmp/marketingclaw/acpx/codex-acp-wrapper.mjs"',
        marketingclawLeaseId: "lease-old",
        pid: 940,
      })),
      save: vi.fn(async () => {}),
    };
          ...
          listProcesses: vi.fn(async () => [
            {
              pid: 930,
              ppid: 1,
              command: `${CODEX_ACP_WRAPPER_COMMAND} ${MARKETINGCLAW_ACPX_LEASE_ID_ARG} lease-old ${MARKETINGCLAW_GATEWAY_INSTANCE_ID_ARG} gateway-test`,
            },
            {
              pid: 940,
              ppid: 1,
              command: `${CODEX_ACP_WRAPPER_COMMAND} ${MARKETINGCLAW_ACPX_LEASE_ID_ARG} lease-current ${MARKETINGCLAW_GATEWAY_INSTANCE_ID_ARG} gateway-test`,
            },
  ...
  it("uses session lease metadata for fallback close cleanup identity checks", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        ...
        agentCommand: 'node "/tmp/marketingclaw/acpx/codex-acp-wrapper.mjs"',
        marketingclawGatewayInstanceId: "gateway-test",
        marketingclawLeaseId: "lease-record",
        pid: 920,
      })),
      save: vi.fn(async () => {}),
    };
          ...
          listProcesses: vi.fn(async () => [
            {
              pid: 920,
              ppid: 1,
              command: `${CODEX_ACP_WRAPPER_COMMAND} ${MARKETINGCLAW_ACPX_LEASE_ID_ARG} other-lease ${MARKETINGCLAW_GATEWAY_INSTANCE_ID_ARG} gateway-test`,
            },
          ]),
          killProcess: vi.fn((pid, signal) => {
            killed.push({ pid, signal });
  ...
  it("routes marketingclaw ensureSession through the bridge-safe delegate when MCP servers are configured", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const bridgeEnsure = vi.spyOn(bridgeSafeDelegate, "ensureSession").mockResolvedValue({
      sessionKey: "agent:marketingclaw:acp:test",
      backend: "acpx",
  ...
  it("routes non-marketingclaw sessions through the default delegate", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("routes handle-based follow-up calls for marketingclaw sessions through the bridge-safe delegate", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const handle: Parameters<NonNullable<AcpRuntime["getStatus"]>>[0]["handle"] = {
      sessionKey: "agent:marketingclaw:acp:test",
      backend: "acpx",
      runtimeSessionName: "marketingclaw-session-handle",
    };
  ...
  it("keeps MCP-enabled routing when the marketingclaw agent is overridden to a non-bridge adapter", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      mcpServers: [{ name: "tools", command: "mcp-tools" }] as never,
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "marketingclaw" ? "codex" : agentName),
        list: () => ["codex", "marketingclaw"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for any agent mapped to the marketingclaw bridge command", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      mcpServers: [{ name: "tools", command: "mcp-tools" }] as never,
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "codex" ? "marketingclaw acp" : agentName),
        list: () => ["codex", "marketingclaw"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for documented env-wrapped marketingclaw bridge commands", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "marketingclaw" ? DOCUMENTED_MARKETINGCLAW_BRIDGE_COMMAND : agentName,
        list: () => ["codex", "marketingclaw"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for local node marketingclaw entrypoints", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "marketingclaw"
            ? "env MARKETINGCLAW_HIDE_BANNER=1 node marketingclaw.mjs acp"
            : agentName,
        list: () => ["codex", "marketingclaw"],
      },
    });
  ...
  it("routes follow-up calls by persisted agent command before current config", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:marketingclaw:acp:test",
        agentCommand: DOCUMENTED_MARKETINGCLAW_BRIDGE_COMMAND,
      })),
      save: vi.fn(async () => {}),
    };
    ...
    const status = await runtime.getStatus({
      handle: {
        sessionKey: "agent:marketingclaw:acp:test",
        backend: "acpx",
        runtimeSessionName: "agent:marketingclaw:acp:test",
      },
    });
  ...
  it("probes through the bridge-safe delegate when probeAgent resolves to marketingclaw bridge", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
