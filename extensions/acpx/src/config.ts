...
import { formatPluginConfigIssue } from "marketingclaw/plugin-sdk/extension-shared";
...
const ACPX_PLUGIN_TOOLS_MCP_SERVER_NAME = "marketingclaw-plugin-tools";
const ACPX_MARKETINGCLAW_TOOLS_MCP_SERVER_NAME = "marketingclaw-tools";
const requireFromHere = createRequire(import.meta.url);

function isAcpxPluginRoot(dir: string): boolean {
  return (
    fs.existsSync(path.join(dir, "marketingclaw.plugin.json")) &&
...
function resolveAcpxPluginRootFromMarketingClawLayout(moduleUrl: string): string | null {
  let cursor = path.dirname(fileURLToPath(moduleUrl));
  for (let i = 0; i < 5; i += 1) {
    const candidates = [
...
export function resolveAcpxPluginRoot(moduleUrl: string = import.meta.url): string {
  ...
  return (
    ...
    // Scan common MarketingClaw layouts before falling back to the nearest path guess.
    resolveAcpxPluginRootFromMarketingClawLayout(moduleUrl) ??
    resolvedRoot
  );
}
...
function resolveMarketingClawRoot(currentRoot: string): string {
  if (
    path.basename(currentRoot) === "acpx" &&
    path.basename(path.dirname(currentRoot)) === "extensions"
  ) {
...
function resolvePluginToolsMcpServerConfig(moduleUrl: string = import.meta.url): McpServerConfig {
  ...
  const marketingClawRoot = resolveMarketingClawRoot(pluginRoot);
  const distEntry = path.join(marketingClawRoot, "dist", "mcp", "plugin-tools-serve.js");
  ...
  const sourceEntry = path.join(marketingClawRoot, "src", "mcp", "plugin-tools-serve.ts");
  return {
    command: process.execPath,
...
function resolveMarketingClawToolsMcpServerConfig(
  moduleUrl: string = import.meta.url,
): McpServerConfig {
  const pluginRoot = resolveAcpxPluginRoot(moduleUrl);
  const marketingClawRoot = resolveMarketingClawRoot(pluginRoot);
  const distEntry = path.join(marketingClawRoot, "dist", "mcp", "marketingclaw-tools-serve.js");
  if (fs.existsSync(distEntry)) {
    return {
      command: process.execPath,
      args: [distEntry],
  ...
  const sourceEntry = path.join(marketingClawRoot, "src", "mcp", "marketingclaw-tools-serve.ts");
  return {
    command: process.execPath,
    args: ["--import", resolveTsxImportSpecifier(), sourceEntry],
  };
...
function resolveConfiguredMcpServers(params: {
  ...
  marketingClawToolsMcpBridge: boolean;
  moduleUrl?: string;
  ...
  if (params.marketingClawToolsMcpBridge && resolved[ACPX_MARKETINGCLAW_TOOLS_MCP_SERVER_NAME]) {
    throw new Error(
      `mcpServers.${ACPX_MARKETINGCLAW_TOOLS_MCP_SERVER_NAME} is reserved when marketingClawToolsMcpBridge=true`,
    );
  }
  ...
  if (params.marketingClawToolsMcpBridge) {
    resolved[ACPX_MARKETINGCLAW_TOOLS_MCP_SERVER_NAME] = resolveMarketingClawToolsMcpServerConfig(
      params.moduleUrl,
    );
  }
  return resolved;
...
/** Convert MarketingClaw MCP server config into ACPX runtime MCP server entries. */
...
export function resolveAcpxPluginConfig(params: {
  ...
  const marketingClawToolsMcpBridge = normalized.marketingClawToolsMcpBridge === true;
