...
import {
  ...
  MARKETINGCLAW_ACPX_LEASE_ID_ENV,
...
const RUN_CONFIGURED_COMMAND_SENTINEL = "--marketingclaw-run-configured";
...
function readManifestDependencyVersion(packageName: string): string {
  ...
  if (typeof version !== "string" || version.trim() === "") {
    throw new Error(`Missing ${packageName} dependency version in @marketingclaw/acpx manifest`);
  }
  return version;
}
...
async function resolveInstalledCodexAcpBinPath(): Promise<string | undefined> {
  // Keep MarketingClaw's isolated CODEX_HOME wrapper, but launch the plugin-local
...
const marketingClawWrapperArgs = new Set([
  ${quoteCommandPart(MARKETINGCLAW_ACPX_LEASE_ID_ARG)},
  ${quoteCommandPart(MARKETINGCLAW_GATEWAY_INSTANCE_ID_ARG)},
]);

function readMarketingClawWrapperArg(args, name) {
...
function resolveStderrLogPath(args) {
  ...
  const leaseId =
    process.env[${JSON.stringify(MARKETINGCLAW_ACPX_LEASE_ID_ENV)}] ||
    readMarketingClawWrapperArg(args, ${quoteCommandPart(MARKETINGCLAW_ACPX_LEASE_ID_ARG)}) ||
    "pid-" + process.pid;
...
function stripMarketingClawWrapperArgs(args) {
  const stripped = [];
  for (let index = 0; index < args.length; index += 1) {
    const value = args[index];
    if (marketingClawWrapperArgs.has(value)) {
      index += 1;
      continue;
    }
    stripped.push(value);
...
const configuredArgs = stripMarketingClawWrapperArgs(rawConfiguredArgs);
...
if (!command) {
  console.error("[marketingclaw] missing configured ${params.displayName} ACP command");
  process.exit(1);
}
...
child.on("error", (error) => {
  console.error(\`[marketingclaw] failed to launch ${params.displayName} ACP wrapper: \${error.message}\`);
  process.exit(1);
});
...
function buildClaudeAcpWrapperScript(installedBinPath?: string): string {
  return buildAdapterWrapperScript({
    displayName: "Claude",
    // This package is patched in MarketingClaw; fallback must not float to an unpatched newer release.
