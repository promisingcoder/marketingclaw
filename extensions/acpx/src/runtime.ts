/**
 * MarketingClaw ACPX runtime adapter. It wraps the upstream acpx runtime with
 * MarketingClaw session metadata, lease tracking, model scoping, and cleanup policy.
 */
...
import { redactSensitiveText } from "marketingclaw/plugin-sdk/security-runtime";
...
import {
  cleanupMarketingClawOwnedAcpxProcessTree,
  isMarketingClawLeaseAwareAcpxProcessCommand,
...
type MarketingClawAcpxRuntimeOptions = AcpRuntimeOptions & {
  marketingclawWrapperRoot?: string;
  marketingclawGatewayInstanceId?: string;
  marketingclawProcessLeaseStore?: AcpxProcessLeaseStore;
  marketingclawToolsMcpBridgeEnabled?: boolean;
};
type AcpxRuntimeTestOptions = Record<string, unknown> & {
  marketingclawProcessCleanup?: AcpxProcessCleanupDeps;
};
type MarketingClawRuntimeTurnInput = Parameters<NonNullable<AcpRuntime["startTurn"]>>[0];
type MarketingClawRuntimeEnsureInput = Parameters<AcpRuntime["ensureSession"]>[0];
...
const ACPX_MARKETINGCLAW_TOOLS_MCP_SERVER_NAME = "marketingclaw-tools";
const MARKETINGCLAW_TOOLS_MCP_AGENT_SESSION_KEY_ENV = "MARKETINGCLAW_TOOLS_MCP_AGENT_SESSION_KEY";

type ResetAwareSessionStore = AcpSessionStore & {
  markFresh: (sessionKey: string) => void;
};

type MarketingClawLeaseSessionMetadata = {
  marketingclawLeaseId: string;
  marketingclawGatewayInstanceId: string;
};

function withMarketingClawManagedTurnTimeout<T extends object>(input: T): T & { timeoutMs: 0 } {
  // MarketingClaw owns ACP turn deadlines. acpx treats timeout after partial agent
...
function withMarketingClawLeaseSessionMetadata<T extends object>(
  record: T,
  metadata: MarketingClawLeaseSessionMetadata,
): T & MarketingClawLeaseSessionMetadata {
  return {
    ...record,
    marketingclawLeaseId: metadata.marketingclawLeaseId,
    marketingclawGatewayInstanceId: metadata.marketingclawGatewayInstanceId,
  };
}
...
function readMarketingClawLeaseIdFromRecord(record: unknown): string | undefined {
  if (typeof record !== "object" || record === null) {
    return undefined;
  }
  const { marketingclawLeaseId } = record as { marketingclawLeaseId?: unknown };
  return typeof marketingclawLeaseId === "string"
    ? marketingclawLeaseId.trim() || undefined
    : undefined;
}

function readMarketingClawGatewayInstanceIdFromRecord(record: unknown): string | undefined {
  if (typeof record !== "object" || record === null) {
    return undefined;
  }
  const { marketingclawGatewayInstanceId } = record as { marketingclawGatewayInstanceId?: unknown };
  return typeof marketingclawGatewayInstanceId === "string"
    ? marketingclawGatewayInstanceId.trim() || undefined
    : undefined;
}
...
function createResetAwareSessionStore(
  ...
  return {
    async load(sessionId: string): Promise<AcpLoadedSessionRecord> {
      ...
      return withMarketingClawLeaseSessionMetadata(record, {
        marketingclawLeaseId: lease.leaseId,
        marketingclawGatewayInstanceId: lease.gatewayInstanceId,
      });
    },
    async save(record: AcpSessionRecord): Promise<void> {
      ...
      if (
        ...
        recordToSave = withMarketingClawLeaseSessionMetadata(
          {
            ...record,
          ...
          {
            marketingclawLeaseId: launch.leaseId,
            marketingclawGatewayInstanceId: launch.gatewayInstanceId,
          },
        );
      }
...
const MARKETINGCLAW_BRIDGE_EXECUTABLE = "marketingclaw";
const MARKETINGCLAW_BRIDGE_SUBCOMMAND = "acp";
const CODEX_ACP_AGENT_ID = "codex";
const CODEX_ACP_MARKETINGCLAW_PREFIX = "openai/";
const CLAUDE_ACP_MARKETINGCLAW_PREFIX = "anthropic/";
...
function isMarketingClawBridgeCommand(command: string | undefined): boolean {
  if (!command) {
    return false;
  }
  const parts = unwrapEnvCommand(splitCommandParts(command.trim()));
  if (basename(parts[0] ?? "") === MARKETINGCLAW_BRIDGE_EXECUTABLE) {
    return parts[1] === MARKETINGCLAW_BRIDGE_SUBCOMMAND;
  }
  ...
  return (
    /^marketingclaw(?:\.[cm]?js)?$/i.test(scriptName) &&
    parts[2] === MARKETINGCLAW_BRIDGE_SUBCOMMAND
  );
}
...
// See promisingcoder/marketingclaw#73071.
...
function normalizeCodexAcpModelOverride(
  ...
  let value = raw;
  if (value.toLowerCase().startsWith(CODEX_ACP_MARKETINGCLAW_PREFIX)) {
    value = value.slice(CODEX_ACP_MARKETINGCLAW_PREFIX.length);
  }
...
function normalizeClaudeAcpModelOverride(rawModel: string | undefined): string | undefined {
  ...
  if (!raw.toLowerCase().startsWith(CLAUDE_ACP_MARKETINGCLAW_PREFIX)) {
    return raw;
  }
  return raw.slice(CLAUDE_ACP_MARKETINGCLAW_PREFIX.length).trim() || undefined;
}

function withAcpxSessionOptions(input: MarketingClawRuntimeEnsureInput): AcpxDelegateEnsureInput {
...
async function ensureDelegateSessionWithModelFallback(
  ...
  input: MarketingClawRuntimeEnsureInput,
...
function shouldUseBridgeSafeDelegateForCommand(command: string | undefined): boolean {
  return isMarketingClawBridgeCommand(command);
}
...
function withMarketingClawToolsMcpSessionEnv(params: {
  ...
  const nextServers = params.mcpServers.map((server): AcpxMcpServer => {
    if (server.name !== ACPX_MARKETINGCLAW_TOOLS_MCP_SERVER_NAME || !("command" in server)) {
      return server;
    }
    changed = true;
    const env = [
      ...server.env.filter((entry) => entry.name !== MARKETINGCLAW_TOOLS_MCP_AGENT_SESSION_KEY_ENV),
      {
        name: MARKETINGCLAW_TOOLS_MCP_AGENT_SESSION_KEY_ENV,
        value: sessionKey,
      },
    ];
    return { ...server, env };
...
/** MarketingClaw-managed ACP runtime implementation backed by the upstream acpx runtime. */
export class AcpxRuntime implements AcpRuntime {
  ...
  private readonly marketingclawToolsMcpBridgeEnabled: boolean;
  private readonly marketingclawToolsSessionDelegates = new Map<string, BaseAcpxRuntime>();
  ...
  constructor(options: MarketingClawAcpxRuntimeOptions, testOptions?: AcpxRuntimeTestOptions) {
    const { marketingclawProcessCleanup, ...delegateTestOptions } = testOptions ?? {};
    this.processCleanupDeps = marketingclawProcessCleanup;
    this.wrapperRoot = options.marketingclawWrapperRoot;
    this.gatewayInstanceId = options.marketingclawGatewayInstanceId;
    this.processLeaseStore = options.marketingclawProcessLeaseStore;
    this.marketingclawToolsMcpBridgeEnabled = options.marketingclawToolsMcpBridgeEnabled === true;
    ...
    this.bridgeSafeDelegate = shouldUseDistinctBridgeDelegate(options)
      ...
      : this.delegate;
    this.probeDelegate = this.marketingclawToolsMcpBridgeEnabled
  ...
  private resolveDelegateForSession(params: {
    ...
    return this.resolveMarketingClawToolsDelegateForSession(params.sessionKey);
  }

  private resolveMarketingClawToolsDelegateForSession(sessionKey: string): BaseAcpxRuntime {
    if (!this.marketingclawToolsMcpBridgeEnabled) {
    ...
    const cached = this.marketingclawToolsSessionDelegates.get(normalizedSessionKey);
    if (cached) {
      return cached;
    }
    // Upstream acpx captures mcpServers at runtime construction. The managed
    // MarketingClaw tools bridge needs per-session identity, so cache one delegate
    // per session with the scoped MCP env already embedded.
    const delegate = new BaseAcpxRuntime(
      {
        ...this.delegateOptions,
        mcpServers: withMarketingClawToolsMcpSessionEnv({
          enabled: this.marketingclawToolsMcpBridgeEnabled,
    ...
    this.marketingclawToolsSessionDelegates.set(normalizedSessionKey, delegate);
    return delegate;
  }

  private releaseMarketingClawToolsDelegateForSession(sessionKey: string): void {
    if (!this.marketingclawToolsMcpBridgeEnabled) {
      return;
    }
    const normalizedSessionKey = sessionKey.trim();
    ...
    this.marketingclawToolsSessionDelegates.delete(normalizedSessionKey);
  }
  ...
  private async readCodexTurnFailureStderr(params: { handle: AcpRuntimeHandle }): Promise<string> {
    ...
    return readCodexWrapperStderrTail({
      wrapperRoot: this.wrapperRoot,
      leaseId: readMarketingClawLeaseIdFromRecord(record),
    });
  }
  ...
  private async cleanupProcessTreeForRecord(
    ...
    const leaseId = readMarketingClawLeaseIdFromRecord(record);
    ...
    if (lease && lease.gatewayInstanceId === this.gatewayInstanceId && lease.rootPid > 0) {
      ...
      const result = await cleanupMarketingClawOwnedAcpxProcessTree({
    ...
    const expectedGatewayInstanceId = readMarketingClawGatewayInstanceIdFromRecord(record);
    await cleanupMarketingClawOwnedAcpxProcessTree({
      rootPid,
      rootCommand,
  ...
  async *runTurn(input: Parameters<AcpRuntime["runTurn"]>[0]): AsyncIterable<AcpRuntimeEvent> {
    ...
    try {
      for await (const event of delegate.runTurn(withMarketingClawManagedTurnTimeout(input))) {
        if (
          event.type !== "error" ||
          !isCodexAcpCommand(command) ||
  ...
  startTurn(input: MarketingClawRuntimeTurnInput): AcpRuntimeTurn {
    ...
    const turnPromise = Promise.all([
      ...
      try {
        return {
          command,
          turn: delegate.startTurn(withMarketingClawManagedTurnTimeout(input)),
        };
      } catch (error) {
        if (!isCodexAcpCommand(command) || !isGenericInternalAcpError(error)) {
          throw error;
  ...
  async close(input: Parameters<AcpRuntime["close"]>[0]): Promise<void> {
    ...
    if (closeSucceeded) {
      this.releaseMarketingClawToolsDelegateForSession(input.handle.sessionKey);
    }
