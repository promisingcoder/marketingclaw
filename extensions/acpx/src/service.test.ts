...
const { cleanupMarketingClawOwnedAcpxProcessTreeMock } = vi.hoisted(() => ({
  cleanupMarketingClawOwnedAcpxProcessTreeMock: vi.fn(
    async (): Promise<{
...
const { reapStaleMarketingClawOwnedAcpxOrphansMock } = vi.hoisted(() => ({
  reapStaleMarketingClawOwnedAcpxOrphansMock: vi.fn(
    async (): Promise<{
      inspectedPids: number[];
      terminatedPids: number[];
      skippedReason?: string;
...
vi.mock("./process-reaper.js", () => ({
  cleanupMarketingClawOwnedAcpxProcessTree: cleanupMarketingClawOwnedAcpxProcessTreeMock,
  reapStaleMarketingClawOwnedAcpxOrphans: reapStaleMarketingClawOwnedAcpxOrphansMock,
}));

import { getAcpRuntimeBackend } from "../runtime-api.js";
import type { MarketingClawPluginServiceContext } from "../runtime-api.js";
...
const previousEnv = {
  MARKETINGCLAW_ACPX_RUNTIME_STARTUP_PROBE: process.env.MARKETINGCLAW_ACPX_RUNTIME_STARTUP_PROBE,
  MARKETINGCLAW_SKIP_ACPX_RUNTIME: process.env.MARKETINGCLAW_SKIP_ACPX_RUNTIME,
  MARKETINGCLAW_SKIP_ACPX_RUNTIME_PROBE: process.env.MARKETINGCLAW_SKIP_ACPX_RUNTIME_PROBE,
};
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-acpx-service-"));
  tempDirs.push(dir);
  return dir;
}
...
afterEach(async () => {
  ...
  cleanupMarketingClawOwnedAcpxProcessTreeMock.mockClear();
  reapStaleMarketingClawOwnedAcpxOrphansMock.mockClear();
  acpxRuntimeConstructorMock.mockClear();
  createAgentRegistryMock.mockClear();
  createFileSessionStoreMock.mockClear();
  restoreEnv("MARKETINGCLAW_ACPX_RUNTIME_STARTUP_PROBE");
  restoreEnv("MARKETINGCLAW_SKIP_ACPX_RUNTIME");
  restoreEnv("MARKETINGCLAW_SKIP_ACPX_RUNTIME_PROBE");
  for (const dir of tempDirs.splice(0)) {
    await fs.rm(dir, { recursive: true, force: true });
  }
});

function createServiceContext(workspaceDir: string): MarketingClawPluginServiceContext {
  return {
    workspaceDir,
    stateDir: path.join(workspaceDir, ".marketingclaw-plugin-state"),
    config: {},
    logger: {
      info: vi.fn(),
      warn: vi.fn(),
...
function createOpenKeyedStore(ctx: MarketingClawPluginServiceContext) {
  const env = { ...process.env, MARKETINGCLAW_STATE_DIR: ctx.stateDir };
...
function openGatewayInstanceStore(ctx: MarketingClawPluginServiceContext) {
...
function openProcessLeaseStore(ctx: MarketingClawPluginServiceContext) {
...
describe("createAcpxRuntimeService", () => {
  ...
  it("skips the startup probe and does not advertise backend health when explicitly disabled", async () => {
    process.env.MARKETINGCLAW_ACPX_RUNTIME_STARTUP_PROBE = "0";
    delete process.env.MARKETINGCLAW_SKIP_ACPX_RUNTIME_PROBE;
    const workspaceDir = await makeTempDir();
  ...
  it("waits for the embedded runtime startup probe before resolving by default", async () => {
    delete process.env.MARKETINGCLAW_ACPX_RUNTIME_STARTUP_PROBE;
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
  ...
  it("reaps stale ACPX process leases from the generated wrapper root at startup", async () => {
    ...
    cleanupMarketingClawOwnedAcpxProcessTreeMock.mockResolvedValueOnce({
    ...
    expect(cleanupMarketingClawOwnedAcpxProcessTreeMock).toHaveBeenCalledWith({
      rootPid: 101,
    ...
    expect(ctx.logger.info).toHaveBeenCalledWith(
      "reaped 2 stale MarketingClaw-owned ACPX processes",
    );
  ...
  it("runs wrapper-root orphan cleanup before dropping pending ACPX leases", async () => {
    ...
    await openProcessLeaseStore(ctx).register(lease.leaseId, lease);
    reapStaleMarketingClawOwnedAcpxOrphansMock.mockResolvedValueOnce({
    ...
    expect(cleanupMarketingClawOwnedAcpxProcessTreeMock).not.toHaveBeenCalled();
    expect(reapStaleMarketingClawOwnedAcpxOrphansMock).toHaveBeenCalledWith({
      wrapperRoot,
      deps: processCleanupDeps,
    });
    expect(ctx.logger.info).toHaveBeenCalledWith(
  ...
  it("runs the embedded runtime probe at startup when explicitly enabled and reports health", async () => {
    process.env.MARKETINGCLAW_ACPX_RUNTIME_STARTUP_PROBE = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    const probeAvailability = vi.fn(async () => {});
    const runtime = createMockRuntime({
  ...
  it("lets the skip env override the opt-in embedded runtime startup probe without advertising health", async () => {
    process.env.MARKETINGCLAW_ACPX_RUNTIME_STARTUP_PROBE = "1";
    process.env.MARKETINGCLAW_SKIP_ACPX_RUNTIME_PROBE = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    const probeAvailability = vi.fn(async () => {});
    const runtime = createMockRuntime({
  ...
  it("can skip the embedded runtime backend via env", async () => {
    process.env.MARKETINGCLAW_SKIP_ACPX_RUNTIME = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
