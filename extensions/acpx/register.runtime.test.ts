...
vi.mock("marketingclaw/plugin-sdk/acp-runtime-backend", () => ({
...
const previousSkipRuntime = process.env.MARKETINGCLAW_SKIP_ACPX_RUNTIME;

function restoreEnv(): void {
  if (previousSkipRuntime === undefined) {
    delete process.env.MARKETINGCLAW_SKIP_ACPX_RUNTIME;
  } else {
    process.env.MARKETINGCLAW_SKIP_ACPX_RUNTIME = previousSkipRuntime;
  }
}

function createServiceContext() {
  return {
    workspaceDir: "/tmp/marketingclaw-acpx-register-test",
    stateDir: "/tmp/marketingclaw-acpx-register-test/state",
    config: {},
    logger: {
      info: vi.fn(),
      warn: vi.fn(),
