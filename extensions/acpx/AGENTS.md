...
The ACPX extension is a thin MarketingClaw wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `marketingclaw/acpx`, not in this extension.
...
Use this flow when MarketingClaw needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `marketingclaw/acpx` repo first.
2. In MarketingClaw, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild MarketingClaw and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for MarketingClaw ACP validation.
...
If a change feels like shared ACP runtime behavior instead of MarketingClaw-specific glue, move it to `marketingclaw/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
