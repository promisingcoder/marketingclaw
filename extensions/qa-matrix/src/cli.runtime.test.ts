...
describe("matrix qa cli runtime", () => {
  const originalRunNodeOutputLog = process.env.MARKETINGCLAW_RUN_NODE_OUTPUT_LOG;

  afterEach(async () => {
    vi.clearAllMocks();
    if (originalRunNodeOutputLog === undefined) {
      delete process.env.MARKETINGCLAW_RUN_NODE_OUTPUT_LOG;
    } else {
      process.env.MARKETINGCLAW_RUN_NODE_OUTPUT_LOG = originalRunNodeOutputLog;
    }
  ...
  it("reuses a run-node output log instead of installing a nested tee", async () => {
    ...
    process.env.MARKETINGCLAW_RUN_NODE_OUTPUT_LOG = outputPath;
    runMatrixQaLive.mockResolvedValue({
      reportPath: "/tmp/matrix-report.md",
      summaryPath: "/tmp/matrix-summary.json",
      observedEventsPath: "/tmp/matrix-events.json",
