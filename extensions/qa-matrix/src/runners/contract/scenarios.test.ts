...
const {
  ...
  resolveMatrixQaMarketingClawCliEntryPath,
  runMatrixQaMarketingClawCli,
  startMatrixQaMarketingClawCli,
} = vi.hoisted(() => ({
  formatMatrixQaCliCommand: (args: string[]) => `marketingclaw ${args.join(" ")}`,
  redactMatrixQaCliOutput: (text: string) => text,
  resolveMatrixQaMarketingClawCliEntryPath: (cwd: string) => `${cwd}/dist/index.js`,
  runMatrixQaMarketingClawCli: vi.fn(),
  startMatrixQaMarketingClawCli: vi.fn(),
}));
...
async function writeMatrixInboundDedupePluginStateEntry(params: {
  ...
  const databasePath = path.join(params.stateRoot, "state", "marketingclaw.sqlite");
...
describe("matrix live qa scenarios", () => {
  beforeEach(() => {
    ...
    runMatrixQaMarketingClawCli.mockReset();
    startMatrixQaMarketingClawCli.mockReset();
  ...
  it("merges default and scenario-requested Matrix topology once per run", () => {
    expect(
      scenarioTesting.buildMatrixQaTopologyForScenarios({
        defaultRoomName: "MarketingClaw Matrix QA run",
        scenarios: [
      ...
      rooms: [
        {
          ...
          kind: "group",
          members: ["driver", "observer", "sut"],
          name: "MarketingClaw Matrix QA run",
          requireMention: true,
        },
        {
          key: "driver-dm",
  ...
  it("runs the Matrix E2EE state_after regression scenario through the gateway fault proxy", async () => {
    ...
    try {
      ...
      const result = await runMatrixQaScenario(scenario, {
        ...
        driverDeviceId: "DRIVERDEVICE",
        gatewayRuntimeEnv: {
          MARKETINGCLAW_CONFIG_PATH: gatewayConfigPath,
  ...
  it("runs Matrix self-verification through the interactive CLI command", async () => {
    ...
    try {
      ...
      startMatrixQaMarketingClawCli.mockReturnValue({
        args: ["matrix", "verify", "self", "--account", "cli"],
        endStdin,
        kill,
      ...
      runMatrixQaMarketingClawCli.mockImplementation(async ({ args, env, stdin }) => {
        if (!cliAccountConfigDuringRun && env.MARKETINGCLAW_CONFIG_PATH) {
          const cliConfig = JSON.parse(
            await readFile(String(env.MARKETINGCLAW_CONFIG_PATH), "utf8"),
          ) as {
            channels?: {
              matrix?: {
      ...
      const result = await runMatrixQaScenario(scenario, {
        ...
        driverPassword: "driver-password",
        gatewayRuntimeEnv: {
          MARKETINGCLAW_CONFIG_PATH: "/tmp/gateway-config.json",
          MARKETINGCLAW_STATE_DIR: "/tmp/gateway-state",
          PATH: process.env.PATH,
        },
        outputDir,
      });
      ...
      expect(startMatrixQaMarketingClawCli).toHaveBeenCalledTimes(1);
      expect(
        mockObjectArg(startMatrixQaMarketingClawCli, "startMatrixQaMarketingClawCli").args,
      ).toEqual(["matrix", "verify", "self", "--account", "cli", "--timeout-ms", "8000"]);
      expect(
        mockObjectArg(startMatrixQaMarketingClawCli, "startMatrixQaMarketingClawCli").timeoutMs,
      ).toBe(16_000);
      ...
      expect(registrationRequest?.deviceName).toBe(
        "MarketingClaw Matrix QA CLI Self Verification Owner",
      );
      if (
      ...
      expect(runMatrixQaMarketingClawCli).toHaveBeenCalledTimes(2);
      expect(runMatrixQaMarketingClawCli.mock.calls.map(([params]) => params.args)).toEqual([
        [
          "matrix",
          "verify",
          "backup",
      ...
      expect(mockObjectArg(runMatrixQaMarketingClawCli, "runMatrixQaMarketingClawCli").stdin).toBe(
        "encoded-recovery-key\n",
      );
      const cliEnv = mockObjectArg(startMatrixQaMarketingClawCli, "startMatrixQaMarketingClawCli")
        .env as Record<string, unknown>;
      expect(cliEnv?.MARKETINGCLAW_STATE_DIR).toContain("marketingclaw-matrix-cli-qa-");
      expect(cliEnv?.MARKETINGCLAW_CONFIG_PATH).toContain("marketingclaw-matrix-cli-qa-");
      const configPath = String(cliEnv?.MARKETINGCLAW_CONFIG_PATH);
      ...
      await expectPathMissing(String(cliEnv?.MARKETINGCLAW_STATE_DIR));
  ...
  it("runs Matrix account add --enable-e2ee through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaMarketingClawCli.mockImplementation(async ({ args, env }) => {
        if (env.MARKETINGCLAW_CONFIG_PATH) {
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "MarketingClaw Matrix QA CLI Account Add Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix encryption setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaMarketingClawCli.mockImplementation(async ({ args, env }) => {
        if (!initialAccountConfig && env.MARKETINGCLAW_CONFIG_PATH) {
          const initialConfig = JSON.parse(
            await readFile(String(env.MARKETINGCLAW_CONFIG_PATH), "utf8"),
          ) as {
            channels?: {
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "MarketingClaw Matrix QA CLI Encryption Setup Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix encryption setup idempotency through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "MarketingClaw Matrix QA CLI Encryption Idempotent Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix encryption setup bootstrap failure through the CLI QA scenario", async () => {
    ...
    try {
      ...
      const wait = vi
        .fn()
        .mockRejectedValue(new Error("marketingclaw matrix encryption setup exited 1"));
      const kill = vi.fn();
      startMatrixQaMarketingClawCli.mockReturnValue({
      ...
      expect(
        (
          mockObjectArg(startMatrixQaMarketingClawCli, "startMatrixQaMarketingClawCli")
            .env as Record<string, unknown>
        ).MARKETINGCLAW_CONFIG_PATH,
      ).toContain("marketingclaw-matrix-e2ee-setup-qa-");
      expect(output).toHaveBeenCalledTimes(1);
      expect(wait).toHaveBeenCalledTimes(1);
      expect(kill).toHaveBeenCalledTimes(1);
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "MarketingClaw Matrix QA CLI Encryption Failure Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix recovery-key setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "MarketingClaw Matrix QA CLI Recovery Key Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix invalid recovery-key setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(kill).toHaveBeenCalledTimes(1);
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "MarketingClaw Matrix QA CLI Invalid Recovery Key Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix multi-account encryption setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaMarketingClawCli.mockImplementation(async ({ args, env }) => {
        const configPath = String(env.MARKETINGCLAW_CONFIG_PATH);
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "MarketingClaw Matrix QA CLI Multi Account Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
    ...
    try {
      ...
      expect(
        registrationRequests.some(
          (request) =>
            request.deviceName === "MarketingClaw Matrix QA CLI Setup Gateway" &&
      ...
      expect(
        registrationRequests.some(
          (request) =>
            request.deviceName === "MarketingClaw Matrix QA CLI Setup Driver" &&
            request.registrationToken === "registration-token",
        ),
      ).toBe(true);
      expect(createPrivateRoom).toHaveBeenCalledWith({
