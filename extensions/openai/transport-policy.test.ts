...
describe("openai transport policy", () => {
  ...
  it("builds native turn state for direct OpenAI routes", () => {
    ...
    expect(state?.headers?.["x-marketingclaw-session-id"]).toBe("session-123");
    expect(state?.headers?.["x-marketingclaw-turn-id"]).toBe("turn-123");
    expect(state?.headers?.["x-marketingclaw-turn-attempt"]).toBe("2");
    expect(state?.metadata?.marketingclaw_session_id).toBe("session-123");
    expect(state?.metadata?.marketingclaw_turn_id).toBe("turn-123");
    expect(state?.metadata?.marketingclaw_turn_attempt).toBe("2");
    expect(state?.metadata?.marketingclaw_transport).toBe("websocket");
  });
  ...
  it("returns websocket session headers and cooldown for native routes", () => {
    ...
    expect(policy?.headers?.["x-marketingclaw-session-id"]).toBe("session-123");
