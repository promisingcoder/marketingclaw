...
import { formatCliCommand } from "marketingclaw/plugin-sdk/setup-tools";
...
const openAICodexOAuthOriginator = "marketingclaw";
const localManualFallbackDelayMs = 15_000;
const localManualFallbackGraceMs = 1_000;
const openAIAuthProbeUrl =
  "https://auth.openai.com/oauth/authorize?response_type=code&client_id=marketingclaw-preflight&redirect_uri=http%3A%2F%2Flocalhost%3A1455%2Fauth%2Fcallback&scope=openid+profile+email";
  ...
  if (/unsupported_country_region_territory/i.test(message)) {
    return createOpenAICodexOAuthError(
      ...
      [
        ...
        "If you normally use a proxy, verify HTTPS_PROXY, HTTP_PROXY, or ALL_PROXY is set for the MarketingClaw process and then retry `marketingclaw models auth login --provider openai`.",
      ].join(" "),
      error,
    );
  }
...
export async function loginOpenAICodexOAuth(params: {
  ...
  await prompter.note(
    isRemote
      ? [
          ...
          "If this MarketingClaw process can receive the browser callback, sign-in may finish automatically before you paste.",
        ].join("\n")
      : [
  ...
  try {
    ...
    await prompter.note(
      "Trouble with OAuth? See https://docs.marketingclaw.ai/start/faq",
      "OAuth help",
    );
