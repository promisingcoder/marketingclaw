...
import type { ResolvedTtsConfig } from "marketingclaw/plugin-sdk/agent-runtime";
import { AuthStorage, ModelRegistry } from "marketingclaw/plugin-sdk/agent-sessions";
import type { MarketingClawConfig } from "marketingclaw/plugin-sdk/config-contracts";
import { encodePngRgba, fillPixel } from "marketingclaw/plugin-sdk/media-runtime";
import {
  registerProviderPlugin,
  requireRegisteredProvider,
} from "marketingclaw/plugin-sdk/plugin-test-runtime";
import { runRealtimeSttLiveTest } from "marketingclaw/plugin-sdk/provider-test-contracts";
...
const LIVE_MODEL_ID = process.env.MARKETINGCLAW_LIVE_OPENAI_PLUGIN_MODEL?.trim() || "gpt-5.5";
const LIVE_IMAGE_MODEL = process.env.MARKETINGCLAW_LIVE_OPENAI_IMAGE_MODEL?.trim() || "gpt-image-2";
const LIVE_VISION_MODEL =
  process.env.MARKETINGCLAW_LIVE_OPENAI_VISION_MODEL?.trim() || "gpt-5.4-mini";
const liveEnabled = OPENAI_API_KEY.trim().length > 0 && process.env.MARKETINGCLAW_LIVE_TEST === "1";
...
function createLiveConfig(): MarketingClawConfig {
...
describeLive("openai plugin live", () => {
  ...
  it("lists voices and synthesizes audio through the registered speech provider", async () => {
    ...
    const audioFile = await speechProvider.synthesize({
      text: "MarketingClaw integration test OK.",
      cfg,
  ...
  it("streams realtime STT through the registered transcription provider", async () => {
    ...
    const cfg = createLiveConfig();
    const ttsConfig = createLiveTtsConfig();
    const phrase = "Testing MarketingClaw OpenAI realtime transcription integration test OK.";
