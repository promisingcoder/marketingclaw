...
function resolveSessionHeaders(params: {
  ...
  return {
    ...
    "x-marketingclaw-session-id": sessionId,
  };
}
...
export function resolveOpenAITransportTurnState(
  ...
  return {
    headers: {
      ...sessionHeaders,
      "x-marketingclaw-turn-id": turnId,
      "x-marketingclaw-turn-attempt": attempt,
    },
    metadata: {
      marketingclaw_session_id: sessionHeaders["x-marketingclaw-session-id"] ?? "",
      marketingclaw_turn_id: turnId,
      marketingclaw_turn_attempt: attempt,
      marketingclaw_transport: ctx.transport,
    },
  };
}
