...
function expectedResponseCreateEvent() {
  return expect.objectContaining({
    ...
    event_id: expect.stringMatching(/^marketingclaw-response-create-/),
  });
}
...
function expectedResponseCancelEvent() {
  return expect.objectContaining({
    ...
    event_id: expect.stringMatching(/^marketingclaw-response-cancel-/),
  });
}
...
describe("buildOpenAIRealtimeVoiceProvider", () => {
  ...
  it("adds MarketingClaw attribution headers to native realtime websocket requests", () => {
  ...
  it("returns browser-safe MarketingClaw attribution headers for native WebRTC offers", async () => {
    vi.stubEnv("MARKETINGCLAW_VERSION", "2026.3.22");
  ...
  it("resolves keychain OPENAI_API_KEY refs before creating browser sessions", async () => {
    vi.stubEnv("OPENAI_API_KEY", "keychain:marketingclaw:OPENAI_REALTIME_BROWSER_TEST");
  ...
  it("resolves and caches keychain OPENAI_API_KEY refs before creating bridges", async () => {
    vi.stubEnv("OPENAI_API_KEY", "keychain:marketingclaw:OPENAI_REALTIME_BRIDGE_TEST");
  ...
  it("does not resolve keychain refs during configured checks", () => {
    vi.stubEnv("OPENAI_API_KEY", "keychain:marketingclaw:OPENAI_REALTIME_CONFIGURED_TEST");
    const provider = buildOpenAIRealtimeVoiceProvider();
  ...
  it("fails closed when keychain refs cannot be resolved", async () => {
    vi.stubEnv("OPENAI_API_KEY", "keychain:marketingclaw:OPENAI_REALTIME_MISSING_TEST");
    resolveProviderAuthProfileApiKeyMock.mockResolvedValueOnce(undefined);
  ...
  it("emits tool calls from realtime conversation item done events", async () => {
    ...
    expect(onEvent).toHaveBeenCalledWith({
      direction: "server",
      type: "conversation.item.done",
      detail: "function_call name=marketingclaw_agent_consult",
    });
  });
