...
describe("active-memory plugin", () => {
  ...
  const api: any = {
    ...
    runtime: {
      agent: {
        runEmbeddedAgent,
        session: {
          resolveStorePath: vi.fn(() => "/tmp/marketingclaw-session-store.json"),
          ...
          patchSessionEntry: vi.fn(
            async (params: {
              ...
              await hoisted.updateSessionStore(
                "/tmp/marketingclaw-session-store.json",
  ...
  beforeEach(async () => {
    ...
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-active-memory-test-"));
    configFile = {
      plugins: {
        entries: {
          "active-memory": {
    ...
    plugin.register(api as unknown as MarketingClawPluginApi);
  });
  ...
  it("keeps subagent transcripts off disk by default by using a temp session file", async () => {
    ...
    expect(sessionFile).toMatch(/marketingclaw-active-memory-.*\/session\.jsonl$/);
