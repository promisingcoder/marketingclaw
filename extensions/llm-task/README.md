...
`marketingclaw.invoke --each`) without adding new MarketingClaw code per workflow.
...
This extension depends on MarketingClaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** MarketingClaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.marketingclaw/extensions` as a standalone plugin directory.
