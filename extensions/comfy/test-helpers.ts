...
export function buildComfyConfig(config: Record<string, unknown>): MarketingClawConfig {
  return {
    plugins: {
      entries: {
        comfy: { config },
...
export function buildLegacyComfyConfig(config: Record<string, unknown>): MarketingClawConfig {
  return {
    models: {
      providers: {
        comfy: config,
