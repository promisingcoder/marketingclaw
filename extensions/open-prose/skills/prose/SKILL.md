...
metadata: { "marketingclaw": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == MarketingClaw `sessions_spawn`
- **File I/O** == MarketingClaw `read`/`write`
- **Remote fetch** == MarketingClaw `web_fetch` (or `exec` with curl when POST is required)
