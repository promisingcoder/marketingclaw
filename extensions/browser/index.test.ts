...
type BrowserAutoEnableProbe = Parameters<MarketingClawPluginApi["registerAutoEnableProbe"]>[0];
...
describe("browser plugin", () => {
  ...
  it("bundles the browser automation skill with the plugin", () => {
    const manifest = JSON.parse(
      fs.readFileSync(path.join(__dirname, "marketingclaw.plugin.json"), "utf8"),
    ) as { skills?: string[] };
  ...
  it("registers a lazy browser control service", async () => {
    ...
    await service.start({ config: {}, stateDir: "/tmp/marketingclaw", logger: { warn: vi.fn() } });
    expect(runtimeApiMocks.createBrowserPluginService).not.toHaveBeenCalled();

    await service.stop({ config: {}, stateDir: "/tmp/marketingclaw", logger: { warn: vi.fn() } });
  ...
  it("declares setup auto-enable reasons for browser config surfaces", () => {
    ...
    expect(probe({ config: { browser: { defaultProfile: "marketingclaw" } }, env: {} })).toBe(
      "browser configured",
    );
    ...
    expect(
      probe({ config: { browser: { defaultProfile: "marketingclaw", enabled: false } }, env: {} }),
    ).toBeNull();
  });
});
