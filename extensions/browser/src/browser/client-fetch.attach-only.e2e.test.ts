...
import { clearRuntimeConfigSnapshot } from "marketingclaw/plugin-sdk/runtime-config-snapshot";
...
describe("browser client fetch attachOnly diagnostics", () => {
  ...
  it("does not suggest gateway restart when an attachOnly CDP endpoint hangs", async () => {
    tempHome = await createTempHomeEnv("marketingclaw-browser-client-fetch-live-");
    ...
    const configPath = path.join(tempHome.home, ".marketingclaw", "marketingclaw.json");
    await fs.writeFile(
      configPath,
      JSON.stringify(
        {
    ...
    try {
      ...
      expect(message).toContain("browser profile is external to MarketingClaw");
      expect(message).toContain("Restarting the MarketingClaw gateway will not launch it");
      expect(message).not.toContain("Restart the MarketingClaw gateway");
