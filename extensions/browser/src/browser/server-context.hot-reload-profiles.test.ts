...
describe("server-context hot-reload profiles", () => {
  ...
  it("forProfile hot-reloads newly added profiles from config", () => {
    // Start with only marketingclaw profile
    ...
    // 2. Simulate adding a new profile to config (like user editing marketingclaw.json)
  ...
  it("forProfile refreshes existing profile config after getRuntimeConfig cache updates", () => {
    ...
    mockState.cfgProfiles.marketingclaw = { cdpPort: 19999, color: "#FF4500" };
    ...
    expect(state.resolved.profiles.marketingclaw?.cdpPort).toBe(19999);
  });
  ...
  it("marks existing runtime state for reconcile when profile invariants change", () => {
    const cfg = getRuntimeConfig();
    const resolved = resolveBrowserConfig(cfg.browser, cfg);
    const marketingclawProfile = requireValue(
      resolveProfile(resolved, "marketingclaw"),
      "marketingclaw profile missing",
    );
    ...
    const runtime = requireValue(
      state.profiles.get("marketingclaw"),
      "marketingclaw runtime missing",
    );
  ...
  it("marks local managed runtime state for reconcile when profile headless changes", () => {
    ...
    expect(marketingclawProfile.headless).toBe(true);
    const state: BrowserServerState = {
      server: null,
      port: 18791,
      resolved,
    ...
    mockState.cfgProfiles.marketingclaw = {
      cdpPort: 18800,
      color: "#FF4500",
      headless: false,
    };
  ...
  it("marks local managed runtime state for reconcile when profile executablePath changes", () => {
    ...
    expect(marketingclawProfile.executablePath).toBe("/usr/bin/chrome-old");
    const state: BrowserServerState = {
      server: null,
      port: 18791,
      resolved,
  ...
  it("does not reconcile remote cdp runtime when only headless changes", () => {
    ...
    expect(remoteProfile.driver).toBe("marketingclaw");
    ...
    const runtime = requireValue(state.profiles.get("remote"), "remote runtime missing");
    expect(runtime.profile.driver).toBe("marketingclaw");
