...
vi.mock("../config/config.js", async () => {
  ...
  const loadConfig = () => ({
    browser: {
      ...
      profiles: {
        marketingclaw: { cdpPort: testPort + 1, color: "#FF4500" },
      },
    },
  });
  return {
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.MARKETINGCLAW_GATEWAY_PORT;
    process.env.MARKETINGCLAW_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.MARKETINGCLAW_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.MARKETINGCLAW_GATEWAY_PASSWORD;
