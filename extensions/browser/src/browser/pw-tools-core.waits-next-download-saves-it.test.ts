...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredMarketingClawTmpDir: vi.fn(() => "/tmp/marketingclaw"),
}));
...
let tmpDirModule: typeof import("../infra/tmp-marketingclaw-dir.js");
...
describe("pw-tools-core", () => {
  ...
  beforeAll(async () => {
    ...
    tmpDirModule = await import("../infra/tmp-marketingclaw-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredMarketingClawTmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredMarketingClawTmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredMarketingClawTmpDir.mockReturnValue("/tmp/marketingclaw");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "marketingclaw-browser-download-test-"),
    );
    try {
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredMarketingClawTmpDir.mockReturnValue("/tmp/marketingclaw-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "marketingclaw-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "marketingclaw-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredMarketingClawTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(outPath)).not.toBe(
      path.resolve(path.join(path.sep, "tmp", "marketingclaw-preferred", "downloads")),
    );
    expect(path.basename(outPath)).toContain(path.basename(res.path));
    expect(path.basename(outPath)).toMatch(/\.part$/);
    await expect(fs.readFile(res.path, "utf8")).resolves.toBe("download-content");
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "marketingclaw-preferred", "downloads")}${path.sep}`),
    );
  });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir(async (tempDir) => {
        ...
        tmpDirMocks.resolvePreferredMarketingClawTmpDir.mockReturnValue(tempDir);
