...
function localMarketingClawProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "marketingclaw",
...
function createLocalMarketingClawResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localMarketingClawProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveMarketingClawUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-reset-"));
    const profileDir = path.join(tempRoot, "marketingclaw");
    ...
    const ops = createLocalMarketingClawResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveMarketingClawUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-reset-no-own-"));
    const profileDir = path.join(tempRoot, "marketingclaw");
    fs.mkdirSync(profileDir, { recursive: true });
