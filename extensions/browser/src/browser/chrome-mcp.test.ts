  ...
  const callTool = vi.fn(async ({ name, arguments: args }: ToolCall) => {
    if (name === "list_pages") {
      const pageLines = [
        ...
        "2: https://github.com/promisingcoder/marketingclaw/pull/45318",
      ];
      if (createdPageOpen) {
...
describe("chrome MCP page parsing", () => {
  ...
  it("parses list_pages text responses when structuredContent is missing", async () => {
    ...
    expect(tabs).toEqual([
      ...
      },
      {
        targetId: "2",
        title: "",
        url: "https://github.com/promisingcoder/marketingclaw/pull/45318",
        type: "page",
      },
    ]);
  });
  ...
  it("redacts remote CDP URL secrets from attach failures", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-chrome-mcp-test-"));
