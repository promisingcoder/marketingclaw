...
vi.mock("marketingclaw/plugin-sdk/ssrf-runtime-internal", () => ({
...
vi.mock("../infra/tmp-marketingclaw-dir.js", () => ({
  resolvePreferredMarketingClawTmpDir: () => "/tmp/marketingclaw-browser-test",
}));
...
import {
  buildMarketingClawChromeLaunchArgs,
  getChromeWebSocketUrl,
  isChromeCdpReady,
  isChromeReachable,
  launchMarketingClawChrome,
  resolveMarketingClawUserDataDir,
  stopMarketingClawChrome,
} from "./chrome.js";
 ...
 * not exercise: launchMarketingClawChrome (with child_process.spawn mocked),
 * canRunCdpHealthCommand all branches, canOpenWebSocket failure,
 * stopMarketingClawChrome SIGKILL fallback, fs.exists() catch, default
 * profile name, buildMarketingClawChromeLaunchArgs branches, and friends.
 */
...
describe("chrome.ts internal", () => {
  ...
  describe("resolveMarketingClawUserDataDir", () => {
    it("falls back to the default profile name when none is supplied", () => {
      const dir = resolveMarketingClawUserDataDir();
      expect(dir.endsWith(path.join("marketingclaw", "user-data"))).toBe(true);
    });

    it("respects an explicit profile name", () => {
      const dir = resolveMarketingClawUserDataDir("my-profile");
      expect(dir.endsWith(path.join("my-profile", "user-data"))).toBe(true);
    });
  });

  describe("buildMarketingClawChromeLaunchArgs branches", () => {
    ...
    it("toggles headless args", () => {
      const args = buildMarketingClawChromeLaunchArgs({
    ...
    it("lets a request headless override beat env and profile headed settings", () => {
      const args = buildMarketingClawChromeLaunchArgs({
        ...
        env: { MARKETINGCLAW_BROWSER_HEADLESS: "0" },
      });
      expect(args).toContain("--headless=new");
      expect(args).toContain("--disable-gpu");
    });
  ...
  describe("fs.exists() catch branch", () => {
    it("treats a throwing fs.existsSync (for prefs files) as non-existent to force bootstrap", async () => {
      ...
      // exists() invocation inside launchMarketingClawChrome.
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const running = await launchMarketingClawChrome(resolved, profile);
  ...
  describe("launchMarketingClawChrome", () => {
    let tmpDir = "";

    beforeEach(async () => {
      tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), "marketingclaw-launch-"));
    });
    ...
    it("rejects a remote profile before attempting to spawn", async () => {
      ...
      await expect(launchMarketingClawChrome(makeResolved(), profile)).rejects.toThrow(
    ...
    it("returns structured no-display details before spawning headed Chrome", async () => {
      ...
      const error = await launchMarketingClawChrome(makeResolved(), profile, {
        platform: "linux",
        env: { DISPLAY: undefined, WAYLAND_DISPLAY: undefined },
    ...
    it.each([
      ...
      async ({ cdpUrl, configuredProbeHost }) => {
        ...
        await expect(launchMarketingClawChrome(makeResolved(), profile)).rejects.toThrow(portBusy);
    ...
    it("completes successfully when Chrome reports /json/version and CDP is reachable", async () => {
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const running = await launchMarketingClawChrome(makeResolved(), profile);
          expect(running.pid).toBe(4242);
    ...
    it("accepts a ready CDP diagnostic after the launch HTTP probe expires", async () => {
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          const port = new URL(baseUrl).port;
          const profile = makeProfile(Number(port));
          const running = await launchMarketingClawChrome(
    ...
    it("clears stale singleton locks even when the profile-in-use marker rolls out of the stderr tail", async () => {
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const userDataDir = resolveMarketingClawUserDataDir(profile.name);
        ...
        for (const testCase of [
          ...
          await withMockChromeCdpServer({
            ...
            run: async (baseUrl) => {
              ...
              const userDataDir = resolveMarketingClawUserDataDir(
                `${profile.name}-${testCase.pid}`,
              );
              ...
              try {
                await expect(
                  launchMarketingClawChrome(makeResolved(), profileWithUniqueName),
    ...
    it("throws with stderr hint + sandbox hint when CDP never becomes reachable", async () => {
      ...
      try {
        ...
        await expect(launchMarketingClawChrome(resolved, profile)).rejects.toThrow(
    ...
    it("keeps only a bounded UTF-8-safe newest stderr tail when launch fails after large stderr", async () => {
      ...
      let message = "";
      try {
        await launchMarketingClawChrome(makeResolved({ localLaunchTimeoutMs: 1 }), profile);
      } catch (err) {
    ...
    it("keeps early missing-display diagnostics for launch hints after the stderr tail rolls", async () => {
      ...
      try {
        ...
        let message = "";
        try {
          await launchMarketingClawChrome(
  ...
  describe("stopMarketingClawChrome SIGKILL fallback", () => {
    it("escalates to SIGKILL when CDP keeps reporting reachable past the deadline", async () => {
      ...
      await stopMarketingClawChrome(
        { proc, cdpPort: 12345 } as unknown as Parameters<typeof stopMarketingClawChrome>[0],
        1,
      );
  ...
  describe("launchMarketingClawChrome remaining branches", () => {
    it("skips decoration entirely when the profile is already decorated", async () => {
      ...
      const stageDir = await fsp.mkdtemp(path.join(os.tmpdir(), "marketingclaw-decorated-"));
      try {
        const profileName = path.basename(stageDir);
        const colorHex = "#FF4500";
        const colorInt = ((0xff << 24) | 0xff4500) >> 0;
        const userDataDir = path.join(resolveMarketingClawUserDataDir(profileName));
        ...
        const staged = resolveMarketingClawUserDataDir(path.basename(stageDir));
    ...
    it("falls back to the default color when profile.color is undefined", async () => {
      // Covers the `profile.color ?? DEFAULT_MARKETINGCLAW_BROWSER_COLOR` coalescing.
      vi.spyOn(fs, "existsSync").mockImplementation((p) => {
        const s = String(p);
        if (
          s.includes("Google Chrome") ||
    ...
    it("buffers stderr chunks when Chrome emits diagnostics while CDP comes up", async () => {
      ...
      const configDir = await fsp.mkdtemp(path.join(os.tmpdir(), "marketingclaw-redact-off-"));
      ...
      let message = "";
      try {
        await launchMarketingClawChrome(resolved, profile);
      } catch (err) {
        message = err instanceof Error ? err.message : String(err);
      }
      expect(message).toContain("Chrome stderr:");
    ...
    it("logs a warning when decorateMarketingClawProfile throws and still returns a running Chrome", async () => {
      // Covers the decoration catch branch (log.warn).
      const { decorateMarketingClawProfile } = await import("./chrome.profile-decoration.js");
      vi.spyOn(fs, "existsSync").mockImplementation((p) => {
        const s = String(p);
        if (
          s.includes("Google Chrome") ||
      ...
      const decorationSpy = vi
        .spyOn({ decorateMarketingClawProfile }, "decorateMarketingClawProfile")
        .mockImplementation(() => {
      ...
      const writeSpy = vi.spyOn(fs, "writeFileSync").mockImplementation((p) => {
        const s = String(p);
        if (s.endsWith(".marketingclaw-profile-decorated") || s.endsWith("Preferences")) {
  ...
  describe("launchMarketingClawChrome managed-proxy CDP bypass", () => {
    const makeLoopbackProfile = (cdpPort: number): ResolvedBrowserProfile =>
      ({
        name: "marketingclaw-bypass",
        color: "#FF4500",
        cdpPort,
        cdpUrl: `http://127.0.0.1:${cdpPort}`,
        cdpIsLoopback: true,
    ...
    it("surfaces loopbackMode=block as BrowserProfileUnavailableError without spawning Chrome", async () => {
      ...
      await expect(launchMarketingClawChrome(makeResolved(), profile)).rejects.toBeInstanceOf(
    ...
    it("does not register a bypass for a remote attachOnly CDP URL (loopback gate)", async () => {
      stubExecutableAndPrefsExist();
      // For this test we want launchMarketingClawChrome to reject before any
      ...
      const remoteProfile = {
        name: "marketingclaw-remote",
        color: "#FF4500",
        cdpPort: 19222,
        cdpUrl: "http://browserless.example.com:19222",
        cdpIsLoopback: false,
      } as unknown as ResolvedBrowserProfile;
      await expect(launchMarketingClawChrome(makeResolved(), remoteProfile)).rejects.toThrow(
        /is remote; cannot launch local Chrome/,
      );
