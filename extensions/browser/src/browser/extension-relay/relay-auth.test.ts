...
beforeEach(() => {
  stateDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-relay-auth-")));
...
describe("extension relay host-local secret", () => {
  ...
  it("gives different hosts (state dirs) different secrets", () => {
    ...
    const otherDir = fs.realpathSync(
      fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-relay-auth-2-")),
    );
    try {
      const b = ensureExtensionRelayToken({ ...process.env, MARKETINGCLAW_STATE_DIR: otherDir });
      expect(b).not.toBe(a);
    } finally {
