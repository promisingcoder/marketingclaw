...
import { createPluginSetupWizardStatus } from "marketingclaw/plugin-sdk/plugin-test-runtime";
...
describe("zalouser setup plugin", () => {
  it("builds setup status without an initialized runtime", async () => {
    const stateDir = await mkdtemp(path.join(os.tmpdir(), "marketingclaw-zalouser-setup-"));
