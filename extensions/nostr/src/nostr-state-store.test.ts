...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-nostr-"));
  process.env.MARKETINGCLAW_STATE_DIR = dir;
  resetPluginStateStoreForTests();
  setNostrRuntime({
    state: {
      openKeyedStore: (options: OpenKeyedStoreOptions) =>
        createPluginStateKeyedStoreForTests("nostr", {
          ...options,
          env: { ...process.env, MARKETINGCLAW_STATE_DIR: dir },
        }),
      resolveStateDir: (env, homedir) => {
        const stateEnv = env ?? process.env;
        const override = stateEnv.MARKETINGCLAW_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        const resolveHome = homedir ?? os.homedir;
        return path.join(resolveHome(), ".marketingclaw");
      },
    },
  } as PluginRuntime);
  try {
