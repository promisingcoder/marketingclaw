...
async function withMockedOpenAiMemoryPlugin<T>(params: {
  ...
  vi.resetModules();
  vi.doMock("marketingclaw/plugin-sdk/runtime-env", () => ({
...
describe("memory plugin e2e", () => {
  const { getDbPath, getTmpDir } = installTmpDirHarness({ prefix: "marketingclaw-memory-test-" });
  ...
  test("uses provider adapter auth when embedding apiKey is omitted", async () => {
    ...
    vi.resetModules();
    vi.doMock("marketingclaw/plugin-sdk/memory-core-host-engine-embeddings", () => ({
    ...
    try {
      ...
      expect(providerOptions.agentDir).toBe("/tmp/marketingclaw-agent");
      ...
      vi.doUnmock("marketingclaw/plugin-sdk/memory-core-host-engine-embeddings");
      vi.doUnmock("openai");
      vi.doUnmock("./lancedb-runtime.js");
      vi.resetModules();
    }
    ...
    await withMockedOpenAiMemoryPlugin({
      ...
      run: async (dynamicMemoryPlugin) => {
        ...
        try {
          ...
          await program.parseAsync(["node", "marketingclaw", "ltm", "list", "--limit", "+03"]);
  ...
  test("sanitizeForMemoryCapture strips sender label from real room-label envelope shapes", () => {
    ...
    expect(
      sanitizeForMemoryCapture(
        "[Discord MarketingClaw #dev channel id:456 +5m] Alice: I prefer dark mode",
      ),
    ).toBe("I prefer dark mode");
    expect(
      sanitizeForMemoryCapture("[Telegram MarketingClaw id:-100] Alice: I prefer dark mode"),
    ).toBe("I prefer dark mode");
  ...
  test("escapeMemoryForPrompt strips media attached annotations before escaping", () => {
    expect(
      escapeMemoryForPrompt(
        "User sent image [media attached: /Users/alex/.marketingclaw/media/photo.jpg (image/jpeg)] and said hello",
      ),
