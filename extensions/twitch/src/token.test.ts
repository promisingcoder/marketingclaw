...
describe("token", () => {
  const originalAccessToken = process.env.MARKETINGCLAW_TWITCH_ACCESS_TOKEN;
  ...
  afterEach(() => {
    ...
    if (originalAccessToken === undefined) {
      delete process.env.MARKETINGCLAW_TWITCH_ACCESS_TOKEN;
    } else {
      process.env.MARKETINGCLAW_TWITCH_ACCESS_TOKEN = originalAccessToken;
    }
  });
  ...
  describe("resolveTwitchToken", () => {
    ...
    it("should prioritize config token over env var (simplified config)", () => {
      process.env.MARKETINGCLAW_TWITCH_ACCESS_TOKEN = "oauth:env-token";
