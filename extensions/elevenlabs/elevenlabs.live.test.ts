...
describeLive("elevenlabs plugin live", () => {
  it("synthesizes speech through the registered provider with eleven_v3", async () => {
    ...
    const audioFile = await provider.synthesize({
      text: "MarketingClaw ElevenLabs eleven v three text to speech integration test OK.",
  ...
  it("transcribes synthesized speech through the media provider", async () => {
    const phrase = "Testing MarketingClaw ElevenLabs speech to text integration OK.";
    ...
    expect(normalized).toContain("marketingclaw");
    expect(normalized).toMatch(/(?:elevenlabs|11labs)/);
  }, 90_000);

  it("streams realtime STT through the registered transcription provider", async () => {
    const provider = buildElevenLabsRealtimeTranscriptionProvider();
    const phrase = "Testing MarketingClaw ElevenLabs realtime transcription integration OK.";
