 ...
 * `__MARKETINGCLAW_REDACTED__` literal anywhere in the output throws
 ...
 *
 * @module @marketingclaw/oc-path/emit
 */
...
export function emitMd(ast: MdAst, opts: EmitOptions = {}): string {
  ...
  if (mode === "roundtrip") {
    ...
    if (!acceptPreExisting && ast.raw.includes("__MARKETINGCLAW_REDACTED__")) {
      guardSentinel("__MARKETINGCLAW_REDACTED__", `${guardPath}/[raw]`);
    }
    return ast.raw;
  }
