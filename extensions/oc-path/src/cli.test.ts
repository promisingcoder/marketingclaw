/**
 * Smoke tests for the `marketingclaw path` CLI handlers.
 *
...
describe("marketingclaw path CLI", () => {
  ...
  describe("set", () => {
    ...
    it("CLI-S08 sets slash-deep JSONC paths and parsed JSON values", async () => {
      const filePath = join(workspaceDir, "marketingclaw.json");
      writeFileSync(
        filePath,
      ...
      await pathSetCommand(
        "oc://marketingclaw.json/gateway/auth/token",
      ...
      await pathSetCommand(
        "oc://marketingclaw.json/agents/list/0/tools/exec/security",
        "allowlist",
        { cwd: workspaceDir, json: true },
        rt2,
      );
