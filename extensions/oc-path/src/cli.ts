/**
 * `marketingclaw path` — shell access to the OcPath substrate verbs.
 *
...
export function registerPathCli(program: Command): void {
  const path = program
    ...
    .addHelpText("after", "\nDocs: https://docs.marketingclaw.ai/cli/path\n");
  ...
  // Bare `marketingclaw path` prints help and exits 0 (matches the core
  // applyParentDefaultHelpAction contract — see marketingclaw#73077).
  path.action(() => {
    path.outputHelp();
    process.exitCode = 0;
  });
