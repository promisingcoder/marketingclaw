...
vi.mock("marketingclaw/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<object>("marketingclaw/plugin-sdk/setup");
  return {
    ...actual,
...
vi.mock("marketingclaw/plugin-sdk/channel-config-schema", async () => {
  const actual = await vi.importActual<object>("marketingclaw/plugin-sdk/channel-config-schema");
  return {
    ...actual,
...
vi.mock("marketingclaw/plugin-sdk/webhook-ingress", async () => {
  const actual = await vi.importActual<object>("marketingclaw/plugin-sdk/webhook-ingress");
  return {
    ...actual,
...
vi.mock("./runtime.js", () => ({
  getSynologyRuntime: vi.fn(() => ({
    ...
    channel: {
      ...
      session: {
        resolveStorePath: vi.fn(() => "/tmp/marketingclaw/synology-chat-sessions.json"),
