...
export function createCopilotAgentHarness(
  ...
  >();
  // Maps MarketingClaw session id (from AgentHarnessAttemptParams.sessionId) to
  ...
  return {
    ...
    async runAttempt(params: AgentHarnessAttemptParams): Promise<AgentHarnessAttemptResult> {
      const attemptPromise = (async () => {
        ...
        const marketingclawSessionId =
          typeof params.sessionId === "string" ? params.sessionId : undefined;

        // Dogfood finding #4: reuse the SDK session across turns within
        // the same MarketingClaw session so that the GitHub Copilot agent runtime's prompt
        ...
        const compactionCleanupPending =
          marketingclawSessionId !== undefined &&
          hasPendingDeferredCompactionCleanup(marketingclawSessionId);
        const replayBlocked =
          marketingclawSessionId !== undefined &&
          (compactionCleanupPending || resetBlockedStoredSessions.has(marketingclawSessionId));
        const tracked =
          marketingclawSessionId && !replayBlocked
            ? trackedSessions.get(marketingclawSessionId)
            : undefined;
        const stored = marketingclawSessionId
          ? replayBlocked
            ? undefined
            : lookupStoredBinding(options?.sessionStore, marketingclawSessionId)
          : undefined;
        ...
        return runCopilotAttempt(effectiveParams, {
          pool,
          onSessionEstablished: marketingclawSessionId
            ? ({
                ...
                trackedSessions.set(marketingclawSessionId, {
                  sdkSessionId,
                ...
                registerStoredBinding(options?.sessionStore, marketingclawSessionId, {
                  schemaVersion: 2,
                  sdkSessionId,
                  compatKey: currentCompatKey,
                  compactKey: currentCompactKey,
                ...
                resetBlockedStoredSessions.delete(marketingclawSessionId);
              }
            : undefined,
          onDeferredCompaction: marketingclawSessionId
            ? ({
                abort,
                cleanup,
                sdkSessionId,
                ...
                const trackedBinding = trackedSessions.get(marketingclawSessionId);
                const storedBinding = lookupStoredBinding(
                  options?.sessionStore,
                  marketingclawSessionId,
                );
                ...
                resetBlockedStoredSessions.add(marketingclawSessionId);
                void cleanup.then((outcome) => {
                  const currentTracked = trackedSessions.get(marketingclawSessionId);
                  ...
                  if (stillOwnsTrackedSession) {
                    trackedSessions.delete(marketingclawSessionId);
                  }
                  if (stillOwnsStoredSession) {
                    deleteStoredBinding(options?.sessionStore, marketingclawSessionId);
                  }
                  if (stillOwnsTrackedSession || stillOwnsStoredSession) {
                    resetBlockedStoredSessions.add(marketingclawSessionId);
                  }
    ...
    async reset(params: AgentHarnessResetParams): Promise<void> {
      ...
      const tracked = trackedSessions.get(marketingclawSessionId);
      const stored = lookupStoredBinding(options?.sessionStore, marketingclawSessionId);
      resetBlockedStoredSessions.add(marketingclawSessionId);
      await abortDeferredCompactionCleanups(marketingclawSessionId);
      const currentStored = lookupStoredBinding(options?.sessionStore, marketingclawSessionId);
      const stillOwnsStoredSession =
        stored !== undefined && currentStored?.sdkSessionId === stored.sdkSessionId;
      if (stillOwnsStoredSession) {
        if (deleteStoredBinding(options?.sessionStore, marketingclawSessionId)) {
          resetBlockedStoredSessions.delete(marketingclawSessionId);
        }
      } else {
        resetBlockedStoredSessions.delete(marketingclawSessionId);
      ...
      if (trackedSessions.get(marketingclawSessionId)?.sdkSessionId === tracked.sdkSessionId) {
        trackedSessions.delete(marketingclawSessionId);
      }
      try {
    ...
    async compact(
      ...
      // The SDK owns Copilot history compaction. MarketingClaw only resumes
      // the tracked SDK session and calls the session-scoped RPC; durable
      // MarketingClaw session/transcript state stays in SQLite, with no marker
      ...
      if (hasPendingDeferredCompactionCleanup(marketingclawSessionId)) {
        return {
          ok: false,
          compacted: false,
      ...
      if (!compatibleTracked) {
        ...
        // needs the live SessionConfig with MarketingClaw hooks/tools, so preserve the
