...
import type { AnyAgentTool, SandboxContext } from "marketingclaw/plugin-sdk/agent-harness-runtime";
import { afterEach, describe, expect, it, vi } from "vitest";
import {
  createCopilotToolBridge,
  convertMarketingClawToolToSdkTool,
  supportsModelTools,
} from "./tool-bridge.js";
...
describe("supportsModelTools", () => {
  it("returns true for github-copilot and false otherwise", () => {
    ...
    expect(supportsModelTools("marketingclaw")).toBe(false);
...
describe("createCopilotToolBridge", () => {
  it("returns empty arrays for unsupported providers without calling the seam", async () => {
    const createMarketingClawCodingTools = vi.fn(async () => [makeTool()]);
    ...
    expect(createMarketingClawCodingTools).toHaveBeenCalledTimes(0);
  });

  it("allows vetted BYOK providers to expose model tools", async () => {
    const sourceTools = [makeTool()];
    const createMarketingClawCodingTools = vi.fn(async () => sourceTools);
    ...
    expect(createMarketingClawCodingTools).toHaveBeenCalledWith(
  ...
  it("forwards supported fields to injected createMarketingClawCodingTools", async () => {
    ...
    expect(createMarketingClawCodingTools).toHaveBeenCalledTimes(1);
  ...
  it("returns sdkTools and sourceTools with matching lengths", async () => {
    ...
    const result = await createCopilotToolBridge({
      agentId: "agent-1",
      createMarketingClawCodingTools: async () => sourceTools,
      modelId: "gpt-4o",
      modelProvider: "github-copilot",
      sessionId: "session-1",
    });
  ...
  it("compacts the Copilot tool surface behind tool_search controls when enabled", async () => {
    const createMarketingClawCodingTools = vi.fn(async (opts: unknown) => {
  ...
  it("compacts the Copilot tool surface behind code-mode exec/wait when enabled", async () => {
    const createMarketingClawCodingTools = vi.fn(async () => [
      makeTool({ name: "fake_hidden" }),
      makeTool({ name: "read" }),
    ]);
  ...
  it("throws when createMarketingClawCodingTools returns a non-array", async () => {
    await expect(
      createCopilotToolBridge({
        agentId: "agent-1",
        createMarketingClawCodingTools: async () => ({ tools: [] }) as never,
        modelId: "gpt-4o",
        modelProvider: "github-copilot",
        sessionId: "session-1",
      }),
    ).rejects.toThrow("createMarketingClawCodingTools must return an array");
  });

  it("throws when createMarketingClawCodingTools rejects and includes the cause", async () => {
    await expect(
      createCopilotToolBridge({
        agentId: "agent-1",
        createMarketingClawCodingTools: async () => {
  ...
  // F6: PI-parity tool context. The bridged MarketingClaw tools register
  ...
  describe("PI-parity attempt context (F6)", () => {
    function captureCall() {
      ...
      return {
        createMarketingClawCodingTools,
        getOpts: () =>
          (createMarketingClawCodingTools.mock.calls[0] as unknown[] | undefined)?.[0] as Record<
            string,
            unknown
          >,
      };
    ...
    it("forwards identity, owner/policy, and channel/routing fields from attemptParams", async () => {
      const { createMarketingClawCodingTools, getOpts } = captureCall();
    ...
    it("prefers the unscoped toolAuthProfileStore when building MarketingClaw tools", async () => {
      const { createMarketingClawCodingTools, getOpts } = captureCall();
  ...
  describe("sandbox forwarding (PR #86155 [P1])", () => {
    ...
    it("defaults sandbox to undefined and derives spawnWorkspaceDir from workspaceDir when no sandbox is passed (back-compat)", async () => {
      ...
      const opts = (createMarketingClawCodingTools.mock.calls[0] as unknown[] | undefined)?.[0] as {
        sandbox?: unknown;
    ...
    it("forwards an explicit sandbox and spawnWorkspaceDir verbatim to createMarketingClawCodingTools", async () => {
  ...
  // MarketingClaw attempt would suppress. These tests pin the contract.
  describe("tool-surface gating (PR #86155 [P1] round-6)", () => {
    it("short-circuits when attemptParams.disableTools is true and never calls createMarketingClawCodingTools", async () => {
      const createMarketingClawCodingTools = vi.fn(async () => [makeTool()]);
      const result = await createCopilotToolBridge({
        agentId: "agent-1",
    ...
    it("leaves the tool list unchanged when toolsAllow is undefined", async () => {
      ...
      const createMarketingClawCodingTools = vi.fn(async () => tools);
      const result = await createCopilotToolBridge({
        agentId: "agent-1",
  ...
  describe("tool-name aliases (PR #86155 [P1] round-7)", () => {
    ...
    it("does not discard lean-mode overrides after tool construction", async () => {
      const result = await createCopilotToolBridge({
        ...
        createMarketingClawCodingTools: async () => [makeTool({ name: "image_generate" })],
        modelId: "gpt-4o",
        modelProvider: "github-copilot",
        sessionId: "session-1",
      });
    ...
    it("keeps core tools available for glob allowlists", async () => {
      ...
      const options = (
        createMarketingClawCodingTools.mock.calls[0] as unknown[] | undefined
      )?.[0] as {
        toolConstructionPlan?: { includeMarketingClawTools?: boolean };
      };
      expect(options?.toolConstructionPlan?.includeMarketingClawTools).toBe(true);
    });
...
describe("convertMarketingClawToolToSdkTool", () => {
  it("throws on empty and non-string names", () => {
    expect(() => convertMarketingClawToolToSdkTool(makeTool({ name: "" as never }), {})).toThrow(
      "tool name must be a non-empty string",
    );
    expect(() => convertMarketingClawToolToSdkTool(makeTool({ name: 42 as never }), {})).toThrow(
      "tool name must be a non-empty string",
    );
  });
  ...
  it("throws on non-function execute", () => {
    expect(() =>
      convertMarketingClawToolToSdkTool(makeTool({ execute: "nope" as never }), {}),
  ...
  it("preserves name, description, and parameters exactly", () => {
    ...
    const result = convertMarketingClawToolToSdkTool(sourceTool, {});
  ...
  it("sets skipPermission: true so MarketingClaw's wrapped-tool internal enforcement handles permission decisions (PI-parity model)", () => {
    // Per the harness docs: every bridged MarketingClaw tool comes from
    // `createMarketingClawCodingTools`, which already wraps each tool with
    ...
    // for kind: "custom-tool" would either short-circuit MarketingClaw's
    // richer enforcement (allow-all) or block every call (reject-all).
    // Setting `skipPermission: true` lets the wrapped execute() run
    // MarketingClaw's hook with the right context — mirrors codex
    // (`extensions/codex/src/app-server/dynamic-tools.ts`).
    const result = convertMarketingClawToolToSdkTool(makeTool(), {}) as SdkTool & {
  ...
  it("marks every bridged tool as overridesBuiltInTool so MarketingClaw owns names that collide with Copilot CLI built-ins (edit/read/write/bash/...)", () => {
    // Real-world dogfood found that marketingclaw's createMarketingClawCodingTools
    ...
    for (const name of ["edit", "read", "write", "bash", "live_echo"]) {
      const result = convertMarketingClawToolToSdkTool(makeTool({ name }), {}) as SdkTool & {
  ...
  it("returns a failure result when the signal is already aborted", async () => {
    ...
    controller.abort();
    const sourceTool = makeTool();
    const sdkTool = convertMarketingClawToolToSdkTool(sourceTool, {
      abortSignal: controller.signal,
    });
  ...
  it("calls beforeExecute with the invocation context before execute", async () => {
    ...
    const sourceTool = makeTool();
    const sdkTool = convertMarketingClawToolToSdkTool(sourceTool, { beforeExecute });
  ...
  it("calls prepareArguments and passes the prepared args and toolCallId to execute", async () => {
    ...
    const sdkTool = convertMarketingClawToolToSdkTool(sourceTool, { onToolCompleted });
  ...
  it("converts single text content to an exact textResultForLlm", async () => {
    ...
    const sdkTool = convertMarketingClawToolToSdkTool(makeTool({}, sourceResult), {
      onAgentToolResult,
    });
  ...
  it("reports thrown tool failures to the private result observer", async () => {
    ...
    const onAgentToolResult = vi.fn();
    const sdkTool = convertMarketingClawToolToSdkTool(
      makeTool({
        execute: vi.fn(async () => {
          throw error;
        }),
  ...
  it("reports returned MarketingClaw error results to both tool observers", async () => {
    const onAgentToolResult = vi.fn();
    const onToolCompleted = vi.fn();
    const sourceResult = {
