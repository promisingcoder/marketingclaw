...
// Filtering them out of the MarketingClaw bootstrap injection avoids
...
/**
 * Loads MarketingClaw workspace bootstrap files (IDENTITY.md, SOUL.md,
...
export async function resolveCopilotWorkspaceBootstrapContext(params: {
  ...
  try {
    ...
    // SOUL.md / IDENTITY.md / .marketingclaw conventions live) onto the
...
export function renderCopilotWorkspaceBootstrapInstructions(
  ...
  const lines: string[] = [
    "MarketingClaw loaded these user-editable workspace files. Treat them as project/user context. The Copilot SDK loads AGENTS.md natively from its instruction directories, so AGENTS.md is not repeated here.",
    "",
    "# Project Context",
    "",
