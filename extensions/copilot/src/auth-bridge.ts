 ...
 *     use cases.
 *   - Falls back to MARKETINGCLAW_GITHUB_TOKEN, COPILOT_GITHUB_TOKEN,
 ...
 *     (`<marketingClawHome>/.marketingclaw/agents/<agentId>/copilot`, or
 *     `<agentDir>/copilot` when an agent directory is supplied) that
 *     respects `MARKETINGCLAW_HOME` for the home directory root.
 ...
 *   4. MARKETINGCLAW_GITHUB_TOKEN, then COPILOT_GITHUB_TOKEN, then
...
function resolveCopilotHome(args: {
  ...
  // "copilot" subdir so it cannot collide with MarketingClaw's own files
  ...
  const marketingClawHome = readString(args.env.MARKETINGCLAW_HOME);
  const rootHome = marketingClawHome ? resolve(marketingClawHome) : safeHomeDir(args.homeDir);
  // Per-agent isolation per proposal section 3.6:
  //   <marketingClawHome>/.marketingclaw/agents/<agentId>/copilot
  return resolve(join(rootHome, ".marketingclaw", "agents", args.agentId, "copilot"));
}
...
function readEnvTokenFallback(
  ...
  // MARKETINGCLAW_GITHUB_TOKEN is the harness-specific override and stays at
  ...
  const candidates: Array<{ name: string; value: string | undefined }> = [
    { name: "MARKETINGCLAW_GITHUB_TOKEN", value: readString(env.MARKETINGCLAW_GITHUB_TOKEN) },
