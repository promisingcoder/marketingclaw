 ...
 * MarketingClaw audit transcript that sits next to (but is distinct from) the
 * SDK's own session storage.
 *
 * The MarketingClaw shell (src/agents/command/attempt-execution.ts) already
 ...
 * assistant + toolResult) into the MarketingClaw transcript via the same
...
export function attachCopilotMirrorIdentity<T extends AgentMessage>(
  ...
  const existing = record["__marketingclaw"];
  const baseMeta =
  ...
  return {
    ...record,
    __marketingclaw: { ...baseMeta, [MIRROR_IDENTITY_META_KEY]: identity },
  } as unknown as T;
}
 ...
 * the MarketingClaw audit trail just misses the intermediate messages for
 * this turn.
 */
