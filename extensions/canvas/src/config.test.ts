...
describe("Canvas plugin config", () => {
  const originalSkipCanvasHost = process.env.MARKETINGCLAW_SKIP_CANVAS_HOST;

  afterEach(() => {
    if (originalSkipCanvasHost === undefined) {
      delete process.env.MARKETINGCLAW_SKIP_CANVAS_HOST;
    } else {
      process.env.MARKETINGCLAW_SKIP_CANVAS_HOST = originalSkipCanvasHost;
    }
  });
  ...
  it("honors truthy skip-canvas env values before host registration", () => {
    for (const value of ["1", "true", " yes ", "ON"]) {
      process.env.MARKETINGCLAW_SKIP_CANVAS_HOST = value;
