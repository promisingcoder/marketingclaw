...
describe("canvas documents", () => {
  it("builds entry urls for materialized path documents under managed storage", async () => {
    const stateDir = await mkdtemp(path.join(tmpdir(), "marketingclaw-canvas-documents-"));
    tempDirs.push(stateDir);
    const workspaceDir = await mkdtemp(
      path.join(tmpdir(), "marketingclaw-canvas-documents-workspace-"),
    );
    ...
    expect(document.entryUrl).toContain("/__marketingclaw__/canvas/documents/");
  ...
  it("normalizes nested local entrypoint urls", () => {
    ...
    expect(url).toBe("/__marketingclaw__/canvas/documents/cv_example/collection.media/index.html");
  });
  ...
  it("encodes special characters in hosted entrypoint path segments", () => {
    ...
    expect(url).toBe(
      "/__marketingclaw__/canvas/documents/cv_example/bundle%231/entry%2520point%3F.html",
    );
  });
  ...
  it("materializes inline html bundles as index documents", async () => {
    ...
    expect(document.entryUrl).toBe(`/__marketingclaw__/canvas/documents/${document.id}/index.html`);
  });
    ...
    expect(resolveCanvasDocumentAssets(document, { stateDir })).toEqual([
      {
        ...
        url: `/__marketingclaw__/canvas/documents/${document.id}/collection.media/audio.mp3`,
      },
    ]);
    expect(
    ...
    ).toEqual([
      {
        ...
        url: `http://127.0.0.1:19003/__marketingclaw__/canvas/documents/${document.id}/collection.media/audio.mp3`,
      },
    ]);
  });
  ...
  it("rejects traversal-style document ids in hosted canvas paths", async () => {
    ...
    expect(
      resolveCanvasHttpPathToLocalPath(
        "/__marketingclaw__/canvas/documents/../collection.media/index.html",
        { stateDir },
      ),
    ).toBeNull();
  });
  ...
  it("rejects malformed encoded hosted canvas document paths", async () => {
    ...
    expect(
      resolveCanvasHttpPathToLocalPath(
        `/__marketingclaw__/canvas/documents/${documentId}/%E0%A4%A.html`,
        { stateDir },
      ),
    ).toBeNull();
    expect(
      resolveCanvasHttpPathToLocalPath(
        `/__marketingclaw__/canvas/documents/${documentId}/%25E0%25A4%25A.html`,
        { stateDir },
      ),
