...
vi.mock("marketingclaw/plugin-sdk/channel-actions", async (importOriginal) => ({
  ...(await importOriginal<typeof import("marketingclaw/plugin-sdk/channel-actions")>()),
...
describe("Canvas tool", () => {
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      tempRoot = await mkdtemp(path.join(os.tmpdir(), "marketingclaw-canvas-tool-"));
  ...
  it("applies configured image limits to canvas snapshots", async () => {
    ...
    expect(imageResultParams?.path).toMatch(/marketingclaw-canvas-snapshot-.*\.png$/);
