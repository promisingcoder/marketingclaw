...
import { jsonResult, readStringParam } from "marketingclaw/plugin-sdk/channel-actions";
import type { MarketingClawConfig } from "marketingclaw/plugin-sdk/config-contracts";
import type { AnyAgentTool } from "marketingclaw/plugin-sdk/plugin-entry";
...
function buildWidgetDocument(title: string, widgetCode: string): string {
  ...
  const sizeReporter =
    ...
    'if(h&&h!==last){last=h;window.parent.postMessage({type:"marketingclaw:widget-size",height:h},"*");}};' +
