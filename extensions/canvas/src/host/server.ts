...
import { isTruthyEnvValue, type RuntimeEnv } from "marketingclaw/plugin-sdk/runtime-env";
...
<title>MarketingClaw Canvas</title>
<style>
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.marketingclawCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.marketingclawCanvasA2UIAction &&
        typeof window.marketingclawCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.marketingclawSendUserAction === "function";
  ...
  window.addEventListener("marketingclaw:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android MarketingClaw node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.marketingclawSendUserAction === "function"
        ? window.marketingclawSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.MARKETINGCLAW_SKIP_CANVAS_HOST)) {
    return true;
  }
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>MarketingClaw Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
