---
name: canvas
description: "Present HTML on connected MarketingClaw node canvases, navigate/eval/snapshot, and debug canvas host URLs."
metadata: { "marketingclaw": { "emoji": "🖼️" } }
---
...
Active config: `$MARKETINGCLAW_CONFIG_PATH` or `~/.marketingclaw/marketingclaw.json`.
...
4. Present the hosted URL: `/__marketingclaw__/canvas/<file>.html`.
5. Use `snapshot` when the user needs proof.

## URL shape

```text
http://<gateway-host>:<gateway.port>/__marketingclaw__/canvas/index.html
http://<gateway-host>:<gateway.port>/__marketingclaw__/canvas/games/snake.html
```

Path mapping:

- `/__marketingclaw__/canvas/index.html` -> `<canvas host root>/index.html`
- `/__marketingclaw__/canvas/games/snake.html` -> `<canvas host root>/games/snake.html`
