...
- Extension production code should import from `marketingclaw/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `marketingclaw.plugin.json` and the package
  `marketingclaw` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `marketingclaw/plugin-sdk/*`.
