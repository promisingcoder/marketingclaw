...
describe("mattermost monitor slash", () => {
  ...
  it("registers deduped default and native skill commands across teams", async () => {
    vi.stubEnv("MARKETINGCLAW_GATEWAY_PORT", "18888");
    ...
    resolveCallbackUrl.mockReturnValue("https://marketingclaw.test/slash");
    ...
    expect(firstRegistration).toEqual({
      ...
      callbackUrl: "https://marketingclaw.test/slash",
      commands: [
        { trigger: "ping", description: "ping" },
        {
          trigger: "oc_skill",
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "mattermost: slash commands registered (2 commands across 2 teams, callback=https://marketingclaw.test/slash)",
    );
  });

  it("falls back to the configured gateway port when the env port is out of range", async () => {
    vi.stubEnv("MARKETINGCLAW_GATEWAY_PORT", "65536");
