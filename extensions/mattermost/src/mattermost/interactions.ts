...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__marketingclaw_channel_id";
...
type InteractionCallbackConfig = Pick<MarketingClawConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "marketingclaw-mattermost-interactions")
    .update(botToken)
    .digest("hex");
}
