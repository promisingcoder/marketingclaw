// Shared User-Agent for xAI sidecar HTTP/WS requests; mirrors `formatMarketingClawUserAgent`.

import { MARKETINGCLAW_VERSION as PACKAGE_VERSION } from "marketingclaw/plugin-sdk/agent-harness-runtime";

const ORIGINATOR = "marketingclaw";
...
function resolveXaiUserAgentVersion(): string {
  ...
  const envVersion = trimToUndefined(process.env.MARKETINGCLAW_VERSION);
  if (envVersion) {
    return envVersion;
  }
  ...
  return (
    trimToUndefined(process.env.MARKETINGCLAW_SERVICE_VERSION) ??
...
// an empty record so the marketingclaw identity is not forwarded to the proxy.
