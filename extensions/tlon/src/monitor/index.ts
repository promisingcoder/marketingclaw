// Tlon plugin entrypoint registers its MarketingClaw integration.
...
import { asFiniteNumber } from "marketingclaw/plugin-sdk/string-coerce-runtime";
...
export async function monitorTlonProvider(opts: MonitorTlonOpts = {}): Promise<void> {
  ...
  const cfg = core.config.current() as MarketingClawConfig;
      ...
      if (senders.size > 0 && !senders.has(senderShip)) {
        runtime.log?.(
          `[tlon] ⚠️ SECURITY: Multiple users sharing DM session. ` +
            `Configure "session.dmScope: per-channel-peer" in MarketingClaw config.`,
        );
        ...
        if (!sharedSessionWarningSent && effectiveOwnerShip) {
          ...
          const warningMsg =
            ...
            `Fix: Add to your MarketingClaw config:\n` +
            `session:\n  dmScope: "per-channel-peer"\n\n` +
            `Docs: https://docs.marketingclaw.ai/concepts/session#secure-dm-mode`;
