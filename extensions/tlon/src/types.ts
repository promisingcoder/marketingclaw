...
function resolveTlonChannelConfig(cfg: MarketingClawConfig): TlonAccountConfig | undefined {
...
export function listTlonAccountIds(cfg: MarketingClawConfig): string[] {
  const base = resolveTlonChannelConfig(cfg);
  if (!base) {
    return [];
  }
