...
vi.mock("marketingclaw/plugin-sdk/provider-auth", async () => {
  const actual = await vi.importActual<typeof import("marketingclaw/plugin-sdk/provider-auth")>(
    "marketingclaw/plugin-sdk/provider-auth",
  );
  return {
    ...actual,
...
describe("DEEPINFRA_MODELS_URL", () => {
  it("points at /v1/openai/models with the marketingclaw sort + filter=with_meta gate", () => {
    expect(DEEPINFRA_MODELS_URL).toBe(
      "https://api.deepinfra.com/v1/openai/models?sort_by=marketingclaw&filter=with_meta",
    );
  });
});
...
describe("hasDeepInfraApiKey", () => {
  ...
  it("falls back to the auth-profile store when no env var is set", () => {
    ...
    expect(hasDeepInfraApiKey({ env: {}, agentDir: "/tmp/marketingclaw-agent" })).toBe(true);
  ...
  it("returns false when env is empty and the auth-profile store has no deepinfra profile", () => {
    ...
    expect(hasDeepInfraApiKey({ env: {}, agentDir: "/tmp/marketingclaw-agent" })).toBe(false);
...
describe("discoverDeepInfraModels (chat-only shim)", () => {
  ...
  it("fetches the marketingclaw-projection endpoint and parses chat-surface entries when an API key is configured", async () => {
