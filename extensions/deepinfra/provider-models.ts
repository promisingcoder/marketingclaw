...
export const DEEPINFRA_MODELS_URL = `${DEEPINFRA_BASE_URL}/models?sort_by=marketingclaw&filter=with_meta`;
...
// Static fallback. Chat rows live in marketingclaw.plugin.json (manifest-validated);
...
// `/v1/openai/models?sort_by=marketingclaw&filter=with_meta` projection is the
...
// `isProviderApiKeyConfigured` helper (covers SecretRef / `MARKETINGCLAW_LIVE_*`
// indirection too).
