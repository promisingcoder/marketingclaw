...
import { keepHttpServerTaskAlive, waitUntilAbort } from "marketingclaw/plugin-sdk/channel-outbound";
...
function spawnRaftBridge(params: {
  ...
  // Raft owns the fixed bridge command. MarketingClaw passes profile/loopback
...
export async function startRaftGatewayAccount(
  ...
  if (!ctx.channelRuntime) {
    throw new Error(
      "Raft requires MarketingClaw channel runtime support. Update MarketingClaw and retry.",
    );
  }
  ...
  const server = createServer((request, response) => {
    void (async () => {
      ...
      if (
        ...
        // Raft drains runtime activity after each wake pass. MarketingClaw has no
      ...
      // Raft owns wake metadata and its schema evolution. MarketingClaw accepts only
