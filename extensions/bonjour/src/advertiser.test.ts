...
function enableAdvertiserUnitMode(hostname = "test-host") {
  ...
  process.env.MARKETINGCLAW_MDNS_HOSTNAME = hostname;
}
...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    expect(gatewayCall?.[0]?.type).toBe("marketclaw-gw");
  ...
  it("honors truthy MARKETINGCLAW_DISABLE_BONJOUR values", async () => {
    enableAdvertiserUnitMode();
    process.env.MARKETINGCLAW_DISABLE_BONJOUR = "true";
  ...
  it("auto-disables Bonjour on Fly Machines without Docker sentinel files", async () => {
    ...
    process.env.FLY_APP_NAME = "marketingclaw-clawcks-test";
  ...
  it("honors explicit Bonjour opt-in inside detected containers", async () => {
    enableAdvertiserUnitMode();
    process.env.MARKETINGCLAW_DISABLE_BONJOUR = "0";
    vi.spyOn(fs, "existsSync").mockImplementation((filePath) => String(filePath) === "/.dockerenv");
  ...
  it("suppresses ciao self-probe retry console noise while advertising", async () => {
    ...
    try {
      ...
      console.log(
        "[test._marketclaw-gw._tcp.local.] failed probing with reason: Error: Can't probe for a service which is announced already. Received announcing for service test._marketclaw-gw._tcp.local.. Trying again in 2 seconds!",
      );
  ...
  it("defers probing recovery while a name conflict is still settling", async () => {
    ...
    await vi.advanceTimersByTimeAsync(10_000);
    listenerMap.get("name-change")?.("test-host (MarketingClaw) (2)");

    await vi.advanceTimersByTimeAsync(15_000);

    expect(createService).toHaveBeenCalledTimes(1);
    expectWarnContaining('name conflict resolved; newName="test-host (MarketingClaw) (2)"');
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("Mac (MarketingClaw)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
  ...
  it("falls back to marketingclaw when system hostname is invalid for DNS", async () => {
    // Allow advertiser to run in unit tests.
    delete process.env.VITEST;
    process.env.NODE_ENV = "development";
    delete process.env.MARKETINGCLAW_MDNS_HOSTNAME;
    ...
    const [gatewayCall] = createService.mock.calls as Array<[ServiceCall]>;
    expect(gatewayCall?.[0]?.hostname).toBe("marketingclaw");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("marketingclaw.local");
  ...
  it("truncates reported Kubernetes service name at the DNS label byte limit", async () => {
    ...
    expectDnsLabelByteLength(`${reportedHostname} (MarketingClaw)`, 64);
  ...
  it("truncates multi-byte hostname within DNS label byte limit", async () => {
    // 21 CJK characters = 63 bytes in UTF-8, adding " (MarketingClaw)" pushes over
  ...
  it("uses system hostname when MARKETINGCLAW_MDNS_HOSTNAME is unset", async () => {
    // Allow advertiser to run in unit tests.
    delete process.env.VITEST;
    process.env.NODE_ENV = "development";
    delete process.env.MARKETINGCLAW_MDNS_HOSTNAME;
