---
name: imsg
description: "Use the imsg CLI from MarketingClaw agents for iMessage/SMS DMs, groups, replies, reactions, polls, watching, and private-API actions."
...
Use `imsg` when an MarketingClaw agent must act through the user's local macOS Messages.app account: inspect iMessage/SMS history, choose the correct DM or group, send messages/files, reply, react, vote in polls, or use private-API iMessage features.

Do not use this skill for Telegram, Signal, WhatsApp, Discord, Slack, or for replying inside the current MarketingClaw conversation when the configured channel already routes the reply.
...
- Private API mode is strongly encouraged for MarketingClaw iMessage. It unlocks replies, precise tapbacks, effects, polls, attachment replies, read/typing actions, and group management. Basic mode is a fallback for reads plus plain text/file send.
- Full Disk Access for the process context that runs `imsg` or MarketingClaw; reads fail without Messages DB access.
...
Use the private API bridge for the native iMessage actions MarketingClaw users normally expect:
...
For MarketingClaw channel setup, check bridge availability early:
