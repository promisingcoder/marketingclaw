...
import { runCommandWithTimeout } from "marketingclaw/plugin-sdk/process-runtime";
...
import { detectBinary } from "marketingclaw/plugin-sdk/setup";
import {
...
function resolveIMessageNonMacHostError(
  ...
  return "iMessage via the default imsg CLI must run on macOS. Run MarketingClaw on the signed-in Messages Mac, or set channels.imessage.cliPath to an SSH wrapper that runs imsg on that Mac.";
}
...
// subcommand (added by marketingclaw/imsg#114, which lets a single bridge call
...
export async function probeIMessagePrivateApi(
  ...
  try {
    ...
    // marketingclaw/imsg#114. We do this even when the bridge is unavailable
