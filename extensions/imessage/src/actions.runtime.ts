...
async function withTempFile<T>(input: TempFileInput, fn: (path: string) => Promise<T>): Promise<T> {
  const dir = await mkdtemp(join(resolvePreferredMarketingClawTmpDir(), "marketingclaw-imessage-"));
...
export const imessageActionsRuntime = {
  ...
  async sendRichMessage(params: {
    ...
    // build that accepts `send-rich --file` (marketingclaw/imsg#114); callers
