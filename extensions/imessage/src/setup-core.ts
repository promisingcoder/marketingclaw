...
export const imessageDmPolicy = {
  ...
  resolveConfigKeys: (_cfg: MarketingClawConfig, accountId?: string) => {
  ...
  getCurrent: (cfg: MarketingClawConfig, accountId?: string) => {
...
function resolveIMessageCliPath(params: { cfg: MarketingClawConfig; accountId: string }) {
...
export const imessageCompletionNote = {
  ...
  lines: [
    "For the usual setup, run MarketingClaw on the Mac signed into Messages.",
    ...
    "After Private API setup, run `imsg launch`, then `marketingclaw channels status --probe`.",
    "Ensure MarketingClaw has Full Disk Access to Messages DB.",
...
export const imessageSetupStatusBase = {
  ...
  configuredScore: 1,
  unconfiguredScore: 0,
  resolveConfigured: ({ cfg, accountId }: { cfg: MarketingClawConfig; accountId?: string }) =>
