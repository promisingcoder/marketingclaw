 ...
 * MarketingClaw-owned tools, Codex native-tool fallback rules, sandbox shell shims,
...
import { resolveAgentDir } from "marketingclaw/plugin-sdk/agent-runtime";
import { isToolAllowed } from "marketingclaw/plugin-sdk/sandbox";
...
type MarketingClawCodingToolsOptions = NonNullable<
  Parameters<
    (typeof import("marketingclaw/plugin-sdk/agent-harness"))["createMarketingClawCodingTools"]
  >[0]
>;
type MarketingClawExecOptions = NonNullable<MarketingClawCodingToolsOptions["exec"]>;

/** Factory seam for constructing MarketingClaw runtime tools without eagerly loading agent-harness. */
export type MarketingClawCodingToolsFactory =
  (typeof import("marketingclaw/plugin-sdk/agent-harness"))["createMarketingClawCodingTools"];
type MarketingClawDynamicTool = ReturnType<MarketingClawCodingToolsFactory>[number];
type MarketingClawSandboxContext = Awaited<ReturnType<typeof resolveSandboxContext>>;
...
export type DynamicToolBuildParams = {
  ...
  sandbox: MarketingClawSandboxContext;
...
let marketingClawCodingToolsFactoryForTests: MarketingClawCodingToolsFactory | undefined;

/** Overrides the runtime tool factory for tests that need deterministic tool catalogs. */
export function setMarketingClawCodingToolsFactoryForTests(
  factory: MarketingClawCodingToolsFactory,
): void {
  marketingClawCodingToolsFactoryForTests = factory;
}

/** Clears the test-only runtime tool factory override. */
export function resetMarketingClawCodingToolsFactoryForTests(): void {
  marketingClawCodingToolsFactoryForTests = undefined;
}

/** Splits sandbox and run session keys so tool calls can bind to both scopes when needed. */
export function resolveMarketingClawCodingToolsSessionKeys(
  params: EmbeddedRunAttemptParams,
  sandboxSessionKey: string,
): Pick<MarketingClawCodingToolsOptions, "sessionKey" | "runSessionKey"> {
  return {
...
export async function buildDynamicTools(input: DynamicToolBuildParams) {
  ...
  const agentHarness = await import("marketingclaw/plugin-sdk/agent-harness");
  const createMarketingClawCodingTools =
    marketingClawCodingToolsFactoryForTests ?? agentHarness.createMarketingClawCodingTools;
  toolBuildStages.mark("load-agent-harness-tools");
  const sessionKeys = resolveMarketingClawCodingToolsSessionKeys(params, input.sandboxSessionKey);
  ...
  const allTools = createMarketingClawCodingTools({
    ...
    modelCompat:
      params.model.compat && typeof params.model.compat === "object"
        ? (params.model.compat as MarketingClawCodingToolsOptions["modelCompat"])
        : undefined,
  ...
  toolBuildStages.mark("create-marketingclaw-coding-tools");
...
export function shouldEnableCodexAppServerNativeToolSurface(
  ...
  // capability, so narrow MarketingClaw allowlists must fail closed rather than
...
/** Returns true when MarketingClaw policy requires the Node-owned exec/process tools instead. */
...
function canCodexAppServerNativeToolSurfaceHonorSandbox(
  sandbox: MarketingClawSandboxContext | undefined,
  ...
  // active MarketingClaw sandboxing must disable the native surface and route shell
...
/** Requires a Codex sandbox environment only when native tools must run inside MarketingClaw sandboxing. */
...
/** Projects a local MarketingClaw workspace cwd into the remote Codex app-server workspace root. */
export function mapCodexAppServerRemoteWorkspacePath(params: {
  ...
  if (!normalizedValue.startsWith(prefix)) {
    throw new Error(
      `Codex remoteWorkspaceRoot is configured but cwd ${params.value} is outside MarketingClaw workspace root ${params.localWorkspaceRoot}; refusing to send a gateway-local cwd to the remote Codex app-server.`,
    );
  }
...
/** Converts MarketingClaw sandbox networking into Codex's external-sandbox policy shape. */
export function resolveCodexExternalSandboxPolicyForMarketingClawSandbox(
  sandbox: MarketingClawSandboxContext | undefined,
): CodexSandboxPolicy {
  return {
    type: "externalSandbox",
    networkAccess: codexNetworkAccessForMarketingClawSandbox(sandbox) ? "enabled" : "restricted",
  };
}

function codexNetworkAccessForMarketingClawSandbox(
  sandbox: MarketingClawSandboxContext | undefined,
): boolean {
...
export function addSandboxShellDynamicToolsIfAvailable(
  filteredTools: MarketingClawDynamicTool[],
  allTools: MarketingClawDynamicTool[],
  input: DynamicToolBuildParams,
): MarketingClawDynamicTool[] {
  if (
  ...
  const sandboxExecTool: MarketingClawDynamicTool = {
    ...execTool,
    name: "sandbox_exec",
    description:
      "Run a shell command through MarketingClaw's configured sandbox backend for this session. Use when MarketingClaw sandboxing is active or when a command must execute in the sandbox backend, such as an SSH-backed sandbox or Docker container-path bind layout. Use Codex's native shell only when no MarketingClaw sandbox is active and native Code Mode is available.",
  ...
  const sandboxProcessTool: MarketingClawDynamicTool = {
    ...processTool,
    name: "sandbox_process",
    description:
      "Manage sandbox_exec sessions that were started through MarketingClaw's configured sandbox backend for this session: list, poll, log, write, send-keys, submit, paste, kill, clear, or remove. Use only for sandbox_exec follow-up; use Codex's native shell session handling only when no MarketingClaw sandbox is active and native Code Mode is available.",
  };
...
function addNodeShellDynamicToolsIfNeeded(
  ...
  const toolsToAppend: MarketingClawDynamicTool[] = [];
  if (
...
function createNodeExecDynamicTool(
  execTool: MarketingClawDynamicTool,
  ...
  return {
    ...
    description:
      "Run a shell command on the MarketingClaw configured remote node for this session. This tool always uses MarketingClaw host=node internally and follows the existing node exec approval and allowlist policy. Use node_process for follow-up on backgrounded node_exec sessions. Use Codex's native shell for local app-server work.",
...
function createNodeProcessDynamicTool(
  processTool: MarketingClawDynamicTool,
): MarketingClawDynamicTool {
  return {
    ...processTool,
    name: CODEX_NODE_PROCESS_DYNAMIC_TOOL_NAME,
    description:
      "Manage node_exec sessions that were started on the MarketingClaw configured remote node for this session: list, poll, log, write, send-keys, submit, paste, kill, clear, or remove. Use only for node_exec follow-up; use Codex's native shell session handling for local app-server work.",
  };
}
...
function hideNodeExecDynamicToolParameters(parameters: MarketingClawDynamicTool["parameters"]) {
...
): Pick<MarketingClawExecOptions, "host" | "node"> | undefined {
