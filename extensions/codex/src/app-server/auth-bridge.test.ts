...
vi.mock("marketingclaw/plugin-sdk/agent-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("marketingclaw/plugin-sdk/agent-runtime")>();
  return {
    ...actual,
    resolveApiKeyForProfile: async (
      ...
      if (params.forceRefresh || (oauthCredential.expires ?? 0) <= Date.now()) {
        ...
        if (refreshed?.access) {
          ...
          if (params.agentDir || process.env.MARKETINGCLAW_STATE_DIR) {
...
describe("bridgeCodexAppServerStartOptions", () => {
  it("preserves persisted provenance when preparing a supplied base store", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-codex-app-server-"));
  ...
  it("uses the native user Codex home for coexistence mode", async () => {
    await withTempDir("marketingclaw-codex-user-home-", async (root) => {
  ...
  it("applies native Codex CLI OAuth when no MarketingClaw auth profile exists", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-codex-app-server-"));
    const agentDir = path.join(root, "agent");
  ...
  it("answers refresh from native Codex CLI OAuth without persisting an MarketingClaw profile", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-codex-app-server-"));
    const agentDir = path.join(root, "agent");
    const codexHome = path.join(root, "codex-cli");
