...
import { buildCodexUserMcpServersThreadConfigPatch } from "marketingclaw/plugin-sdk/codex-mcp-projection";
import { listRegisteredPluginAgentPromptGuidance } from "marketingclaw/plugin-sdk/plugin-runtime";
...
export async function startOrResumeThread(params: {
  ...
  return await params.bindingStore.withLease(bindingIdentity, async () => {
    ...
    if (!binding && bindingIdentity.kind === "session" && bindingIdentity.sessionKey) {
      // Reset may rotate the MarketingClaw session while this plugin is unloaded. Only
...
function buildDefaultCollaborationInstructions(): string {
  ...
  // is null. MarketingClaw adds per-turn workspace instructions here, so preserve that
...
function buildCronCollaborationInstructions(): string {
  return [
    "This is an MarketingClaw cron automation turn. Apply these instructions only to this scheduled job; ordinary chat turns should stay in Codex Default mode.",
...
function buildHeartbeatCollaborationInstructions(): string {
  return [
    "This is an MarketingClaw heartbeat turn. Apply these instructions only to this heartbeat wake; ordinary chat turns should stay in Codex Default mode.",
    "When you are ready to end the heartbeat, prefer the structured `heartbeat_respond` tool so MarketingClaw can record the wake outcome and notification decision. If `heartbeat_respond` is not already available and `tool_search` is available, search for `heartbeat_respond`, load it, then call it. Use `notify=false` when nothing should visibly interrupt the user.",
...
export function buildDeveloperInstructions(
  ...
  const sections = [
    "You are a personal agent running inside MarketingClaw. MarketingClaw has dynamic tools for MarketingClaw-owned messaging, cron, sessions, media, gateway, and nodes.",
    ...
    "Use Codex native `spawn_agent` for Codex subagents. `spawn_agent` and the other native collaboration tools may be deferred: when `spawn_agent` is not directly listed, load it with `tool_search` before spawning. Use MarketingClaw `sessions_spawn` only for MarketingClaw or ACP delegation, never as a substitute for `spawn_agent`.",
...
function buildDeferredDynamicToolManifest(
  ...
  return `Deferred searchable MarketingClaw dynamic tools available: ${deferredToolNames.join(", ")}. Use \`tool_search\` to load exact callable specs before use.`;
}
...
function buildVisibleReplyInstruction(
  ...
  if (messageToolAvailable) {
    return "For the current source conversation, reply normally in your final assistant message; MarketingClaw will deliver it through the active source conversation. Use `message` only for explicit out-of-band sends, media/file sends, or sends to a different target.";
  }
  return "For the current source conversation, reply normally in your final assistant message; MarketingClaw will deliver it through the active source conversation.";
}
...
export function resolveCodexAppServerModelProvider(params: {
  ...
  if (!normalized || normalizedLower === "codex") {
    // `codex` is MarketingClaw's virtual provider; let Codex app-server keep its
  ...
  if (isCodexAppServerNativeAuthProfile(params) && normalizedLower === "openai") {
    // When MarketingClaw is forwarding ChatGPT/Codex OAuth, `openai` is Codex's
