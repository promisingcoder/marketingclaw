/**
 * Hosts the local MarketingClaw sandbox exec-server that Codex app-server native
...
const SANDBOX_EXEC_SERVERS = new Map<string, Promise<MarketingClawExecServer>>();
  ...
  await Promise.all(
    servers.map(async (entry) => {
      if (entry.status === "fulfilled") {
        ...
        await closeMarketingClawExecServer(entry.value);
      }
    }),
  );
}
...
export async function ensureCodexSandboxExecServerEnvironment(params: {
  ...
  if (!canExposeLocalExecServerToAppServer(params.appServerStartOptions)) {
    throw new Error(
      "MarketingClaw Codex exec-server uses a local loopback URL and cannot be registered with a remote Codex app-server.",
    );
  }
  const execServer = await acquireMarketingClawExecServer(params.sandbox);
  try {
    ...
    await releaseMarketingClawExecServer(execServer);
...
export async function releaseCodexSandboxExecServerEnvironment(
  ...
  if (server) {
    await releaseMarketingClawExecServer(server);
  }
}
...
async function acquireMarketingClawExecServer(
  sandbox: SandboxContext,
): Promise<MarketingClawExecServer> {
  const key = sandbox.runtimeId;
  while (true) {
    const existing = SANDBOX_EXEC_SERVERS.get(key);
    const promise = existing ?? startAndRememberMarketingClawExecServer(sandbox);
...
function startAndRememberMarketingClawExecServer(
  sandbox: SandboxContext,
): Promise<MarketingClawExecServer> {
  const created = startMarketingClawExecServer(sandbox);
  const key = sandbox.runtimeId;
...
async function startMarketingClawExecServer(
  sandbox: SandboxContext,
): Promise<MarketingClawExecServer> {
  ...
  if (!address || typeof address === "string") {
    throw new Error("MarketingClaw Codex exec-server did not bind to a TCP port.");
  }
  const environmentId = buildEnvironmentId(sandbox);
  const authPath = `/marketingclaw-${randomUUID()}`;
  const url = `ws://127.0.0.1:${(address as AddressInfo).port}${authPath}`;
  const execServer: MarketingClawExecServer = {
    authPath,
    closed: false,
    environmentId,
    refCount: 0,
...
async function releaseMarketingClawExecServer(execServer: MarketingClawExecServer): Promise<void> {
  ...
  await closeMarketingClawExecServer(execServer);
}

async function closeMarketingClawExecServer(execServer: MarketingClawExecServer): Promise<void> {
  if (execServer.closed) {
    return;
  }
...
function buildEnvironmentId(sandbox: SandboxContext): string {
  ...
  return `marketingclaw-sandbox-${hash}`;
}
...
function handleConnection(execServer: MarketingClawExecServer, socket: WebSocket): void {
...
async function dispatchRequest(
  ...
  switch (request.method) {
    ...
    default:
      throw new Error(`Unsupported MarketingClaw sandbox exec-server method: ${request.method}`);
  }
}
