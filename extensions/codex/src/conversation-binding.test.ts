...
import type { ExecApprovalsFile } from "marketingclaw/plugin-sdk/exec-approvals-runtime";
import { upsertSessionEntry } from "marketingclaw/plugin-sdk/session-store-runtime";
...
vi.mock("marketingclaw/plugin-sdk/exec-approvals-runtime", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("marketingclaw/plugin-sdk/exec-approvals-runtime")>();
  return {
    ...actual,
    loadExecApprovals: execApprovalsRuntimeMocks.loadExecApprovals,
  };
});
vi.mock("marketingclaw/plugin-sdk/agent-runtime", () => agentRuntimeMocks);
...
describe("codex conversation binding", () => {
  beforeEach(async () => {
    resetCodexTestBindingStore();
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-codex-binding-"));
  });
  ...
  it("rejects binding when configured exec auto mode may need unrouted human approvals", async () => {
    ...
    await expect(
      ...
      "MarketingClaw native Codex conversation binding cannot route interactive approvals yet",
    );
  ...
  it("blocks bound Codex app-server turns when the current MarketingClaw session is sandboxed", async () => {
    const sessionFile = path.join(tempDir, "session.jsonl");
    await writeTestConversationBinding(sessionFile, { threadId: "thread-1", cwd: tempDir });
    ...
    expect(result).toEqual({
      ...
      reply: {
        text: expect.stringContaining(
          "Codex-native Codex app-server conversation binding is unavailable because MarketingClaw sandboxing is active for this session.",
        ),
      },
    });
    expect(sharedClientMocks.getSharedCodexAppServerClient).not.toHaveBeenCalled();
  ...
  it("blocks bound Codex app-server turns when exec host=node is active", async () => {
    ...
    expect(result).toEqual({
      ...
      reply: {
        text: expect.stringContaining(
          "Codex-native Codex app-server conversation binding is unavailable because MarketingClaw exec host=node is active for this session.",
        ),
      },
    });
    expect(sharedClientMocks.getSharedCodexAppServerClient).not.toHaveBeenCalled();
  ...
  it("blocks bound Codex app-server turns when the binding agent uses node exec without a session key", async () => {
    ...
    expect(result?.reply?.text).toContain("MarketingClaw exec host=node is active");
    expect(sharedClientMocks.getSharedCodexAppServerClient).not.toHaveBeenCalled();
  });
  ...
  it("blocks bound Codex CLI node turns when the current MarketingClaw session is sandboxed", async () => {
    ...
    expect(result).toEqual({
      ...
      reply: {
        text: expect.stringContaining(
          "Codex-native Codex CLI node conversation binding is unavailable because MarketingClaw sandboxing is active for this session.",
        ),
      },
    });
  ...
  it("refreshes stale network-proxy bound app-server threads before the turn", async () => {
    ...
    await writeTestConversationBinding(sessionFile, {
      threadId: "thread-old",
      cwd: tempDir,
      networkProxyProfileName: "marketingclaw-network-stale",
