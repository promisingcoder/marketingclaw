...
import { resolveSessionAgentIds } from "marketingclaw/plugin-sdk/agent-runtime";
...
const NATIVE_CONVERSATION_INTERACTIVE_APPROVALS_UNAVAILABLE =
  "MarketingClaw native Codex conversation binding cannot route interactive approvals yet; use the Codex harness or explicit /acp spawn codex for that workflow.";
...
async function resolveConversationAppServerRuntime(params: {
  ...
  const runtime = resolveCodexAppServerRuntimeOptions({
    ...
    marketingClawSandboxActive: Boolean(sandboxForPolicy?.enabled),
  });
  return { execPolicy, runtime };
}

const CODEX_CONVERSATION_GLOBAL_STATE = Symbol.for("marketingclaw.codex.conversationBinding");
const CODEX_CONVERSATION_THREAD_DEVELOPER_INSTRUCTIONS =
  "This Codex thread is bound to an MarketingClaw conversation. Answer normally; MarketingClaw will deliver your final response back to the conversation.";
      ...
      if (request.method === "item/tool/call") {
        return {
          contentItems: [
            {
              type: "inputText",
              text: "MarketingClaw native Codex conversation binding does not expose dynamic MarketingClaw tools yet.",
            },
          ],
          success: false,
        };
...
function resolveConversationExecPolicy(params: {
  ...
  const agentId =
    ...
    (params.config
      ...
      : undefined);
  return resolveMarketingClawExecPolicyForCodexAppServer({
    config: params.config,
    agentId,
