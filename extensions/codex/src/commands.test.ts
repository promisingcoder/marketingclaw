...
import { saveSessionStore } from "marketingclaw/plugin-sdk/session-store-runtime";
...
function expectedDiagnosticsTargetBlock(params: {
  ...
  return [
    ...
    ...(params.sessionKey ? [`MarketingClaw session key: \`${params.sessionKey}\``] : []),
    ...(params.sessionId ? [`MarketingClaw session id: \`${params.sessionId}\``] : []),
...
describe("codex command", () => {
  beforeEach(async () => {
    resetCodexTestBindingStore();
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-codex-command-"));
  ...
  it("lists Codex sub-plugins through the /codex plugins command surface", async () => {
    ...
    expectResultTextContains(result, "marketingclaw.json");
  });
  ...
  it("enables and disables Codex sub-plugins through the /codex plugins command surface", async () => {
    ...
    expectResultTextContains(disabled, "google-calendar: disabled in marketingclaw.json");
    ...
    expectResultTextContains(enabled, "google-calendar: enabled in marketingclaw.json");
  ...
  it("attaches the current session to an existing Codex thread", async () => {
    ...
    await expect(
      ...
      text: "Attached this MarketingClaw session to Codex thread thread-123.",
    });
  ...
  it("reclaims an unloaded plugin's stale generation before attaching a thread", async () => {
    ...
    expect(result.text).toBe("Attached this MarketingClaw session to Codex thread thread-new.");
  ...
  it("does not report a resumed thread as attached after a generation conflict", async () => {
    ...
    expect(result.text).not.toContain("Attached this MarketingClaw session");
  });
  ...
  it.each([
    ...
    expect(result.text).toContain(
      "Codex-native /codex " +
        args.split(/\s+/u)[0] +
        " is unavailable because MarketingClaw sandboxing is active for this session.",
    );
    expect(codexControlRequest).not.toHaveBeenCalled();
  ...
  it.each([
    ...
    expect(result.text).toContain(
      "Codex-native /codex " +
        args.split(/\s+/u)[0] +
        " is unavailable because MarketingClaw exec host=node is active for this session.",
    );
    expect(codexControlRequest).not.toHaveBeenCalled();
    expect(steerCodexConversationTurn).not.toHaveBeenCalled();
    expect(setCodexConversationModel).not.toHaveBeenCalled();
  ...
  it("blocks config-level exec host=node without a session key", async () => {
    ...
    expect(result.text).toContain(
      "Codex-native /codex bind is unavailable because MarketingClaw exec host=node is active for this session.",
    );
  });
  ...
  it("explains compaction when no Codex thread is attached", async () => {
    ...
    await expect(
      ...
      text: "No Codex thread is attached to this MarketingClaw session yet.",
    });
  });
  ...
  it("asks before sending diagnostics feedback for the attached Codex thread", async () => {
    ...
    expect(safeCodexControlRequest).toHaveBeenCalledWith(
      ...
      CODEX_CONTROL_METHODS.feedback,
      {
        ...
        includeLogs: true,
        tags: {
          source: "marketingclaw-diagnostics",
          channel: "test",
        },
      },
      {
  ...
  it("uploads all Codex diagnostics sessions and reports their channel/thread breakdown", async () => {
    ...
    expect(request.text).toContain("MarketingClaw session key: `agent:first:whatsapp:one`");
    expect(request.text).toContain("MarketingClaw session id: `session-one`");
    expect(request.text).toContain("Codex thread id: `thread-111`");
    expect(request.text).toContain("MarketingClaw session key: `agent:second:discord:two`");
    expect(request.text).toContain("MarketingClaw session id: `session-two`");
  ...
  it("uses the host agent for diagnostics inventory sessions with unscoped keys", async () => {
    ...
    expect(request.text).toContain("MarketingClaw session key: `global`");
  ...
  it("sanitizes diagnostics upload errors before showing them", async () => {
    ...
    await expect(
      ...
      text: [
        ...
        "- channel test, MarketingClaw session session-1, Codex thread &lt;\uff20U123&gt;: bad??? &lt;\uff20U123&gt; \uff3btrusted\uff3d\uff08https://evil\uff09 \uff20here",
        "Inspect locally:",
  ...
  it("does not throttle diagnostics retries after upload failures", async () => {
    ...
    await expect(
      ...
      text: [
        "Could not send Codex diagnostics:",
        "- channel test, MarketingClaw session session-1, Codex thread thread-retry: temporary outage",
        "Inspect locally:",
  ...
  it("omits inline diagnostics resume commands for unsafe thread ids", async () => {
    ...
    await expect(
      ...
      text: [
        ...
        "Session 1",
        "Channel: test",
        "MarketingClaw session id: `session-1`",
