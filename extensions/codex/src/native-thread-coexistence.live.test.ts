...
const LIVE =
  process.env.MARKETINGCLAW_LIVE_TEST === "1" &&
  process.env.MARKETINGCLAW_LIVE_CODEX_THREAD_COEXISTENCE === "1";
...
describeLive("native Codex thread coexistence", () => {
  it("shares thread storage across independent app-server processes", async () => {
    await withTempDir("marketingclaw-codex-coexistence-", async (root) => {
      try {
        ...
        const started = await withClient(clientOptions, async (first) => {
          ...
          await first.request(
            "thread/name/set",
            { threadId: response.thread.id, name: "MarketingClaw coexistence source" },
            { timeoutMs: 60_000 },
          );
          return response;
        });
        ...
        const forked = await withClient(clientOptions, async (second) => {
          ...
          await second.request(
            "thread/name/set",
            { threadId: response.thread.id, name: "MarketingClaw coexistence fork" },
            { timeoutMs: 60_000 },
          );
          await second.request(
            "thread/archive",
          ...
          expect(finalList.data).toEqual(
            expect.arrayContaining([
              expect.objectContaining({
                id: forked.thread.id,
                name: "MarketingClaw coexistence fork",
              }),
            ]),
          );
        });
