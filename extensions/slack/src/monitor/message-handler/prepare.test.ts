...
vi.mock("marketingclaw/plugin-sdk/runtime-env", async (importOriginal) => {
  const actual = await importOriginal<typeof import("marketingclaw/plugin-sdk/runtime-env")>();
  return {
    ...actual,
...
describe("slack prepareSlackMessage inbound contract", () => {
  const storeFixture = createSlackSessionStoreFixture("marketingclaw-slack-thread-");
  ...
  function createThreadSlackCtx(params: { cfg: MarketingClawConfig; replies: unknown }) {
...
Second paragraph should still reach the agent after Slack's preview cutoff.`;
  ...
  it("records skipped no-mention room images as pending history media", async () => {
    ...
    try {
      const slackCtx = createInboundSlackCtx({
        cfg: { channels: { slack: { enabled: true } } } as MarketingClawConfig,
  ...
  it("keeps a root app mention and URL-only Slack thread follow-up on one parent session", async () => {
    ...
    slackCtx.resolveChannelName = async () => ({ name: "proj-marketingclaw", type: "channel" });
    ...
    const followUp = await prepareSlackMessage({
      ...
      message: {
        ...
        channel_type: "channel",
        user: "U_BEK",
        text: "https://github.com/promisingcoder/marketingclaw/issues/50621",
...
describe("prepareSlackMessage sender prefix", () => {
  function createSenderPrefixCtx(params: {
    ...
    return {
      cfg: {
        agents: {
          defaults: { model: "anthropic/claude-opus-4-5", workspace: "/tmp/marketingclaw" },
        },
  ...
  it("prefixes channel bodies with sender label and annotates Slack mention tokens", async () => {
    const ctx = createSenderPrefixCtx({
      channels: {},
      slashCommand: { command: "/marketingclaw", enabled: true },
    });
...
describe("slack thread.requireExplicitMention", () => {
  const storeFixture = createSlackSessionStoreFixture("marketingclaw-slack-explicit-mention-");
  ...
  it("drops thread reply without explicit mention when requireExplicitMention is true", async () => {
    ...
    const { storePath } = storeFixture.makeTmpStorePath();
    vi.spyOn(
      await import("marketingclaw/plugin-sdk/session-store-runtime"),
      "resolveStorePath",
