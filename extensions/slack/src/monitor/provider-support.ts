...
import { asOptionalRecord as asRecord } from "marketingclaw/plugin-sdk/string-coerce-runtime";
...
const MARKETINGCLAW_SLACK_CLIENT_PING_TIMEOUT_MS = 15_000;
const MARKETINGCLAW_SLACK_SOCKET_START_FAILED_EVENT = "unable_to_socket_mode_start";
const MARKETINGCLAW_SLACK_NATIVE_RECONNECT_OBSERVER_KEY =
  "__marketingclawNativeReconnectFailureObserver";
...
function installSlackNativeReconnectFailureObserver(receiver: unknown) {
  ...
  if (Reflect.get(client, MARKETINGCLAW_SLACK_NATIVE_RECONNECT_OBSERVER_KEY)) {
    return;
  }
  ...
  Reflect.set(client, MARKETINGCLAW_SLACK_NATIVE_RECONNECT_OBSERVER_KEY, true);
  Reflect.set(
    client,
    ...
    function patchedDelayReconnectAttempt(this: object, callback: unknown) {
      ...
      const delayMs =
        (Number.isFinite(pingTimeoutMs) && pingTimeoutMs >= 0
          ? pingTimeoutMs
          : MARKETINGCLAW_SLACK_CLIENT_PING_TIMEOUT_MS) * nextFailureCount;
        ...
        setTimeout(() => {
          ...
          Promise.resolve(callback.call(this)).then(resolve, (error: unknown) => {
            if (callback === Reflect.get(this, "start")) {
              emit.call(this, MARKETINGCLAW_SLACK_SOCKET_START_FAILED_EVENT, error);
              resolve(undefined);
              return;
            }
...
export function shouldSkipMarketingClawSlackSelfEvent(args: SlackSelfFilterArgs): boolean {
...
export function createSlackBoltApp(params: {
  ...
  const socketModeReceiverOptions: SlackSocketModeReceiverOptions = {
    ...
    clientPingTimeout:
      params.socketMode?.clientPingTimeout ?? MARKETINGCLAW_SLACK_CLIENT_PING_TIMEOUT_MS,
  ...
  const app = new params.interop.App({
    ...
    // consumes that promise, tripping MarketingClaw's fatal unhandled-rejection path.
    tokenVerificationEnabled: false,
    ...(receiver ? { receiver } : {}),
  });
  app.use(async (args) => {
    if (shouldSkipMarketingClawSlackSelfEvent(args)) {
      return;
    }
    await args.next();
  });
