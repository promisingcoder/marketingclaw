...
vi.mock("marketingclaw/plugin-sdk/target-resolver-runtime", async (orig) => {
...
vi.mock("marketingclaw/plugin-sdk/extension-shared", async (orig) => {
  const original = (await orig()) as Record<string, unknown>;
  return {
    ...original,
...
function makeMinimalSlackConfig(
  ...
  return { channels: { slack } } as MarketingClawConfig;
}
