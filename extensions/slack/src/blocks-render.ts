...
function readSlackMarketingClawBlockIndex(blockId: string, prefix: string): number | undefined {
...
/** Resolve existing MarketingClaw Block Kit indexes so appended controls keep stable unique IDs. */
export function resolveSlackInteractiveBlockOffsets(
  ...
  for (const block of blocks ?? []) {
    ...
    buttonIndexOffset = Math.max(
      buttonIndexOffset,
      readSlackMarketingClawBlockIndex(blockId, "marketingclaw_reply_buttons_") ?? 0,
    );
    selectIndexOffset = Math.max(
      selectIndexOffset,
      readSlackMarketingClawBlockIndex(blockId, "marketingclaw_reply_select_") ?? 0,
    );
  }
...
export function buildSlackInteractiveBlocks(
  ...
  return reduceInteractiveReply(interactive, initialState, (state, block) => {
    ...
    if (block.type === "buttons") {
      ...
      state.blocks.push({
        type: "actions",
        block_id: `marketingclaw_reply_buttons_${++state.buttonIndex}`,
        elements,
      });
      return state;
    }
    ...
    state.blocks.push({
      type: "actions",
      block_id: `marketingclaw_reply_select_${++state.selectIndex}`,
      elements: [
        {
          type: "static_select",
...
function buildSlackPresentationButtonBlock(
  ...
  return elements.length > 0
    ? {
        type: "actions",
        block_id: `marketingclaw_reply_buttons_${buttonIndex}`,
        elements,
      }
    : undefined;
}
...
function buildSlackPresentationSelectBlock(
  ...
  return options.length > 0
    ? {
        type: "actions",
        block_id: `marketingclaw_reply_select_${selectIndex}`,
        elements: [
          {
            type: "static_select",
