...
import { hasConfiguredSecretInput } from "marketingclaw/plugin-sdk/secret-input";
import {
...
function enableSlackAccount(cfg: MarketingClawConfig, accountId: string): MarketingClawConfig {
...
function hasSlackInteractiveRepliesConfig(cfg: MarketingClawConfig, accountId: string): boolean {
...
function createSlackTokenCredential(params: {
  ...
  return {
    ...
    inspect: ({ cfg, accountId }: { cfg: MarketingClawConfig; accountId: string }) => {
    ...
    applyUseEnv: ({ cfg, accountId }: { cfg: MarketingClawConfig; accountId: string }) =>
...
export function createSlackSetupWizardBase(handlers: {
  ...
  return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: MarketingClawConfig; accountId: string }) =>
        resolveSlackAccount({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: MarketingClawConfig; accountId: string }) =>
        Object.entries(resolveSlackAccount({ cfg, accountId }).config.channels ?? {})
          .filter(([, value]) => value?.enabled !== false)
          .map(([key]) => key),
      updatePrompt: ({ cfg, accountId }: { cfg: MarketingClawConfig; accountId: string }) =>
    ...
    disable: (cfg: MarketingClawConfig) => setSetupChannelEnabled(cfg, channel, false),
