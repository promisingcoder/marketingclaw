...
import { OPENROUTER_THINKING_STREAM_HOOKS } from "marketingclaw/plugin-sdk/provider-stream-family";
...
function mergeOpenRouterAuthHeaders(options: Parameters<StreamFn>[2]): Parameters<StreamFn>[2] {
  ...
  if (!headers.has("http-referer")) {
    headers.set("HTTP-Referer", "https://marketingclaw.ai");
  }
  if (!headers.has("x-openrouter-title")) {
    headers.set("X-OpenRouter-Title", "MarketingClaw");
  }
  return {
    ...options,
