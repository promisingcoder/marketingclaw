...
describe("qqbot message adapter", () => {
    ...
    const proofResults = await verifyChannelMessageAdapterCapabilityProofs({
      ...
      proofs: {
        ...
        media: async () => {
          ...
          expect(sent.mediaLocalRoots).toEqual(["/tmp/marketingclaw-sandbox"]);
  ...
  it("forwards scoped media access through outbound text and media sends", async () => {
    ...
    const mediaLocalRoots = ["/tmp/marketingclaw-sandbox"];

    sendTextMock.mockResolvedValueOnce({ messageId: "qq-text-media-1" });
    await qqbotPlugin.outbound?.sendText?.({
      cfg,
      to: "qqbot:c2c:user-1",
      text: "<qqmedia>/tmp/marketingclaw-sandbox/report.docx</qqmedia>",
      mediaAccess,
      mediaLocalRoots,
      mediaReadFile,
    });
    ...
    await qqbotPlugin.outbound?.sendMedia?.({
      ...
      to: "qqbot:c2c:user-1",
      text: "report",
      mediaUrl: "/tmp/marketingclaw-sandbox/report.docx",
      mediaAccess,
      mediaLocalRoots,
      mediaReadFile,
    });
    const sentMedia = latestMockArg(sendMediaMock, "sendMedia") as SentMediaParams;
    expect(sentMedia.mediaUrl).toBe("/tmp/marketingclaw-sandbox/report.docx");
