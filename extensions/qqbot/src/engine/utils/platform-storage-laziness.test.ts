...
describe("qqbot storage laziness", () => {
  ...
  it("does not create ~/.marketingclaw/qqbot from module imports or read-only probes", async () => {
    ...
    const qqbotRoot = path.join(homeDir, ".marketingclaw", "qqbot");
  ...
  it("creates storage when qqbot persists runtime state", async () => {
    ...
    const qqbotRoot = path.join(homeDir, ".marketingclaw", "qqbot");
    const sqlitePath = path.join(stateDir, "state", "marketingclaw.sqlite");
