 ...
 * silk-wasm availability checks — all without importing `marketingclaw/plugin-sdk`.
 ...
 * `MARKETINGCLAW_HOME`. QQ Bot still checks this tree for legacy state imports and
...
/**
 * Resolve the effective MarketingClaw home directory.
 *
 * Mirrors the contract from core (`src/infra/home-dir.ts::resolveEffectiveHomeDir`)
 * so QQ Bot media roots live under the same tree the rest of MarketingClaw treats as
 * `~`. The extension cannot import the core helper directly (it is a separate
 * package with `marketingclaw` as a peer dependency), so this re-implements the
 * minimal contract:
 *
 * 1. `MARKETINGCLAW_HOME` when set (with `~` / `~/...` expanded against the OS home).
 ...
 */
function resolveMarketingClawHome(): string {
  const raw = process.env.MARKETINGCLAW_HOME?.trim();
...
/**
 * Return a legacy path under `~/.marketingclaw/qqbot` without creating it.
 *
...
export function getQQBotDataPath(...subPaths: string[]): string {
  return path.join(getHomeDir(), ".marketingclaw", "qqbot", ...subPaths);
}

/** Return a path under `~/.marketingclaw/qqbot`, creating it on demand. */
...
/**
 * Return a path under `<marketingclaw-home>/.marketingclaw/media/qqbot` without creating it.
 *
 * Unlike `getQQBotDataPath`, this lives under MarketingClaw's core media allowlist
 * so downloaded images and audio can be accessed by framework media tooling.
 * The base honors `MARKETINGCLAW_HOME` (when set) so files written by agents into
 * the MarketingClaw-managed media tree are reachable by this plugin even when
 * `HOME` and `MARKETINGCLAW_HOME` differ (Docker, multi-user hosts). Fixes #83562.
 */
export function getQQBotMediaPath(...subPaths: string[]): string {
  return path.join(resolveMarketingClawHome(), ".marketingclaw", "media", "qqbot", ...subPaths);
}

/** Return a path under `<marketingclaw-home>/.marketingclaw/media/qqbot`, creating it on demand. */
...
/**
 * Return `<marketingclaw-home>/.marketingclaw/media`, MarketingClaw's shared media root.
 *
 * This mirrors the directory that core's `buildMediaLocalRoots` exposes as an
 * allowlisted location (see `marketingclaw/src/media/local-roots.ts`). Using it as a
 ...
 * {@link getQQBotMediaPath}, the base honors `MARKETINGCLAW_HOME`.
 */
function getMarketingClawMediaDir(): string {
  return path.join(resolveMarketingClawHome(), ".marketingclaw", "media");
}
...
export function resolveQQBotLocalMediaPath(p: string): string {
  ...
  const marketingclawHomeDir = resolveMarketingClawHome();
  const mediaRoot = getQQBotMediaPath();
  const dataRoot = getQQBotDataPath();
  // When MARKETINGCLAW_HOME differs from HOME we have to consider workspace roots
  // under both trees: agents may be configured with `~`-relative paths (HOME)
  // or with the MarketingClaw-managed home tree. Deduplicate when they match.
  const workspaceRoots = Array.from(
    new Set([
      path.join(osHomeDir, ".marketingclaw", "workspace", "qqbot"),
      path.join(marketingclawHomeDir, ".marketingclaw", "workspace", "qqbot"),
    ]),
  );
...
export function resolveQQBotPayloadLocalFilePath(p: string): string | null {
  ...
  // Trust both the QQ Bot-owned subdirectory and MarketingClaw's shared `~/.marketingclaw/media`
  ...
  const allowedRoots = [getMarketingClawMediaDir(), getQQBotMediaPath()];
