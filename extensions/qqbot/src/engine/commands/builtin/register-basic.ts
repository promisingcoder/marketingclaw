...
const QQBOT_PLUGIN_GITHUB_URL =
  "https://github.com/promisingcoder/marketingclaw/tree/main/extensions/qqbot";
const QQBOT_UPGRADE_GUIDE_URL = "https://q.qq.com/qqbot/marketingclaw/upgrade.html";
...
export function registerBasicBotCommands(registry: SlashCommandRegistry): void {
  ...
  registry.register({
    name: "bot-ping",
    description: "测试 MarketingClaw 与 QQ 之间的网络延迟",
    usage: [
      `/bot-ping`,
      ``,
      `测试当前 MarketingClaw 宿主机与 QQ 服务器之间的网络延迟。`,
      `返回网络传输耗时和插件处理耗时。`,
    ].join("\n"),
    handler: (ctx) => {
      const now = Date.now();
  ...
  registry.register({
    name: "bot-version",
    description: "查看 QQBot 插件版本和 MarketingClaw 框架版本",
    c2cOnly: true,
    usage: [`/bot-version`, ``, `查看当前 QQBot 插件版本和 MarketingClaw 框架版本。`].join("\n"),
    handler: async () => {
      const frameworkVersion = resolveRuntimeServiceVersion();
      const ver = getPluginVersionString();
      const lines = [
        `🦞 MarketingClaw 框架版本：${frameworkVersion}`,
        `🤖 QQBot 插件版本：v${ver}`,
