 ...
 * under `~/.marketingclaw/media/qqbot/downloads/` without appId subdivision.
...
export function registerClearStorageCommands(registry: SlashCommandRegistry): void {
  registry.register({
    ...
    handler: (ctx) => {
      ...
      const displayDir = `~/.marketingclaw/media/qqbot/downloads`;
      ...
      if (!isForce) {
        ...
        lines.push(
          ...
          `---`,
          ``,
          `确认清理后，上述保存在 MarketingClaw 运行主机磁盘上的文件将永久删除，后续对话过程中 AI 无法再找回相关文件。`,
          `‼️ 点击指令确认删除`,
