...
metadata: { "marketingclaw": { "emoji": "📸", "requires": { "config": ["channels.qqbot"] } } }
---
...
- 用户发来的**图片**会由 QQBot 运行时下载到 MarketingClaw 管理的 QQBot media 目录，路径只作为当前会话的附件上下文使用。
...
5. **本地路径仍受安全根限制**：只能发送当前会话授权的 agent workspace、scoped media roots、MarketingClaw 媒体目录或 QQBot 媒体目录内的文件；不要使用 `..` 逃出工作区。
