...
import * as providerAuth from "marketingclaw/plugin-sdk/provider-auth-runtime";
import * as providerHttp from "marketingclaw/plugin-sdk/provider-http";
import { expectExplicitVideoGenerationCapabilities } from "marketingclaw/plugin-sdk/provider-test-contracts";
...
describe("fal video generation provider", () => {
  ...
  it("submits HeyGen video-agent requests without unsupported fal controls", async () => {
    ...
    const result = await provider.generateVideo({
      ...
      prompt: "A founder explains MarketingClaw in a concise studio video",
      durationSeconds: 8,
      aspectRatio: "16:9",
      resolution: "720P",
      audio: true,
