...
describe("web logout", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsPromises.mkdtemp(
      path.join(os.tmpdir(), "marketingclaw-test-web-logout-"),
    );
    previousOAuthDir = process.env.MARKETINGCLAW_OAUTH_DIR;
    process.env.MARKETINGCLAW_OAUTH_DIR = path.join(fixtureRoot, "oauth");
    ({ logoutWeb } = await import("./auth-store.js"));
  });

  afterAll(async () => {
    if (previousOAuthDir === undefined) {
      delete process.env.MARKETINGCLAW_OAUTH_DIR;
    } else {
      process.env.MARKETINGCLAW_OAUTH_DIR = previousOAuthDir;
    }
  ...
  it("does not delete through symlinked auth dirs inside the MarketingClaw auth root", async () => {
  ...
  it("does not delete through intermediate symlinks inside the MarketingClaw auth root", async () => {
