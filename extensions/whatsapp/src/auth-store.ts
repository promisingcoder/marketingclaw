...
import { info, success } from "marketingclaw/plugin-sdk/runtime-env";
...
export async function pickWebChannel(
  ...
  if (!auth.exists) {
    throw new Error(
      `No WhatsApp Web session found. Run \`${formatCliCommand("marketingclaw channels login --channel whatsapp --verbose")}\` to link.`,
    );
  }
  return choice;
}
