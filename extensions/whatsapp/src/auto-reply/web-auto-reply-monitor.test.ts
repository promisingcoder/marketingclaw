...
beforeEach(async () => {
  sessionDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-group-gating-"));
...
const makeConfig = (overrides: Record<string, unknown>) =>
  ({
    ...
    ...overrides,
  }) as unknown as import("marketingclaw/plugin-sdk/config-contracts").MarketingClawConfig;
...
function makeInboundCfg(messagePrefix = "") {
  return {
    agents: { defaults: { workspace: "/tmp/marketingclaw" } },
...
describe("applyGroupGating", () => {
  ...
  it("does not treat group mention gating as self-chat under implicit self fallback", async () => {
    ...
    const { result, groupHistories } = await runGroupGating({
      ...
      msg: createGroupMessage({
        id: "g-other-mention",
        body: "@marketingclaw please check this",
