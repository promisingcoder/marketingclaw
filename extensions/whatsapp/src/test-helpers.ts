...
import { formatEnvelopeTimestamp } from "marketingclaw/plugin-sdk/channel-test-helpers";
...
const CONFIG_KEY = Symbol.for("marketingclaw:testConfigMock");
const SOURCE_CONFIG_KEY = Symbol.for("marketingclaw:testSourceConfigMock");
...
vi.mock("./auto-reply/monitor/message-line.runtime.js", () => ({
  ...
  resolveMessagePrefix: (
    ...
    return params?.hasAllowFrom === true ? "" : "[marketingclaw]";
  },
}));

vi.mock("./auth-store.runtime.js", () => ({
  resolveOAuthDir: () => "/tmp/marketingclaw-oauth",
}));

vi.mock("./session.runtime.js", () => {
  const created = createMockBaileys();
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("marketingclaw:lastSocket")] =
    created.lastSocket;
  return {
    ...created.mod,
  };
...
export function getLastSocket(): MockBaileysSocket {
  const getter = (globalThis as Record<PropertyKey, unknown>)[
    Symbol.for("marketingclaw:lastSocket")
  ];
  if (typeof getter === "function") {
