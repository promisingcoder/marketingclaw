...
import { createQueuedWizardPrompter } from "marketingclaw/plugin-sdk/plugin-test-runtime";
...
vi.mock("marketingclaw/plugin-sdk/setup", async () => {
  ...
  return {
    ...
    setSetupChannelEnabled: (cfg: MarketingClawConfig, channel: string, enabled: boolean) => ({
      ...cfg,
      channels: {
        ...cfg.channels,
        [channel]: {
          ...(cfg.channels?.[
            channel as keyof NonNullable<MarketingClawConfig["channels"]>
          ] as object),
          enabled,
        },
      },
