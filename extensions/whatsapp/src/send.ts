...
import { generateSecureUuid } from "marketingclaw/plugin-sdk/core";
import { redactIdentifier } from "marketingclaw/plugin-sdk/logging-core";
import {
  convertMarkdownTables,
  resolveMarkdownTableMode,
} from "marketingclaw/plugin-sdk/markdown-table-runtime";
import { requireRuntimeConfig } from "marketingclaw/plugin-sdk/plugin-config-runtime";
import { normalizePollInput, type PollInput } from "marketingclaw/plugin-sdk/poll-runtime";
import { createSubsystemLogger, getChildLogger } from "marketingclaw/plugin-sdk/runtime-env";
import {
...
function requireOutboundActiveWebListener(params: {
  ...
  if (!listener) {
    throw new Error(
      `No active WhatsApp Web listener (account: ${resolvedAccountId}). Start the gateway, then link WhatsApp with: ${formatCliCommand(`marketingclaw channels login --channel whatsapp --account ${resolvedAccountId}`)}.`,
    );
  }
...
export async function sendPollWhatsApp(
  to: string,
  poll: PollInput,
  options: { verbose: boolean; accountId?: string; cfg: MarketingClawConfig },
): Promise<{ messageId: string; toJid: string }> {
  const correlationId = generateSecureUuid();
  const startedAt = Date.now();
