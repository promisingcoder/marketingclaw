    ...
    collectIssues: ({ account, accountId, issues }) => {
      ...
      if (!linked) {
        issues.push({
          ...
          fix: `Run: ${formatCliCommand("marketingclaw channels login")} (scan QR on the gateway host).`,
        });
        return;
      }
      ...
      if (healthState === "stale") {
        ...
        issues.push({
          ...
          fix: `Run: ${formatCliCommand("marketingclaw doctor")} (or restart the gateway). If it persists, relink via channels login and check logs.`,
        });
        return;
      }
      ...
      if (
        ...
        issues.push({
          ...
          fix: `Watch: ${formatCliCommand("marketingclaw logs --follow")} and run ${formatCliCommand("marketingclaw channels status --probe")} if disconnects continue. If it keeps flapping, restart the gateway or relink via channels login.`,
        });
        return;
      }
