...
let MARKETINGCLAW_WHATSAPP_WEB_SOCKET_URL_ENV: typeof import("./session.js").MARKETINGCLAW_WHATSAPP_WEB_SOCKET_URL_ENV;
...
function createTempCaFile(contents: string): string {
  const dir = createTempAuthDir("marketingclaw-wa-proxy-ca");
...
describe("web session", () => {
  beforeAll(async () => {
    ({
      ...
      formatError,
      logWebSelfId,
      MARKETINGCLAW_WHATSAPP_WEB_SOCKET_URL_ENV,
      waitForWaConnection,
  ...
  it("creates WA socket with QR handler", async () => {
    const authDir = createTempAuthDir("marketingclaw-wa-creds-test");
  ...
  it("prints compact terminal QR output when requested", async () => {
    const authDir = createTempAuthDir("marketingclaw-wa-terminal-qr");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const authDir = createTempAuthDir("marketingclaw-wa-creds-symlink-runtime");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const rootDir = createTempAuthDir("marketingclaw-wa-authdir-symlink-runtime");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const rootDir = createTempAuthDir("marketingclaw-wa-auth-parent-symlink-runtime");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const authDir = createTempAuthDir("marketingclaw-wa-creds-symlink-save");
      const targetPath = path.join(authDir, "target-creds.json");
      const credsPath = path.join(authDir, "creds.json");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const rootDir = createTempAuthDir("marketingclaw-wa-creds-parent-symlink-save");
      const targetBaseDir = path.join(rootDir, "target-base");
      const linkedBaseDir = path.join(rootDir, "linked-base");
      const authDir = path.join(linkedBaseDir, "default");
      fsSync.mkdirSync(targetBaseDir);
  ...
  it("uses MARKETINGCLAW_WHATSAPP_WEB_SOCKET_URL as the default Baileys WebSocket URL", async () => {
    vi.stubEnv(MARKETINGCLAW_WHATSAPP_WEB_SOCKET_URL_ENV, " ws://127.0.0.1:49153/ws/chat ");
  ...
  it("ignores blank Baileys WebSocket URL environment overrides", async () => {
    vi.stubEnv(MARKETINGCLAW_WHATSAPP_WEB_SOCKET_URL_ENV, " ");
  ...
  it("rejects invalid MARKETINGCLAW_WHATSAPP_WEB_SOCKET_URL values", async () => {
    vi.stubEnv(MARKETINGCLAW_WHATSAPP_WEB_SOCKET_URL_ENV, "http://127.0.0.1:14567/ws");

    await expect(createWaSocket(false, false)).rejects.toThrow(
      "MARKETINGCLAW_WHATSAPP_WEB_SOCKET_URL must use ws:// or wss://.",
    );
  ...
  it("preserves explicit Baileys WebSocket URL options over invalid environment", async () => {
    vi.stubEnv(MARKETINGCLAW_WHATSAPP_WEB_SOCKET_URL_ENV, "http://127.0.0.1:49153/ws/chat");
  ...
  it("logWebSelfId prints cached E.164 when creds exist", () => {
    const authDir = createTempAuthDir("marketingclaw-wa-log-self");
    fsSync.writeFileSync(
  ...
  it("logWebSelfId prints cached lid details when creds include a lid", () => {
    const authDir = createTempAuthDir("marketingclaw-wa-log-self-lid");
    fsSync.writeFileSync(
      path.join(authDir, "creds.json"),
      JSON.stringify({
        me: {
  ...
  it("does not clobber creds backup when creds.json is corrupted", async () => {
    const authDir = createTempAuthDir("marketingclaw-wa-corrupt-backup");
  ...
  it("serializes creds.update saves to avoid overlapping writes", async () => {
    ...
    const authDir = createTempAuthDir("marketingclaw-wa-queue");
    const openMock = mockFsOpenForCredsWrites({
  ...
  it("lets different authDir queues flush independently", async () => {
    ...
    const authDirA = createTempAuthDir("marketingclaw-wa-a");
    const authDirB = createTempAuthDir("marketingclaw-wa-b");
    const onError = vi.fn();
  ...
  it("rotates creds backup when creds.json is valid JSON", async () => {
    const authDir = createTempAuthDir("marketingclaw-wa-rotate-backup");
    const credsPath = path.join(authDir, "creds.json");
    const backupPath = path.join(authDir, "creds.json.bak");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const authDir = createTempAuthDir("marketingclaw-wa-rotate-backup-symlink");
      const credsPath = path.join(authDir, "creds.json");
      const backupPath = path.join(authDir, "creds.json.bak");
  ...
  it("writes creds.json atomically via temp file and rename", async () => {
    ...
    try {
      await writeCredsJsonAtomically("/tmp/marketingclaw-oauth/whatsapp/default", {
        me: { id: "123@s.whatsapp.net" },
      });

      const write = firstWriteFileCall(openMock.writeFileSpy);
      expect(write.path).toContain(
        path.join("/tmp", "marketingclaw-oauth", "whatsapp", "default", ".creds."),
      );
      expect(typeof write.data).toBe("string");
      expect(write.options.mode).toBe(0o600);
      expect(write.options.flag).toBe("wx");
      ...
      expect(chmodSpy).toHaveBeenCalledWith(
        path.join("/tmp", "marketingclaw-oauth", "whatsapp", "default", "creds.json"),
        0o600,
      );
  ...
  it("keeps the previous creds.json valid if the atomic rename fails", async () => {
    const authDir = createTempAuthDir("marketingclaw-wa-creds-atomic");
    const credsPath = path.join(authDir, "creds.json");
