...
describe("WhatsApp auth dir profile resolution", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["MARKETINGCLAW_STATE_DIR", "MARKETINGCLAW_OAUTH_DIR"]);
    tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-wa-profile-"));
  ...
  it("resolves the default web auth dir from MARKETINGCLAW_STATE_DIR at call time", async () => {
