...
import { resolveChannelTtsVoiceDelivery } from "marketingclaw/plugin-sdk/channel-targets";
import type {
  MarketingClawConfig,
  ResolvedTtsPersona,
  TtsAutoMode,
...
import { transcodeAudioBuffer } from "marketingclaw/plugin-sdk/media-runtime";
...
import { isVerbose, logVerbose } from "marketingclaw/plugin-sdk/runtime-env";
import {
  tempWorkspaceSync,
...
import {
  ...
  resolveUserPath,
  truncateUtf16Safe,
} from "marketingclaw/plugin-sdk/text-utility-runtime";
import {
...
function resolveTtsPrefsPathValue(prefsPath: string | undefined): string {
  ...
  const envPath = process.env.MARKETINGCLAW_TTS_PREFS?.trim();
  if (envPath) {
...
function sortSpeechProvidersForAutoSelection(cfg?: MarketingClawConfig) {
...
function resolveTtsRuntimeConfig(cfg: MarketingClawConfig): MarketingClawConfig {
  return (
    selectApplicableRuntimeConfig({
      inputConfig: cfg,
