...
vi.mock("marketingclaw/plugin-sdk/channel-targets", () => ({
...
function createTtsConfig(prefsName: string): MarketingClawConfig {
  return {
    messages: {
      tts: {
        enabled: true,
...
describe("speech-core native voice-note routing", () => {
  ...
  it("tells generic TTS guidance to defer to MEMORY voice-delivery instructions", () => {
    const hint = buildTtsSystemPromptHint(
      createTtsConfig("marketingclaw-speech-core-tts-hint-test"),
    );
  ...
  it("marks Discord auto TTS replies as native voice messages", async () => {
    await expectTtsPayloadResult({
      channel: "discord",
      prefsName: "marketingclaw-speech-core-tts-test",
  ...
  it("keeps compatible audio-file synthesis deliverable as a voice memo", async () => {
    await expectTtsPayloadResult({
      ...
      prefsName: "marketingclaw-speech-core-tts-voice-memo-mp3-test",
  ...
  it("does not mark unsupported audio-file output as a voice memo", async () => {
    await expectTtsPayloadResult({
      channel: "voice-memo-chat",
      prefsName: "marketingclaw-speech-core-tts-voice-memo-ogg-test",
  ...
  it("pre-transcodes synthesized mp3 to opus-in-CAF when the host can satisfy preferAudioFileFormat", async () => {
    ...
    await expectTtsPayloadResult({
      channel: "voice-memo-chat",
      prefsName: "marketingclaw-speech-core-tts-voice-memo-caf-transcode-test",
  ...
  it("falls back to the original mp3 buffer when the host transcoder fails", async () => {
    ...
    await expectTtsPayloadResult({
      channel: "voice-memo-chat",
      prefsName: "marketingclaw-speech-core-tts-voice-memo-caf-fallback-test",
    ...
    const result = await synthesizeSpeech({
      ...
      cfg: {
        ...
        messages: {
          tts: {
            enabled: true,
            prefsPath: "/tmp/marketingclaw-speech-core-voice-model-default-test.json",
          },
        },
      } as MarketingClawConfig,
      disableFallback: true,
      ...
      cfg: {
        ...
        messages: {
          tts: {
            enabled: true,
            provider: "openrouter",
            prefsPath: "/tmp/marketingclaw-speech-core-explicit-model-alias-test.json",
            providers: {
              openrouter: {
    ...
    const result = await synthesizeSpeech({
      ...
      cfg: {
        ...
        messages: {
          tts: {
            enabled: true,
            prefsPath: "/tmp/marketingclaw-speech-core-voice-model-fallback-test.json",
          },
        },
      } as MarketingClawConfig,
    });
      ...
      cfg: {
        ...
        messages: {
          tts: {
            enabled: true,
            prefsPath:
              "/tmp/marketingclaw-speech-core-same-provider-voice-model-fallback-test.json",
          },
        },
      } as MarketingClawConfig,
    });
      ...
      cfg: {
        ...
        messages: {
          tts: {
            enabled: true,
            provider: "openai",
            prefsPath: "/tmp/marketingclaw-speech-core-realtime-voice-model-ignored-test.json",
          },
        },
      } as MarketingClawConfig,
      disableFallback: true,
    ...
    const result = await synthesizeSpeech({
      ...
      cfg: {
        ...
        messages: {
          tts: {
            enabled: true,
            prefsPath: "/tmp/marketingclaw-speech-core-supported-voice-model-provider-test.json",
          },
        },
      } as MarketingClawConfig,
    });
  ...
  it.each(["feishu", "whatsapp"] as const)(
    ...
    async (channel) => {
      ...
      await expectTtsPayloadResult({
        channel,
        prefsName: `marketingclaw-speech-core-tts-${channel}-mp3-test`,
  ...
  it("keeps non-native voice-note channels as regular audio files", async () => {
    await expectTtsPayloadResult({
      channel: "slack",
      prefsName: "marketingclaw-speech-core-tts-slack-test",
  ...
  it("synthesizes explicitly tagged short hidden TTS text", async () => {
    const cfg = createTtsConfig("marketingclaw-speech-core-short-hidden-tts-test");
    let mediaDir: string | undefined;
    try {
      const result = await maybeApplyTtsToPayload({
        payload: {
  ...
  it("truncates long TTS text on a UTF-16 boundary", async () => {
    const prefsName = "marketingclaw-speech-core-utf16-truncate-test";
  ...
  it("skips block delivery kind in final mode (accumulated final tail synthesizes instead)", async () => {
    synthesizeMock.mockClear();
    const cfg = createTtsConfig("marketingclaw-speech-core-block-kind-tts-test");
    const result = await maybeApplyTtsToPayload({
  ...
  it("skips tool delivery kind in final mode", async () => {
    synthesizeMock.mockClear();
    const cfg = createTtsConfig("marketingclaw-speech-core-tool-kind-tts-test");
    const result = await maybeApplyTtsToPayload({
  ...
  it("keeps skipping untagged short TTS text", async () => {
    const cfg = createTtsConfig("marketingclaw-speech-core-short-plain-tts-test");
    const result = await maybeApplyTtsToPayload({
      payload: {
        text: "hello",
        audioAsVoice: true,
  ...
  it("skips auto TTS for legacy final media directives", async () => {
    synthesizeMock.mockClear();
    const cfg = createTtsConfig("marketingclaw-speech-core-media-directive-tts-test");
    const result = await maybeApplyTtsToPayload({
  ...
  it("keeps skipping explicit tagged TTS text that strips to empty markdown", async () => {
    const cfg = createTtsConfig("marketingclaw-speech-core-empty-hidden-tts-test");
    const result = await maybeApplyTtsToPayload({
      payload: {
  ...
  it("selects persona preferred provider before config fallback", () => {
    ...
    const prefsPath = "/tmp/marketingclaw-speech-core-persona-provider.json";
    ...
    const cfg: MarketingClawConfig = {
      messages: {
        tts: {
          enabled: true,
          provider: "mock",
          prefsPath: "/tmp/marketingclaw-speech-core-persona-merge.json",
          providers: {
            mock: {
              model: "base-model",
              voice: "base-voice",
