...
import type { MarketingClawConfig } from "./config-utils.js";
...
const rootMemoryConfig = (workspaceDir: string): MarketingClawConfig =>
  ({
    agents: {
...
describe("resolveMemoryBackendConfig", () => {
  it("defaults to builtin backend when config missing", () => {
    const cfg = { agents: { defaults: { workspace: "/tmp/memory-test" } } } as MarketingClawConfig;
  ...
  it("preserves unquoted Windows absolute qmd command paths", () => {
    ...
    const cfg = {
      agents: { defaults: { workspace: String.raw`C:\Users\penny\.marketingclaw\workspace` } },
      memory: {
        backend: "qmd",
        qmd: {
          command,
