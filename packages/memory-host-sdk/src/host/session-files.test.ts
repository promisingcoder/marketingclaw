...
beforeEach(() => {
  ...
  Reflect.set(process.env, "MARKETINGCLAW_STATE_DIR", tmpDir);
  clearRuntimeConfigSnapshot();
  clearConfigCache();
});
...
describe("memory session sync targets", () => {
  it("parses deprecated canonical MarketingClaw transcript paths into sync identity", () => {
...
describe("buildSessionEntry", () => {
  it("returns lineMap tracking original JSONL line numbers", async () => {
    ...
    const jsonlLines = [
      ...
      JSON.stringify({ type: "custom", customType: "marketingclaw.cache-ttl", data: {} }),
