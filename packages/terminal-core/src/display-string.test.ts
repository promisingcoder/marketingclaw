...
function stubHome(home: string, marketingclawHome = ""): void {
  vi.stubEnv("HOME", home);
  vi.stubEnv("USERPROFILE", "");
  vi.stubEnv("MARKETINGCLAW_HOME", marketingclawHome);
}
...
describe("displayString", () => {
  ...
  it("uses MARKETINGCLAW_HOME as the display prefix", () => {
    const home = path.resolve("test-home", "alice");
    const marketingclawHome = path.resolve("test-marketingclaw-home");
    stubHome(home, marketingclawHome);

    expect(displayString(marketingclawHome)).toBe("$MARKETINGCLAW_HOME");
    expect(displayString(`${marketingclawHome}/state`)).toBe("$MARKETINGCLAW_HOME/state");
    expect(displayString(`${marketingclawHome}2/state`)).toBe(`${marketingclawHome}2/state`);
  });
});
