# MarketingClaw Android Versioning
...
```text
refs/marketingclaw/mobile-releases/android/<versionName>-<versionCode>
```

Example:

```text
refs/marketingclaw/mobile-releases/android/2026.6.10-2026061008
```
...
core MarketingClaw release machinery.
...
If `MATCH_PASSWORD` is not set, the existing manual Gradle-property signing path still works: provide `MARKETINGCLAW_ANDROID_STORE_FILE`, `MARKETINGCLAW_ANDROID_STORE_PASSWORD`, `MARKETINGCLAW_ANDROID_KEY_ALIAS`, and `MARKETINGCLAW_ANDROID_KEY_PASSWORD` through your local Gradle user properties before running release tasks.
