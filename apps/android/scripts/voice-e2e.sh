...
PACKAGE_NAME="ai.marketingclaw.app"
RECEIVER="$PACKAGE_NAME/.VoiceE2eReceiver"
RUN_ACTION="ai.marketingclaw.app.debug.RUN_VOICE_E2E"
OPEN_ACTION="ai.marketingclaw.app.debug.OPEN_VOICE_E2E"
PORT=18789
HOST="127.0.0.1"
MODE="both"
...
ARTIFACT_DIR="/tmp/marketingclaw-android-voice-e2e-$(date +%Y%m%d-%H%M%S)"
...
if [[ "$START_GATEWAY" -eq 1 ]]; then
  if command -v bws_get_secret >/dev/null 2>&1; then
    MARKETINGCLAW_OPENAI_API_KEY="$(bws_get_secret MARKETINGCLAW_OPENAI_API_KEY)"
  else
    MARKETINGCLAW_OPENAI_API_KEY="$(zsh -ic 'bws_get_secret MARKETINGCLAW_OPENAI_API_KEY')"
  fi
  (
    cd "$ROOT_DIR"
    OPENAI_API_KEY="$MARKETINGCLAW_OPENAI_API_KEY" \
      pnpm marketingclaw gateway run \
        --port "$PORT" \
        --auth none \
        --bind loopback \
        --force \
  ...
  unset MARKETINGCLAW_OPENAI_API_KEY
fi
...
adb logcat -d -v time |
  rg -i 'MarketingClaw|TalkMode|MicCapture|AudioRecord|SpeechRecognizer|realtime|talk.session|appendAudio|transcript|Talk failed|Transcription failed|Speech network|VoiceE2E' |
