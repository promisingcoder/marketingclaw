...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: MarketingClawProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: MarketingClawProtocol.AnyCodable] = [
            "presence": .init([MarketingClawProtocol.AnyCodable(entry)]),
        ]
        let payload = MarketingClawProtocol.AnyCodable(payloadMap)
