...
struct GatewayEnvironmentTests {
    @Test func `semver parses common forms`() {
        ...
        // Product-prefixed output from `marketingclaw --version` should NOT parse as semver
        // (the prefix must be stripped by the caller, not the parser).
        #expect(Semver.parse("MarketingClaw 2026.3.23-1") == nil)
    }

    @Test func `gateway version output strips product prefix before parsing`() {
        let normalized = GatewayEnvironment.normalizeGatewayVersionOutput("  MarketingClaw 2026.3.23-1 \n")
    ...
    @Test func `gateway version output strips trailing commit hash`() {
        let normalized = GatewayEnvironment.normalizeGatewayVersionOutput("MarketingClaw 2026.4.2 (d74a122)")
        ...
        let normalized2 = GatewayEnvironment.normalizeGatewayVersionOutput("MarketingClaw 2026.4.2-1 (d74a122)")
