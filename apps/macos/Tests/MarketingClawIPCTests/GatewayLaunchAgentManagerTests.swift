...
struct GatewayLaunchAgentManagerTests {
    @Test func `attach only runtime override does not uninstall gateway launch agent`() throws {
        let dir = FileManager().temporaryDirectory
            .appendingPathComponent("marketingclaw-attach-only-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func `launch agent plist snapshot parses args and env`() throws {
        let url = FileManager().temporaryDirectory
            .appendingPathComponent("marketingclaw-launchd-\(UUID().uuidString).plist")
        let plist: [String: Any] = [
            "ProgramArguments": ["marketingclaw", "gateway", "--port", "18789", "--bind", "loopback"],
            "EnvironmentVariables": [
                "MARKETINGCLAW_GATEWAY_TOKEN": " secret ",
                "MARKETINGCLAW_GATEWAY_PASSWORD": "pw",
            ],
        ]
    ...
    @Test func `launch agent plist snapshot merges canonical generated environment`() throws {
        let directory = FileManager().temporaryDirectory
            .appendingPathComponent("marketingclaw-launchd-env-\(UUID().uuidString)", isDirectory: true)
        let plistURL = directory.appendingPathComponent("ai.marketingclaw.gateway.plist")
        let environmentFileURL = directory.appendingPathComponent("ai.marketingclaw.gateway.env")
        let wrapperURL = directory.appendingPathComponent("ai.marketingclaw.gateway-env-wrapper.sh")
        ...
        export MARKETINGCLAW_GATEWAY_PASSWORD='service'\\''pass'
        export MARKETINGCLAW_GATEWAY_TOKEN=' service-token '
        ...
        let plist: [String: Any] = [
            ...
            "EnvironmentVariables": ["MARKETINGCLAW_GATEWAY_TOKEN": "stale-inline-token"],
        ]
        let data = try PropertyListSerialization.data(fromPropertyList: plist, format: .xml, options: 0)
    ...
    @Test func `launch agent plist snapshot ignores unreferenced generated environment`() throws {
        ...
        try FileManager().createDirectory(at: directory, withIntermediateDirectories: true)
        try "#!/bin/sh\n".write(to: wrapperURL, atomically: true, encoding: .utf8)
        try "export MARKETINGCLAW_GATEWAY_TOKEN='unreferenced-token'\n"
            .write(to: environmentFileURL, atomically: true, encoding: .utf8)
        let plist: [String: Any] = [
