...
struct HostEnvSanitizerTests {
    @Test func `sanitize blocks shell trace variables`() {
        ...
        #expect(env["PS4"] == nil)
        #expect(env["MARKETINGCLAW_TEST"] == "1")
    }
    ...
    @Test func `sanitize shell wrapper allows only explicit override keys`() {
        ...
        #expect(env["LANG"] == "C")
        #expect(env["LC_ALL"] == "C")
        #expect(env["MARKETINGCLAW_TOKEN"] == nil)
        #expect(env["PS4"] == nil)
    }

    @Test func `sanitize non shell wrapper keeps regular overrides`() {
        let env = HostEnvSanitizer.sanitize(overrides: ["MARKETINGCLAW_TOKEN": "secret"])
        #expect(env["MARKETINGCLAW_TOKEN"] == "secret")
    }
