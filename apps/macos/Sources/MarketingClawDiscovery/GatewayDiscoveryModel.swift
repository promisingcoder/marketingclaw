...
public final class GatewayDiscoveryModel {
    ...
    private let logger = Logger(subsystem: "ai.marketingclaw", category: "gateway-discovery")
    ...
    public func start() {
        ...
        for domain in MarketingClawBonjour.gatewayServiceDomains {
            let browser = GatewayDiscoveryBrowserSupport.makeBrowser(
                serviceType: MarketingClawBonjour.gatewayServiceType,
                domain: domain,
                queueLabelPrefix: "ai.marketingclaw.macos.gateway-discovery",
    ...
    public func refreshWideAreaFallbackNow(timeoutSeconds: TimeInterval = 5.0) {
        guard let domain = MarketingClawBonjour.wideAreaGatewayServiceDomain else { return }
    ...
    private func updateGateways(for domain: String) {
        ...
        if let wideAreaDomain = MarketingClawBonjour.wideAreaGatewayServiceDomain,
    ...
    private var hasUsableWideAreaResults: Bool {
        guard let domain = MarketingClawBonjour.wideAreaGatewayServiceDomain else { return false }
    ...
    private nonisolated static func prettifyInstanceName(_ decodedName: String) -> String {
        ...
        let stripped = normalized.replacingOccurrences(of: " (MarketingClaw)", with: "")
