...
struct MarketingClawMacCLI {
    static func main() async {
        ...
        case let .unknown(exitCode):
            fputs("marketingclaw-mac: unknown command\n", stderr)
            printUsage()
            exit(exitCode)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      marketingclaw-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
      ...
      marketingclaw-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      marketingclaw-mac connect
      marketingclaw-mac configure-remote --ssh-target user@gateway.local --remote-port 18789
      marketingclaw-mac connect --url ws://127.0.0.1:18789 --json
      marketingclaw-mac discover --timeout 3000 --json
      marketingclaw-mac wizard --mode local
    """)
}
