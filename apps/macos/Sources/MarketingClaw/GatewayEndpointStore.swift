...
actor GatewayEndpointStore {
    ...
    private static let staticLogger = Logger(subsystem: "ai.marketingclaw", category: "gateway-endpoint")
        ...
        launchdSnapshot: LaunchAgentPlistSnapshot?) -> String?
    {
        ...
        let raw = env["MARKETINGCLAW_GATEWAY_PASSWORD"] ?? ""
        ...
        if !trimmed.isEmpty {
                ...
                !configPassword.isEmpty
            {
                self.warnEnvOverrideOnce(
                    kind: .password,
                    envVar: "MARKETINGCLAW_GATEWAY_PASSWORD",
    ...
    {
        let serviceEnv = launchdSnapshot?.environment ?? [:]
        let raw = env["MARKETINGCLAW_GATEWAY_TOKEN"] ?? ""
        let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
        if !trimmed.isEmpty {
                ...
                configToken != trimmed
            {
                self.warnEnvOverrideOnce(
                    kind: .token,
                    envVar: "MARKETINGCLAW_GATEWAY_TOKEN",
    ...
    private let logger = Logger(subsystem: "ai.marketingclaw", category: "gateway-endpoint")
    ...
    init(deps: Deps = .live) {
        ...
        if let modeRaw {
            ...
            let seen = UserDefaults.standard.bool(forKey: "marketingclaw.onboardingSeen")
        ...
        let bind = GatewayEndpointStore.resolveGatewayBindMode(
            ...
            env: ProcessInfo.processInfo.environment)
        let customBindHost = GatewayEndpointStore.resolveGatewayCustomBindHost(root: MarketingClawConfigFile.loadDict())
    ...
    {
        if let envBind = env["MARKETINGCLAW_GATEWAY_BIND"] {
    ...
    private static func resolveGatewayScheme(
        ...
        env: [String: String]) -> String
    {
        if let envValue = env["MARKETINGCLAW_GATEWAY_TLS"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !envValue.isEmpty
        {
