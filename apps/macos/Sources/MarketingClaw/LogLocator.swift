...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["MARKETINGCLAW_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/marketingclaw")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("marketingclaw-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("marketingclaw-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/marketingclaw/ (rolling or stdout), or nil if none exist.
