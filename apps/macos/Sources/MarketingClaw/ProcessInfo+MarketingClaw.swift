...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.marketingclaw.mac`) even if the shipped
    ...
    {
        if environment["MARKETINGCLAW_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "marketingclaw.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "marketingclaw.nixMode") { return true }
