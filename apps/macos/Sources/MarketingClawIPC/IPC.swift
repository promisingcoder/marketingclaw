...
public struct CanvasShowResult: Codable, Sendable {
    /// Session directory on disk (e.g. `~/Library/Application Support/MarketingClaw/canvas/<session>/`).
...
public let controlSocketPath: String = {
    ...
    return home
        .appendingPathComponent("Library/Application Support/MarketingClaw/control.sock")
        .path
}()
