...
/// Serializes state-directory tests and pins `MARKETINGCLAW_STATE_DIR` to a fresh
...
/// (app group container or ~/Library/Application Support/MarketingClaw), so real
...
struct StateDirectoryIsolationTrait: TestTrait, TestScoping {
    ...
    private static let restoreStateDirPath: String = {
        if let raw = getenv("MARKETINGCLAW_STATE_DIR") {
        ...
        let quarantine = FileManager.default.temporaryDirectory
            .appendingPathComponent("marketingclaw-test-state-quarantine-\(UUID().uuidString)", isDirectory: true)
        ...
        performing function: @Sendable () async throws -> Void) async throws
    {
        ...
        let stateDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("marketingclaw-test-state-\(UUID().uuidString)", isDirectory: true)
        var thrown: (any Error)?
        do {
            try FileManager.default.createDirectory(at: stateDir, withIntermediateDirectories: true)
            setenv("MARKETINGCLAW_STATE_DIR", stateDir.path, 1)
            try await function()
        } catch {
            thrown = error
        }
        ...
        setenv("MARKETINGCLAW_STATE_DIR", restorePath, 1)
