...
@Suite struct ShareToAgentDeepLinkTests {
    @Test func appGroupIdentifierUsesCanonicalMarketingClawGroup() {
        #expect(MarketingClawAppGroup.canonicalIdentifier == "group.ai.marketingclaw.app.shared")
    }
    ...
    @Test func buildURLEncodesAgentRoute() {
        ...
        guard case let .agent(agent)? = parsed else {
            Issue.record("Expected marketingclaw://agent deep link")
            return
        }
