...
@Suite(.serialized) struct MarketingClawAppDelegateTests {
    @Test @MainActor func `resolves registry model before view task assigns delegate model`() {
        let registryModel = NodeAppModel()
        MarketingClawAppModelRegistry.appModel = registryModel
        defer { MarketingClawAppModelRegistry.appModel = nil }

        let delegate = MarketingClawAppDelegate()
    ...
    @Test @MainActor func `derives background refresh task identifier from app bundle identifier`() {
        let delegate = MarketingClawAppDelegate()
        let bundleIdentifier = Bundle.main.bundleIdentifier ?? "ai.marketingclaw.app.tests"
    ...
    @Test @MainActor func `stages a gateway URL when the model is ready`() async throws {
        MarketingClawAppModelRegistry.appModel = nil
        defer { MarketingClawAppModelRegistry.appModel = nil }
        let model = NodeAppModel()
        let delegate = MarketingClawAppDelegate()
        delegate.appModel = model
        let url = try #require(URL(
            string: "marketingclaw://gateway?host=gateway.example.com&port=443&tls=1&token=tok"))
