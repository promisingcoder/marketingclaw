...
@Suite struct VoiceWakePreferencesTests {
    @Test func sanitizeTriggerWordsTrimsAndDropsEmpty() {
        #expect(VoiceWakePreferences.sanitizeTriggerWords([" marketingclaw ", "", " \nclaude\t"]) == ["marketingclaw", "claude"])
    }
    ...
    @Test func displayStringUsesSanitizedWords() {
        #expect(VoiceWakePreferences.displayString(for: ["", " "]) == "marketingclaw, claude")
    }
