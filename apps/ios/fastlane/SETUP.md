# fastlane setup (MarketingClaw iOS)
...
APP_STORE_CONNECT_KEYCHAIN_SERVICE=marketingclaw-app-store-connect-key
...
```bash
APP_STORE_CONNECT_APP_IDENTIFIER=ai.marketingclaw.app
# or
...
Tip: run `scripts/ios-team-id.sh --require-canonical` from repo root to verify the canonical MarketingClaw iOS team (`FWJYW4S8P8`) is available locally. Fastlane uses the same canonical-only path when `IOS_DEVELOPMENT_TEAM` is missing, and rejects non-canonical teams for release archives.
...
For local/manual iOS builds that stay on direct APNs, configure the gateway host separately with `MARKETINGCLAW_APNS_TEAM_ID`, `MARKETINGCLAW_APNS_KEY_ID`, and either `MARKETINGCLAW_APNS_PRIVATE_KEY_P8` or `MARKETINGCLAW_APNS_PRIVATE_KEY_PATH`. Those gateway runtime env vars are separate from Fastlane's `.env`.
...
The screenshot lane runs the app with `--marketingclaw-screenshot-mode`, which enters the built-in connected screenshot fixture instead of pairing with a live gateway. By default it chooses one available large iPhone simulator and one available 13-inch iPad simulator from the installed Xcode runtime; override devices with a comma-separated `MARKETINGCLAW_SNAPSHOT_DEVICES` value when the requested simulators exist locally.
...
- confirm `apps/ios/build/app-store/MarketingClaw-<version>.ipa` exists
...
- The release flow regenerates `apps/ios/MarketingClaw.xcodeproj` from `apps/ios/project.yml` before archiving
...
- App Store release uses `MarketingClawPushMode=appStore`, which derives the canonical production hosted relay, production APNs, production relay profile, and `appleStrict` proof. The release lane rejects custom production relay URL overrides.
