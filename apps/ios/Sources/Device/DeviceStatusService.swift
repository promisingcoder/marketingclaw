...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    private func batteryStatus() -> MarketingClawBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: MarketingClawBatteryState = switch device.batteryState {
        ...
        return MarketingClawBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> MarketingClawThermalStatusPayload {
        let state: MarketingClawThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return MarketingClawThermalStatusPayload(state: state)
    }

    private func storageStatus() -> MarketingClawStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return MarketingClawStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
