...
final class MotionService: MotionServicing {
    func activities(params: MarketingClawMotionActivityParams) async throws -> MarketingClawMotionActivityPayload {
        ...
        let mapped: [MarketingClawMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
            manager.queryActivityStarting(from: start, to: end, to: OperationQueue()) { activity, error in
                if let error {
                    ...
                    let entries = sliced.map { entry in
                        MarketingClawMotionActivityEntry(
        ...
        return MarketingClawMotionActivityPayload(activities: mapped)
    }
        ...
        return try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    ...
                    let formatter = ISO8601DateFormatter()
                    let payload = MarketingClawPedometerPayload(
