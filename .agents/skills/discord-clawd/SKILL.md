---
name: discord-clawd
description: Use to talk to the Discord-backed MarketingClaw agent/session; not for archive search.
---
...
Use the MarketingClaw relay helper:

```bash
cd ~/Projects/agent-scripts
python3 skills/marketingclaw-relay/scripts/marketingclaw_relay.py targets
python3 skills/marketingclaw-relay/scripts/marketingclaw_relay.py resolve --target maintainers
```

If the target alias exists, prefer a private ask first:

```bash
python3 skills/marketingclaw-relay/scripts/marketingclaw_relay.py ask \
  --target maintainers \
