---
name: claw-score
description: Audit or refresh MarketingClaw maturity scorecard docs from root taxonomy, maturity scores, and QA evidence artifacts without using maintainer discrawl data or committed inventory reports.
---

# claw-score

Use this skill when working on the MarketingClaw maturity scorecard in this repo.
This is the marketingclaw-local version of the maintainer `claw-score` workflow:
...
`qa-evidence.json` artifact shape generated by MarketingClaw QA workflows.
...
Run from the marketingclaw repo root.
...
```bash
pnpm marketingclaw qa coverage --json
```
...
By default, Completeness measures how fully MarketingClaw exposes the intended
...
tree to marketingclaw. Evidence-enriched scorecard outputs belong in short-lived
