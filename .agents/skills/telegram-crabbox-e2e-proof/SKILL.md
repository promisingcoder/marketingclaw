...
Run from the MarketingClaw repo and branch under test:

```bash
proof_cmd="${MARKETINGCLAW_TELEGRAM_USER_PROOF_CMD:-marketingclaw-telegram-user-crabbox-proof}"
"$proof_cmd" start \
  --tdlib-url http://artifacts.marketingclaw.ai/tdlib-v1.8.0-linux-x64.tgz \
...
- starts a mock MarketingClaw Telegram SUT from the current checkout
...
"$proof_cmd" run \
  --session .artifacts/qa-e2e/telegram-user-crabbox/pr-review/session.json \
  -- bash -lc 'source /tmp/marketingclaw-telegram-user-crabbox/env.sh && python3 /tmp/marketingclaw-telegram-user-crabbox/user-driver.py transcript --limit 20 --json'
```

Useful remote user-driver commands:

```bash
source /tmp/marketingclaw-telegram-user-crabbox/env.sh
python3 /tmp/marketingclaw-telegram-user-crabbox/user-driver.py status --json
python3 /tmp/marketingclaw-telegram-user-crabbox/user-driver.py chats --json
python3 /tmp/marketingclaw-telegram-user-crabbox/user-driver.py transcript --limit 20 --json
python3 /tmp/marketingclaw-telegram-user-crabbox/user-driver.py send --text '/status@{sut}'
python3 /tmp/marketingclaw-telegram-user-crabbox/user-driver.py probe --text '@{sut} Reply exactly: USER-E2E-{run}' --expect USER-E2E-
```
