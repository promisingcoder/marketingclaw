---
name: clawsweeper
description: "Use for all ClawSweeper work: MarketingClaw issue/PR sweep reports, commit-review reports, repair jobs, cloud fix PRs, @clawsweeper maintainer mention commands, trusted ClawSweeper-reviewed autofix/automerge, GitHub Actions monitoring, permissions, gates, and manual backfills."
---

# ClawSweeper

ClawSweeper lives at `~/Projects/clawsweeper`. It is the one MarketingClaw
...
- Target app permissions: read target scan context; write issues and pull
  ...
  inputs; Actions write on `marketingclaw/clawsweeper` for comment-router
...
```bash
pnpm run repair:create-job -- \
  --from-report ../clawsweeper/records/marketingclaw-marketingclaw/items/123.md
```
...
```bash
pnpm run repair:validate-job -- jobs/marketingclaw/inbox/clawsweeper-marketingclaw-marketingclaw-123.md
pnpm run repair:dispatch -- jobs/marketingclaw/inbox/clawsweeper-marketingclaw-marketingclaw-123.md \
  --mode autonomous \
...
```bash
gh variable set CLAWSWEEPER_ALLOW_EXECUTE --repo marketingclaw/clawsweeper --body 1
gh variable set CLAWSWEEPER_ALLOW_FIX_PR --repo marketingclaw/clawsweeper --body 1
gh variable set CLAWSWEEPER_ALLOW_MERGE --repo marketingclaw/clawsweeper --body 1
gh variable set CLAWSWEEPER_ALLOW_AUTOMERGE --repo marketingclaw/clawsweeper --body 1
```
...
@marketingclaw-clawsweeper fix ci
@marketingclaw-clawsweeper[bot] fix ci
```
...
leave the local MarketingClaw checkout on the PR branch. After the PR is created,
...
escalation, and sensitive data exposure to central MarketingClaw security handling.
...
```bash
gh run list --repo marketingclaw/clawsweeper --workflow "ClawSweeper Commit Review" \
  --limit 12 --json databaseId,displayTitle,event,status,conclusion,createdAt,updatedAt,url
gh run list --repo marketingclaw/clawsweeper --workflow "repair cluster worker" \
  --limit 12 --json databaseId,displayTitle,event,status,conclusion,createdAt,updatedAt,url
gh run list --repo marketingclaw/clawsweeper --workflow "repair comment router" \
  --limit 12 --json databaseId,displayTitle,event,status,conclusion,createdAt,updatedAt,url
```
