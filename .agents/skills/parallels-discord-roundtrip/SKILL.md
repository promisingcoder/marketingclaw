...
```bash
export MARKETINGCLAW_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.marketingclaw/marketingclaw.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env MARKETINGCLAW_PARALLELS_DISCORD_TOKEN \
...
- Use the `marketingclaw` wrapper for guest `message send/read`; `node marketingclaw.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/marketingclaw-parallels-smoke.*`
...
- host inbound nonce appears in `marketingclaw message read` output
