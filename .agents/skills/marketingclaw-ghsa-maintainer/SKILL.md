---
name: marketingclaw-ghsa-maintainer
description: "Inspect, patch, validate, publish, or confirm MarketingClaw GHSA security advisories and private-fork state."
---

# MarketingClaw GHSA Maintainer

Use this skill for repo security advisory workflow only. Keep general release work in `release-marketingclaw-maintainer`.
...
```bash
fork=$(gh api /repos/promisingcoder/marketingclaw/security-advisories/<GHSA> | jq -r .private_fork.full_name)
...
```bash
gh api -X PATCH /repos/promisingcoder/marketingclaw/security-advisories/<GHSA> \
