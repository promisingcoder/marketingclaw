...
marketingclaw_node_version_matches() {
  local actual="$1"
  local requested="$2"
  if [[ -z "$requested" ]]; then
    return 0
  ...
  case "$requested" in
    *x)
      ...
      if [[ "${requested%%.*}" == "22" ]]; then
        marketingclaw_node_version_at_least "$actual" "22.19.0"
      fi
      ;;
    *.*.*)
      [[ "$actual" == "$requested" ]]
...
marketingclaw_node_version_at_least() {
  local actual="$1"
  local minimum="$2"
...
marketingclaw_active_node_version() {
  node -p 'process.versions.node' 2>/dev/null || true
}

marketingclaw_prepend_node_bin() {
...
marketingclaw_find_toolcache_node() {
  ...
  for root in \
    ...
    "${MARKETINGCLAW_CONTAINER_TOOL_CACHE:-/__t}" \
    "/opt/hostedtoolcache" \
  ...
  for node_root in ${roots[@]+"${roots[@]}"}; do
    while IFS= read -r candidate; do
      ...
      if marketingclaw_node_version_matches "$candidate_version" "$requested_node"; then
        printf '%s\n' "$candidate"
        return 0
      fi
...
marketingclaw_resolve_node_download_version() {
  local requested_node="$1"
  ...
  curl -fsSL https://nodejs.org/dist/index.json |
    MARKETINGCLAW_NODE_PREFIX="$prefix" python3 -c 'import json, os, sys
prefix = os.environ["MARKETINGCLAW_NODE_PREFIX"]
...
marketingclaw_node_download_platform() {
...
marketingclaw_download_node() {
  local requested_node="$1"
  local version platform archive_url install_root temp_root
  version="$(marketingclaw_resolve_node_download_version "$requested_node")"
  platform="$(marketingclaw_node_download_platform)" || return 1
  temp_root="${RUNNER_TEMP:-/tmp}"
  if command -v cygpath >/dev/null 2>&1; then
    temp_root="$(cygpath -u "$temp_root" 2>/dev/null || printf '%s\n' "$temp_root")"
  fi
  install_root="${temp_root}/marketingclaw-node-${version}-${platform}"
  if [[ "$platform" == win-* ]]; then
    ...
    if command -v pwsh >/dev/null 2>&1; then
      ...
      marketingclaw_prepend_node_bin "$node_bin_dir" "$ps_bin_dir"
      ...
      marketingclaw_prepend_node_bin "$node_bin_dir"
    fi
  else
    ...
    marketingclaw_prepend_node_bin "$install_root/bin"
  fi
}

marketingclaw_ensure_node() {
  local requested_node="${1:-}"
  ...
  active_node_version="$(marketingclaw_active_node_version)"
  if marketingclaw_node_version_matches "$active_node_version" "$requested_node"; then
    echo "Using active Node ${active_node_version} at $(command -v node)"
    return 0
  fi

  node_bin="$(marketingclaw_find_toolcache_node "$requested_node" || true)"
  if [[ -n "$node_bin" ]]; then
    echo "Using Node $("$node_bin" -p 'process.versions.node') from $node_bin"
    marketingclaw_prepend_node_bin "$(dirname "$node_bin")"
  else
    marketingclaw_download_node "$requested_node" || true
  fi
