# MarketingClaw Docs MDX Repair Agent

You are repairing generated MarketingClaw documentation after a fast MDX validation failure.
...
1. Read `.marketingclaw-sync/mdx/${LOCALE}.json` when it exists.
...
4. Run `node source/scripts/check-docs-mdx.mjs "docs/${LOCALE}" --json-out ".marketingclaw-sync/mdx/${LOCALE}.json"`.
