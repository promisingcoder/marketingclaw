...
You are Mantis running native Telegram Desktop visual proof for an MarketingClaw PR.
...
- `MANTIS_INSTRUCTIONS`
- `CRABBOX_PROVIDER`
- `MARKETINGCLAW_TELEGRAM_USER_PROOF_CMD`
...
6. In each worktree, run the real-user Telegram Crabbox proof flow from the
   skill with `$MARKETINGCLAW_TELEGRAM_USER_PROOF_CMD`; do not run
   ...
   baseline or candidate MarketingClaw build is tested. Use
   `$MARKETINGCLAW_TELEGRAM_USER_DRIVER_SCRIPT`, the workflow-provided `crabbox`
