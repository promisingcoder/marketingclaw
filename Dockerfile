...
ARG MARKETINGCLAW_EXTENSIONS=""
ARG MARKETINGCLAW_BUNDLED_PLUGIN_DIR=extensions
ARG MARKETINGCLAW_DOCKER_BUILD_NODE_OPTIONS="--max-old-space-size=8192"
ARG MARKETINGCLAW_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB=""
ARG MARKETINGCLAW_DOCKER_BUILD_SKIP_DTS=1
...
ARG MARKETINGCLAW_NODE_BOOKWORM_SLIM_DIGEST="sha256:242549cd46785b480c832479a730f4f2a20865d61ea2e404fdb2a5c3d3b73ecf"
...
FROM ${MARKETINGCLAW_NODE_BOOKWORM_IMAGE} AS workspace-deps
ARG MARKETINGCLAW_EXTENSIONS
ARG MARKETINGCLAW_BUNDLED_PLUGIN_DIR
...
RUN --mount=type=bind,source=packages,target=/tmp/packages,readonly \
    --mount=type=bind,source=${MARKETINGCLAW_BUNDLED_PLUGIN_DIR},target=/tmp/${MARKETINGCLAW_BUNDLED_PLUGIN_DIR},readonly \
    mkdir -p /out/packages "/out/${MARKETINGCLAW_BUNDLED_PLUGIN_DIR}" && \
    ...
    for ext in $(printf '%s\n' "$MARKETINGCLAW_EXTENSIONS" | tr ',' ' '); do \
      if [ -f "/tmp/${MARKETINGCLAW_BUNDLED_PLUGIN_DIR}/$ext/package.json" ]; then \
        mkdir -p "/out/${MARKETINGCLAW_BUNDLED_PLUGIN_DIR}/$ext" && \
        cp "/tmp/${MARKETINGCLAW_BUNDLED_PLUGIN_DIR}/$ext/package.json" "/out/${MARKETINGCLAW_BUNDLED_PLUGIN_DIR}/$ext/package.json"; \
      fi; \
    done

# ── Stage 2: Build ──────────────────────────────────────────────
FROM ${MARKETINGCLAW_BUN_IMAGE} AS bun-binary
FROM ${MARKETINGCLAW_NODE_BOOKWORM_IMAGE} AS build
ARG MARKETINGCLAW_BUNDLED_PLUGIN_DIR
ARG MARKETINGCLAW_EXTENSIONS
ARG MARKETINGCLAW_DOCKER_BUILD_NODE_OPTIONS
ARG MARKETINGCLAW_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB
ARG MARKETINGCLAW_DOCKER_BUILD_SKIP_DTS
...
RUN pnpm_config_verify_deps_before_run=false pnpm canvas:a2ui:bundle || \
    (echo "A2UI bundle: creating stub (non-fatal)" && \
     ...
     rm -rf vendor/a2ui apps/shared/MarketingClawKit/Tools/CanvasA2UI)
RUN if printf '%s\n' "$MARKETINGCLAW_EXTENSIONS" | tr ',' ' ' | tr ' ' '\n' | grep -qx 'qa-lab'; then \
...
ENV MARKETINGCLAW_PREFER_PNPM=1
...
RUN --mount=type=cache,id=marketingclaw-pnpm-store,target=/root/.local/share/pnpm/store,sharing=locked \
    ...
    if [ -L /app/node_modules/@marketingclaw/ai ]; then \
      ai_runtime_target="$(readlink -f /app/node_modules/@marketingclaw/ai)" && \
      ai_runtime_tmp="$(mktemp -d)" && \
      cp -a "$ai_runtime_target" "$ai_runtime_tmp/ai" && \
      rm /app/node_modules/@marketingclaw/ai && \
      mv "$ai_runtime_tmp/ai" /app/node_modules/@marketingclaw/ai && \
      rmdir "$ai_runtime_tmp"; \
    fi && \
    rm -rf \
      /app/node_modules/marketingclaw \
      /app/node_modules/.bin/marketingclaw \
      /app/node_modules/.pnpm/openclaw@*/node_modules/marketingclaw && \
    node scripts/check-package-dist-imports.mjs /app

# ── Runtime base image ──────────────────────────────────────────
FROM ${MARKETINGCLAW_NODE_BOOKWORM_SLIM_IMAGE} AS base-runtime
ARG MARKETINGCLAW_NODE_BOOKWORM_SLIM_DIGEST
LABEL org.opencontainers.image.base.name="docker.io/library/node:24-bookworm-slim" \
  org.opencontainers.image.base.digest="${MARKETINGCLAW_NODE_BOOKWORM_SLIM_DIGEST}"
...
# - https://docs.marketingclaw.ai/install/docker
LABEL org.opencontainers.image.source="https://github.com/promisingcoder/marketingclaw" \
  org.opencontainers.image.url="https://marketingclaw.ai" \
  org.opencontainers.image.documentation="https://docs.marketingclaw.ai/install/docker" \
  org.opencontainers.image.licenses="MIT" \
  org.opencontainers.image.title="MarketingClaw" \
  org.opencontainers.image.description="MarketingClaw gateway and CLI runtime container image"
...
RUN --mount=type=cache,id=marketingclaw-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=marketingclaw-bookworm-apt-lists,target=/var/lib/apt,sharing=locked \
    apt-get update && \
...
COPY --from=runtime-assets --chown=node:node /app/marketingclaw.mjs .
COPY --from=runtime-assets --chown=node:node /app/src/agents/templates ./src/agents/templates
COPY --from=runtime-assets --chown=node:node /app/${MARKETINGCLAW_BUNDLED_PLUGIN_DIR} ./${MARKETINGCLAW_BUNDLED_PLUGIN_DIR}
...
# Example: docker build --build-arg MARKETINGCLAW_IMAGE_APT_PACKAGES="python3 wget" .
# Legacy alias: MARKETINGCLAW_DOCKER_APT_PACKAGES is still accepted as a fallback.
ARG MARKETINGCLAW_IMAGE_APT_PACKAGES
ARG MARKETINGCLAW_DOCKER_APT_PACKAGES=""
RUN --mount=type=cache,id=marketingclaw-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=marketingclaw-bookworm-apt-lists,target=/var/lib/apt,sharing=locked \
    packages="${MARKETINGCLAW_IMAGE_APT_PACKAGES-$MARKETINGCLAW_DOCKER_APT_PACKAGES}"; \
    if [ -n "$packages" ]; then \
      apt-get update && \
...
# Example: docker build --build-arg MARKETINGCLAW_IMAGE_PIP_PACKAGES="requests humanize" .
ARG MARKETINGCLAW_IMAGE_PIP_PACKAGES=""
RUN --mount=type=cache,id=marketingclaw-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=marketingclaw-bookworm-apt-lists,target=/var/lib/apt,sharing=locked \
...
# Build with: docker build --build-arg MARKETINGCLAW_INSTALL_BROWSER=1 ...
# Adds ~300MB but eliminates the 60-90s Playwright install on every container start.
# Must run after node_modules COPY so playwright-core is available.
ARG MARKETINGCLAW_INSTALL_BROWSER=""
...
# Build with: docker build --build-arg MARKETINGCLAW_INSTALL_DOCKER_CLI=1 ...
# Adds ~50MB. Only the CLI is installed — no Docker daemon.
# Required for agents.defaults.sandbox to function in Docker deployments.
ARG MARKETINGCLAW_INSTALL_DOCKER_CLI=""
ARG MARKETINGCLAW_DOCKER_GPG_FINGERPRINT="9DC858229FC7DD38854AE2D88D81803C0EBFCD88"
RUN --mount=type=cache,id=marketingclaw-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=marketingclaw-bookworm-apt-lists,target=/var/lib/apt,sharing=locked \
    if [ -n "$MARKETINGCLAW_INSTALL_DOCKER_CLI" ]; then \
      apt-get update && \
      DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends \
      ...
      # Update MARKETINGCLAW_DOCKER_GPG_FINGERPRINT when Docker rotates release keys.
      curl -fsSL https://download.docker.com/linux/debian/gpg -o /tmp/docker.gpg.asc && \
      expected_fingerprint="$(printf '%s' "$MARKETINGCLAW_DOCKER_GPG_FINGERPRINT" | tr '[:lower:]' '[:upper:]' | tr -d '[:space:]')" && \
...
RUN ln -sf /app/marketingclaw.mjs /usr/local/bin/marketingclaw \
 && chmod 755 /app/marketingclaw.mjs
...
# the leaf /home/node/.config/marketingclaw inherits the correct parent permissions.
...
RUN install -d -m 0755 -o node -g node /home/node/.config && \
    install -d -m 0700 -o node -g node \
      /home/node/.marketingclaw \
      /home/node/.marketingclaw/workspace \
      /home/node/.config/marketingclaw && \
...
ENTRYPOINT ["tini", "-s", "--"]
CMD ["node", "marketingclaw.mjs", "gateway"]
