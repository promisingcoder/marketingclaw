...
const resolveAgentWorkspaceDir = vi.hoisted(() =>
  vi.fn((_cfg?: unknown, _agentId?: unknown) => "/tmp/marketingclaw-workspace"),
);
...
describe("setupChannels workspace shadow exclusion", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    resolveAgentWorkspaceDir.mockReturnValue("/tmp/marketingclaw-workspace");
  ...
  it("preloads configured external plugins from the trusted catalog boundary", async () => {
    ...
    expect(trustedInput.workspaceDir).toBe("/tmp/marketingclaw-workspace");
    ...
    expect(registryInput.workspaceDir).toBe("/tmp/marketingclaw-workspace");
  });
  ...
  it("allowlists ClickClack when it is explicitly selected for setup", async () => {
    const setupWizard = {
      ...
      configure: vi.fn(async ({ cfg }: { cfg: MarketingClawConfig }) => ({
        cfg: {
          ...cfg,
          channels: {
            ...cfg.channels,
  ...
  it("loads the selected bundled catalog plugin without writing explicit plugin enablement", async () => {
    ...
    expect(firstRegistryInput.workspaceDir).toBe("/tmp/marketingclaw-workspace");
    ...
    expect(secondRegistryInput.workspaceDir).toBe("/tmp/marketingclaw-workspace");
  ...
  it(
    ...
    async () => {
      ...
      expect(catalogLookupCall[1]?.workspaceDir).toBe("/tmp/marketingclaw-workspace");
      expect(ensureChannelSetupPluginInstalled).toHaveBeenCalledTimes(1);
      expectExternalCatalogInstallCall();
      expect(note).not.toHaveBeenCalledWith("external-chat plugin not available.", "Channel setup");
      expect(configure).toHaveBeenCalledTimes(1);
