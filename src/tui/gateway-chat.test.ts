...
async function withModeExecProviderFixture(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), `marketingclaw-tui-mode-${label}-`));
...
describe("resolveGatewayConnection", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv([
      ...
      "MARKETINGCLAW_TUI_SETUP_AUTH_SOURCE",
    ]);
    ...
    resolveStateDir.mockImplementation(
      (env: NodeJS.ProcessEnv) => env.MARKETINGCLAW_STATE_DIR ?? "/tmp/marketingclaw",
    );
    resolveConfigPath.mockImplementation(
      (env: NodeJS.ProcessEnv, stateDir: string) =>
        env.MARKETINGCLAW_CONFIG_PATH ?? `${stateDir}/marketingclaw.json`,
    );
    ...
    delete process.env.MARKETINGCLAW_TUI_SETUP_AUTH_SOURCE;
  });
  ...
  it("keeps an explicit Gateway port ahead of active lock metadata", async () => {
    ...
    await withEnvAsync({ MARKETINGCLAW_GATEWAY_PORT: "19001" }, async () => {
      const result = await resolveGatewayConnection({});
  ...
  it("falls back to MARKETINGCLAW_GATEWAY_TOKEN when config token is missing", async () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });
  ...
  it("keeps normal TUI local password mode env precedence by default", async () => {
    ...
    await withEnvAsync({ MARKETINGCLAW_GATEWAY_PASSWORD: "env-password" }, async () => {
      const result = await resolveGatewayConnection({});
  ...
  it("uses configured local password for setup-launched TUI despite stale gateway password env", async () => {
    ...
    await withEnvAsync(
      {
        MARKETINGCLAW_GATEWAY_PASSWORD: "stale-env-password", // pragma: allowlist secret
        MARKETINGCLAW_TUI_SETUP_AUTH_SOURCE: "config",
      },
      async () => {
        const result = await resolveGatewayConnection({});
        expect(result.password).toBe("config-password");
  ...
  it("still resolves env SecretRefs for setup-launched TUI config auth", async () => {
    ...
    await withEnvAsync(
      {
        MARKETINGCLAW_GATEWAY_PASSWORD: "resolved-ref-password", // pragma: allowlist secret
        MARKETINGCLAW_TUI_SETUP_AUTH_SOURCE: "config",
      },
      async () => {
        const result = await resolveGatewayConnection({});
  ...
  it("prefers MARKETINGCLAW_GATEWAY_PASSWORD over remote password fallback", async () => {
    loadConfig.mockReturnValue({
      gateway: {
        mode: "remote",
    ...
    const gatewayPasswordEnv = "MARKETINGCLAW_GATEWAY_PASSWORD"; // pragma: allowlist secret
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-tui-file-secret-"));
