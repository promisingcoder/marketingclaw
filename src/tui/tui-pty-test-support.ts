...
async function writePtyInput(
  ...
  const delayMs = readPositiveIntegerEnv("MARKETINGCLAW_TUI_PTY_TYPE_DELAY_MS");
  if (!delayMs || opts.delay === false) {
    pty.write(data);
    return;
  }
  const chunkSize = readPositiveIntegerEnv("MARKETINGCLAW_TUI_PTY_TYPE_CHUNK_SIZE") ?? 1;
...
function mirrorPtyOutput(data: string) {
  const mirrorPath = process.env.MARKETINGCLAW_TUI_PTY_MIRROR_PATH;
  if (!mirrorPath) {
    return;
  }
...
export function startPty(
  ...
  const pty = spawnPty(command, args, {
    name: "xterm-256color",
    cols: readPtyDimensionEnv("MARKETINGCLAW_TUI_PTY_COLS", 100),
    rows: readPtyDimensionEnv("MARKETINGCLAW_TUI_PTY_ROWS", 30),
    cwd: opts.cwd,
    env: {
      ...process.env,
      ...opts.env,
