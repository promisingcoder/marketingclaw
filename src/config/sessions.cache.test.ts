...
describe("Session Store Cache", () => {
  ...
  afterEach(() => {
    clearSessionStoreCacheForTest();
    delete process.env.MARKETINGCLAW_SESSION_CACHE_TTL_MS;
    delete process.env.MARKETINGCLAW_SESSION_SERIALIZED_CACHE_MAX_BYTES;
  });

  it("bounds the serialized session store cache by total bytes", () => {
    process.env.MARKETINGCLAW_SESSION_SERIALIZED_CACHE_MAX_BYTES = "64";
    clearSessionStoreCacheForTest();
  ...
  it("should respect MARKETINGCLAW_SESSION_CACHE_TTL_MS=0 to disable cache", async () => {
