...
import { isSensitiveUrlConfigPath } from "@marketingclaw/net-policy/redact-sensitive-url";
...
describe("mapSensitivePaths", () => {
  ...
  it("main schema yields correct hints (samples)", () => {
    const schema = MarketingClawSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "MarketingClawConfig";
    const hints = mapSensitivePaths(MarketingClawSchema, "", {});
...
describe("collectMatchingSchemaPaths", () => {
  it("finds base-config URL fields that may embed secrets", () => {
    const paths = collectMatchingSchemaPaths(MarketingClawSchema, "", isSensitiveUrlConfigPath);
