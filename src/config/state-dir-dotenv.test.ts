...
describe("readStateDirDotEnvFromStateDir", () => {
  async function withDotEnv<T>(content: string, run: (dir: string) => T | Promise<T>): Promise<T> {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-dotenv-test-"));
  ...
  it("skips values that are unresolved shell variable references", async () => {
    const content = [
      'SUPERMEMORY_MARKETINGCLAW_API_KEY="${SUPERMEMORY_MARKETINGCLAW_KEY}"',
      "QUOTED_SUPERMEMORY_MARKETINGCLAW_API_KEY='\"$SUPERMEMORY_MARKETINGCLAW_KEY\"'",
    ...
    await withDotEnv(content, async (dir) => {
      const result = readStateDirDotEnvFromStateDir(dir).entries;
      expect(Object.keys(result)).not.toContain("SUPERMEMORY_MARKETINGCLAW_API_KEY");
      expect(Object.keys(result)).not.toContain("QUOTED_SUPERMEMORY_MARKETINGCLAW_API_KEY");
  ...
  it("returns empty object when .env is missing", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-dotenv-missing-"));
    try {
