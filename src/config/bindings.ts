...
export function listConfiguredBindings(cfg: MarketingClawConfig): AgentBinding[] {
  return Array.isArray(cfg.bindings) ? cfg.bindings : [];
}

/** Lists channel route bindings, including legacy bindings without an explicit type. */
export function listRouteBindings(cfg: MarketingClawConfig): AgentRouteBinding[] {
  return listConfiguredBindings(cfg).filter(isRouteBinding);
}

/** Lists ACP conversation bindings only. */
export function listAcpBindings(cfg: MarketingClawConfig): AgentAcpBinding[] {
