...
/**
 * Nix mode detection: When MARKETINGCLAW_NIX_MODE=1, the gateway is running under Nix.
 * In this mode:
...
export function resolveIsNixMode(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.MARKETINGCLAW_NIX_MODE === "1";
}
...
const CONFIG_FILENAME = "marketingclaw.json";
...
/** Build a homedir thunk that respects MARKETINGCLAW_HOME for the given env. */
 ...
 * Can be overridden via MARKETINGCLAW_STATE_DIR.
 * Default: ~/.marketingclaw
 */
export function resolveStateDir(
  ...
  if (env.MARKETINGCLAW_TEST_FAST === "1") {
    return newDir;
  }
...
export function normalizeStateDirEnv(env: NodeJS.ProcessEnv = process.env): void {
  ...
  const marketingclawOverride = env.MARKETINGCLAW_STATE_DIR?.trim();
  if (marketingclawOverride) {
    env.MARKETINGCLAW_STATE_DIR = resolveUserPath(marketingclawOverride, env, effectiveHomedir);
  }
}
 ...
 * outside the config directory. Set via `MARKETINGCLAW_INCLUDE_ROOTS` as a
 ...
 * directory containing `marketingclaw.json`.
 */
export function resolveIncludeRoots(
  env: NodeJS.ProcessEnv = process.env,
  homedir: () => string = envHomedir(env),
): string[] {
  const raw = env.MARKETINGCLAW_INCLUDE_ROOTS?.trim();
  if (!raw) {
    return [];
  }
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
 ...
 * Can be overridden via MARKETINGCLAW_CONFIG_PATH.
 * Default: ~/.marketingclaw/marketingclaw.json (or $MARKETINGCLAW_STATE_DIR/marketingclaw.json)
 */
function resolveCanonicalConfigPath(
  env: NodeJS.ProcessEnv = process.env,
  stateDir: string = resolveStateDir(env, envHomedir(env)),
): string {
  const override = env.MARKETINGCLAW_CONFIG_PATH?.trim();
  if (override) {
...
export function resolveConfigPath(
  ...
  const stateOverride = env.MARKETINGCLAW_STATE_DIR?.trim();
  const candidates = [
...
export function resolveDefaultConfigCandidates(
  ...
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
  const explicit = env.MARKETINGCLAW_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveUserPath(explicit, env, effectiveHomedir)];
  }

  const candidates: string[] = [];
  const marketingclawStateDir = env.MARKETINGCLAW_STATE_DIR?.trim();
  if (marketingclawStateDir) {
    const resolved = resolveUserPath(marketingclawStateDir, env, effectiveHomedir);
 ...
 * Default: os.tmpdir()/marketingclaw-<uid> (uid suffix when available).
 */
export function resolveGatewayLockDir(tmpdir: () => string = os.tmpdir): string {
  const base = tmpdir();
  const uid = typeof process.getuid === "function" ? process.getuid() : undefined;
  const suffix = uid != null ? `marketingclaw-${uid}` : "marketingclaw";
 ...
 *
 * Precedence:
 * - `MARKETINGCLAW_OAUTH_DIR` (explicit override)
...
export function resolveOAuthDir(
  ...
  const override = env.MARKETINGCLAW_OAUTH_DIR?.trim();
  if (override) {
    return resolveUserPath(override, env, envHomedir(env));
  }
...
export function resolveGatewayPort(
  ...
  const envRaw = env.MARKETINGCLAW_GATEWAY_PORT?.trim();
