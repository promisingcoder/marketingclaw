      ...
      agents: {
        defaults: {
          sandbox: {
            docker: {
              binds: [
                "D:/data/marketingclaw/src:/src:ro",
                "D:\\data\\marketingclaw\\output:/output:rw",
              ],
            },
          },
        },
...
describe("sandbox browser binds config", () => {
          ...
          sandbox: {
            browser: {
              binds: [
                ...
                "D:/data/marketingclaw/chrome:/data/chrome-windows:rw",
              ],
            },
          },
        },
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("marketingclaw-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "marketingclaw-sandbox-browser-global" },
      agentBrowser: { network: "marketingclaw-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("marketingclaw-sandbox-browser-agent");
  });
