...
async function writePluginFixture(params: {
  ...
  await fs.writeFile(
    path.join(params.dir, "marketingclaw.plugin.json"),
...
describe("config plugin validation", () => {
  ...
  const suiteEnv = () =>
    ({
      ...
      MARKETINGCLAW_STATE_DIR: path.join(suiteHome, ".marketingclaw"),
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(
      path.join(os.tmpdir(), "marketingclaw-config-plugin-validation-"),
    );
  ...
  describe("missing Codex plugin diagnostics", () => {
    ...
    it("still warns when only one provider model route is pinned to MarketingClaw", () => {
  ...
  it("deduplicates catalog install hints for missing configured official external plugins", () => {
    ...
    expect(res.ok).toBe(true);
    const message =
      "plugin not installed: brave — install the official external plugin with: marketingclaw plugins install @marketingclaw/brave-plugin";
  ...
  it("warns instead of failing when an official external memory slot plugin is not installed", () => {
    ...
    expect(res.ok).toBe(true);
    const slotMessage =
      "plugin not installed: memory-lancedb — gateway will run without persistent memory until installed; install the official external plugin with: marketingclaw plugins install @marketingclaw/memory-lancedb";
    const entryMessage =
      "plugin not installed: memory-lancedb — install the official external plugin with: marketingclaw plugins install @marketingclaw/memory-lancedb";
  ...
  it("deduplicates yuanbao missing-plugin warnings across entries and allow", () => {
    ...
    expect(res.ok).toBe(true);
    const message =
      "plugin not installed: yuanbao — install the official external plugin with: marketingclaw plugins install marketingclaw-plugin-yuanbao@2.15.0";
  ...
  it("warns instead of failing for stale channel config backed by missing plugin refs", () => {
    ...
    expect(res.warnings).toContainEqual({
      ...
      message:
        "unknown channel id: missing-chat (stale channel plugin config ignored; run marketingclaw doctor --fix to remove stale config, or install the plugin)",
    });
    expect(res.warnings).toContainEqual({
      path: "plugins.allow",
      message:
    ...
    expect(res.warnings ?? []).toEqual([
      {
        path: "plugins.allow",
        message:
          "plugin not installed: discord — install the official external plugin with: marketingclaw plugins install @marketingclaw/discord",
      },
    ]);
  });

  it("uses persisted installed-plugin records as stale channel evidence", async () => {
    const stateDir = path.join(suiteHome, ".marketingclaw");
    ...
    try {
      ...
      expect(res.warnings).toContainEqual({
        ...
        message:
          "unknown channel id: missing-sms (stale channel plugin config ignored; run marketingclaw doctor --fix to remove stale config, or install the plugin)",
      });
    } finally {
      await writePersistedInstalledPluginIndex(
        {
  ...
  it("does not fail validation for the implicit default memory slot when plugins config is explicit", () => {
    const res = validateConfigObjectWithPlugins(
      ...
      },
      {
        env: {
          ...suiteEnv(),
          MARKETINGCLAW_BUNDLED_PLUGINS_DIR: path.join(suiteHome, "missing-bundled-plugins"),
        },
      },
    );
    expect(res.ok).toBe(true);
  ...
  it("warns for removed skill-workshop plugin id instead of failing validation", () => {
    ...
    expect(res.ok).toBe(true);
    const message =
      "plugin removed: skill-workshop (stale plugin config ignored; Skill Workshop is built into MarketingClaw skills now. Use skills.workshop settings and marketingclaw skills workshop commands, then remove this plugins config entry)";
  ...
  it("ignores standalone helper scripts in auto-discovered global extensions", async () => {
    const helperPath = path.join(suiteHome, ".marketingclaw", "extensions", "my-helper.mjs");
  ...
  it("surfaces invalid Codex native plugin marketplaces as config diagnostics", () => {
    const res = validateConfigObjectWithPlugins(
      ...
      },
      {
        env: {
          ...suiteEnv(),
          MARKETINGCLAW_BUNDLED_PLUGINS_DIR: path.join(process.cwd(), "extensions"),
        },
      },
    );
  ...
  it("accepts known plugin ids and valid channel/heartbeat enums", () => {
    const res = validateInSuite({
      agents: {
        ...
        list: [{ id: "marketingclaw", heartbeat: { directPolicy: "allow" } }],
      },
      channels: {
        modelByChannel: {
          openai: {
  ...
  it("accepts plugin heartbeat targets", () => {
    const res = validateInSuite({
      agents: { defaults: { heartbeat: { target: "chat" } }, list: [{ id: "marketingclaw" }] },
