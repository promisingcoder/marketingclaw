...
export type McpServerCodexConfig = {
  /** MarketingClaw agent ids that should receive this server in Codex app-server threads. */
  agents?: string[];
...
export type McpServerConfig = {
  ...
  /** HTTP OAuth mode. Tokens are stored in MarketingClaw state, not in config. */
  auth?: "oauth";
  ...
  /** Alias for clientKey. */
  client_key?: string;
  /** Optional per-server MarketingClaw MCP tool selection. */
...
export type McpConfig = {
  /** Named MCP server definitions managed by MarketingClaw. */
