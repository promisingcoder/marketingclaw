...
function projectGatewayDispatchConfig(value: unknown): MarketingClawConfig {
  if (!isPlainRecord(value)) {
    return {};
  }
  ...
  return projected as MarketingClawConfig;
}

// Main session keys are process-local; Gateway dispatch always sees the canonical main key.
function applyGatewayDispatchSessionDefaults(config: MarketingClawConfig): MarketingClawConfig {
...
function resolveGatewayDispatchEnvVars(config: unknown, env: NodeJS.ProcessEnv): unknown {
  if (isPlainRecord(config) && Object.hasOwn(config, "env")) {
    applyConfigEnvVars(config as MarketingClawConfig, env);
  }
