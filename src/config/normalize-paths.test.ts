  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.marketingclaw/logs/marketingclaw.log" },
        hooks: {
          path: "~/.marketingclaw/hooks.json5",
        ...
        channels: {
          telegram: {
            accounts: {
              personal: {
                tokenFile: "~/.marketingclaw/telegram.token",
              },
            },
          },
          whatsapp: {
            accounts: {
              personal: {
                authDir: "~/.marketingclaw/credentials/wa-personal",
              },
            },
          },
          imessage: {
      ...
      expect(cfg.logging?.file).toBe(
        path.join(home, ".marketingclaw", "logs", "marketingclaw.log"),
      );
      expect(cfg.hooks?.path).toBe(path.join(home, ".marketingclaw", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".marketingclaw", "telegram.token"),
      );
      expect(cfg.channels?.whatsapp?.accounts?.personal?.authDir).toBe(
        path.join(home, ".marketingclaw", "credentials", "wa-personal"),
      );
      ...
      expect(cfg.agents?.list?.[0]?.agentDir).toBe(
        path.join(home, ".marketingclaw", "agents", "main"),
      );
