...
const setupRegistryMock = vi.hoisted(() => ({
  resolvePluginSetupAutoEnableReasons: vi.fn(
    (params: { config?: MarketingClawConfig; pluginIds?: readonly string[] }) => {
...
describe("applyPluginAutoEnable core", () => {
  ...
  it("reuses policy-compatible current manifest registry when runtime config differs", () => {
    ...
    const snapshotConfig: MarketingClawConfig = { plugins: { allow: ["existing"] } };
  ...
  it("does not reuse a load-path current manifest registry for a config with default load paths", () => {
    const manifestRegistry = makeRegistry([{ id: "load-path-chat", channels: ["load-path-chat"] }]);
    const snapshotConfig: MarketingClawConfig = {
      plugins: {
        allow: ["existing"],
  ...
  it("does not load plugin manifests for disabled plugin entries under a restrictive allowlist", () => {
    ...
    expect(
      readFileSync.mock.calls.some(
        ([filePath]) =>
          typeof filePath === "string" && filePath.endsWith("marketingclaw.plugin.json"),
      ),
    ).toBe(false);
  });
  ...
  it("ignores agent harness runtime env when auto-enabling plugins", () => {
    const result = applyPluginAutoEnable({
      config: {},
      env: makeIsolatedEnv({ MARKETINGCLAW_AGENT_RUNTIME: "codex" }),
      manifestRegistry: makeRegistry([
        {
          id: "codex",
          channels: [],
  ...
  it("does not auto-enable WhatsApp from persisted auth state alone", () => {
    ...
    const authDir = path.join(
      persistedEnv.MARKETINGCLAW_STATE_DIR ?? "",
      "credentials",
      "whatsapp",
      "default",
    );
  ...
  it("does not reuse same-turn auto-enable results across registry or env inputs", () => {
    const channelConfig: MarketingClawConfig = {
      channels: { apn: { someKey: "value" } },
    };
    ...
    const envConfig: MarketingClawConfig = {
      plugins: {
        entries: {
          browser: {
            config: {},
    ...
    const firstEnv = applyPluginAutoEnable({
      config: envConfig,
      discovery,
      env: makeIsolatedEnv({ MARKETINGCLAW_TEST_CACHE_INPUT: "one" }),
      manifestRegistry,
    });
    const secondEnv = applyPluginAutoEnable({
      config: envConfig,
      discovery,
      env: makeIsolatedEnv({ MARKETINGCLAW_TEST_CACHE_INPUT: "two" }),
      manifestRegistry,
    });
  ...
  it("does not reuse same-turn auto-enable results after env mutates in place", () => {
    ...
    mutableEnv.MARKETINGCLAW_TEST_CACHE_INPUT = "changed";
    const second = applyPluginAutoEnable({
      config,
      discovery: emptyDiscovery,
      env: mutableEnv,
