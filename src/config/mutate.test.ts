...
type MockValidationResult =
  | { ok: true; config: MarketingClawConfig; warnings: MockValidationIssue[] }
...
const validationMocks = vi.hoisted(() => ({
  validateConfigObjectWithPlugins: vi.fn(
    (config: MarketingClawConfig): MockValidationResult => ({
      ok: true,
      config,
      warnings: [],
    }),
...
function createSnapshot(params: {
  ...
  return {
    path: params.path ?? "/tmp/marketingclaw.json",
    exists: true,
...
describe("config mutate helpers", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "marketingclaw-config-mutate-" });
  const originalNixMode = process.env.MARKETINGCLAW_NIX_MODE;
  ...
  afterAll(async () => {
    if (originalNixMode === undefined) {
      ...
      process.env.MARKETINGCLAW_NIX_MODE = originalNixMode;
    }
  ...
  it("preserves config path ownership across transform retries", async () => {
    const initial = createSnapshot({
      hash: "hash-1",
      path: "/tmp/first-marketingclaw.json",
      sourceConfig: { agents: { list: [] } },
    });
    const fresh = createSnapshot({
      hash: "hash-2",
      path: "/tmp/second-marketingclaw.json",
      sourceConfig: { agents: { list: [] } },
    });
    ioMocks.readConfigFileSnapshotForWrite
      .mockResolvedValueOnce({
    ...
    const transform = vi.fn((config: MarketingClawConfig) => ({ nextConfig: config }));
  ...
  it("does not retry transform mutations after config path ownership changes", async () => {
    ...
    await expect(
      transformConfigFileWithRetry({
        transform(config) {
          activeConfigPath = "/tmp/second-marketingclaw.json";
  ...
  it("rejects replace attempts when the active config path changed", async () => {
    ...
    await expect(
      replaceConfigFile({
        baseHash: snapshot.hash,
        nextConfig: { gateway: { port: 19002 } },
        writeOptions: { expectedConfigPath: "/tmp/first-marketingclaw.json" },
      }),
    ).rejects.toThrow("config path changed since last load");
    expect(ioMocks.writeConfigFile).not.toHaveBeenCalled();
  });
  ...
  it("refuses mutate writes in Nix mode before touching disk", async () => {
    ...
    ).rejects.toThrow("MarketingClaw Nix overview: https://docs.marketingclaw.ai/install/nix");
  ...
  it("repairs invalid config through a single-file top-level plugins include", async () => {
    ...
    const pluginsPath = path.join(home, ".marketingclaw", "config", "plugins.json5");
      ...
      `${JSON.stringify(
        {
          entries: {
            old: {
              enabled: true,
              config: { token: "${MARKETINGCLAW_TEST_PLUGIN_TOKEN}" },
            },
          },
        },
        null,
    ...
    ioMocks.readConfigFileSnapshotForWrite
      .mockResolvedValueOnce({
        ...
        writeOptions: {
          ...
          envSnapshotForRestore: { MARKETINGCLAW_TEST_PLUGIN_TOKEN: "plugin-token-runtime" },
    ...
    try {
      await replaceConfigFile({
        ...
        io: {
          env: { MARKETINGCLAW_TEST_PLUGIN_TOKEN: "plugin-token-after-read" },
    ...
    expect(persistedPlugins.entries?.old?.config?.token).toBe("${MARKETINGCLAW_TEST_PLUGIN_TOKEN}");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const configPath = path.join(home, ".marketingclaw", "marketingclaw.json");
      const linkPath = path.join(home, ".marketingclaw", "link");
  ...
  it("rejects direct mutations to external include roots", async () => {
    ...
    await expect(
      replaceConfigFile({
        ...
        nextConfig,
        io: {
          env: { MARKETINGCLAW_INCLUDE_ROOTS: "~/shared" },
          readConfigFileSnapshotForWrite: ioMocks.readConfigFileSnapshotForWrite,
          writeConfigFile: ioMocks.writeConfigFile,
        },
      }),
  ...
  it("does not write an include after the active config path changes during preflight", async () => {
    ...
    const firstConfigPath = path.join(home, "first", "marketingclaw.json");
    const secondConfigPath = path.join(home, "second", "marketingclaw.json");
  ...
  it("rolls back an include write when config path ownership changes during commit", async () => {
    ...
    const assertConfigPathForWrite = () => {
      if (fsNode.readFileSync(pluginsPath, "utf-8") !== initialPluginsRaw) {
        activeConfigPath = "/tmp/other-marketingclaw.json";
      }
  ...
  it.each(["active path", "refreshed snapshot path"] as const)(
    ...
    async (changeKind) => {
      ...
      const configPath = path.join(home, "first", "marketingclaw.json");
      const otherConfigPath = path.join(home, "second", "marketingclaw.json");
      const pluginsPath = path.join(home, "first", "plugins.json5");
      const rootConfig = { plugins: { $include: "./plugins.json5" } };
      const initialPluginsRaw = `${JSON.stringify({ entries: {} }, null, 2)}\n`;
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const configPath = path.join(home, ".marketingclaw", "marketingclaw.json");
      const includeDir = path.join(home, ".marketingclaw", "config");
      const movedIncludeDir = path.join(home, ".marketingclaw", "config-original");
  ...
  it("preflights the restored include payload with the current environment", async () => {
    ...
    const initialPluginsRaw = `${JSON.stringify(
      {
        entries: {
          old: { enabled: true, config: { token: "${MARKETINGCLAW_TEST_INCLUDE_TOKEN}" } },
        },
      },
      null,
      2,
    ...
    const observedSources: MarketingClawConfig[] = [];
    ...
    try {
      ...
      await expect(
        replaceConfigFile({
          ...
          writeOptions: {
            expectedConfigPath: snapshot.path,
            envSnapshotForRestore: { MARKETINGCLAW_TEST_INCLUDE_TOKEN: "old-token" },
            assertConfigPathForWrite: allowConfigPathWrite,
            includeFileTargetsForWrite: { [pluginsPath]: await resolveIncludeTarget(pluginsPath) },
          },
          nextConfig: {
          ...
          io: {
            env: { MARKETINGCLAW_TEST_INCLUDE_TOKEN: "new-token" },
            readConfigFileSnapshotForWrite: ioMocks.readConfigFileSnapshotForWrite,
            writeConfigFile: ioMocks.writeConfigFile,
          },
        }),
  ...
  it("rolls back single-file top-level include writes when runtime refresh fails", async () => {
    ...
    const envKey = "MARKETINGCLAW_TEST_INCLUDE_ROLLBACK_ENV";
    await fs.mkdir(path.dirname(pluginsPath), { recursive: true });
    await fs.writeFile(
      configPath,
      `${JSON.stringify({ plugins: { $include: "./config/plugins.json5" } }, null, 2)}\n`,
  ...
  it("preflights injected root writers before persisting", async () => {
    ...
    const configPath = path.join(home, ".marketingclaw", "marketingclaw.json");
    await fs.mkdir(path.dirname(configPath), { recursive: true });
    const initialConfig = { gateway: { mode: "local" } } satisfies MarketingClawConfig;
    ...
    const injectedWrite = vi.fn(
      async (config: MarketingClawConfig, options?: ConfigWriteOptions) => {
