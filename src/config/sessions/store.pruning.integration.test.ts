...
// Keep integration tests deterministic: never read a real marketingclaw.json.
...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "marketingclaw-pruning-integ-" });
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  it("removes trajectory sidecars for stale sessions pruned on write", async () => {
    ...
    await fs.writeFile(staleTranscript, '{"type":"session"}\n', "utf-8");
    await fs.writeFile(freshTranscript, '{"type":"session"}\n', "utf-8");
    await fs.writeFile(staleRuntime, '{"traceSchema":"marketingclaw-trajectory"}\n', "utf-8");
    await fs.writeFile(freshRuntime, '{"traceSchema":"marketingclaw-trajectory"}\n', "utf-8");
    await fs.writeFile(
      stalePointer,
      JSON.stringify({
