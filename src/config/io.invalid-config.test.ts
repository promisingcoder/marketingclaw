...
describe("config io invalid config formatting", () => {
  ...
  it("formats the logger message with the escaped newline separator", () => {
    expect(formatInvalidConfigLogMessage("/tmp/marketingclaw.json", "- gateway.port: bad")).toBe(
      "Invalid config at /tmp/marketingclaw.json:\\n- gateway.port: bad",
    );
  });

  it("creates INVALID_CONFIG errors with inline details", () => {
    const err = createInvalidConfigError(
      "/tmp/marketingclaw.json",
      "- gateway.port: bad",
    ) as Error & {
      code?: string;
      details?: string;
    ...
    expect(err.message).toBe("Invalid config at /tmp/marketingclaw.json:\n- gateway.port: bad");
  ...
  it("throws INVALID_CONFIG after logging the formatted details", () => {
    ...
    ).toThrowError("Invalid config at /tmp/marketingclaw.json:\n- nope: Unknown key(s): nope");
    expect(logger.error).toHaveBeenCalledOnce();
  });
});
