...
const ETC_MARKETINGCLAW_DIR = path.join(ROOT_DIR, "etc", "marketingclaw");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "marketingclaw.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcMarketingClawPath(...parts: string[]) {
  return path.join(ETC_MARKETINGCLAW_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("rejects absolute path outside config directory (CWE-22)", () => {
    const absolute = etcMarketingClawPath("agents.json");
    ...
    expectResolveIncludeError(
      () =>
        resolve(
          ...
          configPath("sub", "marketingclaw.json"),
        ),
...
describe("resolveConfigIncludeWritePath", () => {
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "marketingclaw-include-write-path-" }, async (tempRoot) => {
        ...
        expect(
          resolveConfigIncludeWritePath({
            configPath: path.join(configDir, "marketingclaw.json"),
...
describe("security: path traversal protection (CWE-22)", () => {
  ...
  describe("edge cases", () => {
    ...
    it("allows include files when the config root path is a symlink", async () => {
      await withTempDir({ prefix: "marketingclaw-includes-symlink-" }, async (tempRoot) => {
        ...
        const result = resolveConfigIncludes(
          ...
          path.join(linkRoot, "marketingclaw.json"),
        );
    ...
    it("rejects include files that are hardlinked aliases", async () => {
      ...
      await withTempDir({ prefix: "marketingclaw-includes-hardlink-" }, async (tempRoot) => {
        const configDir = path.join(tempRoot, "config");
    ...
    it("rejects oversized include files", async () => {
      await withTempDir({ prefix: "marketingclaw-includes-big-" }, async (tempRoot) => {
        const configDir = path.join(tempRoot, "config");
        await fs.mkdir(configDir, { recursive: true });
...
describe("MARKETINGCLAW_INCLUDE_ROOTS allowlist", () => {
  ...
  it("still rejects include paths that fall outside every allowed root", () => {
    const obj = { $include: etcMarketingClawPath("agents.json") };
    expect(() =>
  ...
  it("resolves a symlinked include whose realpath lands inside an allowed root", async () => {
    await withTempDir({ prefix: "marketingclaw-includes-allowed-symlink-" }, async (tempRoot) => {
      const configDir = path.join(tempRoot, "config");
  ...
  it("rejects a symlinked include that escapes both the config directory and every allowed root", async () => {
    await withTempDir({ prefix: "marketingclaw-includes-allowed-escape-" }, async (tempRoot) => {
      const configDir = path.join(tempRoot, "config");
      const allowedDir = path.join(tempRoot, "allowed");
