...
function canReuseUnscopedCurrentPluginMetadataSnapshot(config: MarketingClawConfig): boolean {
...
function isProviderConfigured(cfg: MarketingClawConfig, providerId: string): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: MarketingClawConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
  return isRecord(pluginConfig) && isRecord(pluginConfig.webSearch);
}

function hasPluginOwnedWebFetchConfig(cfg: MarketingClawConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
...
function hasPluginOwnedToolConfig(cfg: MarketingClawConfig, plugin: PluginManifestRecord): boolean {
...
function hasConfiguredWebSearchPluginEntry(cfg: MarketingClawConfig): boolean {
...
function hasConfiguredWebSearchProviderSelection(cfg: MarketingClawConfig): boolean {
...
function hasConfiguredWebFetchPluginEntry(cfg: MarketingClawConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    Boolean(entries) &&
    typeof entries === "object" &&
...
function hasConfiguredSpeechProviderSelection(cfg: MarketingClawConfig): boolean {
  return collectConfiguredSpeechProviderIds(cfg).size > 0;
}

function hasConfiguredPluginConfigEntry(cfg: MarketingClawConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    Boolean(entries) &&
    typeof entries === "object" &&
...
function hasBrowserToolReference(cfg: MarketingClawConfig): boolean {
...
function collectConfiguredPluginEntryIds(cfg: MarketingClawConfig): string[] {
  const entries = cfg.plugins?.entries;
...
function hasOwnPluginEntry(cfg: MarketingClawConfig, pluginId: string): boolean {
  const entries = cfg.plugins?.entries;
  return Boolean(entries) && typeof entries === "object" && Object.hasOwn(entries, pluginId);
}

function isPluginEntryExplicitlyDisabled(cfg: MarketingClawConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function hasNonDisabledPluginEntry(cfg: MarketingClawConfig, pluginId: string): boolean {
...
function hasBrowserSetupAutoEnableRelevantConfig(cfg: MarketingClawConfig): boolean {
...
function hasAcpxSetupAutoEnableRelevantConfig(cfg: MarketingClawConfig): boolean {
...
function hasXaiSetupAutoEnableRelevantConfig(cfg: MarketingClawConfig): boolean {
...
function resolveRelevantSetupAutoEnablePluginIds(cfg: MarketingClawConfig): string[] {
...
function hasSetupAutoEnableRelevantConfig(cfg: MarketingClawConfig): boolean {
  return (
...
function hasPluginEntries(cfg: MarketingClawConfig): boolean {
  const entries = cfg.plugins?.entries;
  return Boolean(entries) && typeof entries === "object" && Object.keys(entries).length > 0;
}

function hasPluginAllowlistWithMaterialEntries(cfg: MarketingClawConfig): boolean {
  if (
...
function arePluginsGloballyDisabled(cfg: MarketingClawConfig): boolean {
...
function isPluginExplicitlyDisabled(cfg: MarketingClawConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: MarketingClawConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function isPluginExplicitlySelected(cfg: MarketingClawConfig, pluginId: string): boolean {
...
function isBuiltInChannelAlreadyEnabled(cfg: MarketingClawConfig, channelId: string): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
