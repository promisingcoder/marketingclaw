...
describe("docs-link-audit", () => {
  ...
  it("cleans anchor audit docs copies when docs.json is invalid", () => {
    ...
    const before = tempEntries("marketingclaw-docs-anchor-audit-");
    try {
      expect(() => prepareAnchorAuditDocsDir(docsRoot)).toThrow();
      const after = tempEntries("marketingclaw-docs-anchor-audit-");
  ...
  it("does not create mirrored docs copies for non-root docs trees", () => {
    ...
    const before = tempEntries("marketingclaw-docs-link-audit-");
    try {
      ...
      const after = tempEntries("marketingclaw-docs-link-audit-");
      expect([...after].filter((entry) => !before.has(entry))).toEqual([]);
    } finally {
      cleanupTempDirs(tempDirs);
    }
    ...
    expect(() =>
      runDocsLinkAuditCli({
        ...
        prepareMirroredDocsDirImpl: () => ({
          ...
          dir: path.join(os.tmpdir(), "marketingclaw-docs-mirrored"),
  ...
  it("uses Mintlify through pnpm dlx for anchor validation", () => {
    ...
    const exitCode = runDocsLinkAuditCli({
      args: ["--anchors"],
      env: { ...process.env, MARKETINGCLAW_DOCS_LINK_SENTINEL: "1" },
    ...
    expect(invocation).toEqual({
      ...
      options: expect.objectContaining({
        cwd: anchorDocsDir,
        env: expect.objectContaining({ MARKETINGCLAW_DOCS_LINK_SENTINEL: "1" }),
        shell: false,
        stdio: "inherit",
      }),
    });
