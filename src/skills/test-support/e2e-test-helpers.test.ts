...
async function withTempSkillDir(
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-skill-helper-"));
  tempDirs.push(root);
  ...
  it("includes optional metadata, body, and frontmatterExtra", async () => {
    await withTempSkillDir("custom-skill", async ({ skillDir }) => {
      await writeSkill({
        ...
        name: "custom-skill",
        description: "Custom",
        metadata: '{"marketingclaw":{"always":true}}',
        frontmatterExtra: "user-invocable: false",
        body: "# Custom Body\n",
      });

      const content = await fs.readFile(path.join(skillDir, "SKILL.md"), "utf-8");
      expect(content).toContain('metadata: {"marketingclaw":{"always":true}}');
