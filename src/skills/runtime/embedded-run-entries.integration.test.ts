...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("marketingclaw-bundled-");
  const workspaceDir = await createTempDir("marketingclaw-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: MarketingClawConfig) {
