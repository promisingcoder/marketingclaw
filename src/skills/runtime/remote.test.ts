...
function createRemoteSkillWorkspace(bin: string): {
  cfg: MarketingClawConfig;
  workspaceDir: string;
} {
  const workspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-remote-skills-"));
  ...
  fs.writeFileSync(
    ...
    [
      ...
      `metadata: { "marketingclaw": { "os": ["darwin"], "requires": { "bins": ["${bin}"] } } }`,
      "---",
      "# Remote Skill",
      "",
    ].join("\n"),
