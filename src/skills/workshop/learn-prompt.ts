...
export function buildLearnPrompt(request: string): string {
  ...
  return [
    "Create one reviewable MarketingClaw skill proposal from the learning request below.",
    "",
    ...
    "",
    "Follow these MarketingClaw skill-authoring standards:",
    ...
    "- Include optional `metadata.marketingclaw` fields such as `emoji` or `requires.bins` only when the gathered sources prove they are true and useful.",
    ...
    "",
    "After the tool call, tell the user the proposal id, the skill name, and that it is pending review. Say that an operator can apply it through the Skill Workshop approval flow or with `marketingclaw skills workshop`.",
  ].join("\n");
}
