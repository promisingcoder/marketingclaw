...
// MarketingClaw process is running with a different HOME (e.g. the per-agent
...
// See https://github.com/promisingcoder/marketingclaw/issues/78063.
...
export type GhConfigDirMismatch = {
  ...
  // Suggested env value the operator should set on the MarketingClaw service to
...
export function formatGhConfigDirMismatchHint(mismatch: GhConfigDirMismatch): string[] {
  ...
  lines.push(
    `  Fix: set GH_CONFIG_DIR=${mismatch.suggestedEnvValue} on the MarketingClaw service environment, then restart the gateway.`,
  );
  return lines;
}
