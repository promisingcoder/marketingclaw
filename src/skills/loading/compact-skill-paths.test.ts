...
describe("compactSkillPaths", () => {
  ...
  it("replaces home directory prefix with ~ in skill locations", () => {
    ...
    const skillDir = path.join(home, ".marketingclaw-test-skills", "test-skill");
  ...
  it("does not compact explicit state-root managed skill paths to OS-home tilde paths", () => {
    ...
    const stateDir = path.join(osHome, ".marketingclaw");
    const skillDir = path.join(stateDir, "skills", "world-cup-soccer-marketingclaw-skill");
    const skillFile = path.join(skillDir, "SKILL.md");

    const prompt = withEnv(
      {
        HOME: osHome,
        MARKETINGCLAW_HOME: osHome,
      ...
      () =>
        buildPromptForFixtureSkill({
          ...
          skillDir,
          name: "world-cup-soccer-marketingclaw-skill",
    ...
    expect(prompt).not.toContain(
      "~/.marketingclaw/skills/world-cup-soccer-marketingclaw-skill/SKILL.md",
    );
  });
  ...
  it("does not compact explicit state-root plugin skill paths to OS-home tilde paths", () => {
    ...
    expect(prompt).toContain(`<location>${skillFile}</location>`);
    expect(prompt).not.toContain("~/.marketingclaw/plugin-skills/calendar-plugin-skill/SKILL.md");
  });
  ...
  it("compacts managed skill paths when OS-home tilde reaches the same path", () => {
    ...
    expect(prompt).toContain(
      "<location>~/.marketingclaw/skills/home-managed-skill/SKILL.md</location>",
    );
  ...
  it("normalizes compacted Windows skill locations to forward slashes", () => {
    ...
    const skillPath = path.win32.join(home, ".marketingclaw-test-skills", "win-skill", "SKILL.md");

    const compactedPath = workspaceSkillsTesting.compactHomePath(skillPath, [home]);

    expect(compactedPath).toBe("~/.marketingclaw-test-skills/win-skill/SKILL.md");
  });

  it("preserves POSIX literal backslashes after home compaction", () => {
    const home = os.homedir();
    const skillDir = path.join(home, ".marketingclaw-test-skills\\literal-skill");
  ...
  it("preserves paths outside home directory", () => {
    const outsideHome = path.join(path.parse(os.homedir()).root, "marketingclaw-external-skills");
  ...
  it("loads skills when the shared state database is unavailable", () => {
    const root = fs.realpathSync(
      fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-skill-load-")),
    );
    ...
    try {
      const prompt = withEnv({ MARKETINGCLAW_STATE_DIR: path.join(blockedParent, "state") }, () =>
        buildPromptForFixtureSkill({
          workspaceRoot: path.join(root, "workspace"),
          skillDir,
