...
beforeAll(async () => {
  fixtureRoot = await fsPromises.realpath(
    await fsPromises.mkdtemp(path.join(os.tmpdir(), "marketingclaw-plugin-skills-sync-")),
  );
});
...
describe("syncSkillsToWorkspace for plugin skills", () => {
  it("syncs plugin skills from symlinked directories to sandbox workspace", async () => {
    ...
    const pluginSkillsDir = path.join(sourceWorkspace, ".marketingclaw", "plugin-skills");
