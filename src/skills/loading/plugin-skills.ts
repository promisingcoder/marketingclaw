 ...
 * plugin skills directory (~/.marketingclaw/plugin-skills/) so the agent SDK can
 * discover them at the conventional file-system path.
 *
 * The plugin-skills directory is fully owned by MarketingClaw — every entry is
...
function publishPluginSkills(skillDirs: string[], opts?: { pluginSkillsDir?: string }): void {
  ...
  // Plugin skill symlinks are owned by MarketingClaw and publish at extra-dir
  ...
  // The plugin-skills directory is fully owned by MarketingClaw: every entry is a
