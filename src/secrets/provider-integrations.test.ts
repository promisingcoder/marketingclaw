...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-secret-provider-integrations-"));
...
describe("secret provider integration presets", () => {
  ...
  it("skips presets whose persisted plugin integration IDs would violate config schema limits", () => {
    ...
    fs.writeFileSync(
      path.join(longPluginRootDir, "marketingclaw.plugin.json"),
      JSON.stringify({
        id: longPluginId,
        secretProviderIntegrations: {
          vault: {
  ...
  it.skipIf(process.platform === "win32")("allows node presets from symlinked plugin roots", () => {
    ...
    fs.writeFileSync(
      path.join(realRoot, "marketingclaw.plugin.json"),
      JSON.stringify({
