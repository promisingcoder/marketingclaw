...
describe("secrets runtime snapshot auth integration", () => {
  ...
  it("recomputes config-derived agent dirs when refreshing active secrets runtime snapshots", async () => {
    await withTempHome("marketingclaw-secrets-runtime-agent-dirs-", async (home) => {
      const mainAgentDir = path.join(home, ".marketingclaw", "agents", "main", "agent");
      const opsAgentDir = path.join(home, ".marketingclaw", "agents", "ops", "agent");
