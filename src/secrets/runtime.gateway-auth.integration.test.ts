...
describe("secrets runtime snapshot gateway-auth integration", () => {
  ...
  it(
    ...
    async () => {
      await withTempHome("marketingclaw-secrets-runtime-gateway-auth-reload-lkg-", async (home) => {
        ...
        const persistedConfig = JSON.parse(
          await fs.readFile(path.join(home, ".marketingclaw", "marketingclaw.json"), "utf8"),
