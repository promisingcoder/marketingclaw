...
vi.mock("../plugins/manifest-metadata-scan.js", async (importOriginal) => {
  ...
  return {
    ...
    listMarketingClawPluginManifestMetadata: (
      params?: Parameters<typeof actual.listMarketingClawPluginManifestMetadata>[0],
    ) => {
      pluginManifestRegistryMocks.listMarketingClawPluginManifestMetadata(params);
      return actual.listMarketingClawPluginManifestMetadata(params);
    },
  };
});
...
describe("model-pricing-cache", () => {
  beforeEach(() => {
    ...
    pluginManifestRegistryMocks.listMarketingClawPluginManifestMetadata.mockClear();
  ...
  it("collects manifest-owned web search plugin model refs without a hardcoded plugin list", () => {
    ...
    } as MarketingClawConfig).map((ref) => modelKey(ref.provider, ref.model));
  ...
  it("does not load plugin manifests for pricing when plugins are globally disabled", async () => {
    ...
    expect(
      pluginManifestRegistryMocks.listMarketingClawPluginManifestMetadata,
    ).not.toHaveBeenCalled();
...
function createManifestRecord(overrides: Partial<PluginManifestRecord>): PluginManifestRecord {
  return {
    ...
    manifestPath: "/tmp/plugin/marketingclaw.plugin.json",
    ...overrides,
  };
}
