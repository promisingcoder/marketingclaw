...
import { warnLegacyMarketingClawEnvVars } from "./env-deprecation.js";
...
export async function resolveGatewayRuntimeConfig(params: {
  ...
  if (tailscaleMode === "funnel" && authMode !== "password") {
    throw new Error(
      "tailscale funnel requires gateway auth mode=password (set gateway.auth.password or MARKETINGCLAW_GATEWAY_PASSWORD)",
    );
  }
  ...
  if (!isLoopbackHost(bindHost) && !hasSharedSecret && authMode !== "trusted-proxy") {
    throw new Error(
      `refusing to bind gateway to ${bindHost}:${params.port} without auth (set gateway.auth.token/password, or set MARKETINGCLAW_GATEWAY_TOKEN/MARKETINGCLAW_GATEWAY_PASSWORD; legacy CLAWDBOT_* and MOLTBOT_* environment variables are ignored)`,
    );
  }
  if (
    controlUiEnabled &&
