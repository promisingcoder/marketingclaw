...
function shouldLogMcpLoopbackHttp(): boolean {
  return (
    isTruthyEnvValue(process.env.MARKETINGCLAW_CLI_BACKEND_LOG_OUTPUT) ||
    isTruthyEnvValue(process.env.MARKETINGCLAW_LIVE_CLI_BACKEND_DEBUG)
  );
}
...
export function resolveMcpHttpBodyTimeoutMs(): number {
  return readPositiveIntEnv(
    "MARKETINGCLAW_MCP_LOOPBACK_BODY_TIMEOUT_MS",
    DEFAULT_MCP_BODY_TIMEOUT_MS,
  );
}

export function resolveMcpCliCaptureKey(req: IncomingMessage): string | undefined {
  return normalizeOptionalString(getHeader(req, "x-marketingclaw-cli-capture-key"));
}
...
export function resolveMcpRequestContext(
  ...
  return {
    ...
    sessionId: normalizeOptionalString(getHeader(req, "x-marketingclaw-session-id")),
    messageProvider:
      normalizeMessageChannel(getHeader(req, "x-marketingclaw-message-channel")) ?? undefined,
    ...
    clientCaps: normalizeMcpClientCapsHeader(getHeader(req, "x-marketingclaw-client-caps")),
    currentChannelId: normalizeOptionalString(getHeader(req, "x-marketingclaw-current-channel-id")),
    currentThreadTs: normalizeOptionalString(getHeader(req, "x-marketingclaw-current-thread-ts")),
    currentMessageId: normalizeOptionalString(getHeader(req, "x-marketingclaw-current-message-id")),
    currentInboundAudio: normalizeMcpBooleanHeader(
      getHeader(req, "x-marketingclaw-current-inbound-audio"),
    ),
    accountId: normalizeOptionalString(getHeader(req, "x-marketingclaw-account-id")),
    inboundEventKind: normalizeMcpInboundEventKind(
      getHeader(req, "x-marketingclaw-inbound-event-kind"),
    ),
    sourceReplyDeliveryMode: normalizeMcpSourceReplyDeliveryMode(
      getHeader(req, "x-marketingclaw-source-reply-delivery-mode"),
    ),
    taskSuggestionDeliveryMode: normalizeMcpTaskSuggestionDeliveryMode(
      getHeader(req, "x-marketingclaw-task-suggestion-delivery-mode"),
    ),
    requireExplicitMessageTarget: normalizeMcpBooleanHeader(
      getHeader(req, "x-marketingclaw-require-explicit-message-target"),
    ),
    senderIsOwner: auth.senderIsOwner,
  };
}
