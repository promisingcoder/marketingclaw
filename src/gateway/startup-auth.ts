...
const HOOKS_GATEWAY_AUTH_REUSE_WARNING =
  "Security warning: hooks.token matches active Gateway shared-secret auth. Startup continues for compatibility; rotate hooks.token or Gateway auth. Run marketingclaw security audit for a full report, and run marketingclaw doctor --fix when the reused hooks.token is persisted in config.";
...
function hasGatewayTokenCandidate(params: {
  ...
  const envToken = trimToUndefined(params.env.MARKETINGCLAW_GATEWAY_TOKEN);
  if (envToken) {
    return true;
  }
  if (
