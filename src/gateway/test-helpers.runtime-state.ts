...
const GATEWAY_TEST_CONFIG_ROOT_KEY = Symbol.for("marketingclaw.gatewayTestHelpers.configRoot");
...
const gatewayTestHoisted = vi.hoisted(() => {
  const key = Symbol.for("marketingclaw.gatewayTestHelpers.hoisted");
...
export const testConfigRoot = resolveGlobalSingleton(GATEWAY_TEST_CONFIG_ROOT_KEY, () => ({
  value: path.join(os.tmpdir(), `marketingclaw-gateway-test-${process.pid}-${crypto.randomUUID()}`),
}));

/** Updates the config root used by gateway config-module mocks. */
export function setTestConfigRoot(root: string): void {
  testConfigRoot.value = root;
  process.env.MARKETINGCLAW_CONFIG_PATH = path.join(root, "marketingclaw.json");
}
