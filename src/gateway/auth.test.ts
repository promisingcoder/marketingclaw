...
describe("gateway auth", () => {
  ...
  it("resolves token/password from MARKETINGCLAW gateway env vars", () => {
    const auth = resolveGatewayAuth({
      authConfig: {},
      env: {
        MARKETINGCLAW_GATEWAY_TOKEN: "env-token",
        MARKETINGCLAW_GATEWAY_PASSWORD: "env-password",
  ...
  it("accepts password mode when env provides MARKETINGCLAW_GATEWAY_PASSWORD", () => {
...
describe("trusted-proxy auth", () => {
  ...
  it("rejects trusted-proxy headers from the host non-loopback interface address", async () => {
    ...
    const res = await authorizeTrustedProxy({
      ...
      headers: {
        "x-forwarded-user": "nick@example.com",
        "x-forwarded-proto": "https",
        "x-marketingclaw-proxy-auth": "present",
      },
    });
  ...
  it.each([
    ...
    },
    {
      ...
      env: {
        MARKETINGCLAW_GATEWAY_TOKEN: "shared-secret",
      } as NodeJS.ProcessEnv,
    },
