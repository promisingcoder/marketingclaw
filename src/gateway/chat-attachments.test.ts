...
const saveMediaBufferMock = vi.hoisted(() =>
  vi.fn(async (_buffer: Buffer, mime?: string, _subdir?: string) => ({
    ...
    path: `/tmp/marketingclaw-test-media/inbound/fake.${mime?.split("/")[1] ?? "bin"}`,
    size: 0,
    contentType: mime,
  })),
);
  ...
  it("rejects oversized images before offload", async () => {
    ...
    await expect(
      parseMessageWithAttachments(
        ...
        {
          maxBytes: resolveChatAttachmentMaxBytes({} as MarketingClawConfig),
          log: { warn: () => {} },
        },
      ),
...
describe("resolveChatAttachmentMaxBytes", () => {
  ...
  const cfgWithMediaMaxMb = (value: unknown): MarketingClawConfig =>
    ({ agents: { defaults: { mediaMaxMb: value } } }) as unknown as MarketingClawConfig;
  ...
  it("falls back to DEFAULT_CHAT_ATTACHMENT_MAX_MB when unset", () => {
    expect(resolveChatAttachmentMaxBytes({} as MarketingClawConfig)).toBe(DEFAULT_BYTES);
    expect(resolveChatAttachmentMaxBytes({ agents: {} } as unknown as MarketingClawConfig)).toBe(
      DEFAULT_BYTES,
    );
  });
