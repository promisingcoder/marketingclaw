...
const LEGACY_MARKETINGCLAW_ENV_NOTE =
  " Legacy CLAWDBOT_* and MOLTBOT_* environment variables are ignored; use MARKETINGCLAW_* names.";
...
export function assertGatewayAuthConfigured(
  ...
  if (auth.mode === "token" && !auth.token) {
    ...
    throw new Error(
      `gateway auth mode is token, but no token was configured (set gateway.auth.token or MARKETINGCLAW_GATEWAY_TOKEN).${LEGACY_MARKETINGCLAW_ENV_NOTE}`,
    );
  }
  if (auth.mode === "password" && !auth.password) {
    if (
      ...
      throw new Error(
        "gateway auth mode is password, but gateway.auth.password contains a provider reference object instead of a resolved string — bootstrap secrets (gateway.auth.password) must be plaintext strings or set via the MARKETINGCLAW_GATEWAY_PASSWORD environment variable because the secrets provider system has not initialised yet at gateway startup", // pragma: allowlist secret
      );
    }
    throw new Error(
      `gateway auth mode is password, but no password was configured.${LEGACY_MARKETINGCLAW_ENV_NOTE}`,
    );
  }
  if (auth.mode === "trusted-proxy") {
    ...
    if (auth.token) {
      throw new Error(
        "gateway auth mode is trusted-proxy, but a shared token is also configured; remove gateway.auth.token / MARKETINGCLAW_GATEWAY_TOKEN because trusted-proxy and token auth are mutually exclusive",
      );
    }
  }
}
