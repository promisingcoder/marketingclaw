...
function createSingleAgentAvatarConfig(workspace: string): MarketingClawConfig {
  return {
...
describe("gateway session utils", () => {
  ...
  test("session list search includes direct-session origin display labels", () => {
    ...
    const store = {
      "agent:main:telegram:direct:42": {
        chatType: "direct",
        channel: "telegram",
        origin: { label: "marketingclaw-tui" },
        updatedAt: 2,
      } as SessionEntry,
    ...
    const listed = listSessionsFromStore({
      ...
      storePath: "",
      store,
      opts: { search: "marketingclaw-tui" },
    });

    expect(listed.sessions.map((session) => session.key)).toEqual([
      "agent:main:telegram:direct:42",
    ]);
    expect(listed.sessions[0]?.displayName).toBe("marketingclaw-tui");
  });
  ...
  test("session rows expose estimated context budget status", () => {
    const row = buildGatewaySessionRow({
      ...
      entry: {
        sessionId: "session-1",
        sessionFile: "/tmp/marketingclaw/agents/main/sessions/session-1.jsonl",
        updatedAt: 1,
        contextBudgetStatus: {
          schemaVersion: 1,
  ...
  test("buildGatewaySessionRow displayName falls through to origin label for direct sessions", () => {
    ...
    expect(row.displayName).toBe("marketingclaw-tui");
  });
  ...
  test("buildGatewaySessionRow displayName uses group display name for group sessions", () => {
    ...
    expect(row.displayName).not.toBe("marketingclaw-tui");
  });
  ...
  test("listAgentsForGateway keeps explicit agents.list scope over disk-only agents (scope boundary)", async () => {
    await withStateDirEnv("marketingclaw-agent-list-scope-", async ({ stateDir }) => {
  ...
  test("listAgentsForGateway reports whether each workspace is a git checkout", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-agent-workspace-git-"));
...
describe("listSessionsFromStore selected model display", () => {
  test("async list yields during bulk transcript title and last-message hydration", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-sessions-list-yield-"));
    try {
  ...
  test("caps transcript title and last-message hydration for bulk list responses", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-sessions-list-cap-"));
    try {
      const storePath = path.join(tmpDir, "sessions.json");
      const store: Record<string, SessionEntry> = {};
      const now = Date.now();
...
describe("resolveSessionModelIdentityRef", () => {
  const resolveLegacyIdentityRef = (cfg: MarketingClawConfig, modelProvider?: string) =>
  ...
  test("infers provider from configured provider catalogs when allowlist is absent", () => {
    ...
    } as unknown as MarketingClawConfig["models"];
