...
let lastCreateMarketingClawToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/marketingclaw-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateMarketingClawToolsContext?.agentTo,
          agentThreadId: lastCreateMarketingClawToolsContext?.agentThreadId,
        },
        inheritedToolDenylist: lastCreateMarketingClawToolsContext?.inheritedToolDenylist,
      }),
    },
    {
      name: "sessions_send",
  ...
  return {
    createMarketingClawTools: (ctx: Record<string, unknown>) => {
      lastCreateMarketingClawToolsContext = ctx;
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateMarketingClawToolsContext = undefined;
...
const gatewayAuthHeaders = () => ({ "x-marketingclaw-scopes": "operator.write" });
const gatewayAdminHeaders = () => ({ "x-marketingclaw-scopes": "operator.admin" });
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateMarketingClawToolsContext?.allowMediaInvokeCommands).toBe(true);
    expect(lastCreateMarketingClawToolsContext?.disablePluginTools).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateMarketingClawToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("keeps plugin tools enabled for non-core tool invokes", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateMarketingClawToolsContext?.disablePluginTools).toBe(false);
  });
  ...
  it("allows the requested plugin tool through Gateway profile filtering", async () => {
    ...
    expect(lastCreateMarketingClawToolsContext?.pluginToolAllowlist).toContain("plugin_doctor");
  });
  ...
  it("uses tools.alsoAllow for optional plugin discovery without loading every plugin tool", async () => {
    ...
    expect(body.result?.permissionFlow).toBe(true);
    expect(lastCreateMarketingClawToolsContext?.pluginToolAllowlist).toContain("plugin_doctor");
    expect(lastCreateMarketingClawToolsContext?.pluginToolAllowlist).not.toContain("*");
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-marketingclaw-message-to": "channel:24514",
        "x-marketingclaw-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
  ...
  it("keeps shared-secret bearer auth as owner for explicitly allowed owner-only tools", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        authorization: "Bearer secret",
        "x-marketingclaw-scopes": "operator.write",
      },
      tool: "nodes",
      sessionKey: "main",
    });
    ...
    expect(lastCreateMarketingClawToolsContext?.senderIsOwner).toBe(true);
  });
  ...
  it("derives sender owner identity from HTTP auth instead of caller headers", async () => {
    ...
    expect(lastCreateMarketingClawToolsContext?.senderIsOwner).toBe(false);
