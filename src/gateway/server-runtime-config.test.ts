...
describe("resolveGatewayRuntimeConfig", () => {
  ...
  describe("token/password auth modes", () => {
    ...
    beforeEach(() => {
      originalToken = process.env.MARKETINGCLAW_GATEWAY_TOKEN;
      delete process.env.MARKETINGCLAW_GATEWAY_TOKEN;
    });

    afterEach(() => {
      if (originalToken !== undefined) {
        process.env.MARKETINGCLAW_GATEWAY_TOKEN = originalToken;
      } else {
        delete process.env.MARKETINGCLAW_GATEWAY_TOKEN;
      }
    });
    ...
    it.each([
      {
        ...
        expectedMessage:
          "gateway auth mode is token, but no token was configured (set gateway.auth.token or MARKETINGCLAW_GATEWAY_TOKEN)",
      },
      {
