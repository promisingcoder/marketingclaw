...
async function writeGatewayConfig(config: Record<string, unknown>) {
  ...
  if (!configPath) {
    throw new Error("MARKETINGCLAW_CONFIG_PATH is required for gateway config tests");
  }
  ...
  it("handles request validation and routing", async () => {
    ...
    try {
      ...
      await expectAgentSessionKeyMatch({
        body: { model: "marketingclaw", messages: [{ role: "user", content: "hi" }] },
        headers: { "x-marketingclaw-agent-id": "beta" },
        matcher: /^agent:beta:/,
      });

      await expectAgentSessionKeyMatch({
        body: {
          model: "marketingclaw/beta",
...

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "marketingclaw", messages: [{ role: "user", content: "hi" }] },
          { "x-marketingclaw-agent-id": "missing-agent" },
        );
        expect(res.status).toBe(400);
...

      {
        ...
        const res = await postChatCompletions(
          ...
          { model: "marketingclaw", messages: [{ role: "user", content: "hi" }] },
          {
            "x-marketingclaw-agent-id": "beta",
            "x-marketingclaw-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
...

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "marketingclaw", messages: [{ role: "user", content: "hi" }] },
          { "x-marketingclaw-session-key": "agent:main:subagent:spoofed" },
        );
        expect(res.status).toBe(400);
        const json = (await res.json()) as { error?: { type?: string; message?: string } };
        expect(json.error?.type).toBe("invalid_request_error");
        expect(json.error?.message).toBe(
          "`x-marketingclaw-session-key` cannot use reserved internal session namespaces.",
        );
        expect(agentCommand).toHaveBeenCalledTimes(0);
      }
...

      {
        ...
        const res = await postChatCompletions(
          ...
          { "x-marketingclaw-message-channel": "custom-client-channel" },
        );
        expect(res.status).toBe(200);
...

      {
        ...
        expect(json.error?.type).toBe("invalid_request_error");
        expect(json.error?.message).toBe(
          "Invalid `model`. Use `marketingclaw` or `marketingclaw/<agentId>`.",
        );
        expect(agentCommand).toHaveBeenCalledTimes(0);
      }
...

      {
        ...
        const json = (await res.json()) as { error?: { type?: string; message?: string } };
        expect(json.error?.type).toBe("invalid_request_error");
        expect(json.error?.message).toBe("Invalid `x-marketingclaw-model`.");
        expect(agentCommand).toHaveBeenCalledTimes(0);
      }
...

      {
        ...
        const choice0 = (json.choices as Array<Record<string, unknown>>)[0] ?? {};
        const msg = (choice0.message as Record<string, unknown> | undefined) ?? {};
        expect(msg.content).toBe("No response from MarketingClaw.");
      }
