...
export function createGatewayConfigModuleMock(actual: GatewayConfigModule): GatewayConfigModule {
  const resolveConfigPath = () => path.join(testConfigRoot.value, "marketingclaw.json");
  ...
  const composeTestConfig = (baseConfig: Record<string, unknown>) => {
    ...
    const defaults = {
      ...
      workspace: path.join(os.tmpdir(), "marketingclaw-gateway-test"),
      ...fileDefaults,
  ...
  return {
    ...
    applyConfigOverrides: (cfg: MarketingClawConfig) =>
