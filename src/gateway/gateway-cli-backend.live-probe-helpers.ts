...
async function callLoopbackJsonRpc(params: {
  ...
  if (params.messageProvider) {
    headers["x-marketingclaw-message-channel"] = params.messageProvider;
  }
  if (params.accountId) {
    headers["x-marketingclaw-account-id"] = params.accountId;
  }
  const timeoutMs = parsePositiveInt(
    params.env?.MARKETINGCLAW_MCP_LOOPBACK_PROBE_TIMEOUT_MS,
    CLI_CRON_MCP_LOOPBACK_REQUEST_TIMEOUT_MS,
    "MARKETINGCLAW_MCP_LOOPBACK_PROBE_TIMEOUT_MS",
  );
  const maxBodyBytes = parsePositiveInt(
    params.env?.MARKETINGCLAW_MCP_LOOPBACK_PROBE_MAX_BODY_BYTES,
    CLI_CRON_MCP_LOOPBACK_MAX_BODY_BYTES,
    "MARKETINGCLAW_MCP_LOOPBACK_PROBE_MAX_BODY_BYTES",
  );
