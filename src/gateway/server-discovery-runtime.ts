...
function resolveDiscoveryAdvertiseTimeoutMs(env: NodeJS.ProcessEnv): number {
  const raw = env.MARKETINGCLAW_GATEWAY_DISCOVERY_ADVERTISE_TIMEOUT_MS?.trim();
  if (!raw) {
...
export async function startGatewayDiscovery(params: {
  ...
  const localDiscoveryEnabled =
    mdnsMode !== "off" &&
    !isTruthyEnvValue(process.env.MARKETINGCLAW_DISABLE_BONJOUR) &&
  ...
  const sshPort = mdnsMinimal
    ? undefined
    : (parseTcpPort(process.env.MARKETINGCLAW_SSH_PORT) ?? undefined);
