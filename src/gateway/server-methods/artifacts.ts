...
import { normalizeOptionalString as asNonEmptyString } from "@marketingclaw/normalization-core/string-coerce";
import {
  ErrorCodes,
  errorShape,
...
function resolveMessageSeq(message: Record<string, unknown>, fallback: number): number {
  const meta = asOptionalRecord(message["__marketingclaw"]);
  const seq = meta?.seq;
