...
const getRuntimeConfig = vi.hoisted(() => vi.fn(() => ({}) as MarketingClawConfig));
const resolveDefaultAgentId = vi.hoisted(() => vi.fn(() => "main"));
const resolveAgentWorkspaceDir = vi.hoisted(() =>
  vi.fn((_cfg: MarketingClawConfig, _agentId: string) => "/tmp/marketingclaw"),
);
const resolveMemorySearchConfig = vi.hoisted(() =>
  vi.fn<(_cfg: MarketingClawConfig, _agentId: string) => { enabled: boolean } | null>(() => ({
    enabled: true,
  })),
);
...
describe("doctor.memory.status", () => {
  beforeEach(() => {
    ...
    resolveAgentWorkspaceDir.mockReset().mockReturnValue("/tmp/marketingclaw");
  ...
  it("includes dreaming counts and managed cron status when workspace data is available", async () => {
    ...
    resolveAgentWorkspaceDir.mockImplementation((cfg: MarketingClawConfig, agentId: string) => {
      if (agentId === "alpha") {
    ...
    const cronList = vi.fn(async () => [
      {
        ...
        payload: {
          kind: "systemEvent",
          text: "__marketingclaw_memory_core_short_term_promotion_dream__",
        },
  ...
  it("scopes dreaming status to the requested agent workspace", async () => {
    ...
    resolveAgentWorkspaceDir.mockImplementation((_cfg: MarketingClawConfig, agentId: string) => {
      if (agentId === "alpha") {
        return alphaWorkspaceDir;
      }
      return mainWorkspaceDir;
  ...
  it("reads dreaming config from the selected memory slot plugin", async () => {
    getRuntimeConfig.mockReturnValue({
      plugins: {
        slots: {
          memory: "memos-local-marketingclaw-plugin",
        },
        entries: {
          "memos-local-marketingclaw-plugin": {
            config: {
              dreaming: {
                enabled: true,
                frequency: "0 */4 * * *",
...
describe("doctor.memory dream actions", () => {
  it("clears grounded-only staged short-term entries without touching the diary", async () => {
    resolveAgentWorkspaceDir.mockReturnValue("/tmp/marketingclaw");
    removeGroundedShortTermCandidates.mockResolvedValue({
      removed: 3,
      storePath: "/tmp/marketingclaw/memory/.dreams/short-term-recall.json",
    });
    const respond = vi.fn();
  ...
  it("repairs contaminated dreaming artifacts for control-ui callers", async () => {
    ...
    repairDreamingArtifacts.mockResolvedValue({
      changed: true,
      archiveDir: "/tmp/marketingclaw/.marketingclaw-repair/dreaming/2026-04-11T22-00-00-000Z",
  ...
  it("dedupes exact dream diary duplicates for control-ui callers", async () => {
    ...
    dedupeDreamDiaryEntries.mockResolvedValue({
      dreamsPath: "/tmp/marketingclaw/DREAMS.md",
      removed: 2,
      kept: 7,
    });
    const respond = vi.fn();
...
describe("doctor.memory.dreamDiary", () => {
  ...
  it("reads DREAMS.md for the requested agent", async () => {
    ...
    resolveAgentWorkspaceDir.mockImplementation((_cfg, agentId) =>
      agentId === "research-analyst" ? workspaceDir : "/tmp/marketingclaw",
    );
    const respond = vi.fn();
...
describe("doctor.memory.remHarness", () => {
  const makeHarnessPreview = (
    ...
    workspaceDir: overrides.workspaceDir ?? "/tmp/marketingclaw",
    nowMs: 0,
    remConfig: {
      enabled: true,
      lookbackDays: 7,
  ...
  beforeEach(() => {
    getRuntimeConfig.mockClear().mockReturnValue({} as MarketingClawConfig);
