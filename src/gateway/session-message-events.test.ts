...
async function createSessionStoreFile(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-session-message-"));
...
describe("session.message websocket events", () => {
  ...
  test("strips blocked original content from live session.message events", async () => {
    ...
    await withOperatorSessionSubscriber(async (ws) => {
      ...
      const payload = messageEvent.payload as {
        message?: { content?: unknown; __marketingclaw?: { beforeAgentRunBlocked?: unknown } };
      };
    ...
    await withOperatorSessionSubscriber(async (ws) => {
      ...
      const payload = messageEvent.payload as {
        message?: {
          role?: unknown;
          content?: unknown;
          __marketingclaw?: { beforeAgentRunBlocked?: unknown };
        };
      };
  ...
  test("prefers carried transcript sequence for live session events", async () => {
    ...
    await withOperatorSessionSubscriber(async (ws) => {
      ...
      expect((message["__marketingclaw"] as { seq?: unknown } | undefined)?.seq).toBe(7);
    });
  });
