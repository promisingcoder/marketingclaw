...
describe("gateway startup log", () => {
  ...
  it("warns when dangerous config flags are enabled", async () => {
    ...
    expect(warn.mock.calls).toEqual([
      [
        "security warning: dangerous config flags enabled: gateway.controlUi.dangerouslyDisableDeviceAuth=true. Run `marketingclaw security audit`.",
      ],
    ]);
  });
  ...
  it("warns when a configured channel has no owning plugin", async () => {
    ...
    expect(warn.mock.calls).toEqual([
      [
        "configured channel warning: channels.missing-chat is configured but no channel plugin is installed or loadable (no-channel-owner). Run `marketingclaw doctor --fix` or install the channel plugin before relying on this channel.",
      ],
    ]);
  });
