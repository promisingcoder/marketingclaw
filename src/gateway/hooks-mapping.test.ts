...
describe("hooks mapping", () => {
  ...
  it("runs transform module", async () => {
    const configDir = makeTempDir(hooksTempDirs, "marketingclaw-config-");
    const transformsRoot = path.join(configDir, "hooks", "transforms");
    fs.mkdirSync(transformsRoot, { recursive: true });
  ...
  it("treats transform-provided session keys as templated by default", async () => {
    const result = await applyGmailTransformSessionKey({
      tempPrefix: "marketingclaw-config-sessionkey-xform-",
  ...
  it("uses transform-provided static session key source metadata", async () => {
    const result = await applyGmailTransformSessionKey({
      tempPrefix: "marketingclaw-config-sessionkey-static-",
      sessionKey: "hook:gmail:{{messages[0].subject}}",
      transformLines: [
        "export default () => ({",
        '  kind: "agent",',
  ...
  it("treats empty transform session keys as absent for source tracking", async () => {
    const result = await applyGmailTransformSessionKey({
      tempPrefix: "marketingclaw-config-sessionkey-empty-",
      sessionKey: "hook:gmail:{{messages[0].subject}}",
      transformLines: [
        "export default () => ({",
        '  kind: "agent",',
  ...
  it("defaults invalid transform session key source metadata to templated", async () => {
    const result = await applyGmailTransformSessionKey({
      tempPrefix: "marketingclaw-config-sessionkey-invalid-",
      transformLines: [
        "export default () => ({",
        '  kind: "agent",',
        '  message: "Transformed",',
  ...
  it("rejects transform module traversal outside transformsDir", () => {
    const configDir = makeTempDir(hooksTempDirs, "marketingclaw-config-traversal-");
    const transformsRoot = path.join(configDir, "hooks", "transforms");
    fs.mkdirSync(transformsRoot, { recursive: true });
    expect(() =>
      resolveHookMappings(
  ...
  it("rejects absolute transform module path outside transformsDir", () => {
    const configDir = makeTempDir(hooksTempDirs, "marketingclaw-config-abs-");
    const transformsRoot = path.join(configDir, "hooks", "transforms");
    fs.mkdirSync(transformsRoot, { recursive: true });
  ...
  it("rejects transformsDir traversal outside the transforms root", () => {
    const configDir = makeTempDir(hooksTempDirs, "marketingclaw-config-xformdir-trav-");
    const transformsRoot = path.join(configDir, "hooks", "transforms");
    fs.mkdirSync(transformsRoot, { recursive: true });
    expect(() =>
      resolveHookMappings(
  ...
  it("rejects transformsDir absolute path outside the transforms root", () => {
    const configDir = makeTempDir(hooksTempDirs, "marketingclaw-config-xformdir-abs-");
    const transformsRoot = path.join(configDir, "hooks", "transforms");
    fs.mkdirSync(transformsRoot, { recursive: true });
    expect(() =>
      resolveHookMappings(
  ...
  it("accepts transformsDir subdirectory within the transforms root", async () => {
    const configDir = makeTempDir(hooksTempDirs, "marketingclaw-config-xformdir-ok-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const configDir = makeTempDir(hooksTempDirs, "marketingclaw-config-symlink-module-");
      const transformsRoot = path.join(configDir, "hooks", "transforms");
      fs.mkdirSync(transformsRoot, { recursive: true });
      const outsideDir = makeTempDir(hooksTempDirs, "marketingclaw-outside-module-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const configDir = makeTempDir(hooksTempDirs, "marketingclaw-config-symlink-dir-");
      const transformsRoot = path.join(configDir, "hooks", "transforms");
      fs.mkdirSync(transformsRoot, { recursive: true });
      const outsideDir = makeTempDir(hooksTempDirs, "marketingclaw-outside-dir-");
  ...
  it.runIf(process.platform !== "win32")("accepts in-root transform module symlink", async () => {
    const configDir = makeTempDir(hooksTempDirs, "marketingclaw-config-symlink-ok-");
    const transformsRoot = path.join(configDir, "hooks", "transforms");
  ...
  it("treats null transform as a handled skip", async () => {
    const configDir = makeTempDir(hooksTempDirs, "marketingclaw-config-skip-");
  ...
  it("caches transform functions by module path and export name", async () => {
    const configDir = makeTempDir(hooksTempDirs, "marketingclaw-hooks-export-");
    const transformsRoot = path.join(configDir, "hooks", "transforms");
    fs.mkdirSync(transformsRoot, { recursive: true });
