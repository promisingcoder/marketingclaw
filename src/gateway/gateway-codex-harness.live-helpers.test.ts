...
describe("gateway codex harness live helpers", () => {
  ...
  it("accepts the current codex status prose from the live harness", () => {
    const text =
      "MarketingClaw is running on `openai/gpt-5.5` with low reasoning/text settings. Context is at `22k/272k` tokens, no compactions, and the current session is `agent:dev:live-codex-harness`.";
  ...
  it("accepts current status prose that reports session context without the session id", () => {
    const text = [
      "MarketingClaw is running on `openai/gpt-5.5` with low reasoning/text settings.",
      "",
  ...
  it("accepts current status prose that reports healthy session context without the session id", () => {
    const text = [
      ...
      "",
      "Session context is healthy: `22k/272k` tokens used, `0` compactions, `53%` cache hit. Current workspace is `/tmp/marketingclaw-live-codex-harness/workspace/dev`.",
    ].join("\n");

    expect(isExpectedCodexStatusCommandText(text)).toBe(true);
  });

  it("accepts current app-server status prose without the MarketingClaw prefix", () => {
    const text = [
      "Status: running on `openai/gpt-5.5` in `/tmp/marketingclaw-live-codex-harness/workspace/dev`.",
      "",
  ...
  it("accepts workspace-only healthy status prose emitted by current codex", () => {
    const text =
      "Working normally. Current workspace: `/tmp/marketingclaw-live-codex-harness/workspace/dev`.";
  ...
  it("accepts the OpenAI Codex status card emitted by the GPT-5.5 Docker harness", () => {
    const text = [
      "MarketingClaw 2026.4.30-beta.1 is running on `openai/gpt-5.5`.",
      "",
      "Session is healthy:",
  ...
  it("accepts the compact status-card pointer emitted by current codex", () => {
    const text = "MarketingClaw status shown above.";
  ...
  it("accepts the online idle status emitted by current codex", () => {
    const text =
      "I'm online in `/tmp/marketingclaw-live-codex-harness-KiaUQ4/workspace/dev`, with workspace-write access. No active task is running right now.";
  ...
  it("accepts the normal-work status emitted by current codex", () => {
    const text =
      "Working normally. Current cwd is `/tmp/marketingclaw-live-codex-harness/workspace/dev`, sandbox is workspace-write, network is restricted, and the current date is 2026-05-09 UTC.";
  ...
  it("rejects status prose for a different codex session", () => {
    const text =
      "MarketingClaw is running on `openai/gpt-5.5` with low reasoning/text settings. Context is at `22k/272k` tokens, no compactions, and the current session is `agent:dev:other`.";
  ...
  it("accepts the GPT-5.5 Docker harness shell fallback", () => {
    const text = [
      ...
      "",
      "Current session model from MarketingClaw status is `openai/gpt-5.5`.",
    ].join("\n");
