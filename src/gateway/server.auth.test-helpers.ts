...
function resolveGatewayTokenOrEnv(): string {
  ...
  if (typeof token !== "string") {
    throw new Error("expected gateway token in test state or MARKETINGCLAW_GATEWAY_TOKEN");
  }
  return token;
}
...
async function startRateLimitedTokenServerWithPairedDeviceToken() {
  ...
  const deviceIdentityPath = nextAuthIdentityPath("marketingclaw-auth-rate-limit");
  try {
...
async function ensurePairedDeviceTokenForCurrentIdentity(ws: WebSocket): Promise<{
  ...
  const deviceIdentityPath = nextAuthIdentityPath("marketingclaw-auth-device");
