...
describe("plugin node capability helpers", () => {
  test("builds scoped host urls from clean base urls", () => {
    ...
    ).toBe("http://127.0.0.1:18789/root/__marketingclaw__/cap/token%20value");
  ...
  test("normalizes scoped urls and moves capability into the query string", () => {
    const normalized = normalizePluginNodeCapabilityScopedUrl(
      "/__marketingclaw__/cap/token%20value/__marketingclaw__/canvas/file.txt?download=1",
    );
    expect(normalized).toEqual({
      pathname: "/__marketingclaw__/canvas/file.txt",
      capability: "token value",
      rewrittenUrl: "/__marketingclaw__/canvas/file.txt?download=1&oc_cap=token+value",
      scopedPath: true,
  ...
  test("replaces scoped capability tokens without nesting capability prefixes", () => {
    expect(
      replacePluginNodeCapabilityInScopedHostUrl(
        "http://127.0.0.1:18789/__marketingclaw__/cap/old-token/__marketingclaw__/a2ui/",
        "new token",
      ),
    ).toBe("http://127.0.0.1:18789/__marketingclaw__/cap/new%20token/__marketingclaw__/a2ui");
  });

  test("marks malformed scoped urls without authorizing a path capability", () => {
    const normalized = normalizePluginNodeCapabilityScopedUrl("/__marketingclaw__/cap/broken");
  ...
  test("refreshes client plugin surface url and stored capability", () => {
    const client = makeClient({
      pluginSurfaceUrls: {
        canvas: "http://127.0.0.1:18789/__marketingclaw__/cap/old-token",
      },
    ...
    expect(refreshed?.scopedUrl).toContain("/__marketingclaw__/cap/");
    expect(refreshed?.scopedUrl).not.toContain("old-token/__marketingclaw__/cap/");
  ...
  test("does not refresh client plugin capabilities when the clock is invalid", () => {
    ...
    expect(client.pluginSurfaceUrls?.canvas).toBe(
      "http://127.0.0.1:18789/__marketingclaw__/cap/old-token",
    );
