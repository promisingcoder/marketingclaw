...
describe("OpenResponses HTTP API (e2e)", () => {
  it("handles OpenResponses request parsing and validation", async () => {
    ...
    try {
      ...
      const resMissingAuth = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
        ...
        body: JSON.stringify({ model: "marketingclaw", input: "hi" }),
      });
      ...
      const resSessionOverride = await postResponses(
        ...
        {
          "x-marketingclaw-agent-id": "beta",
          "x-marketingclaw-session-key": "agent:beta:openresponses:custom",
        },
      );
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resModel = await postResponses(port, { model: "marketingclaw/beta", input: "hi" });
      ...
      expect(invalidOverrideJson.error?.message).toBe("Invalid `x-marketingclaw-model`.");
      expect(agentCommand).toHaveBeenCalledTimes(0);
  ...
  it("accepts write-scoped and admin-scoped HTTP callers", async () => {
    ...
    const streamingResponse = await postResponses(
      port,
      { stream: true, model: "marketingclaw", input: "hi" },
      { "x-marketingclaw-scopes": "operator.admin, operator.write" },
    );
