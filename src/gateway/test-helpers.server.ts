...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.MARKETINGCLAW_CONFIG_PATH) {
    configPaths.add(process.env.MARKETINGCLAW_CONFIG_PATH);
  }
  if (process.env.MARKETINGCLAW_STATE_DIR) {
    configPaths.add(path.join(process.env.MARKETINGCLAW_STATE_DIR, "marketingclaw.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  process.env.MARKETINGCLAW_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "marketingclaw-test-no-bundled-extensions")
    : "marketingclaw-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".marketingclaw-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".marketingclaw-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".marketingclaw-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>marketingclaw-test-control-ui</title>\n",
    "utf-8",
  );
  setTestConfigRoot(tempConfigRoot);
  resetConfigRuntimeState();
...
const CONNECT_CHALLENGE_NONCE_KEY = "__marketingclawTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__marketingclawTestConnectChallengeTracked";
...
export async function startServer(token?: string, opts?: GatewayServerOptions) {
  ...
  const envSnapshot = captureEnv(["MARKETINGCLAW_GATEWAY_TOKEN"]);
  const prev = process.env.MARKETINGCLAW_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.MARKETINGCLAW_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const suiteRoot = process.env.MARKETINGCLAW_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
