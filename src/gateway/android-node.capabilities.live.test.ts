...
const LIVE_ANDROID_NODE = isTruthyEnvValue(process.env.MARKETINGCLAW_LIVE_ANDROID_NODE);
...
function resolveGatewayConnection() {
  ...
  const urlOverride = readString(process.env.MARKETINGCLAW_ANDROID_GATEWAY_URL);
  const details = buildGatewayConnectionDetails({
    config: cfg,
    ...(urlOverride ? { url: urlOverride } : {}),
  });
  const tokenOverride = readString(process.env.MARKETINGCLAW_ANDROID_GATEWAY_TOKEN);
  const passwordOverride = readString(process.env.MARKETINGCLAW_ANDROID_GATEWAY_PASSWORD);
...
async function resolvePolicyConfigForRun(params: {
  ...
  loadLocalConfig?: () => MarketingClawConfig;
...
describe("resolvePolicyConfigForRun", () => {
  ...
  it("still uses local config loading for local loopback runs", async () => {
    const localConfig = { gateway: { bind: "127.0.0.1" } } as unknown as MarketingClawConfig;
...
function selectTargetNode(nodes: NodeListNode[]): NodeListNode {
  const nodeIdOverride = readString(process.env.MARKETINGCLAW_ANDROID_NODE_ID);
  if (nodeIdOverride) {
    const match = nodes.find((node) => node.nodeId === nodeIdOverride);
    if (!match) {
      throw new Error(`MARKETINGCLAW_ANDROID_NODE_ID not found in node.list: ${nodeIdOverride}`);
    }
    return match;
  }

  const nodeNameOverride = readString(process.env.MARKETINGCLAW_ANDROID_NODE_NAME)?.toLowerCase();
  if (nodeNameOverride) {
    ...
    if (!match) {
      throw new Error(
        `MARKETINGCLAW_ANDROID_NODE_NAME not found in node.list: ${nodeNameOverride}`,
      );
    }
    return match;
  }
    ...
    if (!target.connected || !target.paired) {
      ...
      throw new Error(
        [
          ...
          pendingHint,
          "precondition: open app, keep foreground, ensure pairing approved (`marketingclaw nodes pending` / `marketingclaw nodes approve <requestId>`)",
        ].join("\n"),
      );
    }
