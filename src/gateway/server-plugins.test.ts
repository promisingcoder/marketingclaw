...
const loadMarketingClawPlugins = vi.hoisted(() => vi.fn());
...
function getLastPluginLoadOptions(): Record<string, unknown> {
  return requireRecord(
    getLastMockFirstArg(loadMarketingClawPlugins, "plugin load"),
    "plugin load options",
  );
}
...
function getLastPluginLoadLogger(): {
  ...
  const call = getLastMockFirstArg(loadMarketingClawPlugins, "plugin load") as
    | {
        logger?: {
          info: (message: string) => void;
          warn: (message: string) => void;
...
async function createSubagentRuntime(
  ...
  loadMarketingClawPlugins.mockReturnValue(createRegistry([]));
...
beforeEach(() => {
  ...
  loadMarketingClawPlugins.mockReset();
...
describe("loadGatewayPlugins", () => {
  test("logs plugin errors with details", () => {
    ...
    loadMarketingClawPlugins.mockReturnValue(createRegistry(diagnostics));
  ...
  test("pins the initial startup channel registry against later active-registry churn", () => {
    ...
    loadMarketingClawPlugins.mockReturnValue(startupRegistry);
  ...
  test("lets trusted official plugin runtime request admin scope for browser proxy", async () => {
    loadMarketingClawPlugins.mockReturnValue(
  ...
  test("includes docs guidance when a plugin fallback override is not trusted", async () => {
    ...
    await expect(
      ...
      'plugin "voice-call" is not trusted for fallback provider/model override requests. See https://docs.marketingclaw.ai/plugins/sdk-runtime#api-runtime-subagent and search for: plugins.entries.<id>.subagent.allowModelOverride',
    );
  });
  ...
  test("runs registry hook before priming configured bindings", () => {
    ...
    loadMarketingClawPlugins.mockReturnValue(pluginRegistry);
