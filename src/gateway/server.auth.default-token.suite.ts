...
export function registerDefaultAuthTokenSuite(): void {
  describe("default auth (token)", () => {
    ...
    test("closes silent handshakes after timeout", async () => {
      vi.useRealTimers();
      const prevHandshakeTimeout = process.env.MARKETINGCLAW_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.MARKETINGCLAW_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        ...
        if (prevHandshakeTimeout === undefined) {
          delete process.env.MARKETINGCLAW_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.MARKETINGCLAW_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });

    test("prefers MARKETINGCLAW_HANDSHAKE_TIMEOUT_MS and falls back on empty string", () => {
      const prevHandshakeTimeout = process.env.MARKETINGCLAW_HANDSHAKE_TIMEOUT_MS;
      const prevTestHandshakeTimeout = process.env.MARKETINGCLAW_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.MARKETINGCLAW_HANDSHAKE_TIMEOUT_MS = "75";
      process.env.MARKETINGCLAW_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        expect(getPreauthHandshakeTimeoutMsFromEnv()).toBe(75);
        process.env.MARKETINGCLAW_HANDSHAKE_TIMEOUT_MS = "";
        expect(getPreauthHandshakeTimeoutMsFromEnv()).toBe(20);
      } finally {
        if (prevHandshakeTimeout === undefined) {
          delete process.env.MARKETINGCLAW_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.MARKETINGCLAW_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
        if (prevTestHandshakeTimeout === undefined) {
          delete process.env.MARKETINGCLAW_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.MARKETINGCLAW_TEST_HANDSHAKE_TIMEOUT_MS = prevTestHandshakeTimeout;
        }
      }
    });
      ...
      for (const testCase of [
        {
          env: {
            ...
            MARKETINGCLAW_SERVICE_VERSION: "2.4.6-service",
        ...
        {
          env: {
            MARKETINGCLAW_VERSION: "9.9.9-cli",
            MARKETINGCLAW_SERVICE_VERSION: "2.4.6-service",
            npm_package_version: "1.0.0-package",
          },
    ...
    test("hello-ok reports persisted token scopes when reusing an existing device token", async () => {
      ...
      const deviceIdentityPath = path.join(
        os.tmpdir(),
        `marketingclaw-shared-auth-scope-reuse-${randomUUID()}.json`,
      );
    ...
    test("does not grant admin when scopes are omitted", async () => {
      ...
      const { identity, device } = await createSignedDevice({
        ...
        identityPath: path.join(os.tmpdir(), `marketingclaw-test-device-${randomUUID()}.json`),
        nonce,
      });
