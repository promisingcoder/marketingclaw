...
test("sessions.list configuredAgentsOnly keeps configured-agent children and hides unrelated stores", async () => {
  ...
  if (!stateDir) {
    throw new Error("MARKETINGCLAW_STATE_DIR is required for gateway session tests");
  }
  testState.agentsConfig = { list: [{ id: "main", default: true }] };
  const configPath = process.env.MARKETINGCLAW_CONFIG_PATH;
  if (!configPath) {
    throw new Error("MARKETINGCLAW_CONFIG_PATH is required for gateway session tests");
  }
  await fs.writeFile(
    configPath,
