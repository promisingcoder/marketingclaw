...
describe("cron run log", () => {
  ...
  it("appends SQLite rows and prunes by line count", async () => {
    await withRunLogDir("marketingclaw-cron-log-", async (dir) => {
  ...
  it("reads run-log entries synchronously for task reconciliation", async () => {
    await withRunLogDir("marketingclaw-cron-log-sync-", async (dir) => {
      const storePath = storePathForDir(dir);
      await appendCronRunLog({
        storePath,
        entry: {
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      await withRunLogDir("marketingclaw-cron-log-perms-", async (dir) => {
        const storePath = storePathForDir(dir);
        const logPath = path.join(dir, "runs", "job-1.jsonl");
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      await withRunLogDir("marketingclaw-cron-log-dir-perms-", async (dir) => {
        const storePath = storePathForDir(dir);
  ...
  it("reads newest entries and filters by jobId", async () => {
    await withRunLogDir("marketingclaw-cron-log-read-", async (dir) => {
      const storePath = storePathForDir(dir);
  ...
  it("filters run-log pages by runId", async () => {
    await withRunLogDir("marketingclaw-cron-log-runid-", async (dir) => {
      const storePath = storePathForDir(dir);
      await appendCronRunLog({
        storePath,
        entry: {
  ...
  it("ignores invalid and non-finished lines while preserving delivery fields", async () => {
    await withRunLogDir("marketingclaw-cron-log-filter-", async (dir) => {
      const logPath = path.join(dir, "runs", "job-1.jsonl");
  ...
  it("dedupes legacy migration against all existing SQLite rows when archive is blocked", async () => {
    await withRunLogDir("marketingclaw-cron-log-large-migration-", async (dir) => {
      const logPath = path.join(dir, "runs", "job-1.jsonl");
      await fs.mkdir(path.dirname(logPath), { recursive: true });
  ...
  it("does not include raw delivery targets in run-log search", async () => {
    await withRunLogDir("marketingclaw-cron-log-target-query-", async (dir) => {
      const logPath = path.join(dir, "runs", "job-1.jsonl");
      await fs.mkdir(path.dirname(logPath), { recursive: true });
      await fs.writeFile(
        logPath,
  ...
  it("reads and searches run diagnostics", async () => {
    await withRunLogDir("marketingclaw-cron-log-diagnostics-", async (dir) => {
      const storePath = storePathForDir(dir);
  ...
  it("reads telemetry fields", async () => {
    await withRunLogDir("marketingclaw-cron-log-telemetry-", async (dir) => {
      const storePath = storePathForDir(dir);
      const logPath = path.join(dir, "runs", "job-1.jsonl");
  ...
  it("cleans up pending-write bookkeeping after appends complete", async () => {
    await withRunLogDir("marketingclaw-cron-log-pending-", async (dir) => {
      await appendCronRunLog({
  ...
  it("read drains pending fire-and-forget writes", async () => {
    await withRunLogDir("marketingclaw-cron-log-drain-", async (dir) => {
      const storePath = storePathForDir(dir);
  ...
  it("stamps jobNameById onto single-job page entries", async () => {
    await withRunLogDir("marketingclaw-cron-log-jobname-", async (dir) => {
      const storePath = storePathForDir(dir);
      for (const entry of [
        {
          ts: 1,
  ...
  it("normalizes the jobId on write so the write/read roundtrip is symmetric", async () => {
    await withRunLogDir("marketingclaw-cron-log-roundtrip-", async (dir) => {
      const storePath = storePathForDir(dir);
      await appendCronRunLog({
        storePath,
  ...
  it("rejects unsafe job ids on write the same way reads do", async () => {
    await withRunLogDir("marketingclaw-cron-log-write-reject-", async (dir) => {
      const storePath = storePathForDir(dir);
