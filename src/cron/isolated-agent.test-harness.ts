...
/** Runs a test callback with an isolated MarketingClaw home for cron tests. */
export async function withTempCronHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "marketingclaw-cron-" });
}
...
export async function writeSessionStoreEntries(
  ...
  const dir = path.join(home, ".marketingclaw", "sessions");
...
export function makeCfg(
  ...
  const base: MarketingClawConfig = {
    agents: {
      defaults: {
