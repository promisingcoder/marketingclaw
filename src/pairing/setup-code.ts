// Generates setup codes used to pair external channels with MarketingClaw.
...
} from "@marketingclaw/net-policy/ip";
import {
...
function resolvePairingSetupAuthLabel(
  ...
  const envToken = normalizeOptionalString(env.MARKETINGCLAW_GATEWAY_TOKEN);
  const envPassword = normalizeOptionalString(env.MARKETINGCLAW_GATEWAY_PASSWORD);
  const token =
...
export async function resolvePairingSetupFromConfig(
  ...
  const cfgForAuth = await materializeGatewayAuthSecretRefs({
    ...
    hasTokenCandidate: Boolean(normalizeOptionalString(env.MARKETINGCLAW_GATEWAY_TOKEN)),
    hasPasswordCandidate: Boolean(normalizeOptionalString(env.MARKETINGCLAW_GATEWAY_PASSWORD)),
  });
