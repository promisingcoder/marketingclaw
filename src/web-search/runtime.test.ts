...
function getCustomSearchApiKey(config?: MarketingClawConfig): unknown {
...
function createCustomSearchConfig(apiKey: unknown): MarketingClawConfig {
  return {
    plugins: {
      entries: {
        "custom-search": {
...
describe("web search runtime", () => {
  ...
  it("auto-detects a provider from a model-provider auth profile", async () => {
    const agentDir = mkdtempSync(path.join(os.tmpdir(), "marketingclaw-web-search-auth-"));
  ...
  it("auto-detects a provider from the active agent auth profile", async () => {
    const defaultAgentDir = mkdtempSync(
      path.join(os.tmpdir(), "marketingclaw-web-search-default-"),
    );
    const activeAgentDir = mkdtempSync(path.join(os.tmpdir(), "marketingclaw-web-search-active-"));
  ...
  it("passes the active agentDir into selected provider tools", async () => {
    const activeAgentDir = mkdtempSync(
      path.join(os.tmpdir(), "marketingclaw-web-search-tool-agent-"),
    );
