...
describe("environment docs", () => {
  it("documents the trusted sources for provider credentials", async () => {
    ...
    expect(markdown).toContain(
      "MarketingClaw ignores provider credentials and protected runtime controls from workspace `.env`",
    );
    expect(markdown).toContain("~/.marketingclaw/.env");
    expect(markdown).toContain("$MARKETINGCLAW_STATE_DIR/.env");
    expect(markdown).toContain("The config `env` block");
    expect(markdown).toContain("MARKETINGCLAW_LOAD_SHELL_ENV=1");
