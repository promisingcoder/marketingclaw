/**
 * Realtime voice tool definition and helpers for delegating work to MarketingClaw.
 *
...
export const REALTIME_VOICE_AGENT_CONSULT_TOOL_NAME = "marketingclaw_agent_consult";
...
export const REALTIME_VOICE_AGENT_CONSULT_TOOL: RealtimeVoiceTool = {
  ...
  description:
    "Delegate the caller's request to the configured MarketingClaw agent for normal tool-backed work, actions, context, memory, or reasoning before speaking.",
  parameters: {
    type: "object",
    properties: {
      question: {
...
export function buildRealtimeVoiceAgentConsultWorkingResponse(
  ...
  return {
    ...
    message: `Tell the ${audienceLabel} briefly that you are checking, then wait for the final MarketingClaw result before answering with the actual result.`,
  };
}
...
/** Resolve the MarketingClaw tool allowlist paired with the consult exposure policy. */
...
export function buildRealtimeVoiceAgentConsultPolicyInstructions(config: {
  ...
  if (config.consultPolicy === "always") {
    return [
      "Consult behavior:",
      "- Call marketingclaw_agent_consult before every substantive answer.",
  ...
  return [
    ...
    "- Call marketingclaw_agent_consult before answering requests that need facts, memory, current information, tools, workspace state, or the user's MarketingClaw-specific context.",
...
/** Build the delegated MarketingClaw agent prompt for a live voice consult. */
