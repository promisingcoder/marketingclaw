/**
 * Shared realtime voice controls for active MarketingClaw agent runs.
 *
...
export const REALTIME_VOICE_AGENT_CONTROL_TOOL_NAME = "marketingclaw_agent_control";
...
export function buildRealtimeVoiceAgentControlSpeechMessage(text: string): string {
  return [
    "Internal MarketingClaw voice control result.",
    "Do not call marketingclaw_agent_consult or any other tool for this message.",
    "Speak this exact MarketingClaw status to the voice call, without adding, removing, or rephrasing words.",
...
export function formatRealtimeVoiceAgentQueueRejection(
  ...
  if (reason === "compacting") {
    return "MarketingClaw is compacting the active run and cannot accept voice steering yet.";
  }
  if (reason === "not_streaming") {
    return "MarketingClaw has an active run, but it is not currently accepting steering.";
  }
  return mode === "followup"
    ? "MarketingClaw could not queue that follow-up."
    : "MarketingClaw could not steer the active run.";
}
...
export function formatRealtimeVoiceAgentStatus(params: {
  ...
  if (!params.active) {
    ...
    return turnEnded
      ? "MarketingClaw finished the last voice request."
  ...
  if (toolEvent) {
    ...
    if (toolEvent.type === "tool.call") {
      return name ? `MarketingClaw is starting ${name}.` : "MarketingClaw is starting a tool.";
    }
    if (toolEvent.type === "tool.result") {
      return name
        ? `MarketingClaw finished ${name} and is continuing.`
        : "MarketingClaw finished a tool and is continuing.";
    }
    if (toolEvent.type === "tool.progress") {
      return name
        ? `MarketingClaw is working in ${name}${phase ? ` (${phase})` : ""}.`
        : "MarketingClaw is still working.";
    }
  }

  if (params.activity?.activeToolName) {
    return `MarketingClaw is running ${params.activity.activeToolName}.`;
  }
  if (params.activity?.activeWorkKind === "model_call") {
    return "MarketingClaw is waiting on the model.";
  }
  if (params.activity?.activeWorkKind === "embedded_run" || params.activity?.hasActiveEmbeddedRun) {
    return "MarketingClaw is working on the current voice request.";
  }
