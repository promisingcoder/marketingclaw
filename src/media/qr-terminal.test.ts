...
describe("renderQrTerminal", () => {
  ...
  it("delegates terminal rendering to qrcode", async () => {
    await expect(renderQrTerminal("marketingclaw")).resolves.toBe("ASCII-QR");
    expect(toString).toHaveBeenCalledWith("marketingclaw", {
      small: false,
      type: "terminal",
    });
  });
  ...
  it("renders compact QR output without qrcode terminal small mode", async () => {
    const rendered = await renderQrTerminal("marketingclaw", { small: true });
    expect(rendered).toContain("▄");
    expect(create).toHaveBeenCalledWith("marketingclaw");
