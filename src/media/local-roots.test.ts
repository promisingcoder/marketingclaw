...
describe("local media roots", () => {
  function withStateDir<T>(stateDir: string, run: () => T): T {
    return withEnv({ MARKETINGCLAW_STATE_DIR: stateDir }, run);
  }
  ...
  it.each([
    {
      ...
      stateDir: path.join("/tmp", "marketingclaw-media-roots-state"),
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "marketingclaw-agent-media-roots-state"),
  ...
  it.each([
    {
      ...
      stateDir: path.join("/tmp", "marketingclaw-flexible-media-roots-state"),
      cfg: {},
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "marketingclaw-messaging-media-roots-state"),
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "marketingclaw-messaging-fs-media-roots-state"),
      cfg: {
        tools: {
          profile: "messaging",
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "marketingclaw-messaging-read-media-roots-state"),
      cfg: {
        tools: {
          profile: "messaging",
  ...
  it("keeps the config-dir media cache root when state and config paths differ", () => {
    const stateDir = path.join("/tmp", "marketingclaw-legacy-state");
    const configDir = path.join("/tmp", "marketingclaw-current-config");
