...
export const zh_CN = {
  ...
  wizard: {
    ...
    gateway: {
      ...
      plaintextPasswordHint: "直接把密码存入 MarketingClaw 配置",
    ...
    migration: {
      ...
      appliedTitle: "迁移已应用",
      cancelled: "迁移已取消",
      complete: "迁移完成。下一步运行 `marketingclaw doctor`。",
    ...
    remote: {
      ...
      insecureRemoteUrl:
        "远程主机请使用 wss://，或通过 SSH 隧道使用 ws://127.0.0.1/localhost。紧急绕过：可信私有网络可设置 MARKETINGCLAW_ALLOW_INSECURE_PRIVATE_WS=1。",
      ...
      plaintextPasswordHint: "直接把密码存入 MarketingClaw 配置",
      plaintextPasswordLabel: "现在输入密码",
      plaintextTokenHint: "直接把令牌存入 MarketingClaw 配置",
    ...
    guided: {
      ...
      escapeHatches:
        "如需完整的分步向导，请运行 `marketingclaw onboard --classic`。你也可以随时运行 `marketingclaw crestodian`，用自然语言获得设置帮助。",
      ...
      invalidConfigCrestodian:
        "MarketingClaw 配置无效。正在打开 Crestodian，以便在 onboarding 写入任何内容前检查并修复配置。",
      manualChoice: "你想如何连接 AI？",
      nextSteps:
        "工作区：{workspace}\n添加频道：`marketingclaw channels add`\n更喜欢聊天？运行 `marketingclaw crestodian`，然后说 `connect telegram`（或 `connect slack`）。\n打开 dashboard：`marketingclaw dashboard`\n稍后聊天：`marketingclaw`",
      nextStepsTitle: "下一步",
      openChatNow: "现在打开聊天？",
      ...
      skipAi: "暂时跳过 AI 设置",
      skipAiLater:
        "稍后添加 AI：设置 OPENAI_API_KEY 或 ANTHROPIC_API_KEY，或者安装并登录 codex、claude 或 gemini。然后重新运行 `marketingclaw onboard`。",
      testFailed: "AI 检查失败。",
      ...
      ttyRequired:
        "Onboarding 需要交互式 TTY。自动化请使用 `marketingclaw onboard --non-interactive --accept-risk ...`。",
      useClassic: "使用经典分步向导",
      welcomeTitle: "设置选项",
      workspace: "工作区目录",
    },
    ...
    security: {
      ...
      beta: "MarketingClaw 是一个爱好项目，仍处于 beta 阶段。请预期会有边缘问题。",
      confirm: "我理解 MarketingClaw 默认面向个人使用；共享/多用户使用需要加固。继续？",
      hardeningRequired: "如果你不熟悉安全加固和访问控制，请不要运行 MarketingClaw。",
      learnMore: "了解更多",
      notMultitenant: "MarketingClaw 默认不是敌对多租户安全边界。",
      personalAgent: "默认情况下，MarketingClaw 是个人 agent：一个可信操作者边界。",
    ...
    skills: {
      ...
      docsLine: "文档：https://docs.marketingclaw.ai/skills",
      enterEnv: "输入 {env}",
      ...
      manualPrereqsDoctorHint: "请先安装这些工具，然后运行 `marketingclaw doctor` 检查技能和要求。",
    ...
    nextcloudTalk: {
      ...
      helpInstallCommand:
        '2) 运行：./occ talk:bot:install "MarketingClaw" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction',
    ...
    whatsapp: {
      ...
      ownerNumberNote: "需要发送者/所有者号码，以便 MarketingClaw 将你加入允许列表。",
      ...
      separatePhoneLabel: "MarketingClaw 专用独立手机",
    ...
    irc: {
      ...
      nextStatusCommand: "命令：marketingclaw channels status --probe",
    ...
    synologyChat: {
      ...
      incomingWebhookHelpReplies: "这是 MarketingClaw 用来向 Chat 发送回复的 URL。",
    ...
    signal: {
      ...
      multipleEntries: "多个条目请用逗号分隔。",
      nextLinkDevice: '绑定设备：signal-cli link -n "MarketingClaw"',
    ...
    twitch: {
      ...
      envPrompt: "检测到 Twitch 环境变量 MARKETINGCLAW_TWITCH_ACCESS_TOKEN。使用环境 token？",
      helpCopyToken: "3. 复制 token（以 'oauth:' 开头）和 Client ID",
      helpCreateApp: "1. 在 https://dev.twitch.tv/console 创建 Twitch 应用",
      helpEnvVars: "支持环境变量：MARKETINGCLAW_TWITCH_ACCESS_TOKEN",
    ...
    channelsPrimer: {
      ...
      blurbs: {
        ...
        synologyChat: "将 Synology NAS Chat 连接到 MarketingClaw，并启用完整 agent 能力。",
    ...
    search: {
      ...
      configured: "已配置",
      configureLaterHint: "稍后可用 marketingclaw configure --section web 配置",
      docsLine: "文档：{url}",
    ...
    finalize: {
      ...
      controlUiDocs: "文档：https://docs.marketingclaw.ai/web/control-ui",
      dashboardCopyPaste: "在本机浏览器中复制/粘贴这个 URL 来控制 MarketingClaw。",
      dashboardLinkWithToken: "Dashboard 链接（含令牌）：{url}",
      dashboardOpened: "已在浏览器中打开。保留该标签页以控制 MarketingClaw。",
      ...
      gatewayTokenStored:
        "存储位置：$MARKETINGCLAW_CONFIG_PATH（默认 ~/.marketingclaw/marketingclaw.json）中的 gateway.auth.token，或 MARKETINGCLAW_GATEWAY_TOKEN。",
      ...
      optionalApps: "可选应用",
      outroDashboardLink: "Onboarding 完成。使用上面的 dashboard 链接控制 MarketingClaw。",
      outroDashboardOpened: "Onboarding 完成。Dashboard 已打开；保留该标签页以控制 MarketingClaw。",
      ...
      containerSystemdUnavailable:
        "此容器内没有 systemd 用户服务。MarketingClaw 只会跳过后台服务安装；请以前台方式运行 Gateway，或使用你的容器 supervisor。",
      securityReminder:
        "在你的电脑上运行 agent 存在风险，请加固设置：https://docs.marketingclaw.ai/security",
      ...
      webDocs: "文档：https://docs.marketingclaw.ai/tools/web",
      ...
      whatNow: '下一步：https://marketingclaw.ai/showcase（"What People Are Building"）。',
      whatNowTitle: "下一步",
      workspaceBackupTitle: "工作区备份",
      workspaceDocs: "文档：https://docs.marketingclaw.ai/concepts/agent-workspace",
