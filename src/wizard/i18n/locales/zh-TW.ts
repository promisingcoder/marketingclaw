...
export const zh_TW = {
  ...
  wizard: {
    ...
    gateway: {
      ...
      plaintextPasswordHint: "直接把密碼存入 MarketingClaw 設定",
    ...
    migration: {
      ...
      appliedTitle: "遷移已套用",
      cancelled: "遷移已取消",
      complete: "遷移完成。下一步執行 `marketingclaw doctor`。",
    ...
    remote: {
      ...
      insecureRemoteUrl:
        "遠端主機請使用 wss://，或透過 SSH 通道使用 ws://127.0.0.1/localhost。緊急繞過：可信私有網路可設定 MARKETINGCLAW_ALLOW_INSECURE_PRIVATE_WS=1。",
      ...
      plaintextPasswordHint: "直接把密碼存入 MarketingClaw 設定",
      plaintextPasswordLabel: "現在輸入密碼",
      plaintextTokenHint: "直接把權杖存入 MarketingClaw 設定",
    ...
    guided: {
      ...
      escapeHatches:
        "如需完整的分步精靈，請執行 `marketingclaw onboard --classic`。你也可以隨時執行 `marketingclaw crestodian`，用自然語言取得設定協助。",
      ...
      invalidConfigCrestodian:
        "MarketingClaw 設定無效。正在開啟 Crestodian，以便在 onboarding 寫入任何內容前檢查並修復設定。",
      manualChoice: "你想如何連接 AI？",
      nextSteps:
        "工作區：{workspace}\n新增頻道：`marketingclaw channels add`\n偏好聊天？執行 `marketingclaw crestodian`，然後說 `connect telegram`（或 `connect slack`）。\n開啟 dashboard：`marketingclaw dashboard`\n稍後聊天：`marketingclaw`",
      nextStepsTitle: "下一步",
      openChatNow: "現在開啟聊天？",
      ...
      skipAi: "暫時略過 AI 設定",
      skipAiLater:
        "稍後新增 AI：設定 OPENAI_API_KEY 或 ANTHROPIC_API_KEY，或者安裝並登入 codex、claude 或 gemini。然後重新執行 `marketingclaw onboard`。",
      testFailed: "AI 檢查失敗。",
      ...
      ttyRequired:
        "Onboarding 需要互動式 TTY。自動化請使用 `marketingclaw onboard --non-interactive --accept-risk ...`。",
      useClassic: "使用經典分步精靈",
      welcomeTitle: "設定選項",
      workspace: "工作區目錄",
    },
    ...
    security: {
      ...
      beta: "MarketingClaw 是一個興趣專案，仍處於 beta 階段。請預期會有邊緣問題。",
      confirm: "我理解 MarketingClaw 預設面向個人使用；共享/多使用者使用需要加固。繼續？",
      hardeningRequired: "如果你不熟悉安全加固和存取控制，請不要執行 MarketingClaw。",
      learnMore: "了解更多",
      notMultitenant: "MarketingClaw 預設不是敵對多租戶安全邊界。",
      personalAgent: "預設情況下，MarketingClaw 是個人 agent：一個可信操作者邊界。",
    ...
    skills: {
      ...
      docsLine: "文件：https://docs.marketingclaw.ai/skills",
      enterEnv: "輸入 {env}",
      ...
      manualPrereqsDoctorHint: "請先安裝這些工具，然後執行 `marketingclaw doctor` 檢查技能和需求。",
    ...
    nextcloudTalk: {
      ...
      helpInstallCommand:
        '2) 執行：./occ talk:bot:install "MarketingClaw" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction',
    ...
    whatsapp: {
      ...
      ownerNumberNote: "需要傳送者/擁有者號碼，以便 MarketingClaw 將你加入允許清單。",
      ...
      separatePhoneLabel: "MarketingClaw 專用獨立手機",
    ...
    synologyChat: {
      ...
      incomingWebhookHelpReplies: "這是 MarketingClaw 用來向 Chat 傳送回覆的 URL。",
    ...
    signal: {
      ...
      multipleEntries: "多個項目請用逗號分隔。",
      nextLinkDevice: '綁定裝置：signal-cli link -n "MarketingClaw"',
    ...
    twitch: {
      ...
      envPrompt: "偵測到 Twitch 環境變數 MARKETINGCLAW_TWITCH_ACCESS_TOKEN。使用環境 token？",
      helpCopyToken: "3. 複製 token（以 'oauth:' 開頭）和 Client ID",
      helpCreateApp: "1. 在 https://dev.twitch.tv/console 建立 Twitch 應用",
      helpEnvVars: "支援環境變數：MARKETINGCLAW_TWITCH_ACCESS_TOKEN",
    ...
    channelsPrimer: {
      ...
      blurbs: {
        ...
        synologyChat: "將 Synology NAS Chat 連接到 MarketingClaw，並啟用完整 agent 能力。",
    ...
    search: {
      ...
      configured: "已設定",
      configureLaterHint: "稍後可用 marketingclaw configure --section web 設定",
      docsLine: "文件：{url}",
    ...
    finalize: {
      ...
      controlUiDocs: "文件：https://docs.marketingclaw.ai/web/control-ui",
      dashboardCopyPaste: "在本機瀏覽器中複製/貼上這個 URL 來控制 MarketingClaw。",
      dashboardLinkWithToken: "Dashboard 連結（含權杖）：{url}",
      dashboardOpened: "已在瀏覽器中開啟。保留該分頁以控制 MarketingClaw。",
      ...
      gatewayTokenStored:
        "儲存位置：$MARKETINGCLAW_CONFIG_PATH（預設 ~/.marketingclaw/marketingclaw.json）中的 gateway.auth.token，或 MARKETINGCLAW_GATEWAY_TOKEN。",
      ...
      optionalApps: "可選應用",
      outroDashboardLink: "Onboarding 完成。使用上面的 dashboard 連結控制 MarketingClaw。",
      outroDashboardOpened: "Onboarding 完成。Dashboard 已開啟；保留該分頁以控制 MarketingClaw。",
      ...
      containerSystemdUnavailable:
        "此容器內沒有 systemd 使用者服務。MarketingClaw 只會略過背景服務安裝；請以前景方式執行 Gateway，或使用你的容器 supervisor。",
      securityReminder:
        "在你的電腦上執行 agent 存在風險，請加固設定：https://docs.marketingclaw.ai/security",
      ...
      webDocs: "文件：https://docs.marketingclaw.ai/tools/web",
      ...
      workspaceDocs: "文件：https://docs.marketingclaw.ai/concepts/agent-workspace",
