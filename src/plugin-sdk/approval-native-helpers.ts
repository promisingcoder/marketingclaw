...
type ChannelApprovalForwardingEvaluatorParams = {
  channel: string;
  isTransportEnabled: (params: { cfg: MarketingClawConfig; accountId?: string | null }) => boolean;
...
type NativeApprovalChannelRouteGateParams<TTarget extends NativeApprovalTarget> = {
  ...
  isTransportEnabled: (params: { cfg: MarketingClawConfig; accountId?: string | null }) => boolean;
  listAccountIds: (cfg: MarketingClawConfig) => readonly string[];
  resolveDefaultAccountId: (cfg: MarketingClawConfig) => string;
  normalizeForwardTarget: (target: NativeApprovalForwardTarget) => TTarget | null;
