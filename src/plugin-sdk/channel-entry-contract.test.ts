...
import type { MarketingClawPluginApi, PluginRegistrationMode } from "../plugins/types.js";
...
const pluginModuleLoaderJitiFactoryOverrideKey = Symbol.for(
  "marketingclaw.pluginModuleLoaderJitiFactoryOverride",
);
...
function createApi(registrationMode: PluginRegistrationMode): MarketingClawPluginApi {
  return {
    registrationMode,
...
function createBundledChannelEntry(params: {
  importerPath: string;
  pluginId: string;
  registerCliMetadata?: (api: MarketingClawPluginApi) => void;
  registerFull?: (api: MarketingClawPluginApi) => void;
}) {
...
describe("defineBundledChannelEntry", () => {
  it("runs tool registrations without channel sidecar hydration during tool discovery", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-bundled-entry-tools-"));
    ...
    const registerCliMetadata = vi.fn<(api: MarketingClawPluginApi) => void>();
    const registerFull = vi.fn<(api: MarketingClawPluginApi) => void>((api) => {
      api.registerTool(
        {
          name: "channel_tool",
          label: "Channel Tool",
  ...
  it("loads runtime sidecars during discovery registration", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-bundled-entry-runtime-"));
    tempDirs.push(tempRoot);
    const runtimeMarker = path.join(tempRoot, "runtime-loaded");
    ...
    const registerCliMetadata = vi.fn<(api: MarketingClawPluginApi) => void>();
    const registerFull = vi.fn<(api: MarketingClawPluginApi) => void>();
    const entry = createBundledChannelEntry({
      importerPath,
      pluginId,
      registerCliMetadata,
...
describe("defineBundledChannelSetupEntry", () => {
  it("exposes setup-runtime registrations without loading the full channel entry", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-bundled-setup-entry-"));
    tempDirs.push(tempRoot);
    const runtimeMarker = path.join(tempRoot, "runtime-loaded");
    const setupRuntimeRegister = vi.fn<(api: MarketingClawPluginApi) => void>();
...
async function expectBuiltArtifactNodeRequireFastPath(
  ...
  vi.stubEnv("MARKETINGCLAW_PLUGIN_LOAD_PROFILE", "1");
  ...
  try {
    ...
    const tempRoot = fs.mkdtempSync(
      path.join(os.tmpdir(), "marketingclaw-channel-entry-contract-"),
    );
    tempDirs.push(tempRoot);
...
function runCompiledEsmSidecarFastPathProbe(): SpawnSyncReturns<string> {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-channel-entry-contract-"));
  tempDirs.push(tempRoot);
  ...
  fs.writeFileSync(path.join(tempRoot, "marketingclaw.mjs"), "#!/usr/bin/env node\n", "utf8");
  ...
  return spawnSync(process.execPath, ["--import", "tsx", probePath], {
    cwd: process.cwd(),
    encoding: "utf8",
    env: { ...process.env, MARKETINGCLAW_PLUGIN_LOAD_PROFILE: "1" },
  });
}
...
describe("loadBundledEntryExportSync", () => {
  ...
  it("normalizes Windows absolute sidecar paths before module loads them", async () => {
    ...
    vi.doMock("../infra/boundary-file-read.js", () => ({
      openRootFileSync: () => ({
        ok: true,
        path: "C:\\Users\\alice\\marketingclaw\\dist\\extensions\\feishu\\helper.ts",
      ...
      try {
        ...
        expect(
          channelEntryContract.loadBundledEntryExportSync<number>(
            "file:///C:/Users/alice/marketingclaw/dist/extensions/feishu/index.js",
            {
              specifier: "./helper.ts",
              exportName: "load",
            },
        ...
        expect(jitiLoad).toHaveBeenCalledWith(
          "file:///C:/Users/alice/marketingclaw/dist/extensions/feishu/helper.ts",
        );
      } finally {
  ...
  it("can disable source-tree fallback for dist bundled entry checks", () => {
    ...
    fs.writeFileSync(path.join(tempRoot, "package.json"), '{"name":"marketingclaw"}\n', "utf8");
    const pluginRoot = path.join(tempRoot, "dist", "extensions", "telegram");
    ...
    vi.stubEnv("MARKETINGCLAW_DISABLE_BUNDLED_ENTRY_SOURCE_FALLBACK", "1");
