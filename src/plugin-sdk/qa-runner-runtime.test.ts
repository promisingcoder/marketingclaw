...
const resolveMarketingClawPackageRootSync = vi.hoisted(() => vi.fn());
...
describe("plugin-sdk qa-runner-runtime", () => {
  ...
  const originalPrivateQaCli = process.env.MARKETINGCLAW_ENABLE_PRIVATE_QA_CLI;
  ...
  beforeEach(() => {
    ...
    resolveMarketingClawPackageRootSync.mockReset().mockReturnValue(null);
  ...
  it("loads bundled plugin test APIs with the private QA source tree override", async () => {
    const sourceRoot = makePrivateQaSourceRoot(tempDirs, "marketingclaw-qa-test-api-root-");
    resolveMarketingClawPackageRootSync.mockReturnValue(sourceRoot);
    ...
    expect(testApiCall?.env?.MARKETINGCLAW_ENABLE_PRIVATE_QA_CLI).toBe("1");
    expect(testApiCall?.env?.MARKETINGCLAW_BUNDLED_PLUGINS_DIR).toBe(
  ...
  it("prefers the source bundled tree for private qa discovery in repo checkouts", async () => {
    const sourceRoot = makePrivateQaSourceRoot(tempDirs, "marketingclaw-qa-runner-root-");
    resolveMarketingClawPackageRootSync.mockReturnValue(sourceRoot);
    ...
    expect(manifestCall?.env?.MARKETINGCLAW_ENABLE_PRIVATE_QA_CLI).toBe("1");
    expect(manifestCall?.env?.MARKETINGCLAW_BUNDLED_PLUGINS_DIR).toBe(
      path.join(sourceRoot, "extensions"),
    );
    ...
    expect(publicSurfaceCall?.env?.MARKETINGCLAW_ENABLE_PRIVATE_QA_CLI).toBe("1");
    expect(publicSurfaceCall?.env?.MARKETINGCLAW_BUNDLED_PLUGINS_DIR).toBe(
      path.join(sourceRoot, "extensions"),
    );
  });
  ...
  it("fails when runtime registrations include an undeclared command", async () => {
    ...
    expect(() => module.listQaRunnerCliContributions()).toThrow(
      'QA runner plugin "qa-matrix" exported "extra" from runtime-api.js but did not declare it in marketingclaw.plugin.json',
    );
  });
});
