...
describe("channel-streaming", () => {
  ...
  it("keeps compacted raw progress lines from leaking unmatched markdown backticks", () => {
    const line = buildChannelProgressDraftLine(
      {
        ...
        args: {
          command:
            "node scripts/check-something-with-a-very-long-path /tmp/marketingclaw/some/really/deep/path/that/keeps/going/and/going/index.ts --flag value",
        },
      },
      { detailMode: "raw" },
    );
    ...
    expect(text).toBe(
      "Shelling\n\n🛠️ run node script…r…y/deep/path/that/keeps/going/and/going/index.ts --flag value",
    );
