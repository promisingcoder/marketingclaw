...
const MARKETINGCLAW_SOURCE_EXTENSIONS_ROOT = path.resolve(MARKETINGCLAW_PACKAGE_ROOT, "extensions");
...
function resolveFacadeModuleLocationUncached(params: {
  ...
  if (!areBundledPluginsDisabled(env)) {
    ...
    const bundledLocation = resolveBundledFacadeModuleLocation({
      ...
      packageRoot: MARKETINGCLAW_PACKAGE_ROOT,
      bundledPluginsDir,
    });
    if (bundledLocation) {
...
function buildFacadeActivationCheckParams(
  ...
  return {
    ...params,
    location,
    sourceExtensionsRoot: MARKETINGCLAW_SOURCE_EXTENSIONS_ROOT,
    resolutionKey: createFacadeResolutionKey(params),
  };
}
