...
function useBundledPluginDirOverrideForTest(dir: string): void {
  process.env.MARKETINGCLAW_BUNDLED_PLUGINS_DIR = dir;
...
describe("plugin-sdk facade runtime", () => {
  it("honors trusted bundled plugin dir overrides", () => {
    const overrideA = createBundledPluginDir("marketingclaw-facade-runtime-a-", "override-a");
    const overrideB = createBundledPluginDir("marketingclaw-facade-runtime-b-", "override-b");
  ...
  it("falls back to package source surfaces when an override dir is partial", () => {
    const overrideDir = createTrustedBundledFixtureRoot("marketingclaw-facade-runtime-empty-");
  ...
  it("returns the same object identity on repeated calls (sentinel consistency)", () => {
    const dir = createBundledPluginDir("marketingclaw-facade-identity-", "identity-check");
  ...
  it("breaks circular facade re-entry during module evaluation", () => {
    const dir = createBundledPluginDir("marketingclaw-facade-circular-", "circular-ok");
    const location = {
      modulePath: path.join(dir, "demo", "api.js"),
      boundaryRoot: dir,
    };
  ...
  it("back-fills the sentinel before post-load facade tracking re-enters", () => {
    const dir = createBundledPluginDir("marketingclaw-facade-post-load-", "post-load-ok");
    const location = {
      modulePath: path.join(dir, "demo", "api.js"),
      boundaryRoot: dir,
    };
  ...
  it("clears the cache on load failure so retries re-execute", () => {
    const dir = createThrowingPluginDir("marketingclaw-facade-throw-");
    useBundledPluginDirOverrideForTest(dir);
  ...
  it("allows runtime-api facade loads when the bundled plugin is explicitly enabled", () => {
    const dir = createTempDirSync("marketingclaw-facade-runtime-enabled-");
  ...
  it("rejects hardlinked artifacts under installed plugin roots", () => {
    const installedDir = createTempDirSync("marketingclaw-facade-hardlink-");
  ...
  it("keeps hardlinked artifacts loadable under core-shipped roots", () => {
    const rootDir = createTrustedBundledFixtureRoot("marketingclaw-facade-hardlink-bundled-");
    const pluginDir = path.join(rootDir, "demo");
    fs.mkdirSync(pluginDir, { recursive: true });
  ...
  it("resolves a globally-installed plugin whose rootDir basename matches the dirName", () => {
    const lineDir = createTempDirSync("marketingclaw-facade-global-line-");
    ...
    fs.writeFileSync(
      path.join(lineDir, "marketingclaw.plugin.json"),
      JSON.stringify({
        id: "line",
        channels: ["line"],
  ...
  it("resolves a globally-installed plugin public surface from package dist", () => {
    const lineDir = createTempDirSync("marketingclaw-facade-global-line-dist-");
  ...
  it("resolves a globally-installed plugin with an encoded scoped rootDir basename", () => {
    const encodedDir = createTempDirSync("marketingclaw-facade-encoded-line-");
    ...
    fs.writeFileSync(
      path.join(encodedDir, "marketingclaw.plugin.json"),
      JSON.stringify({
        id: "line",
        channels: ["line"],
        configSchema: { type: "object", additionalProperties: false, properties: {} },
  ...
  it("prefers the source runtime snapshot for facade activation checks", () => {
    const dir = createTempDirSync("marketingclaw-facade-source-snapshot-");
    ...
    fs.writeFileSync(
      path.join(dir, "demo", "marketingclaw.plugin.json"),
      JSON.stringify({
        id: "demo",
      }),
      "utf8",
  ...
  it("validates current snapshot against facade boundary config and ignores on mismatch", () => {
    const dir = createTempDirSync("marketingclaw-facade-snapshot-validate-");
    fs.mkdirSync(path.join(dir, "demo"), { recursive: true });
    fs.writeFileSync(
      path.join(dir, "demo", "runtime-api.js"),
    ...
    // Do NOT write marketingclaw.plugin.json on disk to force fallback to registry scan
    useBundledPluginDirOverrideForTest(dir);
      ...
      plugins: [
        {
          ...
          manifestPath: path.join(dir, "demo", "marketingclaw.plugin.json"),
          channels: ["demo"],
          providers: [],
          cliBackends: [],
          skills: [],
