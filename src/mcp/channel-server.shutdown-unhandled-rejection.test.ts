...
vi.mock("./channel-bridge.js", () => ({
  MarketingClawChannelBridge: class MockMarketingClawChannelBridge {
...
describe("serveMarketingClawChannelMcp shutdown", () => {
  ...
  it("does not leak unhandled rejections when shutdown close fails", async () => {
    ...
    const { serveMarketingClawChannelMcp } = await import("./channel-server.js");

    const servePromise = serveMarketingClawChannelMcp({ verbose: false });
