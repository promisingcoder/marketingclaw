...
type EnvSnapshot = {
  marketingclaw?: string;
};

function snapshotCurrentStateDirVars(): EnvSnapshot {
  return {
    marketingclaw: process.env.MARKETINGCLAW_STATE_DIR,
  };
}

function expectStateDirVars(snapshot: EnvSnapshot) {
  expect(process.env.MARKETINGCLAW_STATE_DIR).toBe(snapshot.marketingclaw);
}
...
describe("state-dir-env helpers", () => {
  it("set/snapshot/restore round-trips MARKETINGCLAW_STATE_DIR", () => {
    const prev = snapshotCurrentStateDirVars();
    const snapshot = snapshotStateDirEnv();

    setStateDirEnv("/tmp/marketingclaw-state-dir-test");
    expect(process.env.MARKETINGCLAW_STATE_DIR).toBe("/tmp/marketingclaw-state-dir-test");
  ...
  it("withStateDirEnv sets env for callback and cleans up temp root", async () => {
    ...
    await withStateDirEnv("marketingclaw-state-dir-env-", async ({ tempRoot, stateDir }) => {
  ...
  it("withStateDirEnv restores env and cleans temp root when callback throws", async () => {
    ...
    let capturedStateDir = "";
    await expect(
      withStateDirEnv("marketingclaw-state-dir-env-", async ({ tempRoot, stateDir }) => {
        capturedTempRoot = tempRoot;
        capturedStateDir = stateDir;
