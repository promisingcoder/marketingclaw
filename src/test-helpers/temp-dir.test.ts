...
async function makeParentRoot(): Promise<string> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-temp-dir-helper-test-"));
...
describe("withTempDir", () => {
  it("removes the cached async prefix root when the case finishes", async () => {
    ...
    await withTempDir({ prefix: "marketingclaw-leak-check-", parentDir }, async (dir) => {
  ...
  it("keeps the cached async prefix root while another case is active", async () => {
    ...
    const first = withTempDir({ prefix: "marketingclaw-shared-root-", parentDir }, async (dir) => {
      await fs.writeFile(path.join(dir, "first.txt"), "ok");
      await firstCanFinish;
    });

    await withTempDir({ prefix: "marketingclaw-shared-root-", parentDir }, async (dir) => {
  ...
  it("removes the cached sync prefix root when the case finishes", async () => {
    ...
    withTempDirSync({ prefix: "marketingclaw-leak-check-sync-", parentDir }, (dir) => {
