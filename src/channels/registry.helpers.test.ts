...
describe("channel registry helpers", () => {
  ...
  it("formats Telegram selection lines without a docs prefix and with website extras", () => {
    ...
    expect(line).toContain("https://marketingclaw.ai");
  });

  it("prefers the pinned channel registry when resolving registered plugin channels", () => {
    const startupRegistry = createRegistryWithRegisteredChannel("marketingclaw-weixin", ["weixin"]);
    ...
    expect(listRegisteredChannelPluginIds()).toEqual(["marketingclaw-weixin"]);
    expect(normalizeAnyChannelId("weixin")).toBe("marketingclaw-weixin");
    expect(getRegisteredChannelPluginMeta("MARKETINGCLAW-WEIXIN")?.aliases).toEqual(["weixin"]);
  });
