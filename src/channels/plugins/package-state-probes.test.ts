...
function makeBundledChannelCatalogEntry(params: {
  ...
  return {
    ...
    origin: "bundled",
    rootDir: "/tmp/marketingclaw-channel-plugin",
    channel: {
      id: params.channelId,
      configuredState: {
        env: {
...
describe("channel package-state probes", () => {
  ...
  it("prefers built bundled package-state probes when the catalog root is source", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-package-state-probe-"));
    tempDirs.push(root);
  ...
  it("falls back to source package-state probes when built artifacts are stale", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-package-state-fallback-"));
    tempDirs.push(root);
  ...
  it("preserves source overlay precedence over packaged package-state probes", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-package-state-overlay-"));
    tempDirs.push(root);
    const sourceRoot = path.join(root, "extensions", "matrix");
    const builtRoot = path.join(root, "dist", "extensions", "matrix");
    fs.mkdirSync(sourceRoot, { recursive: true });
  ...
  it("preserves parent-mounted source overlay precedence over packaged package-state probes", () => {
    const root = fs.mkdtempSync(
      path.join(os.tmpdir(), "marketingclaw-package-state-parent-overlay-"),
    );
    tempDirs.push(root);
  ...
  it("tries dist-runtime package-state probes before falling back to source", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-package-state-runtime-"));
    tempDirs.push(root);
    const sourceRoot = path.join(root, "extensions", "matrix");
    const builtRoot = path.join(root, "dist", "extensions", "matrix");
