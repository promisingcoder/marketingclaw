...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  resolveAllowlist: (params: { cfg: MarketingClawConfig; entries: string[] }) => Promise<TResolved>;
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: MarketingClawConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: MarketingClawConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: MarketingClawConfig, policy: ChannelAccessPolicy): MarketingClawConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const resolveAllowlist = vi.fn(
      async (params: { cfg: MarketingClawConfig; entries: string[] }) => {
        calls.push("resolve");
    ...
    const applyAllowlist = vi.fn((params: { cfg: MarketingClawConfig; resolved: string[] }) => {
      calls.push("apply");
