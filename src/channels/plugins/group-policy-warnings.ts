...
type ConfigGroupPolicyWarningCollector<Params extends { cfg: MarketingClawConfig }> = (
  params: Params,
) => string[];
...
export function projectConfigWarningCollector<Params extends { cfg: MarketingClawConfig }>(
  collector: WarningCollector<{ cfg: MarketingClawConfig }>,
): WarningCollector<Params> {
  return projectWarningCollector((params) => ({ cfg: params.cfg }), collector);
}

export function projectConfigAccountIdWarningCollector<
  Params extends { cfg: MarketingClawConfig; accountId?: string | null },
>(
  collector: WarningCollector<{ cfg: MarketingClawConfig; accountId?: string | null }>,
): WarningCollector<Params> {
...
export function projectAccountConfigWarningCollector<
  ResolvedAccount,
  ProjectedCfg,
  Params extends { account: ResolvedAccount; cfg: MarketingClawConfig },
>(
  projectCfg: (cfg: MarketingClawConfig) => ProjectedCfg,
...
export function createAllowlistProviderRestrictSendersWarningCollector<ResolvedAccount>(
  params: {
    providerConfigPresent: (cfg: MarketingClawConfig) => boolean;
  ...
  return createAllowlistProviderGroupPolicyWarningCollector({
    providerConfigPresent: params.providerConfigPresent,
    resolveGroupPolicy: ({ account }: { account: ResolvedAccount; cfg: MarketingClawConfig }) =>
...
export function createAllowlistProviderGroupPolicyWarningCollector<
  Params extends { cfg: MarketingClawConfig },
>(params: {
  providerConfigPresent: (cfg: MarketingClawConfig) => boolean;
