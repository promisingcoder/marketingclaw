...
describe("bundled root-aware plugin lookups", () => {
  it("reads bundled channel ids from the active bundled root without re-importing", async () => {
    const rootA = makeBundledRoot("marketingclaw-bundled-ids-a-");
    const rootB = makeBundledRoot("marketingclaw-bundled-ids-b-");

    vi.doMock("../../plugins/channel-catalog-registry.js", () => ({
      listChannelCatalogEntries: (params?: { env?: NodeJS.ProcessEnv }) => {
        const activeRoot = params?.env?.MARKETINGCLAW_BUNDLED_PLUGINS_DIR;
    ...
    process.env.MARKETINGCLAW_BUNDLED_PLUGINS_DIR = rootA.pluginsDir;
    expect(bundledIds.listBundledChannelPluginIds()).toEqual(["alpha"]);
    expect(bundledIds.listBundledChannelIds()).toEqual(["alpha-chat"]);

    process.env.MARKETINGCLAW_BUNDLED_PLUGINS_DIR = rootB.pluginsDir;
  ...
  it("reads bootstrap plugins from the active bundled root without re-importing", async () => {
    const rootA = makeBundledRoot("marketingclaw-bootstrap-a-");
    const rootB = makeBundledRoot("marketingclaw-bootstrap-b-");

    vi.doMock("./bundled-ids.js", () => ({
      listBundledChannelPluginIdsForRoot: () => {
        if (process.env.MARKETINGCLAW_BUNDLED_PLUGINS_DIR === rootA.pluginsDir) {
          return ["alpha"];
        }
        if (process.env.MARKETINGCLAW_BUNDLED_PLUGINS_DIR === rootB.pluginsDir) {
          return ["beta"];
        }
        return [];
      },
    ...
    vi.doMock("./bundled.js", () => ({
      ...
      getBundledChannelSetupPlugin: (id: string) => {
        const suffix = resolveMockRootSuffix({
          activeRoot: process.env.MARKETINGCLAW_BUNDLED_PLUGINS_DIR,
  ...
  it("retries bootstrap plugin loading after an error", async () => {
    const root = makeBundledRoot("marketingclaw-bootstrap-plugin-throw-");

    vi.doMock("./bundled-ids.js", () => ({
      listBundledChannelPluginIdsForRoot: () =>
        process.env.MARKETINGCLAW_BUNDLED_PLUGINS_DIR === root.pluginsDir ? ["alpha"] : [],
    }));
    ...
    process.env.MARKETINGCLAW_BUNDLED_PLUGINS_DIR = root.pluginsDir;
    expect(bootstrapRegistry.listBootstrapChannelPluginIds()).toEqual(["alpha"]);
  ...
  it("keeps plugin loading independent from bootstrap secrets loading errors", async () => {
    const root = makeBundledRoot("marketingclaw-bootstrap-secrets-throw-");
