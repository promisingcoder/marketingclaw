...
type PluginLoaderModule = {
  loadMarketingClawPlugins: (params: {
...
function listBuiltPluginLoaderModuleCandidateUrls(importerUrl: string): URL[] {
  ...
  // ../../plugins candidates would escape the installed marketingclaw package there.
...
function listManifestChannelAccountIds(cfg: MarketingClawConfig, channelId: string): string[] {
...
function resolveManifestChannelDefaultAccountId(
  cfg: MarketingClawConfig,
  channelId: string,
): string {
  const channelConfig = getChannelConfigRecord(cfg, channelId);
...
function rebindChannelPluginConfig(
  ...
  const rebind = (cfg: MarketingClawConfig) =>
...
export function resolveReadOnlyChannelPluginsForConfig(
  ...
  if (externalPluginIds.length > 0) {
    ...
    if (missingConfiguredChannelIds.length > 0 && options.includeSetupFallbackPlugins === true) {
      ...
      const registry = loadPluginLoaderModule().loadMarketingClawPlugins({
        config: cfg,
        activationSourceConfig: options.activationSourceConfig ?? cfg,
        env,
        workspaceDir,
