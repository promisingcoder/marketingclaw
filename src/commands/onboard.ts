/**
 * Top-level `marketingclaw onboard` command entrypoint.
 *
...
export async function setupWizardCommand(
  ...
  if (
    ...
    runtime.error(
      `Invalid --secret-input-mode. Use "plaintext" or "ref", or run ${formatCliCommand("marketingclaw onboard")} for the interactive setup.`,
    );
    runtime.exit(1);
    return;
  }
  ...
  if (normalizedOpts.resetScope && !VALID_RESET_SCOPES.has(normalizedOpts.resetScope)) {
    runtime.error(
      `Invalid --reset-scope. Use "config", "config+creds+sessions", or "full". Run ${formatCliCommand("marketingclaw onboard --reset --reset-scope config")} for a config-only reset.`,
    );
    runtime.exit(1);
    return;
  }
  ...
  if (normalizedOpts.nonInteractive && normalizedOpts.acceptRisk !== true) {
    ...
    runtime.error(
      [
        ...
        "Read: https://docs.marketingclaw.ai/security",
        `Re-run with: ${formatCliCommand("marketingclaw onboard --non-interactive --accept-risk ...")}`,
      ].join("\n"),
    );
    runtime.exit(1);
    return;
  ...
  if (process.platform === "win32") {
    runtime.log(
      [
        "Windows detected - MarketingClaw runs great on WSL2!",
        "Native Windows might be trickier.",
        "Quick setup: wsl --install (one command, one reboot)",
        "Guide: https://docs.marketingclaw.ai/windows",
      ].join("\n"),
    );
  }
