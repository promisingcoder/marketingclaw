...
const mocks = vi.hoisted(() => ({
  ...
  resolveMarketingClawWrapperPath: vi.fn(),
...
vi.mock("../daemon/program-args.js", () => ({
  ...
  resolveMarketingClawWrapperPath: mocks.resolveMarketingClawWrapperPath,
}));
...
function mockNodeGatewayPlanFixture(
  ...
  const {
    ...
    supported = true,
    warning,
    serviceEnvironment = { MARKETINGCLAW_PORT: "3000" },
  } = params;
  const workingDirectory = Object.hasOwn(params, "workingDirectory")
    ? params.workingDirectory
    : "/Users/me";
  mocks.resolvePreferredNodePath.mockResolvedValue("/opt/node");
  mocks.resolveMarketingClawWrapperPath.mockImplementation(async (value: string | undefined) =>
...
async function buildPluginConfigExecSecretRefPlan(home: string) {
  mockNodeGatewayPlanFixture({ serviceEnvironment: { MARKETINGCLAW_PORT: "3000" } });
...
describe("buildGatewayInstallPlan", () => {
  ...
  // Prevent tests from reading the developer's real ~/.marketingclaw/.env when
  ...
  it("uses provided nodePath and returns plan", async () => {
    ...
    expect(plan.environment).toEqual({ MARKETINGCLAW_PORT: "3000" });
  ...
  it("adds the active marketingclaw command bin directory to the managed service PATH", async () => {
    mockNodeGatewayPlanFixture();
    const originalArgv = process.argv;
    const marketingclawBinPath = path.join(isolatedHome, ".npm-global", "bin", "marketingclaw");
    process.argv = ["node", marketingclawBinPath, "gateway", "install"];
    ...
    ).toStrictEqual(["/opt/homebrew/opt/node/bin", path.dirname(marketingclawBinPath)]);
  });
  ...
  it("uses the state dir as the default macOS launchd working directory", async () => {
    ...
    expect(plan.workingDirectory).toBe(path.join(isolatedHome, ".marketingclaw"));
    expect(mocks.buildServiceEnvironment).toHaveBeenCalledOnce();
  ...
  it("passes MARKETINGCLAW_WRAPPER through program args and managed service env", async () => {
    const wrapperPath = path.resolve("/usr/local/bin/marketingclaw-doppler");
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        MARKETINGCLAW_PORT: "3000",
    ...
    expect(plan.environment.MARKETINGCLAW_WRAPPER).toBe(wrapperPath);
  });

  it("clears a Windows wrapper env that points at the generated gateway.cmd script", async () => {
    const selfWrapperPath = path.join(isolatedHome, ".marketingclaw", "gateway.cmd");
    const warn = vi.fn();
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        MARKETINGCLAW_PORT: "3000",
    ...
    const plan = await buildGatewayInstallPlan({
      env: isolatedPlanEnv({
        MARKETINGCLAW_WRAPPER: selfWrapperPath,
      }),
      port: 3000,
      runtime: "node",
      platform: "win32",
    ...
    expect(plan.environment.MARKETINGCLAW_WRAPPER).toBeUndefined();
    expect(warn).toHaveBeenCalledWith(
      expect.stringContaining(
        "Ignoring MARKETINGCLAW_WRAPPER because it points to the Windows task script",
      ),
    );
  });
  ...
  it("tracks safe config env keys without embedding literal values", async () => {
    ...
    expect(plan.environment.MARKETINGCLAW_PORT).toBe("3000");
    expect(plan.environment.MARKETINGCLAW_SERVICE_MANAGED_ENV_KEYS).toBe(
  ...
  it("renders config env SecretRefs as file-backed managed values on Linux", async () => {
    ...
    expect(plan.environment.MARKETINGCLAW_SERVICE_MANAGED_ENV_KEYS).toBe("DISCORD_BOT_TOKEN");
  });
  ...
  it("keeps config env SecretRefs managed when auth profiles reuse the key", async () => {
    ...
    expect(plan.environment.MARKETINGCLAW_SERVICE_MANAGED_ENV_KEYS).toBe("OPENAI_API_KEY");
  });
  ...
  it("includes passEnv values for configured exec SecretRef providers", async () => {
    ...
    expect(plan.environment.MARKETINGCLAW_SERVICE_MANAGED_ENV_KEYS).toBeUndefined();
  });
  ...
  it("allows safe inherited passEnv names while blocking dangerous exec SecretRef env", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      env: isolatedPlanEnv({
        BASH_ENV: "/tmp/marketingclaw-test-bashenv",
        XDG_CONFIG_HOME: "/tmp/marketingclaw-test-xdg-home",
  ...
  it("does not embed gateway auth SecretRef values into the service environment", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      env: isolatedPlanEnv({
        MARKETINGCLAW_GATEWAY_TOKEN: "gateway-test-token",
      }),
      port: 3000,
      runtime: "node",
      config: {
        gateway: {
          auth: {
            token: { source: "env", provider: "default", id: "MARKETINGCLAW_GATEWAY_TOKEN" },
          },
        },
      },
    });

    expect(plan.environment.MARKETINGCLAW_GATEWAY_TOKEN).toBeUndefined();
    expect(plan.environment.MARKETINGCLAW_SERVICE_MANAGED_ENV_KEYS).toBeUndefined();
  });
  ...
  it("does not inline config env SecretRef values already backed by state-dir dotenv", async () => {
    await writeStateDirDotEnv("DISCORD_BOT_TOKEN=discord-dotenv-token\n", {
      stateDir: path.join(isolatedHome, ".marketingclaw"),
    });
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        MARKETINGCLAW_PORT: "3000",
...
describe("buildGatewayInstallPlan — dotenv merge", () => {
  ...
  it("tracks .env vars with config while preserving service precedence", async () => {
    await writeStateDirDotEnv(
      ...
      {
        stateDir: path.join(tmpDir, ".marketingclaw"),
      },
    );
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
  ...
  it("retains .env values for macOS LaunchAgent env SecretRefs", async () => {
    ...
    expect(plan.environment.MARKETINGCLAW_SERVICE_MANAGED_ENV_KEYS).toBe("MINIMAX_API_KEY");
  });
  ...
  it("retains existing generated env-file SecretRef values for macOS LaunchAgent regeneration", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      existingEnvironment: {
        ...
        MARKETINGCLAW_SERVICE_MANAGED_ENV_KEYS:
  ...
  it("does not retain config env values for macOS LaunchAgent env files", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      existingEnvironment: {
        ...
        MARKETINGCLAW_SERVICE_MANAGED_ENV_KEYS: "BRAVE_API_KEY",
      },
      existingEnvironmentValueSources: {
        BRAVE_API_KEY: "file",
        MARKETINGCLAW_SERVICE_MANAGED_ENV_KEYS: "inline",
  ...
  it("preserves safe custom vars from an existing service env and merges PATH", async () => {
    ...
    expect(plan.environment.MARKETINGCLAW_SERVICE_MARKER).toBeUndefined();
  });
  ...
  it("drops keys that were previously tracked as managed service env", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      existingEnvironment: {
        ...
        BLOGWATCHER_HOME: "/Users/test/.blogwatcher",
        GOPATH: "/Users/test/.local/gopath",
        MARKETINGCLAW_SERVICE_MANAGED_ENV_KEYS: "GOBIN,GOPATH",
      },
    });
  ...
  it("drops legacy inline env values when the key is now managed by .env", async () => {
    ...
    expect(plan.environment.MARKETINGCLAW_SERVICE_MANAGED_ENV_KEYS).toBe("TAVILY_API_KEY");
  ...
  it("keeps source metadata for EnvironmentFile-backed preserved vars", async () => {
    ...
    expect(plan.environment.MARKETINGCLAW_GATEWAY_TOKEN).toBeUndefined();
    expect(plan.environmentValueSources?.MARKETINGCLAW_GATEWAY_TOKEN).toBeUndefined();
  });
...
describe("gatewayInstallErrorHint", () => {
  it("returns platform-specific hints", () => {
    ...
    expect(gatewayInstallErrorHint("linux")).toMatch(
      /(?:marketingclaw|marketingclaw)( --profile isolated)? gateway install/,
    );
  });
});
...
describe("collectPreservedExistingServiceEnvVars — operator opt-in allowlist", () => {
  ...
  it("continues to drop stale MARKETINGCLAW_ALLOW_ROOT", () => {
    const result = collectPreservedExistingServiceEnvVars(
      { MARKETINGCLAW_ALLOW_ROOT: "1" },
      managedKeys,
    );
    expect(result.MARKETINGCLAW_ALLOW_ROOT).toBeUndefined();
  });

  it("preserves MARKETINGCLAW_CLI_CONTAINER_BYPASS and MARKETINGCLAW_CONTAINER_HINT", () => {
    const result = collectPreservedExistingServiceEnvVars(
      {
        MARKETINGCLAW_CLI_CONTAINER_BYPASS: "1",
        MARKETINGCLAW_CONTAINER_HINT: "ci",
      },
      managedKeys,
    );
    expect(result.MARKETINGCLAW_CLI_CONTAINER_BYPASS).toBe("1");
    expect(result.MARKETINGCLAW_CONTAINER_HINT).toBe("ci");
  });

  it("still drops arbitrary MARKETINGCLAW_FOO", () => {
    const result = collectPreservedExistingServiceEnvVars(
      { MARKETINGCLAW_FOO: "bar" },
      managedKeys,
    );
    expect(result.MARKETINGCLAW_FOO).toBeUndefined();
  });

  it("preserves container opt-ins while dropping unrelated MARKETINGCLAW_* keys", () => {
    const result = collectPreservedExistingServiceEnvVars(
      {
        MARKETINGCLAW_CLI_CONTAINER_BYPASS: "1",
        MARKETINGCLAW_CONTAINER_HINT: "ci",
        MARKETINGCLAW_BAZ: "qux",
      },
      managedKeys,
    );
    expect(result.MARKETINGCLAW_CLI_CONTAINER_BYPASS).toBe("1");
    expect(result.MARKETINGCLAW_CONTAINER_HINT).toBe("ci");
    expect(result.MARKETINGCLAW_BAZ).toBeUndefined();
  });
});
