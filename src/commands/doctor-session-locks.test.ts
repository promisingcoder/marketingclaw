...
describe("noteSessionLockHealth", () => {
  let state: MarketingClawTestState;

  beforeEach(async () => {
    note.mockClear();
    state = await createMarketingClawTestState({
      layout: "state-only",
      prefix: "marketingclaw-doctor-locks-",
    });
  });
  ...
  it("uses the supplied env to choose the structured lint state dir", async () => {
    const other = await createMarketingClawTestState({
      layout: "state-only",
      prefix: "marketingclaw-doctor-locks-other-",
      applyEnv: false,
    });
    try {
  ...
  it("preserves report-only live MarketingClaw locks in dry-run repair effects", async () => {
    const sessionsDir = state.sessionsDir();
    await fs.mkdir(sessionsDir, { recursive: true });
    ...
    expect(sessionLockToHealthFinding(lock).fixHint).toBe(
      "MarketingClaw is preserving this live owned lock; inspect the owning process if it appears stuck.",
    );
    expect(sessionLockToRepairEffect(lock)).toEqual({
      kind: "state",
  ...
  it("uses configured stale threshold without removing live MarketingClaw lock files", async () => {
    const sessionsDir = state.sessionsDir();
    await fs.mkdir(sessionsDir, { recursive: true });
  ...
  it("removes fresh live locks when the owner is not an MarketingClaw process", async () => {
    const sessionsDir = state.sessionsDir();
    await fs.mkdir(sessionsDir, { recursive: true });
    ...
    expect(note).toHaveBeenCalledTimes(1);
    const [message] = firstNoteCall();
    expect(message).toContain("stale=yes (non-marketingclaw-owner)");
    expect(message).toContain("[removed]");
    expect(message).toContain("Removed 1 stale session lock file");
