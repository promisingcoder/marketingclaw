 ...
 * those are the values the marketingclaw-agent-driven flow would have used. When
...
function useTempStateDir(): string {
  const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-acp-sessions-state-"));
 ...
 * non-ACP sessions — those run through the marketingclaw-agent-driven flow that
...
describe("sessionsCommand model/modelProvider display for ACP sessions (catalog #20)", () => {
  ...
  it("RED: ACP control-plane session must NOT report the agent-configured model", async () => {
    ...
    // model the marketingclaw-agent-driven flow would have used, NOT what copilot
  ...
  it("RED (fix-shape): ACP control-plane session should report the ACP runtime sentinel", async () => {
    ...
    // into the marketingclaw record would require capturing ACP `session.model_change`
  ...
  it("GREEN control: non-ACP session correctly reports the agent-configured model", async () => {
    ...
    // model IS the right answer because the marketingclaw-agent-driven flow
