// Implements `marketingclaw uninstall`.
...
async function stopAndUninstallService(runtime: RuntimeEnv): Promise<boolean> {
  if (isNixMode) {
    ...
    runtime.error(
      `Nix mode detected; service uninstall is disabled. Manage the service through your Nix profile instead, then run ${formatCliCommand("marketingclaw status")} to verify.`,
    );
    return false;
  }
  ...
  try {
    ...
    runtime.error(
      `Gateway service check failed: ${formatErrorMessage(err)}. Run ${formatCliCommand("marketingclaw gateway status --deep")} for service diagnostics.`,
    );
    return false;
  }
  if (!loaded) {
  ...
  try {
    ...
    runtime.error(
      `Gateway stop failed: ${formatErrorMessage(err)}. Run ${formatCliCommand("marketingclaw gateway status --deep")} before retrying uninstall.`,
    );
  }
  try {
    ...
    runtime.error(
      `Gateway uninstall failed: ${formatErrorMessage(err)}. Run ${formatCliCommand("marketingclaw gateway status --deep")} for the service state.`,
    );
    return false;
  }
}
...
async function removeMacApp(runtime: RuntimeEnv, dryRun?: boolean) {
  ...
  await removePath("/Applications/MarketingClaw.app", runtime, {
    dryRun,
    label: "/Applications/MarketingClaw.app",
  });
}
...
export async function uninstallCommand(runtime: RuntimeEnv, opts: UninstallOptions) {
  ...
  if (!interactive && !opts.yes) {
    runtime.error(
      `Non-interactive uninstall requires --yes. Preview first with ${formatCliCommand("marketingclaw uninstall --dry-run --all")}.`,
    );
    runtime.exit(1);
    return;
  }
  ...
  if (!hadExplicit) {
    ...
    const selection = await multiselectStyled<UninstallScope>({
      ...
      options: [
        ...
        { value: "state", label: "State + config", hint: "~/.marketingclaw" },
        { value: "workspace", label: "Workspace", hint: "agent files" },
        {
          value: "app",
          label: "macOS app",
          hint: "/Applications/MarketingClaw.app",
        },
      ],
