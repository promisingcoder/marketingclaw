...
function getWebSearchConfig(config: MarketingClawConfig | undefined, pluginId: string) {
  return (config as WebSearchConfigRecord | undefined)?.plugins?.entries?.[pluginId]?.config
    ?.webSearch;
}

function ensureWebSearchConfig(config: MarketingClawConfig, pluginId: string) {
...
function createSearchProviderEntry(id: string): PluginWebSearchProviderEntry {
  ...
  const entry: PluginWebSearchProviderEntry = {
    ...
    applySelectionConfig: (config) => {
      const next: MarketingClawConfig = { ...config, plugins: { ...config.plugins } };
      const entries = { ...next.plugins?.entries } as NonNullable<
        NonNullable<MarketingClawConfig["plugins"]>["entries"]
      >;
...
function createPerplexityConfig(apiKey: string, enabled?: boolean): MarketingClawConfig {
  return {
    tools: {
      web: {
        search: {
...
function pluginWebSearchApiKey(config: MarketingClawConfig, pluginId: string): unknown {
  const entry = (
...
function createDisabledFirecrawlConfig(apiKey?: string): MarketingClawConfig {
  return {
    tools: {
      web: {
        search: {
...
function readFirecrawlPluginApiKey(config: MarketingClawConfig): string | undefined {
...
async function runQuickstartPerplexitySetup(
  apiKey: string,
  enabled?: boolean,
): Promise<{ result: MarketingClawConfig; prompter: WizardPrompter }> {
  const cfg = createPerplexityConfig(apiKey, enabled);
...
describe("setupSearch", () => {
  ...
  it("sets provider keys and enables plugin entries", async () => {
    ...
    const kimiCfg: MarketingClawConfig = {};
