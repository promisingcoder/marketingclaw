...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-gateway-health-auth-"));
  try {
...
describe("resolveGatewayHealthProbeToken", () => {
  const originalGatewayToken = process.env.MARKETINGCLAW_GATEWAY_TOKEN;
  const originalGatewayPassword = process.env.MARKETINGCLAW_GATEWAY_PASSWORD;

  afterEach(() => {
    if (originalGatewayToken === undefined) {
      delete process.env.MARKETINGCLAW_GATEWAY_TOKEN;
    } else {
      process.env.MARKETINGCLAW_GATEWAY_TOKEN = originalGatewayToken;
    }
    if (originalGatewayPassword === undefined) {
      delete process.env.MARKETINGCLAW_GATEWAY_PASSWORD;
    } else {
      process.env.MARKETINGCLAW_GATEWAY_PASSWORD = originalGatewayPassword;
    }
  });
  ...
  it("resolves file SecretRefs for the local onboarding health probe without persisting plaintext", async () => {
    await withTempDir(async (dir) => {
      ...
      process.env.MARKETINGCLAW_GATEWAY_TOKEN = "stale-env-token";
  ...
  it("does not fall back to stale MARKETINGCLAW_GATEWAY_TOKEN when a SecretRef is unresolved", async () => {
    await withTempDir(async (dir) => {
      process.env.MARKETINGCLAW_GATEWAY_TOKEN = "stale-env-token";
  ...
  it("resolves password auth for the local onboarding health probe", async () => {
    process.env.MARKETINGCLAW_GATEWAY_TOKEN = "stale-env-token";
    process.env.MARKETINGCLAW_GATEWAY_PASSWORD = "resolved-password"; // pragma: allowlist secret
