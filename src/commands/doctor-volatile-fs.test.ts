...
describe("detectLinuxVolatileStateDir", () => {
  const TMPFS_MOUNT_INFO = [
    ...
    "35 22 0:35 / /home/user/.marketingclaw rw - tmpfs tmpfs rw,size=1048576k",
  ].join("\n");
  const RAMFS_MOUNT_INFO = [
    "22 1 0:21 / / rw,relatime - ext4 /dev/sda1 rw",
    "35 22 0:35 / /home/user/.marketingclaw rw - ramfs ramfs rw",
  ].join("\n");
  ...
  it.each([
    ...
    const result = detectLinuxVolatileStateDir("/home/user/.marketingclaw", {
      platform: "linux",
      mountInfo,
    ...
    expect(result).toMatchObject({
      path: "/home/user/.marketingclaw",
      mountPoint: "/home/user/.marketingclaw",
      fsType,
    });
  });
  ...
  it("uses the most specific matching mount", () => {
    const mountInfo = [
      ...
      "35 30 0:35 / /home/user/.marketingclaw rw - tmpfs tmpfs rw",
    ].join("\n");

    expect(
      detectLinuxVolatileStateDir("/home/user/.marketingclaw", {
        platform: "linux",
        mountInfo,
        resolveRealPath: (targetPath) => targetPath,
      }),
  ...
  it("detects a missing state directory through an existing symlink", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-doctor-volatile-"));
    try {
      ...
      const result = detectLinuxVolatileStateDir(path.join(stateLink, "marketingclaw"), {
        platform: "linux",
        mountInfo: [
          "22 1 0:21 / / rw,relatime - ext4 /dev/sda1 rw",
      ...
      expect(result).toMatchObject({
        path: path.join(resolvedVolatileMount, "marketingclaw"),
...
describe("formatLinuxVolatileStateDirWarning", () => {
  it("covers all SQLite state and sidecar files under the volatile state directory", () => {
    const warning = formatLinuxVolatileStateDirWarning("~/.marketingclaw", {
      path: "/home/user/.marketingclaw",
      mountPoint: "/home/user/.marketingclaw",
      fsType: "tmpfs",
    });
    ...
    expect(warning).toContain("MARKETINGCLAW_STATE_DIR");
  });
});
