...
const hasUsableCustomProviderApiKey = vi.hoisted(() =>
  vi.fn<(_cfg?: MarketingClawConfig, _provider?: string, _env?: NodeJS.ProcessEnv) => boolean>(
    () => false,
  ),
);
...
const createProviderAuthChecker = vi.hoisted(() =>
  vi.fn(
    (params: { cfg?: MarketingClawConfig; workspaceDir?: string; env?: NodeJS.ProcessEnv }) =>
...
describe("promptDefaultModel", () => {
  it("adds runtime-route hints for canonical OpenAI models", async () => {
    ...
    await promptDefaultModel({
      config: { agents: { defaults: {} } } as MarketingClawConfig,
      prompter,
      allowKeep: false,
      includeManual: false,
  ...
  it("uses the configured default agent dir for provider-scoped catalog auth", async () => {
    ...
    const env = {
      ...process.env,
      MARKETINGCLAW_STATE_DIR: "/tmp/marketingclaw-picker-state",
    };
    const config = {
      agents: {
    ...
    expect(loadPreferredProviderPickerCatalog).toHaveBeenCalledWith({
      cfg: config,
      preferredProvider: "nvidia",
      agentDir: "/tmp/marketingclaw-picker-state/agents/worker/agent",
      env,
    });
  });
  ...
  it("supports configuring vLLM during setup", async () => {
    ...
    const prompter = makePrompter({ select });
    const config = { agents: { defaults: {} } } as MarketingClawConfig;
