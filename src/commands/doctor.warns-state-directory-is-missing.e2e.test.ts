...
describe("doctor command", () => {
  ...
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    await withEnvAsync({ MARKETINGCLAW_STATE_DIR: missingDir }, async () => {
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
        workspaceSuggestions: false,
      });
  ...
  it("skips gateway auth warning when MARKETINGCLAW_GATEWAY_TOKEN is set", async () => {
    mockDoctorConfigSnapshot({
      config: {
        gateway: { mode: "local" },
      },
    ...
    const prevToken = process.env.MARKETINGCLAW_GATEWAY_TOKEN;
    process.env.MARKETINGCLAW_GATEWAY_TOKEN = "env-token-1234567890";
    try {
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
        workspaceSuggestions: false,
  ...
  it("warns when token and password are both configured and gateway.auth.mode is unset", async () => {
    ...
    expect(String(gatewayAuthNote[0])).toContain(
      "marketingclaw config set gateway.auth.mode password",
    );
  });
  ...
  it("keeps doctor read-only when gateway token is SecretRef-managed but unresolved", async () => {
    ...
    const previousToken = process.env.MARKETINGCLAW_GATEWAY_TOKEN;
    delete process.env.MARKETINGCLAW_GATEWAY_TOKEN;
    try {
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
      ...
      if (previousToken === undefined) {
        ...
        process.env.MARKETINGCLAW_GATEWAY_TOKEN = previousToken;
      }
    }
  ...
  it("does not let MARKETINGCLAW_GATEWAY_TOKEN hide an unresolved SecretRef-managed token", async () => {
    mockDoctorConfigSnapshot({
      config: {
        gateway: {
          mode: "local",
          auth: {
            ...
            token: {
              source: "env",
              provider: "default",
              id: "MARKETINGCLAW_MISSING_GATEWAY_REF_TOKEN",
            },
          },
        },
        secrets: {
    ...
    const previousFallbackToken = process.env.MARKETINGCLAW_GATEWAY_TOKEN;
    const previousRefToken = process.env.MARKETINGCLAW_MISSING_GATEWAY_REF_TOKEN;
    process.env.MARKETINGCLAW_GATEWAY_TOKEN = "fallback-token-1234567890";
    delete process.env.MARKETINGCLAW_MISSING_GATEWAY_REF_TOKEN;
    try {
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
        workspaceSuggestions: false,
      ...
      if (previousFallbackToken === undefined) {
        ...
        process.env.MARKETINGCLAW_GATEWAY_TOKEN = previousFallbackToken;
      }
      if (previousRefToken === undefined) {
        delete process.env.MARKETINGCLAW_MISSING_GATEWAY_REF_TOKEN;
      } else {
        process.env.MARKETINGCLAW_MISSING_GATEWAY_REF_TOKEN = previousRefToken;
      }
    }
  ...
  it("skips token-mode exec token probes even when env password is set", async () => {
    ...
    const previousPassword = process.env.MARKETINGCLAW_GATEWAY_PASSWORD;
    process.env.MARKETINGCLAW_GATEWAY_PASSWORD = "fallback-password";
    try {
      callGateway.mockClear();
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
      ...
      if (previousPassword === undefined) {
        ...
        process.env.MARKETINGCLAW_GATEWAY_PASSWORD = previousPassword;
      }
    }
  ...
  it("skips password-mode exec password probes even when env token is set", async () => {
    ...
    const previousToken = process.env.MARKETINGCLAW_GATEWAY_TOKEN;
    process.env.MARKETINGCLAW_GATEWAY_TOKEN = "fallback-token";
    try {
      callGateway.mockClear();
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
  ...
  it("skips gateway auth warning when SecretRef-managed token resolves", async () => {
    ...
    const previousToken = process.env.MARKETINGCLAW_GATEWAY_TOKEN;
    process.env.MARKETINGCLAW_GATEWAY_TOKEN = "resolved-token-1234567890";
    try {
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
        workspaceSuggestions: false,
