...
function heartbeatTemplateAnalysisToHealthFinding(
  ...
  return {
    ...
    fixHint: 'Run "marketingclaw doctor --fix" to replace it with the clean heartbeat template.',
  };
}
  ...
  if (!params.shouldRepair) {
    note(
      [
        ...
        'Run "marketingclaw doctor --fix" to replace it with the clean heartbeat template.',
      ].join("\n"),
      "Heartbeat template",
    );
    return;
