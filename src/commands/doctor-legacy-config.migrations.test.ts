...
vi.mock("../plugins/setup-registry.js", () => ({
  ...
  runPluginSetupConfigMigrations: ({ config }: { config: MarketingClawConfig }) => ({
    config,
    changes: [],
  }),
}));
...
vi.mock("./doctor/shared/channel-legacy-config-migrate.js", () => ({
  applyChannelDoctorCompatibilityMigrations: (cfg: MarketingClawConfig) => ({
    next: cfg,
    changes: [],
  }),
}));
...
vi.mock("../secrets/target-registry.js", () => {
  ...
  return {
    discoverConfigSecretTargets: (cfg: MarketingClawConfig) => {
      const targets: Array<{
        entry: typeof entry;
        path: string;
        pathSegments: string[];
...
describe("normalizeCompatibilityConfigValues", () => {
  ...
  beforeAll(() => {
    ...
    tempOauthDir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-oauth-"));
    process.env.MARKETINGCLAW_OAUTH_DIR = tempOauthDir;
  });
  ...
  it("does not report talk provider normalization for realtime voice aliases", () => {
    ...
    const res = normalizeCompatibilityConfigValues(input as MarketingClawConfig);
