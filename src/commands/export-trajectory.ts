...
export async function exportTrajectoryCommand(
  ...
  if (!sessionKey) {
    runtime.error(
      `--session-key is required. Run ${formatCliCommand("marketingclaw sessions")} to choose a session.`,
    );
    runtime.exit(1);
    return;
  }
  ...
  if (!entry?.sessionId) {
    runtime.error(
      `Session not found: ${sessionKey}. Run ${formatCliCommand("marketingclaw sessions")} to see available sessions.`,
    );
    runtime.exit(1);
    return;
  }
  ...
  if (!(await pathExists(sessionFile))) {
    runtime.error(
      `Session file not found for ${sessionKey}. Run ${formatCliCommand("marketingclaw doctor")} to inspect session storage.`,
    );
    runtime.exit(1);
    return;
  }
