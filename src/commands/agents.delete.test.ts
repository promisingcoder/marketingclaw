...
describe("agents delete command", () => {
  ...
  it("routes deletion through the Gateway when reachable", async () => {
    await withStateDirEnv("marketingclaw-agents-delete-gateway-", async ({ stateDir }) => {
      const now = Date.now();
  ...
  it("falls back to local deletion when the optional Gateway probe needs credentials", async () => {
    await withStateDirEnv("marketingclaw-agents-delete-gateway-auth-", async ({ stateDir }) => {
      const now = Date.now();
      const cfg: MarketingClawConfig = {
        agents: {
          list: [
  ...
  it("purges deleted agent entries from the session store", async () => {
    await withStateDirEnv("marketingclaw-agents-delete-", async ({ stateDir }) => {
      const now = Date.now();
      const cfg: MarketingClawConfig = {
        agents: {
          list: [
  ...
  it("trashes workspace attestations during local deletion", async () => {
    await withStateDirEnv("marketingclaw-agents-delete-attestation-", async ({ stateDir }) => {
      const cfg: MarketingClawConfig = {
        agents: {
          list: [
            { id: "main", workspace: path.join(stateDir, "workspace-main") },
  ...
  it("purges legacy main-alias entries owned by the deleted default agent", async () => {
    await withStateDirEnv("marketingclaw-agents-delete-main-alias-", async ({ stateDir }) => {
      const now = Date.now();
      const cfg: MarketingClawConfig = {
        agents: {
  ...
  it("preserves shared-store legacy default keys when deleting another agent", async () => {
    await withStateDirEnv("marketingclaw-agents-delete-shared-store-", async ({ stateDir }) => {
      const now = Date.now();
      const cfg: MarketingClawConfig = {
  ...
  it("skips workspace removal when another agent shares the same workspace (#70890)", async () => {
    await withStateDirEnv("marketingclaw-agents-delete-shared-workspace-", async ({ stateDir }) => {
  ...
  it("skips workspace removal when another agent workspace overlaps a child path (#70890)", async () => {
    await withStateDirEnv(
      "marketingclaw-agents-delete-overlapping-workspace-",
      async ({ stateDir }) => {
        const sharedWorkspace = path.join(stateDir, "workspace-shared");
  ...
  it("skips workspace removal when deleting a parent workspace that contains another agent workspace (#70890)", async () => {
    await withStateDirEnv("marketingclaw-agents-delete-parent-workspace-", async ({ stateDir }) => {
      const sharedWorkspace = path.join(stateDir, "workspace-shared");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withStateDirEnv(
        "marketingclaw-agents-delete-symlink-workspace-",
        async ({ stateDir }) => {
  ...
  it("trashes workspace when no other agent shares it", async () => {
    await withStateDirEnv("marketingclaw-agents-delete-unique-workspace-", async ({ stateDir }) => {
