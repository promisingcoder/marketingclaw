...
vi.mock("../config/paths.js", () => ({
  resolveStateDir: () => process.env.MARKETINGCLAW_STATE_DIR ?? "/tmp/marketingclaw-state",
}));
...
vi.mock("../agents/auth-profiles/profiles.js", async () => {
  ...
  const upsert = (params: { profileId: string; credential: unknown; agentDir?: string }) => {
    const stateDir = process.env.MARKETINGCLAW_STATE_DIR ?? "/tmp/marketingclaw-state";
...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under the default agent dir", async () => {
    const env = await setupAuthTestEnv("marketingclaw-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-oauth-sync-"));
    ...
    setTestEnvValue("MARKETINGCLAW_AGENT_DIR", kidAgentDir);
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-oauth-nosync-"));
    lifecycle.setStateDir(tempStateDir);
    setTestEnvValue("MARKETINGCLAW_STATE_DIR", tempStateDir);
  ...
  it("syncs siblings from explicit agentDir outside MARKETINGCLAW_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-oauth-external-"));
    lifecycle.setStateDir(tempStateDir);
    setTestEnvValue("MARKETINGCLAW_STATE_DIR", tempStateDir);

    // Create standard-layout agents tree *outside* MARKETINGCLAW_STATE_DIR
...
describe("upsertApiKeyProfile secret refs", () => {
  ...
  it("handles plaintext, ref mode, and inline env-ref provider keys", async () => {
    const env = await setupAuthTestEnv("marketingclaw-onboard-auth-credentials-");
    lifecycle.setStateDir(env.stateDir);
  ...
  it("stores provider-specific env refs and metadata in ref mode", async () => {
    const env = await setupAuthTestEnv("marketingclaw-onboard-auth-credentials-provider-ref-");
    lifecycle.setStateDir(env.stateDir);
...
describe("upsertApiKeyProfile", () => {
  const lifecycle = createAuthTestLifecycle(["MARKETINGCLAW_STATE_DIR", "MARKETINGCLAW_AGENT_DIR"]);
  ...
  it("writes to the default agent dir", async () => {
    const env = await setupAuthTestEnv("marketingclaw-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
    const defaultAgentDir = path.join(env.stateDir, "agents", "main", "agent");
