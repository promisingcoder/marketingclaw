...
const resolveMarketingClawPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  ...
  loadMarketingClawPlugins: () => createEmptyPluginRegistry(),
...
vi.mock("../config/config.js", async () => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/marketingclaw.json",
    createConfigIO,
...
vi.mock("marketingclaw/plugin-sdk/provider-auth", () => ({
  isNonSecretApiKeyMarker: () => false,
}));

vi.mock("marketingclaw/plugin-sdk/provider-model-shared", () => ({
  DEFAULT_CONTEXT_TOKENS: 32768,
  normalizeProviderId: (value: string) => normalizeLowercaseStringOrEmpty(value),
}));

vi.mock("marketingclaw/plugin-sdk/provider-stream-shared", () => ({
...
beforeEach(() => {
  ...
  resolveMarketingClawPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.MARKETINGCLAW_STATE_DIR;
  originalUpdateInProgress = process.env.MARKETINGCLAW_UPDATE_IN_PROGRESS;
  process.env.MARKETINGCLAW_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-doctor-state-"));
