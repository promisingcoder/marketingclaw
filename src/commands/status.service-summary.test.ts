...
describe("readServiceStatusSummary", () => {
  it("marks MarketingClaw-managed services as installed", async () => {
    const summary = await readServiceStatusSummary(
      createService({
        isLoaded: vi.fn(async () => true),
        readCommand: vi.fn(async () => ({ programArguments: ["marketingclaw", "gateway", "run"] })),
    ...
    expect(summary.managedByMarketingClaw).toBe(true);
  ...
  it("marks running unmanaged services as externally managed", async () => {
    ...
    expect(summary.installed).toBe(true);
    expect(summary.managedByMarketingClaw).toBe(false);
  ...
  it("passes command environment to runtime and loaded checks", async () => {
    const isLoaded = vi.fn(async ({ env }: GatewayServiceEnvArgs) => {
      return env?.MARKETINGCLAW_GATEWAY_PORT === "18789";
    });
    const readRuntime = vi.fn(async (env?: NodeJS.ProcessEnv) => ({
      status:
        env?.MARKETINGCLAW_GATEWAY_PORT === "18789" ? ("running" as const) : ("unknown" as const),
    }));
    ...
    const summary = await readServiceStatusSummary(
      createService({
        ...
        readCommand: vi.fn(async () => ({
          programArguments: ["marketingclaw", "gateway", "run", "--port", "18789"],
    ...
    expect(loadedArgs?.env?.MARKETINGCLAW_GATEWAY_PORT).toBe("18789");
    const runtimeEnv = requireMockArg(readRuntime, "readRuntime") as NodeJS.ProcessEnv;
    expect(runtimeEnv?.MARKETINGCLAW_GATEWAY_PORT).toBe("18789");
    expect(summary.installed).toBe(true);
  ...
  it("includes service layout diagnostics and flags source checkout entrypoints", async () => {
    await withTempDir({ prefix: "marketingclaw-status-service-layout-" }, async (root) => {
      ...
      const serviceFile = path.join(root, "marketingclaw-gateway.service");
      ...
      expect(layout.sourcePathReal).toBe(path.join(realRoot, "marketingclaw-gateway.service"));
