...
async function runGatewayHealthCheck(params: {
  ...
  const token = process.env.MARKETINGCLAW_GATEWAY_TOKEN ?? configuredToken;
  const password = process.env.MARKETINGCLAW_GATEWAY_PASSWORD ?? configuredPassword;
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from marketingclaw.json",
        },
      ],
...
async function promptWebToolsConfig(
  ...
  type WebSearchConfig = NonNullable<NonNullable<MarketingClawConfig["tools"]>["web"]>["search"];
...
export async function runConfigureWizard(
  ...
  try {
    intro(opts.command === "update" ? "MarketingClaw update wizard" : "MarketingClaw configure");
    ...
    const baseConfig: MarketingClawConfig = snapshot.valid
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("marketingclaw doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const promptGatewayRunMode = async (): Promise<OnboardMode> => {
      ...
      const localProbePromise = (async () => {
        ...
        return probeGatewayReachable({
          url: localUrl,
          token: process.env.MARKETINGCLAW_GATEWAY_TOKEN ?? baseLocalProbeToken,
          password: process.env.MARKETINGCLAW_GATEWAY_PASSWORD ?? baseLocalProbePassword,
    ...
    if (shouldSkipGatewaySummary) {
      ...
      if (remoteUrl) {
        note(
          ["Remote Gateway:", remoteUrl, "Docs: https://docs.marketingclaw.ai/gateway/remote"].join(
            "\n",
          ),
          "Gateway",
        );
    ...
    note(
      [
        ...
        "Docs: https://docs.marketingclaw.ai/web/control-ui",
      ].join("\n"),
      "Control UI",
    );
