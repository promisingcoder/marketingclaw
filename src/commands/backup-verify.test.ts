...
const TEST_ARCHIVE_ROOT = "2026-03-09T00-00-00.000Z-marketingclaw-backup";
...
async function createArchiveWithManifestContent(
  ...
  const payloadArchivePath =
    options.payloadArchivePath ??
    `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.marketingclaw/payload.txt`;
  try {
...
describe("backupVerifyCommand", () => {
  ...
  it("verifies a valid backup archive", async () => {
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-backup-verify-out-"));
    try {
      ...
      const payloadArchivePath = `${archiveRoot}/payload/posix/tmp/.marketingclaw/state.txt`;
      await fs.writeFile(
        manifestPath,
  ...
  it("fails when the archive does not contain a manifest", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-backup-no-manifest-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
  ...
  it("fails when the manifest references a missing asset payload", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-backup-missing-asset-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
      const rootName = "2026-03-09T00-00-00.000Z-marketingclaw-backup";
        ...
        assets: [
          {
            kind: "state",
            sourcePath: "/tmp/.marketingclaw",
            archivePath: `${rootName}/payload/posix/tmp/.marketingclaw`,
          },
        ],
      };
      await fs.writeFile(
  ...
  it("reports malformed manifest JSON without leaking parser internals", async () => {
    await createArchiveWithManifestContent(
      {
        tempPrefix: "marketingclaw-backup-bad-manifest-json-",
  ...
  it("rejects oversized manifest entries without retaining the full body", async () => {
    await createArchiveWithManifestContent(
      {
        tempPrefix: "marketingclaw-backup-huge-manifest-",
  ...
  it("rejects unsafe archive paths", async () => {
    for (const { tempPrefix, archivePath, error } of [
      {
        tempPrefix: "marketingclaw-backup-traversal-",
        archivePath: `${TEST_ARCHIVE_ROOT}/payload/../escaped.txt`,
        error: /path traversal segments/i,
      },
      {
        tempPrefix: "marketingclaw-backup-backslash-",
        archivePath: `${TEST_ARCHIVE_ROOT}/payload\\..\\escaped.txt`,
  ...
  it("rejects unsafe hardlink targets", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-backup-linkpath-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    const payloadArchivePath = `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.marketingclaw/target.txt`;
    const hardlinkArchivePath = `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.marketingclaw/hardlink.txt`;
    try {
  ...
  it("accepts root-relative internal hardlink targets from older backups", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "marketingclaw-backup-rootless-linkpath-"),
    );
    const archivePath = path.join(tempDir, "backup.tar.gz");
    const rootRelativeTargetPath = "payload/posix/tmp/.marketingclaw/target.txt";
  ...
  it("rejects hardlink targets missing from archive entries", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "marketingclaw-backup-missing-linkpath-"),
    );
    const archivePath = path.join(tempDir, "broken.tar.gz");
    const payloadArchivePath = `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.marketingclaw/target.txt`;
    const hardlinkArchivePath = `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.marketingclaw/hardlink.txt`;
    const missingTargetPath = `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.marketingclaw/missing-target.txt`;
    try {
      const archive = gzipSync(
        Buffer.concat([
          encodeTarEntry({
  ...
  it("ignores payload manifest.json files when locating the backup manifest", async () => {
    ...
    try {
      ...
      const stateArchivePath = `${archiveRoot}/payload/posix/tmp/.marketingclaw/state.txt`;
  ...
  it("rejects duplicate manifest and payload entries", async () => {
    const payloadArchivePath = `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.marketingclaw/payload.txt`;
    for (const options of [
      {
        tempPrefix: "marketingclaw-backup-duplicate-manifest-",
        payloads: [{ fileName: "payload.txt", contents: "payload\n" }],
        buildTarEntries: ({
          manifestPath,
          payloadPaths,
      ...
      {
        tempPrefix: "marketingclaw-backup-duplicate-payload-",
        payloads: [
