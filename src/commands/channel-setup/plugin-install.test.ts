...
import { bundledPluginRoot, bundledPluginRootAt } from "marketingclaw/plugin-sdk/test-fixtures";
...
const discoverMarketingClawPlugins = vi.fn((_args?: unknown) => ({
  candidates: [],
  diagnostics: [],
}));
vi.mock("../../plugins/discovery.js", () => ({
  discoverMarketingClawPlugins: (args: unknown) => discoverMarketingClawPlugins(args),
}));
  ...
  resolveBundledPluginSources.mockReturnValue(
    new Map([
      [
        ...
        {
          pluginId: "bundled-chat",
          localPath: bundledPluginRootAt("/opt/marketingclaw", "bundled-chat"),
          npmSpec: bundledChatNpmSpec,
        },
      ],
    ]),
...
function createManifestRecord(
  ...
  return {
    ...
    hooks: [],
    origin: "bundled",
    rootDir: `/tmp/marketingclaw-test/${id}`,
    source: `/tmp/marketingclaw-test/${id}/index.ts`,
    manifestPath: `/tmp/marketingclaw-test/${id}/marketingclaw.plugin.json`,
    ...rest,
  };
}
...
function expectSetupSnapshotDoesNotScopeToPlugin(params: {
  ...
  expect(loadMarketingClawPlugins).toHaveBeenCalledTimes(1);
  expect(requireMockCallArg(vi.mocked(loadMarketingClawPlugins), 0).onlyPluginIds).toStrictEqual(
    [],
  );
}
...
beforeEach(() => {
  ...
  discoverMarketingClawPlugins.mockReturnValue({ candidates: [], diagnostics: [] });
...
async function runInitialValueForChannel(channel: "dev" | "beta") {
  ...
  const select = vi.fn((async <T extends string>() => "skip" as T) as WizardPrompter["select"]);
  const prompter = makePrompter({ select: select as unknown as WizardPrompter["select"] });
  const cfg: MarketingClawConfig = { update: { channel } };
...
function expectLoadMarketingClawPluginFields(expected: Record<string, unknown>, callIndex = 0) {
  expectRecordFields(
    requireMockCallArg(vi.mocked(loadMarketingClawPlugins), callIndex),
    "loadMarketingClawPlugins args",
    expected,
  );
}
...
describe("ensureChannelSetupPluginInstalled", () => {
  it("installs from npm and enables the plugin", async () => {
    ...
    const cfg: MarketingClawConfig = { plugins: { allow: ["bundled-chat"] } };
  ...
  it("installs npm channel plugins into the active profile extensions dir", async () => {
    ...
    const profileStateDir = "/tmp/marketingclaw-ledger-channel";
    process.env.MARKETINGCLAW_STATE_DIR = profileStateDir;
    vi.mocked(fs.existsSync).mockReturnValue(false);
    installPluginFromNpmSpec.mockResolvedValue({
      ok: true,
      pluginId: "bundled-chat",
  ...
  it("installs npm beta on the beta channel without persisting the beta tag", async () => {
    ...
    const cfg: MarketingClawConfig = { update: { channel: "beta" } };
    vi.mocked(fs.existsSync).mockReturnValue(false);
    installPluginFromNpmSpec.mockResolvedValue({
      ok: true,
    ...
    expect(result.cfg.plugins?.installs?.["wecom-marketingclaw-plugin"]?.spec).toBe(
      "@openclaw/wecom",
    );
  });
  ...
  it("defaults to bundled local path on beta channel when available", async () => {
    ...
    expectRecordFields(
      ...
      {
        value: "local",
        hint: bundledPluginRootAt("/opt/marketingclaw", "bundled-chat"),
      },
    );
  });
  ...
  it("offers ClawHub as the first-class install source for channel catalog entries", async () => {
    ...
    expectRecordFields(options[0], "clawhub option", {
      value: "clawhub",
      label: "Download from ClawHub (clawhub:marketingclaw/clawhub-chat@2026.5.2)",
    });
    expectRecordFields(options[1], "skip option", {
      value: "skip",
    });
  ...
  it("skips the install prompt when autoConfirmSingleSource is set and only npm is available", async () => {
    ...
    expect(select).not.toHaveBeenCalled();
    expect(result.installed).toBe(true);
    expect(result.pluginId).toBe("wecom-marketingclaw-plugin");
  });
  ...
  it("loads setup snapshots from the auto-enabled config snapshot", () => {
    ...
    expectLoadMarketingClawPluginFields({
      config: autoEnabledConfig,
  ...
  it("scopes snapshots by activation-declared channel ownership when direct channel lists are empty", () => {
    ...
    loadPluginManifestRegistry.mockImplementation((args: unknown) => {
      if (
        isRecord(args) &&
        args.config === cfg &&
        args.workspaceDir === "/tmp/marketingclaw-workspace" &&
