...
export function applyNonInteractiveGatewayConfig(params: {
  ...
  const envGatewayToken = normalizeGatewayTokenInput(process.env.MARKETINGCLAW_GATEWAY_TOKEN);
  ...
  // plaintext > ambient MARKETINGCLAW_GATEWAY_TOKEN > randomToken(). Ambient env
  ...
  if (authMode === "token") {
    if (gatewayTokenRefEnv) {
      ...
      if (!isValidEnvSecretRefId(gatewayTokenRefEnv)) {
        runtime.error(
          "Invalid --gateway-token-ref-env. Use an environment variable name like MARKETINGCLAW_GATEWAY_TOKEN.",
        );
        runtime.exit(1);
        return null;
      }
      ...
      if (!resolvedFromEnv) {
        runtime.error(
          `Environment variable "${gatewayTokenRefEnv}" is missing or empty. Export it first, then rerun ${formatCliCommand("marketingclaw onboard --non-interactive")}.`,
        );
        runtime.exit(1);
        return null;
      }
      ...
      // branch, an ambient MARKETINGCLAW_GATEWAY_TOKEN (or randomToken() fallback)
