...
function makeTempDir() {
  return makeTrackedTempDir("marketingclaw-doctor-plugin-registry", tempDirs);
}
...
function createBundledCandidate(params: {
  ...
  fs.writeFileSync(
    path.join(params.rootDir, "marketingclaw.plugin.json"),
    JSON.stringify({
      id: params.id,
      name: params.id,
      configSchema: { type: "object" },
...
function expectedPluginIndexRecord(params: {
  ...
  return {
    ...
    manifestPath: path.join(params.rootDir, "marketingclaw.plugin.json"),
...
describe("maybeRepairPluginRegistryState", () => {
  ...
  it("warns about stale managed npm packages that shadow bundled plugins", async () => {
    ...
    expect(vi.mocked(note).mock.calls.join("\n")).toContain("@marketingclaw/google-meet@2026.5.2");
  ...
  it("removes stale managed npm packages from the package lock during repair", async () => {
    ...
    expect(packageLock.packages).not.toHaveProperty("node_modules/@marketingclaw/google-meet");
    expect(packageLock.dependencies).not.toHaveProperty("@marketingclaw/google-meet");
    expect(packageLock.dependencies).toHaveProperty("other-plugin");
  });

  it("repairs managed npm marketingclaw peer links during registry repair", async () => {
    const stateDir = makeTempDir();
    const managed = createManagedNpmPlugin({
      stateDir,
      id: "codex",
    ...
    const linkPath = path.join(managed.packageDir, "node_modules", "marketingclaw");
    expect(fs.lstatSync(linkPath).isSymbolicLink()).toBe(true);
    expect(fs.realpathSync(linkPath)).toBe(fs.realpathSync(process.cwd()));
    expect(vi.mocked(note).mock.calls.join("\n")).toContain(
      "Repaired MarketingClaw host peer link",
    );
  });

  it("warns about broken managed npm marketingclaw peer links without repairing them", async () => {
    const stateDir = makeTempDir();
    const managed = createManagedNpmPlugin({
      stateDir,
      id: "codex",
    ...
    const linkPath = path.join(managed.packageDir, "node_modules", "marketingclaw");
    const notes = vi.mocked(note).mock.calls.join("\n");
    expect(notes).toContain("Managed npm MarketingClaw host peer links need repair");
    expect(notes).toContain("codex-plugin");
    expect(notes).toContain("marketingclaw doctor --fix");
