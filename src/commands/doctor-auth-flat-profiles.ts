...
function listAuthProfileRepairCandidates(
  ...
  const envAgentDir =
    readNonEmptyString(env.MARKETINGCLAW_AGENT_DIR) ?? readNonEmptyString(env.PI_CODING_AGENT_DIR);
  if (envAgentDir) {
...
function coerceLegacyConfigAuthProfileStore(cfg: MarketingClawConfig): AuthProfileStore | null {
...
export async function maybeMigrateAuthProfileJsonStoresToSqlite(params: {
  ...
  note(
    [
      ...
      `- ${formatCliCommand("marketingclaw doctor --fix")} imports legacy auth profile JSON into the per-agent SQLite database and removes the old files after backup.`,
    ].join("\n"),
    ...
    try {
      ...
      if (unresolvedSidecarProfileIds.size > 0) {
        ...
        result.warnings.push(
          `Left ${unresolvedSidecarProfileIds.size} legacy OAuth sidecar profile${unresolvedSidecarProfileIds.size === 1 ? "" : "s"} in ${shortenHomePath(candidate.authPath)}; rerun ${formatCliCommand("marketingclaw doctor --fix")} after sidecar migration or re-authenticate those profiles.`,
        );
      }
...
function ensureConfigAuthProfiles(config: MarketingClawConfig): Record<string, AuthProfileConfig> {
...
export async function maybeRepairLegacyFlatAuthProfileStores(params: {
  ...
  const noteLines = [
    ...
    ...awsSdkMarkerStores.map(
      (entry) =>
        `- ${shortenHomePath(entry.authPath)} contains aws-sdk profile markers that belong in marketingclaw.json auth.profiles.`,
    ),
  ];
  if (legacyStores.length > 0) {
    noteLines.push(
      `- The gateway expects the canonical version/profiles store; ${formatCliCommand("marketingclaw doctor --fix")} rewrites this legacy shape with a backup.`,
    );
  }
  if (awsSdkMarkerStores.length > 0) {
    noteLines.push(
      `- AWS SDK profile markers are routing metadata, not stored credentials; ${formatCliCommand("marketingclaw doctor --fix")} moves them to config with a backup.`,
    );
  }
  note(noteLines.join("\n"), "Auth profiles");
...
export async function maybeRepairCanonicalApiKeyFieldAlias(params: {
  ...
  noteLines.push(
    `- Runtime auth parsing only reads canonical "key" and "keyRef" fields, so these profiles are silently skipped; ${formatCliCommand("marketingclaw doctor --fix")} rewrites "api_key" to "key" with a backup.`,
  );
  note(noteLines.join("\n"), "Auth profiles");
