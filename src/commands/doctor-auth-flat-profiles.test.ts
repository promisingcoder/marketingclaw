...
async function makeTestState(): Promise<MarketingClawTestState> {
  const state = await createMarketingClawTestState({
    layout: "state-only",
    prefix: "marketingclaw-doctor-flat-auth-",
    env: {
...
describe("maybeMigrateAuthProfileJsonStoresToSqlite", () => {
  ...
  it("infers config credential provider and mode before stripping config", async () => {
    const cases: Array<{ profileId: string; cfg: MarketingClawConfig; now: number }> = [
      {
...
describe("maybeRepairOpenAICodexAuthConfig", () => {
  it("renames legacy OpenAI Codex config profiles and merges auth order", () => {
    ...
    const migrated = result.config as MarketingClawConfig & {
      agents?: {
        defaults?: {
