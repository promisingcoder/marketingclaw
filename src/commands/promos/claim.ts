...
async function fetchLivePromotion(slug: string): Promise<ClawHubPromotion> {
  try {
    ...
    if (error instanceof ClawHubRequestError && error.status === 404) {
      throw new Error(
        `Promotion "${slug}" was not found or is not live. See ${formatCliCommand("marketingclaw promos list")}.`,
        { cause: error },
      );
    }
    throw error;
...
function requireUnchangedClaimContract(
  ...
  throw new Error(
    `Promotion "${initial.slug}" changed while the claim was in progress; no promotional models were added. Any provider credentials you just configured were kept. Run ${formatCliCommand("marketingclaw promos list")} and retry.`,
  );
}
...
function resolveManifestPluginPackageNames(pluginId: string, cfg: MarketingClawConfig): string[] {
...
function resolveAuthChoice(
  ...
  if (!entry) {
    throw new Error(
      `Promotion "${promotion.slug}" requires auth choice "${authChoiceId}", which this MarketingClaw version does not know. Update MarketingClaw and retry.`,
    );
  }
...
function requirePromotionPlugins(
  ...
  throw new Error(
    `Promotion "${promotion.slug}" requires plugin package "${unsupported[0]}", but ${authChoiceLabel} does not provide it in this MarketingClaw version. Update MarketingClaw and retry.`,
  );
}
...
async function ensureProviderAuth(params: {
  ...
  if (!catalogEntry) {
    throw new Error(
      `No credentials configured for provider "${provider}". Add one with ${formatCliCommand("marketingclaw models auth add")} and retry.`,
    );
  }
  ...
  const applied = await applyAuthChoiceLoadedPluginProvider({
    ...
    config: structuredClone(snapshot.sourceConfig ?? snapshot.config) as MarketingClawConfig,
...
export async function promosClaimCommand(
  ...
  if (makeDefault && suggested) {
    ...
    runtime.log(
      `  Revert anytime with ${formatCliCommand("marketingclaw models set <previous-model>")}.`,
    );
  } else if (suggested) {
    runtime.log(
      `  Try it: ${formatCliCommand(`marketingclaw models set ${suggested.modelRef}`)} (promotion ends ${new Date(promotion.endsAt).toLocaleDateString()}).`,
    );
  }
}
