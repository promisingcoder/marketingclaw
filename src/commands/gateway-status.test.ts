...
describe("gateway-status command", () => {
  ...
  it("includes diagnostic next steps when no gateway is reachable or discoverable", async () => {
    ...
    expect(warning?.message).toContain("marketingclaw gateway status --deep --require-rpc");
  ...
  it("suppresses unresolved SecretRef auth warnings when probe is reachable", async () => {
    const { runtime, runtimeLogs, runtimeErrors } = createRuntimeCapture();
    await withEnvAsync(
      { MISSING_GATEWAY_TOKEN: undefined, MARKETINGCLAW_GATEWAY_TOKEN: undefined },
      async () => {
  ...
  it("does not resolve local token SecretRef when MARKETINGCLAW_GATEWAY_TOKEN is set", async () => {
    const { runtime, runtimeLogs, runtimeErrors } = createRuntimeCapture();
    await withEnvAsync(
      {
    ...
    await withEnvAsync(
      {
        ...
        MARKETINGCLAW_GATEWAY_URL: "wss://env-gateway.example/ws",
      },
      async () => {
        await runGatewayStatus(runtime, { timeout: "15000", json: true, port: "19080" });
      },
