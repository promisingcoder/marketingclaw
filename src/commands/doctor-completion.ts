...
export type ShellCompletionStatus = {
  ...
  cacheExists: boolean;
  cachePath: string;
  /** True if profile uses slow dynamic pattern like `source <(marketingclaw completion ...)` */
    ...
    return [
      {
        ...
        path: pathLocal,
        fixHint: "Run `marketingclaw doctor --fix` to upgrade to cached completion.",
      },
    ];
  }
    ...
    return [
      {
        ...
        path: pathLocal,
        fixHint: `Run \`marketingclaw completion --write-state\` or \`marketingclaw doctor --fix\` to regenerate ${status.cachePath}.`,
      },
    ];
  }
  return [];
