  ...
  return {
    ...
    ensureAgentWorkspace: vi.fn(
      async (params?: { dir?: string; skipOptionalBootstrapFiles?: string[] }) => ({
        dir: params?.dir ?? path.join(home, ".marketingclaw", "workspace"),
      }),
    ),
    ...
    mkdir: vi.fn(async () => {}),
    resolveSessionTranscriptsDir: vi.fn(() => path.join(home, ".marketingclaw", "sessions")),
...
describe("setupCommand", () => {
  it("writes gateway.mode=local on first run", async () => {
    await withTempHome(async (home) => {
      ...
      const workspace = path.join(home, ".marketingclaw", "workspace");
  ...
  it("explains that plain setup only initializes local files", async () => {
    await withTempHome(async (home) => {
      ...
      expect(runtime.log.mock.calls.map((call) => String(call[0])).slice(-5)).toStrictEqual([
        ...
        "Next guided path: marketingclaw onboard.",
        "Next targeted changes: marketingclaw configure for models, channels, Gateway, plugins, skills, and health checks.",
        "Add a chat channel later: marketingclaw channels add.",
      ]);
    });
  });
