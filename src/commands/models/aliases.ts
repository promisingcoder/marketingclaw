  ...
  const updated = await updateConfig((cfg) => {
    ...
    if (!found) {
      ...
      if (
        ...
        throw new Error(
          `Cannot remove "${alias}": it is a built-in alias for "${builtinTarget}" provided automatically by MarketingClaw and is not stored in your config file. To shadow it with a different target, run ${formatCliCommand(`marketingclaw models aliases add ${alias} <model>`)}.`,
        );
      }
      throw new Error(
        `Alias not found: ${alias}. Run ${formatCliCommand("marketingclaw models aliases list")} to see configured aliases.`,
      );
    }
    return {
      ...cfg,
