...
describe("resolveConfiguredEntries", () => {
  ...
  it("canonicalizes manifest-owned provider aliases in configured rows", () => {
    vi.stubEnv("MARKETINGCLAW_BUNDLED_PLUGINS_DIR", path.resolve("extensions"));
  ...
  it("recovers bundled source aliases when stale dist metadata omits them", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-model-alias-source-"));
    try {
      ...
      fs.writeFileSync(
        path.join(sourcePluginRoot, "marketingclaw.plugin.json"),
        JSON.stringify({
          id: "zai",
            ...
            plugins: [
              {
                ...
                manifestPath: path.join(distPluginRoot, "marketingclaw.plugin.json"),
              },
            ],
          },
        },
