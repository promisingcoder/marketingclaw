...
vi.mock("../config/paths.js", () => ({
  resolveStateDir: () => "/tmp/marketingclaw-migrate-command-test",
}));
  ...
  const items: MigrationPlan["items"] = [
    {
      ...
      target: "/tmp/marketingclaw/workspace/skills/alpha",
      details: {
        skillName: "alpha",
    ...
    },
    {
      ...
      target: "/tmp/marketingclaw/workspace/skills/beta",
      details: {
        skillName: "beta",
...
describe("migrateApplyCommand", () => {
  ...
  beforeEach(async () => {
    await fs.rm("/tmp/marketingclaw-migrate-command-test", { force: true, recursive: true });
    ...
    mocks.backupCreateCommand.mockResolvedValue({ archivePath: "/tmp/marketingclaw-backup.tgz" });
  });
  ...
  it("creates a verified backup before applying a conflict-free migration", async () => {
    ...
    expect(applyContext.backupPath).toBe("/tmp/marketingclaw-backup.tgz");
    expect(String(applyContext.reportDir)).toContain("/migration/hermes/");
    expect(firstAppliedPlan()).toBe(planned);
    expect(result.backupPath).toBe("/tmp/marketingclaw-backup.tgz");
  });
  ...
  it("prints only the final result for root apply in JSON mode", async () => {
    ...
    expect(logPayload.providerId).toBe("hermes");
    expect(logPayload.backupPath).toBe("/tmp/marketingclaw-backup.tgz");
    expect(logPayload.items?.[0]?.details?.value?.time?.env?.OPENAI_API_KEY).toBe("[redacted]");
