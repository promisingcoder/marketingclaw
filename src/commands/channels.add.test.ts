...
const channelWizardMocks = vi.hoisted(() => {
  ...
  return {
    prompter,
    setupChannels: vi.fn(async (...args: unknown[]) => args[0] as MarketingClawConfig),
  };
});
    ...
    await channelsAddCommand(
      {
        channel: "whatsapp",
        account: "work",
        authDir: "/tmp/marketingclaw-wa-auth",
      },
      runtime,
      { hasFlags: true },
    );
