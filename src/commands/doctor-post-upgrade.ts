...
export async function runPostUpgradeProbes(params: {
  ...
  if (!installs) {
    findings.push({
      ...
      message:
        "Installed plugin index is missing, unreadable, or malformed. Run `marketingclaw plugins registry --refresh` to rebuild it before post-upgrade validation.",
    });
  ...
  for (const record of installs.plugins) {
    ...
    if (pkgRelPath) {
      ...
      const entries = pkg.marketingclaw?.extensions ?? [];
      if (entries.length > 0) {
    ...
    if (record.manifestPath && record.manifestHash) {
      ...
      if (currentHash && currentHash !== record.manifestHash) {
        findings.push({
          ...
          message: `Plugin ${record.pluginId} manifest hash drifted from installs.json snapshot. Run \`marketingclaw plugins registry --refresh\` to re-sync.`,
          plugin: record.pluginId,
        });
      }
    }
