...
const mocks = vi.hoisted(() => ({
  ...
  maybeRepairManagedNpmMarketingClawPeerLinks: vi.fn(),
...
vi.mock("../doctor-plugin-registry.js", () => ({
  maybeRepairManagedNpmMarketingClawPeerLinks: mocks.maybeRepairManagedNpmMarketingClawPeerLinks,
...
vi.mock("./shared/channel-doctor.js", () => ({
  collectChannelDoctorRepairMutations: ({ cfg }: { cfg: MarketingClawConfig }) => {
...
vi.mock("./shared/empty-allowlist-scan.js", () => ({
  scanEmptyAllowlistPolicyWarnings: (cfg: MarketingClawConfig) =>
    cfg.channels?.signal
...
vi.mock("./shared/allowlist-policy-repair.js", () => ({
  maybeRepairAllowlistPolicyAllowFrom: async (cfg: MarketingClawConfig) => ({
    config: cfg,
    changes: [],
  }),
}));
...
vi.mock("./shared/bundled-plugin-load-paths.js", () => ({
  maybeRepairBundledPluginLoadPaths: (cfg: MarketingClawConfig) => ({
    config: cfg,
    changes: [],
  }),
}));
...
vi.mock("./shared/invalid-plugin-config.js", () => ({
  maybeRepairInvalidPluginConfig: (cfg: MarketingClawConfig) => ({
    config: cfg,
    changes: [],
  }),
}));
...
vi.mock("./shared/legacy-tools-by-sender.js", () => ({
  maybeRepairLegacyToolsBySenderKeys: (cfg: MarketingClawConfig) => {
...
vi.mock("./shared/exec-safe-bins.js", () => ({
  maybeRepairExecSafeBinProfiles: (cfg: MarketingClawConfig) => ({
    config: cfg,
    changes: [],
  }),
}));
...
describe("doctor repair sequencing", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.applyPluginAutoEnable.mockImplementation((params: { config: MarketingClawConfig }) => ({
      config: params.config,
      changes: [],
    }));
    mocks.materializePluginAutoEnableCandidates.mockImplementation(
      (params: { config: MarketingClawConfig }) => ({
        config: params.config,
        changes: [],
      }),
    );
    ...
    mocks.maybeRepairGroupAllowFromFallback.mockImplementation((cfg: MarketingClawConfig) => ({
      config: cfg,
      changes: [],
    }));
    mocks.maybeRepairManagedNpmMarketingClawPeerLinks.mockResolvedValue(false);
    ...
    mocks.maybeRepairOpenAICodexAuthConfig.mockImplementation((cfg: MarketingClawConfig) => ({
      changes: [],
      config: cfg,
      warnings: [],
    }));
    ...
    mocks.maybeRepairOpenPolicyAllowFrom.mockImplementation((cfg: MarketingClawConfig) => ({
      config: cfg,
      changes: [],
    }));
    ...
    mocks.maybeRepairStalePluginConfig.mockImplementation((cfg: MarketingClawConfig) => ({
      config: cfg,
      changes: [],
    }));
  });
  ...
  it("repairs managed npm plugin drift before missing plugin install repair", async () => {
    ...
    mocks.maybeRepairManagedNpmMarketingClawPeerLinks.mockImplementation(async () => {
      events.push("marketingclaw-peer-links");
      return true;
    });
    ...
    expect(mocks.maybeRepairManagedNpmMarketingClawPeerLinks).toHaveBeenCalledOnce();
    const peerLinkCall = mocks.maybeRepairManagedNpmMarketingClawPeerLinks.mock.calls[0]?.[0];
  ...
  it("repairs stale OAuth shadows before importing and removing auth JSON", async () => {
    ...
    const result = await runDoctorRepairSequence({
      state: {
        ...
        candidate: {} as MarketingClawConfig,
        pendingChanges: false,
        fixHints: [],
      },
      doctorFixCommand: "marketingclaw doctor --fix",
  ...
  it("auto-enables newly installed configured plugins after doctor repair", async () => {
    ...
    mocks.applyPluginAutoEnable.mockImplementationOnce(
      (params: { config: MarketingClawConfig }) => ({
        config: {
          ...params.config,
          plugins: {
  ...
  it("surfaces ClawHub notices from successful missing configured plugin repair", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      ...
      warnings: [],
      notices: [
        'ClawHub trust warning for "@marketingclaw/brave-plugin@1.2.3": scan=pending; reasons=pending.',
      ],
    });
    mocks.maybeRepairStalePluginConfig.mockImplementationOnce((cfg: MarketingClawConfig) => ({
      config: {
        ...cfg,
        plugins: {
          ...cfg.plugins,
  ...
  it("moves legacy Codex routes to canonical OpenAI before missing plugin install repair", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockImplementationOnce(
      async (params: { cfg: MarketingClawConfig }) => {
  ...
  it("runs group allowFrom fallback migration after open-policy allowFrom repair", async () => {
    const events: string[] = [];
    mocks.maybeRepairOpenPolicyAllowFrom.mockImplementationOnce((cfg: MarketingClawConfig) => {
    ...
    mocks.maybeRepairGroupAllowFromFallback.mockImplementationOnce((cfg: MarketingClawConfig) => {
  ...
  it("does not remove deferred configured plugins during the package update doctor pass", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      changes: [
        'Skipped package-manager repair for configured plugin "brave" during package update; rerun "marketingclaw doctor --fix" after the update completes.',
      ],
      warnings: [],
    });
    const result = await runDoctorRepairSequence({
  ...
  it("preserves configured plugins when their install repair fails", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      changes: [],
      warnings: [
        'Failed to install missing configured plugin "brave" from @marketingclaw/brave-plugin: package install failed',
      ],
  ...
  it("preserves configured channels when their install repair fails", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      changes: [],
      warnings: [
        'Failed to install missing configured channel plugin "whatsapp" from @marketingclaw/whatsapp: package install failed',
      ],
