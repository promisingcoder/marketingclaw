...
const LEGACY_VLLM_QWEN_AGENT_THINKING_FORMAT_RULE: LegacyConfigRule = {
  ...
  message:
    'agents.defaults.models.<vllm-model>.params.qwenThinkingFormat is legacy; run "marketingclaw doctor --fix" to move it to models.providers.vllm.models[].compat.thinkingFormat.',
...
const LEGACY_VLLM_QWEN_PROVIDER_THINKING_FORMAT_RULE: LegacyConfigRule = {
  ...
  message:
    'models.providers.vllm.params.qwenThinkingFormat is legacy; run "marketingclaw doctor --fix" to move it to models.providers.vllm.models[].compat.thinkingFormat.',
...
const LEGACY_VLLM_QWEN_PROVIDER_MODEL_THINKING_FORMAT_RULE: LegacyConfigRule = {
  ...
  message:
    'models.providers.vllm.models[*].params.qwenThinkingFormat is legacy; run "marketingclaw doctor --fix" to move it to models.providers.vllm.models[].compat.thinkingFormat.',
...
const LEGACY_VLLM_QWEN_DEFAULT_PARAMS_THINKING_FORMAT_RULE: LegacyConfigRule = {
  ...
  message:
    'agents.defaults.params.qwenThinkingFormat is legacy; run "marketingclaw doctor --fix" to move it to models.providers.vllm.models[].compat.thinkingFormat.',
...
const LEGACY_VLLM_QWEN_AGENT_PARAMS_THINKING_FORMAT_RULE: LegacyConfigRule = {
  path: ["agents"],
  message:
    'agents.list[].params.qwenThinkingFormat is legacy; run "marketingclaw doctor --fix" to move it to models.providers.vllm.models[].compat.thinkingFormat.',
...
const INVALID_THINKING_FORMAT_RULE: LegacyConfigRule = {
  path: ["models", "providers"],
  message:
    'models.providers.<id>.models[*].compat.thinkingFormat has an unrecognized value; run "marketingclaw doctor --fix" to remove it and restore the runtime default.',
...
const STALE_CONTEXT_WINDOW_RULE: LegacyConfigRule = {
  path: ["models", "providers"],
  message:
    'models.providers.<id>.models[*].contextWindow has a stale catalog value; run "marketingclaw doctor --fix" to repair it.',
...
const RETIRED_MODEL_REF_MESSAGE =
  'Configured retired model refs are no longer in the bundled catalogs; run "marketingclaw doctor --fix" to upgrade them.';
    ...
    legacyRules: [
      {
        path: ["models", "providers"],
        message:
          'models.providers.openai-codex is legacy; run "marketingclaw doctor --fix" to move it to models.providers.openai.',
        match: (value) => hasAutoFixableLegacyOpenAICodexProvider(value),
      },
      {
        path: ["models", "providers"],
        message:
          'openai-codex-responses is legacy; run "marketingclaw doctor --fix" to use openai-chatgpt-responses.',
        match: (value) => {
