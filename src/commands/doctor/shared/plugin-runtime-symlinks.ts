...
export async function noteStalePluginRuntimeSymlinks(
  ...
  const lines = [
    ...
    `  ${PLUGIN_RUNTIME_DEPS_MARKER} directories from a previous MarketingClaw install.`,
  ...
  lines.push("- Repair: run `marketingclaw doctor --fix` to remove the dangling symlinks.");
