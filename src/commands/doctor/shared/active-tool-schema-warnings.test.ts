...
import type { createMarketingClawCodingTools } from "../../../agents/agent-tools.js";
...
const toolState = vi.hoisted(() => ({
  ...
  resolveModel: vi.fn(),
  createTools: vi.fn<typeof createMarketingClawCodingTools>(),
...
vi.mock("../../../agents/agent-tools.js", () => ({
  createMarketingClawCodingTools: (
    options?: Parameters<typeof createMarketingClawCodingTools>[0],
  ) => {
...
describe("active tool schema doctor warnings", () => {
  ...
  it("warns with plugin ownership for active tools blocked by runtime projection", () => {
    ...
    expect(
      collectActiveToolSchemaProjectionWarnings({
        ...
        env: { HOME: "/tmp/marketingclaw-test" },
      }),
    ).toEqual([
      '- agents.main: active tool "fuzzplugin_move_angles" from plugin "fuzzplugin" has unsupported runtime input schema (fuzzplugin_move_angles.parameters.type must be "object"). MarketingClaw will quarantine this tool at runtime; fix or disable the plugin, or remove the tool from active allowlists.',
    ]);
  ...
  it("warns about unreadable active tool entries without crashing", () => {
    ...
    expect(
      ...
      '- agents.main: active tool "tool[0]" has unsupported runtime input schema (tool[0] is unreadable). MarketingClaw will quarantine this tool at runtime; fix or disable the plugin, or remove the tool from active allowlists.',
    ]);
  });
  ...
  it("validates provider-normalized runtime schemas before reporting doctor health", () => {
    ...
    expect(
      ...
      '- agents.main: active tool "fuzzplugin_move_angles" from plugin "fuzzplugin" has unsupported runtime input schema (fuzzplugin_move_angles.parameters.properties.target.$dynamicRef). MarketingClaw will quarantine this tool at runtime; fix or disable the plugin, or remove the tool from active allowlists.',
    ]);
    expect(toolState.createTools).toHaveBeenCalledWith(
