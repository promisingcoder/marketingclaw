...
import { AGENT_MODEL_CONFIG_KEYS } from "@marketingclaw/model-catalog-core/configured-model-refs";
import { normalizeProviderId } from "@marketingclaw/model-catalog-core/provider-id";
import { asOptionalRecord as asMutableRecord } from "@marketingclaw/normalization-core/record-coerce";
...
function resolveImplicitDefaultAgentModelRef(cfg: MarketingClawConfig): string {
...
function collectConfigModelRefs(cfg: MarketingClawConfig): CodexRouteHit[] {
...
function codexPluginAllowlistIsRestrictive(cfg: MarketingClawConfig): boolean {
  const allow = cfg.plugins?.allow;
  return Array.isArray(allow) && allow.length > 0 && !pluginIdListIncludes(allow, "codex");
}

function isCodexPluginUnavailableByConfig(cfg: MarketingClawConfig): boolean {
...
function codexPluginIsBlockedOutsideEntry(cfg: MarketingClawConfig): boolean {
...
}): { cfg: MarketingClawConfig; changes: string[] } {
...
function rewriteAgentModelRefs(params: {
  cfg: MarketingClawConfig;
  preRepairCfg: MarketingClawConfig;
  hits: CodexRouteHit[];
...
function clearConfigLegacyAgentRuntimePolicies(cfg: MarketingClawConfig): string[] {
  const changes: string[] = [];
...
function configRepairWouldClearLegacyRuntimePins(params: { cfg: MarketingClawConfig }): boolean {
...
function rewriteConfigModelRefs(params: { cfg: MarketingClawConfig }): ConfigRouteRepairResult {
...
function formatUnsupportedCompactionWarning(params: {
  ...
  return [
    "- Codex runtime uses native server-side compaction and ignores MarketingClaw compaction summarizer overrides.",
    ...params.hits.map(
...
function formatLegacyLosslessCompactionWarning(params: {
  ...
  return [
    ...
    ...configLines,
    params.canAutoFix
      ? "- Run `marketingclaw doctor --fix`: it migrates legacy Lossless compaction config to the Lossless context-engine slot."
...
function formatDisabledCodexPluginWarning(params: {
  ...
  const fixHint = params.blockedOutsideEntry
    ? "- Enable plugin loading and remove `codex` from plugins.deny, or set the affected OpenAI models to an MarketingClaw runtime policy."
    : "- Run `marketingclaw doctor --fix`: it enables plugins.entries.codex, or set the affected OpenAI models to an MarketingClaw runtime policy.";
  return [
...
function collectCodexAppServerCommandWarnings(cfg: MarketingClawConfig): string[] {
  const plugins = asMutableRecord(cfg.plugins);
  const entries = asMutableRecord(plugins?.entries);
  ...
  if (hits.length > 0) {
    warnings.push(
      [
        ...
        "- Run `marketingclaw doctor --fix`: it rewrites configured model refs and stale sessions to `openai/*`, moves Codex intent to provider/model runtime policy, and clears old whole-agent runtime pins.",
      ].join("\n"),
    );
  }
  ...
  if (fixableHits.length > 0) {
    warnings.push(
      formatUnsupportedCompactionWarning({
        hits: fixableHits,
        fixHint:
          "- Run `marketingclaw doctor --fix`: it removes unsupported Codex compaction overrides.",
      }),
    );
  }
  return warnings;
...
}): { cfg: MarketingClawConfig; warnings: string[]; changes: string[] } {
  const hits = collectConfigModelRefs(params.cfg);
  const disabledCodexPluginHits = collectDisabledCodexPluginRouteHits(params.cfg);
  const ignoreLegacyAgentRuntimePins = configRepairWouldClearLegacyRuntimePins(params);
  const unsupportedCompactionOverrides = collectUnsupportedCodexCompactionOverrides({
...
function clearStaleSessionRuntimePins(entry: SessionEntry): boolean {
  ...
  let changed = false;
  if (entry.agentHarnessId !== undefined && harnessRuntime !== "marketingclaw") {
    delete entry.agentHarnessId;
    changed = true;
  }
  if (entry.agentRuntimeOverride !== undefined && overrideRuntime !== "marketingclaw") {
        ...
        stale.length > 0
          ? [
              [
                ...
                "- Run `marketingclaw doctor --fix` to rewrite stale session model/provider pins across all agent session stores.",
              ].join("\n"),
            ]
          : [],
      changes: [],
