...
const LEGACY_OAUTH_SECRET_KEY_ENV = "MARKETINGCLAW_AUTH_PROFILE_SECRET_KEY";
const LEGACY_OAUTH_SECRET_KEYCHAIN_SERVICE = "MarketingClaw Auth Profile Secrets";
...
function buildLegacyOAuthSecretKey(seed: string): Buffer {
  ...
  return hash("sha256", `marketingclaw:auth-profile-oauth:${seed}`, "buffer");
}
...
function resolveLegacyOAuthSecretKeyFileCandidates(env: NodeJS.ProcessEnv): string[] {
  if (process.platform === "win32") {
    ...
    return uniquePaths([
      root ? path.join(root, "MarketingClaw", LEGACY_OAUTH_SECRET_KEY_FILE_NAME) : undefined,
      home
        ? path.join(home, ".marketingclaw-auth-profile-secrets", LEGACY_OAUTH_SECRET_KEY_FILE_NAME)
        : undefined,
    ]);
  }
...
function resolveLegacyOAuthSecretKeySeeds(env: NodeJS.ProcessEnv): string[] {
  ...
  if (env.NODE_ENV === "test" && env.VITEST === "true") {
    addSeed("marketingclaw-test-oauth-profile-secret-key");
  }
...
function emitKeychainOnlyMigrationHintOnce(profileId: string): void {
  ...
  log.warn(
    "Legacy Codex OAuth credentials are stored only in macOS Keychain on this host. " +
      "Headless paths cannot prompt for Keychain access; run `marketingclaw doctor --fix` " +
