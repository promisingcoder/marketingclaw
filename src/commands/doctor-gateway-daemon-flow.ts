...
function renderEstablishedGatewayConnections(connections: PortConnection[]): string {
  return [
    ...
    "If logs show protocol mismatch after rollback, stop stale MarketingClaw client processes listed here and rerun doctor.",
  ].join("\n");
}
...
export async function maybeRepairGatewayDaemon(params: {
  ...
  if (process.platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(process.env.MARKETINGCLAW_PROFILE);
    note(
      `LaunchAgent loaded; stopping requires "${formatCliCommand("marketingclaw gateway stop")}" or launchctl bootout gui/$UID/${label}.`,
      "Gateway",
    );
  }
