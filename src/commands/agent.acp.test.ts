...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "marketingclaw-agent-acp-" });
}

function createAcpEnabledConfig(home: string, storePath: string): MarketingClawConfig {
  return {
    acp: {
      enabled: true,
      backend: "acpx",
...
function mockConfigWithAcpOverrides(
  home: string,
  storePath: string,
  acpOverrides: Partial<NonNullable<MarketingClawConfig["acp"]>>,
) {
  const cfg = createAcpEnabledConfig(home, storePath);
  cfg.acp = {
    ...cfg.acp,
...
describe("agentCommand ACP runtime routing", () => {
  ...
  it("blocks ACP turns when disabled by policy", async () => {
    ...
    ] satisfies Array<Partial<NonNullable<MarketingClawConfig["acp"]>>>) {
