 ...
 *
 *   - RED: ACP-keyed session resolves to `id: "marketingclaw"`, `source: "implicit"`.
...
/**
 * Build a minimal `MarketingClawConfig` that mirrors the deployed scenario:
 ...
 *
 * Result: the old metadata resolver fell through to the implicit "marketingclaw"
 * branch — which is the bug under test.
 */
function buildConfigWithoutAgentRuntimePolicy(): MarketingClawConfig {
  return {
    agents: {
      list: [
        {
...
describe("sessions --json agentRuntime classifier (catalog #18)", () => {
  it("RED→GREEN: ACP session key is no longer misclassified (overlay applies)", () => {
    ...
    // resolved metadata said id="marketingclaw", source="implicit".
    ...
    expect(
      agentRuntime.id,
      `ACP session ${ACP_SESSION_KEY} should no longer be misclassified as "auto" or "marketingclaw". ` +
  ...
  it("FIX-SHAPE expectation: ACP session should resolve to 'acpx'", () => {
    ...
    // from "marketingclaw" and reflects the actual runtime driving the session.
