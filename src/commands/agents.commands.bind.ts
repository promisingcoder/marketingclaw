...
function resolveTargetAgentIdOrExit(params: {
  ...
  if (!agentId) {
    params.runtime.error(
      `Unable to resolve agent id. Run ${formatCliCommand("marketingclaw agents list")} to choose one.`,
    );
  ...
  if (!hasAgent(params.cfg, agentId)) {
    params.runtime.error(
      `Agent "${agentId}" not found. Run ${formatCliCommand("marketingclaw agents list")} to see configured agents.`,
    );
    params.runtime.exit(1);
    return null;
  }
...
export async function agentsBindingsCommand(
  ...
  if (opts.agent && !filterAgentId) {
    runtime.error(
      `Agent id is required. Run ${formatCliCommand("marketingclaw agents list")} to choose one.`,
    );
    runtime.exit(1);
    return;
  }
  if (filterAgentId && !hasAgent(cfg, filterAgentId)) {
    runtime.error(
      `Agent "${filterAgentId}" not found. Run ${formatCliCommand("marketingclaw agents list")} to see configured agents.`,
    );
    runtime.exit(1);
    return;
  }
