...
describe("status daemon summary", () => {
  it("preserves service layout diagnostics for status output", async () => {
    mocks.readServiceStatusSummary.mockResolvedValueOnce({
      ...
      layout: {
        execStart: "/usr/bin/node /opt/marketingclaw/dist/entry.js gateway",
        sourceScope: "system",
    ...
    expect(summary.layout?.execStart).toBe(
      "/usr/bin/node /opt/marketingclaw/dist/entry.js gateway",
    );
