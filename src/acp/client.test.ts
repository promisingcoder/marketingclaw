...
const createTempDir = () => tempDirs.make("marketingclaw-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets MARKETINGCLAW_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "marketingclaw",
    });

    expect(env.MARKETINGCLAW_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("marketingclaw");
  });

  it("overrides pre-existing MARKETINGCLAW_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      MARKETINGCLAW_SHELL: "wrong",
    });
    expect(env.MARKETINGCLAW_SHELL).toBe("acp-client");
  });
  ...
  it("preserves MARKETINGCLAW_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        MARKETINGCLAW_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["MARKETINGCLAW_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default MarketingClaw bridge", () => {
    ...
    expect(env.MARKETINGCLAW_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.MARKETINGCLAW_SHELL).toBe("acp-client");
  });
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "marketingclaw",
        serverArgs: ["acp"],
        defaultServerCommand: "marketingclaw",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("MARKETINGCLAW_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "marketingclaw", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "marketingclaw", "dist", "entry.js");
    const shimPath = path.join(dir, "marketingclaw.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\marketingclaw\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
  ...
  it("prompts for search when rawInput path escapes cwd", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/marketingclaw-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
  ...
  it("auto-approves search when query-like title text contains a path label", async () => {
    await expectAutoAllowWithoutPrompt({
      ...
      cwd: "/tmp/marketingclaw-acp-cwd/workspace",
    });
  });
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/marketingclaw-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/marketingclaw-acp-cwd",
    });
