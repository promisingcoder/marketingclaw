...
describe("handleSystemRunInvoke mac app exec host routing", () => {
  ...
  let sharedMarketingClawHome = "";
  let sharedRuntimeBinDir = "";
  let sharedFixtureId = 0;
  let previousMarketingClawHome: string | undefined;
  const sharedRuntimeBins = new Set<string>();

  beforeAll(() => {
    sharedFixtureRoot = fs.realpathSync(
      fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-node-host-fixtures-")),
    );
    sharedMarketingClawHome = path.join(sharedFixtureRoot, "marketingclaw-home");
    sharedRuntimeBinDir = path.join(sharedFixtureRoot, "bin");
    fs.mkdirSync(sharedMarketingClawHome, { recursive: true });
  ...
  beforeEach(() => {
    ...
    process.env.MARKETINGCLAW_HOME = sharedMarketingClawHome;
  ...
  async function withTempApprovalsHome<T>(params: {
    ...
    const tempHome = sharedMarketingClawHome;
  ...
  it("uses auto reviewer for system.run approval misses when exec mode is auto", async () => {
    const tmp = createFixtureDir("marketingclaw-system-run-auto-review-");
  ...
  it("does not auto-review direct system.run approval misses without an approval plan", async () => {
    const tmp = createFixtureDir("marketingclaw-system-run-auto-review-no-plan-");
    const executablePath = createTempExecutable({ dir: tmp, name: "read-info" });
    setRuntimeConfigSnapshot({
      tools: {
        exec: {
  ...
  it("does not auto-review direct system.run security audit suppression edits", async () => {
    const tmp = createFixtureDir("marketingclaw-system-run-auto-review-suppression-");
    const executablePath = createTempExecutable({ dir: tmp, name: "marketingclaw" });
    setRuntimeConfigSnapshot({
      tools: {
        exec: {
          mode: "auto",
  ...
  it("defers to human approval when system.run auto reviewer asks", async () => {
    const tmp = createFixtureDir("marketingclaw-system-run-auto-review-ask-");
    const executablePath = createTempExecutable({ dir: tmp, name: "read-info" });
    setRuntimeConfigSnapshot({
      tools: {
        exec: {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      for (const testCase of approvedEnvShellWrapperCases) {
        const tmp = createFixtureDir("marketingclaw-approved-wrapper-");
  ...
  it("denies shell payload carriers in allowlist mode without explicit approval", async () => {
    ...
    > = [
      ...
      },
      {
        ...
        command:
          process.platform === "win32"
            ? ["cmd.exe", "/d", "/s", "/c", "marketingclaw status; id"]
            : ["/bin/sh", "-lc", "marketingclaw status; id"],
        approvalRequired: true,
      },
      {
        ...
        command:
          process.platform === "win32"
            ? ["cmd.exe", "/d", "/s", "/c", "marketingclaw status; cat /etc/passwd"]
            : ["/bin/sh", "-lc", "marketingclaw status; cat /etc/passwd"],
        approvalRequired: true,
      },
      {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const { runCommand, sendInvokeResult } = await runSystemInvoke({
        ...
        security: "allowlist",
        ask: "off",
        command: ["/bin/sh", "-lc", "head -c${IFS}16${IFS}${MARKETINGCLAW_CONFIG_PATH}"],
        rawCommand: "head -c${IFS}16${IFS}${MARKETINGCLAW_CONFIG_PATH}",
      });
  ...
  it("denies abbreviated PowerShell encoded payloads even when the wrapper is allowlisted", async () => {
    const binDir = createFixtureDir("marketingclaw-pwsh-allowlist-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withPathTokenCommand({
        tmpPrefix: "marketingclaw-approval-path-pin-",
        run: async ({ expected }) => {
          const { runCommand, sendInvokeResult } = await runSystemInvoke({
            preferMacAppExecHost: false,
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const sendInvokeResult = vi.fn(async () => {});
      await withPathTokenCommand({
        tmpPrefix: "marketingclaw-allowlist-path-pin-",
      ...
      for (const testCase of [
        {
          label: "cwd symlink",
          setup: () => {
            const tmp = createFixtureDir("marketingclaw-approval-cwd-link-");
        ...
        },
        {
          ...
          setup: () => {
            const tmp = createFixtureDir("marketingclaw-approval-cwd-parent-link-");
  ...
  it("uses canonical executable path for approval-based relative command execution", async () => {
    const tmp = createFixtureDir("marketingclaw-approval-cwd-real-");
  ...
  it("denies approval-based execution when cwd identity drifts before execution", async () => {
    const tmp = createFixtureDir("marketingclaw-approval-cwd-drift-");
    const fallback = createFixtureDir("marketingclaw-approval-cwd-drift-alt-");
    const script = path.join(tmp, "run.sh");
    fs.writeFileSync(script, "#!/bin/sh\necho SAFE\n");
    fs.chmodSync(script, 0o755);
    const canonicalCwd = fs.realpathSync(tmp);
  ...
  it("validates approved script operand bindings at dispatch", async () => {
    for (const mutate of [true, false]) {
      const tmp = createFixtureDir(
        mutate ? "marketingclaw-approval-script-drift-" : "marketingclaw-approval-script-stable-",
      );
  ...
  it("validates approved runtime script operand bindings at dispatch", async () => {
    await withFakeRuntimeOnPath({
      runtime: "tsx",
      run: async () => {
        const tmp = createFixtureDir("marketingclaw-approval-tsx-script-drift-");
        ...
        const missingBindingTmp = createFixtureDir("marketingclaw-approval-tsx-missing-binding-");
  ...
  it("denies ./sh wrapper spoof in allowlist on-miss mode before execution", async () => {
    const marker = path.join(
      os.tmpdir(),
      `marketingclaw-wrapper-spoof-${process.pid}-${Date.now()}`,
    );
    const runCommand = vi.fn(async () => {
  ...
  it("does not persist allow-always approvals for strict inline-eval carriers", async () => {
    ...
    try {
      await withTempApprovalsHome({
        approvals: createAllowlistOnMissApprovals(),
        run: async () => {
          const tempDir = createFixtureDir("marketingclaw-inline-eval-bin-");
  ...
  it("persists benign awk allow-always approvals in strict inline-eval mode without reopening inline carriers", async () => {
    ...
    try {
      await withTempApprovalsHome({
        approvals: createAllowlistOnMissApprovals(),
        run: async () => {
          const tempDir = createFixtureDir("marketingclaw-inline-eval-awk-");
          const executablePath = createTempExecutable({
            dir: tempDir,
            name: "awk",
          });
  ...
  it("does not persist allow-always approvals for strict inline-eval make carriers", async () => {
    ...
    try {
      await withTempApprovalsHome({
        approvals: createAllowlistOnMissApprovals(),
        run: async () => {
          const tempDir = createFixtureDir("marketingclaw-inline-eval-make-");
          const executablePath = createTempExecutable({
            dir: tempDir,
            name: "make",
          });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = createFixtureDir("marketingclaw-shell-wrapper-inner-");
  ...
  it("keeps cmd.exe transport wrappers approval-gated on Windows", async () => {
    ...
    try {
      for (const testCase of [
        ...
        const tempDir = createFixtureDir("marketingclaw-cmd-wrapper-allow-");
  ...
  it("reuses exact-command durable trust for shell-wrapper reruns", async () => {
    ...
    const tempDir = createFixtureDir("marketingclaw-shell-wrapper-allow-");
    const prepared = buildSystemRunApprovalPlan({
      command: ["/bin/sh", "-c", "cd ."],
      cwd: tempDir,
    });
