...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        MARKETINGCLAW_BASE_ENV: "base",
        MARKETINGCLAW_TO_REMOVE: undefined,
      },
      env: {
        MARKETINGCLAW_TEST_ENV: "ok",
      },
    });

    expect(resolved.MARKETINGCLAW_BASE_ENV).toBe("base");
    expect(resolved.MARKETINGCLAW_TEST_ENV).toBe("ok");
    expect(resolved.MARKETINGCLAW_TO_REMOVE).toBeUndefined();
    expect(resolved.MARKETINGCLAW_CLI).toBe(MARKETINGCLAW_CLI_ENV_VALUE);
  });
