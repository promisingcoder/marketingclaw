...
const resolveMarketingClawPackageRootSyncMock = vi.fn();
...
vi.mock("../infra/marketingclaw-root.js", () => ({
  resolveMarketingClawPackageRootSync: (...args: unknown[]) =>
    resolveMarketingClawPackageRootSyncMock(...args),
}));
...
const suiteTempRootTracker = createSuiteTempRootTracker("marketingclaw-plugin-install-npm-spec");
...
function writeInstalledNpmPlugin(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      marketingclaw: params.marketingclaw ?? { extensions: ["./dist/index.js"] },
      ...(params.dependency
...
type MockNpmPackage = {
  ...
  materializesRootMarketingClaw?: boolean;
...
function writeNpmRootPackageLock(params: {
  ...
  for (const pkg of params.packages) {
    ...
    if (pkg.materializesRootMarketingClaw) {
      lockPackages["node_modules/marketingclaw"] = {
        peer: true,
        version: "2026.5.3",
      };
    }
...
function prunePluginLocalMarketingClawPeerLinks(npmRoot: string) {
  ...
  for (const entry of fs.readdirSync(nodeModulesDir, { withFileTypes: true })) {
    ...
    for (const packageDir of packageDirs) {
      ...
      fs.rmSync(path.join(packageNodeModulesDir, "marketingclaw"), {
        recursive: true,
        force: true,
      });
    }
  ...
  runCommandWithTimeoutMock.mockImplementation(
    async (argv: string[], options?: { cwd?: string }) => {
      ...
      if (viewPackage) {
        return successfulSpawn(
          JSON.stringify({
            ...
            ...(viewPackage.marketingclaw ? { marketingclaw: viewPackage.marketingclaw } : {}),
          }),
        );
      }
      ...
      if (isManagedNpmInstallCommand(argv)) {
        ...
        prunePluginLocalMarketingClawPeerLinks(npmRoot);
        for (const packageName of Object.keys(manifest.dependencies ?? {})) {
          if (packageName === "marketingclaw") {
            const marketingclawRoot = path.join(npmRoot, "node_modules", "marketingclaw");
            fs.mkdirSync(marketingclawRoot, { recursive: true });
            fs.writeFileSync(
              path.join(marketingclawRoot, "package.json"),
              JSON.stringify({ name: "marketingclaw", version: "0.0.0-test" }),
              "utf8",
            );
            continue;
          }
          ...
          if (pkg.materializesRootMarketingClaw) {
            ...
            fs.writeFileSync(
              path.join(marketingclawRoot, "package.json"),
              JSON.stringify({ name: "marketingclaw", version: "2026.5.3" }),
              "utf8",
            );
          }
      ...
      if (argv[0] === "npm" && argv[1] === "uninstall") {
        ...
        if (packageName === "marketingclaw") {
          ...
          fs.rmSync(path.join(npmRoot, "node_modules", "marketingclaw"), {
            recursive: true,
            force: true,
          });
          return successfulSpawn();
...
beforeEach(() => {
  ...
  resolveMarketingClawPackageRootSyncMock.mockReset();
  const hostRoot = suiteTempRootTracker.makeTempDir();
  fs.writeFileSync(
    path.join(hostRoot, "package.json"),
    `${JSON.stringify({ name: "marketingclaw", version: "0.0.0-test" }, null, 2)}\n`,
    "utf8",
  );
  resolveMarketingClawPackageRootSyncMock.mockReturnValue(hostRoot);
  vi.unstubAllEnvs();
  process.env.NPM_CONFIG_GLOBALCONFIG = npmGlobalConfigPath;
});
...
beforeAll(async () => {
  ...
  const archivePath = path.join(stateDir, "marketingclaw-pack-demo-1.2.3.tgz");
  ...
  mockNpmViewAndInstallMany([
    ...
    {
      spec: "@marketingclaw/voice-call@0.0.1",
...
describe("installPluginFromNpmSpec", () => {
  it("installs npm pack archives through the managed npm root", async () => {
    ...
    expect(result.npmTarballName).toBe("marketingclaw-pack-demo-1.2.3.tgz");
    expectNpmInstallIntoProject({
      calls,
      npmRoot,
      packageName: "@openclaw/pack-demo",
    });
    expect(dependencySpec).toMatch(/^file:\.\/_marketingclaw-pack-archives\/.+\.tgz$/);
  ...
  it("rejects npm pack archive metadata with traversal package names", async () => {
    ...
    expect(fs.existsSync(path.join(npmRoot, "_marketingclaw-pack-archives"))).toBe(false);
  ...
  it("updates staged npm pack archives when dangerous-looking code is present", async () => {
    ...
    const archiveV1Path = path.join(stateDir, "marketingclaw-pack-demo-1.0.0.tgz");
    const archiveV2Path = path.join(stateDir, "marketingclaw-pack-demo-2.0.0.tgz");
  ...
  it("installs staged npm pack archives with dangerous-looking code", async () => {
    ...
    const npmRoot = path.join(stateDir, "npm");
    const packageName = "@openclaw/pack-demo";
    const archivePath = path.join(stateDir, "marketingclaw-pack-demo-2.0.0.tgz");
  ...
  it("installs npm plugins into .marketingclaw/npm", async () => {
    ...
    expect(result.targetDir).toBe(
      resolveTestPluginPackageDir(npmRoot, "@marketingclaw/voice-call"),
    );
    expect(result.npmResolution?.resolvedSpec).toBe("@marketingclaw/voice-call@0.0.1");
  ...
  it("repairs omitted current-platform packages with a fresh npm cache", async () => {
    ...
    expect(repairCacheDir).toContain("marketingclaw-npm-cache-");
  ...
  it("quarantines and rebuilds a corrupt managed npm project after npm from-argument failures", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const packageName = "@marketingclaw/voice-call";
    const warnings: string[] = [];
    const npmProjectRoot = resolvePluginNpmProjectDir({ npmDir: npmRoot, packageName });
    ...
    const quarantineParent = path.join(npmProjectRoot, "_marketingclaw-quarantined-npm-projects");
  ...
  it.runIf(process.platform !== "win32")(
    "does not let managed marketingclaw peer links poison later npm installs",
    async () => {
      const stateDir = suiteTempRootTracker.makeTempDir();
      const npmRoot = path.join(stateDir, "npm");
      ...
      mockNpmViewAndInstallMany([
        {
          ...
          npmRoot,
          peerDependencies: { marketingclaw: "^2026.0.0" },
        },
        {
      ...
      expect(
        fs.lstatSync(path.join(peerPluginDir, "node_modules", "marketingclaw")).isSymbolicLink(),
      ).toBe(true);
      ...
      if (!second.ok) {
        expect(second.error).not.toContain("peer-plugin/node_modules/marketingclaw");
      }
      expect(
        fs.lstatSync(path.join(peerPluginDir, "node_modules", "marketingclaw")).isSymbolicLink(),
      ).toBe(true);
  ...
  it("rejects managed npm plugins when their marketingclaw peer link cannot be repaired", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const warnings: string[] = [];

    resolveMarketingClawPackageRootSyncMock.mockReturnValue(null);
    mockNpmViewAndInstall({
      spec: "@marketingclaw/codex@2026.5.7",
      packageName: "@marketingclaw/codex",
      version: "2026.5.7",
      pluginId: "@marketingclaw/codex",
      npmRoot,
      peerDependencies: { marketingclaw: ">=2026.5.7" },
    });
    ...
    expect(result.error).toContain("@marketingclaw/codex");
    expect(result.error).toContain("plugin-local node_modules/marketingclaw link");
    expect(
      warnings.some((warning) => warning.includes("Could not locate marketingclaw package root")),
    ).toBe(true);
    expect(fs.existsSync(resolveTestPluginPackageDir(npmRoot, "@marketingclaw/codex"))).toBe(false);
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@marketingclaw/codex",
    });
  ...
  it("rejects exact npm plugins whose package compatibility requires a newer host", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    vi.stubEnv("MARKETINGCLAW_COMPATIBILITY_HOST_VERSION", "2026.5.10-beta.1");

    mockNpmViewAndInstall({
      spec: "@marketingclaw/whatsapp@2026.5.27",
      packageName: "@marketingclaw/whatsapp",
      version: "2026.5.27",
      pluginId: "whatsapp",
      npmRoot,
      peerDependencies: { marketingclaw: ">=2026.5.27" },
      marketingclaw: {
        extensions: ["./dist/index.js"],
    ...
    expect(fs.existsSync(path.join(npmRoot, "node_modules", "@marketingclaw", "whatsapp"))).toBe(
      false,
    );
    expect(fs.existsSync(path.join(npmRoot, "package.json"))).toBe(false);
    expect(
  ...
  it("installs the newest compatible npm version for unpinned plugins", async () => {
    ...
    mockNpmViewAndInstallMany([
      ...
      {
        spec: "@marketingclaw/whatsapp@2026.5.26",
        packageName: "@marketingclaw/whatsapp",
        version: "2026.5.26",
        pluginId: "whatsapp",
        npmRoot,
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@marketingclaw/whatsapp@2026.5.26");
    expect(result.npmResolution?.version).toBe("2026.5.26");
    expect(warnings.join("\n")).toContain(
      "using newest compatible @marketingclaw/whatsapp@2026.5.26",
    );
    expect(
      JSON.parse(
        fs.readFileSync(
          path.join(
            resolveTestPluginPackageDir(npmRoot, "@marketingclaw/whatsapp"),
            "package.json",
          ),
          "utf8",
        ),
  ...
  it("preserves an existing npm plugin by resolving update metadata to a compatible version", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        private: true,
        dependencies: {
          "@marketingclaw/whatsapp": "2026.5.26",
        },
      }),
      "utf8",
    );
    ...
    expect(managedManifest.dependencies?.["@marketingclaw/whatsapp"]).toBe("2026.5.26");
    expect(
      runCommandWithTimeoutMock.mock.calls.some(([argv]) => isManagedNpmInstallCommand(argv)),
    ).toBe(true);
  });
  ...
  it("resolves incompatible prerelease tags to a compatible prerelease version", async () => {
    ...
    mockNpmViewAndInstallMany([
      {
        spec: "@marketingclaw/msteams@beta",
        packageName: "@marketingclaw/msteams",
      ...
      {
        spec: "@marketingclaw/msteams@2026.5.28-beta.3",
        packageName: "@marketingclaw/msteams",
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@marketingclaw/msteams@2026.5.28-beta.3");
    expect(result.npmResolution?.version).toBe("2026.5.28-beta.3");
    expect(warnings.join("\n")).toContain(
      "using newest compatible @marketingclaw/msteams@2026.5.28-beta.3",
    );
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@marketingclaw/msteams",
    ...
    expect(managedManifest.dependencies?.["@marketingclaw/msteams"]).toBe("2026.5.28-beta.3");
  });
  ...
  it.runIf(process.platform !== "win32")(
    "repairs root marketingclaw materialized by npm peer handling",
    async () => {
      const stateDir = suiteTempRootTracker.makeTempDir();
      const npmRoot = path.join(stateDir, "npm");
      ...
      mockNpmViewAndInstall({
        ...
        npmRoot,
        peerDependencies: { marketingclaw: "^2026.0.0" },
        materializesRootMarketingClaw: true,
      });
      ...
      expect(fs.existsSync(path.join(npmProjectRoot, "node_modules", "marketingclaw"))).toBe(false);
      ...
      expect(
        fs
          .lstatSync(path.join(requiredPeerPluginDir, "node_modules", "marketingclaw"))
          .isSymbolicLink(),
      ).toBe(true);
    },
  );

  it("repairs stale managed marketingclaw root packages before npm plugin installs", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
    ...
    fs.mkdirSync(path.join(npmProjectRoot, "node_modules", "marketingclaw"), { recursive: true });
    fs.writeFileSync(
      path.join(npmProjectRoot, "package.json"),
      JSON.stringify(
        {
      ...
      `${JSON.stringify(
        {
          ...
          packages: {
            ...
            "node_modules/marketingclaw": {
              version: "2026.5.4",
              resolved: "https://registry.npmjs.org/marketingclaw/-/marketingclaw-2026.5.4.tgz",
            },
          },
          dependencies: {
            marketingclaw: {
    ...
    fs.writeFileSync(
      path.join(npmProjectRoot, "node_modules", "marketingclaw", "package.json"),
      JSON.stringify({
        name: "marketingclaw",
        version: "2026.5.4",
      }),
    ...
    mockNpmViewAndInstall({
      spec: "@marketingclaw/discord@beta",
      packageName: "@marketingclaw/discord",
      version: "2026.5.5-beta.1",
      pluginId: "discord",
      npmRoot,
      peerDependencies: { marketingclaw: ">=2026.5.5-beta.1" },
    ...
    expect(manifest.dependencies).not.toHaveProperty("marketingclaw");
    expect(manifest.dependencies?.["@marketingclaw/discord"]).toBe("2026.5.5-beta.1");
    const lockfile = JSON.parse(
      fs.readFileSync(path.join(npmProjectRoot, "package-lock.json"), "utf8"),
    ) as {
      packages?: Record<string, unknown>;
  ...
  it("preserves the active host marketingclaw runtime package during npm plugin installs", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    ...
    resolveMarketingClawPackageRootSyncMock.mockReturnValue(hostPackageRoot);
    mockNpmViewAndInstall({
      spec: "@xdarkicex/marketingclaw-memory-libravdb@1.4.69",
      packageName: "@xdarkicex/marketingclaw-memory-libravdb",
      version: "1.4.69",
    ...
    expect(baseManifest.dependencies?.marketingclaw).toBe("2026.5.12-beta.6");
    expect(baseManifest.dependencies?.["@xdarkicex/marketingclaw-memory-libravdb"]).toBeUndefined();
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@xdarkicex/marketingclaw-memory-libravdb",
    });
    ...
    expect(projectManifest.dependencies?.["@xdarkicex/marketingclaw-memory-libravdb"]).toBe(
      "1.4.69",
    );
    expect(fs.existsSync(hostPackageRoot)).toBe(true);
    expect(result.targetDir).toBe(
      resolveTestPluginPackageDir(npmRoot, "@xdarkicex/marketingclaw-memory-libravdb"),
    );
    expect(
  ...
  it("rolls back the managed npm root when npm install fails", async () => {
    ...
    runCommandWithTimeoutMock.mockImplementation(
      async (argv: string[], options?: { cwd?: string }) => {
        if (
          JSON.stringify(argv) === JSON.stringify(npmViewArgv("@marketingclaw/voice-call@0.0.1"))
        ) {
          return successfulSpawn(
            JSON.stringify({
        ...
        if (argv[0] === "npm" && argv[1] === "uninstall") {
          if (!(argv as string[]).includes("--legacy-peer-deps")) {
            fs.mkdirSync(path.join(options?.cwd ?? npmRoot, "node_modules", "marketingclaw"), {
              recursive: true,
            });
          }
          return successfulSpawn("");
    ...
    await expect(
      fs.promises.access(path.join(npmProjectRoot, "node_modules", "marketingclaw")),
    ).rejects.toHaveProperty("code", "ENOENT");
  });

  it("does not fail rollback snapshots on plugin-local marketingclaw peer symlinks", async () => {
    const npmRoot = path.join(suiteTempRootTracker.makeTempDir(), "npm");
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@marketingclaw/codex",
    ...
    writeNpmRootPackageLock({
      npmRoot: npmProjectRoot,
      dependencies: { "@marketingclaw/codex": "0.0.1" },
      packages: [
        {
          packageName: "@marketingclaw/codex",
          version: "0.0.1",
    ...
    const installedDir = writeInstalledNpmPlugin({
      ...
      packageName: "@marketingclaw/codex",
      version: "0.0.1",
      peerDependencies: { marketingclaw: "*" },
    });
    const peerLink = path.join(installedDir, "node_modules", "marketingclaw");
    ...
    runCommandWithTimeoutMock.mockImplementation(
      async (argv: string[], options?: { cwd?: string }) => {
        if (JSON.stringify(argv) === JSON.stringify(npmViewArgv("@marketingclaw/codex@0.0.2"))) {
          return successfulSpawn(
            JSON.stringify({
    ...
    try {
      const result = await installPluginFromNpmSpec({
        spec: "@marketingclaw/codex@0.0.2",
        npmDir: npmRoot,
        mode: "update",
        logger: { info: () => {}, warn: () => {} },
      });
      ...
      async (argv: string[], options?: { cwd?: string }) => {
        if (isManagedNpmInstallCommand(argv)) {
          ...
          const manifest = JSON.parse(
            ...
            marketingclaw?: { managedOverrides?: string[] };
          };
          if (installAttempts === 1) {
            expect(manifest.overrides?.["node-domexception"]).toBe(
              "npm:@nolyfill/domexception@1.0.28",
            );
            expect(manifest.marketingclaw?.managedOverrides).toEqual([
              "axios",
              "nested",
              "node-domexception",
            ]);
          ...
          expect(manifest.marketingclaw?.managedOverrides).toEqual(["axios", "nested"]);
        }
    ...
    async ({ spec, pluginId, indexJs }) => {
      ...
      expect(
        warnings.some((warning) =>
          warning.includes("allowed because it is an official MarketingClaw package"),
        ),
      ).toBe(false);
    },
  );
  ...
  it("rejects duplicate npm installs unless update mode is requested", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const installRoot = resolveTestPluginPackageDir(npmRoot, "@marketingclaw/voice-call");
  ...
  it("allows duplicate npm installs in update mode", async () => {
    ...
    mockNpmViewAndInstall({
      spec: "@marketingclaw/voice-call@0.0.2",
      packageName: "@marketingclaw/voice-call",
      version: "0.0.2",
      pluginId: "voice-call",
      npmRoot,
  ...
  it("preserves previously installed sibling plugins during npm install", async () => {
    ...
    mockNpmViewAndInstallMany([
      ...
      {
        spec: "@marketingclaw/whatsapp@0.0.1",
        packageName: "@marketingclaw/whatsapp",
        version: "0.0.1",
        pluginId: "whatsapp",
        npmRoot,
    ...
    expect(fs.existsSync(resolveTestPluginPackageDir(npmRoot, "@marketingclaw/voice-call"))).toBe(
      true,
    );
    expect(fs.existsSync(resolveTestPluginPackageDir(npmRoot, "@marketingclaw/whatsapp"))).toBe(
      true,
    );
  });
  ...
  it("rejects implicit prerelease npm specs with beta guidance", async () => {
    ...
    if (!rejected.ok) {
      ...
      expect(rejected.error).toContain('"@marketingclaw/voice-call@beta"');
    }
  });
  ...
  it("falls back to the latest stable version for official prerelease packages", async () => {
    ...
    expect(officialFallback.npmResolution?.resolvedSpec).toBe("@marketingclaw/voice-call@0.0.1");
    expect(warnings.join("\n")).toContain("falling back to stable @marketingclaw/voice-call@0.0.1");
  });
  ...
  it("keeps stable correction versions when resolving official npm packages", async () => {
    ...
    expect(stableCorrection.npmResolution?.resolvedSpec).toBe(
      "@marketingclaw/voice-call@2026.5.3-1",
    );
  ...
  it("uses the newest prerelease when an official package has no stable versions", async () => {
    ...
    expect(prereleaseOnly.npmResolution?.resolvedSpec).toBe(
      "@marketingclaw/voice-call@0.0.2-beta.1",
    );
    expect(prereleaseOnlyWarnings.join("\n")).toContain("has no stable npm versions yet");
    expect(prereleaseOnlyWarnings.join("\n")).toContain(
      "using newest prerelease @marketingclaw/voice-call@0.0.2-beta.1",
    );
  });
  ...
  it("accepts explicit prerelease npm dist-tags", async () => {
    ...
    expect(accepted.npmResolution?.resolvedSpec).toBe("@marketingclaw/voice-call@0.0.2-beta.1");
    expectNpmInstallIntoProject({
      calls: runCommandWithTimeoutMock.mock.calls,
      npmRoot,
      packageName: "@marketingclaw/voice-call",
