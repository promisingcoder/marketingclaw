...
const fixtureRoot = makeTrackedTempDir("marketingclaw-sdk-alias-root", fixtureTempDirs);
let tempDirIndex = 0;
...
function createTrustedMarketingClawPackageFixture(version: string) {
  const root = makeTempDir();
  fs.writeFileSync(path.join(root, "marketingclaw.mjs"), "export {};\n", "utf-8");
  fs.writeFileSync(
...
function createExtensionApiAliasFixture(params?: {
  ...
  fs.writeFileSync(
    path.join(root, "package.json"),
    JSON.stringify({ name: "marketingclaw", type: "module" }, null, 2),
    "utf-8",
  );
  fs.writeFileSync(path.join(root, "marketingclaw.mjs"), "export {};\n", "utf-8");
  fs.writeFileSync(srcFile, params?.srcBody ?? "export {};\n", "utf-8");
...
function createBundledPluginPackagePublicSurfaceAliasFixture() {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: "@marketingclaw/slack", type: "module" }, null, 2),
    "utf-8",
  );
...
function createUserInstalledPluginSdkAliasFixture() {
  ...
  const externalPluginRoot = path.join(makeTempDir(), ".marketingclaw", "extensions", "demo");
...
function expectPluginSdkAliasTargets(
  ...
  expect(fs.realpathSync(aliases["marketingclaw/plugin-sdk"] ?? "")).toBe(
  ...
  if (params.channelRuntimePath) {
    expect(fs.realpathSync(aliases["marketingclaw/plugin-sdk/channel-runtime"] ?? "")).toBe(
  ...
  if (params.pluginEntryPath) {
    expect(fs.realpathSync(aliases["marketingclaw/plugin-sdk/plugin-entry"] ?? "")).toBe(
...
function expectCwdFallbackPluginSdkAliasResolution(params: {
  ...
  const resolved = withCwd(params.fixture.root, () =>
    resolvePluginSdkAlias({
      ...
      distFile: "channel-runtime.js",
      modulePath: "/tmp/tsx-cache/marketingclaw-loader.js",
...
describe("plugin sdk alias helpers", () => {
  it.each([
    ...
    },
    {
      ...
      modulePath: () => "/tmp/tsx-cache/marketingclaw-loader.js",
      argv1: (root: string) => path.join(root, "marketingclaw.mjs"),
      srcFile: "index.ts",
      distFile: "index.js",
      env: { NODE_ENV: undefined },
      expected: "src" as const,
  ...
  it("resolves extension-api aliases from an explicit dev source root", () => {
    ...
    expect(fs.realpathSync(aliases["marketingclaw/extension-api"] ?? "")).toBe(
  ...
  it("adds private qa plugin-sdk subpaths for trusted local checkouts when enabled", () => {
    ...
    const subpaths = withEnv({ MARKETINGCLAW_ENABLE_PRIVATE_QA_CLI: "1" }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: path.join(fixture.root, "src", "plugins", "loader.ts"),
      }),
    );
  ...
  it("resolves public QA plugin-sdk aliases without enabling private QA mode", () => {
    ...
    const subpaths = withEnv({ MARKETINGCLAW_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      listPluginSdkExportedSubpaths({ modulePath: sourcePluginEntry }),
    );
    const aliases = withEnv(
      { MARKETINGCLAW_ENABLE_PRIVATE_QA_CLI: undefined, NODE_ENV: undefined },
    ...
    expect(fs.realpathSync(aliases["marketingclaw/plugin-sdk/qa-runner-runtime"] ?? "")).toBe(
      fs.realpathSync(sourceQaRunnerPath),
    );
    expect(
      fs.realpathSync(aliases["marketingclaw/plugin-sdk/qa-live-transport-scenarios"] ?? ""),
    ).toBe(fs.realpathSync(distQaLiveTransportScenariosPath));
    expect(aliases["marketingclaw/plugin-sdk/qa-runtime"]).toBeUndefined();
  });
  ...
  it("adds non-QA private Codex helper subpaths only for trusted Codex plugins", () => {
    ...
    const { packageRoot: installedCodexRoot, pluginEntry: installedCodexEntry } =
      writeInstalledPluginEntry({
        installRoot: path.join(makeTempDir(), ".marketingclaw", "npm"),
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: "@marketingclaw/codex", type: "module" }, null, 2),
      "utf-8",
    );
    fs.writeFileSync(shadowCodexEntry, 'export const plugin = "shadow";\n', "utf-8");

    const codexSubpaths = withEnv({ MARKETINGCLAW_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: sourceCodexEntry,
      }),
    );
    const otherSubpaths = withEnv({ MARKETINGCLAW_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: sourceOtherEntry,
      }),
    );
    const installedCodexSubpaths = withCwd(installedCodexRoot, () =>
      withEnv({ MARKETINGCLAW_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
        listPluginSdkExportedSubpaths({
          modulePath: installedCodexEntry,
          argv1: path.join(fixture.root, "marketingclaw.mjs"),
        }),
      ),
    );
  ...
  it("does not reuse a non-private cached subpath list after private qa gets enabled", () => {
    ...
    const privateSubpaths = withEnv({ MARKETINGCLAW_ENABLE_PRIVATE_QA_CLI: "1" }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: path.join(fixture.root, "src", "plugins", "loader.ts"),
      }),
    );
  ...
  it.each([
    {
      name: "does not derive plugin-sdk subpaths from cwd fallback when package root is not an MarketingClaw root",
      fixture: () =>
        createPluginSdkAliasFixture({
  ...
  it("adds private qa plugin-sdk aliases for source plugins when enabled", () => {
    ...
    const aliases = withEnv({ MARKETINGCLAW_ENABLE_PRIVATE_QA_CLI: "1", NODE_ENV: undefined }, () =>
      buildPluginLoaderAliasMap(sourcePluginEntry),
    );
    ...
    expect(fs.realpathSync(aliases["marketingclaw/plugin-sdk/qa-runtime"] ?? "")).toBe(
      fs.realpathSync(sourceQaRuntimePath),
    );
    expect(fs.realpathSync(aliases["marketingclaw/plugin-sdk/qa-channel"] ?? "")).toBe(
      fs.realpathSync(sourceQaChannelPath),
    );
    expect(fs.realpathSync(aliases["marketingclaw/plugin-sdk/qa-channel-protocol"] ?? "")).toBe(
      fs.realpathSync(sourceQaChannelProtocolPath),
    );
    expect(fs.realpathSync(aliases["marketingclaw/plugin-sdk/qa-lab"] ?? "")).toBe(
  ...
  it("aliases non-QA private plugin-sdk subpaths for trusted Codex runtime loading", () => {
    ...
    const devRootAliases = withEnv(
      ...
      () =>
        buildPluginLoaderAliasMap(
          distCodexEntry,
          path.join(fixture.root, "marketingclaw.mjs"),
          undefined,
          "dist",
          devFixture.root,
        ),
    ...
    const installedAliases = withCwd(installedCodexRoot, () =>
      withEnv({ MARKETINGCLAW_ENABLE_PRIVATE_QA_CLI: undefined, NODE_ENV: undefined }, () =>
        buildPluginLoaderAliasMap(
          installedCodexEntry,
          path.join(fixture.root, "marketingclaw.mjs"),
          undefined,
    ...
    expect(fs.realpathSync(aliases["marketingclaw/plugin-sdk/codex-mcp-projection"] ?? "")).toBe(
      fs.realpathSync(sourceCodexMcpProjectionPath),
    );
    expect(
      fs.realpathSync(aliases["marketingclaw/plugin-sdk/codex-native-task-runtime"] ?? ""),
    ).toBe(fs.realpathSync(sourceCodexNativeTaskRuntimePath));
    expect(
      fs.realpathSync(installedAliases["marketingclaw/plugin-sdk/codex-mcp-projection"] ?? ""),
    ).toBe(fs.realpathSync(distCodexMcpProjectionPath));
    expect(
      fs.realpathSync(installedAliases["marketingclaw/plugin-sdk/codex-native-task-runtime"] ?? ""),
    ).toBe(fs.realpathSync(distCodexNativeTaskRuntimePath));
    expect(fs.realpathSync(devRootAliases["marketingclaw/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(devRootAlias),
    );
    expect(
      fs.realpathSync(devRootAliases["marketingclaw/plugin-sdk/codex-mcp-projection"] ?? ""),
    ).toBe(fs.realpathSync(devCodexMcpProjectionPath));
    expect(
      fs.realpathSync(devRootAliases["marketingclaw/plugin-sdk/codex-native-task-runtime"] ?? ""),
    ).toBe(fs.realpathSync(devCodexNativeTaskRuntimePath));
    expect(aliases["marketingclaw/plugin-sdk/qa-runtime"]).toBeUndefined();
    expect(otherAliases["marketingclaw/plugin-sdk/codex-mcp-projection"]).toBeUndefined();
    expect(otherAliases["marketingclaw/plugin-sdk/codex-native-task-runtime"]).toBeUndefined();
    expect(installedOtherAliases["marketingclaw/plugin-sdk/codex-mcp-projection"]).toBeUndefined();
    expect(
      installedOtherAliases["marketingclaw/plugin-sdk/codex-native-task-runtime"],
    ).toBeUndefined();
    expect(shadowCodexAliases["marketingclaw/plugin-sdk/codex-mcp-projection"]).toBeUndefined();
    expect(
      shadowCodexAliases["marketingclaw/plugin-sdk/codex-native-task-runtime"],
    ).toBeUndefined();
  });
  ...
  it("aliases the SSRF internal helper only for bundled local IPC owner plugins", async () => {
    ...
    const entryBody = [
      'import { ssrfInternal } from "marketingclaw/plugin-sdk/ssrf-runtime-internal";',
    ...
    const sourceSubpaths = withEnv({ MARKETINGCLAW_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: sourceOllamaEntry,
      }),
    );
    const sourceBrowserSubpaths = withEnv({ MARKETINGCLAW_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: sourceBrowserEntry,
      }),
    );
    const privateQaOtherSubpaths = withEnv({ MARKETINGCLAW_ENABLE_PRIVATE_QA_CLI: "1" }, () =>
      listPluginSdkExportedSubpaths({
    ...
    const privateQaOtherAliases = withEnv(
      { MARKETINGCLAW_ENABLE_PRIVATE_QA_CLI: "1", NODE_ENV: undefined },
      () => buildPluginLoaderAliasMap(sourceOtherPluginEntry),
    );
    ...
    expect(
      fs.realpathSync(sourceAliases["marketingclaw/plugin-sdk/ssrf-runtime-internal"] ?? ""),
    ).toBe(fs.realpathSync(sourceSsrFInternalPath));
    expect(
      fs.realpathSync(sourceBrowserAliases["marketingclaw/plugin-sdk/ssrf-runtime-internal"] ?? ""),
    ).toBe(fs.realpathSync(sourceSsrFInternalPath));
    expect(
      fs.realpathSync(distAliases["marketingclaw/plugin-sdk/ssrf-runtime-internal"] ?? ""),
    ).toBe(fs.realpathSync(distSsrFInternalPath));
    expect(
      fs.realpathSync(distBrowserAliases["marketingclaw/plugin-sdk/ssrf-runtime-internal"] ?? ""),
    ).toBe(fs.realpathSync(distSsrFInternalPath));
    expect(
      fs.realpathSync(distRuntimeAliases["marketingclaw/plugin-sdk/ssrf-runtime-internal"] ?? ""),
    ).toBe(fs.realpathSync(distSsrFInternalPath));
    expect(
      fs.realpathSync(
        distRuntimeBrowserAliases["marketingclaw/plugin-sdk/ssrf-runtime-internal"] ?? "",
      ),
    ).toBe(fs.realpathSync(distSsrFInternalPath));
    expect(otherAliases["marketingclaw/plugin-sdk/ssrf-runtime-internal"]).toBeUndefined();
    expect(privateQaOtherAliases["marketingclaw/plugin-sdk/ssrf-runtime-internal"]).toBeUndefined();
    expect(installedAliases["marketingclaw/plugin-sdk/ssrf-runtime-internal"]).toBeUndefined();
  ...
  it("aliases workspace packages to source when dist artifacts are missing", () => {
    ...
    expect(fs.realpathSync(aliases["@marketingclaw/gateway-client"] ?? "")).toBe(
      fs.realpathSync(gatewayClient.srcFile),
    );
    expect(fs.realpathSync(aliases["@marketingclaw/gateway-client/timeouts"] ?? "")).toBe(
      fs.realpathSync(gatewayClientTimeouts.srcFile),
    );
    expect(fs.realpathSync(aliases["@marketingclaw/gateway-protocol"] ?? "")).toBe(
      fs.realpathSync(gatewayProtocol.srcFile),
    );
    expect(fs.realpathSync(aliases["@marketingclaw/gateway-protocol/schema"] ?? "")).toBe(
      fs.realpathSync(gatewayProtocolSchema.srcFile),
    );
    expect(fs.realpathSync(aliases["@marketingclaw/gateway-protocol/frame-guards"] ?? "")).toBe(
      fs.realpathSync(gatewayProtocolFrameGuards.srcFile),
    );
    expect(fs.realpathSync(aliases["@marketingclaw/markdown-core"] ?? "")).toBe(
      fs.realpathSync(markdownCore.srcFile),
    );
    expect(fs.realpathSync(aliases["@marketingclaw/markdown-core/tables"] ?? "")).toBe(
      fs.realpathSync(markdownCoreTables.srcFile),
    );
    expect(fs.realpathSync(aliases["@marketingclaw/media-generation-core"] ?? "")).toBe(
      fs.realpathSync(mediaGenerationCore.srcFile),
    );
    expect(fs.realpathSync(aliases["@marketingclaw/media-generation-core/model-ref"] ?? "")).toBe(
      fs.realpathSync(mediaGenerationModelRef.srcFile),
    );
    expect(fs.realpathSync(aliases["@marketingclaw/media-core"] ?? "")).toBe(
      fs.realpathSync(mediaCore.srcFile),
    );
    expect(fs.realpathSync(aliases["@marketingclaw/media-core/mime"] ?? "")).toBe(
      fs.realpathSync(mediaCoreMime.srcFile),
    );
    expect(fs.realpathSync(aliases["@marketingclaw/acp-core"] ?? "")).toBe(
      fs.realpathSync(acpCore.srcFile),
    );
    expect(fs.realpathSync(aliases["@marketingclaw/acp-core/runtime/types"] ?? "")).toBe(
      fs.realpathSync(acpCoreRuntimeTypes.srcFile),
    );
    expect(fs.realpathSync(aliases["@marketingclaw/normalization-core"] ?? "")).toBe(
      fs.realpathSync(normalizationCore.srcFile),
    );
    expect(
      fs.realpathSync(aliases["@marketingclaw/normalization-core/boolean-coercion"] ?? ""),
    ).toBe(fs.realpathSync(normalizationBooleanCoercion.srcFile));
    expect(fs.realpathSync(aliases["@marketingclaw/normalization-core/string-coerce"] ?? "")).toBe(
      fs.realpathSync(normalizationStringCoerce.srcFile),
    );
    expect(fs.realpathSync(aliases["@marketingclaw/terminal-core"] ?? "")).toBe(
      fs.realpathSync(terminalCore.srcFile),
    );
    expect(fs.realpathSync(aliases["@marketingclaw/terminal-core/theme"] ?? "")).toBe(
      fs.realpathSync(terminalCoreTheme.srcFile),
    );
    expect(fs.realpathSync(aliases["@marketingclaw/net-policy"] ?? "")).toBe(
      fs.realpathSync(netPolicy.srcFile),
    );
    expect(fs.realpathSync(aliases["@marketingclaw/net-policy/ip"] ?? "")).toBe(
      fs.realpathSync(netPolicyIp.srcFile),
    );
    expect(fs.realpathSync(aliases["@marketingclaw/net-policy/url-protocol"] ?? "")).toBe(
      fs.realpathSync(netPolicyUrlProtocol.srcFile),
    );
    expect(fs.realpathSync(aliases["@marketingclaw/model-catalog-core/provider-id"] ?? "")).toBe(
  ...
  it("aliases workspace package subpaths to dist when available", () => {
    ...
    expect(fs.realpathSync(aliases["@marketingclaw/gateway-client/readiness"] ?? "")).toBe(
      fs.realpathSync(gatewayClient.distFile),
    );
    expect(
      fs.realpathSync(aliases["@marketingclaw/gateway-protocol/connect-error-details"] ?? ""),
    ).toBe(fs.realpathSync(gatewayProtocol.distFile));
    expect(fs.realpathSync(aliases["@marketingclaw/gateway-protocol/frame-guards"] ?? "")).toBe(
      fs.realpathSync(gatewayProtocolFrameGuards.distFile),
    );
    expect(fs.realpathSync(aliases["@marketingclaw/markdown-core/render"] ?? "")).toBe(
      fs.realpathSync(markdownCore.distFile),
    );
    expect(fs.realpathSync(aliases["@marketingclaw/media-generation-core/catalog"] ?? "")).toBe(
      fs.realpathSync(mediaGenerationCore.distFile),
    );
    expect(fs.realpathSync(aliases["@marketingclaw/acp-core/normalize-text"] ?? "")).toBe(
      fs.realpathSync(acpCoreRootDistFile),
    );
    expect(fs.realpathSync(aliases["@marketingclaw/normalization-core/record-coerce"] ?? "")).toBe(
      fs.realpathSync(normalizationCoreRootDistFile),
    );
    expect(fs.realpathSync(aliases["@marketingclaw/terminal-core/links"] ?? "")).toBe(
    ...
    expect(
      fs.realpathSync(
        aliases["@marketingclaw/model-catalog-core/provider-model-id-normalize"] ?? "",
      ),
  ...
  it("derives acp-core aliases from packaged root dist when package metadata is absent", () => {
    ...
    const cwdWithoutMarketingClawPackage = makeTempDir();

    const aliases = withCwd(cwdWithoutMarketingClawPackage, () =>
      withEnv({ NODE_ENV: undefined }, () =>
        buildPluginLoaderAliasMap(sourcePluginEntry, undefined, undefined, "dist"),
      ),
    );

    expect(fs.realpathSync(aliases["@marketingclaw/acp-core/runtime/errors"] ?? "")).toBe(
  ...
  it("aliases bundled plugin package public surfaces for source plugin transforms", () => {
    ...
    expect(fs.realpathSync(aliases["@marketingclaw/slack/api.js"] ?? "")).toBe(
      fs.realpathSync(sourceApiPath),
    );
    expect(fs.realpathSync(aliases["@marketingclaw/slack/runtime-api.js"] ?? "")).toBe(
      fs.realpathSync(sourceRuntimeApiPath),
    );
    expect(aliases["@marketingclaw/slack/test-api.js"]).toBeUndefined();
    expect(aliases["@marketingclaw/slack/internal.js"]).toBeUndefined();
  });
  ...
  it("aliases bundled plugin package test surfaces only in private QA mode", () => {
    ...
    expect(fs.realpathSync(aliases["@marketingclaw/slack/test-api.js"] ?? "")).toBe(
  ...
  it("falls back to source plugin-sdk subpath aliases when dist chunks are stale", () => {
    ...
    expect(fs.realpathSync(distAliases["marketingclaw/plugin-sdk/provider-entry"] ?? "")).toBe(
  ...
  it("resolves plugin-sdk aliases for user-installed plugins via the running marketingclaw argv hint", () => {
    const {
      externalPluginEntry,
      externalPluginRoot,
      fixture,
  ...
  it("resolves plugin-sdk aliases for user-installed plugins via moduleUrl hint", () => {
    ...
    // This covers installations where argv1 does not resolve to the marketingclaw root
    // (e.g. single-binary distributions or custom process launchers).
    // Use marketingclaw.mjs which is created by createPluginSdkAliasFixture (bin+marker mode).
    // Use fixture.root as cwd so process.cwd() fallback also resolves to fixture, not the
    // real marketingclaw repo root in the test runner environment.
    const loaderModuleUrl = pathToFileURL(path.join(fixture.root, "marketingclaw.mjs")).href;
    ...
    // runner binary, inside the marketingclaw repo), which resolves before moduleUrl is checked.
    // An empty string is falsy so resolveTrustedMarketingClawRootFromArgvHint returns null,
  ...
  it.each([
    {
      name: "does not resolve plugin-sdk alias files from cwd fallback when package root is not an MarketingClaw root",
      fixture: () =>
        createPluginSdkAliasFixture({
          srcFile: "channel-runtime.ts",
          distFile: "channel-runtime.js",
  ...
  it("configures the plugin loader native-first boundary to prefer native dist modules", () => {
    ...
    expect(options.nativeModules).toEqual(["marketingclaw"]);
  ...
  it("preserves configured jiti native modules while adding marketingclaw", () => {
    const options = withEnv(
      { JITI_NATIVE_MODULES: JSON.stringify(["native-addon", "marketingclaw"]) },
      () => buildPluginLoaderJitiOptions({}),
    );

    expect(options.nativeModules).toEqual(["native-addon", "marketingclaw"]);
  });
  ...
  it("scopes plugin loader module config by plugin-sdk resolution", () => {
    ...
    expect(fs.realpathSync(auto.aliasMap["marketingclaw/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(sourceRootAlias),
    );
    expect(fs.realpathSync(dist.aliasMap["marketingclaw/plugin-sdk"] ?? "")).toBe(
...
`,
    ...
    const withAlias = createJiti(sourceLoaderBaseUrl, {
      ...buildPluginLoaderJitiOptions({
        "marketingclaw/plugin-sdk/channel-outbound": copiedChannelRuntimeShim,
        "@marketingclaw/plugin-sdk/channel-outbound": copiedChannelRuntimeShim,
      }),
      tryNative: false,
    });
  ...
  it("falls back to ancestor runtime candidates when package-root markers are unavailable", () => {
    const root = makeTempDir();
    const distFile = path.join(root, "dist", "plugins", "runtime", "index.js");
    const loaderCachePath = path.join(root, ".cache", "tsx", "marketingclaw-loader.js");
    mkdirSafeDir(path.dirname(distFile));
    ...
    expect(
      resolvePluginRuntimeModulePath({
        ...
        argv1: path.join(root, "bin", "marketingclaw"),
        pluginSdkResolution: "dist",
      }),
    ).toBe(distFile);
  });
  ...
  it("uses the default startup argv hint for runtime fallback when argv1 is omitted", () => {
    ...
    const loaderCachePath = path.join(loaderCacheRoot, "tsx", "marketingclaw-loader.js");
    ...
    process.argv[1] = path.join(root, "bin", "marketingclaw");
    try {
      expect(
        resolvePluginRuntimeModulePath({
          modulePath: loaderCachePath,
  ...
  it("resolves runtime fallback through symlinked startup argv", () => {
    const root = makeTempDir();
    const distFile = path.join(root, "dist", "plugins", "runtime", "index.js");
    const binFile = path.join(root, "bin", "marketingclaw");
    const shimRoot = makeTempDir();
    const shimFile = path.join(shimRoot, "bin", "marketingclaw");
    const loaderCachePath = path.join(makeTempDir(), "tsx", "marketingclaw-loader.js");
    mkdirSafeDir(path.dirname(distFile));
  ...
  it("resolves runtime fallback through npm .bin startup argv", () => {
    const root = makeTempDir();
    const packageRoot = path.join(root, "node_modules", "marketingclaw");
    ...
    const binFile = path.join(root, "node_modules", ".bin", "marketingclaw");
    const loaderCachePath = path.join(makeTempDir(), "tsx", "marketingclaw-loader.js");
    mkdirSafeDir(path.dirname(distFile));
...
describe("buildPluginLoaderAliasMap memoization", () => {
  ...
  it("returns different references for different modulePath inputs", () => {
    ...
    expect(aliasA["marketingclaw/plugin-sdk"]).not.toBe(aliasB["marketingclaw/plugin-sdk"]);
  });
  ...
  it("returns different references when an explicit dev source root differs", () => {
    ...
    expect(fs.realpathSync(stableAliases["marketingclaw/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(stableRootAlias),
    );
    expect(fs.realpathSync(devAliases["marketingclaw/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(devRootAlias),
    );
  });
  ...
  it("does not reuse a public alias map after private qa aliases are enabled", () => {
    ...
    const publicAliases = withEnv({ MARKETINGCLAW_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      buildPluginLoaderAliasMap(entry),
    );
    const privateAliases = withEnv({ MARKETINGCLAW_ENABLE_PRIVATE_QA_CLI: "1" }, () =>
      buildPluginLoaderAliasMap(entry),
    );

    expect(publicAliases).not.toBe(privateAliases);
    expect(publicAliases["marketingclaw/plugin-sdk/qa-runtime"]).toBeUndefined();
    expect(fs.realpathSync(privateAliases["marketingclaw/plugin-sdk/qa-runtime"] ?? "")).toBe(
      fs.realpathSync(sourceQaRuntimePath),
    );
  });
  ...
  it("does not reuse a development alias map in production mode", () => {
    ...
    expect(fs.realpathSync(developmentAliases["marketingclaw/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(sourceRootAlias),
    );
    expect(fs.realpathSync(productionAliases["marketingclaw/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(distRootAlias),
    );
  });
...
describe("buildPluginLoaderJitiOptions", () => {
  it("scopes jiti fs cache by MarketingClaw package version and install metadata", () => {
    const root = createTrustedMarketingClawPackageFixture("1.2.3-beta.4");
    ...
    expect(fsCache).toContain(
      path.join(tmpDir, "jiti", "marketingclaw", "1.2.3-beta.4") + path.sep,
    );
  ...
  it("preserves jiti's tmpdir guard when TMPDIR resolves to cwd", () => {
    ...
    expect(guardedFsCache).toContain(path.join("jiti", "marketingclaw", "1.2.3-beta.4") + path.sep);
    expect(guardedFsCache.startsWith(path.join(root, "jiti") + path.sep)).toBe(false);
    expect(respectedFsCache).toContain(
      path.join(root, "jiti", "marketingclaw", "1.2.3-beta.4") + path.sep,
    );
  });

  it("adds the versioned fs cache directory to plugin loader jiti options", () => {
    const root = createTrustedMarketingClawPackageFixture("2.0.0");
    const tmpDir = path.join(root, "tmp");

    const options = withEnv({ TMPDIR: tmpDir }, () =>
      buildPluginLoaderJitiOptions(
        { "marketingclaw/plugin-sdk": path.join(root, "dist", "plugin-sdk", "root-alias.cjs") },
        { modulePath: path.join(root, "dist", "plugins", "loader.js") },
      ),
    );

    expect(options.fsCache).toContain(path.join(tmpDir, "jiti", "marketingclaw", "2.0.0"));
  });
  ...
  it("pre-normalizes and marks alias maps for source transforms", () => {
    ...
    const aliasMap = {
      "marketingclaw/plugin-sdk/core": "/repo/src/plugin-sdk/core.ts",
      "marketingclaw/plugin-sdk": "/repo/src/plugin-sdk/root-alias.cjs",
      "@marketingclaw/plugin-sdk": "/repo/src/plugin-sdk/root-alias.cjs",
    };
