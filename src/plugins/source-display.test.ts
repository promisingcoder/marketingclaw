...
const PLUGIN_SOURCE_ROOTS = {
  ...
  global: path.resolve(path.sep, "Users", "x", ".marketingclaw", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".marketingclaw", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("ignores untrusted explicit env override for the stock source root", () => {
    const homeDir = path.resolve(path.sep, "tmp", "marketingclaw-home");
    const rawEnv = {
    ...
    expectResolvedSourceRoots({
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".marketingclaw", "extensions"),
      },
    });
  });
});
