...
const excludeOptionalEnv = { MARKETINGCLAW_INCLUDE_OPTIONAL_BUNDLED: "0" } as const;
...
function createPlugin(
  ...
  params: {
    ...
    packageMarketingClaw?: Record<string, unknown>;
  },
) {
  const pluginDir = path.join(repoRoot, "extensions", params.id);
  fs.mkdirSync(pluginDir, { recursive: true });
  writeJson(path.join(pluginDir, "marketingclaw.plugin.json"), {
    id: params.id,
  ...
  writeJson(path.join(pluginDir, "package.json"), {
    ...
    ...(params.packageMarketingClaw ? { marketingclaw: params.packageMarketingClaw } : {}),
  });
  return pluginDir;
}
...
function readBundledManifest(repoRoot: string, pluginId: string): Record<string, unknown> {
  return JSON.parse(
    fs.readFileSync(
      path.join(repoRoot, "dist", "extensions", pluginId, "marketingclaw.plugin.json"),
      "utf8",
    ),
...
function readBundledPackageJson(repoRoot: string, pluginId: string) {
  ...
  ) as { marketingclaw?: { extensions?: string[] } };
}
...
function createTlonSkillPlugin(repoRoot: string, skillPath = "node_modules/@tloncorp/tlon-skill") {
  return createPlugin(repoRoot, {
    id: "tlon",
    packageName: "@marketingclaw/tlon",
    manifest: { skills: [skillPath] },
    packageMarketingClaw: { extensions: ["./index.ts"] },
  });
}
...
describe("copyBundledPluginMetadata", () => {
  it("copies plugin manifests, package metadata, and local skill directories", () => {
    const repoRoot = makeRepoRoot("marketingclaw-bundled-plugin-meta-");
    const pluginDir = createPlugin(repoRoot, {
      id: "acpx",
      packageName: "@marketingclaw/acpx",
    ...
    expect(
      fs.existsSync(path.join(repoRoot, "dist", "extensions", "acpx", "marketingclaw.plugin.json")),
    ).toBe(true);
    expect(
      fs.readFileSync(
    ...
    expect(packageJson.marketingclaw?.extensions).toEqual(["./index.js"]);
  });

  it("copies generated bundled channel config schemas into dist manifests", () => {
    const repoRoot = makeRepoRoot("marketingclaw-bundled-channel-config-meta-");
    createPlugin(repoRoot, {
      id: "telegram",
      packageName: "@marketingclaw/telegram",
      manifest: {
        channels: ["telegram"],
        channelConfigs: {
          telegram: {
  ...
  it("relocates node_modules-backed skill paths into bundled-skills and rewrites the manifest", () => {
    const repoRoot = makeRepoRoot("marketingclaw-bundled-plugin-node-modules-");
  ...
  it("falls back to repo-root hoisted node_modules skill paths", () => {
    const repoRoot = makeRepoRoot("marketingclaw-bundled-plugin-hoisted-skill-");
    const pluginDir = createTlonSkillPlugin(repoRoot);
  ...
  it("omits missing declared skill paths and removes stale generated outputs", () => {
    const repoRoot = makeRepoRoot("marketingclaw-bundled-plugin-missing-skill-");
  ...
  it("retries transient skill copy races from concurrent runtime postbuilds", () => {
    const repoRoot = makeRepoRoot("marketingclaw-bundled-plugin-retry-");
    const pluginDir = createPlugin(repoRoot, {
      id: "diffs",
  ...
  it("removes generated outputs for plugins no longer present in source", () => {
    const repoRoot = makeRepoRoot("marketingclaw-bundled-plugin-removed-");
    const staleBundledSkillDir = path.join(
      repoRoot,
      "dist",
      "extensions",
    ...
    writeJson(
      path.join(repoRoot, "dist", "extensions", "removed-plugin", "marketingclaw.plugin.json"),
      {
        id: "removed-plugin",
        configSchema: { type: "object" },
  ...
  it("removes stale dist outputs when a source extension directory no longer has a manifest", () => {
    const repoRoot = makeRepoRoot("marketingclaw-bundled-plugin-manifestless-source-");
    ...
    writeJson(path.join(staleDistDir, "marketingclaw.plugin.json"), {
  ...
  it("removes non-packaged private QA plugin metadata unless private QA build is enabled", () => {
    const repoRoot = makeRepoRoot("marketingclaw-private-qa-metadata-");
    createPlugin(repoRoot, {
      id: "qa-lab",
      packageName: "@marketingclaw/qa-lab",
      packageMarketingClaw: { extensions: ["./index.ts"] },
    });
    ...
    copyBundledPluginMetadataWithEnv({
      repoRoot,
      env: { MARKETINGCLAW_BUILD_PRIVATE_QA: "1" } as NodeJS.ProcessEnv,
    });

    expect(fs.existsSync(path.join(staleDistDir, "marketingclaw.plugin.json"))).toBe(true);
  ...
  it.each([
    ...
    },
    {
      ...
      packageMarketingClaw: {
        extensions: ["./index.ts"],
        install: { npmSpec: "@marketingclaw/whatsapp" },
      },
      env: {},
      expectedExists: false,
    },
  ] as const)("$name", ({ pluginId, packageName, packageMarketingClaw, env, expectedExists }) => {
    const repoRoot = makeRepoRoot(`marketingclaw-bundled-plugin-${pluginId}-`);
    createPlugin(repoRoot, {
      id: pluginId,
      packageName,
      packageMarketingClaw,
  ...
  it("removes build-excluded bundled plugin metadata", () => {
    const repoRoot = makeRepoRoot("marketingclaw-bundled-plugin-excluded-meta-");
    createPlugin(repoRoot, {
      id: "qqbot",
      packageName: "@marketingclaw/qqbot",
      packageMarketingClaw: {
        extensions: ["./index.ts"],
  ...
  it("preserves manifest-less runtime support package outputs and copies package metadata", () => {
    const repoRoot = makeRepoRoot("marketingclaw-bundled-runtime-support-");
    const pluginDir = path.join(repoRoot, "extensions", "image-generation-core");
    fs.mkdirSync(pluginDir, { recursive: true });
    writeJson(path.join(pluginDir, "package.json"), {
      name: "@marketingclaw/image-generation-core",
      version: "0.0.1",
      private: true,
      type: "module",
    });
