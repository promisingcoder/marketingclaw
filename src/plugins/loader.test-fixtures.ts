...
>["config"];
export type PluginRegistry = ReturnType<typeof loadMarketingClawPlugins>;
...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "marketingclaw-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.MARKETINGCLAW_BUNDLED_PLUGINS_DIR;
const prevDisableBundledPlugins = process.env.MARKETINGCLAW_DISABLE_BUNDLED_PLUGINS;
...
export function loadBundleFixture(params: {
  ...
  const bundleRoot = path.join(workspaceDir, ".marketingclaw", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ MARKETINGCLAW_STATE_DIR: stateDir, ...params.env }, () =>
...
export function resetPluginLoaderTestStateForTest() {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.MARKETINGCLAW_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
  if (prevDisableBundledPlugins === undefined) {
    delete process.env.MARKETINGCLAW_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.MARKETINGCLAW_DISABLE_BUNDLED_PLUGINS = prevDisableBundledPlugins;
  }
}
