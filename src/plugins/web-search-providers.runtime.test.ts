...
let loadMarketingClawPluginsMock: ReturnType<typeof vi.fn>;
...
function createWebSearchEnv(overrides?: Partial<NodeJS.ProcessEnv>) {
  return {
    MARKETINGCLAW_HOME: "/tmp/marketingclaw-home",
    ...overrides,
    ...
    plugins: [
      {
        ...
        manifestPath: "/tmp/brave/marketingclaw.plugin.json",
        channels: [],
        providers: [],
        cliBackends: [],
        syntheticAuthRefs: [],
      ...
      },
      {
        ...
        manifestPath: "/tmp/noise/marketingclaw.plugin.json",
        channels: [],
        providers: [],
        cliBackends: [],
        syntheticAuthRefs: [],
...
function expectLoaderCallCount(count: number) {
  expect(loadMarketingClawPluginsMock).toHaveBeenCalledTimes(count);
}
...
function expectScopedWebSearchCandidates(pluginIds: readonly string[]) {
  ...
  expect(
    requireLastCallFirstArg(loadMarketingClawPluginsMock, "loadMarketingClawPlugins").onlyPluginIds,
  ).toEqual([...pluginIds]);
}
...
describe("resolvePluginWebSearchProviders", () => {
  ...
  beforeEach(() => {
    ...
    loadMarketingClawPluginsMock = vi
      .spyOn(loaderModule, "loadMarketingClawPlugins")
  ...
  it("resolves current config contents when config changes in place", () => {
    ...
    const env = createWebSearchEnv({ MARKETINGCLAW_HOME: "/tmp/marketingclaw-home-a" });
  ...
  it("resolves current env contents when env changes in place", () => {
    ...
    expectSnapshotLoaderCalls({
      ...
      env,
      mutate: () => {
        env.MARKETINGCLAW_HOME = "/tmp/marketingclaw-home-b";
      },
      expectedLoaderCalls: 2,
    });
  });
  ...
  it("does not reuse snapshot provider loads across host Vitest env changes", () => {
    ...
    expect(loadMarketingClawPluginsMock).toHaveBeenCalledTimes(2);
  });
