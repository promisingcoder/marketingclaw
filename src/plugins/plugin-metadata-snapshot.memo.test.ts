...
function tempStateDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-metadata-memo-"));
  tempDirs.push(dir);
  return dir;
}
...
function writePersistedIndex(params: {
  ...
  const manifestPath = params.manifestPath ?? path.join(pluginDir, "marketingclaw.plugin.json");
...
function writeRecoverableNpmPlugin(params: {
  ...
  writeJson(path.join(packageDir, "marketingclaw.plugin.json"), { id: params.pluginId });
}
...
function makeIndex(
  ...
  const manifestPath = options.manifestPath ?? path.join(rootDir, "marketingclaw.plugin.json");
  return {
    version: 1,
    hostContractVersion: "test",
    compatRegistryVersion: "test",
...
function makeManifestRegistry(pluginId = "demo"): PluginManifestRegistry {
  const plugin: PluginManifestRecord = {
    ...
    manifestPath: `/plugins/${pluginId}/marketingclaw.plugin.json`,
    origin: "global",
  };
...
describe("loadPluginMetadataSnapshot process memo", () => {
  ...
  it("does not emit metadata scan spans for hot memo hits", () => {
    ...
    const env = {
      ...
      MARKETINGCLAW_DIAGNOSTICS_TIMELINE_PATH: timelinePath,
    };
    touchPersistedIndex(stateDir);
    loadPluginRegistrySnapshotWithMetadata.mockReturnValue({
      source: "persisted",
  ...
  it("reuses the expanded freshness fingerprint on hot cache hits", () => {
    const stateDir = tempStateDir();
    const manifestPath = path.join(stateDir, "extensions", "demo", "marketingclaw.plugin.json");
  ...
  it.each([
    ["manifest", "marketingclaw.plugin.json", "manifestPath"],
  ...
  it("does not fingerprint persisted plugin paths outside the plugin root", () => {
    const stateDir = tempStateDir();
    const outsideManifestPath = path.join(stateDir, "outside", "marketingclaw.plugin.json");
