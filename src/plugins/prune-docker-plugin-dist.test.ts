...
function writeDistPluginFile(repoRoot: string, root: "dist" | "dist-runtime", pluginId: string) {
  ...
  fs.writeFileSync(path.join(pluginDir, "marketingclaw.plugin.json"), "{}\n", "utf8");
}
...
describe("pruneDockerPluginDist", () => {
  ...
  it("removes package-excluded plugin runtime artifacts unless Docker explicitly opts it in", () => {
    const repoRoot = makeRepoRoot("marketingclaw-docker-plugin-dist-");
    ...
    const removed = pruneDockerPluginDist({
      repoRoot,
      env: { MARKETINGCLAW_EXTENSIONS: "diagnostics-otel" } as NodeJS.ProcessEnv,
    });
  ...
  it("honors custom bundled plugin source roots when pruning Docker runtime importers", () => {
    const repoRoot = makeRepoRoot("marketingclaw-docker-plugin-source-");
    writeJsonFile(path.join(repoRoot, "package.json"), {
    ...
    const removed = pruneDockerPluginDist({
      repoRoot,
      env: {
        MARKETINGCLAW_BUNDLED_PLUGIN_DIR: "plugins",
  ...
  it("removes node_modules dependency closure that only omitted Docker plugins need", () => {
    const repoRoot = makeRepoRoot("marketingclaw-docker-plugin-node-modules-");
    writeJsonFile(path.join(repoRoot, "package.json"), {
    ...
    writeNodePackage(repoRoot, "@marketingclaw/acpx");
    writeNodePackage(repoRoot, "@marketingclaw/codex");
    ...
    const removed = pruneDockerPluginDist({
      repoRoot,
      env: { MARKETINGCLAW_EXTENSIONS: "codex" } as NodeJS.ProcessEnv,
    });

    expect(removed).toEqual([
      "node_modules/@marketingclaw/acpx",
