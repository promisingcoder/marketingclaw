...
const mocks = vi.hoisted(() => ({
  ...
  loadMarketingClawPluginCliRegistry: vi.fn(),
  loadMarketingClawPlugins: vi.fn(),
...
vi.mock("./loader.js", () => ({
  loadMarketingClawPluginCliRegistry: (...args: unknown[]) =>
    mocks.loadMarketingClawPluginCliRegistry(...args),
  loadMarketingClawPlugins: (...args: unknown[]) => mocks.loadMarketingClawPlugins(...args),
}));
...
function expectAutoEnabledCliLoad(params: {
  rawConfig: MarketingClawConfig;
  autoEnabledConfig: MarketingClawConfig;
  ...
  const loadOptions = getMockCallObject(mocks.loadMarketingClawPlugins);
...
describe("registerPluginCliCommands", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadMarketingClawPluginCliRegistry.mockReset();
    mocks.loadMarketingClawPluginCliRegistry.mockResolvedValue(createCliRegistry());
    mocks.loadMarketingClawPlugins.mockReset();
    mocks.loadMarketingClawPlugins.mockReturnValue({
      ...createCliRegistry(),
      diagnostics: [],
    });
    ...
    mocks.loadConfig.mockReturnValue({} as MarketingClawConfig);
  ...
  it("skips plugin CLI registrars when commands already exist", async () => {
    ...
    await registerPluginCliCommands(program, {} as MarketingClawConfig);
  ...
  it("forwards an explicit env to plugin loading", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as MarketingClawConfig, env);
  ...
  it("injects gateway-backed node runtime into plugin CLI commands", async () => {
    await registerPluginCliCommands(createProgram(), {} as MarketingClawConfig);

    const loadOptions = getMockCallObject(mocks.loadMarketingClawPlugins) as {
  ...
  it("reuses loaded plugin CLI entries on repeat calls for the same program", async () => {
    ...
    expect(mocks.loadMarketingClawPlugins).toHaveBeenCalledTimes(1);
  });

  it("reloads plugin CLI entries when the requested primary command changes", async () => {
    const program = createProgram();

    await registerPluginCliCommands(program, {} as MarketingClawConfig, undefined, undefined, {
      primary: "memory",
    });
    await registerPluginCliCommands(program, {} as MarketingClawConfig);

    expect(mocks.loadMarketingClawPlugins).toHaveBeenCalledTimes(2);
  });

  it("reloads plugin CLI entries when config or environment identity changes", async () => {
    const program = createProgram();
    const configA = {} as MarketingClawConfig;
    const configB = { plugins: {} } as MarketingClawConfig;
    const envA = { MARKETINGCLAW_HOME: "/tmp/a" } as NodeJS.ProcessEnv;
    const envB = { MARKETINGCLAW_HOME: "/tmp/b" } as NodeJS.ProcessEnv;
    ...
    expect(mocks.loadMarketingClawPlugins).toHaveBeenCalledTimes(3);
  });
  ...
  it("loads root-help descriptors through the dedicated non-activating CLI collector", async () => {
    ...
    mocks.loadMarketingClawPluginCliRegistry.mockResolvedValue({
      cliRegistrars: [
        {
          pluginId: "matrix",
          register: vi.fn(),
    ...
    const registryOptions = getMockCallObject(mocks.loadMarketingClawPluginCliRegistry);
  ...
  it("keeps root-help descriptor load failures quiet", async () => {
    ...
    mocks.loadMarketingClawPluginCliRegistry.mockImplementationOnce(
  ...
  it("keeps runtime CLI command registration on the full plugin loader for legacy channel plugins", async () => {
    ...
    expect(mocks.loadMarketingClawPluginCliRegistry).not.toHaveBeenCalled();
  });
  ...
  it("scopes full CLI loading through CLI metadata when manifest planning finds no plugin match", async () => {
    ...
    expect(mocks.loadMarketingClawPluginCliRegistry).toHaveBeenCalled();
    const loadOptions = getMockCallObject(mocks.loadMarketingClawPlugins);
    expect(loadOptions.onlyPluginIds).toEqual(["memory-core"]);
  });
  ...
  it("scopes nested CLI loading through CLI metadata parent paths", async () => {
    ...
    mocks.loadMarketingClawPluginCliRegistry.mockResolvedValue(nestedRegistry);
    mocks.loadMarketingClawPlugins.mockReturnValue(nestedRegistry);
    const program = createProgram("nodes");
    program.exitOverride();
  ...
  it("skips full plugin runtime loading when no metadata owns the requested primary", async () => {
    ...
    expect(mocks.loadMarketingClawPluginCliRegistry).toHaveBeenCalled();
    expect(mocks.loadMarketingClawPlugins).not.toHaveBeenCalled();
  ...
  it("loads validated plugin CLI config when the snapshot is valid", async () => {
    const loadedConfig = { plugins: { enabled: true } } as MarketingClawConfig;
    mocks.readConfigFileSnapshot.mockResolvedValueOnce({
      valid: true,
      config: loadedConfig,
    });
