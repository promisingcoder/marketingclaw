...
export function resolvePluginVersionDriftUpdateCommand(entry: PluginVersionDriftEntry): string {
  ...
  if (exactNpmPackageName) {
    ...
    if (parseRegistryNpmSpec(exactNpmTarget)?.selectorKind === "exact-version") {
      return `marketingclaw plugins update ${exactNpmTarget}`;
    }
  }
  return `marketingclaw plugins update ${entry.pluginId}`;
}
...
function isPluginEnabled(config: MarketingClawConfig | undefined, pluginId: string): boolean {
 ...
 *   `version` field of the installed marketingclaw package.json).
 ...
 * @param params.config The merged daemon-side MarketingClawConfig (optional).
