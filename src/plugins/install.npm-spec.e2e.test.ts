...
async function makeTempDir(label: string): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), `marketingclaw-${label}-`));
  tempDirs.push(dir);
  return dir;
}

function configWithInstalledPackageTreeBlockPolicy(): MarketingClawConfig {
  return {
    security: {
      installPolicy: {
        enabled: true,
...
async function packPlugin(params: {
  ...
  await fs.writeFile(
    path.join(packageDir, "marketingclaw.plugin.json"),
    `${JSON.stringify(
      {
        id: params.pluginId,
            ...
            versions: Object.fromEntries(
              [...pkg.versionsByVersion.entries()].map(([version, entry]) => [
                version,
                {
                  name: pkg.packageName,
                  version,
                  ...(entry.marketingclaw ? { marketingclaw: entry.marketingclaw } : {}),
...
describe("installPluginFromNpmSpec e2e", () => {
  it("installs the newest compatible stable package when npm latest requires a newer plugin API", async () => {
    ...
    const versions = [
      await packPlugin({
        ...
        version: "2026.5.26",
        rootDir,
        marketingclaw: compatibleMarketingClaw,
      }),
      await packPlugin({
        packageName,
        pluginId: packageName,
        version: "2026.5.27",
        rootDir,
        marketingclaw: incompatibleMarketingClaw,
      }),
    ];
    const registry = await startStaticRegistry([{ packageName, latest: "2026.5.27", versions }]);
    process.env.NPM_CONFIG_REGISTRY = registry;
    process.env.npm_config_registry = registry;
    const previousHostVersion = process.env.MARKETINGCLAW_COMPATIBILITY_HOST_VERSION;
    process.env.MARKETINGCLAW_COMPATIBILITY_HOST_VERSION = "2026.5.10-beta.1";
    ...
    try {
      ...
      if (previousHostVersion === undefined) {
        ...
        process.env.MARKETINGCLAW_COMPATIBILITY_HOST_VERSION = previousHostVersion;
      }
    }
  });

  it("scrubs root marketingclaw materialized by required npm peers", async () => {
    ...
    const marketingClawVersions = [
      await packPlugin({
        packageName: "marketingclaw",
        pluginId: "registry-marketingclaw-copy",
        version: "2026.0.0",
        rootDir,
      }),
    ];
    const registry = await startStaticRegistry([
      ...
      { packageName: "marketingclaw", latest: "2026.0.0", versions: marketingClawVersions },
    ]);
    process.env.NPM_CONFIG_REGISTRY = registry;
    process.env.npm_config_registry = registry;
    ...
    const rawMarketingClawLockEntry = rawLock.packages?.["node_modules/marketingclaw"] as
      | { peer?: unknown; version?: unknown }
      | undefined;
    expect(rawMarketingClawLockEntry?.peer).toBe(true);
    expect(rawMarketingClawLockEntry?.version).toBe("2026.0.0");
    ...
    expect(lock.packages?.["node_modules/marketingclaw"]).toBeUndefined();
    await expect(
      fs.lstat(path.join(projectRoot, "node_modules", "marketingclaw")),
    ).rejects.toHaveProperty("code", "ENOENT");
    await expect(
      fs
        .lstat(path.join(result.targetDir, "node_modules", "marketingclaw"))
  ...
  it("plans peers from installed optional dependencies", async () => {
    ...
    const rootManifest = JSON.parse(
      ...
      dependencies?: Record<string, string>;
      marketingclaw?: { managedPeerDependencies?: string[] };
    };
    expect(["1.0.0", "^1.0.0"]).toContain(rootManifest.dependencies?.[runtimePeer]);
    expect(rootManifest.marketingclaw?.managedPeerDependencies ?? []).toContain(runtimePeer);
  });
  ...
  it("leaves legacy flat-root peer dependencies alone during isolated later installs", async () => {
    ...
    expect(rootManifest.marketingclaw?.managedPeerDependencies ?? []).not.toContain(runtimePeer);
  });
  ...
  it("falls back to the legacy npm peer mode inside the plugin project when npm cannot plan third-party peers", async () => {
    ...
    expect(rootManifest.marketingclaw?.managedPeerDependencies ?? []).not.toContain(missingPeer);
    await expect(
      fs.lstat(path.join(projectRoot, "node_modules", blockedPlugin, "package.json")),
    ).resolves.toBeTruthy();
  });
  ...
  it("does not take ownership of an existing root dependency observed as a peer", async () => {
    ...
    expect(rootManifest.dependencies?.[blockedPlugin]).toBeUndefined();
    expect(rootManifest.dependencies?.[runtimePeer]).toBeUndefined();
    expect(rootManifest.marketingclaw?.managedPeerDependencies ?? []).not.toContain(
  ...
  it("scrubs host peers inside each isolated npm project", async () => {
    ...
    const registry = await startStaticRegistry([
      {
        ...
        versions: [
          await packPlugin({
            packageName: codexName,
            peerDependencies: { marketingclaw: ">=2026.5.5-beta.2" },
      ...
      },
      {
        ...
        versions: [
          await packPlugin({
            packageName: opikName,
            peerDependencies: { marketingclaw: ">=2026.3.2" },
            peerDependenciesMeta: {},
            pluginId: opikName,
            version: "1.0.0",
            rootDir,
    ...
    await expect(
      fs
        .lstat(path.join(first.targetDir, "node_modules", "marketingclaw"))
        .then((stat) => stat.isSymbolicLink()),
    ).resolves.toBe(true);
    await expect(
      fs
        .lstat(path.join(second.targetDir, "node_modules", "marketingclaw"))
        .then((stat) => stat.isSymbolicLink()),
    ).resolves.toBe(true);
  });

  it("keeps an earlier isolated marketingclaw peer link after later plugin installs", async () => {
    ...
    const peerLink = path.join(first.targetDir, "node_modules", "marketingclaw");
    ...
    expect(manifest.dependencies?.marketingclaw).toBeUndefined();
    const lock = JSON.parse(
