...
describe("plugin lifecycle trace", () => {
  const originalTraceEnv = process.env.MARKETINGCLAW_PLUGIN_LIFECYCLE_TRACE;
  ...
  afterEach(() => {
    if (originalTraceEnv === undefined) {
      delete process.env.MARKETINGCLAW_PLUGIN_LIFECYCLE_TRACE;
    } else {
      process.env.MARKETINGCLAW_PLUGIN_LIFECYCLE_TRACE = originalTraceEnv;
    }
  ...
  it("emits a successful sync phase with details when enabled", () => {
    process.env.MARKETINGCLAW_PLUGIN_LIFECYCLE_TRACE = "1";
  ...
  it("emits failed sync phases before rethrowing", () => {
    process.env.MARKETINGCLAW_PLUGIN_LIFECYCLE_TRACE = "true";
  ...
  it("emits failed async phases before rejecting", async () => {
    process.env.MARKETINGCLAW_PLUGIN_LIFECYCLE_TRACE = "yes";
