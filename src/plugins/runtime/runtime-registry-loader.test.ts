...
function loadOptions(index = 0) {
  return requireRecord(
    mocks.loadMarketingClawPlugins.mock.calls[index]?.[0],
    `load options ${index}`,
  );
}
...
describe("ensurePluginRegistryLoaded", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadMarketingClawPlugins.mockReturnValue(createEmptyPluginRegistry());
    mocks.resolveRuntimePluginRegistry.mockImplementation(
      (...args: Parameters<typeof mocks.loadMarketingClawPlugins>) =>
        mocks.loadMarketingClawPlugins(...args),
    );
  ...
  it("uses the shared runtime load context for configured-channel loads", () => {
    ...
    const env = { HOME: "/tmp/marketingclaw-home" } as NodeJS.ProcessEnv;
  ...
  it("does not reuse non-empty all-scope registries without loader compatibility", () => {
    ...
    mocks.loadMarketingClawPlugins.mockClear();
