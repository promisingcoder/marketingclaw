...
function npmRecord(
  ...
  const resolvedName = overrides.resolvedName ?? "@marketingclaw/whatsapp";
  return {
    source: "npm",
...
describe("detectPluginVersionDrift", () => {
  it("returns empty drifts when all externalized plugins match the gateway", () => {
    const result = detectPluginVersionDrift({
      ...
      installRecords: {
        ...
        discord: npmRecord("2026.5.4", { resolvedName: "@marketingclaw/discord" }),
      },
    });
    ...
    const result = detectPluginVersionDrift({
      ...
      installRecords: {
        whatsapp: npmRecord("2026.5.3", {
          ...
          spec: "@marketingclaw/whatsapp@2026.5.3",
        }),
        discord: npmRecord("2026.5.4", { resolvedName: "@marketingclaw/discord" }),
      },
    });
  ...
  it("treats a build-qualifier suffix on either side as matching (2026.5.4-1 ≈ 2026.5.4)", () => {
    const result = detectPluginVersionDrift({
      ...
      installRecords: {
        ...
        discord: npmRecord("2026.5.4-1", { resolvedName: "@marketingclaw/discord" }),
      },
    });
  ...
  it("ignores official catalog installs pinned to independent package versions", () => {
    const result = detectPluginVersionDrift({
      gatewayVersion: "2026.5.4",
      installRecords: {
        "marketingclaw-plugin-yuanbao": npmRecord("2.13.1", {
          resolvedName: "marketingclaw-plugin-yuanbao",
          spec: "marketingclaw-plugin-yuanbao@2.13.1",
        }),
      },
    });
  ...
  it("ignores exact catalog pins even when the pin matches the gateway version", () => {
    const result = detectPluginVersionDrift({
      ...
      installRecords: {
        "wecom-marketingclaw-plugin": npmRecord("2026.5.6", {
          resolvedName: "@wecom/wecom-marketingclaw-plugin",
          spec: "@wecom/wecom-marketingclaw-plugin@2026.5.6",
        }),
      },
    });
    ...
    const result = detectPluginVersionDrift({
      ...
      installRecords: {
        ...
        archive: {
          ...
          spec: "@marketingclaw/whatsapp@archive",
        },
        local: {
          source: "path",
          resolvedName: "@marketingclaw/whatsapp",
  ...
  it("falls back to the install record's `version` field when `resolvedVersion` is absent", () => {
    const result = detectPluginVersionDrift({
      ...
      installRecords: {
        whatsapp: {
          source: "npm",
          spec: "@marketingclaw/whatsapp@latest",
          resolvedName: "@marketingclaw/whatsapp",
          version: "2026.5.3",
        },
      },
  ...
  it("skips plugins with no recorded version (cannot detect drift)", () => {
    const result = detectPluginVersionDrift({
      gatewayVersion: "2026.5.4",
      installRecords: {
        whatsapp: { source: "npm", spec: "@marketingclaw/whatsapp@latest" },
      },
    });
  ...
  it("skips plugins that are explicitly disabled in config", () => {
    ...
    const result = detectPluginVersionDrift({
      ...
      installRecords: {
        whatsapp: npmRecord("2026.5.3"),
        discord: npmRecord("2026.5.3", { resolvedName: "@marketingclaw/discord" }),
      },
      config,
    });
  ...
  it("includes plugins with no entry in config (default-enabled)", () => {
    const config: MarketingClawConfig = { plugins: { entries: {} } } as MarketingClawConfig;
    const result = detectPluginVersionDrift({
      gatewayVersion: "2026.5.4",
      installRecords: {
        whatsapp: npmRecord("2026.5.3"),
  ...
  it("returns drifts sorted by pluginId for deterministic output", () => {
    const result = detectPluginVersionDrift({
      ...
      installRecords: {
        whatsapp: npmRecord("2026.5.3"),
        discord: npmRecord("2026.5.3", { resolvedName: "@marketingclaw/discord" }),
        matrix: npmRecord("2026.5.3", { resolvedName: "@marketingclaw/matrix" }),
      },
    });
...
describe("resolvePluginVersionDriftUpdateCommand", () => {
  it("uses an exact npm package target when the drifted install is pinned", () => {
    ...
    ).toBe("marketingclaw plugins update @marketingclaw/brave-plugin@2026.6.10-beta.1");
  });
  ...
  it("keeps plugin-id updates for floating npm install records", () => {
    ...
    ).toBe("marketingclaw plugins update brave");
  });
