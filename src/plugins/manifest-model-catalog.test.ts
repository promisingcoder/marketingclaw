...
function makePluginDir() {
  return makeTrackedTempDir("marketingclaw-manifest-model-catalog", tempDirs);
}

function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "marketingclaw.plugin.json"), JSON.stringify(manifest), "utf8");
}
