...
vi.mock("./loader.js", () => ({
  ...
  loadMarketingClawPlugins: mocks.loadMarketingClawPlugins,
...
describe("web-provider-runtime-shared", () => {
  ...
  it("caches setup web provider plugin loads by default", () => {
    ...
    const mapRegistryProviders = vi.fn(() => ["provider"]);
    mocks.loadMarketingClawPlugins.mockReturnValue(loadedRegistry as never);
    ...
    expect(providers).toEqual(["provider"]);
    expect(mockArg(mocks.loadMarketingClawPlugins).cache).toBe(true);
    expect(mockArg(mocks.loadMarketingClawPlugins).onlyPluginIds).toEqual(["brave"]);
  });
  ...
  it("falls back to plugin loading when bundled runtime artifacts do not cover the scope", () => {
    ...
    mocks.loadMarketingClawPlugins.mockReturnValue(fallbackRegistry as never);
  ...
  it("loads the plugin registry when runtime activation is explicitly requested", () => {
    ...
    expect(mocks.loadMarketingClawPlugins).toHaveBeenCalledTimes(1);
    expect(mockArg(mocks.loadMarketingClawPlugins).activate).toBe(true);
  });
  ...
  it("keeps explicit setup web provider cache opt-outs", () => {
    ...
    expect(mockArg(mocks.loadMarketingClawPlugins).cache).toBe(false);
    expect(mockArg(mocks.loadMarketingClawPlugins).onlyPluginIds).toEqual(["brave"]);
  });
});
