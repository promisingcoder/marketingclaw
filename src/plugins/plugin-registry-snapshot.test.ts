...
function makeTempDir() {
  return makeTrackedTempDir("marketingclaw-plugin-registry-snapshot", tempDirs);
}

function createHermeticEnv(rootDir: string): NodeJS.ProcessEnv {
  return {
    MARKETINGCLAW_BUNDLED_PLUGINS_DIR: path.join(rootDir, "bundled"),
...
function writeBundledPlugin(rootDir: string, pluginId: string, entryPath: string) {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      version: "1.0.0",
      marketingclaw: { extensions: [`./${entryPath}`] },
    }),
    "utf8",
  );
}
...
describe("loadPluginRegistrySnapshotWithMetadata", () => {
  ...
  it("recovers managed npm plugins missing from a stale persisted registry", () => {
    ...
    expect(result.snapshot.installRecords.whatsapp).toEqual({
      source: "npm",
      spec: "@marketingclaw/whatsapp@2026.5.2",
      installPath: whatsappDir,
      version: "2026.5.2",
      resolvedName: "@marketingclaw/whatsapp",
      resolvedVersion: "2026.5.2",
      resolvedSpec: "@marketingclaw/whatsapp@2026.5.2",
    });
  ...
  it("does not trust package-owned retained npm marker files during recovery", () => {
    ...
    fs.writeFileSync(
      path.join(codexDir, ".marketingclaw-retained-npm-install.json"),
  ...
  it("keeps persisted manifestless Claude bundles on the fast path", () => {
    ...
    const stateDir = path.join(tempRoot, "state");
    const env = { ...createHermeticEnv(tempRoot), MARKETINGCLAW_DISABLE_BUNDLED_PLUGINS: "1" };
    const config = {
      plugins: {
        load: { paths: [rootDir] },
      },
  ...
  it("refreshes a memoized derived snapshot when workspace plugins are installed", () => {
    ...
    writePackagePlugin(path.join(workspaceDir, ".marketingclaw", "extensions", "demo"));
  ...
  it("keeps persisted package plugins when file hashes match", () => {
    ...
    expect(record.manifestFile.size).toBe(
      fs.statSync(path.join(rootDir, "marketingclaw.plugin.json")).size,
    );
  ...
  it("keeps mixed source-checkout bundled roots from the same checkout", () => {
    const tempRoot = makeTempDir();
    const packageRoot = path.join(tempRoot, "marketingclaw");
