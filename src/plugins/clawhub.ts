...
function validateClawHubPluginPackage(params: {
  ...
  if (pkg.family === "skill") {
    ...
    return buildClawHubInstallFailure(
      `"${pkg.name}" is a skill. Use "marketingclaw skills install ${installRef}" instead.`,
  ...
  if (
    ...
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires plugin API ${compatibility.pluginApiRange}, but this MarketingClaw runtime exposes ${runtimeVersion}.`,
  ...
  if (
    ...
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires MarketingClaw >=${compatibility.minGatewayVersion}, but this host is ${runtimeVersion}.`,
