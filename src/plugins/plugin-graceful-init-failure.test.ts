...
const fixtureRoot = makeTrackedTempDir("marketingclaw-plugin-graceful", fixtureTempDirs);
let tempDirIndex = 0;
const { loadMarketingClawPlugins, clearPluginLoaderCache } = await import("./loader.js");
...
function readPluginId(pluginPath: string): string {
  const manifestPath = path.join(path.dirname(pluginPath), "marketingclaw.plugin.json");
...
describe("graceful plugin initialization failure", () => {
  ...
  it("logs a startup summary grouped by failure phase", async () => {
    ...
    expect(summary).toContain("marketingclaw plugins inspect <id> --runtime --json");
    expect(summary).toContain("marketingclaw plugins list");
  });
});
