...
const nixEnv: NodeJS.ProcessEnv = { MARKETINGCLAW_NIX_MODE: "1" };
...
describe("plugin hardlink policy", () => {
  ...
  it("does not treat MARKETINGCLAW_NIX_MODE as enough by itself", () => {
    expect(
      shouldRejectHardlinkedPluginFiles({
        origin: "config",
        rootDir: "/tmp/plugin",
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      expect(isNixStorePluginRoot("/nix/store/abc-marketingclaw-plugin")).toBe(true);
      expect(isNixStorePluginRoot("/tmp/nix/store/abc-marketingclaw-plugin")).toBe(false);
      expect(
        shouldRejectHardlinkedPluginFiles({
          origin: "config",
          rootDir: "/nix/store/abc-marketingclaw-plugin",
          env: nixEnv,
        }),
      ).toBe(false);
    },
