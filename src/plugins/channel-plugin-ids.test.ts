...
const listExplicitlyDisabledChannelIdsForConfig = vi.hoisted(() =>
  vi.fn((config: MarketingClawConfig) => {
...
function withManifestLoadPaths<T extends { id: string }>(
  ...
  return {
    ...
    manifestPath: `/tmp/plugins/${plugin.id}/marketingclaw.plugin.json`,
    skills: [],
    hooks: [],
    ...plugin,
  };
...
describe("resolveGatewayStartupPluginIds", () => {
  beforeEach(() => {
    listPotentialConfiguredChannelIds
      .mockReset()
      .mockImplementation((config: MarketingClawConfig) => {
  ...
  it("does not treat persisted auth alone as gateway startup intent", () => {
    listPotentialConfiguredChannelIds.mockImplementation(
      (
        configForTest: MarketingClawConfig,
    ...
    expectStartupPluginIdsCase({
      ...
      env: createPluginPlanningTestEnv({
        MARKETINGCLAW_STATE_DIR: "/tmp/marketingclaw-with-persisted-demo-channel",
      }),
      expected: ["browser", "memory-core"],
    });
  });
  ...
  it("does not include Codex when an OpenAI model is manually pinned to MarketingClaw", () => {
    expectStartupPluginIdsCase({
      config: {
        agents: {
          defaults: {
  ...
  it("ignores env runtime overrides during startup planning", () => {
    expectStartupPluginIdsCase({
      ...
      env: { MARKETINGCLAW_AGENT_RUNTIME: "codex" },
      expected: ["demo-channel", "browser", "memory-core"],
    });
  });
