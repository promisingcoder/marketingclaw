...
function createManifestPlugin(id: string): PluginManifestRecord {
  return {
    ...
    rootDir: `/tmp/${id}`,
    source: "bundled",
    manifestPath: `/tmp/${id}/marketingclaw.plugin.json`,
...
describe("resolvePluginDiscoveryProvidersRuntime", () => {
  ...
  it("clears bundled dist discovery chunks from the dist root", () => {
    ...
    mocks.loadPluginMetadataSnapshot.mockReturnValue({
      ...
      manifestRegistry: {
        plugins: [
          {
            ...createManifestPlugin("deepseek"),
            rootDir: "/tmp/marketingclaw/dist/extensions/deepseek",
            manifestPath: "/tmp/marketingclaw/dist/extensions/deepseek/marketingclaw.plugin.json",
            providerDiscoverySource:
              "/tmp/marketingclaw/dist/extensions/deepseek/provider-discovery.js",
          },
        ],
        diagnostics: [],
      },
    ...
    expect(mocks.clearNativeRequireJavaScriptModuleCache).toHaveBeenCalledWith(
      "/tmp/marketingclaw/dist/extensions/deepseek/provider-discovery.js",
      { dependencyRoot: "/tmp/marketingclaw/dist" },
    );
  });
