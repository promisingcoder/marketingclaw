...
import { resolveMarketingClawDevSourceRoot } from "./dev-source-root.js";
...
function resolvePluginLoaderJitiNativeModules(): string[] {
  try {
    ...
    const nativeModules = Array.isArray(configured)
      ...
      : [];
    return [...new Set([...nativeModules, "marketingclaw"])];
  } catch {
...
function hasTrustedMarketingClawRootIndicator(params: {
  packageRoot: string;
  ...
  const hasMarketingClawBin =
    (typeof params.packageJson.bin === "string" &&
      normalizeLowercaseStringOrEmpty(params.packageJson.bin).includes("marketingclaw")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin.marketingclaw === "string");
  const hasMarketingClawEntrypoint = fs.existsSync(
    path.join(params.packageRoot, "marketingclaw.mjs"),
  );
  return hasCliEntryExport || hasMarketingClawBin || hasMarketingClawEntrypoint;
}
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  if (!hasTrustedMarketingClawRootIndicator({ packageRoot, packageJson: pkg })) {
    return null;
  }
...
function resolveTrustedMarketingClawRootFromArgvHint(params: {
  argv1?: string;
  cwd: string;
}): string | null {
  if (!params.argv1) {
  ...
  return hasTrustedMarketingClawRootIndicator({ packageRoot, packageJson }) ? packageRoot : null;
}
...
function resolveDevSourceRootParam(params: { devSourceRoot?: string | null }): string | null {
  return params.devSourceRoot !== undefined
    ...
    : resolveMarketingClawDevSourceRoot(process.env);
}
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  const cwd = params.cwd ?? path.dirname(params.modulePath);
  const fromCwd = resolveMarketingClawPackageRootSync({ cwd });
  const fromExplicitHints =
    resolveTrustedMarketingClawRootFromArgvHint({ cwd, argv1: params.argv1 }) ??
    (params.moduleUrl
...
const PLUGIN_SDK_PACKAGE_NAMES = ["marketingclaw/plugin-sdk", "@marketingclaw/plugin-sdk"] as const;
...
const PRIVATE_PLUGIN_SDK_SUBPATH_OWNERS: readonly PrivatePluginSdkSubpathOwner[] = [
  {
    ...
    officialInstalledPackageName: "@marketingclaw/codex",
...
const WORKSPACE_PACKAGE_ALIAS_ENTRIES: WorkspacePackageAliasEntry[] = [
  {
    packageName: "@marketingclaw/gateway-client",
  ...
  {
    packageName: "@marketingclaw/gateway-protocol",
  ...
  {
    packageName: "@marketingclaw/markdown-core",
  ...
  {
    packageName: "@marketingclaw/media-generation-core",
  ...
  {
    packageName: "@marketingclaw/media-core",
  ...
  {
    packageName: "@marketingclaw/normalization-core",
  ...
  {
    packageName: "@marketingclaw/terminal-core",
  ...
  {
    packageName: "@marketingclaw/net-policy",
  ...
  {
    packageName: "@marketingclaw/model-catalog-core",
...
export function listWorkspacePackageExportAliasEntries(params: {
  ...
  const fallbackPackageRoot = resolveMarketingClawPackageRootSync({ cwd: process.cwd() });
  const packageJson =
...
function readBundledPluginPackageName(packageJsonPath: string): string | null {
  ...
  return name.startsWith("@marketingclaw/") ? name : null;
}
...
function shouldIncludePrivateLocalOnlyPluginSdkSubpaths() {
  return process.env.MARKETINGCLAW_ENABLE_PRIVATE_QA_CLI === "1";
}
...
export function resolvePluginSdkScopedAliasMap(
  ...
  for (const subpath of listPluginSdkExportedSubpaths({
    ...
    for (const kind of orderedKinds) {
      ...
      if (Object.hasOwn(aliasMap, `marketingclaw/plugin-sdk/${subpath}`)) {
        break;
      }
    }
  }
...
export function buildPluginLoaderAliasMap(
  ...
  const result: Record<string, string> = {
    ...(extensionApiAlias
      ? { "marketingclaw/extension-api": normalizeJitiAliasTargetPath(extensionApiAlias) }
      : {}),
...
export function buildPluginLoaderJitiOptions(
  ...
  return {
    ...
    tryNative: true,
    // When jiti must transform a plugin entry, keep MarketingClaw's own package
...
export function isBundledPluginExtensionPath(params: {
  modulePath: string;
  marketingClawPackageRoot: string;
  bundledPluginsDir?: string;
}): boolean {
  const normalizedModulePath = path.resolve(params.modulePath);
  const roots = [
    params.bundledPluginsDir ? path.resolve(params.bundledPluginsDir) : null,
    path.join(params.marketingClawPackageRoot, "extensions"),
    path.join(params.marketingClawPackageRoot, "dist", "extensions"),
    path.join(params.marketingClawPackageRoot, "dist-runtime", "extensions"),
