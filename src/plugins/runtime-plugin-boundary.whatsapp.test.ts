...
function createBundledWhatsAppRuntimeFixture() {
  const rootDir = makeTrackedTempDir("marketingclaw-whatsapp-boundary", tempDirs);
  for (const [relativePath, value] of Object.entries({
    ...
    "dist/connection-controller-registry.js": [
      'const key = Symbol.for("marketingclaw.whatsapp.connectionControllerRegistry");',
      "const g = globalThis;",
      "if (!g[key]) {",
...
function createExternalTypeScriptRuntimePackageFixture() {
  const rootDir = makeTrackedTempDir("marketingclaw-external-boundary-ts", tempDirs);
  writeRuntimeFixtureText(
    rootDir,
    "package.json",
    JSON.stringify(
      {
        name: "marketingclaw-external-ts-runtime",
        type: "module",
      },
      null,
      2,
...
describe("runtime plugin boundary whatsapp seam", () => {
  ...
  it("rejects bundled TypeScript runtime modules instead of using the source loader", () => {
    const rootDir = makeTrackedTempDir("marketingclaw-bundled-boundary-ts", tempDirs);
