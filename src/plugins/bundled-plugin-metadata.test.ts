...
function listExternalRepoBundledPluginManifestDirs(): string[] | null {
  ...
  return manifestFiles
    .flatMap((file) => {
      const match = /^extensions\/([^/]+)\/marketingclaw\.plugin\.json$/u.exec(file);
...
function listGitRepoBundledPluginManifestFiles(): string[] | null {
  return listGitTrackedFiles({ repoRoot, pathspecs: "extensions/*/marketingclaw.plugin.json" });
}
...
function createRepoBundledManifestRegistry(): PluginManifestRegistry {
  return {
    plugins: listRepoBundledPluginManifests().map(({ manifest, dirName }) => ({
      ...
      manifestPath: path.join(repoRoot, "extensions", dirName, "marketingclaw.plugin.json"),
...
describe("bundled plugin metadata", () => {
  ...
  it("prefers built generated paths when present and falls back to source paths", () => {
    const tempRoot = createGeneratedPluginTempRoot("marketingclaw-bundled-plugin-metadata-");
  ...
  it("uses dist-runtime generated paths before source fallback when packaged dist is absent", () => {
    const tempRoot = createGeneratedPluginTempRoot(
      "marketingclaw-bundled-plugin-runtime-metadata-",
    );
    const pluginRoot = path.join(tempRoot, "extensions", "plugin");
  ...
  it("resolves plugin-local generated entry paths when the plugin dir is provided", () => {
    const tempRoot = createGeneratedPluginTempRoot("marketingclaw-bundled-plugin-metadata-local-");
  ...
  it("keeps generated entry path resolution inside bundled plugin roots", () => {
    const tempRoot = createGeneratedPluginTempRoot("marketingclaw-bundled-plugin-path-contained-");
  ...
  it("scans direct plugin-tree overrides and resolves generated paths from that scan dir", () => {
    const tempRoot = createGeneratedPluginTempRoot("marketingclaw-bundled-plugin-direct-tree-");
  ...
  it("reflects bundled manifest edits on the next metadata read", () => {
    const tempRoot = createGeneratedPluginTempRoot("marketingclaw-bundled-plugin-fresh-");
    const pluginRoot = path.join(tempRoot, "extensions", "alpha");
  ...
  it("prefers direct scan-dir overrides over nested dist artifacts within the same override root", () => {
    const pluginsDir = createGeneratedPluginTempRoot(
      "marketingclaw-bundled-plugin-direct-priority-",
    );
    const pluginRoot = path.join(pluginsDir, "alpha");
  ...
  it("resolves bundled repo entry paths from dist before workspace source", () => {
    const tempRoot = createGeneratedPluginTempRoot("marketingclaw-bundled-plugin-repo-entry-");
    const pluginRoot = path.join(tempRoot, "extensions", "alpha");
    const distPluginRoot = path.join(tempRoot, "dist", "extensions", "alpha");
  ...
  it("keeps bundled repo entry path resolution inside the plugin directory", () => {
    const tempRoot = createGeneratedPluginTempRoot("marketingclaw-bundled-plugin-repo-contained-");
    const pluginRoot = path.join(tempRoot, "extensions", "alpha");
  ...
  it("merges runtime channel schema metadata with manifest-owned channel config fields", () => {
    const tempRoot = createGeneratedPluginTempRoot("marketingclaw-bundled-plugin-channel-configs-");
    ...
    writeJson(path.join(tempRoot, "extensions", "alpha", "marketingclaw.plugin.json"), {
      id: "alpha",
      channels: ["alpha"],
      configSchema: { type: "object" },
      channelConfigs: {
  ...
  it("captures top-level public surface artifacts without duplicating the primary entrypoints", () => {
    const tempRoot = createGeneratedPluginTempRoot(
      "marketingclaw-bundled-plugin-public-artifacts-",
    );
  ...
  it("loads channel config metadata from built public surfaces in dist-only roots", () => {
    const tempRoot = createGeneratedPluginTempRoot("marketingclaw-bundled-plugin-dist-config-");
    ...
    writeJson(path.join(distRoot, "extensions", "alpha", "marketingclaw.plugin.json"), {
      id: "alpha",
      configSchema: {
        type: "object",
        properties: {},
  ...
  it("does not probe broad runtime public surfaces for channel config metadata", () => {
    const tempRoot = createGeneratedPluginTempRoot(
      "marketingclaw-bundled-plugin-dist-config-runtime-",
    );
    const distRoot = path.join(tempRoot, "dist");
