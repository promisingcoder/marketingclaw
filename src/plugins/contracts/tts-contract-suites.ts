...
import type { ResolvedTtsConfig, SpeechProviderPlugin } from "marketingclaw/plugin-sdk/speech-core";
import { withEnv, withEnvAsync } from "marketingclaw/plugin-sdk/test-env";
...
type TtsRuntimeModule = typeof import("marketingclaw/plugin-sdk/tts-runtime");
type TtsCoreModule = typeof import("marketingclaw/plugin-sdk/speech-core");
...
const speechCoreRuntimeApiModuleId = resolveWorkspacePackagePublicModuleUrl({
  packageName: "@marketingclaw/speech-core",
  artifactBasename: "runtime-api.js",
});

let ttsRuntime: TtsRuntimeModule;
let ttsRuntimeInitialized = false;
let completeSimple: typeof import("marketingclaw/plugin-sdk/llm").completeSimple;
...
vi.mock("marketingclaw/plugin-sdk/llm", () => {
...
function asLegacyTtsConfig(value: unknown): MarketingClawConfig {
  return value as MarketingClawConfig;
}

function asLegacyMarketingClawConfig(value: Record<string, unknown>): MarketingClawConfig {
  return value as unknown as MarketingClawConfig;
}
...
function createOpenAiTelephonyCfg(model: "tts-1" | "gpt-4o-mini-tts"): MarketingClawConfig {
...
const loadTtsCore = createLazyRuntimeModule(() => import("marketingclaw/plugin-sdk/speech-core"));
...
function createResolvedSummarizationConfig(cfg: MarketingClawConfig): ResolvedTtsConfig {
  const rawConfig =
...
async function setupSummarizationMocks() {
  ...
  ({ completeSimple } = await import("marketingclaw/plugin-sdk/llm"));
    ...
    describe("getTtsProvider", () => {
      ...
      it("passes cfg into auto-selection so model-provider Google keys can configure TTS", () => {
        withIsolatedSpeechProviderEnv(
          ...
          () => {
            const cfg = asLegacyMarketingClawConfig({
              agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
              models: {
                providers: {
                  google: {
...
export function describeTtsAutoApplyContract() {
  describe("tts auto-apply contract", () => {
    ...
    const baseCfg: MarketingClawConfig = asLegacyMarketingClawConfig({
      agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
      messages: {
        tts: {
          auto: "inbound",
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.MARKETINGCLAW_TTS_PREFS;
      process.env.MARKETINGCLAW_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      try {
        await withMockedSpeechFetch(run, 1);
      } finally {
        process.env.MARKETINGCLAW_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: MarketingClawConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,
