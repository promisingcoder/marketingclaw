...
const PLUGIN_SDK_SUBPATH_PATTERN = /marketingclaw\/plugin-sdk\/([a-z0-9][a-z0-9-]*)\b/g;
...
const DEPRECATED_EXTENSION_SDK_SPECIFIERS = new Set([
  ...
  "marketingclaw/plugin-sdk/channel-config-schema-legacy",
  "marketingclaw/plugin-sdk/compat",
  "marketingclaw/plugin-sdk/testing",
...
const LEGACY_MEMORY_EMBEDDING_PROVIDER_MANIFEST_FILES = new Set([
  "extensions/amazon-bedrock/marketingclaw.plugin.json",
  "extensions/deepinfra/marketingclaw.plugin.json",
  "extensions/github-copilot/marketingclaw.plugin.json",
  "extensions/google/marketingclaw.plugin.json",
  "extensions/lmstudio/marketingclaw.plugin.json",
  "extensions/memory-core/marketingclaw.plugin.json",
  "extensions/mistral/marketingclaw.plugin.json",
  "extensions/ollama/marketingclaw.plugin.json",
  "extensions/openai/marketingclaw.plugin.json",
  "extensions/voyage/marketingclaw.plugin.json",
]);
...
function collectDeprecatedExtensionSdkImports(): Array<{ file: string; specifier: string }> {
  const leaks: Array<{ file: string; specifier: string }> = [];
  const importPatterns = [
    /\b(?:import|export)\b[\s\S]*?\bfrom\s*["'](marketingclaw\/plugin-sdk(?:\/[a-z0-9][a-z0-9-]*)?)["']/g,
    /\bimport\s*\(\s*["'](marketingclaw\/plugin-sdk(?:\/[a-z0-9][a-z0-9-]*)?)["']\s*\)/g,
    /\bvi\.(?:mock|doMock)\s*\(\s*["'](marketingclaw\/plugin-sdk(?:\/[a-z0-9][a-z0-9-]*)?)["']/g,
  ];
  for (const file of collectExtensionFiles(resolve(REPO_ROOT, "extensions"))) {
    const repoRelativePath = toRepoRelativePath(file);
    const source = fs.readFileSync(file, "utf8");
...
function collectNewDeprecatedMemoryEmbeddingProviderManifestFiles(): string[] {
  ...
  const manifestFiles =
    listGitTrackedFiles({
      repoRoot: REPO_ROOT,
      pathspecs: "extensions/**/marketingclaw.plugin.json",
    }) ?? [];
...
function collectDeprecatedTestBarrelImports(): string[] {
  const leaks: Array<{ file: string; specifier: string }> = [];
  const importPatterns = [
    /\b(?:import|export)\b[\s\S]*?\bfrom\s*["'](marketingclaw\/plugin-sdk\/(?:testing|test-utils))["']/g,
    /\bimport\s*\(\s*["'](marketingclaw\/plugin-sdk\/(?:testing|test-utils))["']\s*\)/g,
    /\bvi\.(?:mock|doMock)\s*\(\s*["'](marketingclaw\/plugin-sdk\/(?:testing|test-utils))["']/g,
  ];
...
function collectCrossOwnerReservedSdkImports(): Array<{
  ...
  for (const file of collectExtensionFiles(resolve(REPO_ROOT, "extensions"))) {
    ...
    for (const match of source.matchAll(importPattern)) {
      ...
      leaks.push({
        file: repoRelativePath,
        specifier: `marketingclaw/plugin-sdk/${subpath}`,
        owner,
      });
    }
  }
    ...
    for (const importPattern of importPatterns) {
      for (const match of source.matchAll(importPattern)) {
        ...
        if (subpath && subpaths.has(subpath)) {
          imports.add(`${repoRelativePath}: marketingclaw/plugin-sdk/${subpath}`);
        }
      }
    }
  }
...
describe("plugin-sdk package contract guardrails", () => {
  ...
  it("keeps curated public plugin-sdk references on exported built subpaths", () => {
    ...
    for (const reference of collectPluginSdkSubpathReferences()) {
      ...
      failures.push(
        `${reference.file} references marketingclaw/plugin-sdk/${reference.subpath}, but ${reference.subpath} is missing from ${missingFrom.join(" and ")}`,
      );
    }
  ...
  it("keeps Matrix dependencies local to the Matrix plugin", () => {
    ...
    expect(rootRuntimeDeps.has("@marketingclaw/plugin-package-contract")).toBe(false);
  });
