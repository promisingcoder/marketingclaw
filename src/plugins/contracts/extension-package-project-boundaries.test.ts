...
const MEMORY_HOST_SDK_ALLOWED_CORE_BRIDGE_FILES = [
  "packages/memory-host-sdk/src/host/marketingclaw-runtime-auth.ts",
  "packages/memory-host-sdk/src/host/marketingclaw-runtime-network.ts",
  "packages/memory-host-sdk/src/host/marketingclaw-runtime.ts",
] as const;
const MEMORY_HOST_SDK_RUNTIME_ADAPTER_FILES = [
  "packages/memory-host-sdk/src/host/marketingclaw-runtime-agent.ts",
  "packages/memory-host-sdk/src/host/marketingclaw-runtime-cli.ts",
  "packages/memory-host-sdk/src/host/marketingclaw-runtime-config.ts",
  "packages/memory-host-sdk/src/host/marketingclaw-runtime-io.ts",
  "packages/memory-host-sdk/src/host/marketingclaw-runtime-memory.ts",
  "packages/memory-host-sdk/src/host/marketingclaw-runtime-session.ts",
] as const;
...
function collectMarketingClawRuntimeDirectImportFiles(relativeDir: string): string[] {
  return collectCodeFiles(relativeDir).filter((file) => {
    const source = fs.readFileSync(resolve(REPO_ROOT, file), "utf8");
    return source.includes('"./marketingclaw-runtime.js"');
  });
}
...
describe("opt-in extension package boundaries", () => {
  ...
  it("keeps plugin-sdk package types generated from the package build, not a hand-maintained types bridge", () => {
    ...
    expect(packageJson.name).toBe("@marketingclaw/plugin-sdk");
  ...
  it("keeps memory-host-sdk as a private package-owned contract surface", () => {
    ...
    expect(packageJson.name).toBe("@marketingclaw/memory-host-sdk");
    ...
    expect(collectMarketingClawRuntimeDirectImportFiles("packages/memory-host-sdk/src")).toEqual([
