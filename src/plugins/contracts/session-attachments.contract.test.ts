...
import { FILE_TYPE_SNIFF_MAX_BYTES } from "@marketingclaw/media-core/mime";
import {
...
async function withSessionStore(
  ...
  const stateDir = await fs.mkdtemp(
    path.join(resolvePreferredMarketingClawTmpDir(), "marketingclaw-session-attachments-"),
  );
...
describe("plugin session attachments", () => {
  afterEach(() => {
    ...
    delete (globalThis as { proofAttachmentApi?: MarketingClawPluginApi }).proofAttachmentApi;
