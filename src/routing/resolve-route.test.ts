...
const resolveRoute = (
  params: Omit<Parameters<typeof resolveAgentRoute>[0], "cfg"> & { cfg?: MarketingClawConfig },
) =>
  resolveAgentRoute({
    cfg: params.cfg ?? {},
    ...params,
...
describe("resolveAgentRoute", () => {
  ...
  const expectRouteResolutionCase = (params: {
    routeParams: Omit<Parameters<typeof resolveRoute>[0], "cfg"> & { cfg: MarketingClawConfig };
    expected: ResolvedRouteExpectation;
  }) => {
  ...
  test.each([
    ...
    },
    {
      ...
      cfg: {
        agents: {
          list: [{ id: "home", default: true, workspace: "~/marketingclaw-home" }],
        },
      } satisfies MarketingClawConfig,
      channel: "whatsapp" as const,
      accountId: "biz",
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<MarketingClawConfig["bindings"]>[number];
