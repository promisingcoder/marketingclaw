...
describe("createCodexNativeWebSearchWrapper", () => {
  ...
  it("does not enable code-mode transport enforcement when config is on but controls are inactive", () => {
    ...
    expect(observedOptions[0]?.marketingclawCodeModeToolSurface).toBeUndefined();
  ...
  it("enforces the code-mode transport surface when the run enables it at agent scope", () => {
    ...
    expect(observedOptions[0]?.marketingclawCodeModeToolSurface).toBe(true);
    expect(payloads[0]?.tools).toEqual([
      { type: "function", name: "exec" },
      { type: "function", name: "wait" },
    ]);
...
describe("createOpenAIAttributionHeadersWrapper", () => {
  it("routes native Codex traffic through the MarketingClaw transport so attribution survives MarketingClaw defaults", () => {
    let codexCalls = 0;
    ...
    expect(capturedHeaders?.originator).toBe("marketingclaw");
    expect(capturedHeaders?.["User-Agent"]).toMatch(/^marketingclaw\//);
  });
  ...
  it("keeps existing wrapped Codex streams so runtime OAuth injection is preserved", () => {
    ...
    expect(capturedOptions?.headers?.originator).toBe("marketingclaw");
    expect(capturedOptions?.headers?.["User-Agent"]).toMatch(/^marketingclaw\//);
  });
});
