import type { Context, Model } from "@marketingclaw/ai";
...
// ever reached. This proves the stream facade installs MarketingClaw's guarded
...
// Importing the facade installs the MarketingClaw AI transport host ports.
import "../stream.js";
...
describe("Anthropic Cloudflare guard-specific SSRF blocking proof", () => {
  ...
  it("blocks a private-IP request before globalThis.fetch is called (guard-specific behavior)", async () => {
    ...
    const { streamAnthropic } = await import("@marketingclaw/ai/internal/anthropic");
