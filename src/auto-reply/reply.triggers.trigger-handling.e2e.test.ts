...
async function expectNextRunUsesTargetSession(
  ...
  const runParams = firstMockCallArg(params.runEmbeddedAgentMock, "embedded MarketingClaw agent");
...
describe("trigger handling", () => {
  ...
  it("acknowledges bare /new without invoking the model or loading startup memory", async () => {
    await withTempHome(async (home) => {
      const workspaceDir = join(home, "marketingclaw");
    ...
    await withTempHome(async (home) => {
      ...
      for (const testCase of thinkCases) {
        ...
        if (testCase.assertPrompt) {
          const prompt =
            firstMockCallArg(runEmbeddedAgentMock, "embedded MarketingClaw agent").prompt ?? "";
  ...
  it("resolves heartbeat model selection from overrides", async () => {
    await withTempHome(async (home) => {
      ...
      for (const testCase of modelCases) {
        ...
        const call = firstMockCallArg(runEmbeddedAgentMock, "embedded MarketingClaw agent");
  ...
  it("compacts worker sessions via the agent session file", async () => {
    await withTempHome(async (home) => {
      ...
      expect(getCompactEmbeddedAgentSessionMock()).toHaveBeenCalledOnce();
      expect(
        firstMockCallArg(getCompactEmbeddedAgentSessionMock(), "embedded MarketingClaw compaction")
          .sessionFile,
  ...
  it("applies native model auth profile overrides to the target session", async () => {
    await withTempHome(async (home) => {
      ...
      runEmbeddedAgentMock.mockReset();
      const storePath = requireSessionStorePath(cfg);
      const authDir = join(home, ".marketingclaw", "agents", "main", "agent");
