...
beforeAll(async () => {
  suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-session-suite-"));
});
...
describe("initSessionState guarded initialization", () => {
  it("rejects inbound work for an archived session", async () => {
    const storePath = await createStorePath("marketingclaw-session-init-archived-");
    ...
    await expect(
      initSessionState({
        ...
        cfg: { session: { store: storePath } } as MarketingClawConfig,
  ...
  it("serializes concurrent initializers before reading the guarded snapshot", async () => {
    const storePath = await createStorePath("marketingclaw-session-init-race-");
...
describe("initSessionState thread forking", () => {
  it("forks a new session from the parent session file", async () => {
    ...
    const root = await makeCaseDir("marketingclaw-thread-session-");
  ...
  it("forks from parent when thread session key already exists but was not forked yet", async () => {
    const warn = vi.spyOn(console, "warn").mockImplementation(() => {});
    const root = await makeCaseDir("marketingclaw-thread-session-existing-");
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir);
  ...
  it("skips fork and creates fresh session when parent tokens exceed threshold", async () => {
    const root = await makeCaseDir("marketingclaw-thread-session-overflow-");
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir);
  ...
  it("skips fork when resolved parent token estimate exceeds threshold", async () => {
    const root = await makeCaseDir("marketingclaw-thread-session-overflow-estimated-");
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir);
  ...
  it("records topic-specific session files when MessageThreadId is present", async () => {
    const root = await makeCaseDir("marketingclaw-topic-session-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("records topic-specific session files from SessionKey when MessageThreadId is absent", async () => {
    const root = await makeCaseDir("marketingclaw-topic-session-key-");
    const storePath = path.join(root, "sessions.json");
...
describe("initSessionState RawBody", () => {
  it("uses RawBody for command extraction and reset triggers when Body contains wrapped context", async () => {
    const root = await makeCaseDir("marketingclaw-rawbody-");
    const storePath = path.join(root, "sessions.json");
    const cfg = { session: { store: storePath } } as MarketingClawConfig;
  ...
  it("preserves argument casing while still matching reset triggers case-insensitively", async () => {
    const root = await makeCaseDir("marketingclaw-rawbody-reset-case-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("drops cached skills snapshot when /new rotates an existing session", async () => {
    const root = await makeCaseDir("marketingclaw-rawbody-reset-skills-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("drains stale system events when /new rotates an existing session", async () => {
    const root = await makeCaseDir("marketingclaw-rawbody-reset-system-events-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("preserves a user model override across an implicit daily stale rollover (#90119)", async () => {
    ...
    const root = await makeCaseDir("marketingclaw-daily-rollover-model-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("preserves user-set behavior and pinned state across an implicit daily stale rollover (#92562)", async () => {
    ...
    const root = await makeCaseDir("marketingclaw-daily-rollover-behavior-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("preserves usage footer mode across daily rollover", async () => {
    const root = await makeCaseDir("marketingclaw-daily-rollover-usage-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("preserves user labels across dashboard session stale rollover (#101451)", async () => {
    const root = await makeCaseDir("marketingclaw-dashboard-rollover-label-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("clears an auto-fallback model override on an implicit daily stale rollover (#90119)", async () => {
    ...
    const root = await makeCaseDir("marketingclaw-daily-rollover-fallback-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("preserves behavior overrides while clearing an auto-fallback model override on one implicit daily rollover (#92562)", async () => {
    ...
    const root = await makeCaseDir("marketingclaw-daily-rollover-mixed-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("rotates local session state for /new on bound ACP sessions", async () => {
    const root = await makeCaseDir("marketingclaw-rawbody-acp-reset-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("rotates local session state for ACP /new when no matching conversation binding exists", async () => {
    const root = await makeCaseDir("marketingclaw-rawbody-acp-reset-no-conversation-");
    const storePath = path.join(root, "sessions.json");
    const sessionKey = "agent:codex:acp:binding:discord:default:feedface";
    const existingSessionId = "session-existing";
    const now = Date.now();
  ...
  it("keeps custom reset triggers working on bound ACP sessions", async () => {
    const root = await makeCaseDir("marketingclaw-rawbody-acp-custom-reset-");
    const storePath = path.join(root, "sessions.json");
    const sessionKey = "agent:codex:acp:binding:discord:default:feedface";
    const existingSessionId = "session-existing";
    const now = Date.now();
  ...
  it("keeps normal /new behavior for unbound ACP-shaped session keys", async () => {
    const root = await makeCaseDir("marketingclaw-rawbody-acp-unbound-reset-");
    const storePath = path.join(root, "sessions.json");
    const sessionKey = "agent:codex:acp:binding:discord:default:feedface";
    const existingSessionId = "session-existing";
    const now = Date.now();
  ...
  it("does not suppress /new when active conversation binding points to a non-ACP session", async () => {
    const root = await makeCaseDir("marketingclaw-rawbody-acp-nonacp-binding-");
    const storePath = path.join(root, "sessions.json");
    const sessionKey = "agent:codex:acp:binding:discord:default:feedface";
    const existingSessionId = "session-existing";
    const now = Date.now();
  ...
  it("does not suppress /new when active target session key is non-ACP even with configured ACP binding", async () => {
    const root = await makeCaseDir("marketingclaw-rawbody-acp-configured-fallback-target-");
    const storePath = path.join(root, "sessions.json");
    const channelId = "1478836151241412759";
  ...
  it("uses the default per-agent sessions store when config store is unset", async () => {
    const root = await makeCaseDir("marketingclaw-session-store-default-");
    const stateDir = path.join(root, ".marketingclaw");
  ...
  it.each([
    ...
    const storePath = await createStorePath("marketingclaw-generic-current-binding-");
...
describe("initSessionState reset policy", () => {
  ...
  it("defaults to daily reset at 4am local time", async () => {
    ...
    const root = await makeCaseDir("marketingclaw-reset-daily-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("treats sessions as stale before the daily reset when updated before yesterday's boundary", async () => {
    ...
    const root = await makeCaseDir("marketingclaw-reset-daily-edge-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("expires sessions when idle timeout wins over daily reset", async () => {
    ...
    const root = await makeCaseDir("marketingclaw-reset-idle-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("preserves idle rollover when an ordinary send asserts the current session id", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 30, 0));
    const root = await makeCaseDir("marketingclaw-reset-idle-requested-session-ordinary-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("reuses an idle-expired session when a reconnecting client requests current session resume", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 30, 0));
    const root = await makeCaseDir("marketingclaw-reset-idle-requested-session-");
    const storePath = path.join(root, "sessions.json");
    const sessionKey = "agent:main:main";
  ...
  it("does not reuse an idle-expired session for a stale asserted session id", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 30, 0));
    const root = await makeCaseDir("marketingclaw-reset-idle-stale-requested-session-");
    const storePath = path.join(root, "sessions.json");
    const sessionKey = "agent:main:main";
  ...
  it("drains stale system events when idle rollover creates a new session", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 30, 0));
    const root = await makeCaseDir("marketingclaw-reset-idle-system-events-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("reuses completed run entries while the session is still fresh", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 30, 0));
    const root = await makeCaseDir("marketingclaw-reset-terminal-entry-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("recovers failed group sessions without rotating the transcript", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 30, 0));
    const root = await makeCaseDir("marketingclaw-reset-failed-entry-");
    const storePath = path.join(root, "sessions.json");
    ...
    const result = await initSessionState({
      ctx: {
        Body: "@marketingclaw hello",
        RawBody: "@marketingclaw hello",
        CommandBody: "@marketingclaw hello",
        SessionKey: sessionKey,
        ChatType: "group",
        Provider: "telegram",
        BotUsername: "marketingclaw",
      },
      cfg,
      commandAuthorized: true,
    });
  ...
  it("keeps the existing stale session for /reset soft", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 30, 0));
    const root = await makeCaseDir("marketingclaw-reset-soft-stale-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("keeps the existing stale session for /reset: soft", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 30, 0));
    const root = await makeCaseDir("marketingclaw-reset-soft-colon-stale-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("keeps the existing stale session for multiline /reset soft tails", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 30, 0));
    const root = await makeCaseDir("marketingclaw-reset-soft-multiline-stale-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("keeps multiline slash skill payloads on the current session", async () => {
    const root = await makeCaseDir("marketingclaw-skill-multiline-session-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("does not preserve a stale session for unauthorized /reset soft", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 30, 0));
    const root = await makeCaseDir("marketingclaw-reset-soft-stale-unauthorized-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("uses per-type overrides for thread sessions", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 0, 0));
    const root = await makeCaseDir("marketingclaw-reset-thread-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("detects thread sessions without thread key suffix", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 0, 0));
    const root = await makeCaseDir("marketingclaw-reset-thread-nosuffix-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("defaults to daily resets when only resetByType is configured", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 0, 0));
    const root = await makeCaseDir("marketingclaw-reset-type-default-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("keeps legacy idleMinutes behavior without reset config", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 0, 0));
    const root = await makeCaseDir("marketingclaw-reset-legacy-");
    const storePath = path.join(root, "sessions.json");
...
describe("initSessionState browser tab cleanup", () => {
  it("closes tracked browser tabs when idle session expires", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 30, 0));
    const storePath = await createStorePath("marketingclaw-tab-cleanup-idle-");
  ...
  it("skips browser tab cleanup when root browser support is disabled", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 30, 0));
    const storePath = await createStorePath("marketingclaw-tab-cleanup-browser-disabled-");
  ...
  it("skips browser tab cleanup when the browser plugin entry is disabled", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 30, 0));
    const storePath = await createStorePath("marketingclaw-tab-cleanup-browser-plugin-disabled-");
  ...
  it("closes tracked browser tabs on explicit /new reset", async () => {
    const storePath = await createStorePath("marketingclaw-tab-cleanup-reset-");
  ...
  it("does not close browser tabs for a fresh session without previous state", async () => {
    const storePath = await createStorePath("marketingclaw-tab-cleanup-fresh-");
  ...
  it("includes the peer-scoped runtime key for direct-message cleanup", async () => {
    const storePath = await createStorePath("marketingclaw-tab-cleanup-peer-key-");
...
describe("initSessionState channel reset overrides", () => {
  it("uses channel-specific reset policy when configured", async () => {
    const root = await makeCaseDir("marketingclaw-channel-idle-");
    const storePath = path.join(root, "sessions.json");
...
describe("initSessionState reset triggers in WhatsApp groups", () => {
  ...
  function makeCfg(params: { storePath: string; allowFrom: string[] }): MarketingClawConfig {
    return {
  ...
  it("applies WhatsApp group reset authorization across sender variants", async () => {
    ...
    const storePath = await createStorePath("marketingclaw-group-reset");
    const cases = [
      {
  ...
  it("starts a fresh session when a scoped WhatsApp group entry only contains activation state", async () => {
    ...
    const storePath = await createStorePath("marketingclaw-group-activation-backfill-");
    await writeSessionStoreFast(storePath, {
      [sessionKey]: {
...
describe("initSessionState reset triggers in Slack channels", () => {
  ...
  it("supports mention-prefixed Slack reset commands and preserves args", async () => {
    ...
    const storePath = await createStorePath("marketingclaw-slack-channel-new-");
    await seedSessionStore({
      storePath,
      sessionKey,
      sessionId: existingSessionId,
...
describe("initSessionState preserves behavior overrides across /new and /reset", () => {
  ...
  it("preserves behavior overrides across /new and /reset", async () => {
    const storePath = await createStorePath("marketingclaw-reset-overrides-");
  ...
  it("preserves usage family metadata across /new and /reset", async () => {
    const storePath = await createStorePath("marketingclaw-reset-usage-family-");
  ...
  it("preserves selected auth profile overrides across /new and /reset", async () => {
    const storePath = await createStorePath("marketingclaw-reset-model-auth-");
  ...
  it("clears auto-sourced model/provider/auth overrides on /new and /reset (#69301)", async () => {
    const storePath = await createStorePath("marketingclaw-reset-auto-overrides-");
  ...
  it("clears recovered auto fallback model overrides without modelOverrideSource on /new and /reset", async () => {
    const storePath = await createStorePath("marketingclaw-reset-recovered-auto-fallback-");
    const sessionKey = "agent:main:telegram:direct:6761477233";
  ...
  it("clears recovered Codex fallback metadata for Discord channel sessions on /new", async () => {
    const storePath = await createStorePath("marketingclaw-reset-discord-codex-fallback-");
  ...
  it("clears stale runtime model cache fields on /new and /reset (#77322)", async () => {
    const storePath = await createStorePath("marketingclaw-reset-runtime-model-cache-");
  ...
  it("preserves spawned session ownership metadata across /new and /reset", async () => {
    const storePath = await createStorePath("marketingclaw-reset-spawned-metadata-");
  ...
  it("requires operator.admin when Provider is internal even if Surface carries external metadata", async () => {
    const storePath = await createStorePath("marketingclaw-internal-reset-provider-authoritative-");
  ...
  it("keeps the existing session for /reset soft", async () => {
    const storePath = await createStorePath("marketingclaw-soft-reset-session-");
  ...
  it("keeps the existing session for /reset newline soft", async () => {
    const storePath = await createStorePath("marketingclaw-reset-newline-soft-");
  ...
  it("archives the old session store entry on /new", async () => {
    const storePath = await createStorePath("marketingclaw-archive-old-");
  ...
  it("drains foreign work before replacing a reply session", async () => {
    const storePath = await createStorePath("marketingclaw-rollover-admission-");
    ...
    const initialization = initSessionState({
      ...
      cfg: { session: { store: storePath, idleMinutes: 999 } } as MarketingClawConfig,
      commandAuthorized: true,
    });
  ...
  it("keeps the initiating reply admission during an in-band rollover", async () => {
    const storePath = await createStorePath("marketingclaw-rollover-initiator-");
  ...
  it("cancels a competing admitted rollover without deadlocking the session", async () => {
    const storePath = await createStorePath("marketingclaw-rollover-contenders-");
  ...
  it.each([
    ...
    const storePath = await createStorePath("marketingclaw-rollover-revalidation-");
    ...
    const initialization = initSessionState({
      ...
      cfg: { session: { store: storePath, idleMinutes: 1 } } as MarketingClawConfig,
      commandAuthorized: true,
    });
  ...
  it("archives the old session transcript on daily/scheduled reset (stale session)", async () => {
    ...
    try {
      ...
      vi.setSystemTime(new Date(2026, 0, 18, 5, 0, 0));
      const storePath = await createStorePath("marketingclaw-stale-archive-");
  ...
  it("defers implicit daily rollover while the same session has an active run", async () => {
    ...
    try {
      vi.setSystemTime(new Date(2026, 0, 18, 5, 0, 0));
      const storePath = await createStorePath("marketingclaw-active-stale-archive-");
  ...
  it("does not defer stale archival for the current turn's queued reservation", async () => {
    ...
    try {
      vi.setSystemTime(new Date(2026, 0, 18, 5, 0, 0));
      const storePath = await createStorePath("marketingclaw-queued-stale-archive-");
  ...
  it("does not defer stale archival for a different active session id", async () => {
    ...
    try {
      vi.setSystemTime(new Date(2026, 0, 18, 5, 0, 0));
      const storePath = await createStorePath("marketingclaw-active-other-stale-archive-");
  ...
  it("keeps provider-owned CLI sessions on implicit daily reset boundaries", async () => {
    ...
    try {
      vi.setSystemTime(new Date(2026, 0, 18, 5, 0, 0));
      const storePath = await createStorePath("marketingclaw-cli-implicit-reset-");
  ...
  it("honors explicit reset policies for provider-owned CLI sessions", async () => {
    const storePath = await createStorePath("marketingclaw-cli-explicit-reset-");
  ...
  it("disposes the previous bundle MCP runtime on session rollover", async () => {
    const storePath = await createStorePath("marketingclaw-stale-runtime-dispose-");
  ...
  it("idle-based new session does NOT preserve overrides (no entry to read)", async () => {
    const storePath = await createStorePath("marketingclaw-idle-no-preserve-");
...
describe("drainFormattedSystemEvents", () => {
  ...
  it("keeps channel summary lines prefixed as trusted system output on new main sessions", async () => {
    ...
    const result = await drainFormattedSystemEvents({
      cfg: { channels: {} } as MarketingClawConfig,
      sessionKey: "agent:main:main",
      isMainSession: true,
      isNewSession: true,
    });
...
describe("persistSessionUsageUpdate", () => {
  ...
  it("uses lastCallUsage for totalTokens when provided", async () => {
    const storePath = await createStorePath("marketingclaw-usage-");
  ...
  it("keeps the prior total stale when last-call context is unavailable", async () => {
    const storePath = await createStorePath("marketingclaw-usage-unavailable-context-");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
      sessionKey,
  ...
  it("marks a fresh zero stale when a completed run has no context snapshot", async () => {
    const storePath = await createStorePath("marketingclaw-usage-no-snapshot-");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
      sessionKey,
  ...
  it("accounts exhausted-run usage without committing its model and persists CLI binding", async () => {
    const storePath = await createStorePath("marketingclaw-usage-exhausted-");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
      sessionKey,
  ...
  it("accounts goal usage when fresh token snapshots are persisted", async () => {
    const storePath = await createStorePath("marketingclaw-usage-goal-");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
      sessionKey,
  ...
  it("uses lastCallUsage cache counters when available", async () => {
    const storePath = await createStorePath("marketingclaw-usage-cache-");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
      sessionKey,
  ...
  it("treats CLI usage as a fresh context snapshot when requested", async () => {
    const storePath = await createStorePath("marketingclaw-usage-cli-");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
      sessionKey,
  ...
  it("clears stale CLI binding when usage update reports an unflushed replacement", async () => {
    const storePath = await createStorePath("marketingclaw-usage-cli-clear-");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
      sessionKey,
  ...
  it("prefers fresh final usage over zero compactionTokensAfter", async () => {
    const storePath = await createStorePath("marketingclaw-usage-compaction-reset-");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
      sessionKey,
  ...
  it("prefers fresh lastCallUsage over positive compactionTokensAfter", async () => {
    const storePath = await createStorePath("marketingclaw-usage-compaction-positive-");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
      sessionKey,
  ...
  it("uses positive compactionTokensAfter when final usage has no prompt total", async () => {
    const storePath = await createStorePath("marketingclaw-usage-compaction-fallback-");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
      sessionKey,
  ...
  it("snapshots estimatedCostUsd instead of accumulating (fixes #69347)", async () => {
    const storePath = await createStorePath("marketingclaw-usage-cost-");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
      sessionKey,
  ...
  it("preserves the displayed session model when heartbeat usage uses a heartbeat model", async () => {
    const storePath = await createStorePath("marketingclaw-usage-heartbeat-model-");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
      sessionKey,
  ...
  it("persists heartbeat CLI binding while preserving displayed session model", async () => {
    const storePath = await createStorePath("marketingclaw-usage-heartbeat-cli-binding-");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
      sessionKey,
  ...
  it("honors heartbeat CLI binding clears while preserving displayed session model", async () => {
    const storePath = await createStorePath("marketingclaw-usage-heartbeat-cli-clear-");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
      sessionKey,
  ...
  it("preserves the displayed session model when an internal announce uses fallback", async () => {
    const storePath = await createStorePath("marketingclaw-usage-internal-announce-model-");
  ...
  it("persists zero estimatedCostUsd for free priced models", async () => {
    const storePath = await createStorePath("marketingclaw-usage-free-cost-");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
      sessionKey,
