...
const workspaceHarness = createCommandWorkspaceHarness("marketingclaw-command-mcp-");
...
function buildCfg(): MarketingClawConfig {
  return {
    commands: {
      text: true,
      mcp: true,
...
describe("handleCommands /mcp", () => {
  ...
  it("writes MCP config and shows it back", async () => {
    await withTempHome("marketingclaw-command-mcp-home-", async () => {
