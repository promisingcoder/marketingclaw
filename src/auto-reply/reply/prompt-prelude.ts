...
const ROOM_EVENT_PROMPT = "[MarketingClaw room event]";
...
export function buildReplyPromptEnvelopeBase(
  ...
  // conversation, while the MarketingClaw marker remains current-turn context only.
  const transcriptBody = params.isHeartbeat
    ? HEARTBEAT_TRANSCRIPT_PROMPT
    : params.isBareSessionReset
      ? softResetTail || `[MarketingClaw session ${params.startupAction}]`
      : isRoomEvent
