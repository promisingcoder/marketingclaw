...
import { scheduleGatewaySigusr1Restart, triggerMarketingClawRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting MarketingClaw in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  let sentinelWritten = false;
  ...
  const restartMethod = triggerMarketingClawRestart();
  if (!restartMethod.ok) {
    if (sentinelWritten) {
      await clearRestartSentinel();
    }
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting MarketingClaw via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
