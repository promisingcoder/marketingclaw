...
describe("runMemoryFlushIfNeeded", () => {
  ...
  beforeEach(async () => {
    rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-memory-unit-"));
  ...
  it("skips MarketingClaw preflight compaction for persisted Codex runtime sessions", async () => {
    registerMemoryFlushPlanResolverForTest(() => ({
      softThresholdTokens: 4_000,
      forceFlushTranscriptBytes: 1_000_000_000,
      reserveTokensFloor: 0,
  ...
  it("keeps the OpenAI API context window for persisted MarketingClaw runtime overrides", async () => {
    registerMemoryFlushPlanResolverForTest(() => ({
      softThresholdTokens: 4_000,
      forceFlushTranscriptBytes: 1_000_000_000,
      reserveTokensFloor: 0,
