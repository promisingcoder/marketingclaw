...
vi.mock("../../agents/model-selection.js", async () => {
  ...
  return {
    ...actual,
    isCliProvider: (provider: string, cfg?: MarketingClawConfig) => {
...
vi.mock("../../cron/store.js", () => {
  const resolveCronPath = (storePath?: string) => storePath ?? "/tmp/marketingclaw-cron-store.json";
  return {
...
describe("runReplyAgent auto-compaction token update", () => {
  ...
  beforeAll(async () => {
    ...
    await runBaseReplyWithAgentMeta({
      tmpPrefix: "marketingclaw-usage-warm-",
      agentMeta: {
  ...
  it("updates totalTokens from lastCallUsage even without compaction", async () => {
    const { sessionKey, stored } = await runBaseReplyWithAgentMeta({
      tmpPrefix: "marketingclaw-usage-last-",
      agentMeta: {
  ...
  it("keeps an unarmed preflight drain visible instead of dropping the reply", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-preflight-drain-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
    const sessionEntry = {
      sessionId: "session",
  ...
  it("reports live diagnostic context from promptTokens, not provider usage totals", async () => {
    const { usageEvent } = await runBaseReplyWithAgentMeta({
      tmpPrefix: "marketingclaw-usage-diagnostic-",
  ...
  it("falls back to last-call prompt usage for live diagnostic context", async () => {
    const { usageEvent } = await runBaseReplyWithAgentMeta({
      tmpPrefix: "marketingclaw-usage-diagnostic-last-",
      collectDiagnostics: true,
      agentMeta: {
        usage: { input: 75_000, output: 5_000, cacheRead: 25_000, total: 105_000 },
        lastCallUsage: {
  ...
  it("reads opted-in post-compaction context from the queued workspace instead of process cwd", async () => {
    const workspaceDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "marketingclaw-post-compaction-workspace-"),
    );
    const cwdDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-post-compaction-cwd-"));
    await fs.writeFile(
    ...
    try {
      const { sessionKey } = await runBaseReplyWithAgentMeta({
        tmpPrefix: "marketingclaw-post-compaction-workspace-root-",
        workspaceDir,
        config: {
          agents: {
            defaults: {
...
describe("runReplyAgent Active Memory inline debug", () => {
  it("appends inline Active Memory status payload when verbose is enabled", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-active-memory-inline-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
  ...
  it("appends raw trace payloads when trace raw is enabled", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-trace-raw-usage-"));
    const storePath = path.join(tmp, "sessions.json");
  ...
  it("does not emit persisted trace output to an unauthorized sender", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-trace-raw-unauthorized-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionFile = path.join(tmp, "session.jsonl");
    const sessionKey = "main";
    const sessionEntry: SessionEntry = {
  ...
  it("escapes markdown fence delimiters inside raw trace blocks", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-trace-raw-fence-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionFile = path.join(tmp, "session.jsonl");
    const sessionKey = "main";
    const sessionEntry: SessionEntry = {
...
describe("runReplyAgent private message_tool_only final warning (#85714)", () => {
  async function runPrivateFinalCase(params: {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-stranded-"));
    const storePath = path.join(tmp, "sessions.json");
