...
vi.mock("../../agents/workspace.js", () => ({
  DEFAULT_AGENT_WORKSPACE_DIR: "/tmp/marketingclaw-workspace",
...
describe("getReplyFromConfig fast test bootstrap", () => {
  ...
  it("fails fast on unmarked config overrides in strict fast-test mode", async () => {
    await expect(
      getReplyFromConfig(buildGetReplyCtx(), undefined, {} as MarketingClawConfig),
  ...
  it("skips getRuntimeConfig, workspace bootstrap, and session bootstrap for marked test configs", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-fast-reply-"));
  ...
  it("still merges partial config overrides against getRuntimeConfig()", async () => {
    vi.stubEnv("MARKETINGCLAW_ALLOW_SLOW_REPLY_TESTS", "1");
    vi.mocked(loadConfigMock).mockReturnValue({
      channels: {
        telegram: {
  ...
  it("clears stale ack-only heartbeat pending delivery before running heartbeat", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-heartbeat-pending-clear-"));
  ...
  it("keeps non-ack heartbeat pending delivery without direct replay", async () => {
    const home = await fs.mkdtemp(
      path.join(os.tmpdir(), "marketingclaw-heartbeat-pending-replay-"),
    );
    const storePath = path.join(home, "sessions.json");
    const sessionKey = "agent:main:telegram:123";
    await seedFastPathSessionStore(storePath, {
  ...
  it("does not replay stale heartbeat pending delivery", async () => {
    const home = await fs.mkdtemp(
      path.join(os.tmpdir(), "marketingclaw-heartbeat-pending-suppress-"),
    );
    const storePath = path.join(home, "sessions.json");
    const sessionKey = "agent:main:telegram:123";
    await seedFastPathSessionStore(storePath, {
  ...
  it("handles native /status before workspace bootstrap", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-native-status-fast-"));
    ...
    expect(reply.text.includes("MarketingClaw")).toBe(true);
  ...
  it("uses configured agent thinking defaults for native /status", async () => {
    const home = await fs.mkdtemp(
      path.join(os.tmpdir(), "marketingclaw-native-status-agent-think-"),
    );
    const targetSessionKey = "agent:main:telegram:123";
    const cfg = markCompleteReplyConfig({
      agents: {
  ...
  it("uses the target session thinking override for native /status", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-native-status-think-"));
    const storePath = path.join(home, "sessions.json");
    const targetSessionKey = "agent:main:telegram:123";
    await seedFastPathSessionStore(storePath, {
      [targetSessionKey]: {
  ...
  it("handles native slash directives before workspace bootstrap", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-native-slash-fast-"));
    const targetSessionKey = "agent:main:telegram:123";
    const cfg = markCompleteReplyConfig({
      agents: {
        defaults: {
  ...
  it("continues native slash goal starts with the rewritten command-safe prompt", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-native-goal-fast-"));
    const targetSessionKey = "agent:main:telegram:123";
    const storePath = path.join(home, "sessions.json");
    const cfg = markCompleteReplyConfig({
      agents: {
  ...
  it("uses native command target session keys during fast bootstrap", () => {
    const result = initFastReplySessionState({
      ...
      cfg: { session: { store: "/tmp/sessions.json" } } as MarketingClawConfig,
      agentId: "main",
      commandAuthorized: true,
  ...
  it("preserves usage footer mode during fast reset bootstrap", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-fast-reset-usage-"));
    const storePath = path.join(home, "sessions.json");
    const sessionKey = "agent:main:telegram:123";
    await seedFastPathSessionStore(storePath, {
      [sessionKey]: {
  ...
  it("keeps the existing session for /reset newline soft during fast bootstrap", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-fast-reset-newline-soft-"));
    const storePath = path.join(home, "sessions.json");
    const sessionKey = "agent:main:telegram:123";
    await seedFastPathSessionStore(storePath, {
      [sessionKey]: {
  ...
  it("keeps the existing session for /reset: soft during fast bootstrap", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-fast-reset-colon-soft-"));
    const storePath = path.join(home, "sessions.json");
    const sessionKey = "agent:main:telegram:123";
    await seedFastPathSessionStore(storePath, {
      [sessionKey]: {
