...
const acpMocks = vi.hoisted(() => ({
  ...
  readAcpSessionEntry: vi.fn<
    (params: { sessionKey: string; cfg?: MarketingClawConfig }) => unknown
  >(() => null),
  readAcpSessionMeta: vi.fn<(params: { sessionKey: string; cfg?: MarketingClawConfig }) => unknown>(
    () => null,
  ),
...
const ttsMocks = vi.hoisted(() => {
  ...
  return {
    ...
    resolveTtsConfig: vi.fn((_cfg: MarketingClawConfig) => ({ mode: "final" })),
  };
});
...
vi.mock("../../tts/tts.js", () => ({
  ...
  resolveTtsConfig: (cfg: MarketingClawConfig) => ttsMocks.resolveTtsConfig(cfg),
}));
...
vi.mock("./dispatch-acp-session.runtime.js", () => ({
  readAcpSessionEntry: (params: { sessionKey: string; cfg?: MarketingClawConfig }) =>
    acpMocks.readAcpSessionEntry(params),
}));
vi.mock("../../tts/tts-config.js", () => ({
  normalizeTtsAutoMode: (value: unknown) => ttsMocks.normalizeTtsAutoMode(value),
  resolveConfiguredTtsMode: (cfg: MarketingClawConfig) => ttsMocks.resolveTtsConfig(cfg).mode,
...
function createMockAcpSessionManager() {
  return {
    resolveSession: (params: { cfg: MarketingClawConfig; sessionKey: string }) => {
...
describe("dispatchReplyFromConfig", () => {
  ...
  it("skips a Telegram topic heartbeat turn while a reply operation is active", async () => {
    ...
    const result = await dispatchReplyFromConfig({
      ctx: buildTestCtx({
        ...
        TransportThreadId: 3731,
        To: "telegram:-1003774691294:topic:3731",
        BodyForAgent: "[MarketingClaw heartbeat poll]",
      }),
      cfg: automaticGroupReplyConfig,
      dispatcher,
  ...
  it("clears stale active reply operations for terminal sessions and retries admission", async () => {
    ...
    const result = await dispatchReplyFromConfig({
      ctx: buildTestCtx({
        ...
        BodyForAgent: "@marketingclaw recover",
      }),
      cfg: automaticGroupReplyConfig,
      dispatcher,
      replyResolver,
  ...
  it("does not pre-latch terminal tool-error suppression when diagnostics are disabled", async () => {
    ...
    await dispatchReplyFromConfig({
      ctx,
      cfg: { diagnostics: { enabled: false } } as MarketingClawConfig,
      dispatcher,
      replyResolver,
    });
  ...
  it("retargets reply_dispatch to a bound generic ACP session before model fallback", async () => {
    ...
    acpMocks.readAcpSessionEntry.mockImplementation(
      (params: { sessionKey: string; cfg?: MarketingClawConfig }) =>
  ...
  it("normalizes accumulated block TTS-only media before final delivery", async () => {
    ...
    replyMediaPathMocks.createReplyMediaPathNormalizer.mockReturnValue(
      async (payload: ReplyPayload) => ({
        ...payload,
        mediaUrl: "/tmp/marketingclaw-media/normalized-tts.ogg",
        mediaUrls: ["/tmp/marketingclaw-media/normalized-tts.ogg"],
      }),
    );
    const dispatcher = createDispatcher();
    const ctx = buildTestCtx({
    ...
    const finalPayload = firstFinalReplyPayload(dispatcher);
    expect(finalPayload?.mediaUrl).toBe("/tmp/marketingclaw-media/normalized-tts.ogg");
    expect(finalPayload?.mediaUrls).toStrictEqual(["/tmp/marketingclaw-media/normalized-tts.ogg"]);
  ...
  it("emits diagnostics when enabled", async () => {
    setNoAbort();
    const cfg = { diagnostics: { enabled: true } } as MarketingClawConfig;
    const dispatcher = createDispatcher();
    const ctx = buildTestCtx({
      Provider: "slack",
      Surface: "slack",
  ...
  it("routes plugin-owned bindings to the owning plugin before generic inbound claim broadcast", async () => {
    ...
    hookMocks.registry.plugins = [{ id: "marketingclaw-codex-app-server", status: "loaded" }];
    hookMocks.runner.runInboundClaimForPluginOutcome.mockResolvedValue({
      status: "handled",
      result: { handled: true },
    });
    sessionBindingMocks.resolveByConversation.mockReturnValue({
      ...
      metadata: {
        ...
        pluginRoot: "/Users/huntharo/github/marketingclaw-app-server",
        data: {
    ...
    const inboundClaimCall = hookMocks.runner.runInboundClaimForPluginOutcome.mock
      ...
      | undefined;
    expect(inboundClaimCall?.[0]).toBe("marketingclaw-codex-app-server");
  ...
  it("stages remote iMessage media before plugin-bound inbound claim metadata reaches Codex", async () => {
    ...
    const stagedPath =
      "/tmp/marketingclaw-proof/.marketingclaw/media/remote-cache/agent-main-imessage/photo.jpg";
    stageSandboxMediaMocks.stageSandboxMedia.mockImplementationOnce(async (paramsUnknown) => {
      ...
      expect(params.workspaceDir).toContain(".marketingclaw/workspace");
    ...
    sessionBindingMocks.resolveByConversation.mockReturnValue({
      ...
      metadata: {
        pluginBindingOwner: "plugin",
        pluginId: "marketingclaw-codex-app-server",
        pluginRoot: "/plugins/marketingclaw-codex-app-server",
        data: {
          kind: "codex-app-server-session",
          version: 1,
          sessionFile: "/tmp/session.jsonl",
  ...
  it("does not run plugin-owned binding delivery when the caller already aborted", async () => {
    ...
    sessionBindingMocks.resolveByConversation.mockReturnValue({
      ...
      metadata: {
        pluginBindingOwner: "plugin",
        pluginId: "marketingclaw-codex-app-server",
        pluginRoot: "/workspace/marketingclaw-app-server",
        data: {
          kind: "codex-app-server-session",
          version: 1,
          sessionFile: "/tmp/session.jsonl",
  ...
  it("falls back to MarketingClaw once per startup when a bound plugin is missing", async () => {
    setNoAbort();
    hookMocks.runner.hasHooks.mockImplementation(
      ((hookName?: string) =>
        hookName === "inbound_claim" || hookName === "message_received") as () => boolean,
    ...
    const replyResolver = vi.fn(
      async () => ({ text: "marketingclaw fallback" }) satisfies ReplyPayload,
    );
  ...
  it("falls back to MarketingClaw when the bound plugin is loaded but has no inbound_claim handler", async () => {
    setNoAbort();
    hookMocks.runner.hasHooks.mockImplementation(
      ((hookName?: string) =>
        hookName === "inbound_claim" || hookName === "message_received") as () => boolean,
  ...
  it("passes the loaded config plus configOverride patch to replyResolver when provided", async () => {
    ...
    let receivedCfg: MarketingClawConfig | undefined;
    const replyResolver = async (
      _ctx: MsgContext,
      _opts?: GetReplyOptions,
      cfgArg?: MarketingClawConfig,
    ) => {
      receivedCfg = cfgArg;
      return { text: "hi" } satisfies ReplyPayload;
    };
  ...
  it("passes the already loaded config to replyResolver when configOverride is not provided", async () => {
    setNoAbort();
    const cfg = { agents: { defaults: { userTimezone: "UTC" } } } as MarketingClawConfig;
    const dispatcher = createDispatcher();
...
describe("sendPolicy deny — suppress delivery, not processing (#53328)", () => {
  ...
  it("passes suppressUserDelivery to tail reply_dispatch when sendPolicy is deny", async () => {
    ...
    await dispatchReplyFromConfig({
      ctx,
      cfg: { diagnostics: { enabled: true } } as MarketingClawConfig,
      dispatcher,
  ...
  it("suppresses tool result delivery when sendPolicy is deny", async () => {
    ...
    const replyResolver = vi.fn(
      async (_ctx: MsgContext, opts?: GetReplyOptions, _cfg?: MarketingClawConfig) => {
  ...
  it("keeps unmentioned plugin-bound fallback from ordinary group agent dispatch", async () => {
    ...
    expect(claimCall[0]).toBe("marketingclaw-codex-app-server");
  ...
  it("lets authorized control commands without CommandSource escape plugin-bound fallback", async () => {
    ...
    const cfg = { messages: { visibleReplies: "message_tool" } } as MarketingClawConfig;
    const dispatcher = createDispatcher();
    ...
    const ctx = buildTestCtx({
      ...
      ChatType: "group",
      GroupSubject: "Dev",
      Body: "/reset@marketingclaw",
      RawBody: "/reset@marketingclaw",
      CommandBody: "/reset@marketingclaw",
  ...
  it("delivers fast auto progress in message-tool-only mode without verbose progress", async () => {
    ...
    const replyResolver = vi.fn(async (_ctx: MsgContext, opts?: GetReplyOptions) => {
      await opts?.onToolResult?.({
        ...
        channelData: { marketingclawProgressKind: "fast-mode-auto" },
      });
      return { text: "NO_REPLY" } satisfies ReplyPayload;
    });
  ...
  it("preserves configured message-tool delivery for internal WebChat direct replies", async () => {
    ...
    const result = await dispatchReplyFromConfig({
      ...
      cfg: { messages: { visibleReplies: "message_tool" } } as MarketingClawConfig,
      dispatcher,
      replyResolver,
    });
  ...
  it("lets config restore automatic Codex direct source delivery", async () => {
    ...
    const result = await dispatchReplyFromConfig({
      ...
      cfg: { messages: { visibleReplies: "automatic" } } as MarketingClawConfig,
      dispatcher,
      replyResolver,
    });
  ...
  it("falls back when a channel precomputed message-tool-only delivery but the message tool is unavailable", async () => {
    ...
    const result = await dispatchReplyFromConfig({
      ...
      cfg: { tools: { allow: ["read"] } } as MarketingClawConfig,
      dispatcher,
      replyResolver,
      replyOptions: {
        sourceReplyDeliveryMode: "message_tool_only",
