...
vi.mock("../../agents/acp-spawn.js", () => ({
  resolveAcpSpawnRuntimePolicyError: (params: { cfg?: MarketingClawConfig }) =>
...
function createDiscordParams(commandBody: string, cfg: MarketingClawConfig = baseCfg) {
...
function createThreadParams(commandBody: string, cfg: MarketingClawConfig = baseCfg) {
...
function createConversationParams(
  ...
  cfg: MarketingClawConfig = baseCfg,
) {
  const params = buildCommandTestParams(commandBody, cfg, {
...
async function runDiscordAcpCommand(commandBody: string, cfg: MarketingClawConfig = baseCfg) {
  return handleAcpCommand(createDiscordParams(commandBody, cfg), true);
}

async function runThreadAcpCommand(commandBody: string, cfg: MarketingClawConfig = baseCfg) {
  return handleAcpCommand(createThreadParams(commandBody, cfg), true);
}

async function runTelegramAcpCommand(commandBody: string, cfg: MarketingClawConfig = baseCfg) {
...
async function runTelegramDmAcpCommand(commandBody: string, cfg: MarketingClawConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
async function runSlackDmAcpCommand(commandBody: string, cfg: MarketingClawConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
function createMatrixThreadParams(commandBody: string, cfg: MarketingClawConfig = baseCfg) {
...
async function runMatrixAcpCommand(commandBody: string, cfg: MarketingClawConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
async function runMatrixThreadAcpCommand(commandBody: string, cfg: MarketingClawConfig = baseCfg) {
  return handleAcpCommand(createMatrixThreadParams(commandBody, cfg), true);
}

async function runFeishuDmAcpCommand(commandBody: string, cfg: MarketingClawConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
async function runLineDmAcpCommand(commandBody: string, cfg: MarketingClawConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
async function runIMessageDmAcpCommand(commandBody: string, cfg: MarketingClawConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
describe("/acp command", () => {
  ...
  it("inherits the target agent workspace when /acp spawn omits --cwd", async () => {
    ...
    const workspace = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-codex-"));
    try {
      const cfg = {
        ...baseCfg,
        agents: {
  ...
  it("sanitizes leaked task and runtime details in ACP status output", async () => {
    ...
    expect(result?.reply?.text).not.toContain("MarketingClaw runtime context (internal):");
