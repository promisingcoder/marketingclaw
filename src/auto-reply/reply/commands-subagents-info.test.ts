...
const TEST_SESSION_STORE_PATH = path.join(
  os.tmpdir(),
  `marketingclaw-commands-subagents-info-${process.pid}.json`,
);

function buildCommandTestConfig(): MarketingClawConfig {
  return {
    ...baseCommandTestConfig,
    session: {
...
describe("subagents info", () => {
  ...
  it("sanitizes leaked task details in /subagents info", () => {
    ...
    expect(text).not.toContain("MarketingClaw runtime context (internal):");
