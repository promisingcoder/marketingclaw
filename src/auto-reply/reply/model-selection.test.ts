...
describe("createModelSelectionState catalog loading", () => {
  ...
  it("preserves OpenAI API-key session auth when model policy explicitly pins MarketingClaw", async () => {
...
describe("createModelSelectionState respects session model override", () => {
  ...
  it("adopts a concurrent valid model while repairing a stale override", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-model-repair-race-"));
  ...
  it("rejects stale-model repair when the session rotates during persistence", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-model-repair-rotation-"));
    const storePath = path.join(tempRoot, "sessions.json");
    const cfg = {
      agents: {
        defaults: {
