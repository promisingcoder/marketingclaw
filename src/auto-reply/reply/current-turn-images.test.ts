...
const originalStateDirEnv = process.env.MARKETINGCLAW_STATE_DIR;

function restoreProcessState() {
  if (originalStateDirEnv === undefined) {
    deleteTestEnvValue("MARKETINGCLAW_STATE_DIR");
  } else {
    setTestEnvValue("MARKETINGCLAW_STATE_DIR", originalStateDirEnv);
  }
}
...
describe("resolveCurrentTurnImages", () => {
  ...
  it("hydrates Telegram-style state-relative media into native prompt images", async () => {
    await withTempDir({ prefix: "marketingclaw-current-turn-images-" }, async (base) => {
  ...
  it("appends extracted PDF page images without dropping current image attachments", async () => {
    await withTempDir({ prefix: "marketingclaw-current-turn-pdf-images-" }, async (base) => {
  ...
  it("orders extracted PDF page images before later current image attachments", async () => {
    await withTempDir({ prefix: "marketingclaw-current-turn-pdf-order-" }, async (base) => {
      const imagePath = path.join(base, "photo.png");
