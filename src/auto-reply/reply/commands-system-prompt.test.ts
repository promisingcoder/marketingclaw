...
const { createMarketingClawCodingToolsMock } = vi.hoisted(() => ({
  createMarketingClawCodingToolsMock: vi.fn(() => []),
}));
...
vi.mock("../../agents/agent-tools.js", () => ({
  createMarketingClawCodingTools: createMarketingClawCodingToolsMock,
}));
...
describe("resolveCommandsSystemPromptBundle", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    createMarketingClawCodingToolsMock.mockClear();
  ...
  it("opts command tool builds into gateway subagent binding", async () => {
    ...
    const toolParams = requireFirstArg(
      vi.mocked(createMarketingClawCodingTools),
  ...
  it("uses materialized sandbox skill paths for sandbox command prompts", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-command-sandbox-skills-"));
    try {
      ...
      vi.mocked(resolveReusableWorkspaceSkillSnapshot).mockReturnValue({
        snapshot: {
          prompt:
            "<available_skills>~/.npm-global/lib/node_modules/marketingclaw/skills/gog/SKILL.md</available_skills>",
          skills: [],
          resolvedSkills: [],
        },
        shouldRefresh: false,
  ...
  it("uses config-backed prompt settings for the target agent", async () => {
    ...
    createMarketingClawCodingToolsMock.mockReturnValue([{ name: "sessions_spawn" }] as never);
    const params = makeParams();
    params.cfg = {
      agents: {
        defaults: {
