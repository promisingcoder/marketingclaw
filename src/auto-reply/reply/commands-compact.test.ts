...
function requireResolveSessionAgentIdCall(index = 0) {
  ...
  )?.[0] as { sessionKey?: string; config?: MarketingClawConfig } | undefined;
  if (!call) {
...
function requireResolveAgentDirCall(index = 0) {
  const call = resolveAgentDirMock.mock.calls[index] as [MarketingClawConfig, string] | undefined;
  if (!call) {
...
describe("handleCompactCommand", () => {
  ...
  it("routes manual compaction with explicit trigger and context metadata", async () => {
    ...
    const result = await handleCompactCommand(
      {
        ...buildCompactParams("/compact", {
          commands: { text: true },
          channels: { whatsapp: { allowFrom: ["*"] } },
          session: { store: "/tmp/marketingclaw-session-store.json" },
        } as MarketingClawConfig),
        ctx: {
          Provider: "whatsapp",
          Surface: "whatsapp",
        ...
        agentDir: "/tmp/marketingclaw-agent-compact",
    ...
    expect(call.agentDir).toBe("/tmp/marketingclaw-agent-compact");
