...
const resolvePreferredMarketingClawTmpDirMock = vi.hoisted(() => vi.fn(() => os.tmpdir()));
const resolveTaskScriptPathMock = vi.hoisted(() =>
  vi.fn((env: Record<string, string | undefined>) => {
    const home = env.USERPROFILE || env.HOME || os.homedir();
    return path.join(home, ".marketingclaw", "gateway.cmd");
  }),
);
...
vi.mock("./tmp-marketingclaw-dir.js", () => ({
...
describe("relaunchGatewayScheduledTask", () => {
  ...
  beforeEach(() => {
    spawnMock.mockReset();
    resolvePreferredMarketingClawTmpDirMock.mockReset();
    resolvePreferredMarketingClawTmpDirMock.mockReturnValue(os.tmpdir());
  ...
  it("writes a detached schtasks relaunch helper", () => {
    ...
    const result = relaunchGatewayScheduledTask({ MARKETINGCLAW_PROFILE: "work" });
    const cmdExePath = getWindowsCmdExePath();

    expect(result.ok).toBe(true);
    expect(result.method).toBe("schtasks");
    expect(result.tried).toContain('schtasks /Run /TN "MarketingClaw Gateway (work)"');
    ...
    expect(script).toContain(
      'marketingclaw restart attempt source=windows-task-handoff target="MarketingClaw Gateway (work)"',
    );
    expect(script).toContain(
      `powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command "$task = Get-ScheduledTask -TaskName 'MarketingClaw Gateway (work)' -ErrorAction SilentlyContinue; if ($null -ne $task -and $task.State -eq 'Running') { exit 0 }; exit 1" >nul 2>&1`,
    );
    expect(script).not.toContain("findstr");
    expect(script).toContain('schtasks /Run /TN "MarketingClaw Gateway (work)" >>');
    expect(script.indexOf("powershell.exe -NoProfile")).toBeLessThan(
      script.indexOf('schtasks /Run /TN "MarketingClaw Gateway (work)"'),
    );
    expect(script).toContain('del "%~f0" >nul 2>&1');
  });

  it("prefers MARKETINGCLAW_WINDOWS_TASK_NAME overrides", () => {
    spawnMock.mockImplementation((_file: string, args: string[]) => {
      createdScriptPaths.add(decodeCmdPathArg(args[3]));
      return { unref: vi.fn() };
    });
    ...
    const scriptPath = [...createdScriptPaths][0];
    const script = fs.readFileSync(scriptPath, "utf8");
    expect(script).toContain('schtasks /Run /TN "MarketingClaw Gateway (custom)" >>');
  });
  ...
  it("escapes custom task names in the PowerShell running-task probe", () => {
    ...
    relaunchGatewayScheduledTask({
      MARKETINGCLAW_WINDOWS_TASK_NAME: "MarketingClaw Gateway (Bob's work)",
    });

    const scriptPath = [...createdScriptPaths][0];
    const script = fs.readFileSync(scriptPath, "utf8");
    expect(script).toContain(
      `-Command "$task = Get-ScheduledTask -TaskName 'MarketingClaw Gateway (Bob''s work)' -ErrorAction SilentlyContinue; if ($null -ne $task -and $task.State -eq 'Running') { exit 0 }; exit 1"`,
    );
    expect(script).not.toContain("findstr");
  });
  ...
  it("quotes the cmd /c script path when temp paths contain metacharacters", () => {
    const unref = vi.fn();
    const metacharTmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw&(restart)-"));
    createdTmpDirs.add(metacharTmpDir);
    resolvePreferredMarketingClawTmpDirMock.mockReturnValue(metacharTmpDir);
    spawnMock.mockReturnValue({ unref });

    relaunchGatewayScheduledTask({ MARKETINGCLAW_PROFILE: "work" });
  ...
  it("includes startup fallback", () => {
    const taskScriptDir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-state-"));
