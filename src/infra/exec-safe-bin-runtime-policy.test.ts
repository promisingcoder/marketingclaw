...
describe("exec safe-bin runtime policy", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "marketingclaw-safe-bin-trusted-symlink-" }, async (root) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "marketingclaw-safe-bin-trusted-retarget-" }, async (root) => {
        const trustedDir = path.join(root, "bin");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "marketingclaw-safe-bin-trusted-nonexec-" }, async (root) => {
        const trustedDir = path.join(root, "bin");
  ...
  it("emits runtime warning when explicitly trusted dir is writable", async () => {
    ...
    await withTempDir({ prefix: "marketingclaw-safe-bin-runtime-" }, async (dir) => {
      try {
