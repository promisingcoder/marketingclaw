...
describe("resolveLocalVitestMaxWorkers", () => {
  ...
  it("lets MARKETINGCLAW_VITEST_MAX_WORKERS override the inferred cap", () => {
    expect(
      resolveLocalVitestMaxWorkers(
        {
  ...
  it("respects the legacy MARKETINGCLAW_TEST_WORKERS override too", () => {
    expect(
      resolveLocalVitestMaxWorkers(
        {
          MARKETINGCLAW_TEST_WORKERS: "3",
        },
        {
          cpuCount: 16,
          loadAverage1m: 0,
...
describe("resolveLocalVitestScheduling", () => {
  ...
  it("allows disabling the system throttle probe explicitly", () => {
    expect(
      resolveLocalVitestScheduling(
        {
          MARKETINGCLAW_VITEST_DISABLE_SYSTEM_THROTTLE: "1",
        },
        {
          cpuCount: 16,
          loadAverage1m: 0.5,
...
describe("base vitest config", () => {
  ...
  it("classifies Crabbox shared dependencies as external dependencies", () => {
    expect(baseConfig.test?.deps?.moduleDirectories).toEqual([
      "/node_modules/",
      "/marketingclaw-pnpm-node-modules/",
    ]);
    ...
    expect(
      externalPatterns.some(
        (pattern) =>
          ...
          pattern.test("/tmp/marketingclaw-pnpm-node-modules/some-dep/dist/index.mjs"),
      ),
    ).toBe(true);
    expect(
      externalPatterns.some(
        (pattern) =>
          pattern instanceof RegExp &&
          pattern.test("/tmp/marketingclaw-pnpm-node-modules/vite/dist/client/env.mjs"),
      ),
    ).toBe(false);
  });
});
...
describe("test scripts", () => {
  it("keeps test scripts on the native thread-first configs", () => {
    ...
    expect(pkg.scripts?.["test:gateway"]).toBe(
      "node scripts/run-with-env.mjs MARKETINGCLAW_GATEWAY_PROJECT_SHARDS=1 -- node scripts/run-vitest.mjs run --config test/vitest/vitest.gateway.config.ts",
    );
