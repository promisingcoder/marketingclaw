...
vi.mock("./marketingclaw-root.js", () => ({
  resolveMarketingClawPackageRoot: vi.fn(async () => null),
  resolveMarketingClawPackageRootSync: vi.fn(() => null),
}));
...
let marketingclawRoot: typeof import("./marketingclaw-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    marketingclawRoot = await import("./marketingclaw-root.js");
  });
  ...
  it("resolves dist control-ui index path for symlinked argv1 via realpath", async () => {
    const pkgRoot = abs("fixtures/bun-global/marketingclaw");
    const wrapperArgv1 = abs("fixtures/bin/marketingclaw");
  ...
  it("uses resolveMarketingClawPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/marketingclaw");
    (
      marketingclawRoot.resolveMarketingClawPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/marketingclaw"))).resolves.toBe(
      path.join(pkgRoot, "dist", "control-ui", "index.html"),
    );
  });
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "marketingclaw" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "marketingclaw.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-marketingclaw");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", () => {
    const pkgRoot = abs("fixtures/marketingclaw-bundle");
    (
      marketingclawRoot.resolveMarketingClawPackageRootSync as unknown as ReturnType<typeof vi.fn>
  ...
  it("prefers packaged app Control UI assets in Contents/Resources", () => {
    const execPath = abs("fixtures/MarketingClaw.app/Contents/MacOS/MarketingClaw");
    const bundledUiDir = abs("fixtures/MarketingClaw.app/Contents/Resources/control-ui");
  ...
  it("detects package-proven control-ui roots", () => {
    const pkgRoot = abs("fixtures/marketingclaw-package-root");
    const uiDir = path.join(pkgRoot, "dist", "control-ui");
    setDir(uiDir);
    setFile(path.join(uiDir, "index.html"), "<html></html>\n");
    (
