// Covers preferred MarketingClaw temp directory resolution.
...
} from "./tmp-marketingclaw-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredMarketingClawTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `marketingclaw-${uid}`);
}
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredMarketingClawTmpDir({
    ...
    lstatSync: vi.fn((target: string) => {
      if (target === POSIX_MARKETINGCLAW_TMP_DIR) {
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredMarketingClawTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredMarketingClawTmpDir", () => {
  it("prefers /tmp/marketingclaw when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_MARKETINGCLAW_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/marketingclaw when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_MARKETINGCLAW_TMP_DIR, {
      recursive: true,
      mode: 0o700,
    });
    expect(tmpdir).not.toHaveBeenCalled();
  ...
  it.each([
    {
      name: "falls back to os.tmpdir()/marketingclaw when /tmp/marketingclaw is not a directory",
      lstatSync: vi.fn(() => makeDirStat({ isDirectory: false, mode: 0o100644 })),
    },
    {
      name: "falls back to os.tmpdir()/marketingclaw when /tmp is not writable",
      lstatSync: vi.fn(() => {
        throw nodeErrorWithCode("ENOENT");
      }),
    ...
    {
      name: "falls back when /tmp/marketingclaw exists but is not writable",
    ...
    {
      name: "falls back when /tmp/marketingclaw is a symlink",
      lstatSync: symlinkTmpDirLstat(),
    },
    {
      name: "falls back when /tmp/marketingclaw is not owned by the current user",
      lstatSync: vi.fn(() => makeDirStat({ uid: 0 })),
    },
    {
      name: "falls back when /tmp/marketingclaw is group/other writable",
  ...
  it("repairs existing /tmp/marketingclaw permissions when they are too broad", () => {
    let preferredMode = 0o40777;
    const chmodSync = vi.fn((target: string, mode: number) => {
      if (target === POSIX_MARKETINGCLAW_TMP_DIR && mode === 0o700) {
    ...
    expect(resolved).toBe(POSIX_MARKETINGCLAW_TMP_DIR);
    expect(chmodSync).toHaveBeenCalledWith(POSIX_MARKETINGCLAW_TMP_DIR, 0o700);
    expect(warn).toHaveBeenCalledWith(
      `[marketingclaw] tightened permissions on temp dir: ${POSIX_MARKETINGCLAW_TMP_DIR}`,
    );
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("repairs /tmp/marketingclaw after create when the initial mode stays too broad", () => {
    ...
    const chmodSync = vi.fn((target: string, mode: number) => {
      chmodCalls += 1;
      if (target === POSIX_MARKETINGCLAW_TMP_DIR && mode === 0o700 && chmodCalls > 1) {
        preferredMode = 0o40700;
      }
    });
    const warn = vi.fn();
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback MarketingClaw temp dir/);
  });
  ...
  it("uses an unscoped fallback suffix when process uid is unavailable", () => {
    ...
    const fallbackPath = path.join(tmpdirPath, "marketingclaw");
  ...
  it("repairs existing fallback directory when permissions are too broad", () => {
    ...
    expect(chmodSync).toHaveBeenCalledWith(fallbackPath, 0o700);
    expect(warn).toHaveBeenCalledWith(
      `[marketingclaw] tightened permissions on temp dir: ${fallbackPath}`,
    );
  });

  it("uses /tmp/marketingclaw when another process tightened permissions before repair", () => {
  ...
  it("uses /tmp/marketingclaw when chmod loses a concurrent repair race", () => {
    const chmodSync = vi.fn((target: string, mode: number) => {
      if (target === POSIX_MARKETINGCLAW_TMP_DIR && mode === 0o700) {
  ...
  it("throws when the fallback directory cannot be created", () => {
    expect(() =>
      resolvePreferredMarketingClawTmpDir({
        accessSync: readOnlyTmpAccessSync(),
        lstatSync: vi.fn((target: string) => {
          if (target === POSIX_MARKETINGCLAW_TMP_DIR || target === fallbackTmp()) {
            throw nodeErrorWithCode("ENOENT");
          }
          return secureDirStat();
        }),
    ...
    ).toThrow(/Unable to create fallback MarketingClaw temp dir/);
  });
  ...
  it("skips the POSIX preferred path on Windows even when /tmp is accessible (#60713)", () => {
    ...
    // create it), the previous code path returned `/tmp/marketingclaw` and routed
    // log files / TTS temp files there instead of `%TEMP%\marketingclaw`. The
    // platform: "win32" branch must skip the POSIX path entirely.
    const winFallback = path.win32.join("C:\\Users\\u\\AppData\\Local\\Temp", "marketingclaw-501");
    const accessSync = vi.fn();
    const lstatSync = vi.fn((target: string) => {
      if (target === POSIX_MARKETINGCLAW_TMP_DIR || target === winFallback) {
        return secureDirStat();
      }
      throw nodeErrorWithCode("ENOENT");
    });
    ...
    const result = resolvePreferredMarketingClawTmpDir({
      platform: "win32",
      accessSync,
      lstatSync,
      mkdirSync,
    ...
    expect(result).not.toBe(POSIX_MARKETINGCLAW_TMP_DIR);
    expect(tmpdir).toHaveBeenCalled();
  });
  ...
  it("still uses the POSIX preferred path on non-Windows platforms when available", () => {
    ...
    expect(result).toBe(POSIX_MARKETINGCLAW_TMP_DIR);
  });
});
