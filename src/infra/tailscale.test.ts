...
describe("tailscale helpers", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["MARKETINGCLAW_TEST_TAILSCALE_BINARY", "NODE_ENV", "VITEST"]);
    process.env.MARKETINGCLAW_TEST_TAILSCALE_BINARY = "tailscale";
  ...
  it("allows the test binary override in explicit test environments", () => {
    process.env.MARKETINGCLAW_TEST_TAILSCALE_BINARY = "/tmp/test-tailscale";
  ...
  it("ignores the test binary override outside test environments", () => {
    process.env.MARKETINGCLAW_TEST_TAILSCALE_BINARY = "/tmp/attacker-tailscale";
  ...
  it("enableTailscaleServe passes a configured service name", async () => {
    ...
    await enableTailscaleServe(3000, exec as never, "svc:marketingclaw");
    ...
    expectExecCall(
      ...
      1,
      tailscaleBin,
      ["serve", "--service=svc:marketingclaw", "--bg", "--yes", "3000"],
      {
        maxBuffer: 200_000,
        timeoutMs: 15_000,
      },
  ...
  it("disableTailscaleServe disables only the configured service name", async () => {
    ...
    await disableTailscaleServe(exec as never, "svc:marketingclaw");

    expect(exec).toHaveBeenCalledTimes(1);
    expectExecCall(exec, 1, tailscaleBin, ["serve", "clear", "svc:marketingclaw"], {
      maxBuffer: 200_000,
      timeoutMs: 15_000,
    });
  });
