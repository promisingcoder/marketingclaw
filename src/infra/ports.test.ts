...
describe("ports helpers", () => {
  ...
  it("prints an MarketingClaw-specific hint when port details look like another MarketingClaw instance", async () => {
    const runtime = {
      error: vi.fn(),
      log: vi.fn(),
      exit: vi.fn() as unknown as (code: number) => never,
    ...
    await handlePortError(
      new PortInUseError(18789, "node dist/index.js marketingclaw gateway"),
      18789,
      "gateway start",
      runtime,
    ).catch(() => {});
    ...
    const messages = runtime.error.mock.calls.map((call) => stripAnsi(String(call[0] ?? "")));
    expect(messages.join("\n")).toContain("another MarketingClaw instance is already running");
  });
});
...
describeUnix("inspectPortUsage", () => {
  ...
  it("falls back to ss when lsof is unavailable", async () => {
    ...
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command === "ps") {
        if (argv.includes("command=")) {
          return {
            stdout: "node /tmp/marketingclaw/dist/index.js gateway --port 18789\n",
            stderr: "",
            code: 0,
          };
        }
    ...
    try {
      ...
      expect(result.listeners[0]?.commandLine).toContain("marketingclaw");
  ...
  it("does not match ss listener ports by substring", async () => {
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command === "ss") {
        return {
          stdout: 'LISTEN 0 4096 127.0.0.1:18789 0.0.0.0:* users:(("marketingclaw",pid=123,fd=12))',
          stderr: "",
          code: 0,
        };
      }
      ...
      if (command === "ss") {
        return {
          stdout: [
            ...
            'LISTEN 0 4096 127.0.0.1:18789 0.0.0.0:* users:(("marketingclaw",pid=123,fd=12))',
      ...
      if (command === "ps") {
        ...
        if (argv.includes("command=")) {
          return {
            stdout:
              pid === "111"
                ? "node /tmp/newer-marketingclaw/dist/index.js logs --follow\n"
                : pid === "222"
                  ? "node /tmp/older-marketingclaw/dist/index.js gateway run\n"
  ...
  it("deduplicates repeated lsof listener records for one process address", async () => {
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command === "ps") {
        if (argv.includes("command=")) {
          return {
            stdout: "node /tmp/marketingclaw/dist/index.js gateway run\n",
            stderr: "",
            code: 0,
          };
        }
  ...
  it("preserves malformed lsof pid records as unknown-pid listener diagnostics", async () => {
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command === "ps") {
        if (argv.includes("command=")) {
          return {
            stdout: "bun /tmp/marketingclaw/dist/index.js gateway run\n",
            stderr: "",
            code: 0,
          };
        }
  ...
  it("reports multiple lsof socket records for one process", async () => {
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command === "ps") {
        if (argv.includes("command=")) {
          return {
            stdout: "node /tmp/newer-marketingclaw/dist/index.js logs --follow\n",
            stderr: "",
            code: 0,
          };
        }
...
describe("inspectPortUsage on Windows", () => {
  it("reports established gateway client connections from netstat", async () => {
    ...
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command === getWindowsPowerShellExePath()) {
        return {
          stdout:
            '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\me\\AppData\\Roaming\\npm\\node_modules\\marketingclaw\\dist\\index.js logs --follow\r\n',
          stderr: "",
          code: 0,
        };
      }
    ...
    expect(result.connections[0]?.commandLine).toContain("marketingclaw");
  });

  it("uses PowerShell process command lines to classify MarketingClaw listeners", async () => {
    setPlatform("win32");
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      const [command] = argv;
      if (command === getWindowsSystem32ExePath("netstat.exe")) {
      ...
      if (command === getWindowsPowerShellExePath()) {
        return {
          stdout:
            '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\me\\AppData\\Roaming\\npm\\node_modules\\marketingclaw\\dist\\index.js gateway run\r\n',
          stderr: "",
          code: 0,
        };
      }
  ...
  it("reports localized Windows listener rows without requiring English state text", async () => {
    ...
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command === getWindowsPowerShellExePath()) {
        return {
          stdout: "node.exe C:\\marketingclaw\\dist\\index.js gateway run\r\n",
          stderr: "",
          code: 0,
        };
      }
  ...
  it("falls back to wmic when PowerShell cannot read the command line", async () => {
    ...
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command === getWindowsWmicExePath()) {
        return {
          stdout: "CommandLine=node.exe C:\\marketingclaw\\dist\\index.js gateway run\r\n",
          stderr: "",
          code: 0,
        };
      }
