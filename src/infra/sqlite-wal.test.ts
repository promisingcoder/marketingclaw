...
describe("sqlite WAL maintenance", () => {
  ...
  it("uses rollback journaling for databases on NFS-backed volumes", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-sqlite-nfs-"));
    try {
      const db = createMockDb();
      vi.spyOn(process, "platform", "get").mockReturnValue("linux");
      ...
      const maintenance = configureSqliteWalMaintenance(db, {
        ...
        databasePath: path.join(tempDir, "missing", "marketingclaw.sqlite"),
      });
  ...
  it.each([
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-sqlite-network-"));
    try {
      const db = createMockDb();
      vi.spyOn(process, "platform", "get").mockReturnValue("linux");
      ...
      configureSqliteWalMaintenance(db, {
        checkpointIntervalMs: 0,
        databasePath: path.join(tempDir, "marketingclaw.sqlite"),
      });
  ...
  it.each([
    String.raw`\\server\share\marketingclaw.sqlite`,
    String.raw`\\?\UNC\server\share\marketingclaw.sqlite`,
    "//server/share/marketingclaw.sqlite",
    "//?/UNC/server/share/marketingclaw.sqlite",
  ...
  it("uses rollback journaling for mapped Windows network drives", () => {
    const db = createMockDb();
    const databasePath = String.raw`Z:\state\marketingclaw.sqlite`;
    vi.spyOn(process, "platform", "get").mockReturnValue("win32");
    const realpath = vi
      .spyOn(fs.realpathSync, "native")
      .mockReturnValue(String.raw`\\server\share\state\marketingclaw.sqlite`);
  ...
  it("does not treat namespaced Windows local drives as UNC paths", () => {
    const db = createMockDb();
    const databasePath = String.raw`\\?\C:\state\marketingclaw.sqlite`;
    vi.spyOn(process, "platform", "get").mockReturnValue("win32");
  ...
  it("uses rollback journaling when Windows cannot classify an opened drive path", () => {
    const db = createMockDb();
    const databasePath = String.raw`Z:\restricted\marketingclaw.sqlite`;
    vi.spyOn(process, "platform", "get").mockReturnValue("win32");
  ...
  it("refuses fuse.sshfs mountinfo entries", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-sqlite-sshfs-"));
    try {
      const db = createMockDb();
      vi.spyOn(process, "platform", "get").mockReturnValue("linux");
      vi.spyOn(fs, "statfsSync").mockReturnValue(statfsFixture(0));
  ...
  it("refuses symlinked paths into fuse.sshfs mounts", () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-sqlite-sshfs-link-"));
    ...
    try {
      ...
      expect(() =>
        configureSqliteWalMaintenance(createMockDb(), {
          checkpointIntervalMs: 0,
          databasePath: path.join(linkedDir, "marketingclaw.sqlite"),
        }),
  ...
  it("matches raw mount paths when the existing path canonicalizes elsewhere", () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-sqlite-sshfs-prefix-"));
    ...
    try {
      ...
      expect(() =>
        configureSqliteWalMaintenance(createMockDb(), {
          checkpointIntervalMs: 0,
          databasePath: path.join(rawMountDir, "marketingclaw.sqlite"),
        }),
      ).toThrow(/SSHFS.*refusing to open/);
    } finally {
      fs.rmSync(tempDir, { recursive: true, force: true });
  ...
  it("uses macOS SMB mount filesystem names", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-sqlite-smb-"));
    try {
      const db = createMockDb();
      vi.spyOn(process, "platform", "get").mockReturnValue("darwin");
      vi.spyOn(fs, "statfsSync").mockReturnValue(statfsFixture(0));
  ...
  it.each([
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-sqlite-sshfs-macfuse-"));
    try {
      const db = createMockDb();
      vi.spyOn(process, "platform", "get").mockReturnValue("darwin");
      vi.spyOn(fs, "statfsSync").mockReturnValue(statfsFixture(0));
  ...
  it("keeps WAL enabled for non-remote macFUSE mounts", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-sqlite-macfuse-"));
    try {
      const db = createMockDb();
      vi.spyOn(process, "platform", "get").mockReturnValue("darwin");
      vi.spyOn(fs, "statfsSync").mockReturnValue(statfsFixture(0));
