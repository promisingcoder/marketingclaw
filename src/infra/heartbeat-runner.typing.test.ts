...
function createHeartbeatConfig(params: {
  tmpDir: string;
  storePath: string;
  session?: MarketingClawConfig["session"];
...
async function seedTelegramSession(storePath: string, cfg: MarketingClawConfig) {
...
function expectTypingCall(
  ...
  expected: { cfg: MarketingClawConfig; to: string },
) {
