...
export async function handlePortError(
  ...
  if (err instanceof PortInUseError || (isErrno(err) && err.code === "EADDRINUSE")) {
    ...
    if (details) {
      ...
      if (/marketingclaw|src\/index\.ts|dist\/index\.js/.test(details)) {
        runtime.error(
          warn(
            "It looks like another MarketingClaw instance is already running. Stop it or pick a different port.",
          ),
        );
      }
    }
