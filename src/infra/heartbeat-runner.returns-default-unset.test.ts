...
beforeAll(async () => {
  ...
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-heartbeat-suite-"));
});
...
describe("resolveHeartbeatPrompt", () => {
  it.each([
    { name: "default prompt", cfg: {} as MarketingClawConfig, expected: HEARTBEAT_PROMPT },
    {
...
describe("runHeartbeatOnce", () => {
  ...
  it.each([
    ["the heartbeat main session", (cfg: MarketingClawConfig) => resolveMainSessionKey(cfg)],
  ...
  it.each([
    {
      ...
      message: "Group alert",
      applyOverride: ({ cfg, sessionKey }: { cfg: MarketingClawConfig; sessionKey: string }) => {
  ...
  it("loads the default agent session from templated stores", async () => {
    const tmpDir = await createCaseDir("marketingclaw-hb");
...
`,
  ...
  it("keeps non-task HEARTBEAT.md context while stripping blank-line-separated task blocks", async () => {
    const tmpDir = await createCaseDir("marketingclaw-hb-tasks-context");
    const storePath = path.join(tmpDir, "sessions.json");
    const workspaceDir = path.join(tmpDir, "workspace");
    await fs.mkdir(workspaceDir, { recursive: true });
    await fs.writeFile(
...
`,
  ...
  it("strips documented unindented task entries while keeping following top-level bullets", async () => {
    const tmpDir = await createCaseDir("marketingclaw-hb-unindented-tasks-context");
    const storePath = path.join(tmpDir, "sessions.json");
    const workspaceDir = path.join(tmpDir, "workspace");
    await fs.mkdir(workspaceDir, { recursive: true });
    await fs.writeFile(
