...
describe("startProxy", () => {
  ...
  function writeTempCa(contents = "proxy-ca"): string {
    const dir = mkdtempSync(path.join(os.tmpdir(), "marketingclaw-proxy-lifecycle-ca-"));
    tempDirs.push(dir);
  ...
  it("uses MARKETINGCLAW_PROXY_URL when config proxyUrl is omitted", async () => {
    process.env["MARKETINGCLAW_PROXY_URL"] = "http://127.0.0.1:3128";
  ...
  it("prefers config proxyUrl over MARKETINGCLAW_PROXY_URL", async () => {
    process.env["MARKETINGCLAW_PROXY_URL"] = "http://127.0.0.1:3128";
  ...
  it("uses HTTPS proxy URLs from MARKETINGCLAW_PROXY_URL", async () => {
    process.env["MARKETINGCLAW_PROXY_URL"] = "https://127.0.0.1:3128";
  ...
  it("passes configured proxy CA trust to Proxyline", async () => {
    ...
    expect(process.env["MARKETINGCLAW_PROXY_CA_FILE"]).toBe(caFile);
    expect(installGlobalProxyMock).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it("does not load configured proxy CA files for plain HTTP proxy URLs", async () => {
    const missingCaFile = path.join(os.tmpdir(), "marketingclaw-missing-http-proxy-ca.pem");
  ...
  it("sets process proxy env vars for inherited clients", async () => {
    ...
    expect(process.env["MARKETINGCLAW_PROXY_LOOPBACK_MODE"]).toBe("gateway-only");
  });
  ...
  it("persists loopbackMode in env for forked child CLIs", async () => {
    ...
    expect(process.env["MARKETINGCLAW_PROXY_LOOPBACK_MODE"]).toBe("block");
  ...
  it("restores previous proxy env and stops Proxyline on stop", async () => {
    ...
    expect(process.env["MARKETINGCLAW_PROXY_ACTIVE"]).toBeUndefined();
    expect(proxylineStopMock).toHaveBeenCalledOnce();
    expect(forceResetGlobalDispatcherMock).toHaveBeenCalledTimes(2);
  });
