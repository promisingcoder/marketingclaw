...
const baseZoneOpts: WideAreaGatewayZoneOpts = {
  domain: "marketingclaw.internal.",
  gatewayPort: 18789,
  displayName: "Mac Studio (MarketingClaw)",
...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { value: "marketingclaw.internal", expected: "marketingclaw.internal." },
    { value: "marketingclaw.internal.", expected: "marketingclaw.internal." },
    { value: "  marketingclaw.internal  ", expected: "marketingclaw.internal." },
    { value: "", expected: null },
    { value: "   ", expected: null },
  ...
  it.each(["../../x", "foo/bar", "foo\\bar", "evil\nrecords", "marketingclaw..internal"])(
  ...
  it.each([
    {
      ...
      params: {
        env: { MARKETINGCLAW_WIDE_AREA_DOMAIN: "env.internal" } as NodeJS.ProcessEnv,
    ...
    },
    {
      ...
      params: {
        env: { MARKETINGCLAW_WIDE_AREA_DOMAIN: "   " } as NodeJS.ProcessEnv,
        configDomain: " ",
      },
      expected: null,
    },
    ...
    },
    {
      ...
      params: {
        env: { MARKETINGCLAW_WIDE_AREA_DOMAIN: "foo/bar" } as NodeJS.ProcessEnv,
      },
      expected: null,
    },
  ...
  it("builds valid zone paths under the DNS config directory", () => {
    ...
    const zonePath = getWideAreaZonePath("marketingclaw.internal.");

    expect(zonePath).toBe(path.join(dnsDir, "marketingclaw.internal.db"));
    expect(path.relative(dnsDir, zonePath)).toBe("marketingclaw.internal.db");
  });
});
...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    ...
    expectZoneRecords(txt, [
      `$ORIGIN marketingclaw.internal.`,
      `studio-london IN A 100.123.224.76`,
      `studio-london IN AAAA fd7a:115c:a1e0::8801:e04c`,
      `_marketclaw-gw._tcp IN PTR studio-london._marketclaw-gw._tcp`,
      `studio-london._marketclaw-gw._tcp IN SRV 0 0 18789 studio-london`,
      `displayName=Mac Studio (MarketingClaw)`,
      `gatewayPort=18789`,
      `sshPort=22`,
      `cliPath=/opt/homebrew/bin/marketingclaw`,
  ...
  it.each([
    ...
    },
    {
      ...
      records: [
        `$ORIGIN marketingclaw.internal.`,
        `studio-london IN A 100.123.224.76`,
        `studio-london._marketclaw-gw._tcp IN TXT`,
        `displayName=Mac Studio (MarketingClaw)`,
        `gatewayTls=1`,
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    expect(result).toEqual({
      zonePath: getWideAreaZonePath("marketingclaw.internal."),
      changed: false,
    });
    expect(writeSpy).not.toHaveBeenCalled();
  });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    expect(writeSpy).toHaveBeenCalledWith(
      getWideAreaZonePath("marketingclaw.internal."),
      expectedZoneText,
      "utf-8",
    );
  });
