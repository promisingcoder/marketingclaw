...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.MARKETINGCLAW_WIDE_AREA_DOMAIN ?? null;
  try {
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*marketingclaw-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "marketingclaw";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "marketingclaw");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "marketclaw-gw");
  ...
  records.push(`_marketclaw-gw._tcp IN PTR ${instanceLabel}._marketclaw-gw._tcp`);
  records.push(`${instanceLabel}._marketclaw-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._marketclaw-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; marketingclaw-content-hash: ${contentHash}\n${contentBody}`;
}
