...
describe("package dist inventory", () => {
  it("tracks missing and stale dist files", async () => {
    await withTempDir({ prefix: "marketingclaw-dist-inventory-" }, async (packageRoot) => {
  ...
  it("keeps npm-omitted dist artifacts out of the inventory", async () => {
    await withTempDir({ prefix: "marketingclaw-dist-inventory-pack-" }, async (packageRoot) => {
  ...
  it("honors package files exclusions when writing the dist inventory", async () => {
    await withTempDir(
      { prefix: "marketingclaw-dist-inventory-package-files-" },
      async (packageRoot) => {
  ...
  it("keeps transient plugin dependency trees out of the inventory", async () => {
    await withTempDir(
      { prefix: "marketingclaw-dist-inventory-plugin-deps-" },
      async (packageRoot) => {
  ...
  it("omits packaged extension node_modules while keeping extension runtime files", async () => {
    await withTempDir(
      { prefix: "marketingclaw-dist-inventory-extension-node-modules-" },
      async (packageRoot) => {
  ...
  it("keeps publishable externalized bundled plugin dist trees out of the inventory", async () => {
    await withTempDir(
      { prefix: "marketingclaw-dist-inventory-externalized-" },
      async (packageRoot) => {
  ...
  it("keeps publishable core-package runtime plugin dist trees in the inventory", async () => {
    await withTempDir(
      { prefix: "marketingclaw-dist-inventory-core-runtime-" },
      async (packageRoot) => {
  ...
  it("reports runtime-created install staging dirs during installed dist verification", async () => {
    await withTempDir({ prefix: "marketingclaw-dist-inventory-stage-" }, async (packageRoot) => {
      ...
      const suffixedStageFile = path.join(
        ...
        "extensions",
        "browser",
        ".marketingclaw-install-stage-AbC123",
        "node_modules",
        "playwright-core",
        "package.json",
      );
      ...
      await expect(collectLegacyPluginDependencyStagingDebrisPaths(packageRoot)).resolves.toEqual([
        "dist/extensions/brave/.marketingclaw-install-stage",
        "dist/extensions/browser/.marketingclaw-install-stage-AbC123",
      ]);
    });
  });
  ...
  it("matches install-stage paths case-insensitively across path segments", () => {
    expect(
      isLegacyPluginDependencyInstallStagePath(
        "dist/extensions/brave/.marketingclaw-install-stage/node_modules/typebox/package.json",
      ),
    ).toBe(true);
    expect(
      isLegacyPluginDependencyInstallStagePath(
        "dist/Extensions/browser/.MARKETINGCLAW-INSTALL-STAGE-AbC123/node_modules/playwright-core/package.json",
      ),
    ).toBe(true);
    expect(
      isLegacyPluginDependencyInstallStagePath(
    ...
    expect(
      isLegacyPluginDependencyInstallStagePath(
        "dist/extensions/browser/.marketingclaw-runtime-deps-copy-AbC123/package.json",
      ),
    ).toBe(false);
    expect(
      isLegacyPluginDependencyInstallStagePath("dist/extensions/.marketingclaw-install-stage"),
    ).toBe(false);
  });

  it("rejects pre-populated install-stage debris at publish time", async () => {
    await withTempDir(
      { prefix: "marketingclaw-dist-inventory-stage-publish-" },
      async (packageRoot) => {
        ...
        await expect(collectLegacyPluginDependencyStagingDebrisPaths(packageRoot)).resolves.toEqual(
          [
            "dist/extensions/browser/.marketingclaw-install-stage-AbC123",
            "dist/extensions/evil/.marketingclaw-install-stage",
          ],
        );
  ...
  it("rejects mixed-case install-stage debris on case-sensitive release builders", async () => {
    await withTempDir(
      { prefix: "marketingclaw-dist-inventory-stage-extensions-case-" },
      async (packageRoot) => {
    ...
    await withTempDir(
      { prefix: "marketingclaw-dist-inventory-stage-root-case-" },
      async (packageRoot) => {
        const mixedCaseStage = path.join(
          packageRoot,
          "Dist",
  ...
  it("treats a missing dist/extensions tree as no staging debris", async () => {
    await withTempDir(
      { prefix: "marketingclaw-dist-inventory-no-extensions-" },
      async (packageRoot) => {
  ...
  it("returns null when the inventory is missing", async () => {
    await withTempDir({ prefix: "marketingclaw-dist-inventory-missing-" }, async (packageRoot) => {
      await fs.mkdir(path.join(packageRoot, "dist"), { recursive: true });
  ...
  it("rejects symlinked dist entries", async () => {
    await withTempDir({ prefix: "marketingclaw-dist-inventory-symlink-" }, async (packageRoot) => {
