...
async function withIdentity(
  ...
  await withTempDir("marketingclaw-device-identity-", async (dir) => {
...
describe("device identity crypto helpers", () => {
  it("loads an existing identity without creating a missing file", async () => {
    await withTempDir("marketingclaw-device-identity-readonly-", async (dir) => {
  ...
  it("loads Swift raw-key identity files without generating a new device id", async () => {
    await withTempDir("marketingclaw-device-identity-swift-", async (dir) => {
      const identityPath = path.join(dir, "identity", "device.json");
  ...
  it("does not overwrite recognized invalid identity files", async () => {
    await withTempDir("marketingclaw-device-identity-invalid-", async (dir) => {
      const identityPath = path.join(dir, "identity", "device.json");
      fs.mkdirSync(path.dirname(identityPath), { recursive: true });
      fs.writeFileSync(
        identityPath,
  ...
  it("does not overwrite existing unrecognized identity files", async () => {
    await withTempDir("marketingclaw-device-identity-unrecognized-", async (dir) => {
      const identityPath = path.join(dir, "identity", "device.json");
      fs.mkdirSync(path.dirname(identityPath), { recursive: true });
      fs.writeFileSync(
        identityPath,
  ...
  it("does not migrate Swift raw-key identity files with mismatched key material", async () => {
    await withTempDir("marketingclaw-device-identity-swift-invalid-", async (dir) => {
      const identityPath = path.join(dir, "identity", "device.json");
      fs.mkdirSync(path.dirname(identityPath), { recursive: true });
      fs.writeFileSync(
        identityPath,
