...
const canCreateDirectorySymlinks = (() => {
  ...
  try {
    probeDir = fsSync.mkdtempSync(
      path.join(os.tmpdir(), "marketingclaw-install-safe-dir-symlink-probe-"),
    );
...
describe("unscopedPackageName", () => {
  it.each([
    { value: "@marketingclaw/matrix", expected: "matrix" },
...
describe("packageNameMatchesId", () => {
  it.each([
    { packageName: "@marketingclaw/matrix", id: "matrix", expected: true },
    { packageName: "@marketingclaw/matrix", id: "@marketingclaw/matrix", expected: true },
    { packageName: "@marketingclaw/matrix", id: "quietchat", expected: false },
    { packageName: " ", id: "matrix", expected: false },
    { packageName: "@marketingclaw/matrix", id: " ", expected: false },
...
describe("resolveSafeInstallDir", () => {
  it("resolves install dirs under the base directory", () => {
    expect(
      resolveSafeInstallDir({
        baseDir: "/tmp/plugins",
        id: "@marketingclaw/matrix",
        invalidNameMessage: "invalid plugin name",
      }),
    ).toEqual({
      ok: true,
      path: path.join("/tmp/plugins", "@marketingclaw__matrix"),
    });
  });
...
describe("assertCanonicalPathWithinBase", () => {
  it("accepts in-base directories", async () => {
    await withTempDir({ prefix: "marketingclaw-install-safe-" }, async (baseDir) => {
  ...
  it.skipIf(!canCreateDirectorySymlinks)(
    ...
    async () => {
      await withTempDir({ prefix: "marketingclaw-install-safe-" }, async (baseDir) => {
        await withTempDir({ prefix: "marketingclaw-install-safe-outside-" }, async (outsideDir) => {
  ...
  it.skipIf(!canCreateDirectorySymlinks)(
    ...
    async () => {
      await withTempDir({ prefix: "marketingclaw-install-safe-" }, async (parentDir) => {
