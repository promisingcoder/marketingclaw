...
function createMarketingClawBusyResult(pid: number, overrides: Partial<MockLsofResult> = {}) {
  return createLsofResult({
    stdout: lsofOutput([{ pid, cmd: "marketingclaw-gateway" }]),
    ...overrides,
  });
}
...
function installInitialBusyPoll(
  ...
  mockSpawnSync.mockImplementation((command: unknown) => {
    ...
    call += 1;
    if (call === 1) {
      return createMarketingClawBusyResult(stalePid);
    }
...
describe.skipIf(isWindows)("restart-stale-pids", () => {
  ...
  describe("findGatewayPidsOnPortSync", () => {
    ...
    it("parses marketingclaw-gateway pids and excludes the current process", () => {
      const stalePid = process.pid + 1;
      mockSpawnSync.mockReturnValue({
        error: null,
        status: 0,
        stdout: lsofOutput([
          { pid: stalePid, cmd: "marketingclaw-gateway" },
          { pid: process.pid, cmd: "marketingclaw-gateway" },
        ]),
        stderr: "",
      });
    ...
    it("verifies argv when lsof reports the node process name instead of marketingclaw", () => {
      ...
      mockSpawnSync.mockImplementation((command: unknown) => {
        if (command === "ps") {
          return {
            error: null,
            status: 0,
            stdout: "node /opt/marketingclaw/dist/entry.js gateway\n",
            stderr: "",
          };
        }
        return {
    ...
    it("excludes ancestor pids so a sidecar cannot kill its parent gateway — regression for #68451", () => {
      // Regression: marketingclaw-weixin sidecar (child of the gateway) invoked
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: parentGatewayPid, cmd: "marketingclaw-gateway" },
          { pid: unrelatedStalePid, cmd: "marketingclaw-gateway" },
        ]),
        stderr: "",
      });
      ...
      () => {
        ...
        mockReadFileSync.mockImplementation((path: unknown): string => {
          if (path === `/proc/${directParentPid}/status`) {
            return `Name:\tmarketingclaw-gateway\nPid:\t${directParentPid}\nPPid:\t${grandparentPid}\n`;
          }
        ...
        mockSpawnSync.mockReturnValue({
          ...
          status: 0,
          stdout: lsofOutput([
            { pid: directParentPid, cmd: "marketingclaw-gateway" },
            { pid: grandparentPid, cmd: "marketingclaw-gateway" },
            { pid: benignStalePid, cmd: "marketingclaw-gateway" },
          ]),
          stderr: "",
        });
    ...
    it("excludes PID 1 when the direct parent gateway is the container entrypoint — container topology", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: 1, cmd: "marketingclaw-gateway" },
          { pid: benignStalePid, cmd: "marketingclaw-gateway" },
        ]),
        stderr: "",
      });
    ...
    it.skipIf(process.platform !== "linux")(
      ...
      () => {
        ...
        mockSpawnSync.mockReturnValue({
          ...
          status: 0,
          stdout: lsofOutput([
            { pid: pluginHostPid, cmd: "marketingclaw-gateway" },
            { pid: gatewayGrandparentPid, cmd: "marketingclaw-gateway" },
          ]),
          stderr: "",
        });
      ...
      try {
        mockSpawnSync.mockImplementation((command: unknown, args: unknown) => {
          ...
          return {
            ...
            status: 0,
            stdout: lsofOutput([
              { pid: toolHostPid, cmd: "marketingclaw-gateway" },
              { pid: gatewayGrandparentPid, cmd: "marketingclaw-gateway" },
              { pid: benignStalePid, cmd: "marketingclaw-gateway" },
            ]),
            stderr: "",
    ...
    it("excludes pids whose command does not include 'marketingclaw'", () => {
      ...
      try {
        mockSpawnSync.mockImplementation((command: unknown, args: unknown) => {
          ...
          return {
            error: null,
            status: 0,
            stdout: lsofOutput([{ pid: process.pid + 3152, cmd: "marketingclaw-gateway" }]),
            stderr: "",
          };
        });
    ...
    it("returns verified gateway pids from Windows helpers on win32", () => {
      ...
      try {
        ...
        mockReadWindowsProcessArgs.mockReturnValue(["marketingclaw", "gateway"]);
        expect(findGatewayPidsOnPortSync(18789)).toEqual([stalePid]);
        expect(mockReadWindowsListeningPids).toHaveBeenCalledWith(18789, undefined);
  ...
  describe("parsePidsFromLsofOutput (via findGatewayPidsOnPortSync stdout path)", () => {
    ...
    it("parses multiple marketingclaw pids from a single lsof output block", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: pid1, cmd: "marketingclaw-gateway" },
          { pid: pid2, cmd: "marketingclaw-gateway" },
        ]),
        stderr: "",
      });
      const result = findGatewayPidsOnPortSync(18789);
    ...
    it("returns [] when status 0 but only non-marketingclaw pids present", () => {
      // Port may be bound by an unrelated process. findGatewayPidsOnPortSync
      // only tracks marketingclaw processes — non-marketingclaw listeners are ignored.
  ...
  describe("pollPortOnce — no second lsof spawn (Codex P1 regression)", () => {
    ...
    it("lsof status 1 with non-empty marketingclaw stdout is treated as busy, not free (Linux container edge case)", () => {
      ...
      // status 1 + non-empty marketingclaw stdout must not be treated as port-free.
      const stalePid = process.pid + 601;
      const getCallCount = installInitialBusyPoll(stalePid, (call) => {
        if (call === 2) {
          // status 1 + marketingclaw pid in stdout — container-restricted lsof reports partial results
          return createMarketingClawBusyResult(stalePid, {
            status: 1,
    ...
    it("pollPortOnce outer catch returns { free: null, permanent: false } when resolveLsofCommandSync throws", () => {
      ...
      mockSpawnSync.mockImplementationOnce(() => {
        ...
        return {
          error: null,
          status: 0,
          stdout: lsofOutput([{ pid: stalePid, cmd: "marketingclaw-gateway" }]),
          stderr: "",
        };
      });
    ...
    it("does not kill a protected gateway pid after reparenting", () => {
      ...
      mockSpawnSync.mockImplementation(() => {
        ...
        return lsofCall === 1
          ? createLsofResult({
              stdout: lsofOutput([
                { pid: protectedPid, cmd: "marketingclaw-gateway" },
                { pid: stalePid, cmd: "marketingclaw-gateway" },
              ]),
            })
          : createLsofResult({ status: 1 });
  ...
  describe("parsePidsFromLsofOutput — branch coverage (lines 67-69)", () => {
    it("skips a mid-loop entry when the command does not include 'marketingclaw'", () => {
      // Exercises the false branch of currentCmd.toLowerCase().includes("marketingclaw")
      // inside the mid-loop flush: a non-marketingclaw cmd between two entries must not
      // be pushed, but the following marketingclaw entry still must be.
      const stalePid = process.pid + 700;
      // Mixed output: non-marketingclaw entry first, then marketingclaw entry
    ...
    it("ignores a p-line with an invalid (non-positive) PID — parse false branch", () => {
      ...
      // p0 is invalid (not > 0); the following valid marketingclaw entry must still be found.
      const stdout = `p0\ncmarketingclaw-gateway\np${stalePid}\ncmarketingclaw-gateway\n`;
      mockSpawnSync.mockReturnValue({ error: null, status: 0, stdout, stderr: "" });
      const result = findGatewayPidsOnPortSync(18789);
      expect(result).toContain(stalePid);
  ...
  describe("pollPortOnce — status 1 + non-empty stdout", () => {
    it("keeps polling when status 1 includes a non-marketingclaw listener record", () => {
      ...
      try {
        mockSpawnSync.mockImplementation((command: unknown, args: unknown) => {
          ...
          if (command === "lsof") {
            ...
            return lsofCall === 2
              ? createLsofResult({
                  stdout: lsofOutput([{ pid: gatewayParentPid, cmd: "marketingclaw-gateway" }]),
                })
              : createLsofResult({ status: 1 });
          }
          return createLsofResult();
