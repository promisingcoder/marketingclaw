...
import { bundledDistPluginFile } from "marketingclaw/plugin-sdk/test-fixtures";
...
import {
  ...
  isMainPackageTarget,
  MARKETINGCLAW_MAIN_PACKAGE_SPEC,
...
const execFileSyncMock = vi.hoisted(() => vi.fn(() => "/tmp/marketingclaw-test-global-npmrc\n"));
...
async function writeGlobalPackageJson(packageRoot: string, version = "1.0.0") {
  await fs.writeFile(
    ...
    JSON.stringify({ name: "marketingclaw", version }),
    "utf-8",
  );
}
...
describe("update global helpers", () => {
  ...
  it("prefers explicit package spec overrides", () => {
    envSnapshot = captureEnv(["MARKETINGCLAW_UPDATE_PACKAGE_SPEC"]);
    process.env.MARKETINGCLAW_UPDATE_PACKAGE_SPEC = "file:/tmp/marketingclaw.tgz";

    expect(resolveGlobalInstallSpec({ packageName: "marketingclaw", tag: "latest" })).toBe(
      "file:/tmp/marketingclaw.tgz",
    );
    expect(
      resolveGlobalInstallSpec({
        packageName: "marketingclaw",
        tag: "beta",
        env: { MARKETINGCLAW_UPDATE_PACKAGE_SPEC: "openclaw@next" },
      }),
    ).toBe("openclaw@next");
  });
  ...
  it("resolves scoped package paths from the package manager global root", async () => {
    ...
    await expect(
      ...
      manager: "npm",
      globalRoot,
      packageRoot: path.join(globalRoot, "@kevins8", "marketingclaw"),
    });
  });

  it("maps main and explicit install specs for global installs", () => {
    expect(resolveGlobalInstallSpec({ packageName: "marketingclaw", tag: "main" })).toBe(
      MARKETINGCLAW_MAIN_PACKAGE_SPEC,
    );
    expect(
      resolveGlobalInstallSpec({
    ...
    expect(
      resolveGlobalInstallSpec({
        packageName: "marketingclaw",
        tag: "https://example.com/marketingclaw-main.tgz",
      }),
    ).toBe("https://example.com/marketingclaw-main.tgz");
  });
  ...
  it("resolves portable Git paths from process-local app data only", async () => {
    await withMockedWindowsPlatform(async () => {
      await withTempDir({ prefix: "marketingclaw-update-portable-git-" }, async (base) => {
  ...
  it("classifies main and raw install specs separately from registry selectors", () => {
    ...
    expect(isExplicitPackageInstallSpec("marketingclaw-main.tgz")).toBe(true);
  ...
  it("detects install managers from resolved roots and on-disk presence", async () => {
    await withTempDir({ prefix: "marketingclaw-update-global-" }, async (base) => {
      const npmRoot = path.join(base, "npm-root");
      const pnpmRoot = path.join(base, "pnpm-root");
      const bunRoot = path.join(base, ".bun", "install", "global", "node_modules");
      const pkgRoot = path.join(pnpmRoot, "marketingclaw");
      await fs.mkdir(pkgRoot, { recursive: true });
      await fs.mkdir(path.join(npmRoot, "marketingclaw"), { recursive: true });
      await fs.mkdir(path.join(bunRoot, "marketingclaw"), { recursive: true });
      ...
      await fs.rm(path.join(npmRoot, "marketingclaw"), { recursive: true, force: true });
      await fs.rm(path.join(pnpmRoot, "marketingclaw"), { recursive: true, force: true });
  ...
  it("prefers the owning npm prefix when PATH npm points at a different global root", async () => {
    await withMockedPlatform("darwin", async () => {
      await withTempDir({ prefix: "marketingclaw-update-npm-prefix-" }, async (base) => {
  ...
  it("keeps npm ownership but avoids per-Node npm commands for reinstall", async () => {
    await withMockedPlatform("darwin", async () => {
      await withTempDir({ prefix: "marketingclaw-update-node-version-prefix-" }, async (base) => {
        ...
        for (const layout of layouts) {
          ...
          const pkgRoot = path.join(nodeManagedRoot, "marketingclaw");
  ...
  it("keeps npm self-updates on the running package root when the PATH probe diverges", async () => {
    await withMockedPlatform("darwin", async () => {
      await withTempDir({ prefix: "marketingclaw-update-ephemeral-probe-" }, async (base) => {
        ...
        const pkgRoot = path.join(nvmRoot, "marketingclaw");
  ...
  it("keeps scoped npm self-updates on the running package root", async () => {
    await withMockedPlatform("darwin", async () => {
      await withTempDir({ prefix: "marketingclaw-update-scoped-probe-" }, async (base) => {
        const nvmPrefix = path.join(base, "home", ".nvm", "versions", "node", "v24.5.0");
        const nvmRoot = path.join(nvmPrefix, "lib", "node_modules");
  ...
  it("keeps the npm probe when the package root is not globally installed", async () => {
    await withMockedPlatform("darwin", async () => {
      await withTempDir({ prefix: "marketingclaw-update-probe-only-" }, async (base) => {
        const nvmPrefix = path.join(base, "home", ".nvm", "versions", "node", "v24.5.0");
        const nvmRoot = path.join(nvmPrefix, "lib", "node_modules");
        const pkgRoot = path.join(base, "checkout", "node_modules", "marketingclaw");
        await fs.mkdir(pkgRoot, { recursive: true });
        ...
        await expect(
          ...
          command: "npm",
          globalRoot: nvmRoot,
          packageRoot: path.join(nvmRoot, "marketingclaw"),
        });
      });
    });
  });
  ...
  it("falls back to the running package root when the npm root probe fails", async () => {
    await withMockedPlatform("darwin", async () => {
      await withTempDir({ prefix: "marketingclaw-update-probe-failure-" }, async (base) => {
        const globalRoot = path.join(base, "usr", "local", "lib", "node_modules");
        const pkgRoot = path.join(globalRoot, "marketingclaw");
        await fs.mkdir(pkgRoot, { recursive: true });
  ...
  it("does not infer npm ownership from path shape alone when the owning npm binary is absent", async () => {
    await withTempDir({ prefix: "marketingclaw-update-npm-missing-bin-" }, async (base) => {
  ...
  it("honors an explicitly selected direct npm node_modules package root", async () => {
    await withTempDir({ prefix: "marketingclaw-update-managed-service-root-" }, async (base) => {
      const managedNpmRoot = path.join(base, ".marketingclaw", "npm", "node_modules");
      const pkgRoot = path.join(managedNpmRoot, "marketingclaw");
      ...
      await fs.mkdir(pkgRoot, { recursive: true });
      await fs.mkdir(path.join(otherPnpmRoot, "marketingclaw"), { recursive: true });
  ...
  it("preserves bun ownership for direct node_modules package roots", async () => {
    await withTempDir({ prefix: "marketingclaw-update-managed-bun-root-" }, async (base) => {
      envSnapshot = captureEnv(["BUN_INSTALL"]);
      process.env.BUN_INSTALL = path.join(base, ".bun");
      const bunRoot = path.join(process.env.BUN_INSTALL, "install", "global", "node_modules");
      const pkgRoot = path.join(bunRoot, "marketingclaw");
      const pathNpmRoot = path.join(base, "shell", "lib", "node_modules");
      await fs.mkdir(pkgRoot, { recursive: true });
  ...
  it("prefers npm.cmd for win32-style global npm roots", async () => {
    await withMockedWindowsPlatform(async () => {
      await withTempDir({ prefix: "marketingclaw-update-win32-npm-prefix-" }, async (base) => {
        const npmPrefix = path.join(base, "Roaming", "npm");
        const npmRoot = path.join(npmPrefix, "node_modules");
        const pkgRoot = path.join(npmRoot, "marketingclaw");
  ...
  it("detects custom pnpm global layouts from the running package root", async () => {
    await withTempDir({ prefix: "marketingclaw-update-pnpm-custom-root-" }, async (base) => {
      const customGlobalDir = path.join(base, "custom-pnpm");
      const customGlobalRoot = path.join(customGlobalDir, "5", "node_modules");
      const pkgRoot = path.join(customGlobalRoot, "marketingclaw");
  ...
  it("detects custom pnpm global layouts from virtual-store package roots", async () => {
    await withTempDir({ prefix: "marketingclaw-update-pnpm-virtual-root-" }, async (base) => {
      const customGlobalDir = path.join(base, "custom-pnpm");
      const customGlobalRoot = path.join(customGlobalDir, "5", "node_modules");
      ...
      await expect(
        ...
        command: "pnpm",
        globalRoot: customGlobalRoot,
        packageRoot: path.join(customGlobalRoot, "marketingclaw"),
      });
    });
  });

  it("does not infer pnpm ownership without pnpm node_modules metadata", async () => {
    await withTempDir({ prefix: "marketingclaw-update-pnpm-shape-only-" }, async (base) => {
      const customGlobalDir = path.join(base, "custom-pnpm");
      const customGlobalRoot = path.join(customGlobalDir, "5", "node_modules");
      const pkgRoot = path.join(customGlobalRoot, "marketingclaw");
      const defaultPnpmRoot = path.join(base, "default-pnpm", "5", "node_modules");
      ...
      await expect(
        ...
        packageRoot: path.join(defaultPnpmRoot, "marketingclaw"),
      });
    });
  });
  ...
  it("resolves npm prefix layouts for normal global roots", () => {
    expect(
      resolveNpmGlobalPrefixLayoutFromGlobalRoot("/opt/marketingclaw/lib/node_modules"),
    ).toEqual({
      prefix: "/opt/marketingclaw",
      globalRoot: "/opt/marketingclaw/lib/node_modules",
      binDir: "/opt/marketingclaw/bin",
    });
  ...
  it("cleans only renamed package directories", async () => {
    await withTempDir({ prefix: "marketingclaw-update-cleanup-" }, async (root) => {
      await fs.mkdir(path.join(root, ".marketingclaw-123"), { recursive: true });
      await fs.mkdir(path.join(root, ".marketingclaw-456"), { recursive: true });
      await fs.writeFile(path.join(root, ".marketingclaw-file"), "nope", "utf8");
      await fs.mkdir(path.join(root, "marketingclaw"), { recursive: true });
      ...
      await expect(
        ...
        removed: [".marketingclaw-123", ".marketingclaw-456"],
      });
      const packageDirStat = await fs.stat(path.join(root, "marketingclaw"));
      const markerFileStat = await fs.stat(path.join(root, ".marketingclaw-file"));
  ...
  it("checks installed dist against the packaged inventory", async () => {
    await withTempDir({ prefix: "marketingclaw-update-global-pkg-" }, async (packageRoot) => {
  ...
  it("reports bundled plugin install stages during installed dist verification", async () => {
    await withTempDir(
      { prefix: "marketingclaw-update-global-plugin-stage-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot);
        ...
        for (const stageDir of [
          ".marketingclaw-install-stage",
          ".marketingclaw-install-stage-retry",
        ]) {
        ...
        await expect(collectInstalledGlobalPackageErrors({ packageRoot })).resolves.toEqual([
          "unexpected packaged dist file dist/extensions/brave/.marketingclaw-install-stage-retry/node_modules/typebox/build/compile/code.mjs",
          "unexpected packaged dist file dist/extensions/brave/.marketingclaw-install-stage/node_modules/typebox/build/compile/code.mjs",
        ]);
      },
    );
  });
  ...
  it("flags global package roots that resolve into source checkouts", async () => {
    await withTempDir({ prefix: "marketingclaw-update-global-source-checkout-" }, async (base) => {
  ...
  it("does not require private QA sidecars when the inventory is missing", async () => {
    await withTempDir({ prefix: "marketingclaw-update-global-legacy-" }, async (packageRoot) => {
      await writeGlobalPackageJson(packageRoot);
  ...
  it("fails closed on newer installs when the inventory is missing", async () => {
    await withTempDir(
      { prefix: "marketingclaw-update-global-missing-inventory-new-" },
      async (packageRoot) => {
  ...
  it("rejects invalid inventory files during global verify", async () => {
    await withTempDir(
      { prefix: "marketingclaw-update-global-invalid-inventory-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
  ...
  it("verifies legacy sidecars for installed bundled plugins without inventory", async () => {
    await withTempDir(
      { prefix: "marketingclaw-update-global-legacy-plugin-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot);
        await writeBundledPluginPackageJson(packageRoot, "telegram", "@marketingclaw/telegram");
  ...
  it("still enforces critical sidecars when the inventory omits them", async () => {
    await withTempDir(
      { prefix: "marketingclaw-update-global-critical-sidecars-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
        await writeBundledPluginPackageJson(packageRoot, "telegram", "@marketingclaw/telegram");
        await writePackageDistInventory(packageRoot);
  ...
  it("ignores stale metadata for non-packaged private QA plugins during inventory verify", async () => {
    await withTempDir(
      { prefix: "marketingclaw-update-global-stale-private-qa-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
        await writeBundledPluginPackageJson(packageRoot, "qa-lab", "@marketingclaw/qa-lab");
        await writePackageDistInventory(packageRoot);
