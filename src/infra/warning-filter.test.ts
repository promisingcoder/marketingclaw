...
const warningFilterKey = Symbol.for("marketingclaw.warning-filter");
...
describe("warning filter", () => {
  ...
  it("installs once and suppresses known warnings at emit time", async () => {
    ...
    try {
      ...
      emitWarning("Visible warning", { type: "Warning", code: "MARKETINGCLAW_TEST_WARNING" });
      emitWarning(
        Object.assign(new Error("The punycode module is deprecated."), {
          name: "DeprecationWarning",
          code: "DEP0040",
        }),
        { type: "Warning", code: "MARKETINGCLAW_VISIBLE_OVERRIDE" },
      );
      await flushWarnings();
      expect(
        seenWarnings.find((warning) => warning.code === "MARKETINGCLAW_TEST_WARNING"),
      ).toStrictEqual({
        code: "MARKETINGCLAW_TEST_WARNING",
        name: "Warning",
