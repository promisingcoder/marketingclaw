...
const MATRIX_REPO_INSTALL_COMMAND = `marketingclaw plugins install ${repoInstallSpec("matrix")}`;
...
describe("plugin install path warnings", () => {
  ...
  it("detects stale custom plugin install paths", async () => {
    const issue = await detectPluginInstallPathIssue({
      ...
      install: {
        source: "path",
        sourcePath: "/tmp/marketingclaw-matrix-missing",
        installPath: "/tmp/marketingclaw-matrix-missing",
      },
    });
    ...
    expect(issue).toEqual({
      kind: "missing-path",
      pluginId: "matrix",
      path: "/tmp/marketingclaw-matrix-missing",
    });
    expect(
      formatPluginInstallPathIssue({
        issue: issue!,
        pluginLabel: "Matrix",
        defaultInstallCommand: "marketingclaw plugins install @marketingclaw/matrix",
        repoInstallCommand: MATRIX_REPO_INSTALL_COMMAND,
      }),
    ).toEqual([
      "Matrix is installed from a custom path that no longer exists: /tmp/marketingclaw-matrix-missing",
      'Reinstall with "marketingclaw plugins install @marketingclaw/matrix".',
  ...
  it("uses the second candidate path when the first one is stale", async () => {
    const { issue, pluginPath } = await detectMatrixCustomPathIssue(
      "/tmp/marketingclaw-matrix-missing",
    );
    expect(issue).toEqual({
      kind: "custom-path",
      pluginId: "matrix",
  ...
  it("applies custom command formatting in warning messages", () => {
    expect(
      ...
      'Reinstall with "<marketingclaw plugins install @marketingclaw/matrix>" when you want to return to the standard Matrix plugin.',
