...
function buildGenericNativeExecApprovalFallbackText(params?: { excludeChannel?: string }): string {
  ...
  return clients
    ? `Approve it from the Web UI or terminal UI, or enable a native chat approval client such as ${clients}. If those accounts already know your owner ID via allowFrom or owner config, MarketingClaw can often infer approvers automatically.`
