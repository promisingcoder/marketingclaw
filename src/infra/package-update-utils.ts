...
/** Return true when an installed package needs an marketingclaw peer link repair. */
export function installedPackageNeedsMarketingClawPeerLinkRepair(dir: string): boolean {
  ...
  try {
    fsSync.statSync(path.join(dir, "node_modules", "marketingclaw"));
    return false;
  } catch (error) {
