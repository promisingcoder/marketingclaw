...
const matrixChunkConfig: MarketingClawConfig = {
  channels: { matrix: { textChunkLimit: 4000 } } as MarketingClawConfig["channels"],
};

const expectedPreferredTmpRoot = resolvePreferredMarketingClawTmpDir();
...
async function runChunkedMatrixDelivery(params?: {
  ...
  const cfg: MarketingClawConfig = {
    channels: { matrix: { textChunkLimit: 2 } } as MarketingClawConfig["channels"],
  };
...
describe("deliverOutboundPayloads", () => {
  ...
  it("chunks direct adapter text and preserves delivery overrides across sends", async () => {
    ...
    const results = await deliverOutboundPayloads({
      cfg: { channels: { matrix: { textChunkLimit: 2 } } } as MarketingClawConfig,
      channel: "matrix",
      to: "!room",
      accountId: "default",
  ...
  it("passes delivery config and account context to adapter payload normalization", async () => {
    ...
    const cfg = { channels: { matrix: { enabled: true } } } as unknown as MarketingClawConfig;
    setActivePluginRegistry(
      createTestRegistry([
        {
          pluginId: "matrix",
  ...
  it("uses adapter-provided formatted senders and scoped media roots when available", async () => {
    ...
    const textResults = await deliverOutboundPayloads({
      cfg: { channels: { line: {} } } as MarketingClawConfig,
      channel: "line",
      to: "U123",
      accountId: "default",
    ...
    const cfg = { channels: { line: {} } } as MarketingClawConfig;
    await deliverOutboundPayloads({
      cfg,
      channel: "line",
      to: "U123",
    ...
    expect(
      sendFormattedMediaCall?.mediaLocalRoots?.some((root) =>
        root.endsWith(path.join(".marketingclaw", "workspace-work")),
      ),
    ).toBe(true);
    expect(sendMedia).not.toHaveBeenCalled();
  });
  ...
  it("includes MarketingClaw tmp root in plugin mediaLocalRoots", async () => {
    const sendMatrix = vi.fn().mockResolvedValue({ messageId: "m-media", roomId: "!room" });

    await deliverOutboundPayloads({
      cfg: { channels: { matrix: {} } } as MarketingClawConfig,
      channel: "matrix",
      to: "!room:example",
  ...
  it("lets explicit formatting options override configured chunking", async () => {
    ...
    await deliverOutboundPayloads({
      cfg: { channels: { matrix: { textChunkLimit: 4000 } } } as MarketingClawConfig,
      channel: "matrix",
      to: "!room",
      payloads: [{ text: "abcd" }],
  ...
  it("bails out without sending when a concurrent drain already claimed the queue entry", async () => {
    // Regression for promisingcoder/marketingclaw#70386: if a reconnect or startup drain
