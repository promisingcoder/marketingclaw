...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: MarketingClawConfig | null = null;
   ...
   */
  get(key: string, cfg: MarketingClawConfig): T | undefined {
   ...
   */
  set(key: string, value: T, cfg: MarketingClawConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
   ...
   */
  clear(cfg?: MarketingClawConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: MarketingClawConfig): void {
