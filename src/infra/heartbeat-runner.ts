...
function resolveHeartbeatTimeoutOverrideSeconds(
...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: MarketingClawConfig) => void;
};
...
function hasExplicitHeartbeatAgents(cfg: MarketingClawConfig) {
...
function resolveHeartbeatAgents(cfg: MarketingClawConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
function resolveHeartbeatPromptRaw(cfg: MarketingClawConfig, heartbeat?: HeartbeatConfig) {
  return heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt;
}

export function resolveHeartbeatPrompt(cfg: MarketingClawConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(resolveHeartbeatPromptRaw(cfg, heartbeat));
}

function resolveHeartbeatResponseToolPrompt(cfg: MarketingClawConfig, heartbeat?: HeartbeatConfig) {
...
function resolveHeartbeatAckMaxChars(cfg: MarketingClawConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
function isHeartbeatTypingEnabled(params: { cfg: MarketingClawConfig; hasChatDelivery: boolean }) {
...
function resolveHeartbeatTypingIntervalSeconds(cfg: MarketingClawConfig) {
...
export function startHeartbeatRunner(opts: {
  cfg?: MarketingClawConfig;
  readCurrentConfig?: () => MarketingClawConfig;
  runtime?: RuntimeEnv;
  ...
  const updateConfig = (cfg: MarketingClawConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
