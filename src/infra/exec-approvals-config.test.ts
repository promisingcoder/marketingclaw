...
describe("exec approvals wildcard agent", () => {
  it("merges wildcard allowlist entries with agent entries", () => {
    ...
    const prevMarketingClawHome = process.env.MARKETINGCLAW_HOME;

    try {
      process.env.MARKETINGCLAW_HOME = dir;
      const approvalsPath = path.join(dir, ".marketingclaw", "exec-approvals.json");
      ...
      if (prevMarketingClawHome === undefined) {
        delete process.env.MARKETINGCLAW_HOME;
      } else {
        process.env.MARKETINGCLAW_HOME = prevMarketingClawHome;
      }
    }
  });
});
