...
async function makeTempDir(): Promise<string> {
  return await tempDirs.make("marketingclaw-push-apns-auth-test-");
}
...
describe("push APNs auth and helper coverage", () => {
  ...
  it("prefers inline APNs private key values and unescapes newlines", async () => {
    const resolved = await resolveApnsAuthConfigFromEnv({
      MARKETINGCLAW_APNS_TEAM_ID: "TEAM123",
      MARKETINGCLAW_APNS_KEY_ID: "KEY123",
      MARKETINGCLAW_APNS_PRIVATE_KEY_P8:
        "-----BEGIN PRIVATE KEY-----\\nline-a\\nline-b\\n-----END PRIVATE KEY-----", // pragma: allowlist secret
      MARKETINGCLAW_APNS_PRIVATE_KEY: "ignored",
  ...
  it("falls back to MARKETINGCLAW_APNS_PRIVATE_KEY when MARKETINGCLAW_APNS_PRIVATE_KEY_P8 is blank", async () => {
    const resolved = await resolveApnsAuthConfigFromEnv({
      MARKETINGCLAW_APNS_TEAM_ID: "TEAM123",
      MARKETINGCLAW_APNS_KEY_ID: "KEY123",
      MARKETINGCLAW_APNS_PRIVATE_KEY_P8: "   ",
      MARKETINGCLAW_APNS_PRIVATE_KEY:
        "-----BEGIN PRIVATE KEY-----\\nline-c\\nline-d\\n-----END PRIVATE KEY-----", // pragma: allowlist secret
    } as NodeJS.ProcessEnv);
  ...
  it("reads APNs private keys from MARKETINGCLAW_APNS_PRIVATE_KEY_PATH", async () => {
    ...
    const resolved = await resolveApnsAuthConfigFromEnv({
      MARKETINGCLAW_APNS_TEAM_ID: "TEAM123",
      MARKETINGCLAW_APNS_KEY_ID: "KEY123",
      MARKETINGCLAW_APNS_PRIVATE_KEY_PATH: keyPath,
    } as NodeJS.ProcessEnv);
  ...
  it("reports missing auth fields and path read failures", async () => {
    ...
    const missingKey = await resolveApnsAuthConfigFromEnv({
      MARKETINGCLAW_APNS_TEAM_ID: "TEAM123",
      MARKETINGCLAW_APNS_KEY_ID: "KEY123",
      MARKETINGCLAW_APNS_PRIVATE_KEY_PATH: missingPath,
    } as NodeJS.ProcessEnv);

    expect(missingKey.ok).toBe(false);
    if (!missingKey.ok) {
      expect(missingKey.error).toContain(
        `failed reading MARKETINGCLAW_APNS_PRIVATE_KEY_PATH (${missingPath})`,
      );
    }
  });
