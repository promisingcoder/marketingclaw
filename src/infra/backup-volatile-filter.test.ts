...
const stateDir = "/opt/marketingclaw/state";
...
describe("isVolatileBackupPath", () => {
  ...
  it("does not match paths that escape the anchor via `..`", () => {
    // `/opt/marketingclaw/state/sessions/../config.jsonl` resolves to
    // `/opt/marketingclaw/state/config.jsonl`, which is NOT inside sessions/.
  ...
  it("normalizes Windows-style separators before anchor checks", () => {
    const winStateDir = "C:\\marketingclaw\\state";
  ...
  it("matches tar filter paths when node-tar omits the leading slash", () => {
    expect(
      isVolatileBackupPath("opt/marketingclaw/state/agents/main/sessions/transcript.jsonl", plan),
    ).toBe(true);
  });
