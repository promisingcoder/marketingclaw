...
let listedConfigs: Array<MarketingClawConfig | undefined> = [];
...
describe("video-generation runtime", () => {
    ...
    providers = [
      {
        ...
        async generateVideo(req) {
          seenSupportedDurationHint = (req as Record<symbol, readonly number[] | undefined>)[
            Symbol.for("marketingclaw.videoGeneration.supportedDurations")
          ];
          seenRequest = {
  ...
  it("lists runtime video-generation providers through the provider registry", () => {
    ...
    expect(
      listRuntimeVideoGenerationProviders({ config: {} as MarketingClawConfig }, runtimeDeps),
    ).toEqual(registryProviders);
    expect(listedConfigs).toEqual([{} as MarketingClawConfig]);
  });
  ...
  it("builds a generic config hint without hardcoded provider ids", async () => {
    ...
    await expect(
      runGenerateVideo({ cfg: {} as MarketingClawConfig, prompt: "animate a cat" }),
    ).rejects.toThrow(
