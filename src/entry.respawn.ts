...
export function buildCliRespawnPlan(
  ...
  if (
    ...
    isTruthyEnvValue(env.MARKETINGCLAW_NO_RESPAWN)
  ) {
    return null;
  }
  ...
  if (
    ...
    !isTruthyEnvValue(env[MARKETINGCLAW_NODE_EXTRA_CA_CERTS_READY]) &&
    !env.NODE_EXTRA_CA_CERTS
  ) {
    childEnv.NODE_EXTRA_CA_CERTS = autoNodeExtraCaCerts;
    childEnv[MARKETINGCLAW_NODE_EXTRA_CA_CERTS_READY] = "1";
    needsRespawn = true;
  }

  if (
    !shouldSkipRespawnForArgv(argv) &&
    !isTruthyEnvValue(env[MARKETINGCLAW_NODE_OPTIONS_READY]) &&
    !hasExperimentalWarningSuppressed({ env, execArgv })
  ) {
    childEnv[MARKETINGCLAW_NODE_OPTIONS_READY] = "1";
...
export function runCliRespawnPlan(
  ...
  return runRespawnChildWithSignalBridge({
    ...
    onError: (error) => {
      runtime.writeError(
        "[marketingclaw] Failed to respawn CLI:",
