...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `marketingclaw-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      MARKETINGCLAW_LAUNCHD_LABEL: `ai.marketingclaw.launchd-int-${testId}`,
      MARKETINGCLAW_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
