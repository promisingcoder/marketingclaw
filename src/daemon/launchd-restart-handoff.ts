...
function collectRestartLogEnv(env?: Record<string, string | undefined>): LaunchdRestartLogEnv {
  ...
  return {
    ...
    MARKETINGCLAW_STATE_DIR: source.MARKETINGCLAW_STATE_DIR,
    MARKETINGCLAW_PROFILE: source.MARKETINGCLAW_PROFILE,
  };
}

function resolveLaunchAgentLabel(env?: Record<string, string | undefined>): string {
  const envLabel = normalizeOptionalString(env?.MARKETINGCLAW_LAUNCHD_LABEL);
  if (envLabel) {
    return assertValidLaunchAgentLabel(envLabel);
  }
  return assertValidLaunchAgentLabel(resolveGatewayLaunchAgentLabel(env?.MARKETINGCLAW_PROFILE));
}
...
printf '[%s] marketingclaw restart attempt source=launchd-handoff mode=${mode} target=%s waitPid=%s\\n' "$(date -u +%FT%TZ)" "$service_target" "$wait_pid" >&2
...
if [ "$status" -eq 0 ]; then
  printf '[%s] marketingclaw restart done source=launchd-handoff mode=${mode}\\n' "$(date -u +%FT%TZ)" >&2
else
  printf '[%s] marketingclaw restart failed source=launchd-handoff mode=${mode} status=%s\\n' "$(date -u +%FT%TZ)" "$status" >&2
fi
exit "$status"
`;
  }
...
while [ "$print_retry_count" -gt 0 ]; do
  if launchctl print "$service_target" >/dev/null 2>&1; then
    printf '[%s] marketingclaw restart done source=launchd-handoff mode=${mode} reason=launchd-auto-reload\\n' "$(date -u +%FT%TZ)" >&2
    exit 0
  fi
  ...
  try {
    const child = spawn(
      ...
      [
        ...
        "marketingclaw-launchd-restart-handoff",
