...
function createGatewayAudit({
  ...
  return auditGatewayServiceConfig({
    ...
    command: {
      ...
      environment: {
        PATH: pathLocal,
        ...(serviceToken ? { MARKETINGCLAW_GATEWAY_TOKEN: serviceToken } : {}),
        ...extraEnvironment,
      },
...
async function writeSystemdUnitForAudit(home: string, lines: string[]) {
  ...
  const unitPath = path.join(unitDir, "marketingclaw-gateway.service");
...
describe("auditGatewayServiceConfig", () => {
  ...
  it("accepts Linux minimal PATH with user directories", async () => {
    const env = { HOME: "/tmp/marketingclaw-testuser", PNPM_HOME: "/opt/pnpm" };
  ...
  it("accepts canonical macOS gateway service PATH without user-bin defaults", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-service-audit-home-"));
    try {
  ...
  it("still requires explicit env-configured tool roots in gateway service PATH", async () => {
    const audit = await auditGatewayServiceConfig({
      env: { HOME: "/tmp/marketingclaw-testuser", PNPM_HOME: "/opt/pnpm" },
      platform: "linux",
      command: {
        programArguments: ["/usr/bin/node", "gateway"],
  ...
  it("flags stale Linux version-manager and package-manager PATH entries", async () => {
    const env = { HOME: "/tmp/marketingclaw-testuser-nonminimal" };
    const minimalPath = buildMinimalServicePath({ platform: "linux", env });
    const staleEntries = [
      `${env.HOME}/.volta/bin`,
      `${env.HOME}/.asdf/shims`,
  ...
  it("accepts an expected active MarketingClaw bin even when it looks package-managed", async () => {
    ...
    const audit = await auditGatewayServiceConfig({
      ...
      command: {
        programArguments: [
          ...
          "/opt/marketingclaw/dist/index.js",
          "gateway",
        ],
  ...
  it("accepts Linux fnm aliases/default without requiring the legacy current symlink", async () => {
    const env = {
      HOME: "/tmp/marketingclaw-testuser",
      FNM_DIR: "/tmp/marketingclaw-testuser/.local/share/fnm",
    };
  ...
  it.each(["process", "none"])(
    ...
    async (killMode) => {
      const home = await fs.mkdtemp(
        path.join(os.tmpdir(), "marketingclaw-service-audit-killmode-"),
      );
  ...
  it("does not warn when KillMode is control-group", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-service-audit-killmode-"));
    await writeSystemdUnitForAudit(home, [
      "After=network-online.target",
      "Wants=network-online.target",
      "RestartSec=5",
  ...
  it("accepts systemd RestartSec values with seconds suffixes", async () => {
    const home = await fs.mkdtemp(
      path.join(os.tmpdir(), "marketingclaw-service-audit-restartsec-"),
    );
    await writeSystemdUnitForAudit(home, [
      "After=network-online.target",
      "Wants=network-online.target",
  ...
  it("flags inline managed service env values from the service key list", async () => {
    const audit = await createGatewayAudit({
      extraEnvironment: {
        MARKETINGCLAW_SERVICE_MANAGED_ENV_KEYS: "TAVILY_API_KEY,OPENROUTER_API_KEY",
...
describe("checkTokenDrift", () => {
  ...
  it("detects drift when config has token but service has different token", () => {
    ...
    expect(result).toStrictEqual({
      ...
      detail: "Run `marketingclaw gateway install --force` to sync the token.",
      level: "recommended",
    });
  });
...
describe("gateway service version mismatch detection", () => {
  it("flags stale gateway service version metadata", async () => {
    const audit = await createGatewayAudit({
      extraEnvironment: { MARKETINGCLAW_SERVICE_VERSION: "2026.4.15-beta.1" },
    });
  ...
  it("accepts current gateway service version metadata", async () => {
    const audit = await createGatewayAudit({
      extraEnvironment: { MARKETINGCLAW_SERVICE_VERSION: VERSION },
    });
