...
export const MARKETINGCLAW_WRAPPER_ENV_KEY = "MARKETINGCLAW_WRAPPER";
...
async function resolveCliEntrypointPathForService(): Promise<string> {
  ...
  if (looksLikeDist) {
    ...
    // since symlinks like node_modules/marketingclaw -> .pnpm/openclaw@X.Y.Z/...
...
function appendNodeModulesBinCandidates(
  ...
  // marketingclaw from node_modules/.bin points at the package root sibling.
...
export async function resolveMarketingClawWrapperPath(
  ...
  try {
    ...
    throw new Error(
      `${MARKETINGCLAW_WRAPPER_ENV_KEY} must point to an executable file: ${resolved}${detail}`,
      { cause: error },
    );
  }
  return resolved;
...
async function resolveCliProgramArguments(params: {
  ...
  const wrapperPath = await resolveMarketingClawWrapperPath(params.wrapperPath);
  if (wrapperPath) {
