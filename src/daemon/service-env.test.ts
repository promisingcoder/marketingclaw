...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.MARKETINGCLAW_GATEWAY_PORT).toBe("18789");
    expect(env.MARKETINGCLAW_GATEWAY_TOKEN).toBeUndefined();
    expect(env.MARKETINGCLAW_SERVICE_MARKER).toBe("marketingclaw");
    expect(env.MARKETINGCLAW_SERVICE_KIND).toBe("gateway");
    expect(typeof env.MARKETINGCLAW_SERVICE_VERSION).toBe("string");
    expect(env.MARKETINGCLAW_SYSTEMD_UNIT).toBe("marketingclaw-gateway.service");
    expect(env.MARKETINGCLAW_WINDOWS_TASK_NAME).toBe("MarketingClaw Gateway");
    expect(env.MARKETINGCLAW_WINDOWS_TASK_HIDDEN_LAUNCHER).toBe("1");
    if (process.platform === "darwin") {
      expect(env.MARKETINGCLAW_LAUNCHD_LABEL).toBe("ai.marketingclaw.gateway");
    }
  });

  it("sets the MarketingClaw-owned launchd marker for macOS gateway services", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/Users/user" },
      port: 18789,
      platform: "darwin",
  ...
  it("passes through MARKETINGCLAW_WRAPPER for gateway services", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        MARKETINGCLAW_WRAPPER: " /usr/local/bin/marketingclaw-doppler ",
      },
      port: 18789,
    });

    expect(env.MARKETINGCLAW_WRAPPER).toBe("/usr/local/bin/marketingclaw-doppler");
  });
  ...
  it("uses a durable state temp directory for macOS LaunchAgents", () => {
    ...
    expect(env.TMPDIR).toBe(path.join("/Users/user", ".marketingclaw", "tmp"));
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", MARKETINGCLAW_PROFILE: "work" },
      port: 18789,
    });
    expect(env.MARKETINGCLAW_SYSTEMD_UNIT).toBe("marketingclaw-gateway-work.service");
    expect(env.MARKETINGCLAW_WINDOWS_TASK_NAME).toBe("MarketingClaw Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.MARKETINGCLAW_LAUNCHD_LABEL).toBe("ai.marketingclaw.work");
    }
  });
  ...
  it("preserves explicit systemd unit overrides", () => {
    const env = buildServiceEnvironment({
      env: {
        ...
        MARKETINGCLAW_SYSTEMD_UNIT: "marketingclaw-gateway-maintenance",
      },
      port: 18789,
      platform: "linux",
    });

    expect(env.MARKETINGCLAW_SYSTEMD_UNIT).toBe("marketingclaw-gateway-maintenance.service");
  });
  ...
  it("sets a profile-specific launchd marker for macOS gateway services", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/Users/user", MARKETINGCLAW_PROFILE: "work" },
      port: 18789,
      platform: "darwin",
    });
  ...
  it("forwards proxy URL env fallback for installed gateway services", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        MARKETINGCLAW_PROXY_URL: " http://127.0.0.1:3128 ",
      },
      port: 18789,
    });

    expect(env.MARKETINGCLAW_PROXY_URL).toBe("http://127.0.0.1:3128");
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("sets the MarketingClaw-owned launchd marker for macOS node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/Users/user" },
      platform: "darwin",
    });

    expect(env.MARKETINGCLAW_LAUNCHD_LABEL).toBe("ai.marketingclaw.node");
  });

  it("passes through MARKETINGCLAW_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", MARKETINGCLAW_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.MARKETINGCLAW_GATEWAY_TOKEN).toBe("node-token");
  });

  it("passes through MARKETINGCLAW_ALLOW_INSECURE_PRIVATE_WS for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", MARKETINGCLAW_ALLOW_INSECURE_PRIVATE_WS: " 1 " },
    });
    expect(env.MARKETINGCLAW_ALLOW_INSECURE_PRIVATE_WS).toBe("1");
  });

  it("omits MARKETINGCLAW_GATEWAY_TOKEN when the env var is empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
  ...
  it("marks Windows node tasks for hidden launcher startup", () => {
    ...
    expect(env.MARKETINGCLAW_WINDOWS_TASK_NAME).toBe("MarketingClaw Node");
    expect(env.MARKETINGCLAW_WINDOWS_TASK_HIDDEN_LAUNCHER).toBe("1");
    expect(env.MARKETINGCLAW_TASK_SCRIPT_NAME).toBe("node.cmd");
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".marketingclaw"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", MARKETINGCLAW_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".marketingclaw-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", MARKETINGCLAW_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".marketingclaw"));
  });

  it("uses MARKETINGCLAW_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", MARKETINGCLAW_STATE_DIR: "/var/lib/marketingclaw" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/marketingclaw"));
  });

  it("expands ~ in MARKETINGCLAW_STATE_DIR", () => {
    const env = { HOME: "/Users/test", MARKETINGCLAW_STATE_DIR: "~/marketingclaw-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/marketingclaw-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { MARKETINGCLAW_STATE_DIR: "C:\\State\\marketingclaw" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\marketingclaw");
  });
});
