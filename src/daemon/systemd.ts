...
function resolveSystemdServiceName(env: GatewayServiceEnv): string {
  ...
  return resolveGatewaySystemdServiceName(env.MARKETINGCLAW_PROFILE);
}
...
function sanitizeSystemdUnitBackupContent(params: {
  ...
  // Backups should not retain file-managed secrets that MarketingClaw moved into the
...
function resolveSystemdEnvironmentFilePath(params: {
  ...
  const serviceKind = params.environment?.MARKETINGCLAW_SERVICE_KIND?.trim();
  const filename =
...
function resolveLegacyNodeSystemdEnvironmentFilePath(params: {
  ...
  if (params.environment?.MARKETINGCLAW_SERVICE_KIND?.trim() !== "node") {
    return null;
  }
...
function isNodeSystemdEnvironment(env: GatewayServiceEnv): boolean {
  return env.MARKETINGCLAW_SERVICE_KIND?.trim() === "node";
}
...
async function writeSystemdGatewayEnvironmentFile(params: {
  ...
  /** MarketingClaw-managed keys that must not be preserved from an old env file; stale file values
  ...
  fileManagedKeys?: ReadonlySet<string>;
  /** State-dir .env keys MarketingClaw previously managed but is now skipping (unresolved shell
  ...
  // MarketingClaw-managed keys (identified by inlineManagedKeys) are excluded: a stale
...
async function removeNodeSystemdManagedEnvironmentKeys(env: GatewayServiceEnv): Promise<void> {
  ...
  const managedKeys = new Set([normalizeSystemdEnvironmentKey("MARKETINGCLAW_GATEWAY_TOKEN")]);
