...
import type { Message, Usage } from "marketingclaw/plugin-sdk/llm";
import { afterAll, describe, expect, it } from "vitest";
import { exportTrajectoryBundle, resolveDefaultTrajectoryExportDir } from "./export.js";
import { TRAJECTORY_RUNTIME_FILE_MAX_BYTES, resolveTrajectoryPointerFilePath } from "./paths.js";
import type { TrajectoryEvent } from "./types.js";

const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-trajectory-"));
let tempDirId = 0;
...
describe("exportTrajectoryBundle", () => {
  it("sanitizes session ids in default export directory names", () => {
    ...
    expect(outputDir).toBe(
      path.join(
        ...
        ".marketingclaw",
        "trajectory-exports",
        "marketingclaw-trajectory-___evil_-2026-04-22T08-00-00",
      ),
    );
  });
    ...
    fs.writeFileSync(
      ...
      [
        ...
        },
        {
          ...
          sessionId: "session-1",
          data: {
            harness: { type: "marketingclaw", token: rawSecrets[3] },
            metadata: {
  ...
  it("uses the recorded runtime pointer before current environment overrides", async () => {
    ...
    const previous = process.env.MARKETINGCLAW_TRAJECTORY_DIR;
    process.env.MARKETINGCLAW_TRAJECTORY_DIR = envRuntimeDir;
    try {
      const bundle = await exportTrajectoryBundle({
        outputDir,
        sessionFile,
      ...
      if (previous === undefined) {
        ...
        process.env.MARKETINGCLAW_TRAJECTORY_DIR = previous;
      }
    }
  });
  ...
  it("exports merged runtime and transcript events plus convenience files", async () => {
    ...
    const runtimeEvents: TrajectoryEvent[] = [
      ...
      },
      {
        ...
        sessionId: "session-1",
        data: {
          harness: { type: "marketingclaw", version: "0.1.0" },
