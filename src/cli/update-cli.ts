// Commander wiring for `marketingclaw update`, its status/finalize subcommands, and help text.
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["marketingclaw update repair", "Rerun post-update doctor and plugin convergence."],
          ["marketingclaw update repair --channel beta", "Repair against the beta update channel."],
          ["marketingclaw update repair --json", "JSON output for automation."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          ...
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.marketingclaw.ai/cli/update")}`,
    )
...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    ...
    .addHelpText("after", () => {
      const examples = [
        ["marketingclaw update", "Update a source checkout (git)"],
        [
          "marketingclaw update --channel extended-stable",
          "Switch to the monthly supported npm channel",
        ],
        ["marketingclaw update --channel beta", "Switch to beta channel (git + npm)"],
        ["marketingclaw update --channel dev", "Switch to dev channel (git + npm)"],
        ["marketingclaw update --tag beta", "One-off update to a dist-tag or version"],
        ["marketingclaw update --tag main", "One-off package update from GitHub main"],
        ["marketingclaw update --dry-run", "Preview actions without changing anything"],
        ["marketingclaw update --no-restart", "Update without restarting the service"],
        ["marketingclaw update --json", "Output result as JSON"],
        ["marketingclaw update --yes", "Non-interactive (accept downgrade prompts)"],
        ["marketingclaw update repair", "Repair stranded post-update plugin state"],
        [
          "marketingclaw update --acknowledge-clawhub-risk",
          "Acknowledge ClawHub plugin trust warnings",
        ],
        ["marketingclaw update wizard", "Interactive update wizard"],
        ["marketingclaw --update", "Shorthand for marketingclaw update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run marketingclaw update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.marketingclaw.ai/cli/update")}`;
    })
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["marketingclaw update status", "Show channel + version status."],
          ["marketingclaw update status --json", "JSON output."],
          ["marketingclaw update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
