...
import { readBestEffortConfig, type MarketingClawConfig } from "../config/config.js";
  ...
  if (!params.approvals) {
    return {
      ...
      note: params.nativePolicy
        ? "This node enforces a host-native exec policy; MarketingClaw approvals-file policy math does not apply."
...
export function registerExecApprovalsCli(program: Command) {
  ...
  const approvals = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/approvals", "docs.marketingclaw.ai/cli/approvals")}\n`,
    );
