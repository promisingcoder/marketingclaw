...
const resolveDevWorkspaceDir = (env: NodeJS.ProcessEnv = process.env): string => {
  ...
  const profile = normalizeOptionalLowercaseString(env.MARKETINGCLAW_PROFILE);
  if (profile === "dev") {
    return baseDir;
  }
...
async function ensureDevWorkspace(dir: string) {
  ...
  const [agents, soul, tools, identity, user] = await Promise.all([
    loadDevTemplate(
      "AGENTS.dev.md",
      `# AGENTS.md - MarketingClaw Dev Workspace\n\nDefault dev workspace for marketingclaw gateway --dev.\n`,
    ),
    loadDevTemplate(
      "SOUL.dev.md",
