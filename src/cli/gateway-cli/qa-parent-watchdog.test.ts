...
describe("installQaParentWatchdog", () => {
  ...
  it("exits when the QA parent process disappears", async () => {
    ...
    const handle = installQaParentWatchdog({
      ...
      cwd: () => "/tmp/marketingclaw-qa-suite-test",
      env: {
        [QA_PARENT_PID_ENV]: "12345",
        [QA_STAGED_RUNTIME_ROOT_ENV]: "/repo/.artifacts/qa-runtime/marketingclaw-qa-suite-test",
        [QA_TEMP_ROOT_ENV]: "/tmp/marketingclaw-qa-suite-test",
      },
      exit,
      kill,
      logger,
    ...
    await vi.waitFor(() => {
      ...
      expect(rm).toHaveBeenCalledWith("/tmp/marketingclaw-qa-suite-test");
      expect(rm).toHaveBeenCalledWith("/repo/.artifacts/qa-runtime/marketingclaw-qa-suite-test");
