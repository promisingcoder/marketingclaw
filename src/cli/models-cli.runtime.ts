...
export function rejectAgentScopedModelWrite(
  ...
  throw new Error(
    `marketingclaw models ${commandName} does not support --agent; it only updates global model defaults. Remove --agent, or run ${formatCliCommand("marketingclaw agents list")} and set the per-agent model in agent config.`,
  );
}
