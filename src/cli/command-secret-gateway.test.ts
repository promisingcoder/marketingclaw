...
describe("resolveCommandSecretRefsViaGateway", () => {
  function makeTalkProviderApiKeySecretRefConfig(envKey: string): MarketingClawConfig {
  ...
  async function createExecProviderConfig(refId: string): Promise<{
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-command-secret-exec-"));
    tempRoots.add(root);
    ...
    const resolverScript = [
      ...
      "  fs.writeFileSync(process.env.MARKETINGCLAW_EXEC_MARKER, 'executed');",
        ...
        secrets: {
          providers: {
            default: {
              ...
              env: { MARKETINGCLAW_EXEC_MARKER: markerPath },
