...
describe("argv-invocation", () => {
  it("resolves root help and empty command path", () => {
    expect(resolveCliArgvInvocation(["node", "marketingclaw", "--help"])).toEqual({
  ...
  it("resolves command path and primary with root options", () => {
    expect(
      resolveCliArgvInvocation(["node", "marketingclaw", "--profile", "work", "gateway", "status"]),
    ).toEqual({
      argv: ["node", "marketingclaw", "--profile", "work", "gateway", "status"],
