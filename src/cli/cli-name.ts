...
const DEFAULT_CLI_NAME = "marketingclaw";

const KNOWN_CLI_NAMES = new Set([DEFAULT_CLI_NAME]);
const CLI_PREFIX_RE = /^(?:((?:pnpm|npm|bunx|npx)\s+))?(marketingclaw)\b/;

/** Resolve the displayed CLI binary name from argv, falling back to `marketingclaw`. */
...
/** Replace a leading `marketingclaw` command prefix with the active CLI name. */
