...
describe("runPluginPayloadSmokeCheck", () => {
  let tmpRoot: string;
  beforeEach(async () => {
    tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-payload-smoke-"));
  });
  afterEach(async () => {
  ...
  async function linkMarketingClawPeerToHost(dir: string): Promise<void> {
    await fs.mkdir(path.join(dir, "node_modules"), { recursive: true });
    await fs.symlink(
      resolveTestHostRoot(),
      path.join(dir, "node_modules", "marketingclaw"),
      "junction",
    );
  }
  ...
  it("reports ok for a record whose package.json + main file exist", async () => {
    ...
    await writePackage(
      dir,
      { name: "@marketingclaw/discord", main: "dist/index.js" },
      "module.exports = {};",
    );
  ...
  it("accepts a manifest with no main field (MarketingClaw plugins commonly use `exports` or `marketingclaw.extensions`)", async () => {
  ...
  it("accepts a manifest that declares an existing `marketingclaw.extensions` entry and no `main`", async () => {
    const dir = path.join(tmpRoot, "brave");
    await writePackage(dir, {
  ...
  it("reports a failure when `marketingclaw.extensions` contains invalid entries", async () => {
    const dir = path.join(tmpRoot, "brave");
    await writePackage(dir, {
      name: "@marketingclaw/brave-plugin",
      marketingclaw: { extensions: ["./index.js", " "] },
    ...
    expect(result.failures).toStrictEqual([
      {
        ...
        reason: "missing-extension-entry",
        detail:
          "Plugin extension entry validation failed: package.json marketingclaw.extensions[1] must be a non-empty string",
      },
    ]);
  });
    ...
    expect(result.failures).toStrictEqual([
      {
        ...
        reason: "missing-extension-entry",
        detail:
          "Plugin extension entry validation failed: package.json marketingclaw.extensions is empty",
      },
    ]);
  });
  ...
  it("accepts a packaged TypeScript extension entry when compiled runtime output exists", async () => {
    ...
    await writePackage(dir, {
      ...
      marketingclaw: { extensions: ["./index.ts"] },
    });
  ...
  it("reports a failure when an marketingclaw peer link is missing", async () => {
    const dir = path.join(tmpRoot, "codex");
    await writePackage(
      dir,
      {
        name: "@marketingclaw/codex",
        main: "dist/index.js",
        peerDependencies: { marketingclaw: ">=2026.5.18-beta.1" },
      },
      "export default {};\n",
    );
    ...
    expect(result.failures).toStrictEqual([
      {
        ...
        detail: `Plugin declares peerDependency "marketingclaw" but peer link audit failed: missing ${path.join(
          dir,
          "node_modules",
          "marketingclaw",
        )}.`,
  ...
  it("reports a failure when an marketingclaw peer link is a stale real directory", async () => {
    const dir = path.join(tmpRoot, "codex");
    await writePackage(
      dir,
      {
    ...
    const stalePeerDir = path.join(dir, "node_modules", "marketingclaw");
  ...
  it("reports a failure when an marketingclaw peer link points at the wrong package root", async () => {
    const dir = path.join(tmpRoot, "codex");
    await writePackage(
      dir,
      {
    ...
    const wrongHostRoot = path.join(tmpRoot, "old-marketingclaw");
    await fs.mkdir(wrongHostRoot, { recursive: true });
    await fs.mkdir(path.join(dir, "node_modules"), { recursive: true });
    await fs.symlink(wrongHostRoot, path.join(dir, "node_modules", "marketingclaw"), "junction");
  ...
  it("accepts an marketingclaw peer link when it resolves to the host package root", async () => {
    const dir = path.join(tmpRoot, "codex");
    await writePackage(
      dir,
      {
    ...
    await linkMarketingClawPeerToHost(dir);
  ...
  it("reports a failure when an `marketingclaw.extensions` entry file is missing", async () => {
    const dir = path.join(tmpRoot, "brave");
    await writePackage(dir, {
      name: "@marketingclaw/brave-plugin",
