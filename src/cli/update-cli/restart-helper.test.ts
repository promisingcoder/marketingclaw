...
`,
  ...
  function expectWindowsRestartWaitOrdering(content: string, port = 18789) {
    const stateCheck = "$taskState = Get-MarketingClawScheduledTaskState -TaskName $taskName";
    const runningGuard = 'if ($taskState -eq "Running")';
    const endCommand =
      'Invoke-MarketingClawSchtasksWithTimeout -Arguments @("/End", "/TN", $taskName) -TimeoutSeconds 10';
    const skipEndLog = "marketingclaw restart skipped schtasks end";
    const pollLoop = "for ($attempt = 1; $attempt -le 10; $attempt++)";
    const pollCall = `Get-MarketingClawListenerPids -Port $port`;
    const forceKillBranch = "if ($attempt -eq 10)";
    const forceKillCommand = "Stop-Process -Id $listenerPid -Force";
    const runCommand =
      'Invoke-MarketingClawSchtasksWithTimeout -Arguments @("/Run", "/TN", $taskName) -TimeoutSeconds 30';
  ...
  describe("prepareRestartScript", () => {
    it("creates a systemd restart script on Linux", async () => {
      ...
      expect(content).toContain("systemctl --user restart 'marketingclaw-gateway.service'");
    ...
    it("creates restart scripts in a private temp directory with exclusive creation", async () => {
      ...
      const oldCandidatePath = path.join(os.tmpdir(), `marketingclaw-restart-${timestamp}.sh`);
      const victimDir = await makeTempDir("marketingclaw-restart-helper-victim-");
      ...
      try {
        ...
        expect(path.basename(scriptDir)).toMatch(/^marketingclaw-restart-/);
    ...
    it("uses MARKETINGCLAW_SYSTEMD_UNIT override for systemd scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        MARKETINGCLAW_PROFILE: "default",
        MARKETINGCLAW_SYSTEMD_UNIT: "custom-gateway",
      });
    ...
    it("fails with sudo systemd guidance when the gateway unit is system-scoped", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const tmpDir = await makeTempDir("marketingclaw-restart-helper-");
...
`,
      ...
      const { scriptPath } = await prepareAndReadScript({
        ...
        MARKETINGCLAW_STATE_DIR: path.join(tmpDir, "state"),
      });
      const result = await executeScript(scriptPath, {
        PATH: `${fakeBinDir}:${process.env.PATH ?? ""}`,
        MARKETINGCLAW_SYSTEMCTL_CALLS: callsPath,
      });
      const calls = await fs.readFile(callsPath, "utf-8");

      expect(result.code).toBe(78);
      expect(result.stderr).toContain("system-scoped marketingclaw gateway unit detected");
      expect(result.stderr).toContain("sudo systemctl restart marketingclaw-gateway.service");
      expect(calls).toContain("--user is-active --quiet marketingclaw-gateway.service");
      expect(calls).toContain("is-active --quiet marketingclaw-gateway.service");
      expect(calls).not.toContain("--user restart marketingclaw-gateway.service");
    });
    ...
    it("creates a launchd restart script on macOS", async () => {
      ...
      expect(scriptPath.endsWith(".sh")).toBe(true);
      expect(content).toContain("#!/bin/sh");
      expect(content).toContain("launchctl kickstart -k 'gui/501/ai.marketingclaw.gateway'");
      // Should clear disabled state and fall back to bootstrap when kickstart fails.
      expect(content).toContain("launchctl enable 'gui/501/ai.marketingclaw.gateway'");
    ...
    it("captures macOS launchctl stderr to ~/.marketingclaw/logs/gateway-restart.log (#68486)", async () => {
      ...
      expect(content).toContain(
        "exec >>'/Users/testuser/.marketingclaw/logs/gateway-restart.log' 2>&1",
      );
    ...
    it("uses MARKETINGCLAW_STATE_DIR for the macOS update restart log", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;
      ...
      expect(content).toContain(
        "if mkdir -p '/tmp/marketingclaw-state/logs' 2>/dev/null && : >>'/tmp/marketingclaw-state/logs/gateway-restart.log' 2>/dev/null; then",
      );
      expect(content).toContain("exec >>'/tmp/marketingclaw-state/logs/gateway-restart.log' 2>&1");
      await cleanupScript(scriptPath);
    });
...
`,
      ...
      expect(log).toContain(
        "marketingclaw restart attempt source=update target=ai.marketingclaw.gateway",
      );
      expect(log).toContain("launchctl kickstart -k gui/501/ai.marketingclaw.gateway");
      expect(log).toContain("marketingclaw restart failed source=update status=42");
      expect(log).not.toContain("marketingclaw restart done source=update");
    });
...
`,
      ...
      const { scriptPath } = await prepareAndReadScript({
        MARKETINGCLAW_PROFILE: "default",
        HOME: path.join(tmpDir, "home"),
        MARKETINGCLAW_STATE_DIR: stateFile,
      });
    ...
    it("logs custom macOS launchd labels without shell expansion", async () => {
      ...
      const { scriptPath } = await prepareAndReadScript({
        MARKETINGCLAW_LAUNCHD_LABEL: "ai.marketingclaw.$(echo injected)",
        HOME: path.join(tmpDir, "home"),
        MARKETINGCLAW_STATE_DIR: stateDir,
      });
      ...
      expect(result.code).toBeNull();
      expect(log).toContain("target=ai.marketingclaw.$(echo injected)");
      expect(log).not.toContain("target=ai.marketingclaw.injected");
    });

    it("uses MARKETINGCLAW_LAUNCHD_LABEL override on macOS", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;

      const { scriptPath, content } = await prepareAndReadScript({
        MARKETINGCLAW_PROFILE: "default",
        MARKETINGCLAW_LAUNCHD_LABEL: "com.custom.marketingclaw",
      });
      expect(content).toContain("launchctl kickstart -k 'gui/501/com.custom.marketingclaw'");
      await cleanupScript(scriptPath);
    });
    ...
    it("creates a guarded schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain("$taskName = 'MarketingClaw Gateway'");
      expect(content).toContain("function Invoke-MarketingClawSchtasksWithTimeout");
      expect(content).toContain("function Get-MarketingClawScheduledTaskState");
      expect(content).toContain("function Invoke-MarketingClawStartupLauncher");
      expect(content).toContain("Get-ScheduledTask -TaskName $TaskName");
      expect(content).toContain("marketingclaw restart skipped schtasks end");
      expect(content).toContain(
        '$launcherPath = Join-Path $env:USERPROFILE ".marketingclaw\\gateway.cmd"',
      );
      expect(content).toContain("marketingclaw restart launched startup fallback");
      expectWindowsRestartWaitOrdering(content);
      expect(content).toContain('del "%~f0" >nul 2>&1');
      expect(content).toContain('rmdir "%MARKETINGCLAW_RESTART_SCRIPT_DIR%" >nul 2>&1');
      await cleanupScript(scriptPath);
    });

    it("uses MARKETINGCLAW_WINDOWS_TASK_NAME override on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });

      const { scriptPath, content } = await prepareAndReadScript({
        MARKETINGCLAW_PROFILE: "default",
        MARKETINGCLAW_WINDOWS_TASK_NAME: "MarketingClaw Gateway (custom)",
      });
      expect(content).toContain("$taskName = 'MarketingClaw Gateway (custom)'");
      expect(content).toContain("Get-MarketingClawScheduledTaskState -TaskName $taskName");
      expect(content).toContain(
        'Invoke-MarketingClawSchtasksWithTimeout -Arguments @("/End", "/TN", $taskName) -TimeoutSeconds 10',
      );
      expect(content).toContain("$status = Invoke-MarketingClawStartupLauncher");
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in service names", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        MARKETINGCLAW_PROFILE: "production",
      });
      expect(content).toContain("marketingclaw-gateway-production.service");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in macOS launchd label", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        MARKETINGCLAW_PROFILE: "staging",
      });
      expect(content).toContain("gui/502/ai.marketingclaw.staging");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in Windows task name", async () => {
      ...
      expect(content).toContain("$taskName = 'MarketingClaw Gateway (production)'");
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("escapes single quotes in profile names for shell scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        MARKETINGCLAW_PROFILE: "it's-a-test",
      });
    ...
    it("shell-escapes the label in the plist path on macOS", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        HOME: "/Users/testuser",
        MARKETINGCLAW_LAUNCHD_LABEL: "ai.marketingclaw.it's-a-test",
      });
      // The plist path must also shell-escape the label to prevent injection
      expect(content).toContain("ai.marketingclaw.it'\\''s-a-test.plist");
      await cleanupScript(scriptPath);
    });
    ...
    it("rejects unsafe batch profile names on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });
      const scriptPath = await prepareRestartScript({
        MARKETINGCLAW_PROFILE: "test&whoami",
      });
