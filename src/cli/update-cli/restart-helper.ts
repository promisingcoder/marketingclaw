...
function resolveLaunchdLabel(env: NodeJS.ProcessEnv): string {
  const override = normalizeOptionalString(env.MARKETINGCLAW_LAUNCHD_LABEL);
  if (override) {
    return override;
  }
  return resolveGatewayLaunchAgentLabel(env.MARKETINGCLAW_PROFILE);
}
...
export async function prepareRestartScript(
  ...
  try {
    if (platform === "linux") {
      ...
      filename = `marketingclaw-restart-${timestamp}.sh`;
...
exec 3>&2
${logSetup}
printf '[%s] marketingclaw restart attempt source=update target=%s\\n' "$(date -u +%FT%TZ)" '${escaped}' >&2
if systemctl --user is-active --quiet '${escaped}' || systemctl --user is-enabled --quiet '${escaped}'; then
  if systemctl --user restart '${escaped}'; then
    status=0
    printf '[%s] marketingclaw restart done source=update\\n' "$(date -u +%FT%TZ)" >&2
  else
    status=$?
    printf '[%s] marketingclaw restart failed source=update status=%s\\n' "$(date -u +%FT%TZ)" "$status" >&2
  fi
elif systemctl is-active --quiet '${escaped}' || systemctl is-enabled --quiet '${escaped}'; then
  status=78
  printf '[%s] system-scoped marketingclaw gateway unit detected; update cannot restart it without sudo. Run: sudo systemctl restart %s\\n' "$(date -u +%FT%TZ)" '${escaped}' >&2
...
# Self-cleanup (log is retained under the MarketingClaw state logs directory).
script_dir=$(dirname "$0")
rm -f "$0"
rmdir "$script_dir" 2>/dev/null || true
exit "$status"
`;
      ...
      filename = `marketingclaw-restart-${timestamp}.cmd`;
...
setlocal
set "MARKETINGCLAW_RESTART_SCRIPT=%~f0"
set "MARKETINGCLAW_RESTART_SCRIPT_DIR=%~dp0."
powershell -NoProfile -ExecutionPolicy Bypass -Command "$p=$env:MARKETINGCLAW_RESTART_SCRIPT; $s=Get-Content -Raw -LiteralPath $p; $m='# POWERSHELL'; $i=$s.IndexOf($m); if ($i -lt 0) { exit 1 }; Invoke-Expression $s.Substring($i)"
set "status=%ERRORLEVEL%"
del "%~f0" >nul 2>&1
rmdir "%MARKETINGCLAW_RESTART_SCRIPT_DIR%" >nul 2>&1
exit /b %status%
# POWERSHELL
# Wait briefly to ensure file locks are released after update.
...
try {
  ...
  Add-Content -LiteralPath $logPath -Value "[$(Get-Date -Format o)] marketingclaw restart log initialized"
} catch {
...
function Join-MarketingClawProcessArguments {
...
function Invoke-MarketingClawSchtasksWithTimeout {
  param(
    [string[]]$Arguments,
    [int]$TimeoutSeconds
  )
  ...
  try {
    ...
    $startInfo.Arguments = Join-MarketingClawProcessArguments -Arguments $Arguments
    ...
    if (-not $process.WaitForExit($TimeoutSeconds * 1000)) {
      ...
      Write-RestartLog "marketingclaw restart schtasks timeout source=update args=$($Arguments -join ' ')"
      return 124
    }
    ...
    Write-RestartLog "marketingclaw restart schtasks failed source=update args=$($Arguments -join ' ') error=$($_.Exception.Message)"
    return 1
  }
}

function Get-MarketingClawScheduledTaskState {
...
function Get-MarketingClawListenerPids {
  param([int]$Port)
  $listenerPids = @()
...
function Invoke-MarketingClawStartupLauncher {
  $launcherPath = Join-Path $env:USERPROFILE ".marketingclaw\\gateway.cmd"
  if (-not (Test-Path -LiteralPath $launcherPath)) {
    Write-RestartLog "marketingclaw restart startup launcher missing source=update path=$launcherPath"
    return 1
  }

  try {
    Start-Process -FilePath $launcherPath -WindowStyle Hidden | Out-Null
    Write-RestartLog "marketingclaw restart launched startup fallback source=update path=$launcherPath"
    return 0
  } catch {
    Write-RestartLog "marketingclaw restart startup fallback failed source=update error=$($_.Exception.Message)"
    return 1
  }
}
...
$port = ${port}
Write-RestartLog "marketingclaw restart attempt source=update target=$taskName"

$taskState = Get-MarketingClawScheduledTaskState -TaskName $taskName
if ($taskState -eq "Running") {
  $endStatus = Invoke-MarketingClawSchtasksWithTimeout -Arguments @("/End", "/TN", $taskName) -TimeoutSeconds 10
  if ($endStatus -ne 0) {
    Write-RestartLog "marketingclaw restart schtasks end did not complete cleanly source=update status=$endStatus"
  }
} else {
  Write-RestartLog "marketingclaw restart skipped schtasks end source=update state=$taskState"
}

for ($attempt = 1; $attempt -le 10; $attempt++) {
  $listeners = @(Get-MarketingClawListenerPids -Port $port)
  ...
  if ($attempt -eq 10) {
    foreach ($listenerPid in $listeners) {
      try {
        ...
        Write-RestartLog "marketingclaw restart killed stale listener source=update pid=$listenerPid"
      } catch {
        Write-RestartLog "marketingclaw restart failed to kill stale listener source=update pid=$listenerPid error=$($_.Exception.Message)"
      }
    }
    break
  }
...
$status = Invoke-MarketingClawSchtasksWithTimeout -Arguments @("/Run", "/TN", $taskName) -TimeoutSeconds 30
if ($status -ne 0) {
  $status = Invoke-MarketingClawStartupLauncher
}
if ($status -eq 0) {
  Write-RestartLog "marketingclaw restart done source=update"
} else {
  Write-RestartLog "marketingclaw restart failed source=update status=$status"
}
...
`;
    ...
    const scriptDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-restart-"));
