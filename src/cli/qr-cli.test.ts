...
describe("registerQrCli", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    resetRuntimeCapture();
    vi.stubEnv("MARKETINGCLAW_GATEWAY_TOKEN", "");
    vi.stubEnv("MARKETINGCLAW_GATEWAY_PASSWORD", "");
  ...
  it("renders ASCII QR by default", async () => {
    ...
    expect(output).toContain("marketingclaw devices approve <requestId>");
  });
  ...
  it("uses MARKETINGCLAW_GATEWAY_PASSWORD without resolving local password SecretRef", async () => {
    vi.stubEnv("MARKETINGCLAW_GATEWAY_PASSWORD", "password-from-env");
    loadConfig.mockReturnValue(
      createLocalGatewayConfigWithAuth(
        createLocalGatewayPasswordRefAuth("MISSING_LOCAL_GATEWAY_PASSWORD"),
      ),
