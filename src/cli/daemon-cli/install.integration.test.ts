...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("marketingclaw-daemon-install-int-");
    configPath = path.join(tempHome, "marketingclaw.json");
    process.env.HOME = tempHome;
    process.env.MARKETINGCLAW_STATE_DIR = tempHome;
  ...
  it("refuses service install when config was written by a newer MarketingClaw", async () => {
    await fs.writeFile(
      configPath,
      JSON.stringify(
        {
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.MARKETINGCLAW_GATEWAY_TOKEN).toBeUndefined();
  });
});
