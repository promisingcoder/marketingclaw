...
const loadConfig = vi.fn<() => MarketingClawConfig>(() => ({
  gateway: {
    auth: {
      token: "config-token",
    },
...
function stubServiceGatewayTokenEnv() {
  service.readCommand.mockResolvedValue({
    programArguments: [],
    environment: {
      MARKETINGCLAW_GATEWAY_TOKEN: "service-token",
...
describe("runServiceRestart token drift", () => {
  ...
  beforeEach(() => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: [],
      environment: { MARKETINGCLAW_GATEWAY_TOKEN: "service-token" },
    });
    stubEmptyGatewayEnv();
  });
    ...
    await withUnsupportedGatewayService(async (unsupportedService) => {
      await expect(
        runServiceStart({
          ...
          renderStartHints: () => ["marketingclaw gateway install"],
          opts: { json: true },
          onNotLoaded,
        }),
  ...
  it("prints the container restart hint when restart is requested for a not-loaded service", async () => {
    ...
    vi.stubEnv("MARKETINGCLAW_CONTAINER_HINT", "marketingclaw-demo-container");
    ...
    expect(runtimeLogs).toContain(
      "Start with: Restart the container or the service that manages it for marketingclaw-demo-container.",
    );
  });
  ...
  it("compares restart drift against config token even when caller env is set", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: [],
      environment: { MARKETINGCLAW_GATEWAY_TOKEN: "env-token" },
    });
    vi.stubEnv("MARKETINGCLAW_GATEWAY_TOKEN", "env-token");
  ...
  it("repairs stale loaded services during start before reporting success", async () => {
    service.readCommand.mockResolvedValue({
      programArguments: ["marketingclaw", "gateway"],
    ...
    const repairLoadedService = vi.fn<RepairLoadedService>(async (ctx) => {
      ...
      return {
        ...
        warnings: ["service was installed by MarketingClaw 2026.4.24, current CLI is 2026.5.2"],
        loaded: true,
      };
    });
    ...
    expect(payload.warnings).toEqual(
      expect.arrayContaining([
        expect.stringContaining("service was installed by MarketingClaw"),
  ...
  it("fails start with an install hint when a stale loaded service has no repair callback", async () => {
    ...
    expect(payload.hints).toEqual(["marketingclaw gateway install --force"]);
    expect(service.restart).not.toHaveBeenCalled();
  });
  ...
  it("falls back to not-loaded hints when start finds no install artifacts", async () => {
    ...
    expect(payload.hints?.includes("marketingclaw gateway install")).toBe(true);
    expect(
      payload.hintItems?.some(
        (item) => item.kind === "install" && item.text === "marketingclaw gateway install",
      ),
    ).toBe(true);
    expect(service.restart).not.toHaveBeenCalled();
  });
