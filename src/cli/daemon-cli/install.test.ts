...
const createInstallPlanFixture = vi.hoisted(() => {
  return async (params?: {
    ...
    if (params?.wrapperPath || params?.env?.MARKETINGCLAW_WRAPPER) {
      environment.MARKETINGCLAW_WRAPPER = params.wrapperPath ?? params.env?.MARKETINGCLAW_WRAPPER;
    }
    return {
...
vi.mock("../../daemon/program-args.js", () => ({
  ...
  resolveMarketingClawWrapperPath: async (value: string | undefined) => value?.trim() || undefined,
}));
...
function mockResolvedGatewayTokenSecretRef() {
  resolveSecretInputRefMock.mockReturnValue({
    ref: { source: "env", provider: "default", id: "MARKETINGCLAW_GATEWAY_TOKEN" },
  });
  resolveSecretRefValuesMock.mockResolvedValue(
    new Map([["env:default:MARKETINGCLAW_GATEWAY_TOKEN", "resolved-from-secretref"]]),
  );
}
...
describe("runDaemonInstall", () => {
  ...
  it("does not treat env-template gateway.auth.token as plaintext during install", async () => {
    loadConfigMock.mockReturnValue({
      gateway: { auth: { mode: "token", token: "${MARKETINGCLAW_GATEWAY_TOKEN}" } },
    });
    mockResolvedGatewayTokenSecretRef();
  ...
  it("reinstalls when the loaded service still embeds MARKETINGCLAW_GATEWAY_TOKEN", async () => {
    service.isLoaded.mockResolvedValue(true);
    service.readCommand.mockResolvedValue({
      programArguments: ["marketingclaw", "gateway", "run"],
      environment: {
        MARKETINGCLAW_GATEWAY_TOKEN: "stale-service-token",
      },
    } as never);
    ...
    expect(installDaemonServiceAndEmitMock).toHaveBeenCalledTimes(1);
    expect(actionState.warnings).toContain(
      "Gateway service MARKETINGCLAW_GATEWAY_TOKEN differs from the current install plan; refreshing the install.",
    );
  });
  ...
  it("returns already-installed when the embedded gateway token matches the install plan", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: ["marketingclaw", "gateway", "run"],
      environment: {
        MARKETINGCLAW_GATEWAY_TOKEN: "durable-token",
      },
    } as never);
    buildGatewayInstallPlanMock.mockResolvedValueOnce({
      programArguments: ["marketingclaw", "gateway", "run"],
  ...
  it("preserves wrapper env from an installed but unloaded service during forced reinstall", async () => {
    service.isLoaded.mockResolvedValue(false);
    service.readCommand.mockResolvedValue({
      programArguments: ["/usr/local/bin/marketingclaw-doppler", "gateway", "run"],
      environment: {
    ...
    expectFields(installPlanArg, { wrapperPath: "/usr/local/bin/marketingclaw-doppler" });
  ...
  it("reinstalls when wrapper command matches but wrapper env is missing", async () => {
    ...
    await runDaemonInstall({
      json: true,
      wrapper: "/usr/local/bin/marketingclaw-doppler",
    });

    expect(installDaemonServiceAndEmitMock).toHaveBeenCalledTimes(1);
    expect(actionState.warnings).toContain(
      "Gateway service MARKETINGCLAW_WRAPPER differs from the current wrapper install plan; refreshing the install.",
    );
  });
  ...
  it("does not reinstall when MARKETINGCLAW_GATEWAY_TOKEN comes from an env file", async () => {
    service.isLoaded.mockResolvedValue(true);
    service.readCommand.mockResolvedValue({
      programArguments: ["marketingclaw", "gateway", "run"],
      environment: {
  ...
  it("does not reuse stale service control env during forced reinstall", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: ["marketingclaw", "gateway", "run"],
      environment: {
        MARKETINGCLAW_STATE_DIR: "/tmp/marketingclaw-doctor-manual",
        MARKETINGCLAW_CONFIG_PATH: "/tmp/marketingclaw-doctor-manual/marketingclaw.json",
        MARKETINGCLAW_GATEWAY_TOKEN: "stale-service-token",
    ...
    try {
      ...
      expect(env.MARKETINGCLAW_STATE_DIR).toBeUndefined();
      expect(env.MARKETINGCLAW_CONFIG_PATH).toBeUndefined();
