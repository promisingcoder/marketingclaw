...
describe("pickTagline", () => {
  ...
  it("keeps MARKETINGCLAW_TAGLINE_INDEX behavior in random mode", () => {
    const value = pickTagline({
      mode: "random",
      env: { MARKETINGCLAW_TAGLINE_INDEX: "0" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(
  ...
  it("ignores partial MARKETINGCLAW_TAGLINE_INDEX values", () => {
    expect(
      pickTagline({
        mode: "random",
        env: { MARKETINGCLAW_TAGLINE_INDEX: "1abc" } as NodeJS.ProcessEnv,
        random: () => 0,
      }),
