...
describe("shouldSkipRespawnForArgv", () => {
  it.each([
    ...
    { argv: ["node", "marketingclaw", "terminal"] },
    { argv: ["node", "marketingclaw", "chat"] },
    { argv: ["node", "marketingclaw", "gateway"] },
    { argv: ["node", "marketingclaw", "gateway", "--port", "14720", "--bind", "loopback"] },
    { argv: ["node", "marketingclaw", "gateway", "run", "--port=14720", "--bind", "loopback"] },
    {
      argv: [
        "node",
        "marketingclaw",
  ...
  it.each([
    ...
    { argv: ["node", "marketingclaw", "gateway", "status"] },
    { argv: ["node", "marketingclaw", "gateway", "call", "health"] },
...
describe("shouldSkipStartupEnvironmentRespawnForArgv", () => {
  it.each([
    { argv: ["node", "marketingclaw", "--help"] },
    { argv: ["node", "marketingclaw", "gateway"] },
    { argv: ["node", "marketingclaw", "gateway", "run", "--port=14720"] },
...
describe("dns cli", () => {
  it("prints setup info (no apply)", async () => {
    ...
    try {
      ...
      await program.parseAsync(["dns", "setup", "--domain", "marketingclaw.internal"], {
        from: "user",
      });
      const output = log.mock.calls.map((call) => call.join(" ")).join("\\n");
      expect(output).toContain("DNS setup");
      expect(output).toContain("marketingclaw.internal");
    } finally {
      log.mockRestore();
    }
  });
