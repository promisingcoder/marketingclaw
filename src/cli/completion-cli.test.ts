...
function createCompletionProgram(): Command {
  ...
  program.option(
    "--status-json",
    "Output JSON (alias for `models status --json`) in $MARKETINGCLAW_STATE_DIR",
  );
...
describe("completion-cli", () => {
  it("generates zsh functions for nested subcommands", () => {
    ...
    expect(script).toContain("_marketingclaw_gateway()");
    expect(script).toContain("(status) _marketingclaw_gateway_status ;;");
    expect(script).toContain("(restart) _marketingclaw_gateway_restart ;;");
    expect(script).toContain("--force[Force the action]");
    expect(script).toContain("\\`models status --json\\`");
    expect(script).toContain("\\$MARKETINGCLAW_STATE_DIR");
  });

  it("escapes zsh option descriptions for double-quoted arguments specs", () => {
    const program = new Command()
      .name("marketingclaw")
      .option("--literal", "Use $MARKETINGCLAW_STATE_DIR with `model/list` and John's profile");

    const script = getCompletionScript("zsh", program);

    expect(script).toContain(
      "--literal[Use \\$MARKETINGCLAW_STATE_DIR with \\`model/list\\` and John's profile]",
    );
  ...
  it("defers zsh registration until compinit is available", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-zsh-completion-"));
    try {
      const scriptPath = path.join(tempDir, "marketingclaw.zsh");
      ...
      const result = spawnSync(
        ...
        [
          ...
          `
            ...
            [[ -z "\${_comps[marketingclaw]-}" ]] || exit 10
            [[ "\${precmd_functions[(r)_marketingclaw_register_completion]}" = "_marketingclaw_register_completion" ]] || exit 11
            autoload -Uz compinit
            compinit -C
            _marketingclaw_register_completion
            [[ -z "\${precmd_functions[(r)_marketingclaw_register_completion]}" ]] || exit 12
            [[ "\${_comps[marketingclaw]-}" = "_marketingclaw_root_completion" ]]
          `,
        ],
        {
          encoding: "utf8",
  ...
  it("generates PowerShell command paths without the executable prefix", () => {
    ...
    expect(script).not.toContain("if ($commandPath -eq 'marketingclaw gateway') {");
  ...
  it("generates valid PowerShell root arrays when commands or options are empty", () => {
    const commandsOnly = new Command().name("marketingclaw");
    commandsOnly.command("status");
    const optionsOnly = new Command().name("marketingclaw").option("--json", "JSON output");
    const empty = new Command().name("marketingclaw");
  ...
  it("generates fish completions for root and nested command contexts", () => {
    ...
    expect(script).toContain(
      'complete -c marketingclaw -n "__fish_use_subcommand" -a "gateway" -d \'Gateway commands\'',
    );
    expect(script).toContain(
      'complete -c marketingclaw -n "__marketingclaw_command_path_matches gateway -- -t --token" -a "status" -d \'Show gateway status\'',
    );
    expect(script).toContain(
      "complete -c marketingclaw -n \"__marketingclaw_command_path_matches gateway -- -t --token\" -l force -d 'Force the action'",
    );
    expect(script).toContain(
      "complete -c marketingclaw -n \"__marketingclaw_command_path_matches gateway status -- -t --token\" -l json -d 'JSON output'",
    );
    expect(script).toContain("__marketingclaw_command_path_matches gateway -- -t --token");
  ...
  it("scopes fish value-taking option skips to the active command path", () => {
    ...
    expect(script).toContain("__marketingclaw_command_path_matches agent -- --verbose");
    expect(script).toContain("__marketingclaw_command_path_matches sessions cleanup --");
    expect(script).not.toContain(
      "__marketingclaw_command_path_matches sessions cleanup -- --verbose",
    );
    expect(script).toContain(
      "complete -c marketingclaw -n \"__marketingclaw_command_path_matches sessions cleanup --\" -l dry-run -d 'Preview cleanup'",
    );
  });
  ...
  it("generates valid Bash completion without subcommands", () => {
    ...
    const script = getCompletionScript("bash", new Command().name("marketingclaw"));
...
// Commander aliases are typeable commands (`marketingclaw capability` == `marketingclaw infer`),
...
describe("completion-cli command aliases", () => {
  it("completes root and nested aliases in zsh lists and dispatch", () => {
    ...
    expect(script).toContain("(infer|capability) _marketingclaw_infer ;;");
    expect(script).toContain("'create[Add a job]'");
    expect(script).toContain("(add|create) _marketingclaw_cron_add ;;");
  });
  ...
  it("offers options after a nested alias in bash", () => {
    ...
    const result = spawnSync(
      ...
      [
        ...
        "-c",
        `${script}
COMP_WORDS=(marketingclaw --profile work cron create --a)
COMP_CWORD=5
_marketingclaw_completion
printf '%s\\n' "\${COMPREPLY[@]}"
`,
      ],
      { encoding: "utf8" },
  ...
  it("completes aliases and their subtrees in fish", () => {
    ...
    expect(script).toContain(
      'complete -c marketingclaw -n "__fish_use_subcommand" -a "capability" -d \'Run inference\'',
    );
    expect(script).toContain(
      'complete -c marketingclaw -n "__marketingclaw_command_path_matches capability -- --profile" -a "embed" -d \'Embed text\'',
    );
    expect(script).toContain(
      'complete -c marketingclaw -n "__marketingclaw_command_path_matches cron -- --profile" -a "create" -d \'Add a job\'',
    );
    expect(script).toContain(
      "complete -c marketingclaw -n \"__marketingclaw_command_path_matches cron create -- --profile --at\" -l at -d 'Schedule time'",
    );
  });
