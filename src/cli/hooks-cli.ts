...
function buildHooksReport(config: MarketingClawConfig): HookStatusReport {
...
export function formatHooksList(report: HookStatusReport, opts: HooksListOptions): string {
  ...
  if (hooks.length === 0) {
    const message = opts.eligible
      ? `No eligible hooks found. Run \`${formatCliCommand("marketingclaw hooks list")}\` to see all hooks.`
      : "No hooks found.";
    return message;
  }
...
export function formatHookInfo(
  ...
  if (!hook) {
    ...
    return `Hook "${hookName}" not found. Run \`${formatCliCommand("marketingclaw hooks list")}\` to see available hooks.`;
  }
...
async function enableHook(hookName: string): Promise<void> {
  ...
  const config = (snapshot.sourceConfig ?? snapshot.config) as MarketingClawConfig;
...
export function registerHooksCli(program: Command): void {
  const hooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/hooks", "docs.marketingclaw.ai/cli/hooks")}\n`,
    );
  ...
  hooks
    .command("install")
    .description("Deprecated: install a hook pack via `marketingclaw plugins install`")
    ...
    .action(async (raw: string, opts: { link?: boolean; pin?: boolean }) => {
      defaultRuntime.log(
        theme.warn(
          "`marketingclaw hooks install` is deprecated; use `marketingclaw plugins install`.",
        ),
      );
  ...
  hooks
    .command("update")
    .description("Deprecated: update hook packs via `marketingclaw plugins update`")
    ...
    .action(async (id: string | undefined, opts: HooksUpdateOptions) => {
      defaultRuntime.log(
        theme.warn(
          "`marketingclaw hooks update` is deprecated; use `marketingclaw plugins update`.",
        ),
      );
