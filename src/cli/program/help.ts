...
const EXAMPLES = [
  ["marketingclaw onboard", "Run guided setup for a local Gateway, workspace, auth, and channels."],
  ["marketingclaw setup", "Create the baseline config, workspace, and session folders."],
  [
    "marketingclaw configure",
    "Change models, Gateway, channels, plugins, skills, and health checks.",
  ],
  ["marketingclaw status", "Check Gateway, channel, model, and recent-session status."],
  ["marketingclaw doctor --fix", "Repair common config, service, plugin, and channel problems."],
  ["marketingclaw channels add", "Add or update a chat channel account with guided prompts."],
  ["marketingclaw channels status", "See connected messaging accounts and login state."],
  [
    "marketingclaw --dev gateway",
    "Run a dev Gateway (isolated state/config) on ws://127.0.0.1:19001.",
  ],
  [
    "marketingclaw gateway run --force",
    "Start the Gateway and replace anything bound to its port.",
  ],
  ["marketingclaw models status", "Show model/provider auth health before running agents."],
  ["marketingclaw plugins list", "Inspect enabled, disabled, and installed plugins."],
  [
    'marketingclaw agent --to +15555550123 --message "Run summary" --deliver',
    "Run one agent turn through the Gateway and optionally deliver the reply.",
  ],
  [
    'marketingclaw message send --channel telegram --target @mychat --message "Hi"',
...
export function configureProgramHelp(
  ...
  program
    ...
    .option(
      "--container <name>",
      "Run the CLI inside a running Podman/Docker container named <name> (default: env MARKETINGCLAW_CONTAINER)",
    )
    .option(
      "--dev",
      "Dev profile: isolate state under ~/.marketingclaw-dev, default gateway port 19001, and shift derived ports (browser/canvas)",
    )
    .option(
      "--profile <name>",
      "Use a named profile (isolates MARKETINGCLAW_STATE_DIR/MARKETINGCLAW_CONFIG_PATH under ~/.marketingclaw-<name>)",
    )
    .option(
      "--log-level <level>",
  ...
  if (isRootVersionInvocation(process.argv)) {
    ...
    console.log(
      commit
        ? `MarketingClaw ${ctx.programVersion} (${commit})`
        : `MarketingClaw ${ctx.programVersion}`,
    );
    process.exit(0);
  }

  program.addHelpText("beforeAll", () => {
    if (hasEmittedCliBanner() || process.env.MARKETINGCLAW_SUPPRESS_HELP_BANNER === "1") {
      return "";
    }
    const rich = isRich();
  ...
  program.addHelpText("afterAll", ({ command }) => {
    ...
    const docs = formatDocsLink("/cli", "docs.marketingclaw.ai/cli");
