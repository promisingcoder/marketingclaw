...
describe("command-registry", () => {
  ...
  it("registers doctor placeholder for doctor primary command", () => {
    const program = createProgram();
    registerCoreCliCommands(program, testProgramContext, ["node", "marketingclaw", "doctor"]);
  ...
  it("keeps all placeholders for root help", () => {
    const program = createProgram();
    registerCoreCliCommands(program, testProgramContext, ["node", "marketingclaw", "--help"]);
  ...
  it("registers grouped core entry placeholders without duplicate command errors", async () => {
    const program = createProgram();
    registerCoreCliCommands(program, testProgramContext, ["node", "marketingclaw", "vitest"]);
    program.exitOverride();
    await withProcessArgv(["node", "marketingclaw", "status"], async () => {
      await program.parseAsync(["node", "marketingclaw", "status"]);
    });
