...
describe("reparseProgramFromActionArgs", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    buildParseArgvMock.mockReturnValue(["node", "marketingclaw", "status"]);
  ...
  it("uses action command name + args as fallback argv", async () => {
    const program = new Command().name("marketingclaw");
    setRawArgs(program, ["node", "marketingclaw", "status", "--json"]);
    ...
    expect(buildParseArgvMock).toHaveBeenCalledWith({
      ...
      rawArgs: ["node", "marketingclaw", "status", "--json"],
      fallbackArgv: ["status", "--json"],
    });
    expect(parseAsync).toHaveBeenCalledWith(["node", "marketingclaw", "status"]);
  });

  it("falls back to action args without command name when action has no name", async () => {
    const program = new Command().name("marketingclaw");
    setRawArgs(program, ["node", "marketingclaw"]);
    const parseAsync = vi.spyOn(program, "parseAsync").mockResolvedValue(program);
    const actionCommand = {
      name: () => "",
      parent: program,
  ...
  it("uses root raw args and reparses the root for nested lazy commands", async () => {
    const root = new Command().name("marketingclaw");
    setRawArgs(root, ["node", "marketingclaw", "workspaces", "audit", "export", "--since", "1"]);
    ...
    expect(buildParseArgvMock).toHaveBeenCalledWith({
      programName: "marketingclaw",
      rawArgs: ["node", "marketingclaw", "workspaces", "audit", "export", "--since", "1"],
  ...
  it("hoists a trailing lazy-parent option before the loaded command", async () => {
    ...
    await expectReparseArgv({
      parent: browser,
      action: tabs,
      argv: ["node", "marketingclaw", "browser", "tabs", "--browser-profile", "remote"],
      expected: ["node", "marketingclaw", "browser", "--browser-profile", "remote", "tabs"],
    });
  });

  it("skips root option values that match the parent command name", async () => {
    const root = new Command().name("marketingclaw").option("--profile <name>");
    const browser = root.command("browser").option("--browser-profile <name>");
    const tabs = browser.command("tabs");
    await expectReparseArgv({
      parent: browser,
  ...
  it("hoists parent options after nested lazy commands", async () => {
    ...
    await expectReparseArgv({
      parent: browser,
      action: tab,
      argv: ["node", "marketingclaw", "browser", "tab", "new", "--browser-profile", "work"],
      expected: ["node", "marketingclaw", "browser", "--browser-profile", "work", "tab", "new"],
    });
  });
  ...
  it("leaves a child-owned option collision after the child command", async () => {
    ...
    const argv = ["node", "marketingclaw", "browser", "extension", "pair", "--json"];
  ...
  it("hoists a parent option when only a sibling command owns the same flag", async () => {
    ...
    await expectReparseArgv({
      parent: browser,
      action: cookies,
      argv: ["node", "marketingclaw", "browser", "cookies", "list", "--url", "ws://gateway"],
      expected: ["node", "marketingclaw", "browser", "--url", "ws://gateway", "cookies", "list"],
    });
  });
  ...
  it("keeps a missing parent option value after the loaded command", async () => {
    ...
    const browser = root.command("browser").option("--browser-profile <name>");
    const tabs = browser.command("tabs");
    const argv = ["node", "marketingclaw", "browser", "tabs", "--browser-profile"];
