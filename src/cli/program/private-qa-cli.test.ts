...
describe("private-qa-cli", () => {
  ...
  it("loads the private QA CLI from a source checkout path", async () => {
    ...
    const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-private-qa-source-"));
  ...
  it("loads the private QA CLI from a raw synced source checkout path", async () => {
    process.env.MARKETINGCLAW_ENABLE_PRIVATE_QA_CLI = "1";
    const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-private-qa-raw-source-"));
    tempDirs.push(repoRoot);
    const expectedPaths = new Set([
  ...
  it("rejects non-source package roots even when private QA is enabled", () => {
    process.env.MARKETINGCLAW_ENABLE_PRIVATE_QA_CLI = "1";
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-private-qa-"));
    tempDirs.push(root);
    fs.writeFileSync(
    ...
    ).toThrow("Private QA CLI is only available from an MarketingClaw source checkout.");
  ...
  it("rejects when the private QA env flag is disabled", () => {
    ...
    expect(() => loadPrivateQaCliModule({ importModule })).toThrow(
      "Private QA CLI is only available from an MarketingClaw source checkout.",
    );
    expect(importModule).not.toHaveBeenCalled();
  });
});
