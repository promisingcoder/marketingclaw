...
import { repoInstallSpec } from "marketingclaw/plugin-sdk/test-fixtures";
...
beforeEach(() => {
  ...
  originalHideBanner = process.env.MARKETINGCLAW_HIDE_BANNER;
...
describe("registerPreActionHooks", () => {
  ...
  function buildProgram() {
    const programLocal = new Command().name("marketingclaw");
    programLocal
      .command("agent")
  ...
  it("handles debug mode and config-only command preaction", async () => {
    ...
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "marketingclaw", "status", "--debug"],
    });
    ...
    expect(processTitleSetSpy).toHaveBeenCalledWith("marketingclaw-status");

    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["agents", "list"],
      processArgv: ["node", "marketingclaw", "agents", "list"],
    });
  ...
  it("passes the gateway config recheck to the state migration boundary", async () => {
    await runPreAction({
      parseArgv: ["gateway", "run"],
      processArgv: ["node", "marketingclaw", "gateway", "run"],
    });
  ...
  it("loads plugins for text local agent runs", async () => {
    await runPreAction({
      parseArgv: ["agent"],
      processArgv: ["node", "marketingclaw", "agent", "--local", "--message", "hi"],
    });
  ...
  it("loads plugins for json local agent runs", async () => {
    await runPreAction({
      parseArgv: ["agent"],
      processArgv: ["node", "marketingclaw", "agent", "--local", "--message", "hi", "--json"],
    });
  ...
  it("keeps setup alias and channels add manifest-first", async () => {
    await runPreAction({
      parseArgv: ["onboard"],
      processArgv: ["node", "marketingclaw", "onboard"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "marketingclaw", "channels", "add"],
    });
  ...
  it("skips startup bootstrap for parent default help actions", async () => {
    await runPreAction({
      parseArgv: ["channels"],
      processArgv: ["node", "marketingclaw", "channels"],
    });
  ...
  it("lets configure own config validation and plugin loading", async () => {
    await runPreAction({
      parseArgv: ["configure"],
      processArgv: ["node", "marketingclaw", "configure"],
    });
  ...
  it("lets bare config own config validation and plugin loading", async () => {
    await runPreAction({
      parseArgv: ["config"],
      processArgv: ["node", "marketingclaw", "config"],
    });
  ...
  it("lets guided config sections own config validation and plugin loading", async () => {
    await runPreAction({
      parseArgv: ["config"],
      processArgv: ["node", "marketingclaw", "config", "--section", "models"],
    });
  ...
  it("skips the config guard and plugin loading for doctor lint", async () => {
    await runPreAction({
      parseArgv: ["doctor"],
      processArgv: ["node", "marketingclaw", "doctor", "--lint"],
    });
  ...
  it("only allows invalid config for explicit official recovery reinstall requests", async () => {
    await runPreAction({
      parseArgv: ["plugins", "install", "@marketingclaw/discord"],
      processArgv: ["node", "marketingclaw", "plugins", "install", "@marketingclaw/discord"],
    });
    ...
    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["plugins", "install", "@marketingclaw/discord@2026.5.22"],
      processArgv: [
        "node",
        "marketingclaw",
        "plugins",
        "install",
        "@marketingclaw/discord@2026.5.22",
      ],
    });
    ...
    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["plugins", "install", "@marketingclaw/brave-plugin"],
      processArgv: ["node", "marketingclaw", "plugins", "install", "@marketingclaw/brave-plugin"],
    });
    ...
    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["plugins", "install", "@marketingclaw/slack"],
      processArgv: ["node", "marketingclaw", "plugins", "install", "@marketingclaw/slack"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "marketingclaw", "plugins", "install", "alpha"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "marketingclaw", "plugins", "install", DISCORD_REPO_INSTALL_SPEC],
    });
    ...
    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["plugins", "install", "@marketingclaw/discord", "--marketplace", "local/repo"],
      processArgv: [
        "node",
        "marketingclaw",
        "plugins",
  ...
  it("skips help/version preaction and respects banner opt-out", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "marketingclaw", "--version"],
    });
    ...
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "marketingclaw", "status"],
    });
  ...
  it("applies --json stdout suppression only for explicit JSON output commands", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "marketingclaw", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "marketingclaw", "update", "status", "--json"],
    });
  ...
  it("routes logs to stderr in --json mode so stdout stays clean", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "marketingclaw", "channels", "send", "--json"],
    });
  ...
  it("uses the Commander action path for protocol stdout ownership", async () => {
    await runPreAction({
      parseArgv: ["acp"],
      processArgv: ["node", "marketingclaw", "acp", "--token", "-secret"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "marketingclaw", "acp", "--verbose", "client"],
    });
    ...
    await runPreAction({
      parseArgv: ["mcp", "serve"],
      processArgv: ["node", "marketingclaw", "mcp", "serve"],
    });
  ...
  it("does not preload plugins for agents list JSON output", async () => {
    await runPreAction({
      parseArgv: ["agents", "list"],
      processArgv: ["node", "marketingclaw", "agents", "list", "--json"],
    });
  ...
  it("does not preload plugins for remote agent JSON output", async () => {
    await runPreAction({
      parseArgv: ["agent"],
      processArgv: ["node", "marketingclaw", "agent", "--message", "hi", "--json"],
    });
  ...
  it("bypasses config guard for config validate", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "marketingclaw", "config", "validate"],
    });
  ...
  it("bypasses config guard for config validate when root option values are present", async () => {
    await runPreAction({
      parseArgv: ["config", "validate"],
      processArgv: ["node", "marketingclaw", "--profile", "work", "config", "validate"],
    });
  ...
  it("bypasses config guard for config schema", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "marketingclaw", "config", "schema"],
    });
  ...
  it("bypasses config guard for backup create", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "marketingclaw", "backup", "create", "--json"],
    });
