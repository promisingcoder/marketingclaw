...
describe("configureProgramHelp", () => {
  ...
  beforeEach(() => {
    ...
    originalSuppressHelpBanner = process.env.MARKETINGCLAW_SUPPRESS_HELP_BANNER;
    hasEmittedCliBannerMock.mockReturnValue(false);
    resolveCommitHashMock.mockReturnValue("abc1234");
    delete process.env.MARKETINGCLAW_SUPPRESS_HELP_BANNER;
  });
  ...
  afterEach(() => {
    ...
    if (originalSuppressHelpBanner === undefined) {
      ...
      process.env.MARKETINGCLAW_SUPPRESS_HELP_BANNER = originalSuppressHelpBanner;
    }
  });
  ...
  it("adds root help hint and marks commands with subcommands", () => {
    process.argv = ["node", "marketingclaw", "--help"];
    const program = makeProgramWithCommands();
  ...
  it("includes banner and docs/examples in root help output", () => {
    ...
    expect(help).toContain("https://docs.marketingclaw.ai/cli");
  });

  it("suppresses banner formatting when parent default help requests it", () => {
    process.argv = ["node", "marketingclaw", "channels"];
    process.env.MARKETINGCLAW_SUPPRESS_HELP_BANNER = "1";
    const program = makeProgramWithCommands();
    configureProgramHelp(program, testProgramContext);
  ...
  it("prints version and exits immediately when version flags are present", () => {
    process.argv = ["node", "marketingclaw", "--version"];
    expectVersionExit({ expectedVersion: "MarketingClaw 9.9.9-test (abc1234)" });
  });

  it("prints version and exits immediately without commit metadata", () => {
    process.argv = ["node", "marketingclaw", "--version"];
    resolveCommitHashMock.mockReturnValue(null);
    expectVersionExit({ expectedVersion: "MarketingClaw 9.9.9-test" });
  });

  it("does not treat subcommand --version options as root version requests", () => {
    process.argv = ["node", "marketingclaw", "skills", "verify", "discrawl", "--version", "1.0.0"];
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const exitSpy = vi.spyOn(process, "exit").mockImplementation(((code?: number) => {
      throw new Error(`exit:${code ?? ""}`);
    }) as typeof process.exit);
