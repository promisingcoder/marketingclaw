...
describe("program routes", () => {
  ...
  it("passes parsed agents list flags through", async () => {
    await expect(expectRoute(["agents"]).run(["node", "marketingclaw", "agents"])).resolves.toBe(
      true,
    );
  ...
  it("passes parsed channel read-only route flags through", async () => {
    ...
    await expect(
      listRoute.run(["node", "marketingclaw", "channels", "list", "--json"]),
    ).resolves.toBe(true);
  ...
  it("routes plugins list JSON without importing the full plugins CLI", async () => {
    ...
    expect(route.loadPlugins).toBeUndefined();
    expect(route.canRun?.(["node", "marketingclaw", "plugins", "list"])).toBe(false);

    await expect(
      route.run(["node", "marketingclaw", "plugins", "list", "--json", "--enabled", "--verbose"]),
    ).resolves.toBe(true);
  ...
  it("returns false for plugins list JSON route with unsupported arguments", async () => {
    await expectRunFalse(
      ["plugins", "list"],
      ["node", "marketingclaw", "plugins", "list", "--json", "--wat"],
    );
  });
  ...
  it("returns false for gateway status route when option values are missing", async () => {
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "marketingclaw", "gateway", "status", "--url"],
    );
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "marketingclaw", "gateway", "status", "--token"],
    );
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "marketingclaw", "gateway", "status", "--password"],
    );
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "marketingclaw", "gateway", "status", "--timeout"],
    );
  });
  ...
  it("returns false for gateway status route when probe-only flags are present", async () => {
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "marketingclaw", "gateway", "status", "--ssh", "user@host"],
    );
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "marketingclaw", "gateway", "status", "--ssh-identity", "~/.ssh/id_test"],
    );
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "marketingclaw", "gateway", "status", "--ssh-auto"],
    );
  });
  ...
  it("passes --no-probe through to daemon status", async () => {
    const route = expectRoute(["gateway", "status"]);
    await expect(
      route.run(["node", "marketingclaw", "gateway", "status", "--no-probe"]),
    ).resolves.toBe(true);
  ...
  it("returns false when status timeout flag value is missing", async () => {
    await expectRunFalse(["status"], ["node", "marketingclaw", "status", "--timeout"]);
  });
  ...
  it("returns false for sessions route when --store value is missing", async () => {
    await expectRunFalse(["sessions"], ["node", "marketingclaw", "sessions", "--store"]);
  });

  it("returns false for sessions route when --active value is missing", async () => {
    await expectRunFalse(["sessions"], ["node", "marketingclaw", "sessions", "--active"]);
  });

  it("returns false for sessions route when --agent value is missing", async () => {
    await expectRunFalse(["sessions"], ["node", "marketingclaw", "sessions", "--agent"]);
  });
  ...
  it("returns false for config get route when path argument is missing", async () => {
    await expectRunFalse(["config", "get"], ["node", "marketingclaw", "config", "get", "--json"]);
  });

  it("returns false for config unset route when path argument is missing", async () => {
    await expectRunFalse(["config", "unset"], ["node", "marketingclaw", "config", "unset"]);
  });
  ...
  it("returns false for models list route when --provider value is missing", async () => {
    await expectRunFalse(
      ["models", "list"],
      ["node", "marketingclaw", "models", "list", "--provider"],
    );
  });
  ...
  it("returns false for models status route when probe flags are missing values", async () => {
    await expectRunFalse(
      ["models", "status"],
      ["node", "marketingclaw", "models", "status", "--probe-provider"],
    );
    await expectRunFalse(
      ["models", "status"],
      ["node", "marketingclaw", "models", "status", "--probe-timeout"],
    );
    await expectRunFalse(
      ["models", "status"],
      ["node", "marketingclaw", "models", "status", "--probe-concurrency"],
    );
    await expectRunFalse(
      ["models", "status"],
      ["node", "marketingclaw", "models", "status", "--probe-max-tokens"],
    );
    await expectRunFalse(
      ["models", "status"],
      ["node", "marketingclaw", "models", "status", "--probe-provider", "openai", "--agent"],
    );
  });
  ...
  it("returns false for models status route when --probe-profile has no value", async () => {
    await expectRunFalse(
      ["models", "status"],
      ["node", "marketingclaw", "models", "status", "--probe-profile"],
    );
  });
  ...
  it("routes tasks list JSON through the lean task JSON command", async () => {
    ...
    expect(rootRoute.canRun?.(["node", "marketingclaw", "tasks"])).toBe(false);
    await expect(
      rootRoute.run([
        "node",
        "marketingclaw",
    ...
    await expect(
      listRoute.run(["node", "marketingclaw", "tasks", "list", "--json", "--runtime=cron"]),
    ).resolves.toBe(true);
  ...
  it("routes tasks audit JSON through the lean task JSON command", async () => {
    ...
    expect(route.loadPlugins).toBeUndefined();
    expect(route.canRun?.(["node", "marketingclaw", "tasks", "audit"])).toBe(false);
    await expect(
      route.run([
        "node",
        "marketingclaw",
  ...
  it("returns false for task JSON routes when option values are missing or unknown", async () => {
    await expectRunFalse(["tasks"], ["node", "marketingclaw", "tasks", "--json", "--runtime"]);
    await expectRunFalse(["tasks", "list"], ["node", "marketingclaw", "tasks", "list"]);
    await expectRunFalse(
      ["tasks", "audit"],
      ["node", "marketingclaw", "tasks", "audit", "--json", "--limit"],
    );
    await expectRunFalse(
      ["tasks", "audit"],
      ["node", "marketingclaw", "tasks", "audit", "--json", "--limit", "5abc"],
    );
    await expectRunFalse(
      ["tasks", "audit"],
      ["node", "marketingclaw", "tasks", "audit", "--json", "--unknown"],
    );
    expect(
      findRoutedCommand(["tasks", "cli"], ["node", "marketingclaw", "tasks", "--runtime", "cli"]),
    ).toBeNull();
  });
});
