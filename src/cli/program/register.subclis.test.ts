...
describe("registerSubCliCommands", () => {
  ...
  const originalDisableLazySubcommands = process.env.MARKETINGCLAW_DISABLE_LAZY_SUBCOMMANDS;
  const originalEnablePrivateQaCli = process.env.MARKETINGCLAW_ENABLE_PRIVATE_QA_CLI;
  ...
  beforeEach(() => {
    if (originalDisableLazySubcommands === undefined) {
      delete process.env.MARKETINGCLAW_DISABLE_LAZY_SUBCOMMANDS;
    } else {
      process.env.MARKETINGCLAW_DISABLE_LAZY_SUBCOMMANDS = originalDisableLazySubcommands;
    }
  ...
  afterEach(() => {
    ...
    if (originalEnablePrivateQaCli === undefined) {
      ...
      process.env.MARKETINGCLAW_ENABLE_PRIVATE_QA_CLI = originalEnablePrivateQaCli;
    }
  });

  it("registers the primary placeholder plus completion and dispatches", async () => {
    const program = createRegisteredProgram(["node", "marketingclaw", "acp"]);
  ...
  it("registers placeholders for all subcommands when no primary", () => {
    const program = createRegisteredProgram(["node", "marketingclaw"]);
  ...
  it("registers the infer placeholder and dispatches through the capability registrar", async () => {
    const program = createRegisteredProgram(["node", "marketingclaw", "infer"], "marketingclaw");
  ...
  it("registers the exec-approvals placeholder and dispatches through the approvals registrar", async () => {
    const program = createRegisteredProgram(
      ["node", "marketingclaw", "exec-approvals"],
      "marketingclaw",
    );
  ...
  it("registers only the gateway run surface for gateway startup", async () => {
    const argv = ["node", "marketingclaw", "gateway", "--force"];
    process.argv = argv;
  ...
  it("keeps the full gateway CLI for non-run gateway subcommands", async () => {
    const argv = ["node", "marketingclaw", "gateway", "call", "health"];
    process.argv = argv;
    const program = new Command().name("marketingclaw");
  ...
  it("passes completion context to channel registration", async () => {
    const argv = ["node", "marketingclaw", "completion", "--write-state"];
    const program = new Command().name("marketingclaw");
  ...
  it.each([
    ...
    process.argv = ["node", "marketingclaw", ...args];
    const program = new Command().name("marketingclaw");
  ...
  it("does not preload plugin CLI registrations for bare plugin parent help", async () => {
    process.argv = ["node", "marketingclaw", "plugins"];
    const program = new Command().name("marketingclaw");
