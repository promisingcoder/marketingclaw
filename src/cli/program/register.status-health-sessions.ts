...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["marketingclaw status", "Show channel health + session summary."],
          ["marketingclaw status --all", "Full diagnosis (read-only)."],
          ["marketingclaw status --json", "Machine-readable output."],
          ["marketingclaw status --usage", "Show model provider usage/quota snapshots."],
          [
            "marketingclaw status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["marketingclaw status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.marketingclaw.ai/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.marketingclaw.ai/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
  ...
  const sessionsCmd = addSessionsListOptions(
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["marketingclaw sessions", "List all sessions."],
          ["marketingclaw sessions --agent work", "List sessions for one agent."],
          ["marketingclaw sessions --all-agents", "Aggregate sessions across agents."],
          ["marketingclaw sessions --active 120", "Only last 2 hours."],
          ["marketingclaw sessions --limit 25", "Show the newest 25 sessions."],
          ["marketingclaw sessions --json", "Machine-readable output."],
          ["marketingclaw sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.marketingclaw.ai/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["marketingclaw sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "marketingclaw sessions cleanup --dry-run --fix-missing",
          ...
          ["marketingclaw sessions cleanup --enforce", "Apply maintenance now."],
          ["marketingclaw sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["marketingclaw sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "marketingclaw sessions cleanup --enforce --store ./tmp/sessions.json",
  ...
  sessionsCmd
    ...
    .option("--output <path>", "Output directory name inside .marketingclaw/trajectory-exports")
  ...
  sessionsCmd
    ...
    .action(async (key: string, opts, command) => {
      ...
      // command, e.g. `marketingclaw sessions --agent work compact <key>`. Merge those
  ...
  const commitmentsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["marketingclaw commitments", "List pending inferred follow-ups."],
          ["marketingclaw commitments --all", "List all inferred follow-ups."],
          ["marketingclaw commitments --agent work", "List one agent's inferred follow-ups."],
          ["marketingclaw commitments dismiss cm_abc123", "Dismiss a follow-up."],
        ])}`,
    )
    .action(async (opts) => {
      await runCommandWithRuntime(defaultRuntime, async () => {
