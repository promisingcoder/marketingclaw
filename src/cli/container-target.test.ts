...
describe("parseCliContainerArgs", () => {
  it("extracts a root --container flag before the command", () => {
    expect(
      parseCliContainerArgs(["node", "marketingclaw", "--container", "demo", "status", "--deep"]),
    ).toEqual({
      ok: true,
      container: "demo",
      argv: ["node", "marketingclaw", "status", "--deep"],
    });
  });

  it("accepts the equals form", () => {
    expect(parseCliContainerArgs(["node", "marketingclaw", "--container=demo", "health"])).toEqual({
      ok: true,
      container: "demo",
      argv: ["node", "marketingclaw", "health"],
    });
  });

  it("rejects a missing container value", () => {
    expect(parseCliContainerArgs(["node", "marketingclaw", "--container"])).toEqual({
      ok: false,
  ...
  it("does not consume an adjacent flag as the container value", () => {
    expect(
      parseCliContainerArgs(["node", "marketingclaw", "--container", "--no-color", "status"]),
    ).toEqual({
      ok: false,
      error: "--container requires a value",
    });
  ...
  it("leaves argv unchanged when the flag is absent", () => {
    expect(parseCliContainerArgs(["node", "marketingclaw", "status"])).toEqual({
      ok: true,
      container: null,
  ...
  it("extracts --container after the command like other root options", () => {
    expect(
      parseCliContainerArgs(["node", "marketingclaw", "status", "--container", "demo", "--deep"]),
    ).toEqual({
      ok: true,
      container: "demo",
      argv: ["node", "marketingclaw", "status", "--deep"],
...
describe("resolveCliContainerTarget", () => {
  it("uses argv first and falls back to MARKETINGCLAW_CONTAINER", () => {
    expect(
      resolveCliContainerTarget(["node", "marketingclaw", "--container", "demo", "status"], {}),
    ).toBe("demo");
    expect(resolveCliContainerTarget(["node", "marketingclaw", "status"], {})).toBeNull();
    expect(
      resolveCliContainerTarget(["node", "marketingclaw", "status"], {
        MARKETINGCLAW_CONTAINER: "demo",
...
describe("maybeRunCliInContainer", () => {
  it("passes through when no container target is provided", () => {
    expect(maybeRunCliInContainer(["node", "marketingclaw", "status"], { env: {} })).toEqual({
      handled: false,
      argv: ["node", "marketingclaw", "status"],
    });
  });

  it("uses MARKETINGCLAW_CONTAINER when the flag is absent", () => {
    const spawnSync = vi
      .fn()
      .mockReturnValueOnce({
        status: 0,
    ...
    expect(
      maybeRunCliInContainer(["node", "marketingclaw", "status"], {
        env: { MARKETINGCLAW_CONTAINER: "demo" } as NodeJS.ProcessEnv,
        spawnSync,
      }),
    ).toEqual({
      handled: true,
  ...
  it("clears inherited host routing and gateway env before execing into the child CLI", () => {
    ...
    maybeRunCliInContainer(["node", "marketingclaw", "status"], {
      env: {
        ...
        MARKETINGCLAW_GATEWAY_PASSWORD: "password",
      } as NodeJS.ProcessEnv,
      spawnSync,
    });
  ...
  it("passes the proxy URL env fallback into the child container CLI", () => {
    ...
    maybeRunCliInContainer(["node", "marketingclaw", "status"], {
      env: {
        MARKETINGCLAW_CONTAINER: "demo",
        MARKETINGCLAW_PROXY_URL: " http://proxy.internal:3128 ",
      } as NodeJS.ProcessEnv,
      spawnSync,
    });
  ...
  it.each([
    ...
    expect(() =>
      maybeRunCliInContainer(["node", "marketingclaw", "status"], {
        env: {
          MARKETINGCLAW_CONTAINER: "demo",
          MARKETINGCLAW_PROXY_URL: ` ${proxyUrl} `,
        } as NodeJS.ProcessEnv,
        spawnSync,
      }),
  ...
  it("redacts proxy URL credentials and URL suffixes before rejecting loopback container proxy forwarding", () => {
    ...
    expect(message).toContain("MARKETINGCLAW_PROXY_URL=http://redacted:redacted@127.0.0.1:3128/");
  ...
  it("allows explicitly overridden loopback proxy URL forwarding into a child container CLI", () => {
    ...
    maybeRunCliInContainer(["node", "marketingclaw", "status"], {
      env: {
        ...
        MARKETINGCLAW_CONTAINER_ALLOW_LOOPBACK_PROXY_URL: "1",
      } as NodeJS.ProcessEnv,
      spawnSync,
    });
    ...
    expect(podmanCall[1]).toContain("MARKETINGCLAW_PROXY_URL=http://127.0.0.1:3128");
  ...
  it("executes through podman when the named container is running", () => {
    ...
    expect(
      maybeRunCliInContainer(["node", "marketingclaw", "--container", "demo", "status"], {
        env: {},
        spawnSync,
      }),
    ).toEqual({
  ...
  it("falls back to docker when podman does not have the container", () => {
    ...
    expect(
      maybeRunCliInContainer(["node", "marketingclaw", "--container", "demo", "health"], {
        env: { USER: "marketingclaw" } as NodeJS.ProcessEnv,
        spawnSync,
      }),
    ).toEqual({
      handled: true,
    ...
    expect(spawnSync).toHaveBeenNthCalledWith(
      ...
      {
        stdio: "inherit",
        env: { USER: "marketingclaw", MARKETINGCLAW_CONTAINER: "" },
      },
    );
  });
  ...
  it("checks docker after podman and before failing", () => {
    ...
    expect(spawnSync).toHaveBeenNthCalledWith(
      ...
      {
        stdio: "inherit",
        env: { USER: "somalley", MARKETINGCLAW_CONTAINER: "" },
      },
    );
  ...
  it("allocates a tty for interactive terminal sessions", () => {
    ...
    maybeRunCliInContainer(["node", "marketingclaw", "--container", "demo", "setup"], {
      env: {},
      spawnSync,
      stdinIsTTY: true,
      stdoutIsTTY: true,
  ...
  it("prefers --container over MARKETINGCLAW_CONTAINER", () => {
    const spawnSync = vi
      .fn()
      .mockReturnValueOnce({
        status: 0,
    ...
    expect(
      maybeRunCliInContainer(["node", "marketingclaw", "--container", "flag-demo", "health"], {
        env: { MARKETINGCLAW_CONTAINER: "env-demo" } as NodeJS.ProcessEnv,
        spawnSync,
      }),
    ).toEqual({
      handled: true,
  ...
  it("skips recursion when the bypass env is set", () => {
    expect(
      maybeRunCliInContainer(["node", "marketingclaw", "--container", "demo", "status"], {
        env: { MARKETINGCLAW_CLI_CONTAINER_BYPASS: "1" } as NodeJS.ProcessEnv,
      }),
    ).toEqual({
      handled: false,
      argv: ["node", "marketingclaw", "--container", "demo", "status"],
    });
  });
  ...
  it("blocks updater commands from running inside the container", () => {
    ...
    expect(() =>
      maybeRunCliInContainer(["node", "marketingclaw", "--container", "demo", "update"], {
        env: {},
        spawnSync,
      }),
    ).toThrow(
  ...
  it("blocks update after interleaved root flags", () => {
    ...
    expect(() =>
      maybeRunCliInContainer(
        ["node", "marketingclaw", "--container", "demo", "--no-color", "update"],
        {
          env: {},
          spawnSync,
        },
