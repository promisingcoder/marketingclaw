...
function readDevicePairPublicUrlFromConfig(cfg: MarketingClawConfig): string | undefined {
...
function shouldResolveLocalGatewayPasswordSecret(
  ...
  if (trimToUndefined(env.MARKETINGCLAW_GATEWAY_PASSWORD)) {
    return false;
  }
...
async function resolveLocalGatewayPasswordSecretIfNeeded(cfg: MarketingClawConfig): Promise<void> {
...
export function registerQrCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/qr", "docs.marketingclaw.ai/cli/qr")}\n`,
    )
    .option(
      "--remote",
    ...
    .action(async (opts: QrCliOptions) => {
      try {
        ...
        const lines: string[] = [
          ...
          "Scan this with the MarketingClaw mobile app (Onboarding -> Scan QR).",
          "",
        ];
        ...
        lines.push(
          ...
          `  ${theme.command("marketingclaw devices list")}`,
          `  ${theme.command("marketingclaw devices approve <requestId>")}`,
        );
