...
describe("capability cli", () => {
  ...
  it("passes image files to local model probes", async () => {
    const tempInput = path.join(os.tmpdir(), `marketingclaw-model-run-image-${Date.now()}.png`);
  ...
  it("passes image files to gateway model probes as attachments", async () => {
    const tempInput = path.join(
      os.tmpdir(),
      `marketingclaw-model-run-gateway-image-${Date.now()}.png`,
    );
    await fs.writeFile(tempInput, Buffer.from(PNG_1X1_BASE64, "base64"));
  ...
  it("normalizes HEIC files to JPEG before local model probes", async () => {
    const tempInput = path.join(os.tmpdir(), `marketingclaw-model-run-image-${Date.now()}.heic`);
  ...
  it("rejects non-image files for model probes", async () => {
    const tempInput = path.join(os.tmpdir(), `marketingclaw-model-run-audio-${Date.now()}.mp3`);
  ...
  it("rewrites mismatched explicit image output extensions to the detected file type", async () => {
    ...
    const tempOutput = path.join(os.tmpdir(), `marketingclaw-image-mismatch-${Date.now()}.png`);
  ...
  it("passes image output format, quality, and OpenAI hints through to edit runtime", async () => {
    ...
    const inputPath = path.join(os.tmpdir(), `marketingclaw-image-edit-${Date.now()}.png`);
  ...
  it("forwards --count through to the image edit runtime", async () => {
    ...
    const inputPath = path.join(os.tmpdir(), `marketingclaw-image-edit-count-${Date.now()}.png`);
    await fs.writeFile(inputPath, Buffer.from("png-input"));
  ...
  it("forwards size, aspect ratio, and resolution overrides for image edit", async () => {
    ...
    const tempInput = path.join(os.tmpdir(), `marketingclaw-image-edit-input-${Date.now()}.png`);
    const tempOutput = path.join(os.tmpdir(), `marketingclaw-image-edit-output-${Date.now()}.png`);
  ...
  it("streams url-only generated videos to --output paths", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-video-generate-"));
