...
export function applyCliProfileEnv(params: {
  ...
  env.MARKETINGCLAW_PROFILE = profile;

  const existingStateDir = normalizeOptionalString(env.MARKETINGCLAW_STATE_DIR);
  const stateDir = existingStateDir || resolveProfileStateDir(profile, env, homedir);
  if (!existingStateDir) {
    env.MARKETINGCLAW_STATE_DIR = stateDir;
  }

  if (!normalizeOptionalString(env.MARKETINGCLAW_CONFIG_PATH)) {
    env.MARKETINGCLAW_CONFIG_PATH = path.join(stateDir, "marketingclaw.json");
  }

  if (profile === "dev" && !env.MARKETINGCLAW_GATEWAY_PORT?.trim()) {
    env.MARKETINGCLAW_GATEWAY_PORT = "19001";
  }
}
