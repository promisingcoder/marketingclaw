...
export function formatUnknownChannelMessage(params: {
  ...
  const listCommand = params.listCommand ?? "marketingclaw channels list --all";
...
export function formatUnsupportedChannelActionMessage(params: {
  ...
  const inspectCommand =
    params.inspectCommand ?? `marketingclaw channels capabilities --channel ${params.channel}`;
...
export function formatStrictJsonParseFailure(params: { value: string; cause: unknown }): string {
  ...
  return [
    ...
    `Use valid JSON, for example ${formatInlineCliCommand(
      "marketingclaw config set gateway.port 18789 --strict-json",
    )}.`,
...
export function formatGatewayCommandFailure(params: {
  ...
  const message = raw
    .replace(/\s*Run [`"]?marketingclaw doctor[`"]? for diagnostics\.?/gi, "")
    .replace(/\s+Gateway target:\s+.*$/isu, "")
    .replace(/\s+/g, " ")
    .trim()
    .replace(/[.。]+$/u, "");
  const inspectCommand = params.inspectCommand ?? "marketingclaw gateway status --deep";
...
export function formatMissingPluginMessage(params: {
  ...
  const listCommand = params.listCommand ?? "marketingclaw plugins list";
  const searchHint = params.includeSearch
    ? `, or ${formatInlineCliCommand("marketingclaw plugins search " + params.id)} to look for installable plugins`
    : "";
