...
describe("persistPluginInstall", () => {
  ...
  it("adds installed plugins to restrictive allowlists before enabling", async () => {
    ...
    enablePluginInConfig.mockImplementation((...args: unknown[]) => {
      const [cfg, pluginId] = args as [MarketingClawConfig, string];
  ...
  it("removes a replaced managed install directory before refreshing the registry", async () => {
    ...
    setInstalledPluginIndexInstallRecords({
      codex: {
        source: "clawhub",
        spec: "clawhub:@marketingclaw/codex",
        installPath: "/tmp/marketingclaw/extensions/codex",
      },
    });
    planPluginUninstall.mockReturnValueOnce({
      ...
      directoryRemoval: {
        target: "/tmp/marketingclaw/extensions/codex",
      },
    });
    ...
    await persistPluginInstall({
      ...
      install: {
        ...
        installPath: "/tmp/marketingclaw/npm/node_modules/@marketingclaw/codex",
      },
    });
    ...
    expect(runtimeLogs.join("\n")).toContain(
      "Removed previous plugin install directory: /tmp/marketingclaw/extensions/codex",
    );
  });
  ...
  it("preserves replaced install directories when the new install path overlaps", async () => {
    ...
    await persistPluginInstall({
      ...
      install: {
        source: "npm",
        spec: "@marketingclaw/codex@latest",
        installPath: "/tmp/marketingclaw/npm/node_modules/@marketingclaw/codex",
      },
    });
  ...
  it("preserves replaced npm install directories across generation updates", async () => {
    ...
    enablePluginInConfig.mockReturnValue({ config: enabledConfig });
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-plugin-persist-"));
  ...
  it("warns when an installed npm plugin remains shadowed by a config-selected source", async () => {
    ...
    const next = await persistPluginInstall({
      ...
      install: {
        ...
        installPath: "/tmp/marketingclaw/npm/node_modules/@marketingclaw/discord/index.ts",
      },
    });
    ...
    expect(runtimeLogs.join("\n")).toContain(
      "active config source: /tmp/marketingclaw-upstream/extensions/discord/index.ts",
    );
    expect(runtimeLogs.join("\n")).toContain(
      "installed npm source: /tmp/marketingclaw/npm/node_modules/@marketingclaw/discord/index.ts",
    );
    expect(runtimeLogs.join("\n")).toContain("marketingclaw plugins doctor");
  });
  ...
  it("does not warn when the config-selected source is inside the npm install path", async () => {
      ...
      plugins: [
        {
          id: "discord",
          origin: "config",
          source: "/tmp/marketingclaw/npm/node_modules/@marketingclaw/discord/dist/index.js",
          status: "loaded",
        },
      ],
      diagnostics: [],
    ...
    await persistPluginInstall({
      ...
      install: {
        source: "npm",
        spec: "@marketingclaw/discord",
        installPath: "/tmp/marketingclaw/npm/node_modules/@marketingclaw/discord",
      },
    });
