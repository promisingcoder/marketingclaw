...
describe("json output mode", () => {
  ...
  it("detects json output flags before argv terminators", () => {
    expect(hasJsonOutputFlag(["node", "marketingclaw", "nodes", "list", "--json"])).toBe(true);
    expect(hasJsonOutputFlag(["node", "marketingclaw", "nodes", "list", "--json=true"])).toBe(true);
    expect(hasJsonOutputFlag(["node", "marketingclaw", "nodes", "--", "--json"])).toBe(false);
  });
  ...
  it("temporarily routes console logs to stderr while json output is being prepared", async () => {
    ...
    await withConsoleLogsRoutedToStderrForJson(
      ["node", "marketingclaw", "nodes", "list", "--json"],
      async () => {
