...
describe("cli program (nodes media)", () => {
  ...
  it("runs nodes camera snap and prints two MEDIA paths", async () => {
    ...
    expect(mediaPaths[0]).toContain("marketingclaw-camera-snap-");
    expect(mediaPaths[1]).toContain("marketingclaw-camera-snap-");
  ...
  it("runs nodes camera clip and prints one MEDIA path", async () => {
    ...
    await expectLoggedSingleMediaFile({
      expectedPathPattern: /marketingclaw-camera-clip-front-.*\.mp4$/,
    });
  });
    ...
    it.each([
      {
        ...
        expectedPathPattern: /marketingclaw-camera-snap-front-.*\.jpg$/,
      },
      {
