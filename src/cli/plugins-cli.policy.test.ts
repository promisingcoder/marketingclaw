...
describe("plugins cli policy mutations", () => {
  ...
  function requireFirstWrittenConfig(): MarketingClawConfig {
  ...
  ): NonNullable<NonNullable<MarketingClawConfig["plugins"]>["entries"]> {
  ...
  it("refreshes the persisted plugin registry after enabling a plugin", async () => {
    const sourceConfig = {} as MarketingClawConfig;
    const enabledConfig = {
      plugins: {
        entries: {
          alpha: { enabled: true },
  ...
  it.each(["enable", "disable"] as const)(
    ...
    async (command) => {
      ...
      expect(runtimeErrors).toContain(
        "Plugin not found: missing-plugin. Run `marketingclaw plugins list` to see installed plugins, or `marketingclaw plugins search missing-plugin` to look for installable plugins.",
      );
      expect(enablePluginInConfig).not.toHaveBeenCalled();
      expect(writeConfigFile).not.toHaveBeenCalled();
