...
export function collectDeepProbeFindings(params: {
  ...
  if (params.deep?.gateway?.attempted && !params.deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("marketingclaw status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("marketingclaw security audit --deep")}".`,
    });
  }
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set MARKETINGCLAW_GATEWAY_TOKEN/MARKETINGCLAW_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("marketingclaw security audit --deep")}".`,
    });
  }
  return findings;
}
