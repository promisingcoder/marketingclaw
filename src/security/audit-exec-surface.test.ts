...
describe("security audit exec surface findings", () => {
  // Redirect the MarketingClaw home (MARKETINGCLAW_HOME wins over HOME/USERPROFILE in
  // `resolveRawHomeDir`) to a per-test tempdir so `saveExecApprovals` never
  // touches the real `~/.marketingclaw/exec-approvals.json` on the host running
  // the suite.
  ...
  beforeAll(async () => {
    tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-exec-approvals-"));
  });

  beforeEach(async () => {
    envSnapshot = captureEnv(["MARKETINGCLAW_HOME", "HOME", "USERPROFILE"]);
    const tempDir = path.join(tempRoot, `case-${++tempCaseIndex}`);
    await fs.mkdir(path.join(tempDir, ".marketingclaw"), { recursive: true });
    // MARKETINGCLAW_HOME takes precedence over HOME/USERPROFILE in resolveRawHomeDir,
    // so all three must point at the tempdir to neutralize whichever the host
    // happens to have set.
    process.env.MARKETINGCLAW_HOME = tempDir;
  ...
  it("warns when YOLO exec overrides restrictive Claude permission mode", () => {
    ...
    expect(finding?.detail).toContain("MarketingClaw exec is YOLO");
  });
  ...
  it("does not warn for restrictive Claude permission mode when MarketingClaw exec is restrictive", () => {
    const findings = collectExecRuntimeFindings({
