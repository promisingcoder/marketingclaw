...
describe("security audit hooks ingress findings", () => {
  it("evaluates hooks ingress auth and routing findings", () => {
    ...
    } satisfies NonNullable<MarketingClawConfig["hooks"]>;
    ...
    const cases = [
      ...
      },
      {
        ...
        cfg: { hooks: unrestrictedBaseHooks } satisfies MarketingClawConfig,
        expectedFinding: "hooks.allowed_agent_ids_unrestricted",
        expectedSeverity: "warn" as const,
      },
      {
      ...
      },
      {
        ...
        cfg: { hooks: requestSessionKeyHooks } satisfies MarketingClawConfig,
  ...
  it("flags hooks token reuse of gateway password auth as critical", () => {
    ...
    expect(finding?.remediation).toContain("marketingclaw doctor --fix");
  });
