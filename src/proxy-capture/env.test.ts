...
describe("resolveDebugProxySettings", () => {
  ...
  it("prefers an explicit session id from the environment", () => {
    const settings = resolveDebugProxySettings({
      [MARKETINGCLAW_DEBUG_PROXY_ENABLED]: "1",
      [MARKETINGCLAW_DEBUG_PROXY_SESSION_ID]: "session-explicit",
    });
  ...
  it("retains deprecated capture storage settings for Plugin SDK compatibility", () => {
    const settings = resolveDebugProxySettings({
      MARKETINGCLAW_DEBUG_PROXY_DB_PATH: "/tmp/legacy-capture.sqlite",
      MARKETINGCLAW_DEBUG_PROXY_BLOB_DIR: "/tmp/legacy-capture-blobs",
    });
  ...
  it("does not pass obsolete capture storage overrides to child processes", () => {
    ...
    expect(env.MARKETINGCLAW_DEBUG_PROXY_DB_PATH).toBeUndefined();
    expect(env.MARKETINGCLAW_DEBUG_PROXY_BLOB_DIR).toBeUndefined();
  });
});
