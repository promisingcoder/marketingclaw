...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[marketingclaw proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[marketingclaw proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`marketingclaw proxy coverage\` for details.`,
  );
}
