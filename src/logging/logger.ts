...
function resolveDefaultLogDir(): string {
  return canUseNodeFs() ? resolvePreferredMarketingClawTmpDir() : POSIX_MARKETINGCLAW_TMP_DIR;
}

function resolveDefaultLogFile(defaultLogDir: string): string {
  return canUseNodeFs()
    ? path.join(defaultLogDir, "marketingclaw.log")
    : `${POSIX_MARKETINGCLAW_TMP_DIR}/marketingclaw.log`;
}
...
type LoggerConfigLoader = () => MarketingClawConfig["logging"] | undefined;
...
function canUseSilentVitestFileLogFastPath(envLevel: LogLevel | undefined): boolean {
  return (
    ...
    process.env.MARKETINGCLAW_TEST_FILE_LOG !== "1" &&
    !envLevel &&
...
function resolveDefaultActiveLogFile(): string {
  if (process.env.VITEST === "true" && process.env.MARKETINGCLAW_TEST_FILE_LOG === "1") {
    return path.join(
      process.cwd(),
      ".artifacts",
      "test-logs",
...
function resolveSettings(): ResolvedSettings {
  ...
  const cfg: MarketingClawConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? loadLoggerConfig();
  const defaultLevel =
    process.env.VITEST === "true" && process.env.MARKETINGCLAW_TEST_FILE_LOG !== "1"
      ? "silent"
      : "info";
      ...
      if (currentFileBytes > 0 && nextBytes > settings.maxFileBytes) {
        if (rotateLogFile(activeFile)) {
          ...
          process.stderr.write(
            `[marketingclaw] log file rotation failed; continuing writes file=${activeFile} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
      }
