...
const SUPPORT_EXPORT_PREFIX = "marketingclaw-diagnostics-";
...
function resolveBonjourEnvOverride(
  ...
  const raw = env.MARKETINGCLAW_DISABLE_BONJOUR?.trim().toLowerCase();
  if (!raw) {
    return "unset";
  }
  switch (raw) {
...
function readStabilityBundle(
  ...
  if (target === false) {
    return { status: "missing", dir: "$MARKETINGCLAW_STATE_DIR/logs/stability" };
  }
...
function renderSummary(params: {
  ...
  return [
    "# MarketingClaw Diagnostics Export",
    "",
    ...
    "- `logs/marketingclaw-sanitized.jsonl`: sanitized log summaries and metadata",
