...
function setupHardlinkHookWorkspace(hookName: string): {
  ...
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-hooks-workspace-hardlink-"));
...
describe("hooks workspace", () => {
  it("ignores package.json hook paths that traverse outside package directory", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-hooks-workspace-"));
    const hooksRoot = path.join(root, "hooks");
    fs.mkdirSync(hooksRoot, { recursive: true });
    ...
    const entries = loadHookEntriesFromDir({ dir: hooksRoot, source: "marketingclaw-workspace" });
    expect(hookNames(entries)).not.toContain("outside");
  });

  it("accepts package.json hook paths within package directory", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-hooks-workspace-ok-"));
    const hooksRoot = path.join(root, "hooks");
    fs.mkdirSync(hooksRoot, { recursive: true });
  ...
  it("ignores package.json hook paths that escape via symlink", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-hooks-workspace-link-"));
    const hooksRoot = path.join(root, "hooks");
    fs.mkdirSync(hooksRoot, { recursive: true });
  ...
  it("does not let workspace hooks override managed hooks with the same name", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-hooks-collision-"));
    ...
    expect(entries[0]?.hook.source).toBe("marketingclaw-managed");
  });

  it("treats configured extraDirs as managed hook sources", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-hooks-extra-"));
    const workspaceDir = path.join(root, "workspace");
