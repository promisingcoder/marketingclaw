...
This directory contains hooks that ship with MarketingClaw. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.marketingclaw/workspace`)
...
```bash
marketingclaw hooks enable bootstrap-extra-files
```
...
**Output**: `~/.marketingclaw/logs/commands.log`

**Enable**:

```bash
marketingclaw hooks enable command-logger
```
...
```bash
marketingclaw hooks enable boot-md
```
...
homepage: https://docs.marketingclaw.ai/automation/hooks#my-hook
metadata:
  {
...
- **Managed hooks**: `~/.marketingclaw/hooks/` (shared across workspaces)
...
```bash
marketingclaw hooks enable session-memory
marketingclaw hooks disable command-logger
```

## Configuration

Hooks can be configured in `~/.marketingclaw/marketingclaw.json`:
...
2. Restart gateway: `pkill -9 -f 'marketingclaw.*gateway' && pnpm marketingclaw gateway`
3. Enable the hook: `marketingclaw hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.marketingclaw.ai/automation/hooks
