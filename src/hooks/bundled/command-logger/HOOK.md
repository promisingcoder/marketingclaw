...
homepage: https://docs.marketingclaw.ai/automation/hooks#command-logger
metadata:
  {
    "marketingclaw":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.marketingclaw/logs/commands.log`
...
`~/.marketingclaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.marketingclaw/logs/commands.log ~/.marketingclaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/marketingclaw`:
   ```
   /home/username/.marketingclaw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
...
```bash
tail -n 20 ~/.marketingclaw/logs/commands.log
```

Pretty-print with jq:

```bash
cat ~/.marketingclaw/logs/commands.log | jq .
```

Filter by action:

```bash
grep '"action":"new"' ~/.marketingclaw/logs/commands.log | jq .
```
