...
describe("resolveGroupToolPolicy group context validation", () => {
  ...
  it("keeps specific session group policy ahead of trusted parent caller groupId", () => {
    const scopedCfg: MarketingClawConfig = {
      channels: {
        whatsapp: {
          groups: {
            room: {
...
describe("resolveSubagentToolPolicyForSession", () => {
  ...
  it("uses stored leaf role for flat depth-1 session keys", () => {
    const storePath = path.join(
      os.tmpdir(),
      `marketingclaw-subagent-policy-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
    );
  ...
  it("resolves inherited tool denies from stored subagent sessions", () => {
    const storePath = path.join(
      os.tmpdir(),
      `marketingclaw-subagent-inherited-deny-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
    );
    fs.mkdirSync(path.dirname(storePath), { recursive: true });
    fs.writeFileSync(
      storePath,
  ...
  it("resolves inherited tool allows from stored subagent sessions", () => {
    const storePath = path.join(
      os.tmpdir(),
      `marketingclaw-subagent-inherited-allow-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
    );
    fs.mkdirSync(path.dirname(storePath), { recursive: true });
    fs.writeFileSync(
      storePath,
  ...
  it("keeps configured plugin allows separate from inherited tool allows", () => {
    const storePath = path.join(
      os.tmpdir(),
      `marketingclaw-subagent-inherited-allow-separate-${Date.now()}-${Math.random()
        .toString(16)
        .slice(2)}.json`,
    );
    fs.mkdirSync(path.dirname(storePath), { recursive: true });
  ...
  it("applies inherited tool policy from stored ACP sessions without subagent metadata", () => {
    const storePath = path.join(
      os.tmpdir(),
      `marketingclaw-acp-inherited-deny-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
    );
    fs.mkdirSync(path.dirname(storePath), { recursive: true });
    fs.writeFileSync(
      storePath,
...
describe("resolveEffectiveToolPolicy", () => {
  ...
  it("does not warn an agent profile about inherited global tool sections (#47487)", async () => {
    const warnLogs = createWarnLogCapture("marketingclaw-agent-tools-policy-test");
    try {
      const cfg = {
        tools: {
          exec: { security: "allowlist" },
