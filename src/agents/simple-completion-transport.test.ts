import { registerApiProvider, unregisterApiProviders } from "@marketingclaw/ai/internal/runtime";
...
const createMarketingClawTransportStreamFnForModel = vi.fn();
...
describe("prepareModelForSimpleCompletion", () => {
  ...
  beforeEach(() => {
    ...
    createMarketingClawTransportStreamFnForModel.mockReset();
    ...
    createMarketingClawTransportStreamFnForModel.mockReturnValue(undefined);
  ...
  it("routes provider-owned simple-completion wrappers through an internal API alias", () => {
    ...
    expect(result.api).toBe(
      "marketingclaw-provider-simple:moonshot:kimi-k2.7-code:moonshot-simple-source:https%3A%2F%2Fapi.moonshot.ai%2Fv1",
    );
  ...
  it("uses a custom api alias for Anthropic Vertex simple completions", () => {
    ...
    expect(ensureCustomApiRegistered).toHaveBeenCalledWith(
      "marketingclaw-anthropic-vertex-simple:https%3A%2F%2Fus-central1-aiplatform.googleapis.com",
      "vertex-stream",
    );
    expect(result).toEqual({
      ...model,
      api: "marketingclaw-anthropic-vertex-simple:https%3A%2F%2Fus-central1-aiplatform.googleapis.com",
    });
  });
  ...
  it("uses the Google simple-completion sanitizer alias after transport checks pass through", () => {
    ...
    prepareGoogleSimpleCompletionModel.mockImplementationOnce((m: unknown) => ({
      ...
      api: "marketingclaw-google-generative-ai-simple",
    }));
    resolveProviderStreamFn.mockReturnValueOnce(undefined);
  ...
  it.each([
    ...
    "uses MarketingClaw transport for OpenAI Codex-response simple completions with baseUrl %s",
    (baseUrl, expectedBaseUrl) => {
      ...
      resolveProviderStreamFn.mockReturnValueOnce(undefined);
      createMarketingClawTransportStreamFnForModel.mockReturnValueOnce("codex-transport-stream");
      ...
      expect(createMarketingClawTransportStreamFnForModel).toHaveBeenCalledWith(
        {
          ...model,
          baseUrl: expectedBaseUrl,
        },
