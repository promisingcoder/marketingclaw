...
describe("isSilentOverflowProneModel", () => {
  ...
  // marketingclaw#75799 reporter's setup: an OpenAI-compatible in-house gateway
  ...
  // string. False positives only disable MarketingClaw runtime's secondary compaction path;
  // MarketingClaw's preemptive compaction continues to handle real overflow.
...
describe("applyAgentAutoCompactionGuard", () => {
  // Direct repro of marketingclaw#75799: shared model runtime's silent-overflow detection misfires
  // on a successful turn against z.ai-style providers, triggering MarketingClaw runtime's
  ...
  // state.messages intact; MarketingClaw's preemptive compaction continues to
  ...
  // Default-mode runs against ordinary providers must keep MarketingClaw runtime's auto-compaction
  // enabled. Disabling it across the board would silently remove MarketingClaw runtime's
