 ...
 */
import { parseStrictInteger } from "@marketingclaw/normalization-core/number-coercion";
...
export function buildDockerExecArgs(params: {
  ...
  for (const [key, value] of Object.entries(params.env)) {
    ...
    // PATH is handled separately via MARKETINGCLAW_PREPEND_PATH below.
    if (key === "PATH") {
      continue;
    }
  ...
  if (hasCustomPath) {
    ...
    args.push("-e", `MARKETINGCLAW_PREPEND_PATH=${params.env.PATH}`);
  }
  ...
  const pathExport = hasCustomPath
    ? 'export PATH="${MARKETINGCLAW_PREPEND_PATH}:$PATH"; unset MARKETINGCLAW_PREPEND_PATH; '
    : "";
