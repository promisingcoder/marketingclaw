...
const SETUP_TOKEN_MODEL = process.env.MARKETINGCLAW_LIVE_SETUP_TOKEN_MODEL?.trim() ?? "";
...
async function resolveTokenSource(): Promise<TokenSource> {
  ...
  if (explicitToken) {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-setup-token-"));
  ...
  if (SETUP_TOKEN_RAW && SETUP_TOKEN_RAW !== "1" && SETUP_TOKEN_RAW !== "auto") {
    throw new Error(
      "MARKETINGCLAW_LIVE_SETUP_TOKEN did not look like a setup-token. Use MARKETINGCLAW_LIVE_SETUP_TOKEN_VALUE for raw tokens.",
    );
  }

  if (candidates.length === 0) {
    throw new Error(
      "No Anthropics setup-token profiles found. Set MARKETINGCLAW_LIVE_SETUP_TOKEN_VALUE or MARKETINGCLAW_LIVE_SETUP_TOKEN_PROFILE.",
    );
  }
  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        await ensureMarketingClawModelsJson(cfg, tokenSource.agentDir);
