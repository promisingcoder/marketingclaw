...
describe("readEnvInt", () => {
  ...
  it("reads deprecated PI env integer aliases behind MARKETINGCLAW env names", () => {
    vi.stubEnv("PI_BASH_YIELD_MS", "250");

    expect(readEnvInt("MARKETINGCLAW_BASH_YIELD_MS", "PI_BASH_YIELD_MS")).toBe(250);

    vi.stubEnv("MARKETINGCLAW_BASH_YIELD_MS", "500");

    expect(readEnvInt("MARKETINGCLAW_BASH_YIELD_MS", "PI_BASH_YIELD_MS")).toBe(500);
  });

  it("ignores partial environment integers", () => {
    vi.stubEnv("MARKETINGCLAW_BASH_YIELD_MS", "250ms");
    vi.stubEnv("PI_BASH_YIELD_MS", "500");

    expect(readEnvInt("MARKETINGCLAW_BASH_YIELD_MS", "PI_BASH_YIELD_MS")).toBeUndefined();
  });

  it("reads only strict signed decimal environment integers", () => {
    vi.stubEnv("MARKETINGCLAW_BASH_YIELD_MS", "+250");
    expect(readEnvInt("MARKETINGCLAW_BASH_YIELD_MS", "PI_BASH_YIELD_MS")).toBe(250);

    vi.stubEnv("MARKETINGCLAW_BASH_YIELD_MS", "0x10");
    expect(readEnvInt("MARKETINGCLAW_BASH_YIELD_MS", "PI_BASH_YIELD_MS")).toBeUndefined();

    vi.stubEnv("MARKETINGCLAW_BASH_YIELD_MS", "1e2");
    expect(readEnvInt("MARKETINGCLAW_BASH_YIELD_MS", "PI_BASH_YIELD_MS")).toBeUndefined();
  });

  it("ignores unsafe environment integers", () => {
    vi.stubEnv("MARKETINGCLAW_BASH_YIELD_MS", "9007199254740993");
