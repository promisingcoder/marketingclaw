...
vi.mock("../model-suppression.js", () => {
  ...
  // extensions/qwen/marketingclaw.plugin.json and src/plugins/manifest-model-suppression.ts.
  ...
  return {
    ...
    buildSuppressedBuiltInModelError: ({
      ...
      if (
        ...
        return `Unknown model: ${provider}/gpt-5.3-codex-spark. gpt-5.3-codex-spark is available only through ChatGPT/Codex OAuth. Run \`marketingclaw models auth login --provider openai\` and use openai/gpt-5.3-codex-spark with that OAuth profile; OpenAI API-key auth cannot use this model.`;
      }
      if (isUnsupportedXaiMultiAgentModel(provider, id)) {
        return "Unknown model: xai/grok-4.20-multi-agent-0309. MarketingClaw does not currently support xAI multi-agent models; choose another xAI model. See https://docs.marketingclaw.ai/providers/xai.";
      }
      return undefined;
    },
  };
...
import type { MarketingClawConfig, MarketingClawConfigInput } from "../../config/config.js";
...
describe("resolveModel", () => {
  ...
  it("invalidates agent discovery stores when generated plugin catalogs change", async () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-model-cache-plugin-"));
  ...
  it("invalidates agent discovery stores when inherited default auth changes", async () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-model-cache-"));
    const agentDir = path.join(rootDir, "agent");
  ...
  it("uses the resolved default agent workspace for cached model discovery", () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-model-workspace-"));
    const agentDir = path.join(rootDir, "agent");
  ...
  it("invalidates agent discovery stores when implicit main auth changes without config", async () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-model-cache-state-"));
    const agentDir = path.join(rootDir, "agents", "worker", "agent");
    const mainAgentDir = path.join(rootDir, "agents", "main", "agent");
    fs.mkdirSync(agentDir, { recursive: true });
    fs.mkdirSync(mainAgentDir, { recursive: true });
    try {
      await withEnvAsync({ MARKETINGCLAW_STATE_DIR: rootDir }, async () => {
        mockDiscoveredModel(discoverModels, {
          provider: "openai",
          modelId: "gpt-5.5",
          templateModel: {
  ...
  it("skips MarketingClaw auth and model discovery during dynamic model resolution", async () => {
  ...
  it("prefers user marketingclaw.json config over the Fireworks manifest for the same id", () => {
  ...
  it("defaults missing model cost before handing models to MarketingClaw", () => {
  ...
  it("leaves maxTokens undefined when no configured or catalog value is available (regression: #98295)", () => {
    // Regression for https://github.com/promisingcoder/marketingclaw/issues/98295.
  ...
  it("does not create fallback models from provider overlays alone", () => {
    ...
    } satisfies MarketingClawConfigInput;

    const result = resolveModelForTest(
      "typoProvider",
      "typoed-model",
      "/tmp/agent",
      cfg as unknown as MarketingClawConfig,
    );
  ...
  it.each([
    ...
    expect(result.error).toBe(
      'Unknown model: openai-codex/gpt-5.4. "openai-codex" is a legacy provider ID. Run `marketingclaw doctor --fix` to migrate legacy model and provider config to the current OpenAI format. If the provider has no authenticated profile, run `marketingclaw models status` to check provider auth and re-authenticate if needed. See https://docs.marketingclaw.ai/concepts/model-providers.',
    );
  });
  ...
  it("does not build an openai fallback for removed gpt-5.3-codex-spark", () => {
    ...
    expect(result.model).toBeUndefined();
    expect(result.error).toBe(
      "Unknown model: openai/gpt-5.3-codex-spark. gpt-5.3-codex-spark is available only through ChatGPT/Codex OAuth. Run `marketingclaw models auth login --provider openai` and use openai/gpt-5.3-codex-spark with that OAuth profile; OpenAI API-key auth cannot use this model.",
    );
  });
  ...
  it("does not build a configured fallback for unsupported xAI multi-agent models", () => {
    ...
    expect(result.model).toBeUndefined();
    expect(result.error).toBe(
      "Unknown model: xai/grok-4.20-multi-agent-0309. MarketingClaw does not currently support xAI multi-agent models; choose another xAI model. See https://docs.marketingclaw.ai/providers/xai.",
    );
  });
