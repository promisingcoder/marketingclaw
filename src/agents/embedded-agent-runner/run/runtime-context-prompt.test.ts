// Runtime-context prompt tests keep hidden MarketingClaw context separate from the
...
describe("runtime context prompt submission", () => {
  ...
  it("ignores repeated inline marker mentions without recursive stack growth", () => {
    ...
    const inlineMarkers = Array.from(
      { length: 250 },
      () => "inline <<<BEGIN_MARKETINGCLAW_INTERNAL_CONTEXT>>> marker",
    ).join("\n");
    const effectivePrompt = [
      inlineMarkers,
      "",
    ...
    expect(parts.modelPrompt).toContain("inline <<<BEGIN_MARKETINGCLAW_INTERNAL_CONTEXT>>> marker");
  ...
  it("uses a marker prompt for runtime-only events", () => {
    ...
    expect(parts).toEqual({
      prompt: "Continue the MarketingClaw runtime event.",
      runtimeContext: "internal event",
      runtimeOnly: true,
      runtimeSystemContext: [
        "MarketingClaw runtime event.",
  ...
  it("submits empty-transcript model prompts when persistence is suppressed separately", () => {
    expect(
      resolveRuntimeContextPromptParts({
        effectivePrompt: "[MarketingClaw room event]",
        transcriptPrompt: "",
  ...
  it("labels runtime-only events as system context", () => {
    ...
    expect(parts.runtimeSystemContext).toContain("MarketingClaw runtime event.");
