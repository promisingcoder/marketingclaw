...
export function createEmbeddedRunAuthController(params: {
  ...
  const applyApiKeyInfo = async (candidate?: string): Promise<void> => {
    ...
    if (!apiKeyInfo.apiKey) {
      ...
      // MarketingClaw runtime's authStorage considers the provider authenticated.  Try
      ...
      // sentinel so MarketingClaw runtime's hasConfiguredAuth() passes and the AWS SDK default
