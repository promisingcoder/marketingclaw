...
import {
  ...
  MARKETINGCLAW_RUNTIME_EVENT_HEADER,
} from "../../internal-runtime-context.js";
import type { CurrentInboundPromptContext } from "./params.js";

const MARKETINGCLAW_RUNTIME_EVENT_USER_PROMPT = "Continue the MarketingClaw runtime event.";
...
export function resolveRuntimeContextPromptParts(params: {
  ...
  if (!prompt.trim()) {
    return runtimeContext
      ? {
          prompt: MARKETINGCLAW_RUNTIME_EVENT_USER_PROMPT,
          ...(modelPromptText.trim() && modelPromptText !== MARKETINGCLAW_RUNTIME_EVENT_USER_PROMPT
            ? { modelPrompt: modelPromptText }
            : {}),
          runtimeContext,
          runtimeOnly: true,
