...
function resolveLiveProviderCatalogTimeoutMs(env: NodeJS.ProcessEnv): number | null {
  const live =
    env.MARKETINGCLAW_LIVE_TEST === "1" ||
    env.MARKETINGCLAW_LIVE_GATEWAY === "1" ||
    env.LIVE === "1";
  if (!live) {
    return null;
  }
  const raw = env.MARKETINGCLAW_LIVE_PROVIDER_DISCOVERY_TIMEOUT_MS?.trim();
  if (!raw) {
    return 15_000;
  }
...
function resolveProviderDiscoveryFilter(params: {
  ...
  const testRaw = env.MARKETINGCLAW_TEST_ONLY_PROVIDER_PLUGIN_IDS?.trim();
  if (testRaw) {
  ...
  const rawValues = [
    env.MARKETINGCLAW_LIVE_PROVIDERS?.trim(),
    env.MARKETINGCLAW_LIVE_GATEWAY_PROVIDERS?.trim(),
...
function buildPluginCatalogConfig(ctx: ImplicitProviderContext): MarketingClawConfig {
...
): Promise<NonNullable<MarketingClawConfig["models"]>["providers"]> {
