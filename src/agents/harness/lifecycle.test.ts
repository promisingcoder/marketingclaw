...
describe("AgentHarness lifecycle runner", () => {
  ...
  it("advertises MarketingClaw embedded host capabilities", async () => {
    const harness = createMarketingClawAgentHarness();

    expect(harness.contextEngineHostCapabilities).toEqual(
      MARKETINGCLAW_EMBEDDED_CONTEXT_ENGINE_HOST.capabilities,
    );
  });
