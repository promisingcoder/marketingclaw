...
describe("agent harness registry", () => {
  ...
  it("keeps model-specific harnesses behind plugin registration in auto mode", () => {
    ...
    process.env.MARKETINGCLAW_AGENT_RUNTIME = "auto";
  ...
  it("falls back to MarketingClaw for other models", () => {
    process.env.MARKETINGCLAW_AGENT_RUNTIME = "auto";
  ...
  it("honors explicit provider MarketingClaw runtime policy", () => {
    registerAgentHarness(makeHarness("plugin-harness", { priority: 200 }), {
      ownerPluginId: "plugin-a",
    });
    ...
    expect(
      selectAgentHarness({
        provider: "codex",
        modelId: "gpt-5.4",
        config: providerRuntimeConfig("codex", "marketingclaw"),
      }).id,
    ).toBe("marketingclaw");
  });
