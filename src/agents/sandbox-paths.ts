...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const marketingClawTmpDir = path.resolve(resolvePreferredMarketingClawTmpDir());
  if (!isPathInside(marketingClawTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: marketingClawTmpDir });
  return resolved;
}
