...
describe("buildAgentSystemPrompt", () => {
  it("resolves helper session keys to scoped prompt surfaces", () => {
    ...
    expect(resolveAgentPromptSurfaceForSessionKey("agent:main")).toBe("marketingclaw_main");
    expect(resolveAgentPromptSurfaceForSessionKey(undefined)).toBe("marketingclaw_main");
  });
  ...
  it("includes an MarketingClaw control section", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/marketingclaw",
    });

    expect(prompt).toContain("## MarketingClaw Control");
    expect(prompt).toContain("prefer `gateway` tool");
    expect(prompt).toContain("CLI lifecycle only on explicit user request");
    expect(prompt).toContain("marketingclaw gateway status|restart|start|stop");
  ...
  it("includes embed guidance only for webchat sessions", () => {
    ...
    expect(prompt).toContain(
      '[embed url="/__marketingclaw__/canvas/documents/cv_123/index.html" title="Status" height="320" /]',
    );
    expect(prompt).toContain(
  ...
  it("keeps the MarketingClaw empty-tool fallback on the main prompt surface", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/marketingclaw",
      toolNames: [],
    });

    expect(prompt).toContain("MarketingClaw lists the standard tools above");
  ...
  it("omits ACP harness guidance when ACP is disabled", () => {
    ...
    expect(prompt).toContain("- sessions_spawn: Spawn an isolated sub-agent session");
    expect(prompt).toContain(
      "- agents_list: List MarketingClaw agent ids allowed for sessions_spawn",
    );
  });
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain("Docs: /tmp/marketingclaw/docs");
    expect(prompt).toContain(
      "Docs are authoritative for MarketingClaw self-knowledge: before understanding how MarketingClaw works (memory/daily notes, sessions, tools, Gateway, config, commands, project context), use `Read` or search local docs first; treat AGENTS.md/project context, workspace/profile/memory notes, and `memory_search` as instruction context or user memory, not MarketingClaw design/implementation knowledge.",
    );
  });
  ...
  it("includes docs guidance when docsPath is provided", () => {
    ...
    expect(prompt).toContain("Docs: /tmp/marketingclaw/docs");
    expect(prompt).toContain("Source: /tmp/marketingclaw");
    expect(prompt).toContain(
      "Docs are authoritative for MarketingClaw self-knowledge: before understanding how MarketingClaw works (memory/daily notes, sessions, tools, Gateway, config, commands, project context), use `read` or search local docs first; treat AGENTS.md/project context, workspace/profile/memory notes, and `memory_search` as instruction context or user memory, not MarketingClaw design/implementation knowledge.",
    );
  ...
  it("falls back to public docs and GitHub source guidance when local docs are unavailable", () => {
    ...
    expect(prompt).toContain("Docs: https://docs.marketingclaw.ai");
    expect(prompt).toContain("Source: https://github.com/promisingcoder/marketingclaw");
    expect(prompt).toContain(
  ...
  it("includes bootstrap instructions in system prompt when bootstrap is pending", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/marketingclaw",
      bootstrapMode: "full",
      contextFiles: [{ path: "/tmp/marketingclaw/BOOTSTRAP.md", content: "Ask who I am." }],
    });
    ...
    expect(prompt).toContain("## /tmp/marketingclaw/BOOTSTRAP.md");
  ...
  it("adds ClaudeBot self-update guidance when gateway tool is available", () => {
    ...
    expect(prompt).toContain("## MarketingClaw Self-Update");
  ...
  it("describes Telegram rich text only for rich Telegram runtimes", () => {
    ...
    expect(telegramPrompt).toContain("MarketingClaw renders Telegram-safe rich messages");
  ...
  it("describes sandboxed runtime and elevated when allowed", () => {
    ...
    expect(prompt).toContain(
      "For read/write/edit/apply_patch, file paths resolve against host workspace: /tmp/marketingclaw. For bash/exec commands, use sandbox container paths under /workspace (or relative paths from that workdir), not host paths.",
    );
...
describe("buildAgentBootstrapSystemPromptSections", () => {
  it("can render bootstrap guidance without duplicating Project Context", () => {
    ...
    expect(sections).not.toContain("## /tmp/marketingclaw/BOOTSTRAP.md");
...
describe("buildSubagentSystemPrompt", () => {
  it("renders depth-1 orchestrator guidance, labels, and recovery notes", () => {
    ...
    expect(prompt).toContain("Do not use `exec` (`marketingclaw ...`, `acpx ...`)");
    expect(prompt).toContain("Use `subagents` only for MarketingClaw subagents");
