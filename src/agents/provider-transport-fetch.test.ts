...
describe("buildGuardedModelFetch", () => {
  beforeEach(() => {
    ...
    delete process.env.MARKETINGCLAW_DEBUG_PROXY_URL;
    delete process.env.MARKETINGCLAW_SDK_RETRY_MAX_WAIT_SECONDS;
  });
  ...
  it("does not force explicit debug proxy overrides onto plain HTTP model transports", async () => {
    ...
    process.env.MARKETINGCLAW_DEBUG_PROXY_URL = "http://127.0.0.1:7799";
    const model = {
      id: "kimi-k2.5:cloud",
      provider: "ollama",
      api: "ollama-chat",
  ...
  describe("long retry-after handling", () => {
    ...
    it("respects MARKETINGCLAW_SDK_RETRY_MAX_WAIT_SECONDS", async () => {
      process.env.MARKETINGCLAW_SDK_RETRY_MAX_WAIT_SECONDS = "10";
      fetchWithSsrFGuardMock.mockResolvedValue({
        response: new Response(null, {
          status: 429,
    ...
    it("ignores partial MARKETINGCLAW_SDK_RETRY_MAX_WAIT_SECONDS values", async () => {
      process.env.MARKETINGCLAW_SDK_RETRY_MAX_WAIT_SECONDS = "10s";
      fetchWithSsrFGuardMock.mockResolvedValue({
        response: new Response(null, {
          status: 429,
          headers: { "retry-after": "30" },
    ...
    it.each(["0x10", "1e3"])(
      "ignores non-decimal MARKETINGCLAW_SDK_RETRY_MAX_WAIT_SECONDS values: %s",
      async (value) => {
        process.env.MARKETINGCLAW_SDK_RETRY_MAX_WAIT_SECONDS = value;
        fetchWithSsrFGuardMock.mockResolvedValue({
          response: new Response(null, {
            status: 429,
            headers: { "retry-after": "30" },
    ...
    it("ignores unsafe MARKETINGCLAW_SDK_RETRY_MAX_WAIT_SECONDS values", async () => {
      process.env.MARKETINGCLAW_SDK_RETRY_MAX_WAIT_SECONDS = "9007199254740993";
      fetchWithSsrFGuardMock.mockResolvedValue({
        response: new Response(null, {
          status: 429,
          headers: { "retry-after": "30" },
    ...
    it("can be disabled with MARKETINGCLAW_SDK_RETRY_MAX_WAIT_SECONDS=0", async () => {
      process.env.MARKETINGCLAW_SDK_RETRY_MAX_WAIT_SECONDS = "0";
      fetchWithSsrFGuardMock.mockResolvedValue({
        response: new Response(null, {
          status: 429,
          headers: { "retry-after": "239" },
