...
function resolveCompactionReserveTokensFloor(cfg?: MarketingClawConfig): number {
...
export function resolveEffectiveCompactionMode(cfg?: MarketingClawConfig): AgentCompactionMode {
 ...
 * fires on a successful turn and triggers MarketingClaw runtime's `_runAutoCompaction` from
 * inside `Session.prompt()`, collapsing `agent.state.messages` before the
 * provider call (marketingclaw#75799).
 *
...
/**
 * Disable MarketingClaw runtime's `_checkCompaction → _runAutoCompaction` (which would otherwise
 * fire from inside `Session.prompt()` and reassign `agent.state.messages`
 * before the provider call) when MarketingClaw or a plugin owns compaction:
 * `contextEngineInfo.ownsCompaction === true`, effective safeguard compaction,
 * or an active model that is silent-overflow-prone (marketingclaw#75799).
 * Default-mode runs against ordinary providers keep MarketingClaw runtime's auto-compaction as
 ...
 * rehydrates `compaction.enabled` from disk and silently restores MarketingClaw runtime's
