/**
 * Broad coverage for createMarketingClawCodingTools.
...
import "./test-helpers/fast-marketingclaw-tools.js";
import { wrapToolWithBeforeToolCallHook } from "./agent-tools.before-tool-call.js";
import { createMarketingClawCodingTools } from "./agent-tools.js";
import type { AuthProfileStore } from "./auth-profiles/types.js";
import * as marketingClawPluginTools from "./marketingclaw-plugin-tools.js";
...
function createToolsForStoredSession(storeTemplate: string, sessionKey: string) {
  return createMarketingClawCodingTools({
    sessionKey,
    config: {
      session: {
        store: storeTemplate,
...
function expectNoSubagentControlTools(tools: ReturnType<typeof createMarketingClawCodingTools>) {
...
type MarketingClawCodingTool = ReturnType<typeof createMarketingClawCodingTools>[number];
type MarketingClawToolsOptions = NonNullable<Parameters<typeof createMarketingClawTools>[0]>;

function toolNameList(tools: readonly { name: string }[]): string[] {
  return tools.map((tool) => tool.name);
}

function requireTool(tools: MarketingClawCodingTool[], name: string): MarketingClawCodingTool {
...
function requireToolExecute(
  tool: MarketingClawCodingTool,
): NonNullable<MarketingClawCodingTool["execute"]> {
  if (!tool.execute) {
...
function latestCreateMarketingClawToolsOptions(): MarketingClawToolsOptions {
  const calls = vi.mocked(createMarketingClawTools).mock.calls;
  const lastCall = calls.at(-1);
  const options = lastCall?.[0];
  if (!options) {
    throw new Error("expected createMarketingClawTools call");
  }
  return options;
}
...
function cronCreatorToolNames(
  list: MarketingClawToolsOptions["cronCreatorToolAllowlist"] | undefined,
): string[] | undefined {
  return list?.map((entry) => (typeof entry === "string" ? entry : entry.name));
}

describe("createMarketingClawCodingTools", () => {
  const testConfig: MarketingClawConfig = {};
  ...
  it("exposes gateway config and restart actions to owner sessions", () => {
    const tools = createMarketingClawCodingTools({ config: testConfig });
  ...
  it("passes explicit hook channel ids to wrapped tool hooks", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-hook-channel-"));
  ...
  it("re-wraps existing before_tool_call hooks once with the current context", async () => {
    ...
    vi.mocked(createMarketingClawTools).mockReturnValueOnce([wrapped as never]);

    const tools = createMarketingClawCodingTools({ agentId: "main", sessionId: "session-new" });
  ...
  it("keeps Tool Search controls when core MarketingClaw tools are not materialized", () => {
    const createMarketingClawToolsMock = vi.mocked(createMarketingClawTools);
    createMarketingClawToolsMock.mockClear();
  ...
  it("uses runtime toolsAllow when materializing plugin tools", () => {
    ...
    expect(createMarketingClawToolsMock).toHaveBeenCalledTimes(1);
    const options = latestCreateMarketingClawToolsOptions();
  ...
  it("preserves runtime allowlist groups containing message through restrictive profiles", () => {
    for (const runtimeToolAllowlist of [["group:messaging"], ["group:marketingclaw"], ["*"]]) {
      const tools = createMarketingClawCodingTools({
        config: { tools: { profile: "minimal" } },
  ...
  it("passes source reply delivery mode to MarketingClaw tool construction", () => {
    const createMarketingClawToolsMock = vi.mocked(createMarketingClawTools);
    createMarketingClawToolsMock.mockClear();
    ...
    expect(createMarketingClawToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateMarketingClawToolsOptions().sourceReplyDeliveryMode).toBe(
      "message_tool_only",
    );
  });
  ...
  it("uses the canonical spawn workspace for follow-up task suggestions", () => {
    ...
    expect(latestCreateMarketingClawToolsOptions()).toMatchObject({
      workspaceDir: "/agent/workspace",
      spawnWorkspaceDir: "/host/project",
      cwd: "/host/project",
    });
  ...
  it("keeps an unsandboxed task repo as the follow-up suggestion cwd", () => {
    ...
    expect(latestCreateMarketingClawToolsOptions().cwd).toBe("/task/repo");
  });
  ...
  it("passes plugin suppression into MarketingClaw tool construction plans", () => {
    const createMarketingClawToolsMock = vi.mocked(createMarketingClawTools);
    createMarketingClawToolsMock.mockClear();
    ...
    expect(createMarketingClawToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateMarketingClawToolsOptions().disablePluginTools).toBe(true);
  });

  it("keeps plugin-only construction off the MarketingClaw core factory", () => {
    const createMarketingClawToolsMock = vi.mocked(createMarketingClawTools);
    createMarketingClawToolsMock.mockClear();
  ...
  it("forwards active model metadata to plugin-only tool construction", () => {
    ...
    const resolvePluginToolsSpy = vi
      .spyOn(marketingClawPluginTools, "resolveMarketingClawPluginToolsForOptions")
      .mockReturnValue([]);
  ...
  it("uses tools.alsoAllow for optional plugin discovery without widening to all plugins", () => {
    ...
    expect(createMarketingClawToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateMarketingClawToolsOptions().pluginToolAllowlist).toStrictEqual([
      "lobster",
      DEFAULT_PLUGIN_TOOLS_ALLOWLIST_ENTRY,
    ]);
  });

  it("passes explicit denylist entries to MarketingClaw tool factory planning", () => {
    const createMarketingClawToolsMock = vi.mocked(createMarketingClawTools);
    createMarketingClawToolsMock.mockClear();
    ...
    expect(createMarketingClawToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreateMarketingClawToolsOptions().pluginToolDenylist, ["pdf"]);
  });

  it("passes inherited allowlist entries to MarketingClaw plugin discovery", async () => {
    const createMarketingClawToolsMock = vi.mocked(createMarketingClawTools);
    createMarketingClawToolsMock.mockClear();
    const agentId = `inherited-allow-${Date.now()}-${Math.random().toString(16).slice(2)}`;
    const storeTemplate = path.join(
      os.tmpdir(),
      `marketingclaw-session-store-${agentId}-{agentId}.json`,
    );
    ...
    expect(createMarketingClawToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreateMarketingClawToolsOptions().pluginToolAllowlist, [
      "custom_plugin_tool",
      "sessions_spawn",
    ]);
  });
  ...
  it("passes effective allow-list-restricted tool surface to spawned sessions", () => {
    ...
    expect(createMarketingClawToolsMock).toHaveBeenCalledTimes(1);
    const inheritedAllow = latestCreateMarketingClawToolsOptions().inheritedToolAllowlist;
  ...
  it("passes group-restricted tool surface to cron-created agent turns", () => {
    ...
    expect(createMarketingClawToolsMock).toHaveBeenCalledTimes(1);
    const cronAllow = latestCreateMarketingClawToolsOptions().cronCreatorToolAllowlist;
  ...
  it("lets embedded attempts refresh a caller-owned cron creator tool surface", () => {
    ...
    const cronCreatorToolAllowlistRef: NonNullable<
      MarketingClawToolsOptions["cronCreatorToolAllowlist"]
    > = [];
  ...
  it("records core tool-prep stages for hot-path diagnostics", () => {
    ...
    expectListIncludes(stages, [
      ...
      "base-coding-tools",
      "shell-tools",
      "marketingclaw-tools:test-helper",
      "marketingclaw-tools",
    ...
    expect(stages.indexOf("marketingclaw-tools:test-helper")).toBeLessThan(
      stages.indexOf("marketingclaw-tools"),
    );
    expect(stages.indexOf("schema-normalization")).toBeLessThan(stages.indexOf("tool-hooks"));
  });

  it("preserves action enums in normalized schemas", () => {
    const defaultTools = createMarketingClawCodingTools({ config: testConfig });
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const openAiTools = createMarketingClawCodingTools({
      config: testConfig,
    ...
    const codexTools = createMarketingClawCodingTools({
      config: testConfig,
      modelProvider: "openai",
      modelId: "gpt-5.4",
    });
    ...
    const disabledConfig: MarketingClawConfig = {
      tools: {
        exec: {
    ...
    const disabledOpenAiTools = createMarketingClawCodingTools({
    ...
    const anthropicTools = createMarketingClawCodingTools({
      config: disabledConfig,
    ...
    const allowModelsConfig: MarketingClawConfig = {
      tools: {
        exec: {
    ...
    const allowed = createMarketingClawCodingTools({
    ...
    const denied = createMarketingClawCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
    ...
    const oauthTools = createMarketingClawCodingTools({
      config: testConfig,
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createMarketingClawCodingTools({ messageProvider: "discord" });
    const names = new Set(tools.map((tool) => tool.name));
  ...
  it("separates the canonical message provider from transport tool policy", () => {
    vi.mocked(createMarketingClawTools).mockClear();
    ...
    expect(latestCreateMarketingClawToolsOptions().agentChannel).toBe("discord");
  });
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-depth-policy-"));
    try {
  ...
  it("applies subagent tool policy to ACP children spawned under a subagent envelope", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-acp-subagent-policy-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("applies leaf tool policy for cross-agent subagent sessions when spawnDepth is missing", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-cross-agent-subagent-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("keeps browser out of coding-profile subagents unless profile-stage alsoAllow adds it", () => {
    ...
    const codingSubagent = createMarketingClawCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: baseConfig,
    });
    ...
    const subagentAllowOnly = createMarketingClawCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
        tools: {
    ...
    const profileStageAlsoAllow = createMarketingClawCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
  ...
  it("can keep message available when a cron route needs it under the coding profile", () => {
    const codingTools = createMarketingClawCodingTools({
      config: { tools: { profile: "coding" } },
    });
    expect(toolNameList(codingTools)).not.toContain("message");

    const cronTools = createMarketingClawCodingTools({
      config: { tools: { profile: "coding" } },
      forceMessageTool: true,
    });
  ...
  it("can keep message available when a cron route needs it under a provider coding profile", () => {
    const providerProfileTools = createMarketingClawCodingTools({
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    const googleTools = createMarketingClawCodingTools({
  ...
  it("applies xai model compat for direct Grok tool cleanup", () => {
    const xaiTools = createMarketingClawCodingTools({
      modelProvider: "xai",
      modelCompat: {
  ...
  it("returns image-aware read metadata for images and text-only blocks for text files", async () => {
    const defaultTools = createMarketingClawCodingTools();
    const readTool = requireTool(defaultTools, "read");
    const readExecute = requireToolExecute(readTool);

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-read-"));
    try {
      ...
      const contents = "Hello from marketingclaw read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "marketingclaw-sandbox");
    const sandbox = createAgentToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "marketingclaw-workspace"),
    ...
    const tools = createMarketingClawCodingTools({ sandbox });
  ...
  it("accepts canonical parameters for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-canonical-"));
    try {
      const tools = createMarketingClawCodingTools({ workspaceDir: tmpDir });
  ...
  it("roots memory flush append-only writes in the workspace when cwd differs", async () => {
    const workspaceDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "marketingclaw-memory-workspace-"),
    );
    const taskCwd = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-memory-cwd-"));
  ...
  it("rejects legacy alias parameters", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-legacy-alias-"));
    try {
      const tools = createMarketingClawCodingTools({ workspaceDir: tmpDir });
      const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
  ...
  it("rejects structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-structured-write-"));
    try {
      const tools = createMarketingClawCodingTools({ workspaceDir: tmpDir });
  ...
  it("rejects structured edit payloads", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-structured-edit-"));
    try {
