    ...
    expect(
      resolveProviderDiscoveryFilterForTest({
        env: liveFilterEnv({
          ...
          MARKETINGCLAW_LIVE_PROVIDERS: "claude-cli",
        }),
        resolveOwners,
      }),
    ).toEqual(["anthropic"]);
  ...
  it("honors gateway live provider filters too", () => {
    expect(
      resolveProviderDiscoveryFilterForTest({
        env: liveFilterEnv({
          MARKETINGCLAW_LIVE_TEST: "1",
          MARKETINGCLAW_LIVE_GATEWAY_PROVIDERS: "claude-cli",
        }),
        resolveOwners,
      }),
    ).toEqual(["anthropic"]);
  ...
  it("keeps explicit plugin-id filters when no owning provider plugin exists", () => {
    expect(
      resolveProviderDiscoveryFilterForTest({
        env: liveFilterEnv({
          MARKETINGCLAW_LIVE_TEST: "1",
          MARKETINGCLAW_LIVE_PROVIDERS: "openrouter",
        }),
        resolveOwners,
      }),
    ).toEqual(["openrouter"]);
  ...
  it("does not resolve provider aliases through plugin metadata owners", () => {
    ...
    expect(
      resolveProviderDiscoveryFilterForTest({
        env: liveFilterEnv({
          MARKETINGCLAW_LIVE_TEST: "1",
          MARKETINGCLAW_LIVE_PROVIDERS: "bytedance",
        }),
        resolveOwners: (provider) => resolvePluginMetadataProviderOwnersForTest(snapshot, provider),
      }),
    ).toEqual(["bytedance"]);
