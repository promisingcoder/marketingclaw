...
import type { ImageContent } from "marketingclaw/plugin-sdk/llm";
...
describe("loadPromptRefImages", () => {
  ...
  it("does not reload MarketingClaw CLI image cache paths from prior prompt text", async () => {
    const loadImageFromRefSpy = vi.spyOn(promptImageUtils, "loadImageFromRef");
    const sanitizeImageBlocksSpy = vi.spyOn(toolImages, "sanitizeImageBlocks");
    ...
    await expect(
      loadPromptRefImages({
        prompt:
          'Called the Read tool with {"file_path":"/workspace/.marketingclaw-cli-images/stale.png"}',
        workspaceDir: "/workspace",
      }),
    ).resolves.toStrictEqual([]);
...
describe("buildCliArgs", () => {
  ...
  it("passes Codex system prompts via a model instructions file config override", () => {
    expect(
      buildCliArgs({
        ...
        systemPromptFilePath: "/tmp/marketingclaw/system-prompt.md",
        useResume: false,
      }),
    ).toEqual([
      "exec",
      "--json",
      "-c",
      'model_instructions_file="/tmp/marketingclaw/system-prompt.md"',
    ]);
  });
  ...
  it("passes Claude system prompts through its file flag", () => {
    ...
    ).toEqual(["-p", "--append-system-prompt-file", "/tmp/marketingclaw/system-prompt.md"]);
  });
...
describe("writeCliImages", () => {
  it("uses stable hashed file paths so repeated image hydration reuses the same path", async () => {
    const workspaceDir = await fs.mkdtemp(
      path.join(resolvePreferredMarketingClawTmpDir(), "marketingclaw-cli-write-images-"),
    );
    ...
    try {
      expect(first.paths).toStrictEqual([
        expect.stringMatching(
          new RegExp(
            `^${escapeRegExp(`${resolvePreferredMarketingClawTmpDir()}/marketingclaw-cli-images/`)}.*\\.png$`,
          ),
        ),
      ]);
  ...
  it("uses the shared media extension map for image formats beyond the tiny builtin list", async () => {
    const workspaceDir = await fs.mkdtemp(
      path.join(resolvePreferredMarketingClawTmpDir(), "marketingclaw-cli-write-heic-"),
    );
    const image: ImageContent = {
      type: "image",
      data: "aGVpYy1pbWFnZQ==",
  ...
  it("sweeps stale workspace-scoped CLI image files", async () => {
    const workspaceDir = await fs.mkdtemp(
      path.join(resolvePreferredMarketingClawTmpDir(), "marketingclaw-cli-write-sweep-"),
    );
    const imageRoot = path.join(workspaceDir, ".marketingclaw-cli-images");
  ...
  it("hydrates prompt media refs into codex image args through the helper seams", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredMarketingClawTmpDir(), "marketingclaw-cli-prompt-image-"),
    );
    ...
    try {
      ...
      expect(argv).toStrictEqual([
        ...
        "--image",
        expect.stringContaining("marketingclaw-cli-images"),
      ]);
  ...
  it("appends hydrated prompt media refs for stdin backends through the helper seams", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredMarketingClawTmpDir(), "marketingclaw-cli-prompt-image-generic-"),
    );
    ...
    try {
      ...
      expect(promptWithImages).toContain("marketingclaw-cli-images");
  ...
  it("appends Gemini prompt refs with @-prefixed image paths", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredMarketingClawTmpDir(), "marketingclaw-cli-prompt-image-gemini-"),
    );
    ...
    try {
      ...
      expect(
        prepared.imagePaths?.[0]?.startsWith(path.join(tempDir, ".marketingclaw-cli-images")),
      ).toBe(true);
  ...
  it("prefers explicit images over prompt refs through the helper seams", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredMarketingClawTmpDir(), "marketingclaw-cli-explicit-images-"),
    );
    ...
    try {
      ...
      expect(argv[argv.indexOf("--image") + 1]).toContain("marketingclaw-cli-images");
  ...
  it("merges inline payloads with offloaded refs in attachment order", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-cli-mixed-images-"));
...
describe("writeCliSystemPromptFile", () => {
  it("writes stripped system prompts to a private temp file", async () => {
    ...
    try {
      expect(written.filePath).toContain("marketingclaw-cli-system-prompt-");
