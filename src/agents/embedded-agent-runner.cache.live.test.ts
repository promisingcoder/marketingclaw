...
import type { AssistantMessage, Message, Tool } from "marketingclaw/plugin-sdk/llm";
...
describeCacheLive("embedded agent runner prompt caching (live)", () => {
  beforeAll(async () => {
    liveRunnerRootDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-live-cache-"));
    ...
    previousCacheTraceEnv = {
      enabled: process.env.MARKETINGCLAW_CACHE_TRACE,
      file: process.env.MARKETINGCLAW_CACHE_TRACE_FILE,
      messages: process.env.MARKETINGCLAW_CACHE_TRACE_MESSAGES,
      prompt: process.env.MARKETINGCLAW_CACHE_TRACE_PROMPT,
      system: process.env.MARKETINGCLAW_CACHE_TRACE_SYSTEM,
    };
    setTestEnvValue("MARKETINGCLAW_CACHE_TRACE", "1");
    setTestEnvValue("MARKETINGCLAW_CACHE_TRACE_FILE", liveCacheTraceFile);
    setTestEnvValue("MARKETINGCLAW_CACHE_TRACE_MESSAGES", "0");
    setTestEnvValue("MARKETINGCLAW_CACHE_TRACE_PROMPT", "0");
    setTestEnvValue("MARKETINGCLAW_CACHE_TRACE_SYSTEM", "0");
  }, 120_000);
  ...
  afterAll(async () => {
    if (previousCacheTraceEnv) {
      ...
      restore("MARKETINGCLAW_CACHE_TRACE", previousCacheTraceEnv.enabled);
      restore("MARKETINGCLAW_CACHE_TRACE_FILE", previousCacheTraceEnv.file);
      restore("MARKETINGCLAW_CACHE_TRACE_MESSAGES", previousCacheTraceEnv.messages);
      restore("MARKETINGCLAW_CACHE_TRACE_PROMPT", previousCacheTraceEnv.prompt);
      restore("MARKETINGCLAW_CACHE_TRACE_SYSTEM", previousCacheTraceEnv.system);
    }
  ...
  describe("openai", () => {
    ...
    beforeAll(async () => {
      fixture = await resolveLiveDirectModel({
        ...
        envVar: "MARKETINGCLAW_LIVE_OPENAI_CACHE_MODEL",
  ...
  describe("anthropic", () => {
    ...
    beforeAll(async () => {
      fixture = await resolveLiveDirectModel({
        ...
        envVar: "MARKETINGCLAW_LIVE_ANTHROPIC_CACHE_MODEL",
