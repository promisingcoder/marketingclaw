...
async function withManagedMediaRoot<T>(run: (ctx: { stateDir: string }) => Promise<T>) {
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-managed-media-"));
  try {
    return await withEnvAsync({ MARKETINGCLAW_STATE_DIR: stateDir }, async () => {
...
async function withOutsideHardlinkInMarketingClawTmp<T>(
  params: {
    marketingClawTmpDir: string;
  ...
  const hardlinkPath = path.join(
    params.marketingClawTmpDir,
    makeTmpProbePath(params.hardlinkPrefix),
  );
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.marketingClawTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.marketingClawTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.marketingClawTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const marketingClawTmpDir = resolvePreferredMarketingClawTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred MarketingClaw tmp root",
      media: path.join(marketingClawTmpDir, "image.png"),
      expected: path.join(marketingClawTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred MarketingClaw tmp root",
      media: pathToFileURL(path.join(marketingClawTmpDir, "photo.png")).href,
      expected: path.join(marketingClawTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred MarketingClaw tmp root",
      media: path.join(marketingClawTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(marketingClawTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(marketingClawTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside marketingclaw tmp root",
      media: path.join(os.tmpdir(), "outside-marketingclaw", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked MarketingClaw tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(marketingClawTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(marketingClawTmpDir, { recursive: true });
      const symlinkPath = path.join(marketingClawTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked MarketingClaw tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInMarketingClawTmp(
      {
        marketingClawTmpDir,
  ...
  it("rejects symlinked MarketingClaw tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInMarketingClawTmp(
