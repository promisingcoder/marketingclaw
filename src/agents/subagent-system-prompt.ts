    ...
    lines.push(
      ...
      ...(acpEnabled
        ? [
            ...
            '`agents_list` and `subagents` apply to MarketingClaw sub-agents (`runtime: "subagent"`); ACP harness ids are controlled by `acp.allowedAgents`.',
            "Do not ask users to run slash commands or CLI when `sessions_spawn` can do it directly.",
            "Do not use `exec` (`marketingclaw ...`, `acpx ...`) to spawn ACP sessions.",
            'Use `subagents` only for MarketingClaw subagents (`runtime: "subagent"`).',
