...
let configOverride: MarketingClawConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: MarketingClawConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/promisingcoder/marketingclaw/issues/31298
  ...
  afterAll(() => {
    ...
    process.env.MARKETINGCLAW_TEST_FAST = previousFastTestEnv;
  });
  ...
  beforeEach(() => {
    vi.useRealTimers();
    // MARKETINGCLAW_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("MarketingClaw runtime context (internal):");
