...
describe("host tool tilde expansion (non-workspace mode)", () => {
  ...
  it("edit readFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("marketingclaw-tilde-test-edit-");
  ...
  it("write writeFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("marketingclaw-tilde-test-write-");
  ...
  it("write mkdir expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("marketingclaw-tilde-test-mkdir-");
  ...
  it("ignores MARKETINGCLAW_HOME for write operations", async () => {
    const marketingclawHome = await createTempDir("marketingclaw-home-override-", os.tmpdir());
    const dir = await createTempDir("marketingclaw-tilde-test-write-");
    const testFile = path.join(dir, "os-home-write.txt");

    await withEnvAsync({ MARKETINGCLAW_HOME: marketingclawHome }, async () => {
      createHostWorkspaceWriteTool(marketingclawHome, { workspaceOnly: false });
      ...
      await expectMissingPath(fs.access(path.join(marketingclawHome, path.basename(testFile))));
    });
  });

  it("ignores MARKETINGCLAW_HOME for mkdir operations", async () => {
    const marketingclawHome = await createTempDir("marketingclaw-home-override-", os.tmpdir());
    const dir = await createTempDir("marketingclaw-tilde-test-mkdir-");
    ...
    await withEnvAsync({ MARKETINGCLAW_HOME: marketingclawHome }, async () => {
      ...
      expect((await fs.stat(newDir)).isDirectory()).toBe(true);
      await expectMissingPath(fs.access(path.join(marketingclawHome, path.basename(newDir))));
    });
  });

  it("ignores MARKETINGCLAW_HOME for readFile operations", async () => {
    const marketingclawHome = await createTempDir("marketingclaw-home-override-", os.tmpdir());
    const dir = await createTempDir("marketingclaw-tilde-test-edit-");
    ...
    await withEnvAsync({ MARKETINGCLAW_HOME: marketingclawHome }, async () => {
      createHostWorkspaceEditTool(marketingclawHome, { workspaceOnly: false });
      const content = await readEditOps().readFile(toTildePath(testFile));
  ...
  it("ignores MARKETINGCLAW_HOME for access operations", async () => {
    const marketingclawHome = await createTempDir("marketingclaw-home-override-", os.tmpdir());
    const dir = await createTempDir("marketingclaw-tilde-test-edit-");
