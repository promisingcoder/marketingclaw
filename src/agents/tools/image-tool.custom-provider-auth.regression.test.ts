...
import { resolveImageToolFactoryAvailable } from "../marketingclaw-tools.media-factory-plan.js";
...
function createUserReportedConfig(params?: { includeApiKey?: boolean }): MarketingClawConfig {
...
function createBedrockSdkConfig(): MarketingClawConfig {
  return {
...
async function withEmptyAgentDir<T>(run: (agentDir: string) => Promise<T>): Promise<T> {
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-image-auth-regression-"));
  try {
