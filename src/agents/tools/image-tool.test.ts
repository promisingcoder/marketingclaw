...
import { isInboundPathAllowed } from "@marketingclaw/media-core/inbound-path-policy";
...
type CreateMarketingClawCodingToolsArgs = Parameters<typeof createMarketingClawCodingTools>[0];
type MockMarketingClawToolsOptions = {
...
vi.mock("../auth-profiles.js", () => ({
  ...
  ensureAuthProfileStore: (agentDir?: string) => {
    ...
    if (process.env.MARKETINGCLAW_TEST_CODEX_CLI_OAUTH === "1") {
...
vi.mock("../auth-profiles/external-cli-sync.js", () => ({
  resolveExternalCliAuthProfiles: (
    ...
    if (
      process.env.MARKETINGCLAW_TEST_CODEX_CLI_OAUTH !== "1" ||
...
vi.mock("../model-auth.js", () => ({
  ...
  hasRuntimeAvailableProviderAuth: (params: {
    ...
    if (params.provider === "codex") {
      return process.env.MARKETINGCLAW_TEST_CODEX_ROUTE === "1";
    }
  ...
  hasUsableCustomProviderApiKey: (cfg?: MarketingClawConfig, provider?: string) => {
...
// Client-cap filtering has dedicated coverage; this suite only needs image-tool propagation.
vi.mock("../marketingclaw-tools.js", async () => {
  const { createImageTool: createImageToolLocal } = await import("./image-tool.js");
  return {
    filterToolsByClientCaps: (tools: unknown[]) => tools,
    createMarketingClawTools: vi.fn((options?: MockMarketingClawToolsOptions) => {
...
async function createMarketingClawCodingToolsWithFreshModules(
  options?: CreateMarketingClawCodingToolsArgs,
) {
  installFastLocalImageProviderStubs(minimaxProvider, moonshotProvider);
  return createMarketingClawCodingTools(options);
}

async function withTempAgentDir<T>(run: (agentDir: string) => Promise<T>): Promise<T> {
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-image-"));
  try {
...
async function withTempWorkspacePng(
  ...
  const workspaceParent = await fs.mkdtemp(path.join(parentDir, "marketingclaw-workspace-image-"));
  try {
...
function createMinimaxImageConfig(): MarketingClawConfig {
  return {
    agents: {
      defaults: {
...
async function withTempSandboxState(
  ...
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-image-sandbox-"));
...
describe("image tool implicit imageModel config", () => {
  ...
  registerImageToolEnvReset(priorFetch, [
    ...
    "ZAI_API_KEY",
    "Z_AI_API_KEY",
    "MARKETINGCLAW_TEST_CODEX_CLI_OAUTH",
    "MARKETINGCLAW_TEST_CODEX_ROUTE",
  ...
  it.each(implicitImageRoutingCases)(
    ...
    async ({ cfg, profiles, codexRoute, openAiApiKey, expected }) => {
      if (codexRoute) {
        vi.stubEnv("MARKETINGCLAW_TEST_CODEX_ROUTE", "1");
      }
      if (openAiApiKey) {
  ...
  it("lets external CLI Codex OAuth survive the candidate auth filter", async () => {
    await withTempAgentDir(async (agentDir) => {
      vi.stubEnv("MARKETINGCLAW_TEST_CODEX_CLI_OAUTH", "1");
      vi.stubEnv("MARKETINGCLAW_TEST_CODEX_ROUTE", "1");
  ...
  it("respects fsPolicy.workspaceOnly for non-sandbox image paths", async () => {
    await withTempWorkspacePng(async ({ workspaceDir, imagePath }) => {
      ...
      await withTempAgentDir(async (agentDir) => {
        ...
        const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-outside-"));
  ...
  it("still rejects non-workspace local image paths when workspaceOnly is disabled", async () => {
    ...
    await withTempAgentDir(async (agentDir) => {
      const cfg = createMinimaxImageConfig();
      const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-image-outside-"));
      const outsideImage = path.join(outsideDir, "secret.png");
      await fs.writeFile(outsideImage, Buffer.from(ONE_PIXEL_PNG_B64, "base64"));
      try {
        const tool = createRequiredImageTool({
  ...
  it("allows image paths from the current iMessage account attachment roots", async () => {
    await withTempAgentDir(async (agentDir) => {
      ...
      const attachmentRoot = await fs.mkdtemp(
        path.join(os.tmpdir(), "marketingclaw-imessage-root-"),
      );
  ...
  it("allows image paths from current iMessage wildcard attachment roots", async () => {
    await withTempAgentDir(async (agentDir) => {
      ...
      const attachmentRootParent = await fs.mkdtemp(
        path.join(os.tmpdir(), "marketingclaw-imessage-wildcard-root-"),
      );
  ...
  it("allows workspace images via createMarketingClawCodingTools when workspace root is explicit", async () => {
    await withTempWorkspacePng(async ({ workspaceDir, imagePath }) => {
      const fetch = stubMinimaxOkFetch();
      await withTempAgentDir(async (agentDir) => {
        const cfg = createMinimaxImageConfig();

        const tools = await createMarketingClawCodingToolsWithFreshModules({
          config: cfg,
          agentDir,
          workspaceDir,
        });
  ...
  it("rewrites inbound absolute paths into sandbox media/inbound", async () => {
    await withTempSandboxState(async ({ agentDir, sandboxRoot }) => {
      ...
      const res = await tool.execute("t1", {
        prompt: "Describe the image.",
        image: "@/Users/steipete/.marketingclaw/media/inbound/photo.png",
      });
...
describe("image tool MiniMax VLM routing", () => {
  ...
  async function createMinimaxVlmFixture(baseResp: { status_code: number; status_msg: string }) {
    ...
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-minimax-vlm-"));
    vi.stubEnv("MINIMAX_API_KEY", "minimax-test");
    const cfg = createMinimaxImageConfig();
    const tool = createRequiredImageTool({ config: cfg, agentDir });
    return { fetch, tool };
...
describe("image tool managed inbound media", () => {
  ...
  async function withManagedInboundPng(
    ...
    const stateDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "marketingclaw-image-managed-inbound-"),
    );
