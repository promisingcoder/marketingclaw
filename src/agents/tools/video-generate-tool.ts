...
function hasExplicitVideoGenerationModelConfig(cfg?: MarketingClawConfig): boolean {
...
async function executeVideoGenerationJob(params: {
  effectiveCfg: MarketingClawConfig;
  prompt: string;
  agentDir?: string;
  model?: string;
  size?: string;
...
export function createVideoGenerateTool(options?: {
  ...
  const cfg: MarketingClawConfig = options?.config ?? getRuntimeConfig();
  if (
    !hasGenerationToolAvailability({
      cfg,
