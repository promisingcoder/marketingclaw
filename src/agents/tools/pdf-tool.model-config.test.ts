...
const TEST_AGENT_DIR = "/tmp/marketingclaw-pdf-model-config";
...
vi.mock("./model-config.helpers.js", () => ({
  ...
  hasProviderAuthForTool: ({ provider, cfg }: { provider: string; cfg?: MarketingClawConfig }) => {
  ...
  resolveDefaultModelRef: (cfg?: MarketingClawConfig) => {
