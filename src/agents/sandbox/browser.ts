...
const CDP_SOURCE_RANGE_ENV_KEY = "MARKETINGCLAW_BROWSER_CDP_SOURCE_RANGE";
const CDP_AUTH_TOKEN_ENV_KEY = "MARKETINGCLAW_BROWSER_CDP_AUTH_TOKEN";
const SANDBOX_BROWSER_IMAGE_CONTRACT_LABEL = "org.marketingclaw.sandbox-browser.contract";

function buildSandboxCdpAuthHeader(token: string): string {
  return `Basic ${Buffer.from(`marketingclaw:${token}`).toString("base64")}`;
}

function buildSandboxCdpUrl(params: { cdpPort: number; authToken: string }): string {
  const url = new URL(`http://127.0.0.1:${params.cdpPort}`);
  url.username = "marketingclaw";
...
function buildSandboxBrowserResolvedConfig(params: {
  ...
  return {
    ...
    noSandbox: false,
    attachOnly: true,
    defaultProfile: DEFAULT_MARKETINGCLAW_BROWSER_PROFILE_NAME,
    extraArgs: [],
    tabCleanup: {
      enabled: true,
      idleMinutes: 120,
...
export async function ensureSandboxBrowser(params: {
  ...
  if (hasContainer) {
    ...
    currentHash = await readDockerContainerLabel(containerName, "marketingclaw.configHash");
      ...
      if (isHot) {
        const hint = (() => {
          if (params.cfg.scope === "session") {
            return `marketingclaw sandbox recreate --browser --session ${params.scopeKey}`;
          }
          if (params.cfg.scope === "agent") {
            const agentId = resolveSandboxAgentId(params.scopeKey) ?? "main";
            return `marketingclaw sandbox recreate --browser --agent ${agentId}`;
          }
          return "marketingclaw sandbox recreate --browser --all";
        })();
        defaultRuntime.log(
  ...
  if (!hasContainer) {
    ...
    const args = buildSandboxCreateArgs({
      ...
      labels: {
        ...
        "marketingclaw.browserConfigEpoch": SANDBOX_BROWSER_SECURITY_HASH_EPOCH,
      },
    ...
    args.push("-e", `MARKETINGCLAW_BROWSER_HEADLESS=${params.cfg.browser.headless ? "1" : "0"}`);
    args.push(
      "-e",
      `MARKETINGCLAW_BROWSER_ENABLE_NOVNC=${params.cfg.browser.enableNoVnc ? "1" : "0"}`,
    );
    args.push("-e", `MARKETINGCLAW_BROWSER_CDP_PORT=${params.cfg.browser.cdpPort}`);
    args.push("-e", `${CDP_AUTH_TOKEN_ENV_KEY}=${cdpAuthToken}`);
    args.push(
      "-e",
      `MARKETINGCLAW_BROWSER_AUTO_START_TIMEOUT_MS=${params.cfg.browser.autoStartTimeoutMs}`,
    );
    if (cdpSourceRange) {
      args.push("-e", `${CDP_SOURCE_RANGE_ENV_KEY}=${cdpSourceRange}`);
    }
    args.push("-e", `MARKETINGCLAW_BROWSER_VNC_PORT=${params.cfg.browser.vncPort}`);
    args.push("-e", `MARKETINGCLAW_BROWSER_NOVNC_PORT=${params.cfg.browser.noVncPort}`);
    args.push("-e", "MARKETINGCLAW_BROWSER_NO_SANDBOX=1");
  ...
  const existingProfile = existing
    ? resolveProfile(existing.bridge.state.resolved, DEFAULT_MARKETINGCLAW_BROWSER_PROFILE_NAME)
    : null;
