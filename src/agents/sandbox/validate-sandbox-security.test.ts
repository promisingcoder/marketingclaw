...
describe("getBlockedBindReason", () => {
  ...
  it("still blocks OS-home credential paths when MARKETINGCLAW_HOME points elsewhere", () => {
    withEnv({ HOME: "/home/tester", MARKETINGCLAW_HOME: "/srv/marketingclaw-home" }, () => {
  ...
  it("blocks canonical OS-home aliases for credential paths", () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "marketingclaw-home-"));
...
describe("validateBindMounts", () => {
  it("allows legitimate project directory mounts", () => {
    const projectRoot = mkdtempSync(join(tmpdir(), "marketingclaw-sbx-safe-"));
    expect(
      validateBindMounts([
  ...
  it("allows drive-absolute Windows bind sources", () => {
    expect(validateBindMounts(["D:/data/marketingclaw/src:/src:ro"])).toBeUndefined();
    expect(validateBindMounts(["D:\\data\\marketingclaw\\output:/output:rw"])).toBeUndefined();
  });

  it("compares Windows allowed roots case-insensitively", () => {
    expect(
      validateBindMounts(["d:/DATA/MarketingClaw/src:/src:ro"], {
        allowedSourceRoots: ["D:/data/marketingclaw"],
      }),
    ).toBeUndefined();
  ...
  it("blocks symlink escapes into blocked directories", () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "marketingclaw-sbx-"));
  ...
  it("blocks symlink-parent escapes into blocked paths when leaf does not exist", () => {
    ...
    const missingLeaf = join(link, "marketingclaw-not-created");
    expect(() =>
  ...
  it("blocks bind sources outside allowed roots when allowlist is configured", () => {
    const allowedRoot = mkdtempSync(join(tmpdir(), "marketingclaw-sbx-allowed-root-"));
    const externalRoot = mkdtempSync(join(tmpdir(), "marketingclaw-sbx-external-"));
    expect(() =>
  ...
  it("allows bind sources in allowed roots when allowlist is configured", () => {
    const projectRoot = mkdtempSync(join(tmpdir(), "marketingclaw-sbx-allowed-"));
    expect(
  ...
  it("blocks reserved container target paths by default", () => {
    const projectRoot = mkdtempSync(join(tmpdir(), "marketingclaw-sbx-reserved-default-"));
    expect(() =>
  ...
  it("allows reserved container target paths with explicit dangerous override", () => {
    const projectRoot = mkdtempSync(join(tmpdir(), "marketingclaw-sbx-reserved-"));
    expect(
...
describe("validateApparmorProfile", () => {
  it("allows named profile/undefined", () => {
    expect(validateApparmorProfile("marketingclaw-sandbox")).toBeUndefined();
...
describe("validateSandboxSecurity", () => {
  it("passes with safe config", () => {
    const projectRoot = mkdtempSync(join(tmpdir(), "marketingclaw-sbx-safe-config-"));
    expect(
      validateSandboxSecurity({
