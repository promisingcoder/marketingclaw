...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-docker-mounts-"));
  tmpDirs.push(dir);
  return dir;
}
...
function spawnDockerProcess(command: string, args: string[]) {
  ...
  if (command !== "docker") {
    ...
    args[0] === "inspect" &&
    args[1] === "-f" &&
    args[2]?.includes('index .Config.Labels "marketingclaw.configHash"')
  ) {
...
function createSandboxConfig(
  ...
  return {
    ...
    browser: {
      enabled: false,
      image: "marketingclaw-browser:test",
...
describe("ensureSandboxContainer config-hash recreation", () => {
  ...
  it("recreates shared container when array-order change alters hash", async () => {
    ...
    expect(createCall.args).toContain(`marketingclaw.configHash=${newHash}`);
  ...
  it("applies custom binds after workspace mounts so overlapping binds can override", async () => {
    ...
    const createCall = await ensureSandboxCreateCallForTest({ cfg, workspaceDir });
    expect(createCall.args).toContain(`marketingclaw.configHash=${expectedHash}`);
  ...
  it("stamps the mount format version label on created containers", async () => {
    ...
    expect(createCall.args).toContain(
      `marketingclaw.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`,
    );
  });
});
