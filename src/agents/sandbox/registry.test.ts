...
const {
  TEST_STATE_DIR,
  PREVIOUS_MARKETINGCLAW_STATE_DIR,
  ...
  const baseDir = mkdtempSync(nodePath.join(tmpdir(), "marketingclaw-sandbox-registry-"));
  const previousStateDir = process.env.MARKETINGCLAW_STATE_DIR;
  Reflect.set(process.env, "MARKETINGCLAW_STATE_DIR", baseDir);

  return {
    TEST_STATE_DIR: baseDir,
    PREVIOUS_MARKETINGCLAW_STATE_DIR: previousStateDir,
...
afterAll(async () => {
  ...
  if (PREVIOUS_MARKETINGCLAW_STATE_DIR === undefined) {
    deleteTestEnvValue("MARKETINGCLAW_STATE_DIR");
  } else {
    setTestEnvValue("MARKETINGCLAW_STATE_DIR", PREVIOUS_MARKETINGCLAW_STATE_DIR);
  }
});
