...
function makeTempDir(): string {
  const dir = mkdtempSync(path.join(os.tmpdir(), "marketingclaw-browser-mounts-"));
  tmpDirs.push(dir);
  return dir;
}
...
vi.mock("../../plugin-sdk/browser-profiles.js", () => ({
  ...
  DEFAULT_MARKETINGCLAW_BROWSER_COLOR: "#FF4500",
  DEFAULT_MARKETINGCLAW_BROWSER_PROFILE_NAME: "marketingclaw",
  resolveProfile: (
...
function buildConfig(enableNoVnc: boolean): SandboxConfig {
  return {
    ...
    browser: {
      ...
      containerPrefix: "marketingclaw-sbx-browser-",
...
describe("ensureSandboxBrowser create args", () => {
  ...
  it("rejects stale sandbox browser images without the relay auth contract", async () => {
    ...
    await expect(
      ...
      "Sandbox browser image marketingclaw-sandbox-browser:bookworm-slim is stale or incompatible",
    );
  ...
  it("keeps the browser Dockerfile contract label aligned with the runtime constant", () => {
    ...
    const label = dockerfile.match(
      /^LABEL org\.marketingclaw\.sandbox-browser\.contract="([^"]+)"$/m,
    )?.[1];
  ...
  it("publishes noVNC on loopback and injects noVNC password env", async () => {
    ...
    expect(envEntries).toContain("MARKETINGCLAW_BROWSER_NO_SANDBOX=1");
    const passwordEntry = envEntries.find((entry) =>
      entry.startsWith("MARKETINGCLAW_BROWSER_NOVNC_PASSWORD="),
    );
    expect(passwordEntry).toMatch(/^MARKETINGCLAW_BROWSER_NOVNC_PASSWORD=[A-Za-z0-9]{8}$/);
  ...
  it("does not inject noVNC password env when noVNC is disabled", async () => {
    ...
    const envEntries = collectDockerFlagValues(createArgs ?? [], "-e");
    expect(
      envEntries.filter((entry) => entry.startsWith("MARKETINGCLAW_BROWSER_NOVNC_PASSWORD=")),
    ).toStrictEqual([]);
  ...
  it("includes the explicit env policy epoch in the browser config hash when needed", async () => {
    ...
    const createArgs = requireDockerCreateArgs();
    expect(createArgs).toContain(`marketingclaw.configHash=${expectedHash}`);
  ...
  it("recreates a cached bridge when the SSRF policy changes", async () => {
    ...
    BROWSER_BRIDGES.set("session:test", {
      ...
      containerName: "marketingclaw-sbx-browser-session-test-0661d10a",
  ...
  it("stamps the mount format version label on browser containers", async () => {
    ...
    expect(labels).toContain(`marketingclaw.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  });
  ...
  it("force-removes the browser container when CDP never becomes reachable", async () => {
    ...
    expect(dockerMocks.execDocker).toHaveBeenCalledWith(
      ["rm", "-f", "marketingclaw-sbx-browser-session-test-0661d10a"],
      { allowFailure: true },
    );
  });
  ...
  it("requires auth for the sandbox CDP relay without auto-derived source ranges", async () => {
    ...
    const authEntry = envEntries.find((entry) =>
      entry.startsWith("MARKETINGCLAW_BROWSER_CDP_AUTH_TOKEN="),
    );
    expect(authEntry).toMatch(/^MARKETINGCLAW_BROWSER_CDP_AUTH_TOKEN=[0-9a-f]{48}$/);
    expect(envEntries).not.toContain("MARKETINGCLAW_BROWSER_CDP_SOURCE_RANGE=172.21.0.1/32");

    const token = requireValue(authEntry, "CDP auth env").slice(
      "MARKETINGCLAW_BROWSER_CDP_AUTH_TOKEN=".length,
    );
    const profiles = latestBridgeResolved().profiles as Record<
      string,
      { cdpPort?: number; cdpUrl?: string }
    >;
    expect(profiles.marketingclaw?.cdpPort).toBe(49100);
    expect(profiles.marketingclaw?.cdpUrl).toBe(`http://marketingclaw:${token}@127.0.0.1:49100`);
  });
  ...
  it("passes explicit cdpSourceRange as an additional relay filter", async () => {
    ...
    const createArgs = findDockerArgsCall(dockerMocks.execDocker.mock.calls, "create");
    const envEntries = collectDockerFlagValues(createArgs ?? [], "-e");
    expect(envEntries).toContain("MARKETINGCLAW_BROWSER_CDP_SOURCE_RANGE=10.0.0.0/24");
  });
  ...
  it("does not inject a source range for network=none by default", async () => {
    ...
    const envEntries = collectDockerFlagValues(createArgs, "-e");
    expect(
      envEntries.some((entry) => entry.startsWith("MARKETINGCLAW_BROWSER_CDP_SOURCE_RANGE=")),
    ).toBe(false);
  });
});
