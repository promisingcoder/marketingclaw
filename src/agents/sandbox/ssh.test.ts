...
describe("sandbox ssh helpers", () => {
  it("materializes inline ssh auth data into a temp config", async () => {
    ...
    expect(config).toContain("Host marketingclaw-sandbox");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const realParent = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-ssh-real-"));
      ...
      const target = path.join(root, "workspace", ".marketingclaw", "sandbox-skills");
      await execFileAsync("/bin/sh", [
        "-c",
        ENSURE_REMOTE_REAL_DIRECTORY_SCRIPT,
        "marketingclaw-remote-dir",
        target,
        root,
      ]);

      await expect(
        fs.stat(path.join(realParent, "runtime", "workspace", ".marketingclaw", "sandbox-skills")),
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const realParent = makeTempDir(tempDirs, "marketingclaw-ssh-real-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const root = makeTempDir(tempDirs, "marketingclaw-ssh-workdir-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const root = makeTempDir(tempDirs, "marketingclaw-ssh-root-");
  ...
  it("builds remote workdir validation commands with quoted literal paths", () => {
    ...
    expect(command).toContain("marketingclaw-validate-workdir");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      await fs.symlink(realParent, path.join(root, "workspace", ".marketingclaw"));
      ...
      await expect(
        execFileAsync("/bin/sh", [
          ...
          ENSURE_REMOTE_REAL_DIRECTORY_SCRIPT,
          "marketingclaw-remote-dir",
          path.join(root, "workspace", ".marketingclaw", "sandbox-skills"),
          root,
        ]),
      ).rejects.toThrow(/unsafe remote directory symlink/);
    },
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const localDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-ssh-upload-"));
      ...
      await expect(
        uploadDirectoryToSshTarget({
          session: {
            command: "ssh",
            configPath: "/tmp/marketingclaw-test-ssh-config",
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const localDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-ssh-upload-safe-"));
      tempDirs.push(localDir);
