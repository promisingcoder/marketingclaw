...
import { markMarketingClawExecEnv } from "../../infra/marketingclaw-exec-env.js";
...
export async function ensureDockerImage(image: string) {
  ...
  if (image === DEFAULT_SANDBOX_IMAGE) {
    throw new Error(
      `Sandbox image not found: ${image}. Build it with scripts/sandbox-setup.sh before enabling Docker sandboxing. The default image includes python3 for sandbox write/edit helpers; MarketingClaw will not substitute plain debian:bookworm-slim.`,
    );
  }
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "marketingclaw.sandbox=1");
  args.push("--label", `marketingclaw.sessionKey=${params.scopeKey}`);
  args.push("--label", `marketingclaw.createdAtMs=${createdAtMs}`);
  args.push("--label", `marketingclaw.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  if (params.configHash) {
    args.push("--label", `marketingclaw.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markMarketingClawExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "marketingclaw.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`marketingclaw sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`marketingclaw sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("marketingclaw sandbox recreate --all");
}
