...
async function withTempSessionLockFile(
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-lock-"));
  try {
...
describe("acquireSessionWriteLock", () => {
  ...
  it("reports live MarketingClaw-owned stale locks without removing them", async () => {
    await withTempSessionLockFile(async ({ sessionFile, lockPath }) => {
      const owner = spawn(
        process.execPath,
        ["-e", "setInterval(() => {}, 1000)", "marketingclaw"],
        {
          stdio: "ignore",
        },
      );
  ...
  it("lets session write-lock env override config for emergency tuning", () => {
    expect(
      resolveSessionWriteLockOptions(
        ...
        {
          env: {
            MARKETINGCLAW_SESSION_WRITE_LOCK_ACQUIRE_TIMEOUT_MS: "120000",
            MARKETINGCLAW_SESSION_WRITE_LOCK_STALE_MS: "60000",
            MARKETINGCLAW_SESSION_WRITE_LOCK_MAX_HOLD_MS: "50000",
          },
        },
      ),
    ).toEqual({
  ...
  it("ignores non-decimal and unsafe session write-lock env values", () => {
    expect(
      resolveSessionWriteLockOptions(
        ...
        {
          env: {
            MARKETINGCLAW_SESSION_WRITE_LOCK_ACQUIRE_TIMEOUT_MS: "1e3",
            MARKETINGCLAW_SESSION_WRITE_LOCK_STALE_MS: "0x1000",
            MARKETINGCLAW_SESSION_WRITE_LOCK_MAX_HOLD_MS: "9007199254740993",
          },
        },
      ),
    ).toEqual({
  ...
  it("allows explicit infinite acquire timeout but not infinite stale policy", () => {
    expect(
      resolveSessionWriteLockOptions(undefined, {
        env: {
          MARKETINGCLAW_SESSION_WRITE_LOCK_ACQUIRE_TIMEOUT_MS: "Infinity",
          MARKETINGCLAW_SESSION_WRITE_LOCK_STALE_MS: "Infinity",
        },
      }),
    ).toMatchObject({
      timeoutMs: Number.POSITIVE_INFINITY,
  ...
  it("uses resolved stale policy when cleaning stale lock files", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-lock-policy-"));
    ...
    try {
      ...
      const configOnly = await cleanStaleLockFiles({
        ...
        nowMs,
        removeStale: false,
        readOwnerProcessArgs: () => ["node", "/opt/marketingclaw/marketingclaw.mjs", "doctor"],
      });
      ...
      const envOverride = await cleanStaleLockFiles({
        sessionsDir,
        config: { session: { writeLock: { staleMs: 30_000 } } },
        env: { MARKETINGCLAW_SESSION_WRITE_LOCK_STALE_MS: "60000" },
        nowMs,
        removeStale: false,
        readOwnerProcessArgs: () => ["node", "/opt/marketingclaw/marketingclaw.mjs", "doctor"],
      });
  ...
  it("does not clean live MarketingClaw locks just because holder max hold expired", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-lock-policy-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
    const nowMs = Date.now();
    ...
    try {
      ...
      const result = await cleanStaleLockFiles({
        ...
        nowMs,
        removeStale: true,
        readOwnerProcessArgs: () => ["node", "/opt/marketingclaw/marketingclaw.mjs", "agent"],
      });
  ...
  it("cleans old live .jsonl lock files owned by non-MarketingClaw processes", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
    const nowMs = Date.now();
    const lockPath = path.join(sessionsDir, "old-non-marketingclaw.jsonl.lock");
    ...
    try {
      ...
      expect(lockCleanupRecords(result.cleaned)).toEqual([
        {
          name: "old-non-marketingclaw.jsonl.lock",
          removed: true,
          stale: true,
          staleReasons: ["too-old", "non-marketingclaw-owner"],
        },
      ]);
      await expectPathMissing(lockPath);
    } finally {
  ...
  it("cleans fresh live .jsonl lock files owned by a non-MarketingClaw process", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
      ...
      expect(lockCleanupRecords(result.locks)).toEqual([
        {
          ...
          removed: true,
          stale: true,
          staleReasons: ["non-marketingclaw-owner"],
        },
      ]);
      expect(lockCleanupRecords(result.cleaned)).toEqual([
        {
  ...
  it("cleans fresh live .jsonl lock files owned by generic non-MarketingClaw entrypoints", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
  ...
  it("keeps fresh live .jsonl lock files with MarketingClaw or unknown owners", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });

    const nowMs = Date.now();
    const marketingclawLock = path.join(sessionsDir, "marketingclaw-live.jsonl.lock");
    ...
    try {
      ...
      const marketingclawResult = await cleanStaleLockFiles({
        sessionsDir,
        staleMs: 30_000,
        nowMs,
        removeStale: true,
      ...
      expect(marketingclawResult.cleaned).toEqual([]);
      await expect(fs.access(marketingclawLock)).resolves.toBeUndefined();

      await fs.rm(marketingclawLock, { force: true });
      await fs.writeFile(
        gatewayLock,
        JSON.stringify({
          pid: process.pid,
  ...
  it("removes held locks on termination signals", async () => {
    ...
    try {
      for (const signal of signals) {
        const root = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-lock-cleanup-"));
        try {
          const sessionFile = path.join(root, "sessions.json");
          const lockPath = `${sessionFile}.lock`;
