...
const EXEC_SHELL_SNAPSHOT_ENV = "MARKETINGCLAW_EXEC_SHELL_SNAPSHOT";
...
describe("exec shell snapshots", () => {
  ...
  it("leaves commands unchanged when trusted process env disables snapshots", async () => {
    const stateDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "marketingclaw-snapshot-disabled-state-"),
    );
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-snapshot-disabled-home-"));
    ...
    expect(wrapped).toBe(command);
    expect(fs.existsSync(resolveShellSnapshotDir({ MARKETINGCLAW_STATE_DIR: stateDir }))).toBe(
      false,
    );
  });
  ...
  it("does not honor per-call env for selecting the snapshot state dir", async () => {
    ...
    const trustedStateDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "marketingclaw-snapshot-trusted-state-"),
    );
    const untrustedStateDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "marketingclaw-snapshot-untrusted-state-"),
    );
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-snapshot-state-home-"));
    const untrustedHome = fs.mkdtempSync(
      path.join(os.tmpdir(), "marketingclaw-snapshot-untrusted-home-"),
    );
    ...
    const wrapped = await maybeWrapCommandWithShellSnapshot({
      ...
      env: {
        ...
        MARKETINGCLAW_STATE_DIR: untrustedStateDir,
    ...
    expect(
      fs.existsSync(resolveShellSnapshotDir({ MARKETINGCLAW_STATE_DIR: untrustedStateDir })),
    ).toBe(false);
    expect(
      fs.existsSync(resolveShellSnapshotDir({ MARKETINGCLAW_STATE_DIR: trustedStateDir })),
    ).toBe(true);
  ...
  it("captures bash startup aliases, functions, and safe environment without secrets", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-snapshot-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-snapshot-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-snapshot-cwd-"));
    ...
    fs.writeFileSync(
      ...
      [
        ...
        '[ "$MARKETINGCLAW_SHELL" = exec ] && alias oc_snap_exec_alias="printf marker-ok"',
  ...
  it("captures bash aliases behind common interactive-only guards", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-snapshot-interactive-home-"));
    const stateDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "marketingclaw-snapshot-interactive-state-"),
    );
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-snapshot-interactive-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
    fs.writeFileSync(
      path.join(home, ".bashrc"),
  ...
  it("preserves per-call safe env overrides after trusted capture", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-snapshot-env-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-snapshot-env-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-snapshot-env-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
  ...
  it("preserves per-call env outside the snapshot allowlist", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-snapshot-plugin-env-home-"));
    const stateDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "marketingclaw-snapshot-plugin-env-state-"),
    );
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-snapshot-plugin-env-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
  ...
  it("does not let non-fingerprinted env change captured shell state", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-snapshot-branch-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-snapshot-branch-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-snapshot-branch-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
    fs.writeFileSync(
      path.join(home, ".bashrc"),
  ...
  it("refreshes stale snapshot files when startup files source alias fragments", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-snapshot-refresh-home-"));
    const stateDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "marketingclaw-snapshot-refresh-state-"),
    );
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-snapshot-refresh-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
  ...
  it("refreshes fresh cached snapshots that no longer parse", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-snapshot-corrupt-home-"));
    const stateDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "marketingclaw-snapshot-corrupt-state-"),
    );
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-snapshot-corrupt-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
    ...
    fs.writeFileSync(
      snapshotPath,
      [
        "# MarketingClaw exec shell snapshot. Generated; do not edit.",
  ...
  it("refuses to persist aliases or functions with literal secret-looking values", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-snapshot-secret-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-snapshot-secret-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-snapshot-secret-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
    fs.writeFileSync(
      path.join(home, ".bashrc"),
  ...
  it("captures zsh aliases in sourceable form", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-snapshot-zsh-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-snapshot-zsh-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-snapshot-zsh-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
    fs.writeFileSync(
  ...
  it("captures zsh startup state from ZDOTDIR", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-snapshot-zdot-home-"));
    const zdotdir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-snapshot-zdot-dir-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-snapshot-zdot-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-snapshot-zdot-cwd-"));
