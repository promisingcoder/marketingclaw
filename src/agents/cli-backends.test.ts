...
function createClaudeCliOverrideConfig(config: CliBackendConfig): MarketingClawConfig {
  return {
    agents: {
      defaults: {
        cliBackends: {
...
describe("resolveCliBackendConfig claude-cli defaults", () => {
  it("derives bypassPermissions from MarketingClaw's default YOLO exec policy", () => {
    ...
    expect(resolved?.config.args).toContain("mcp__marketingclaw__*");
    ...
    expect(resolved?.config.resumeArgs).toContain("mcp__marketingclaw__*");
  ...
  it("keeps Claude permission mode unset when MarketingClaw exec policy is not YOLO", () => {
    const resolved = requireCliBackendConfig("claude-cli", {
