...
export const DEFAULT_MAX_OUTPUT = clampWithDefault(
  readEnvInt("MARKETINGCLAW_BASH_MAX_OUTPUT_CHARS", "PI_BASH_MAX_OUTPUT_CHARS"),
  200_000,
  1_000,
  200_000,
);
...
export const DEFAULT_PENDING_MAX_OUTPUT = clampWithDefault(
  readEnvInt("MARKETINGCLAW_BASH_PENDING_MAX_OUTPUT_CHARS"),
  30_000,
  1_000,
  200_000,
);
  ...
  switch (params.failureKind) {
    ...
    case "overall-timeout":
      return typeof params.timeoutSec === "number" && params.timeoutSec > 0
        ? `Command timed out after ${params.timeoutSec} seconds. If this command is expected to take longer, re-run with a higher timeout (e.g., exec timeout=300). If it should keep running, start it with exec background=true or yieldMs so MarketingClaw can register a pollable process session. Do not rely on shell backgrounding with a trailing &.`
        : "Command timed out. If this command is expected to take longer, re-run with a higher timeout (e.g., exec timeout=300). If it should keep running, start it with exec background=true or yieldMs so MarketingClaw can register a pollable process session. Do not rely on shell backgrounding with a trailing &.";
...
function wrapPosixCommandWithPathPrepend(
  ...
  env.MARKETINGCLAW_PREPEND_PATH = pathPrepend.join(path.delimiter);

  return `export PATH="\${MARKETINGCLAW_PREPEND_PATH}\${PATH:+:$PATH}"; unset MARKETINGCLAW_PREPEND_PATH; ${command}`;
}
