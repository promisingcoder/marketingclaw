...
const planMarketingClawModelsJsonMock = vi.fn();
...
function planParamsAt(callIndex: number): {
  ...
  // Planner call shape is the contract between ensureMarketingClawModelsJson and planning.
  const call = planMarketingClawModelsJsonMock.mock.calls[callIndex];
  if (!call) {
...
beforeAll(async () => {
  vi.doMock("./models-config.plan.js", () => ({
    planMarketingClawModelsJson: (...args: unknown[]) => planMarketingClawModelsJsonMock(...args),
  }));
  ...
  ({ ensureMarketingClawModelsJson } = await import("./models-config.js"));
...
beforeEach(() => {
  ...
  planMarketingClawModelsJsonMock
    .mockReset()
...
describe("models-config write serialization", () => {
  it("does not reuse default workspace plugin metadata for explicit agent dirs without workspace", async () => {
    await withModelsTempHome(async (home) => {
      ...
      await ensureMarketingClawModelsJson({}, agentDir);
  ...
  it("reuses current plugin metadata for explicit agent dirs with matching workspace", async () => {
    await withModelsTempHome(async (home) => {
      ...
      await ensureMarketingClawModelsJson({}, agentDir, { workspaceDir });
  ...
  it("writes implicit models.json into the configured default agent dir", async () => {
    await withModelsTempHome(async (home) => {
      ...
      const result = await ensureMarketingClawModelsJson(cfg);

      expect(result.agentDir).toBe(path.join(home, ".marketingclaw", "agents", "ops", "agent"));
      await expect(fs.access(path.join(result.agentDir, "models.json"))).resolves.toBeUndefined();
      await expectMissingPath(
        fs.access(path.join(home, ".marketingclaw", "agents", "main", "agent", "models.json")),
      );
    });
  });
  ...
  it("writes plugin-owned model catalogs beside the agent plugin state", async () => {
    await withModelsTempHome(async (home) => {
      ...
      planMarketingClawModelsJsonMock.mockImplementation(async () => ({
        action: "write",
  ...
  it("removes stale plugin-owned model catalogs", async () => {
    await withModelsTempHome(async (home) => {
      ...
      const result = await ensureMarketingClawModelsJson({}, agentDir);
  ...
  it("keeps generated plugin catalogs on non-authoritative skip plans", async () => {
    await withModelsTempHome(async (home) => {
      ...
      planMarketingClawModelsJsonMock.mockImplementation(async () => ({ action: "skip" }));
  ...
  it("does not reuse scoped startup discovery cache for a different provider scope", async () => {
    await withModelsTempHome(async (home) => {
      ...
      expect(planMarketingClawModelsJsonMock).toHaveBeenCalledTimes(2);
  ...
  it("keeps the ready cache warm after models.json is written", async () => {
    await withModelsTempHome(async () => {
      await ensureMarketingClawModelsJson(CUSTOM_PROXY_MODELS_CONFIG);
      await ensureMarketingClawModelsJson(CUSTOM_PROXY_MODELS_CONFIG);

      expect(planMarketingClawModelsJsonMock).toHaveBeenCalledTimes(1);
    });
  });
  ...
  it("keeps distinct config fingerprints cached without evicting each other", async () => {
    await withModelsTempHome(async () => {
      planMarketingClawModelsJsonMock.mockImplementation(async () => ({ action: "noop" }));
      ...
      await ensureMarketingClawModelsJson(first);
      await ensureMarketingClawModelsJson(second);
      await ensureMarketingClawModelsJson(first);
  ...
  it("serializes concurrent models.json writes to avoid overlap", async () => {
    await withModelsTempHome(async () => {
      ...
      const writes = Promise.all([
        ensureMarketingClawModelsJson(first),
        ensureMarketingClawModelsJson(second),
      ]);
