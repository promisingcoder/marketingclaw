...
function createTempDir(): string {
  return createTempPluginDir(tempDirs, "marketingclaw-codex-ext-");
}
...
describe("agent tool result middleware", () => {
  it("includes plugin-registered middleware and restores it from cache", async () => {
    ...
    process.env.MARKETINGCLAW_BUNDLED_PLUGINS_DIR = tmp;
  ...
  api.registerAgentToolResultMiddleware(async (event) => ({
    ...
    expect(listAgentToolResultMiddlewares("marketingclaw")).toHaveLength(0);
...
export default { id: "tool-result-middleware", register(api) {
  api.registerAgentToolResultMiddleware(middleware, { runtimes: ["marketingclaw"] });
  api.registerAgentToolResultMiddleware(middleware, { runtimes: ["codex"] });
    ...
    expect(listAgentToolResultMiddlewares("marketingclaw")).toHaveLength(1);
    expect(listAgentToolResultMiddlewares("codex")).toHaveLength(1);
  });
  ...
  config: {
    ...
    loadMarketingClawPlugins({ config });
    expect(listAgentToolResultMiddlewares("codex")).toHaveLength(0);
