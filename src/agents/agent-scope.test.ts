...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: MarketingClawConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/marketingclaw" }],
      },
    };
        ...
        list: [
          {
            ...
            agentDir: "~/.marketingclaw/agents/main",
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: MarketingClawConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: MarketingClawConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: MarketingClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgStringModel: MarketingClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgStrictAgentWithDefaultFallbacks: MarketingClawConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgDisable: MarketingClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaultsWithoutAgentModel: MarketingClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
  ...
  it("updates the effective model primary at the winning config layer", () => {
    ...
    const inheritedCfg: MarketingClawConfig = {
      agents: {
        defaults: {
          model: {
            primary: "openai/gpt-5.4",
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: MarketingClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
    ...
    const cfgAgentOverrideOnly: MarketingClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
    ...
    const cfg: MarketingClawConfig = {
      agents: {
        list: [
          {
            id: "restricted",
            workspace: "~/marketingclaw-restricted",
            tools: {
              allow: ["read"],
    ...
    const cfg: MarketingClawConfig = {
      agents: {
        list: [
          {
            id: "family",
            workspace: "~/marketingclaw-family",
            sandbox: {
              mode: "all",
              scope: "agent",
            },
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/marketingclaw");
  });

  it("uses MARKETINGCLAW_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "marketingclaw-home");
    withEnv({ MARKETINGCLAW_HOME: home }, () => {
      const workspace = resolveAgentWorkspaceDir({} as MarketingClawConfig, "main");
      expect(workspace).toBe(path.join(path.resolve(home), ".marketingclaw", "workspace"));
    });
  });

  it("uses MARKETINGCLAW_WORKSPACE_DIR for default agent workspace", () => {
  ...
  it("uses MARKETINGCLAW_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "marketingclaw-home");
    withEnv({ MARKETINGCLAW_HOME: home, MARKETINGCLAW_STATE_DIR: "" }, () => {
      const agentDir = resolveAgentDir({} as MarketingClawConfig, "main");
      expect(agentDir).toBe(
        path.join(path.resolve(home), ".marketingclaw", "agents", "main", "agent"),
      );
    });
  });
  ...
  it("resolves default agentDir from the configured default agent", () => {
    ...
    const agentDir = withEnv({ MARKETINGCLAW_STATE_DIR: stateDir }, () =>
  ...
  it("non-default agent without defaults.workspace falls back to stateDir", () => {
    ...
    const workspace = withEnv({ MARKETINGCLAW_STATE_DIR: stateDir }, () =>
...
describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/marketingclaw-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/marketingclaw-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-agent-scope-"));
