...
export function logRuntimeToolSchemaQuarantine(params: {
  ...
  const summary = params.diagnostics
    ...
    .join("; ");
  log.warn(
    `[tools] quarantined ${params.diagnostics.length} unsupported tool schema${params.diagnostics.length === 1 ? "" : "s"} before model runtime projection: ${summary}. Run marketingclaw doctor for details.`,
  );
}
