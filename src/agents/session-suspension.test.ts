...
vi.mock("./command/session.js", () => ({
  resolveStoredSessionKeyForSessionId: () => ({
    ...
    storePath: "/tmp/marketingclaw-session-suspension-test/sessions.json",
  }),
}));

async function suspendLane(ttlMs: number, cfg: MarketingClawConfig, laneId: CommandLane) {
...
describe("session suspension", () => {
  ...
  it("auto-resumes cron lanes to the cron concurrency default", async () => {
    ...
    await suspendLane(100, {} as MarketingClawConfig, CommandLane.CronNested);
  ...
  it("auto-resumes cron lanes to configured and clamped cron concurrency", async () => {
    ...
    await suspendLane(
      100,
      { cron: { maxConcurrentRuns: 3 } } as MarketingClawConfig,
      CommandLane.Cron,
    );
    await vi.advanceTimersByTimeAsync(100);
    ...
    await suspendLane(
      100,
      { cron: { maxConcurrentRuns: 0 } } as MarketingClawConfig,
      CommandLane.Cron,
    );
    await vi.advanceTimersByTimeAsync(100);
  ...
  it("clamps oversized suspension TTLs for timers and persisted resume time", async () => {
    ...
    await suspendLane(Number.MAX_SAFE_INTEGER, {} as MarketingClawConfig, CommandLane.Main);
