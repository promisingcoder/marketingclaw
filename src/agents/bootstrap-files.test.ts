...
async function createHeartbeatAgentsWorkspace() {
  const workspaceDir = await makeTempWorkspace("marketingclaw-bootstrap-");
...
describe("resolveBootstrapFilesForRun", () => {
  ...
  it("keeps BOOTSTRAP.md when current setup state cannot be read", async () => {
    const workspaceDir = await makeTempWorkspace("marketingclaw-bootstrap-");
    await fs.mkdir(path.join(workspaceDir, "marketingclaw-workspace-state.json"), {
      recursive: true,
    });
    await fs.writeFile(path.join(workspaceDir, "AGENTS.md"), "rules", "utf8");
  ...
  it("ignores stale root BOOTSTRAP.md for home-relative workspace paths", async () => {
    registerBootstrapFileHook();
    const parentDir = await makeTempWorkspace("marketingclaw-bootstrap-home-");
    ...
    const files = await withEnvAsync({ MARKETINGCLAW_HOME: parentDir }, async () =>
  ...
  it("keeps subagent sessions to project and tool bootstrap files", async () => {
    const workspaceDir = await makeTempWorkspace("marketingclaw-bootstrap-subagent-");
    await Promise.all(
      [
  ...
  it("keeps cron sessions on their existing minimal bootstrap files", async () => {
    const workspaceDir = await makeTempWorkspace("marketingclaw-bootstrap-cron-");
    await Promise.all(
      [
        ["AGENTS.md", "project rules"],
        ["TOOLS.md", "tool rules"],
...
describe("hasCompletedBootstrapTurn", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(
      path.join(await fs.realpath("/tmp"), "marketingclaw-bootstrap-turn-"),
    );
  });
