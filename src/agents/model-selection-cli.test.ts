...
describe("isCliProvider", () => {
  ...
  it("returns true for setup-registered cli backends", () => {
    expect(isCliProvider("claude-cli", {} as MarketingClawConfig)).toBe(true);
  });

  it("returns false for provider ids", () => {
    expect(isCliProvider("example-cli", {} as MarketingClawConfig)).toBe(false);
  });
  ...
  it("does not execute setup runtime when descriptor metadata has no matching backend", () => {
    ...
    expect(isCliProvider("openai", {} as MarketingClawConfig)).toBe(false);
  });
});
