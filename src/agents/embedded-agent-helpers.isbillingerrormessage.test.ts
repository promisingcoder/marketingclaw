...
describe("classifyFailoverReason provider messages", () => {
  ...
  it("classifies internal and compatibility error messages", () => {
    ...
    expect(
      classifyFailoverReason(
        'No API key found for provider "openai". Auth store: /tmp/marketingclaw-agent-abc/auth-profiles.json (agentDir: /tmp/marketingclaw-agent-abc).',
      ),
    ).toBe("auth");
  ...
  it("classifies Chinese provider error messages correctly", () => {
    ...
    // from https://github.com/promisingcoder/marketingclaw/issues/56242
    expect(
      classifyFailoverReason(
...
describe("classifyProviderRuntimeFailureKind", () => {
  ...
  it("classifies OAuth refresh timeouts and lock contention distinctly", () => {
    ...
    expect(
      classifyProviderRuntimeFailureKind(
        "file lock timeout for /tmp/marketingclaw-oauth-refresh.lock",
      ),
