...
const CAPTURE_MARKER = "__MARKETINGCLAW_SHELL_SNAPSHOT_CAPTURE__";
const ENV_MARKER = "__MARKETINGCLAW_SHELL_SNAPSHOT_ENV__";
...
async function captureShellSnapshot(opts: ShellSnapshotWrapOptions): Promise<string | null> {
  ...
  const captureOutputDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "marketingclaw-shell-snapshot-"),
  );
...
function buildTrustedSnapshotCaptureEnv(
  ...
  // MARKETINGCLAW_SHELL is injected by the exec runtime, so startup files can keep
  // their documented exec-specific branches without trusting model input.
  if (runtimeEnv.MARKETINGCLAW_SHELL === "exec") {
    env.MARKETINGCLAW_SHELL = "exec";
  }
  return env;
}
