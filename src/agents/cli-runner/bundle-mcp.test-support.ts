...
export function setupCliBundleMcpTestHarness(): void {
  beforeAll(async () => {
    ...
    envSnapshot = captureEnv(["MARKETINGCLAW_BUNDLED_PLUGINS_DIR"]);
    bundleProbeHomeDir = await tempHarness.createTempDir("marketingclaw-cli-bundle-mcp-home-");
    bundleProbeWorkspaceDir = await tempHarness.createTempDir(
      "marketingclaw-cli-bundle-mcp-workspace-",
    );
    const emptyBundledDir = await tempHarness.createTempDir(
      "marketingclaw-cli-bundle-mcp-bundled-",
    );
    setTestEnvValue("MARKETINGCLAW_BUNDLED_PLUGINS_DIR", emptyBundledDir);
...
function createEnabledBundleProbeConfig(): MarketingClawConfig {
  return {
    plugins: {
      entries: {
