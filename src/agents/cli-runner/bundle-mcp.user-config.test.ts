/** Tests merging user MarketingClaw MCP server config into Claude bundle-MCP overlays. */
...
describe("prepareCliBundleMcpConfig user mcp.servers", () => {
  it("merges user-configured mcp.servers from MarketingClaw config", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "marketingclaw-cli-bundle-mcp-user-servers-",
    );
  ...
  it("translates MarketingClaw transport field on user mcp.servers into Claude type", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "marketingclaw-cli-bundle-mcp-user-servers-transport-",
    );
  ...
  it("preserves explicit type and still strips transport on user mcp.servers", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "marketingclaw-cli-bundle-mcp-user-servers-transport-explicit-",
    );
  ...
  it("user mcp.servers do not override the loopback additionalConfig", async () => {
    // The MarketingClaw loopback server is generated runtime state and must win over
    // user config with the same server name.
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "marketingclaw-cli-bundle-mcp-user-servers-loopback-",
    );
  ...
  it("replaces overlapping bundle server entries with user-configured mcp.servers", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "marketingclaw-cli-bundle-mcp-user-servers-replace-",
    );
