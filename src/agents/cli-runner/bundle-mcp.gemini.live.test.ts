...
const LIVE = isLiveTestEnabled(["MARKETINGCLAW_LIVE_CLI_MCP_GEMINI"]);
...
async function startLocalStreamableHttpMcpServer(): Promise<{
  ...
  const mcpServer = new McpServer({ name: "marketingclaw-gemini-live-probe", version: "1.0.0" });
  mcpServer.tool("marketingclaw_live_probe", "MarketingClaw Gemini MCP live probe", async () => ({
...
describeLive("Gemini CLI MCP settings smoke", () => {
  it("connects to an MarketingClaw-configured streamable-http server", async () => {
    const geminiCommand = process.env.MARKETINGCLAW_LIVE_GEMINI_COMMAND ?? "gemini";
    ...
    try {
      ...
      expect(output).toContain("marketingclawLiveProbe");
