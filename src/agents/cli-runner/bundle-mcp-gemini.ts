...
export async function writeGeminiSystemSettings(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-gemini-mcp-"));
...
export async function writeGeminiMcpCaptureSettings(params: {
  ...
  const marketingclaw = isRecord(mcpServers.marketingclaw) ? mcpServers.marketingclaw : {};
  const headers = normalizeStringRecord(marketingclaw.headers) ?? {};
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketingclaw-gemini-mcp-attempt-"));
  const settingsPath = path.join(tempDir, "settings.json");
  await writeJson(
    settingsPath,
    {
