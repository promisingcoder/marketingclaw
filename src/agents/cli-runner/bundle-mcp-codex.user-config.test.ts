/** Tests projecting MarketingClaw user MCP servers into Codex app-server config. */
...
describe("buildCodexUserMcpServersThreadConfigPatch", () => {
  it("returns undefined when cfg has no mcp.servers (regression: #80814)", () => {
    ...
    expect(buildCodexUserMcpServersThreadConfigPatch({} as MarketingClawConfig)).toBeUndefined();
    expect(
      buildCodexUserMcpServersThreadConfigPatch({ mcp: {} } as MarketingClawConfig),
    ).toBeUndefined();
    expect(
      buildCodexUserMcpServersThreadConfigPatch({ mcp: { servers: {} } } as MarketingClawConfig),
    ).toBeUndefined();
  });
  ...
  it("filters Codex-scoped user MCP servers by MarketingClaw agent id", () => {
    // Agent-scoped MCP servers should follow the active MarketingClaw agent, while
  ...
  it("omits scoped Codex MCP servers when no MarketingClaw agent id is available", () => {
    const patch = buildCodexUserMcpServersThreadConfigPatch({
      mcp: {
        servers: {
          atlas: {
