...
describe("prepareCliBundleMcpConfig", () => {
  it("injects a strict empty --mcp-config overlay for bundle-MCP-enabled backends without servers", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "marketingclaw-cli-bundle-mcp-empty-",
    );
  ...
  it("serves only the exclusive config, ignoring user and plugin servers", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "marketingclaw-cli-bundle-mcp-exclusive-",
    );
    ...
    const prepared = await prepareCliBundleMcpConfig({
      ...
      config: { plugins: { enabled: false } },
      exclusiveConfig: {
        mcpServers: { marketingclaw: { command: "node", args: ["crestodian.mjs"] } },
      },
    });
    ...
    expect(raw.mcpServers?.marketingclaw?.args).toEqual(["crestodian.mjs"]);
  ...
  it("strips variadic Claude --mcp-config values and merges every listed config", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "marketingclaw-cli-bundle-mcp-variadic-",
    );
  ...
  it("merges and strips Claude --mcp-config equals form", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "marketingclaw-cli-bundle-mcp-equals-",
    );
  ...
  it("keeps dash-prefixed args after Claude --mcp-config because they terminate variadic values", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "marketingclaw-cli-bundle-mcp-dash-",
    );
  ...
  it("loads workspace bundle MCP plugins from the configured workspace root", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "marketingclaw-cli-bundle-mcp-workspace-root-",
    );
    const pluginRoot = path.join(workspaceDir, ".marketingclaw", "extensions", "workspace-probe");
  ...
  it("merges loopback overlay config with bundle MCP servers", async () => {
    ...
    const prepared = await prepareBundleProbeCliConfig({
      additionalConfig,
      env: {
        MARKETINGCLAW_MCP_TOKEN: "lb-tk-123",
    ...
    const otherEnvPrepared = await prepareBundleProbeCliConfig({
      additionalConfig,
      env: {
        MARKETINGCLAW_MCP_TOKEN: "other-loopback-token",
        MARKETINGCLAW_MCP_CLI_CAPTURE_KEY: "",
      },
    });
    ...
    expect(Object.keys(raw.mcpServers ?? {}).toSorted()).toEqual(["bundleProbe", "marketingclaw"]);
    expect(raw.mcpServers?.marketingclaw?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(raw.mcpServers?.marketingclaw?.headers?.Authorization).toBe("Bearer lb-tk-123");
    expect(raw.mcpServers?.marketingclaw?.headers?.["x-marketingclaw-cli-capture-key"]).toBe("");
    ...
    expect(attemptRaw.mcpServers?.marketingclaw?.headers?.Authorization).toBe("Bearer lb-tk-123");
    expect(attemptRaw.mcpServers?.marketingclaw?.headers?.["x-marketingclaw-cli-capture-key"]).toBe(
      "attempt-123",
    );
  ...
  it("preserves extra env values alongside generated MCP config", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "marketingclaw-cli-bundle-mcp-env-",
    );
    ...
    const prepared = await prepareCliBundleMcpConfig({
      ...
      env: {
        MARKETINGCLAW_MCP_TOKEN: "lb-tk-123",
        MARKETINGCLAW_MCP_SESSION_KEY: "agent:main:telegram:group:chat123",
      },
    });
  ...
  it("leaves args untouched when bundle MCP is disabled", async () => {
    const prepared = await prepareCliBundleMcpConfig({
      ...
      workspaceDir: "/tmp/marketingclaw-bundle-mcp-disabled",
    });
