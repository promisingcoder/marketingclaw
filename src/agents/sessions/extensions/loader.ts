...
import * as bundledLlm from "marketingclaw/plugin-sdk/llm";
...
import { installMarketingClawInternalCorePackageNativeResolver } from "../../../plugins/plugin-sdk-native-resolver.js";
import {
...
const VIRTUAL_MODULES: Record<string, unknown> = {
  ...
  "marketingclaw/plugin-sdk/agent-core": bundledAgentCore,
  "@marketingclaw/plugin-sdk/agent-core": bundledAgentCore,
  "marketingclaw/plugin-sdk/llm": bundledLlm,
  "@marketingclaw/plugin-sdk/llm": bundledLlm,
  "marketingclaw/plugin-sdk/agent-sessions": bundledAgentSessions,
  "@marketingclaw/plugin-sdk/agent-sessions": bundledAgentSessions,
};
...
const EXTENSION_LOADER_ALIAS_IMPORT_PATTERN =
  /(?:@marketingclaw\/plugin-sdk|marketingclaw\/plugin-sdk|@sinclair\/typebox|typebox)(?:\/[A-Za-z0-9_-]+)?/u;
...
function getExtensionLoaderAliases(): Record<string, string> {
  ...
  aliases = {
    ...
    "marketingclaw/plugin-sdk/agent-sessions": agentSessionsEntry,
    "@marketingclaw/plugin-sdk/agent-sessions": agentSessionsEntry,
...
async function loadExtensionSourceTransformModule(
  ...
  if (!extensionSourceTransformLoader) {
    installMarketingClawInternalCorePackageNativeResolver({ moduleUrl: import.meta.url });
...
function readResourceManifest(packageJsonPath: string): ResourceManifest | null {
  try {
    ...
    if (pkg.marketingclaw && typeof pkg.marketingclaw === "object") {
      return pkg.marketingclaw as ResourceManifest;
    }
    return null;
  } catch {
    return null;
 ...
 *
 * Checks for:
 * 1. package.json with "marketingclaw.extensions" field -> returns declared paths
 * 2. index.ts or index.js -> returns the index file
 *
 * Returns resolved paths or null if no entry points found.
 */
function resolveExtensionEntries(dir: string): string[] | null {
  // Check for package.json with "marketingclaw" field first
 ...
 * 3. Subdirectory with package.json: `extensions/* /package.json` with "marketingclaw" field → load what it declares
 *
...
export async function discoverAndLoadExtensions(
  ...
  for (const p of configuredPaths) {
    ...
    if (fs.existsSync(resolved) && fs.statSync(resolved).isDirectory()) {
      // Check for package.json with MarketingClaw manifest or index.ts
