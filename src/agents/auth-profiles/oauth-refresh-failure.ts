...
export function buildOAuthRefreshFailureLoginCommand(
  ...
  if (sanitizedProvider === "claude-cli") {
    ...
    // first, then re-register that auth method with MarketingClaw.
    const claudeLoginCommand = formatCliCommand("claude auth login");
    const marketingclawLoginCommand = formatCliCommand(
      sanitizedProfileId
        ? `marketingclaw models auth login --provider anthropic --method cli --profile-id ${quoteShellArg(sanitizedProfileId)}`
        : "marketingclaw models auth login --provider anthropic --method cli",
    );
    return `${claudeLoginCommand} && ${marketingclawLoginCommand}`;
  }
  return sanitizedProvider
    ? formatCliCommand(
        sanitizedProfileId
          ? `marketingclaw models auth login --provider ${sanitizedProvider} --profile-id ${quoteShellArg(sanitizedProfileId)}`
          : `marketingclaw models auth login --provider ${sanitizedProvider}`,
      )
    : formatCliCommand("marketingclaw models auth login");
}
