...
import { createMarketingClawCodingTools } from "../../../src/plugin-sdk/agent-harness.js";
...
const WORKSPACE_DIR = "/tmp/marketingclaw-happy-path/workspace";
const AGENT_DIR = "/tmp/marketingclaw-happy-path/agent";
const SESSION_FILE = "/tmp/marketingclaw-happy-path/session.jsonl";
...
const CODEX_WORKSPACE_BOOTSTRAP_PROMPT_CONTEXT = [
  "MarketingClaw loaded these user-editable workspace files for the current turn. Codex loads AGENTS.md natively. TOOLS.md is provided as inherited Codex developer instructions. SOUL.md, IDENTITY.md, and USER.md are provided as turn-scoped collaboration instructions so native Codex subagents do not inherit them. HEARTBEAT.md is handled by heartbeat collaboration-mode guidance. Those files are not repeated here.",
  "",
  "# Project Context",
  "",
...
const CODEX_WORKSPACE_THREAD_DEVELOPER_INSTRUCTIONS = [
  ...
  "",
  "MarketingClaw loaded these workspace instruction files from the active agent workspace. Internalize and follow them accordingly.",
  "",
...
const CODEX_WORKSPACE_TURN_SCOPED_DEVELOPER_INSTRUCTIONS = [
  "## MarketingClaw Agent Soul",
  "",
  "MarketingClaw loaded these workspace instruction files from the active agent workspace. They are the canonical definitions of who you are, how you think and work, and the human you work alongside. Internalize and follow them accordingly.",
  "",
...
const CODEX_HEARTBEAT_COLLABORATION_INSTRUCTIONS = [
  "## MarketingClaw Heartbeat Workspace",
  "",
...
const dynamicToolsConfig: MarketingClawConfig = {
  ...baseConfig,
  plugins: {
    enabled: true,
    slots: {
...
function createScenarios(codexApi: CodexPromptSnapshotApi): PromptScenario[] {
  ...
  const discordGroupCtx: TemplateContext = {
    ...
    SenderName: "Pash",
    SenderUsername: "pash",
    GroupSubject: "MarketingClaw maintainers",
    GroupChannel: "#agent-sandbox",
    GroupSpace: "MarketingClaw",
    ConversationLabel: "MarketingClaw/#agent-sandbox",
    WasMentioned: true,
    InboundHistory: [
      {
        sender: "Peter",
      ...
      {
        sender: "Pash",
        body: "@MarketingClaw please verify the Codex happy path too.",
      },
    ],
    Body: "@MarketingClaw can you audit whether this prompt path has conflicting silence instructions?",
  ...
  return [
    {
      ...
      notes: [
        ...
        "A quiet turn is represented by not calling `message(action=send)`; the normal final assistant text is private to MarketingClaw/Codex.",
      ],
      trigger: "user",
      ctx: telegramDirectCtx,
      prompt: createPrompt(
...
function renderModelBoundPromptLayers(params: {
  ...
  const codexConfigInstructions =
    ...
    typeof params.codexSnapshot.threadStartParams.config.instructions === "string"
      ...
      : "";
  const marketingClawDeveloperInstructions = params.codexSnapshot.developerInstructions;
  ...
  return [
    ...
    "",
    "This is the deterministic model-bound layer stack MarketingClaw can snapshot for the Codex happy path. It uses a pinned Codex `gpt-5.5` prompt fixture generated from Codex's model catalog/cache shape, then adds the Codex permission developer text, Codex thread config instructions when present, MarketingClaw developer instructions, turn-scoped collaboration-mode instructions when MarketingClaw provides them, turn input with MarketingClaw runtime context, and the MarketingClaw dynamic tool catalog. Codex can still add runtime-owned context such as native workspace `AGENTS.md`, environment context, memories, app/plugin instructions, and built-in collaboration-mode instructions inside the Codex runtime.",
    "",
    "### Layer Metadata",
    "",
    markdownFence(
    ...
    markdownFence(
      ...
      stableJson({
        ...
        marketingClawDeveloperInstructions: textStats(marketingClawDeveloperInstructions),
    ...
    "",
    "### Developer: MarketingClaw Runtime Instructions",
    "",
    markdownFence("text", marketingClawDeveloperInstructions),
    "",
...
function buildCodexMarketingClawRuntimeContext(): string {
  return [
    "MarketingClaw runtime context for this turn:",
    "Treat this MarketingClaw-provided context as supporting project/user reference for the current request.",
    "",
    "## MarketingClaw Workspace Context",
    "",
    CODEX_WORKSPACE_BOOTSTRAP_PROMPT_CONTEXT,
  ].join("\n");
}

function prependCodexMarketingClawRuntimeContext(prompt: string): string {
  return [buildCodexMarketingClawRuntimeContext(), "", "Current user request:", prompt].join("\n");
}
...
function renderScenarioSnapshot(
  ...
  const codexTurnPromptText = prependCodexMarketingClawRuntimeContext(scenario.prompt);
  ...
  return [
    ...
    "- This captures the MarketingClaw-owned Codex app-server inputs and reconstructs the stable Codex model/permission layers from committed Codex prompt fixtures.",
    ...
    "",
    "## Effective MarketingClaw Config",
    "",
...
function renderReadme(scenarios: PromptScenario[]): string {
  return [
    ...
    "",
    "The Markdown files show selected app-server thread/turn params plus a reconstructed model-bound prompt layer stack: Codex `gpt-5.5` model instructions from a pinned Codex model catalog fixture, Codex permission developer instructions for the happy-path yolo profile, MarketingClaw developer instructions, turn input with simulated MarketingClaw workspace bootstrap runtime context, heartbeat collaboration-mode guidance when applicable, and references to the complete dynamic tool catalog.",
    "",
    "The workspace bootstrap simulation includes dummy workspace contents so prompt reviewers can see how MarketingClaw routes stable profile files into Codex developer instructions, keeps `MEMORY.md` in turn input, and points heartbeat turns at `HEARTBEAT.md` without inlining it. `AGENTS.md` is intentionally not repeated here because Codex loads it natively.",
    "",
    "The tool catalog is pinned to the canonical happy-path MarketingClaw tools so optional locally installed plugin tools do not create fixture churn.",
    "",
    ...
    "",
    "These snapshots are still not a byte-for-byte raw OpenAI request capture. Codex-owned native `AGENTS.md`, environment context, memories, app/plugin instructions, and built-in collaboration-mode instructions can be added inside the Codex runtime after MarketingClaw sends thread and turn params.",
    "",
    "Regenerate with:",
    "",
