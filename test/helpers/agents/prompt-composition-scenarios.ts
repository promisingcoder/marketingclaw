...
async function createToolRichScenario(workspaceDir: string): Promise<PromptScenario> {
  const skillsPrompt = [
    ...
    "<skill><name>release</name><description>Release MarketingClaw safely.</description><location>/skills/release/SKILL.md</location></skill>",
    "</available_skills>",
  ].join("\n");
...
async function createWorkspaceWithPromptCompositionFiles(): Promise<string> {
  const workspaceDir = await makeTempWorkspace("marketingclaw-prompt-cache-");
  await writeWorkspaceFile({
    dir: workspaceDir,
    name: "AGENTS.md",
    content: [
...
export async function createPromptCompositionScenarios(): Promise<{
  ...
  const warningWorkspaceDir = await makeTempWorkspace("marketingclaw-prompt-cache-warning-");
  const scenarios = [
