  ...
  return new Promise((resolve) => {
    const child = spawn(process.execPath, [clientPath], {
      env: {
        ...
        MARKETINGCLAW_OPENAI_CHAT_TOOLS_TIMEOUT_SECONDS: "1",
        PORT: String(port),
        ...env,
      },
...
function runWriteConfig(root: string, env: Record<string, string> = {}) {
  return spawnSync(process.execPath, [writeConfigPath], {
    ...
    env: {
      ...
      MARKETINGCLAW_GATEWAY_TOKEN: "test-token",
      MARKETINGCLAW_OPENAI_CHAT_TOOLS_MODEL: "openai/gpt-5.5",
      MARKETINGCLAW_STATE_DIR: path.join(root, "state"),
      MARKETINGCLAW_TEST_WORKSPACE_DIR: path.join(root, "workspace"),
      PORT: "18789",
      ...env,
    },
  });
...
function runDockerRunnerAuthPreflight(root: string, env: Record<string, string> = {}) {
  return spawnSync("bash", [dockerRunnerPath], {
    ...
    env: {
      ...
      OPENAI_API_KEY: "",
      OPENAI_BASE_URL: "",
      MARKETINGCLAW_OPENAI_CHAT_TOOLS_PROFILE_FILE: path.join(root, "missing.profile"),
      ...env,
    },
  });
}
...
describe("scripts/e2e/lib/openai-chat-tools/client.mjs", () => {
  ...
  it("fails auth preflight before Docker build work starts", () => {
    const root = mkdtempSync(path.join(tmpdir(), "marketingclaw-openai-chat-tools-"));
    try {
  ...
  it("treats placeholder profile auth as missing before Docker build work starts", () => {
    ...
    try {
      ...
      const result = runDockerRunnerAuthPreflight(root, {
        MARKETINGCLAW_OPENAI_CHAT_TOOLS_PROFILE_FILE: profile,
      });
      const output = `${result.stdout}\n${result.stderr}`;
  ...
  it.each([
    ["timeout", "MARKETINGCLAW_OPENAI_CHAT_TOOLS_TIMEOUT_SECONDS", "1e3"],
    ["body limit", "MARKETINGCLAW_OPENAI_CHAT_TOOLS_MAX_BODY_BYTES", "64bytes"],
  ])(
    "rejects invalid Docker runner %s before auth or Docker build work starts",
    (_label, envName, value) => {
      const tempDirs: string[] = [];
      const root = makeTempDir(tempDirs, "marketingclaw-openai-chat-tools-");
      try {
  ...
  it("passes normalized timeout and body limits into the Docker runner", () => {
    ...
    expect(runner).toContain(
      "docker_e2e_read_positive_int_env MARKETINGCLAW_OPENAI_CHAT_TOOLS_TIMEOUT_SECONDS 180",
    );
    expect(runner).toContain(
      "docker_e2e_read_positive_int_env MARKETINGCLAW_OPENAI_CHAT_TOOLS_MAX_BODY_BYTES 1048576",
    );
    expect(runner).toContain(
      '-e "MARKETINGCLAW_OPENAI_CHAT_TOOLS_TIMEOUT_SECONDS=$TIMEOUT_SECONDS"',
    );
    expect(runner).toContain('-e "MARKETINGCLAW_OPENAI_CHAT_TOOLS_MAX_BODY_BYTES=$MAX_BODY_BYTES"');
  });

  it("rejects loose timeout env values instead of parsing numeric prefixes", async () => {
    const result = await runClient(1, {
      MARKETINGCLAW_OPENAI_CHAT_TOOLS_TIMEOUT_SECONDS: "1e3",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("invalid MARKETINGCLAW_OPENAI_CHAT_TOOLS_TIMEOUT_SECONDS: 1e3");
  });

  it("rejects loose body limit env values instead of parsing numeric prefixes", async () => {
    const result = await runClient(1, {
      MARKETINGCLAW_OPENAI_CHAT_TOOLS_MAX_BODY_BYTES: "64bytes",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain(
      "invalid MARKETINGCLAW_OPENAI_CHAT_TOOLS_MAX_BODY_BYTES: 64bytes",
    );
  });
  ...
  it("rejects loose write-config timeout env values", () => {
    ...
    try {
      ...
      expect(result.status).not.toBe(0);
      expect(result.stderr).toContain(
        "invalid MARKETINGCLAW_OPENAI_CHAT_TOOLS_TIMEOUT_SECONDS: 1e3",
      );
    } finally {
      rmSync(root, { force: true, recursive: true });
    }
  ...
  it("writes strict positive timeout and port values into generated config", () => {
    ...
    try {
      const result = runWriteConfig(root, {
        MARKETINGCLAW_OPENAI_CHAT_TOOLS_TIMEOUT_SECONDS: "240",
        PORT: "19001",
      });

      expect(result.status).toBe(0);
      const config = JSON.parse(readFileSync(path.join(root, "marketingclaw.json"), "utf8"));
  ...
  it("accepts a matching chat completions tool call response", async () => {
    const server = createServer((request, response) => {
      ...
      expect(request.headers["x-marketingclaw-model"]).toBe("openai/gpt-5.4-mini");
      response.writeHead(200, { "content-type": "application/json" });
  ...
  it("caps chat completion response bodies before JSON parsing", async () => {
    ...
    try {
      const result = await runClient(port, {
        MARKETINGCLAW_OPENAI_CHAT_TOOLS_MAX_BODY_BYTES: "64",
      });
