...
async function runScheduler(options: ProducerOptions, appendLog: (chunk: unknown) => void) {
  ...
  let packageTgz = process.env.MARKETINGCLAW_CURRENT_PACKAGE_TGZ;
  if (packageTgz) {
    const packageDir = path.join(dockerRunDir, "marketingclaw-package");
    const evidencePackageTgz = path.join(packageDir, "marketingclaw-current.tgz");
  ...
  return await new Promise<{
    ...
    const child = spawn(process.execPath, ["scripts/test-docker-all.mjs"], {
      ...
      env: {
        ...process.env,
        MARKETINGCLAW_DOCKER_ALL_BUILD: "1",
        MARKETINGCLAW_DOCKER_ALL_DRY_RUN: "0",
        MARKETINGCLAW_DOCKER_ALL_LANES: options.lane,
        MARKETINGCLAW_DOCKER_ALL_LOG_DIR: dockerRunDir,
        MARKETINGCLAW_DOCKER_ALL_PARALLELISM: "1",
        MARKETINGCLAW_DOCKER_ALL_PREFLIGHT: "1",
        MARKETINGCLAW_DOCKER_ALL_TIMINGS_FILE: path.join(dockerRunDir, "lane-timings.json"),
        MARKETINGCLAW_DOCKER_ARTIFACT_IDENTITY_PATH: identityPath,
        ...(packageTgz ? { MARKETINGCLAW_CURRENT_PACKAGE_TGZ: packageTgz } : {}),
      },
