  ...
  "update-migration": {
    env: (env) => ({
      MARKETINGCLAW_UPGRADE_SURVIVOR_BASELINE_SPEC:
        env.MARKETINGCLAW_UPGRADE_SURVIVOR_BASELINE_SPEC ?? "openclaw@2026.4.23",
      MARKETINGCLAW_UPGRADE_SURVIVOR_PUBLISHED_BASELINE: "1",
      MARKETINGCLAW_UPGRADE_SURVIVOR_SCENARIO:
        env.MARKETINGCLAW_UPGRADE_SURVIVOR_SCENARIO ?? "plugin-deps-cleanup",
    }),
    script: "scripts/e2e/upgrade-survivor-docker.sh",
  },
  "update-restart-auth": {
    env: (env) => ({
      MARKETINGCLAW_UPGRADE_SURVIVOR_DOCKER_RUN_TIMEOUT:
        env.MARKETINGCLAW_UPGRADE_SURVIVOR_DOCKER_RUN_TIMEOUT ?? "1500s",
      MARKETINGCLAW_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE: "auto-auth",
    }),
    script: "scripts/e2e/upgrade-survivor-docker.sh",
  },
  "upgrade-survivor": {
