...
const LIVE =
  isLiveTestEnabled() &&
  isTruthyEnvValue(process.env.MARKETINGCLAW_LIVE_INFER_CLI_TEST) &&
...
describeLive("image generation infer CLI live", () => {
  it("generates an image through marketingclaw infer", () => {
    const outputBase = path.join(os.tmpdir(), `marketingclaw-infer-image-${process.pid}.png`);
