...
describe("scripts/run-vitest", () => {
  ...
  it("restores the workspace node_modules link from a hydrated pnpm modules directory", () => {
    ...
    expect(
      resolveVitestCliEntry({
        baseDir: "/repo",
        env: { PNPM_CONFIG_MODULES_DIR: "/runner/marketingclaw-pnpm-node-modules" },
        fsImpl: {
          existsSync: (filePath: string) =>
            filePath.replaceAll("\\", "/") ===
            "/runner/marketingclaw-pnpm-node-modules/vitest/package.json",
    ...
    expect(symlinks).toEqual([
      {
        target: "/runner/marketingclaw-pnpm-node-modules",
        path: "/runner/marketingclaw-pnpm-node-modules/node_modules",
        type: "junction",
      },
      {
        target: "/runner/marketingclaw-pnpm-node-modules",
  ...
  it("self-links hydrated pnpm modules when pnpm lowercases the env key", () => {
    ...
    expect(
      resolveVitestCliEntry({
        baseDir: "/repo",
        env: { npm_config_modules_dir: "/runner/marketingclaw-pnpm-node-modules" },
        fsImpl: {
          existsSync: (filePath: string) =>
            filePath.replaceAll("\\", "/") ===
            "/runner/marketingclaw-pnpm-node-modules/vitest/package.json",
        ...
        platform: "win32",
        requireResolve: () => "/runner/marketingclaw-pnpm-node-modules/vitest/package.json",
      }),
    ).toBe("/repo/node_modules/vitest/vitest.mjs");
    expect(symlinks).toEqual([
      {
  ...
  it("keeps tooling-excluded explicit tests on existing routing", () => {
    const argv = ["run", "test/scripts/marketingclaw-e2e-instance.test.ts"];
    expect(resolveImplicitVitestArgs(argv)).toBe(argv);
  });
  ...
  it("allows opting back into Maglev explicitly", () => {
    expect(
      resolveVitestNodeArgs({
        MARKETINGCLAW_VITEST_ENABLE_MAGLEV: "1",
        PATH: "/usr/bin",
      }),
    ).toStrictEqual([]);
  });
  ...
  it("parses the optional no-output timeout env", () => {
    ...
    expect(
      resolveVitestNoOutputTimeoutMs({ MARKETINGCLAW_VITEST_NO_OUTPUT_TIMEOUT_MS: "2500" }),
    ).toBe(2500);
    expect(
      resolveVitestNoOutputTimeoutMs({ MARKETINGCLAW_VITEST_NO_OUTPUT_TIMEOUT_MS: "0" }),
    ).toBeNull();
    expect(
      resolveVitestNoOutputTimeoutMs({ MARKETINGCLAW_VITEST_NO_OUTPUT_TIMEOUT_MS: "1e3" }),
    ).toBeNull();
    expect(
      resolveVitestNoOutputTimeoutMs({ MARKETINGCLAW_VITEST_NO_OUTPUT_TIMEOUT_MS: "2500ms" }),
    ).toBeNull();
  });
  ...
  it("defaults direct non-watch runs to the stall watchdog", () => {
    expect(resolveRunVitestSpawnEnv({ PATH: "/usr/bin" }, ["run"])).toEqual({
      PATH: "/usr/bin",
      MARKETINGCLAW_VITEST_NO_OUTPUT_HEARTBEAT_MS: "30000",
      MARKETINGCLAW_VITEST_NO_OUTPUT_TIMEOUT_MS: "120000",
    });
    ...
    expect(
      resolveRunVitestSpawnEnv(
        { MARKETINGCLAW_VITEST_NO_OUTPUT_TIMEOUT_MS: "0", PATH: "/usr/bin" },
        ["run"],
      ),
    ).toEqual({
  ...
  it("uses a longer default stall watchdog for broad e2e and project shard configs", () => {
    ...
    for (const configArg of [
      ...
      expect(resolveRunVitestSpawnEnv({ PATH: "/usr/bin" }, ["run", configArg])).toEqual({
        PATH: "/usr/bin",
        MARKETINGCLAW_VITEST_NO_OUTPUT_HEARTBEAT_MS: "30000",
        MARKETINGCLAW_VITEST_NO_OUTPUT_TIMEOUT_MS: timeout,
      });
    }
    for (const configArg of [
      ...
      expect(resolveRunVitestSpawnEnv({ PATH: "/usr/bin" }, ["run", configArg])).toEqual({
        PATH: "/usr/bin",
        MARKETINGCLAW_VITEST_NO_OUTPUT_HEARTBEAT_MS: "30000",
        MARKETINGCLAW_VITEST_NO_OUTPUT_TIMEOUT_MS: extraLongTimeout,
      });
    }
    expect(
      resolveDefaultVitestNoOutputTimeoutMs([
  ...
  it("does not force the stall watchdog into delegated runner environments", () => {
    ...
    expect(
      resolveTestProjectsRunnerEnv({
        MARKETINGCLAW_VITEST_NO_OUTPUT_TIMEOUT_MS: "2500",
        PATH: "/usr/bin",
      }),
    ).toEqual({
      MARKETINGCLAW_VITEST_NO_OUTPUT_TIMEOUT_MS: "2500",
  ...
  posixIt("cleans delegated test-project children when the wrapper is signaled", async () => {
    const fixturePath = nodePath.join(
      os.tmpdir(),
      `marketingclaw-run-vitest-delegated-signal-${process.pid}-${Date.now()}.mjs`,
    );
    const childPidPath = nodePath.join(
      os.tmpdir(),
      `marketingclaw-run-vitest-delegated-child-${process.pid}-${Date.now()}.pid`,
    );
    const descendantPidPath = nodePath.join(
      os.tmpdir(),
      `marketingclaw-run-vitest-delegated-descendant-${process.pid}-${Date.now()}.pid`,
    );
    ...
    fs.writeFileSync(
      ...
      [
        ...
        "fs.writeFileSync(process.env.MARKETINGCLAW_DELEGATED_SIGNAL_CHILD_PID, String(process.pid));",
        "fs.writeFileSync(process.env.MARKETINGCLAW_DELEGATED_SIGNAL_DESCENDANT_PID, String(child.pid));",
        "await new Promise(() => {});",
        "",
      ].join("\n"),
    );
    ...
    const runner = spawn(
      ...
      ],
      {
        env: {
          ...process.env,
          MARKETINGCLAW_DELEGATED_SIGNAL_CHILD_PID: childPidPath,
          MARKETINGCLAW_DELEGATED_SIGNAL_DESCENDANT_PID: descendantPidPath,
        },
        stdio: "ignore",
      },
    );
  ...
  posixIt("terminates a silent Vitest child through the watchdog", async () => {
    const watched = spawnWatchedVitestProcess({
      ...
      env: { MARKETINGCLAW_VITEST_NO_OUTPUT_TIMEOUT_MS: "100" },
    });
  ...
  it("caps native Rust worker pools for serial Vitest runs", () => {
    expect(
      resolveVitestSpawnParams(
        {
          MARKETINGCLAW_TEST_PROJECTS_SERIAL: "1",
          PATH: "/usr/bin",
        },
        "darwin",
      ).env,
  ...
  it("does not truncate malformed native worker budgets", () => {
    expect(
      resolveVitestSpawnParams(
        {
          MARKETINGCLAW_TEST_PROJECTS_SERIAL: "1",
          MARKETINGCLAW_VITEST_MAX_WORKERS: "8x",
          PATH: "/usr/bin",
        },
        "darwin",
      ).env,
  ...
  it("parses the optional watchdog heartbeat interval", () => {
    expect(
      resolveVitestNoOutputHeartbeatMs({ MARKETINGCLAW_VITEST_NO_OUTPUT_HEARTBEAT_MS: "120000" }),
    ).toBe(120000);
    expect(
      resolveVitestNoOutputHeartbeatMs({ MARKETINGCLAW_VITEST_NO_OUTPUT_HEARTBEAT_MS: "0" }),
    ).toBeNull();
  });
});
