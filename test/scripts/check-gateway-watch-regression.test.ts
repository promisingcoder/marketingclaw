...
describe("check-gateway-watch-regression", () => {
  ...
  it("refreshes runtime postbuild stamps after build stamps", () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-gateway-watch-stamps-"));
    try {
  ...
  it("removes the isolated watch home after spawn failures", async () => {
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-gateway-watch-output-"));
    const child = new EventEmitter() as EventEmitter & {
      stderr: EventEmitter;
      stdout: EventEmitter;
    };
    ...
    try {
      ...
      expect(spawn.mock.calls[0]?.[2]?.env?.MARKETINGCLAW_RUNTIME_POSTBUILD_STATIC_ASSETS).toBe(
        "0",
      );
