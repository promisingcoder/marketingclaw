...
const scrubbedEnvKeys = [
  ...
  "MOCK_PORT",
  "MOCK_REQUEST_LOG",
  "MARKETINGCLAW_CONFIG_RELOAD_LOG_MAX_READ_BYTES",
  "MARKETINGCLAW_CONFIG_RELOAD_LOG_PATH",
  "MARKETINGCLAW_CONFIG_RELOAD_LOG_TIMEOUT_MS",
  "MARKETINGCLAW_MOCK_OPENAI_PORT",
  "RAW_SCHEMA_ERROR",
  "SUCCESS_MARKER",
];
...
describe("e2e mock and config helper numeric limits", () => {
  it("rejects loose mock OpenAI port env values", () => {
    ...
    const fallbackPort = runScript(mockOpenAiPath, {
      MARKETINGCLAW_MOCK_OPENAI_PORT: "44080http",
    });
    expect(fallbackPort.status).not.toBe(0);
    expect(fallbackPort.stderr).toContain("invalid MARKETINGCLAW_MOCK_OPENAI_PORT: 44080http");
  });
  ...
  it("rejects out-of-range mock OpenAI port env values", () => {
    ...
    const fallbackPort = runScript(mockOpenAiPath, {
      MARKETINGCLAW_MOCK_OPENAI_PORT: "65536",
    });
    expect(fallbackPort.status).not.toBe(0);
    expect(fallbackPort.stderr).toContain("invalid MARKETINGCLAW_MOCK_OPENAI_PORT: 65536");
  });
  ...
  it("rejects loose config-reload log timeout env values", () => {
    const result = runScript(configReloadAssertPath, {
      MARKETINGCLAW_CONFIG_RELOAD_LOG_TIMEOUT_MS: "30000ms",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("invalid MARKETINGCLAW_CONFIG_RELOAD_LOG_TIMEOUT_MS: 30000ms");
  });

  it("rejects loose config-reload log read caps", () => {
    const result = runScript(configReloadAssertPath, {
      MARKETINGCLAW_CONFIG_RELOAD_LOG_MAX_READ_BYTES: "256kb",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain(
      "invalid MARKETINGCLAW_CONFIG_RELOAD_LOG_MAX_READ_BYTES: 256kb",
    );
  });

  it("returns a clear error when mock OpenAI cannot append request logs", async () => {
    const requestLogDirectory = await mkdtemp(join(tmpdir(), "marketingclaw-mock-request-log-"));
    try {
      await withMockServer(
        ...
        async (baseUrl, output) => {
          const response = await fetch(`${baseUrl}/v1/responses`, {
            ...
            body: JSON.stringify({ input: "MARKETINGCLAW_E2E_OK" }),
          });
  ...
  it("returns a clear error when web-search mock cannot append request logs", async () => {
    const requestLogDirectory = await mkdtemp(join(tmpdir(), "marketingclaw-web-search-log-"));
    try {
      await withMockServer(
        webSearchMockPath,
        {
          ...
          SUCCESS_MARKER: "MARKETINGCLAW_SCHEMA_E2E_OK",
        },
        async (baseUrl, output) => {
          const response = await fetch(`${baseUrl}/v1/responses`, {
            method: "POST",
            headers: { "content-type": "application/json" },
            body: JSON.stringify({
              input: "MARKETINGCLAW_SCHEMA_E2E_OK",
