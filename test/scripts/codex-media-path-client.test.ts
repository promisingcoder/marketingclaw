...
function makeTempRoot(): string {
  const root = mkdtempSync(path.join(tmpdir(), "marketingclaw-codex-media-path-"));
  tempRoots.push(root);
  return root;
}
...
describe("codex media path limits", () => {
  it("rejects loose numeric env values instead of parsing prefixes", () => {
    expect(() =>
      readPositiveIntEnv("MARKETINGCLAW_CODEX_MEDIA_PATH_TIMEOUT_SECONDS", 180, {
        MARKETINGCLAW_CODEX_MEDIA_PATH_TIMEOUT_SECONDS: "1e3",
      }),
    ).toThrow("invalid MARKETINGCLAW_CODEX_MEDIA_PATH_TIMEOUT_SECONDS: 1e3");
    expect(() =>
      readPositiveIntEnv("MARKETINGCLAW_CODEX_MEDIA_PATH_LOG_TAIL_MAX_BYTES", 2 * 1024 * 1024, {
        MARKETINGCLAW_CODEX_MEDIA_PATH_LOG_TAIL_MAX_BYTES: "64bytes",
      }),
    ).toThrow("invalid MARKETINGCLAW_CODEX_MEDIA_PATH_LOG_TAIL_MAX_BYTES: 64bytes");
  });
  ...
  it("writes strict positive timeout and port values into generated config", () => {
    ...
    const result = runWriteConfig(root, {
      MARKETINGCLAW_CODEX_MEDIA_PATH_TIMEOUT_SECONDS: "240",
      PORT: "19002",
    });
  ...
  it("rejects loose write-config timeout env values", () => {
    ...
    expect(result.stderr).toContain("invalid MARKETINGCLAW_CODEX_MEDIA_PATH_TIMEOUT_SECONDS: 1e3");
  });
...
describe("codex media path fake app-server", () => {
  it("returns a structured error when request logging fails", async () => {
    ...
    const child: ChildProcessWithoutNullStreams = spawn(process.execPath, [fakeAppServerPath], {
      env: {
        ...process.env,
        MARKETINGCLAW_CODEX_MEDIA_PATH_APP_SERVER_LOG: requestLogDirectory,
      },
