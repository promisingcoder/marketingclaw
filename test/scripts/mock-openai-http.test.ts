...
describe("mock OpenAI HTTP helpers", () => {
  ...
  it("rejects loose numeric env limits instead of parsing prefixes", () => {
    expect(() =>
      readMockOpenAiHttpLimits({
        MARKETINGCLAW_MOCK_OPENAI_REQUEST_MAX_BYTES: "1000ms",
      }),
    ).toThrow("invalid MARKETINGCLAW_MOCK_OPENAI_REQUEST_MAX_BYTES: 1000ms");
    expect(() =>
      readMockOpenAiHttpLimits({
        MARKETINGCLAW_MOCK_OPENAI_REQUEST_LOG_BODY_MAX_BYTES: "1e3",
      }),
    ).toThrow("invalid MARKETINGCLAW_MOCK_OPENAI_REQUEST_LOG_BODY_MAX_BYTES: 1e3");
  });
});
