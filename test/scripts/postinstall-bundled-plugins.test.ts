...
import {
  ...
  pruneMarketingClawCompileCache,
...
describe("bundled plugin postinstall", () => {
  ...
  it("recognizes direct invocation through symlinked temp prefixes", () => {
    ...
    expect(
      isDirectPostinstallInvocation({
        entryPath: "/var/folders/tmp/marketingclaw/scripts/postinstall-bundled-plugins.mjs",
        modulePath:
          "/private/var/folders/tmp/marketingclaw/scripts/postinstall-bundled-plugins.mjs",
        realpathSync,
      }),
    ).toBe(true);
  });
  ...
  it("prunes Node versioned compile cache dirs during package postinstall", () => {
    const configuredBase = path.join("/tmp", "marketingclaw-cache");
    ...
    const readdirSync = vi.fn((value: string) => {
      if (value === configuredBase) {
        return [
          ...
          { name: "marketingclaw", isDirectory: () => true },
    ...
    expect(removed).not.toContain(path.join(configuredBase, "marketingclaw"));
  ...
  it("keeps pruning sibling compile cache dirs after one removal fails", () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      "[postinstall] could not prune MarketingClaw compile cache: Error: locked",
    );
  });

  it("does not warn when compile-cache pruning hits EACCES or EPERM (shared caches)", () => {
    const base = path.join("/tmp", "marketingclaw-shared-compile-cache");
  ...
  it("does not warn when the compile-cache base directory cannot be listed (EACCES)", () => {
    const base = path.join("/tmp", "marketingclaw-compile-cache-no-list");
    const warn = vi.fn();
    const rmSync = vi.fn();
  ...
  it("patches the Baileys upload helper dispatcher guard", async () => {
    const packageRoot = await createTempDirAsync("marketingclaw-baileys-postinstall-");
  ...
  it("prunes source-checkout bundled plugin node_modules", async () => {
    const packageRoot = await createTempDirAsync("marketingclaw-source-checkout-");
  ...
  it("keeps source-checkout prune non-fatal", async () => {
    const packageRoot = await createTempDirAsync("marketingclaw-source-checkout-prune-error-");
    const extensionsDir = path.join(packageRoot, "extensions");
    await fs.mkdir(path.join(packageRoot, ".git"), { recursive: true });
    await fs.mkdir(path.join(packageRoot, "src"), { recursive: true });
  ...
  it("does not prune user-state legacy runtime deps during source-checkout postinstall", async () => {
    const packageRoot = await createTempDirAsync("marketingclaw-source-checkout-state-skip-");
    const home = await createTempDirAsync("marketingclaw-source-checkout-home-");
    const legacyRuntimeRoot = path.join(home, ".marketingclaw", "plugin-runtime-deps");
    await fs.mkdir(path.join(packageRoot, ".git"), { recursive: true });
    await fs.mkdir(path.join(packageRoot, "src"), { recursive: true });
  ...
  it("honors disable env before source-checkout pruning", async () => {
    const packageRoot = await createTempDirAsync("marketingclaw-source-checkout-disabled-");
    const extensionsDir = path.join(packageRoot, "extensions");
    await fs.mkdir(path.join(packageRoot, ".git"), { recursive: true });
    await fs.mkdir(path.join(packageRoot, "src"), { recursive: true });
    ...
    runBundledPluginPostinstall({
      env: { MARKETINGCLAW_DISABLE_BUNDLED_PLUGIN_POSTINSTALL: "1" },
      packageRoot,
      log: { log: vi.fn(), warn: vi.fn() },
    });
  ...
  it("migrates the plugin registry during postinstall from built dist contracts", async () => {
    const packageRoot = await createTempDirAsync("marketingclaw-postinstall-registry-");
    ...
    const result = await runPluginRegistryPostinstallMigration({
      ...
      importModule,
      env: { MARKETINGCLAW_HOME: "/tmp/home" },
      log,
    });
  ...
  it("surfaces deprecated plugin registry migration break-glass warnings", async () => {
    ...
    const migratePluginRegistryForInstall = vi.fn(async () => ({
      ...
      migrated: false,
      preflight: {
        deprecationWarnings: ["MARKETINGCLAW_FORCE_PLUGIN_REGISTRY_MIGRATION is deprecated"],
      },
    }));
    ...
    expect(warn).toHaveBeenCalledWith(
      "[postinstall] MARKETINGCLAW_FORCE_PLUGIN_REGISTRY_MIGRATION is deprecated",
    );
  });
  ...
  it("honors plugin registry postinstall migration disable env", async () => {
    ...
    await expect(
      runPluginRegistryPostinstallMigration({
        packageRoot: "/pkg",
        env: { MARKETINGCLAW_DISABLE_PLUGIN_REGISTRY_MIGRATION: "1" },
        existsSync: vi.fn(() => true),
        importModule,
        log: { log: vi.fn(), warn: vi.fn() },
      }),
  ...
  it("does not disable plugin registry migration for falsey env flag strings", async () => {
    ...
    await expect(
      runPluginRegistryPostinstallMigration({
        packageRoot: "/pkg",
        env: { MARKETINGCLAW_DISABLE_PLUGIN_REGISTRY_MIGRATION: "0" },
        existsSync: vi.fn(() => true),
        importModule,
        log: { log: vi.fn(), warn: vi.fn() },
      }),
  ...
  it("prunes stale dist files from packaged installs", async () => {
    const packageRoot = await createTempDirAsync("marketingclaw-packaged-install-");
  ...
  it("omits unpacked plugin-sdk test helpers from the package dist inventory", async () => {
    const packageRoot = await createTempDirAsync("marketingclaw-packaged-inventory-");
  ...
  it("prunes legacy plugin runtime deps state during packaged postinstall", async () => {
    const prefix = await createTempDirAsync("marketingclaw-packaged-prefix-");
    const packageRoot = path.join(prefix, "lib", "node_modules", "marketingclaw");
    const nodeModulesRoot = path.dirname(packageRoot);
    const home = await createTempDirAsync("marketingclaw-packaged-home-");
    const stateOverride = path.join(home, "custom-state");
    const systemState = path.join(home, "system-state");
    const defaultLegacyRoot = path.join(home, ".marketingclaw", "plugin-runtime-deps");
    ...
    runBundledPluginPostinstall({
      env: {
        HOME: home,
        MARKETINGCLAW_STATE_DIR: stateOverride,
  ...
  it("keeps legacy plugin runtime deps cleanup non-fatal", () => {
    ...
    expect(warn).toHaveBeenNthCalledWith(
      2,
      "[postinstall] could not prune legacy plugin runtime deps /home/alice/.marketingclaw/plugin-runtime-deps: Error: locked",
    );
  });

  it("resolves legacy plugin runtime deps roots from MarketingClaw state env", () => {
    expect(
      collectLegacyPluginRuntimeDepsStateRoots({
        env: {
          HOME: "/users/alice",
          MARKETINGCLAW_HOME: "/srv/marketingclaw-home",
          MARKETINGCLAW_CONFIG_PATH: "~/profile/marketingclaw.json",
          MARKETINGCLAW_STATE_DIR: "~/state",
          STATE_DIRECTORY: "/var/lib/marketingclaw",
        },
        homedir: () => "/users/alice",
      }),
    ).toEqual([
      "/srv/marketingclaw-home/.clawdbot/plugin-runtime-deps",
      "/srv/marketingclaw-home/.marketingclaw/plugin-runtime-deps",
      "/srv/marketingclaw-home/profile/plugin-runtime-deps",
      "/srv/marketingclaw-home/state/plugin-runtime-deps",
      "/var/lib/marketingclaw/plugin-runtime-deps",
    ]);
  });

  it("keeps imported dist chunks even when inventory is stale", async () => {
    const packageRoot = await createTempDirAsync("marketingclaw-packaged-install-import-");
  ...
  it("does not abort dist pruning when a listed chunk disappears before import expansion", async () => {
    const packageRoot = await createTempDirAsync("marketingclaw-packaged-install-missing-chunk-");
  ...
  it("prunes stale private QA files without restoring compat sidecars", async () => {
    const packageRoot = await createTempDirAsync("marketingclaw-packaged-install-qa-compat-");
    const currentFile = path.join(packageRoot, "dist", "entry.js");
    ...
    await expectPathMissing(
      path.join(packageRoot, "dist", "extensions", "qa-channel", "marketingclaw.plugin.json"),
    );
    await expectPathMissing(
  ...
  it("keeps packaged postinstall non-fatal when the dist inventory is missing", async () => {
    const packageRoot = await createTempDirAsync(
      "marketingclaw-packaged-install-missing-inventory-",
    );
    const staleFile = path.join(packageRoot, "dist", "channel-CJUAgRQR.js");
  ...
  it("keeps packaged postinstall non-fatal when the dist inventory is invalid", async () => {
    const packageRoot = await createTempDirAsync(
      "marketingclaw-packaged-install-invalid-inventory-",
    );
    const currentFile = path.join(packageRoot, "dist", "channel-BOa4MfoC.js");
  ...
  it("rejects packaged dist scans that exceed the filesystem entry limit", () => {
    ...
    // several-x headroom fails `npm install -g marketingclaw` for upgrading users.
  ...
  it("prunes sibling empty dist directories after closing parent scans", async () => {
    const packageRoot = await createTempDirAsync("marketingclaw-packaged-install-empty-dirs-");
  ...
  it("prunes stale bundled plugin dependency debris from packaged dist", async () => {
    const packageRoot = await createTempDirAsync("marketingclaw-packaged-install-dist-prune-");
