...
describe("release-beta-smoke", () => {
  ...
  it("rejects malformed positive integer environment limits", () => {
    expect(readPositiveInt(undefined, 60, "MARKETINGCLAW_RELEASE_BETA_SMOKE_COMMAND_MS")).toBe(60);
    expect(readPositiveInt("", 60, "MARKETINGCLAW_RELEASE_BETA_SMOKE_COMMAND_MS")).toBe(60);
    expect(readPositiveInt("25", 60, "MARKETINGCLAW_RELEASE_BETA_SMOKE_COMMAND_MS")).toBe(25);

    for (const raw of ["1e3", "25ms", "1.5", "0", "-1", String(Number.MAX_SAFE_INTEGER + 1)]) {
      expect(() => readPositiveInt(raw, 60, "MARKETINGCLAW_RELEASE_BETA_SMOKE_COMMAND_MS")).toThrow(
        "MARKETINGCLAW_RELEASE_BETA_SMOKE_COMMAND_MS must be a positive integer",
      );
    }
  });
  ...
  it("parses workflow run urls when gh includes them in dispatch output", () => {
    expect(
      parseWorkflowRunIdFromOutput(
        "Dispatched: https://github.com/promisingcoder/marketingclaw/actions/runs/1234567890",
      ),
    ).toBe("1234567890");
  });
  ...
  it("replaces stale Telegram proof placeholders", () => {
    const body = [
      ...
      "",
      "- npm package: https://www.npmjs.com/package/marketingclaw/v/2026.5.20-beta.1",
    ...
    const merged = mergeTelegramProofIntoReleaseBody(
      body,
      "- npm Telegram beta E2E: https://github.com/promisingcoder/marketingclaw/actions/runs/123",
    );
