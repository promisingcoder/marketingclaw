...
describe("scripts/test-live-models-docker.sh", () => {
  it("validates optional live model limits before auth or Docker setup", () => {
    ...
    expect(script).toContain('LIVE_MAX_MODELS="${MARKETINGCLAW_LIVE_MAX_MODELS:-}"');
    expect(script).toContain('[[ -n "$LIVE_MAX_MODELS" && ! "$LIVE_MAX_MODELS" =~ ^\\+?[0-9]+$ ]]');
    expect(script).toContain(
      'marketingclaw_live_read_positive_int_env MARKETINGCLAW_LIVE_MODEL_TIMEOUT_MS "$LIVE_MODEL_TIMEOUT_MS"',
    );
    expect(script).toContain('-e MARKETINGCLAW_LIVE_MAX_MODELS="$LIVE_MAX_MODELS"');
    expect(script).toContain('-e MARKETINGCLAW_LIVE_MODEL_TIMEOUT_MS="$LIVE_MODEL_TIMEOUT_MS"');
  });

  it.each([
    ["max models", "MARKETINGCLAW_LIVE_MAX_MODELS", "3models"],
    ["model timeout", "MARKETINGCLAW_LIVE_MODEL_TIMEOUT_MS", "45s"],
