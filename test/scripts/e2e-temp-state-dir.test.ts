...
describe("E2E temp state dirs", () => {
  it("cleans generated state dirs", async () => {
    const state = await createE2eStateDir("marketingclaw-e2e-temp-state-test-", {
  ...
  it("leaves caller-provided state dirs alone", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "marketingclaw-e2e-temp-state-existing-"));
    try {
      const state = await createE2eStateDir("marketingclaw-e2e-temp-state-test-", {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const root = mkdtempSync(path.join(tmpdir(), "marketingclaw-e2e-temp-state-retry-"));
  ...
  it("cleans generated state dirs on termination signals", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "marketingclaw-e2e-temp-state-signal-"));
    try {
...
const state = await createE2eStateDir("marketingclaw-e2e-temp-state-signal-", {
  MARKETINGCLAW_STATE_DIR: "",
});
