...
describe("Telegram Bot API helper", () => {
  ...
  it("rejects loose numeric env limits instead of parsing prefixes", () => {
    expect(() =>
      readTelegramBotApiLimits({
        MARKETINGCLAW_TELEGRAM_USER_BOT_API_TIMEOUT_MS: "1e3",
      }),
    ).toThrow("invalid MARKETINGCLAW_TELEGRAM_USER_BOT_API_TIMEOUT_MS: 1e3");
    expect(() =>
      readTelegramBotApiLimits({
        MARKETINGCLAW_TELEGRAM_USER_BOT_API_BODY_MAX_BYTES: "1000ms",
      }),
    ).toThrow("invalid MARKETINGCLAW_TELEGRAM_USER_BOT_API_BODY_MAX_BYTES: 1000ms");
    expect(
      readTelegramBotApiLimits({
        MARKETINGCLAW_TELEGRAM_USER_BOT_API_BODY_MAX_BYTES: "2048",
        MARKETINGCLAW_TELEGRAM_USER_BOT_API_TIMEOUT_MS: "15000",
      }),
    ).toEqual({
      bodyMaxBytes: 2048,
      timeoutMs: 15000,
  ...
  it("returns successful Bot API results", async () => {
    const fetchImpl = vi.fn().mockResolvedValue(
      new Response(JSON.stringify({ ok: true, result: { username: "MarketingClawBot" } }), {
        status: 200,
      }),
    );
    ...
    ).resolves.toEqual({ username: "MarketingClawBot" });
