...
describe("release media memory scenario", () => {
  it("fails when packaged plugin listing is broken or omits memory-core", () => {
    ...
    const listIndex = script.indexOf('marketingclaw plugins list --json >"$PLUGINS_JSON"');
  ...
  it("uses a per-run temp root for generated media artifacts", () => {
    ...
    expect(script).not.toContain("/tmp/marketingclaw-release-media-memory-plugins.json");
    expect(script).not.toContain(
      'mkdir -p "$MARKETINGCLAW_STATE_DIR/workspace/memory" /tmp/marketingclaw-release-media-memory',
    );
  });
});
