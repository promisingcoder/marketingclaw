...
export function writeAndroidFixture(params: {
  ...
  const rootDir = makeTempDir(tempDirs, params.prefix ?? "marketingclaw-android-version-");
  ...
  const releaseNotes =
    "MarketingClaw is now available on Android.\n\nConnect to your MarketingClaw Gateway.\n";
  fs.writeFileSync(
    path.join(rootDir, "apps", "android", "CHANGELOG.md"),
    params.changelog ?? `# MarketingClaw Android Changelog\n\n## Unreleased\n\n${releaseNotes}`,
    "utf8",
  );
  fs.writeFileSync(
