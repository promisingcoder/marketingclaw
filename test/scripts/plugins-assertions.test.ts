...
function writeFixtureServerShims(binDir: string, pidPath: string): void {
  ...
  writeFileSync(
    ...
    [
      "#!/bin/bash",
      'printf "%s\\n" "$$" >"$MARKETINGCLAW_TEST_FIXTURE_SERVER_PID"',
      "trap 'exit 0' TERM",
  ...
  it("rejects loose ClawHub preflight limits instead of parsing prefixes", () => {
    const timeoutResult = spawnSync(process.execPath, [ASSERTIONS_SCRIPT, "clawhub-preflight"], {
      ...
      env: {
        ...
        MARKETINGCLAW_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS: "1e3",
      },
    });
    expect(timeoutResult.status).not.toBe(0);
    expect(timeoutResult.stderr).toContain(
      "invalid MARKETINGCLAW_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS: 1e3",
    );
    ...
    const bodyLimitResult = spawnSync(process.execPath, [ASSERTIONS_SCRIPT, "clawhub-preflight"], {
      ...
      env: {
        ...process.env,
        CLAWHUB_PLUGIN_SPEC: "clawhub:@openclaw/kitchen-sink",
        MARKETINGCLAW_PLUGINS_E2E_CLAWHUB_PREFLIGHT_BODY_MAX_BYTES: "1000bytes",
      },
    });
    expect(bodyLimitResult.status).not.toBe(0);
    expect(bodyLimitResult.stderr).toContain(
      "invalid MARKETINGCLAW_PLUGINS_E2E_CLAWHUB_PREFLIGHT_BODY_MAX_BYTES: 1000bytes",
    );
  });
  ...
  it("keeps sweep artifact paths aligned with the assertion scratch root", () => {
    ...
    for (const scriptPath of scripts) {
      ...
      const scriptWithoutDefaultScratch = script.replace(
        'mktemp -d "/tmp/marketingclaw-plugins.XXXXXX"',
        "",
      );
      expect(script).toContain("MARKETINGCLAW_PLUGINS_TMP_DIR");
      expect(scriptWithoutDefaultScratch).not.toMatch(
        /\/tmp\/(?:plugins|marketplace|demo-plugin|is-number|marketingclaw-plugin|marketingclaw-clawhub)/,
      );
    }
  });

  it("cleans the default plugin sweep scratch root", () => {
    const root = mkdtempSync(path.join(tmpdir(), "marketingclaw-plugin-sweep-cleanup-"));
    const marker = path.join(root, "scratch-path.txt");
    try {
      const result = runPluginsSweepShell(
        `
set -euo pipefail
export MARKETINGCLAW_PLUGINS_SWEEP_SOURCE_ONLY=1
source scripts/e2e/lib/plugins/sweep.sh
printf '%s\\n' "$MARKETINGCLAW_PLUGINS_TMP_DIR" > "$MARKER"
test -d "$MARKETINGCLAW_PLUGINS_TMP_DIR"
cleanup_marketingclaw_plugins_sweep
test ! -e "$MARKETINGCLAW_PLUGINS_TMP_DIR"
`,
        { MARKER: marker },
      );
      ...
      expect(scratchRoot).toContain("/tmp/marketingclaw-plugins.");
  ...
  it("preserves caller-provided plugin sweep scratch roots", () => {
    const root = mkdtempSync(path.join(tmpdir(), "marketingclaw-plugin-sweep-caller-"));
...
set -euo pipefail
export MARKETINGCLAW_PLUGINS_SWEEP_SOURCE_ONLY=1
export MARKETINGCLAW_PLUGINS_TMP_DIR="$SCRATCH_ROOT"
source scripts/e2e/lib/plugins/sweep.sh
test -d "$MARKETINGCLAW_PLUGINS_TMP_DIR"
cleanup_marketingclaw_plugins_sweep
test -d "$MARKETINGCLAW_PLUGINS_TMP_DIR"
`,
  ...
  it("scans plugin assertion logs without echoing whole files on failure", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "marketingclaw-plugin-update-log-"));
    try {
      ...
      const pass = await runAssertionAsync(["plugin-dir-update-skipped"], {
        MARKETINGCLAW_PLUGINS_TMP_DIR: passRoot,
      });
      ...
      const fail = await runAssertionAsync(["plugin-dir-update-skipped"], {
        MARKETINGCLAW_PLUGINS_TMP_DIR: failRoot,
      });
      ...
      writeFileSync(
        path.join(invalidRoot, "plugins-invalid-marketingclaw-extensions.log"),
        `marketingclaw.extensions[1]\n${"x".repeat(256 * 1024)}\nmissing validation tail`,
        "utf8",
      );
      writeJson(path.join(invalidRoot, "plugins-invalid-marketingclaw-extensions-list.json"), {
        plugins: [],
      });
      const invalid = await runAssertionAsync(["invalid-marketingclaw-extensions"], {
        HOME: invalidHome,
        MARKETINGCLAW_PLUGINS_TMP_DIR: invalidRoot,
      });
  ...
  it("cleans npm fixture registry children when readiness times out", () => {
    const root = mkdtempSync(path.join(tmpdir(), "marketingclaw-plugin-npm-fixture-cleanup-"));
    try {
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: {
            ...process.env,
            MARKETINGCLAW_TEST_FIXTURE_SERVER_PID: pidPath,
  ...
  it("force-kills stubborn npm fixture registry children during cleanup", () => {
    const root = mkdtempSync(path.join(tmpdir(), "marketingclaw-plugin-npm-fixture-kill-"));
    try {
      const binDir = path.join(root, "bin");
      const fixtureDir = path.join(root, "fixture");
      const pidPath = path.join(root, "server.pid");
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: {
            ...process.env,
            MARKETINGCLAW_PLUGINS_FIXTURE_STOP_ATTEMPTS: "2",
            MARKETINGCLAW_PLUGINS_FIXTURE_STOP_INTERVAL_SECONDS: "0.05",
            MARKETINGCLAW_TEST_FIXTURE_SERVER_PID: pidPath,
            PATH: `${binDir}${path.delimiter}/usr/bin${path.delimiter}/bin`,
          },
        },
    ...
    const result = spawnSync(
      ...
      [
        ...
        [
          "set -euo pipefail",
          "source scripts/e2e/lib/plugins/fixtures.sh",
          "marketingclaw_plugins_signal_fixture_process() { echo signal; }",
          "marketingclaw_plugins_fixture_process_alive() { echo probe; return 1; }",
          "set +e",
          "marketingclaw_plugins_stop_fixture_process 12345",
          'status="$?"',
          "set -e",
          'exit "$status"',
        ].join("\n"),
      ],
      {
        ...
        env: {
          ...process.env,
          MARKETINGCLAW_PLUGINS_FIXTURE_STOP_ATTEMPTS: "2x",
        },
      },
    );

    expect(result.status).toBe(2);
    expect(result.stderr).toContain("invalid MARKETINGCLAW_PLUGINS_FIXTURE_STOP_ATTEMPTS: 2x");
  ...
  it("rejects invalid fixture stop intervals before cleanup polling", () => {
    const result = spawnSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          MARKETINGCLAW_PLUGINS_FIXTURE_STOP_ATTEMPTS: "2",
          MARKETINGCLAW_PLUGINS_FIXTURE_STOP_INTERVAL_SECONDS: "soon",
        },
      },
    );
    ...
    expect(result.stderr).toContain(
      "invalid MARKETINGCLAW_PLUGINS_FIXTURE_STOP_INTERVAL_SECONDS: soon",
    );
    expect(result.stdout).not.toContain("signal");
    expect(result.stdout).not.toContain("probe");
  });
  ...
  it("bounds npm fixture registry logs when readiness fails", () => {
    const root = mkdtempSync(path.join(tmpdir(), "marketingclaw-plugin-npm-fixture-log-"));
    try {
      const binDir = path.join(root, "bin");
      const fixtureDir = path.join(root, "fixture");
      mkdirSync(fixtureDir);
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: {
            ...process.env,
            MARKETINGCLAW_DOCKER_E2E_LOG_PRINT_BYTES: "80",
            PATH: `${binDir}${path.delimiter}/usr/bin${path.delimiter}/bin`,
          },
        },
      );
  ...
  it("keeps npm fixture registry alive after malformed package paths", async () => {
    ...
    const root = makeTempDir(tempDirs, "marketingclaw-plugin-npm-fixture-request-");
    ...
    try {
      ...
      const valid = await requestFixtureRegistry(port, "/@marketingclaw%2Fdemo-plugin-npm");
  ...
  it("serves tarball dependencies using the request-visible registry origin", async () => {
    const tempDirs: string[] = [];
    const root = makeTempDir(tempDirs, "marketingclaw-plugin-npm-fixture-package-");
    const packageDir = path.join(root, "package");
    const portFile = path.join(root, "port");
    const tarballPath = path.join(root, "marketingclaw.tgz");
    ...
    try {
      const port = await waitForPortFile(portFile);
      const response = await requestFixtureRegistry(port, "/marketingclaw", {
        host: `192.0.2.2:${port}`,
      });
      ...
      expect(metadata.versions["2026.7.1-beta.3"].dist.tarball).toBe(
        `http://192.0.2.2:${port}/marketingclaw/-/marketingclaw.tgz`,
      );
    } finally {
      if (child.exitCode === null) {
        child.kill();
  ...
  it("recomputes proxied content length after fetch decodes the response", async () => {
    const tempDirs: string[] = [];
    const root = makeTempDir(tempDirs, "marketingclaw-plugin-npm-fixture-proxy-");
    const portFile = path.join(root, "port");
    const tarballPath = path.join(root, "demo-plugin.tgz");
    ...
    const child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          MARKETINGCLAW_NPM_REGISTRY_UPSTREAM: `http://127.0.0.1:${upstreamAddress.port}`,
        },
        stdio: ["ignore", "pipe", "pipe"],
      },
    );
  ...
  it("does not let absolute-form request targets escape the configured upstream", async () => {
    const tempDirs: string[] = [];
    const root = makeTempDir(tempDirs, "marketingclaw-plugin-npm-fixture-proxy-origin-");
    const portFile = path.join(root, "port");
    const tarballPath = path.join(root, "demo-plugin.tgz");
    ...
    const child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          MARKETINGCLAW_NPM_REGISTRY_UPSTREAM: `http://127.0.0.1:${configuredAddress.port}`,
        },
        stdio: ["ignore", "pipe", "pipe"],
      },
    );
  ...
  it("rejects invalid plugin fixture log byte limits before npm fixture setup", () => {
    const root = mkdtempSync(path.join(tmpdir(), "marketingclaw-plugin-npm-fixture-log-invalid-"));
    try {
      const binDir = path.join(root, "bin");
      const fixtureDir = path.join(root, "fixture");
      mkdirSync(binDir, { recursive: true });
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: {
            ...process.env,
            MARKETINGCLAW_DOCKER_E2E_LOG_PRINT_BYTES: "64kb",
            PATH: `${binDir}${path.delimiter}/usr/bin${path.delimiter}/bin`,
          },
        },
      );
      ...
      expect(result.status).toBe(2);
      expect(result.stderr).toContain("invalid MARKETINGCLAW_DOCKER_E2E_LOG_PRINT_BYTES: 64kb");
  ...
  it("cleans ClawHub fixture children when readiness times out", () => {
    const root = mkdtempSync(path.join(tmpdir(), "marketingclaw-plugin-clawhub-fixture-cleanup-"));
    try {
      const binDir = path.join(root, "bin");
      const cleanupPath = path.join(root, "caller-cleanup");
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: {
            ...process.env,
            MARKETINGCLAW_PLUGINS_E2E_LIVE_CLAWHUB: "0",
            MARKETINGCLAW_PLUGINS_TMP_DIR: tmpDir,
            MARKETINGCLAW_TEST_FIXTURE_SERVER_PID: pidPath,
            PATH: `${binDir}${path.delimiter}/usr/bin${path.delimiter}/bin`,
          },
        },
  ...
  it("rejects invalid plugin fixture log byte limits before ClawHub fixture setup", () => {
    const root = mkdtempSync(
      path.join(tmpdir(), "marketingclaw-plugin-clawhub-fixture-log-invalid-"),
    );
    try {
      const binDir = path.join(root, "bin");
      const tmpDir = path.join(root, "scratch");
  ...
  it("bounds ClawHub fixture server logs when readiness fails", () => {
    const root = mkdtempSync(path.join(tmpdir(), "marketingclaw-plugin-clawhub-fixture-log-"));
    try {
      const binDir = path.join(root, "bin");
      const tmpDir = path.join(root, "scratch");
      mkdirSync(tmpDir);
  ...
  it("uses the configured scratch root and resolves Windows home-relative install paths", () => {
    const root = mkdtempSync(path.join(tmpdir(), "marketingclaw-plugins-assertions-"));
    ...
    try {
      ...
      writeJson(path.join(home, ".marketingclaw", "plugins", "installs.json"), {
        installRecords: {
          "demo-plugin-tgz": {
            source: "archive",
      ...
      const result = spawnSync(process.execPath, [ASSERTIONS_SCRIPT, "plugin-tgz"], {
        ...
        env: {
          ...
          HOME: home,
          MARKETINGCLAW_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS: "1e3",
          MARKETINGCLAW_PLUGINS_TMP_DIR: scratchRoot,
        },
      });
  ...
  it("compares local plugin source paths by canonical path", () => {
    ...
    const installPath = path.join(home, ".marketingclaw", "extensions", "demo-plugin-dir");
  ...
  it("still requires archive managed install directories to be removed", () => {
    ...
    const home = path.join(root, "home");
    const scratchRoot = path.join(root, "scratch");
    const installPath = path.join(home, ".marketingclaw", "extensions", "demo-plugin-tgz");
    mkdirSync(installPath, { recursive: true });
  ...
  it("rejects unreadable config during plugin uninstall proof", () => {
    ...
    const home = path.join(root, "home");
    const scratchRoot = path.join(root, "scratch");
    const removedInstallPath = path.join(home, ".marketingclaw", "extensions", "demo-plugin-tgz");
    ...
    try {
      ...
      expect(result.status).not.toBe(0);
      expect(result.stderr).toContain("failed to read MarketingClaw config");
    } finally {
      rmSync(root, { force: true, recursive: true });
    }
  });
  ...
  it("rejects ClawHub install paths that resolve outside the managed extensions root", () => {
    const tempDirs: string[] = [];
    const root = makeTempDir(tempDirs, "marketingclaw-plugins-clawhub-path-");
    const home = path.join(root, "home");
    const scratchRoot = path.join(root, "scratch");
    const extensionsRoot = path.join(home, ".marketingclaw", "extensions");
    ...
    try {
      writeJson(path.join(scratchRoot, "plugins-clawhub-installed.json"), {
        plugins: [{ id: "marketingclaw-kitchen-sink-fixture", status: "loaded" }],
      });
      writeJson(path.join(scratchRoot, "plugins-clawhub-inspect.json"), {
        plugin: { id: "marketingclaw-kitchen-sink-fixture" },
      });
      writeJson(path.join(home, ".marketingclaw", "plugins", "installs.json"), {
        installRecords: {
          "marketingclaw-kitchen-sink-fixture": {
            artifactFormat: "zip",
      ...
      const result = spawnSync(process.execPath, [ASSERTIONS_SCRIPT, "clawhub-installed"], {
        ...
        env: {
          ...process.env,
          CLAWHUB_PLUGIN_ID: "marketingclaw-kitchen-sink-fixture",
          CLAWHUB_PLUGIN_SPEC: "clawhub:@openclaw/kitchen-sink",
          HOME: home,
          MARKETINGCLAW_PLUGINS_TMP_DIR: scratchRoot,
        },
  ...
  it("times out stalled ClawHub package metadata requests", async () => {
    ...
    try {
      ...
      const result = await runAssertionAsync(["clawhub-preflight"], {
        CLAWHUB_PLUGIN_ID: "marketingclaw-kitchen-sink-fixture",
        CLAWHUB_PLUGIN_SPEC: "clawhub:@openclaw/kitchen-sink",
        MARKETINGCLAW_CLAWHUB_URL: `http://127.0.0.1:${address.port}`,
        MARKETINGCLAW_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS: "25",
      });
  ...
  it("times out stalled ClawHub package metadata bodies", async () => {
    ...
    try {
      ...
      const result = await runAssertionAsync(["clawhub-preflight"], {
        ...
        CLAWHUB_PLUGIN_SPEC: "clawhub:@openclaw/kitchen-sink",
        MARKETINGCLAW_CLAWHUB_URL: `http://127.0.0.1:${address.port}`,
        MARKETINGCLAW_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS: "75",
      });
  ...
  it("bounds ClawHub package metadata response bodies", async () => {
    ...
    try {
      ...
      const result = await runAssertionAsync(["clawhub-preflight"], {
        ...
        CLAWHUB_PLUGIN_SPEC: "clawhub:@openclaw/kitchen-sink",
        MARKETINGCLAW_CLAWHUB_URL: `http://127.0.0.1:${address.port}`,
        MARKETINGCLAW_PLUGINS_E2E_CLAWHUB_PREFLIGHT_BODY_MAX_BYTES: "16",
        MARKETINGCLAW_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS: "1000",
      });
