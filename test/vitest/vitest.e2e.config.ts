...
// Keep e2e runs cheap by default; callers can still override via MARKETINGCLAW_E2E_WORKERS.
const defaultWorkers = isCI ? Math.min(2, Math.max(1, Math.floor(cpuCount * 0.25))) : 1;
const requestedWorkers = Number.parseInt(process.env.MARKETINGCLAW_E2E_WORKERS ?? "", 10);
const e2eWorkers =
  Number.isFinite(requestedWorkers) && requestedWorkers > 0
    ? Math.min(16, requestedWorkers)
    : defaultWorkers;
const verboseE2E = process.env.MARKETINGCLAW_E2E_VERBOSE === "1";
...
export default defineConfig({
  ...
  test: {
    ...
    setupFiles: [
      ...new Set(
        [...(baseTest.setupFiles ?? []), "test/setup-marketingclaw-runtime.ts"].map(
          resolveRepoRootPath,
        ),
      ),
    ],
