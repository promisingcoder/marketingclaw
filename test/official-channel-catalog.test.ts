...
import { bundledPluginRoot } from "marketingclaw/plugin-sdk/test-fixtures";
...
type OfficialChannelInstall = NonNullable<
  NonNullable<OfficialChannelCatalogEntry["marketingclaw"]>["install"]
>;
...
function requireInstall(entry: OfficialChannelCatalogEntry | undefined): OfficialChannelInstall {
  const install = entry?.marketingclaw?.install;
  if (!install) {
...
function summarizeCatalogEntry(entry: OfficialChannelCatalogEntry) {
  return {
    ...
    source: entry.source,
    plugin: entry.marketingclaw?.plugin,
    channel: entry.marketingclaw?.channel,
    install: entry.marketingclaw?.install,
  };
}
...
describe("buildOfficialChannelCatalog", () => {
  it("includes publishable official channel plugins and skips non-publishable entries", () => {
    const repoRoot = makeRepoRoot("marketingclaw-official-channel-catalog-");
    writeJson(path.join(repoRoot, "extensions", "whatsapp", "package.json"), {
      name: "@marketingclaw/whatsapp",
      version: "2026.3.23",
      description: "MarketingClaw WhatsApp channel plugin",
      marketingclaw: {
        channel: {
          id: "whatsapp",
          label: "WhatsApp",
    ...
    expect(
      summarizeCatalogEntry(
        findCatalogEntry(entries, (entry) => entry.name === "@wecom/wecom-marketingclaw-plugin"),
      ),
    ).toEqual({
      name: "@wecom/wecom-marketingclaw-plugin",
      description: "MarketingClaw WeCom channel plugin by the Tencent WeCom team.",
      source: "external",
      plugin: {
    ...
    expect(
      summarizeCatalogEntry(
        findCatalogEntry(entries, (entry) => entry.name === "marketingclaw-plugin-yuanbao"),
      ),
    ).toEqual({
      name: "marketingclaw-plugin-yuanbao",
      description: "MarketingClaw Yuanbao channel plugin by the Tencent Yuanbao team.",
      source: "external",
      plugin: {
    ...
    expect(
      summarizeCatalogEntry(
        findCatalogEntry(entries, (entry) => entry.name === "@marketingclaw/whatsapp"),
      ),
    ).toEqual({
      name: "@marketingclaw/whatsapp",
      description: "MarketingClaw WhatsApp channel plugin",
  ...
  it("keeps third-party official external catalog npm sources exactly pinned", () => {
    const repoRoot = makeRepoRoot("marketingclaw-official-channel-catalog-policy-");
    const entries = buildOfficialChannelCatalog({ repoRoot }).entries.filter(
      (entry) => entry.source === "external" && !entry.name?.startsWith("@marketingclaw/"),
    );
  ...
  it("allows official MarketingClaw channel npm specs without integrity during launch", () => {
    const repoRoot = makeRepoRoot("marketingclaw-official-channel-catalog-marketingclaw-policy-");
    const twitch = buildOfficialChannelCatalog({ repoRoot }).entries.find(
      (entry) => entry.marketingclaw?.channel?.id === "twitch",
    );

    expect({
      name: twitch?.name,
      install: twitch?.marketingclaw?.install,
    }).toEqual({
  ...
  it("preserves ClawHub specs when generating publishable channel catalog entries", () => {
    const repoRoot = makeRepoRoot("marketingclaw-official-channel-catalog-clawhub-");
    ...
    const entry = buildOfficialChannelCatalog({ repoRoot }).entries.find(
      (candidate) => candidate.marketingclaw?.channel?.id === "storepack-chat",
    );
  ...
  it("writes the official catalog under dist", () => {
    const repoRoot = makeRepoRoot("marketingclaw-official-channel-catalog-write-");
    writeJson(path.join(repoRoot, "extensions", "whatsapp", "package.json"), {
      name: "@marketingclaw/whatsapp",
      marketingclaw: {
        channel: {
    ...
    expect(entries.map((entry: { name?: string }) => entry.name)).toContain(
      "@wecom/wecom-marketingclaw-plugin",
    );
    expect(entries.map((entry: { name?: string }) => entry.name)).toContain(
      "marketingclaw-plugin-yuanbao",
    );
    const whatsappEntry = findCatalogEntry(
      entries,
      (entry: { marketingclaw?: { channel?: { id?: string } } }) =>
        entry.marketingclaw?.channel?.id === "whatsapp",
    );
