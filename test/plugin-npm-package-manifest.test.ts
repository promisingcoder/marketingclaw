...
function writePublishablePluginPackage(repoDir: string): string {
  ...
  writeJsonFile(join(packageDir, "marketingclaw.plugin.json"), { id: "diffs" });
...
describe("plugin npm package manifest staging", () => {
  ...
  it("overlays generated channel configs while packing and restores source manifest", () => {
    const repoDir = makeTempRepoRoot(tempDirs, "marketingclaw-plugin-npm-package-manifest-");
    ...
    writeJsonFile(join(packageDir, "marketingclaw.plugin.json"), sourceManifest);
    ...
    const originalText = readFileSync(join(packageDir, "marketingclaw.plugin.json"), "utf8");
    withAugmentedPluginNpmManifestForPackage({ repoRoot: repoDir, packageDir }, () => {
      const stagedManifest = JSON.parse(
        readFileSync(join(packageDir, "marketingclaw.plugin.json"), "utf8"),
      );
      expect(stagedManifest.channelConfigs.twitch.description).toBe("Twitch chat integration");
    });
    expect(readFileSync(join(packageDir, "marketingclaw.plugin.json"), "utf8")).toBe(originalText);
  });

  it("overlays package-local runtime metadata while packing and restores source package json", () => {
    const repoDir = makeTempRepoRoot(tempDirs, "marketingclaw-plugin-npm-package-runtime-");
    ...
    withAugmentedPluginNpmManifestForPackage(
      ...
      () => {
        ...
        expect(stagedPackageJson.marketingclaw.extensions).toEqual(["./index.ts"]);
        expect(stagedPackageJson.marketingclaw.runtimeExtensions).toEqual(["./dist/index.js"]);
        expect(stagedPackageJson.marketingclaw.setupEntry).toBe("./dist/setup-entry.js");
        expect(stagedPackageJson.marketingclaw.runtimeSetupEntry).toBe("./dist/setup-entry.js");
        ...
        expect(stagedPackageJson.peerDependencies.marketingclaw).toBe(">=2026.4.30");
        expect(stagedPackageJson.peerDependenciesMeta.marketingclaw.optional).toBe(true);
      },
    );
  ...
  it("installs and cleans package-local bundled dependencies while packing", () => {
    const repoDir = makeTempRepoRoot(tempDirs, "marketingclaw-plugin-npm-package-bundled-deps-");
    const packageDir = writePublishablePluginPackage(repoDir);
    writeFileText(join(packageDir, "dist", "index.js"), "export {};\n");
    writeFileText(join(packageDir, "dist", "setup-entry.js"), "export {};\n");
    ...
    writeJsonFile(join(packageDir, "package.json"), {
      ...
      devDependencies: {
        "@marketingclaw/plugin-sdk": "workspace:*",
      },
      marketingclaw: {
        extensions: ["./index.ts"],
        setupEntry: "./setup-entry.ts",
  ...
  it("force-installs missing optional bundled dependencies for portable packs", () => {
    const repoDir = makeTempRepoRoot(
      tempDirs,
      "marketingclaw-plugin-npm-package-portable-optional-",
    );
    const packageDir = writePublishablePluginPackage(repoDir);
    writeFileText(join(packageDir, "dist", "index.js"), "export {};\n");
    writeFileText(join(packageDir, "dist", "setup-entry.js"), "export {};\n");
  ...
  it("honors plugin package opt-out for bundled runtime dependencies", () => {
    const repoDir = makeTempRepoRoot(tempDirs, "marketingclaw-plugin-npm-package-bundle-opt-out-");
    const packageDir = writePublishablePluginPackage(repoDir);
    writeFileText(join(packageDir, "dist", "index.js"), "export {};\n");
    writeFileText(join(packageDir, "dist", "setup-entry.js"), "export {};\n");
    writeLocalDependencyPackage(packageDir);
  ...
  it("refuses to pack publishable plugins before package-local runtime files exist", () => {
    const repoDir = makeTempRepoRoot(tempDirs, "marketingclaw-plugin-npm-package-runtime-missing-");
    const packageDir = writePublishablePluginPackage(repoDir);
  ...
  it("refuses package file rules that omit advertised package-local runtime files", () => {
    const repoDir = makeTempRepoRoot(
      tempDirs,
      "marketingclaw-plugin-npm-package-runtime-excluded-",
    );
    const packageDir = writePublishablePluginPackage(repoDir);
    writeFileText(join(packageDir, "dist", "index.js"), "export {};\n");
    writeFileText(join(packageDir, "dist", "setup-entry.js"), "export {};\n");
