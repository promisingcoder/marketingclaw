...
describe("resolveVitestIsolation", () => {
  it("aliases private QA plugin SDK subpaths for source tests only", () => {
    for (const subpath of PRIVATE_PLUGIN_SDK_SUBPATHS) {
      expect(
        findAlias(sharedVitestConfig.resolve.alias, `marketingclaw/plugin-sdk/${subpath}`),
      ).toEqual({
      ...
      expect(() =>
        findAlias(sharedVitestConfig.resolve.alias, `@marketingclaw/plugin-sdk/${subpath}`),
      ).toThrow(`missing alias @marketingclaw/plugin-sdk/${subpath}`);
    }
  });

  it("aliases private core packages to source for clean checkout tests", () => {
    expect(findAlias(sharedVitestConfig.resolve.alias, "@marketingclaw/media-core/mime")).toEqual({
      find: "@marketingclaw/media-core/mime",
      replacement: path.join(process.cwd(), "packages", "media-core", "src", "mime.ts"),
    });
    expect(
      findAlias(sharedVitestConfig.resolve.alias, "@marketingclaw/acp-core/runtime/types"),
    ).toEqual({
      find: "@marketingclaw/acp-core/runtime/types",
  ...
  it("ignores the legacy isolation escape hatches", () => {
    expect(resolveVitestIsolation({ MARKETINGCLAW_TEST_ISOLATE: "1" })).toBe(false);
    expect(resolveVitestIsolation({ MARKETINGCLAW_TEST_NO_ISOLATE: "0" })).toBe(false);
    expect(resolveVitestIsolation({ MARKETINGCLAW_TEST_NO_ISOLATE: "false" })).toBe(false);
  });
...
describe("createScopedVitestConfig", () => {
  ...
  it("loads scoped include overrides from MARKETINGCLAW_VITEST_INCLUDE_FILE", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-vitest-scoped-"));
    try {
...
describe("scoped vitest configs", () => {
  ...
  it("keeps the process lane off the marketingclaw runtime setup", () => {
  ...
  it("keeps selected plugin-sdk and commands light lanes off the marketingclaw runtime setup", () => {
  ...
  it("keeps the ui lane off both the marketingclaw runtime setup and unit-fast excludes", () => {
  ...
  it("loads channel include overrides from MARKETINGCLAW_VITEST_INCLUDE_FILE", () => {
    const tempDirs: string[] = [];
    const tempDir = makeTempDir(tempDirs, "marketingclaw-vitest-channels-");
    try {
      const includeFile = path.join(tempDir, "include.json");
      fs.writeFileSync(
        includeFile,
  ...
  it("keeps tooling tests in their own lane", () => {
    ...
    expect(testConfig.exclude).toContain("test/scripts/marketingclaw-e2e-instance.test.ts");
  ...
  it("runs shell helper tooling tests isolated from shared mocks", () => {
    ...
    expect(testConfig.include).toEqual(["test/scripts/marketingclaw-e2e-instance.test.ts"]);
    expect(testConfig.isolate).toBe(true);
    expect(testConfig.runner).toBeUndefined();
  });
