...
describe("isUnitConfigTestFile", () => {
  ...
  it("rejects files excluded from the unit config", () => {
    ...
    expect(isUnitConfigTestFile("src/infra/marketingclaw-exec-env.test.ts")).toBe(false);
    expect(
      isUnitConfigTestFile(bundledPluginFile("matrix", "src/migration-snapshot.test.ts")),
    ).toBe(false);
    expect(isUnitConfigTestFile("src/infra/marketingclaw-root.test.ts")).toBe(false);
