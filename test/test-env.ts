...
const LIVE_TEST_TRIGGER_ENV_KEYS = [
  "LIVE",
  "MARKETINGCLAW_LIVE_TEST",
  "MARKETINGCLAW_LIVE_GATEWAY",
] as const;
const HERMETIC_TEST_ENV_KEYS = [
  ...LIVE_TEST_TRIGGER_ENV_KEYS,
  "MARKETINGCLAW_LIVE_USE_REAL_HOME",
  "MARKETINGCLAW_BUNDLED_PLUGINS_DIR",
  "MARKETINGCLAW_TEST_TRUST_BUNDLED_PLUGINS_DIR",
...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.MARKETINGCLAW_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    ...
    { key: "MARKETINGCLAW_TEST_FAST", value: process.env.MARKETINGCLAW_TEST_FAST },
    {
      key: "MARKETINGCLAW_STRICT_FAST_REPLY_CONFIG",
      value: process.env.MARKETINGCLAW_STRICT_FAST_REPLY_CONFIG,
    },
    {
      key: "MARKETINGCLAW_ALLOW_SLOW_REPLY_TESTS",
      value: process.env.MARKETINGCLAW_ALLOW_SLOW_REPLY_TESTS,
    },
    {
      key: "MARKETINGCLAW_LIVE_TEST_NORMALIZE_CONFIG",
      value: process.env.MARKETINGCLAW_LIVE_TEST_NORMALIZE_CONFIG,
    },
    ...
    { key: "MARKETINGCLAW_STATE_DIR", value: process.env.MARKETINGCLAW_STATE_DIR },
    { key: "MARKETINGCLAW_CONFIG_PATH", value: process.env.MARKETINGCLAW_CONFIG_PATH },
    { key: "MARKETINGCLAW_GATEWAY_PORT", value: process.env.MARKETINGCLAW_GATEWAY_PORT },
    { key: "MARKETINGCLAW_BRIDGE_ENABLED", value: process.env.MARKETINGCLAW_BRIDGE_ENABLED },
    { key: "MARKETINGCLAW_BRIDGE_HOST", value: process.env.MARKETINGCLAW_BRIDGE_HOST },
    { key: "MARKETINGCLAW_BRIDGE_PORT", value: process.env.MARKETINGCLAW_BRIDGE_PORT },
    { key: "MARKETINGCLAW_CANVAS_HOST_PORT", value: process.env.MARKETINGCLAW_CANVAS_HOST_PORT },
    { key: "MARKETINGCLAW_TEST_HOME", value: process.env.MARKETINGCLAW_TEST_HOME },
    { key: "MARKETINGCLAW_AGENT_DIR", value: process.env.MARKETINGCLAW_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "marketingclaw-test-home-"));

  setTestEnvValue("HOME", tempHome);
  setTestEnvValue("USERPROFILE", tempHome);
  setTestEnvValue("MARKETINGCLAW_TEST_HOME", tempHome);
  setTestEnvValue("MARKETINGCLAW_TEST_FAST", "1");
  setTestEnvValue("MARKETINGCLAW_STRICT_FAST_REPLY_CONFIG", "1");
  deleteTestEnvValue("MARKETINGCLAW_ALLOW_SLOW_REPLY_TESTS");
  ...
  deleteTestEnvValue("MARKETINGCLAW_AGENT_DIR");
  ...
  deleteTestEnvValue("MARKETINGCLAW_BRIDGE_ENABLED");
  deleteTestEnvValue("MARKETINGCLAW_BRIDGE_HOST");
  deleteTestEnvValue("MARKETINGCLAW_BRIDGE_PORT");
  deleteTestEnvValue("MARKETINGCLAW_CANVAS_HOST_PORT");
...
function sanitizeLiveConfig(raw: string): string {
  try {
    ...
    if (!isTruthyEnvValue(process.env.MARKETINGCLAW_LIVE_TEST_NORMALIZE_CONFIG)) {
...
function stageLiveTestState(params: {
  ...
  const rawStateDir = params.env.MARKETINGCLAW_STATE_DIR?.trim();
  let realStateDir = rawStateDir
    ? resolveHomeRelativePath(rawStateDir, params.realHome)
    : path.join(params.realHome, ".marketingclaw");
  const priorIsolatedHome = params.env.MARKETINGCLAW_TEST_HOME?.trim();
  const snapshotHome = params.env.HOME?.trim();
  if (
    priorIsolatedHome &&
    snapshotHome &&
    snapshotHome !== priorIsolatedHome &&
    realStateDir === path.join(priorIsolatedHome, ".marketingclaw")
  ) {
    realStateDir = path.join(params.realHome, ".marketingclaw");
  }
  const tempStateDir = path.join(params.tempHome, ".marketingclaw");
  fs.mkdirSync(tempStateDir, { recursive: true });
  fs.mkdirSync(path.join(params.tempHome, ".gemini"), { recursive: true });

  const realConfigPath = params.env.MARKETINGCLAW_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.MARKETINGCLAW_CONFIG_PATH, params.realHome)
    : path.join(realStateDir, "marketingclaw.json");
  if (fs.existsSync(realConfigPath)) {
    const rawConfig = fs.readFileSync(realConfigPath, "utf8");
    fs.writeFileSync(
      path.join(tempStateDir, "marketingclaw.json"),
...
export function installTestEnv(options?: InstallTestEnvOptions): {
  ...
  const allowRealHome = !hermetic && isTruthyEnvValue(process.env.MARKETINGCLAW_LIVE_USE_REAL_HOME);
  ...
  if (hermetic) {
    ...
    setTestEnvValue("MARKETINGCLAW_TEST_TRUST_BUNDLED_PLUGINS_DIR", "1");
    setTestEnvValue(
      "MARKETINGCLAW_BUNDLED_PLUGINS_DIR",
