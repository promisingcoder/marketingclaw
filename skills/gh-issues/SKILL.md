...
if [ -z "${GH_TOKEN:-}" ]; then
  CONFIG_PATH="${MARKETINGCLAW_CONFIG_PATH:-${MARKETINGCLAW_STATE_DIR:-$HOME/.marketingclaw}/marketingclaw.json}"
...
```text
${MARKETINGCLAW_STATE_DIR:-$HOME/.marketingclaw}/gh-issues-<owner>-<repo>.json
```
...
Send completion/failure with marketingclaw message send if route provided.
```
