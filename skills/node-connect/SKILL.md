---
name: node-connect
description: "Diagnose MarketingClaw Android, iOS, or macOS node pairing, QR/setup code, route, auth, and connection failures."
---

# Node Connect

Goal: find the one real route from node -> gateway, verify MarketingClaw is advertising that route, then fix pairing/auth.
...
- whether `marketingclaw devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `marketingclaw qr --json`. It uses the same setup-code payload Android scans.

```bash
marketingclaw config get gateway.mode
marketingclaw config get gateway.bind
marketingclaw config get gateway.tailscale.mode
marketingclaw config get gateway.remote.url
marketingclaw config get gateway.auth.mode
marketingclaw config get gateway.auth.allowTailscale
marketingclaw config get plugins.entries.device-pair.config.publicUrl
marketingclaw qr --json
marketingclaw devices list
marketingclaw nodes status
```

If this MarketingClaw instance is pointed at a remote gateway, also run:

```bash
marketingclaw qr --remote --json
```
...
`marketingclaw qr --json` success means:
...
If `marketingclaw qr --json` says `Gateway is only bound to loopback`:
...
- `marketingclaw devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run marketingclaw qr again, rescan, then approve the pending device pairing.`
