...
Use for background feature builds, PR reviews, large refactors, and issue-to-PR loops. Do not use for simple edits, read-only lookup, ACP thread-bound work, or any run inside `~/.marketingclaw`, `$MARKETINGCLAW_STATE_DIR`, or active MarketingClaw state dirs.
...
- Worker must send completion/failure via `marketingclaw message send`.
...
- Never checkout branches or run background coding agents in `~/Projects/marketingclaw`; use an isolated checkout.
...
marketingclaw message send --channel <channel> --target '<target>' --message '<brief result>'
Add --account, --reply-to, or --thread-id only when present above.
Do not use marketingclaw system event or heartbeat.
```
...
```bash
PROMPT=$(mktemp -t marketingclaw-worker-prompt.XXXXXX)
cat >"$PROMPT" <<'EOF'
Task.
<notification block>
EOF
