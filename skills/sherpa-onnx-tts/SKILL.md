...
1. Download the runtime for your OS (extracts into `$MARKETINGCLAW_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.marketingclaw/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$MARKETINGCLAW_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.marketingclaw/tools/sherpa-onnx-tts/models`)
...
Then write those resolved paths into the active MarketingClaw config file (`$MARKETINGCLAW_CONFIG_PATH`, default `~/.marketingclaw/marketingclaw.json`):
