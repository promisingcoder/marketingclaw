...
- For durable MarketingClaw skill creation or updates in an agent session, use
...
- Frontmatter needs `name` + `description`; local MarketingClaw skills may also use `metadata`, `homepage`, `allowed-tools`, `user-invocable`, `license`.
...
3. Create or revise the pending proposal through `skill_workshop` when the
   change should persist as an MarketingClaw skill.
