...
- Re-open anytime: `marketingclaw dashboard` (copies the link, opens a browser if possible, prints an SSH hint if headless).
- If clipboard and browser delivery both fail, `marketingclaw dashboard` still prints the clean URL and tells you to append your token (from `MARKETINGCLAW_GATEWAY_TOKEN` or `gateway.auth.token`) as the URL fragment key `token`; it never prints the token value in logs.
...
- **Shared-secret token source**: `gateway.auth.token` (or `MARKETINGCLAW_GATEWAY_TOKEN`). `marketingclaw dashboard` can pass it via URL fragment for one-time bootstrap; the Control UI keeps it in sessionStorage for the current tab and selected gateway URL, not localStorage.
- If `gateway.auth.token` is SecretRef-managed, `marketingclaw dashboard` prints/copies/opens a non-tokenized URL by design, to avoid exposing externally managed tokens in shell logs, clipboard history, or browser-launch arguments. If the ref is unresolved in your current shell, it still prints the non-tokenized URL plus actionable auth setup guidance.
- **Shared-secret password**: use the configured `gateway.auth.password` (or `MARKETINGCLAW_GATEWAY_PASSWORD`). The dashboard does not persist passwords across reloads.
...
- Confirm the gateway is reachable: local `marketingclaw status`; remote, SSH tunnel `ssh -N -L 18789:127.0.0.1:18789 user@gateway-host` then open `http://127.0.0.1:18789/`.
...
- Retrieve or supply the shared secret from the gateway host:
  - Token: `marketingclaw config get gateway.auth.token`
  - Password: resolve the configured `gateway.auth.password` or `MARKETINGCLAW_GATEWAY_PASSWORD`
  - SecretRef-managed token: resolve the external secret provider, or export `MARKETINGCLAW_GATEWAY_TOKEN` in this shell and rerun `marketingclaw dashboard`
  - No shared secret configured: `marketingclaw doctor --generate-gateway-token`
