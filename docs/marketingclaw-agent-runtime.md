---
summary: "Developer workflow for MarketingClaw agent runtime: build, test, and live validation"
title: "MarketingClaw agent runtime workflow"
read_when:
  - Working on MarketingClaw agent runtime code or tests
  - Running agent-runtime lint, typecheck, and live test flows
---

Developer workflow for the agent runtime (`src/agents/`) in the MarketingClaw repo.
...
wrapper (sets `MARKETINGCLAW_LIVE_TEST=1` and needs provider credentials):
...
- Run the Gateway in dev mode (skips channel connections via `MARKETINGCLAW_SKIP_CHANNELS=1`): `pnpm gateway:dev`
- Trigger one agent turn through the Gateway: `pnpm marketingclaw agent --message "Hello" --thinking low`
...
State lives in the MarketingClaw state directory: `~/.marketingclaw` by default, or
`$MARKETINGCLAW_STATE_DIR` when set. Paths relative to that directory:

| Path                                                | Holds                                                              |
| --------------------------------------------------- | ------------------------------------------------------------------ |
| `marketingclaw.json`                                | Config                                                             |
| `state/marketingclaw.sqlite`                        | Shared runtime state database                                      |
| `agents/<agentId>/agent/marketingclaw-agent.sqlite` | Per-agent model auth profiles (API keys + OAuth) and runtime state |
...
- Keep auth: leave `agents/<agentId>/agent/marketingclaw-agent.sqlite` and `credentials/` in place.

Legacy `auth-profiles.json` files are no longer read at runtime;
`marketingclaw doctor --fix` imports them into the SQLite store.
...
- [MarketingClaw agent runtime architecture](/agent-runtime-architecture)
