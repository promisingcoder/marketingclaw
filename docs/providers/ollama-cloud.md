---
summary: "Use Ollama Cloud directly with MarketingClaw"
read_when:
...
MarketingClaw registers `ollama-cloud` as its own provider id so cloud-only
...
```bash
marketingclaw onboard --auth-choice ollama-cloud --ollama-cloud-api-key "<key>"
```
...
- You want the same native Ollama chat API shape MarketingClaw uses for local
...
MarketingClaw discovers Ollama Cloud models live from the hosted catalog:

```bash
marketingclaw models list --provider ollama-cloud
marketingclaw models set ollama-cloud/kimi-k2.6
```
...
nothing, MarketingClaw falls back to the bundled rows `kimi-k2.5:cloud`,
...
MARKETINGCLAW_LIVE_OLLAMA_MODEL=kimi-k2.6 \
pnpm test:live -- extensions/ollama/ollama.live.test.ts
```

The cloud smoke runs text, native stream, and web search; set
`MARKETINGCLAW_LIVE_OLLAMA_WEB_SEARCH=0` to skip web search. It skips embeddings by
default for `https://ollama.com` because Ollama Cloud API keys may not
authorize `/api/embed`; force them with `MARKETINGCLAW_LIVE_OLLAMA_EMBEDDINGS=1`.
...
- Unknown model errors: run `marketingclaw models list --provider ollama-cloud` and
