---
summary: "Use Venice AI privacy-focused models in MarketingClaw"
read_when:
  - You want privacy-focused inference in MarketingClaw
...
<Steps>
  ...
  <Step title="Configure MarketingClaw">
    <Tabs>
      <Tab title="Interactive (recommended)">
        ```bash
        marketingclaw onboard --auth-choice venice-api-key
        ```
  ...
  <Step title="Verify setup">
    ```bash
    marketingclaw agent --model venice/kimi-k2-5 --message "Hello, are you working?"
    ```
  </Step>
</Steps>
...
```bash
marketingclaw models set venice/kimi-k2-5
marketingclaw models list --all --provider venice
```

You can also run `marketingclaw configure` and pick **Model/auth provider > Venice AI**.
...
The bundled catalog above is a manifest-backed seed list. At runtime MarketingClaw
...
`deepseek-v4-flash`, MarketingClaw fills the required `reasoning_content` replay
...
marketingclaw agent --model venice/kimi-k2-5 --message "Quick health check"

# Claude Opus via Venice (anonymized)
marketingclaw agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Uncensored model
marketingclaw agent --model venice/venice-uncensored --message "Draft options"

# Vision model with image
marketingclaw agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Coding model
marketingclaw agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
<AccordionGroup>
  <Accordion title="API key not recognized">
    ```bash
    echo $VENICE_API_KEY
    marketingclaw models list | grep venice
    ```
  ...
  <Accordion title="Model not available">
    Run `marketingclaw models list --all --provider venice` to see currently
