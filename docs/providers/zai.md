---
summary: "Use Z.AI (GLM models) with MarketingClaw"
read_when:
  - You want Z.AI / GLM models in MarketingClaw
...
MarketingClaw uses the `zai` provider with a Z.AI API key.

| Property | Value                                        |
| -------- | -------------------------------------------- |
| Provider | `zai`                                        |
| Package  | `@marketingclaw/zai-provider`                |
| Auth     | `ZAI_API_KEY` (legacy alias: `Z_AI_API_KEY`) |
| API      | Z.AI Chat Completions (Bearer auth)          |

## GLM models

GLM is a model family, not a separate provider. In MarketingClaw, GLM models use
...
<Tabs>
  <Tab title="Auto-detect endpoint">
    **Best for:** most users. MarketingClaw probes supported Z.AI endpoints with your API key and applies the correct base URL automatically.

    <Steps>
      <Step title="Run onboarding">
        ```bash
        marketingclaw onboard --auth-choice zai-api-key
        ```
      </Step>
      <Step title="Verify the model is listed">
        ```bash
        marketingclaw models list --all --provider zai
        ```
      </Step>
    </Steps>
    ...
    **Best for:** users who want to force a specific Coding Plan or general API surface.
    ...
    <Steps>
      <Step title="Pick the right onboarding choice">
        ...
        marketingclaw onboard --auth-choice zai-coding-cn

        # General API
        marketingclaw onboard --auth-choice zai-global

        # General API CN (China region)
        marketingclaw onboard --auth-choice zai-cn
        ```
      </Step>
      <Step title="Verify the model is listed">
        ```bash
...
<Tip>
`zai-api-key` lets MarketingClaw detect the matching Z.AI endpoint from the key and
...
versions and availability can change; run `marketingclaw models list --all --provider zai`
...
<Tabs>
  <Tab title="GLM-5.2">
    Full range: `off`, `low`, `high`, `max` (default `off`). MarketingClaw maps
...
<AccordionGroup>
  ...
  <Accordion title="Preserved thinking">
    ...
    When enabled and thinking is on, MarketingClaw sends
  ...
  <Accordion title="Auth details">
    ...
    - The legacy env var `Z_AI_API_KEY` is still accepted; MarketingClaw copies it to `ZAI_API_KEY` at startup if `ZAI_API_KEY` is unset.
...
<CardGroup cols={2}>
  ...
  <Card title="Configuration reference" href="/gateway/configuration-reference" icon="gear">
    Full MarketingClaw config schema, including provider and model settings.
  </Card>
</CardGroup>
