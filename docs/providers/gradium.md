---
summary: "Use Gradium text-to-speech in MarketingClaw"
read_when:
...
[Gradium](https://gradium.ai) is a text-to-speech provider for MarketingClaw. It renders standard audio replies (WAV), voice-note-compatible Opus output, and 8 kHz u-law audio for telephony surfaces.
...
Output format is chosen automatically by target surface (see [Output](#output)) and is not configurable in `marketingclaw.json`.
...
Among configured TTS providers, Gradium's auto-select order is `30`. See [Text-to-Speech](/tools/tts) for how MarketingClaw picks the active provider when `messages.tts.provider` is not pinned.
