---
summary: "Run MarketingClaw through ds4, a local DeepSeek V4 Flash OpenAI-compatible server"
read_when:
  - You want to run MarketingClaw against antirez/ds4
  - You want a local DeepSeek V4 Flash backend with tool calls
  - You need the MarketingClaw config for ds4-server
title: "ds4"
---

[ds4](https://github.com/antirez/ds4) serves DeepSeek V4 Flash from a local
Metal backend with an OpenAI-compatible `/v1` API. MarketingClaw connects to ds4
through the generic `openai-completions` provider family.

ds4 is not a bundled MarketingClaw provider plugin. Configure it under
...
<Warning>
MarketingClaw agent turns include tool schemas and workspace context. A tiny context
...
<Steps>
  ...
  <Step title="Add the MarketingClaw provider config">
    ...
    marketingclaw infer model run \
      ...
      --thinking off \
      --prompt "Reply with exactly: marketingclaw-ds4-ok" \
      --json
    ```
...
with `--tokens` unless you intentionally want MarketingClaw to request less output
than the server default.

## On-demand startup

MarketingClaw can start ds4 only when a `ds4/...` model is selected. Add
...
If you run that large context, update both the server flags and MarketingClaw model
metadata:
...
Direct HTTP check, bypassing MarketingClaw:
...
MarketingClaw model routing (same as the Quickstart check):
...
<AccordionGroup>
  ...
  <Accordion title="500 prompt exceeds context">
    The configured `--ctx` is too small for the MarketingClaw turn. Raise
  ...
  <Accordion title="The first request is slow">
    ...
    `localService.readyTimeoutMs: 300000` when MarketingClaw starts the server on
    demand.
  </Accordion>
</AccordionGroup>
