...
SenseAudio transcribes inbound audio and voice-note attachments through MarketingClaw's shared `tools.media.audio` pipeline. MarketingClaw posts multipart audio to the OpenAI-compatible transcription endpoint and injects the returned text as `{{Transcript}}` plus an `[Audio]` block.
...
<Steps>
  ...
  <Step title="Send a voice note">
    Send an audio message through any connected channel. MarketingClaw uploads the
...
<Note>
SenseAudio is batch STT only in MarketingClaw. Voice Call realtime transcription
