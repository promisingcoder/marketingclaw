---
summary: "Run MarketingClaw with LM Studio"
read_when:
  - You want to run MarketingClaw with open source models via LM Studio
...
```bash
marketingclaw models set lmstudio/qwen/qwen3.5-9b
```

LM Studio model keys use an `author/model-name` format (e.g. `qwen/qwen3.5-9b`); MarketingClaw model refs
...
```bash
marketingclaw onboard --non-interactive --accept-risk --auth-choice lmstudio
```
...
servers; omit it for unauthenticated servers and MarketingClaw stores a local non-secret marker instead.
...
LM Studio doesn't always emit an OpenAI-shaped `usage` object on streamed responses. MarketingClaw
...
When LM Studio's `/api/v1/models` discovery reports model-specific reasoning options, MarketingClaw
...
"on"]`) while rejecting those literal values on `/v1/chat/completions`; MarketingClaw normalizes that
...
LM Studio supports just-in-time (JIT) model loading, loading models on first request. MarketingClaw
...
disable MarketingClaw's preload step:
