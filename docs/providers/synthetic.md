---
summary: "Use Synthetic's Anthropic-compatible API in MarketingClaw"
read_when:
...
MarketingClaw bundles it as the `synthetic` provider and uses the Anthropic
Messages API.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    marketingclaw onboard --auth-choice synthetic-api-key
    ```
  </Step>
...
<Warning>
MarketingClaw's Anthropic client appends `/v1` to the base URL automatically, so use
...
`marketingclaw models list --provider synthetic` to see all models available on your
account.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Base URL override">
    ...
    MarketingClaw still appends `/v1` automatically.
