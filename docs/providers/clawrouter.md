...
read_when:
  ...
  - You need ClawRouter model discovery or quota reporting in MarketingClaw
---

ClawRouter gives MarketingClaw one policy-scoped key for multiple upstream model
...
the key's budget and aggregate usage on MarketingClaw usage surfaces.
...
MarketingClaw host. The plugin ships bundled with MarketingClaw (`enabledByDefault: true`);
you only need an issued ClawRouter credential.

| Property      | Value                                    |
| ------------- | ---------------------------------------- |
| Provider      | `clawrouter`                             |
| Plugin        | bundled (included in MarketingClaw)      |
| Auth          | `CLAWROUTER_API_KEY`                     |
| Default URL   | `https://clawrouter.marketingclaw.ai`    |
...
<Steps>
  ...
  <Step title="Configure MarketingClaw">
    ...
    marketingclaw onboard --auth-choice clawrouter-api-key
    marketingclaw plugins enable clawrouter
    ```
    ...
    ClawRouter origin; the default is `https://clawrouter.marketingclaw.ai`.

  </Step>
  <Step title="List granted models">
    ```bash
    marketingclaw models list --all --provider clawrouter
    ```
  ...
  <Step title="Select a model">
    ```bash
    marketingclaw models set clawrouter/<provider>/<model>
    ```

    You can also select a returned model for one run with
    `marketingclaw agent --model clawrouter/<provider>/<model> --message "..."`.
...
supported request routes. MarketingClaw does not ship a second, fixed list of
ClawRouter models. A catalog model is advertised as an MarketingClaw model when:
...
Adding a model to a supported ClawRouter provider needs no MarketingClaw release:
...
ClawRouter owns upstream credentials; its catalog tells MarketingClaw which
transport to use, so you never install every upstream company's auth plugin.

| Catalog capability / route                               | MarketingClaw transport |
| -------------------------------------------------------- | ----------------------- |
...
unsupported request format is intentionally not advertised as an MarketingClaw
...
ClawRouter's `/v1/usage` response feeds the normal MarketingClaw provider-usage
...
```bash
marketingclaw status --usage
marketingclaw models status
```

The same provider snapshot is available to `/status` in chat and MarketingClaw's
...
| `401` or `403` from catalog or usage     | Reissue or re-scope the ClawRouter credential; MarketingClaw does not fall back to upstream provider keys.                                     |
...
<CardGroup cols={2}>
  ...
  <Card title="Usage tracking" href="/concepts/usage-tracking" icon="chart-line">
    MarketingClaw usage and status surfaces.
  </Card>
</CardGroup>
