...
read_when:
  - You want to use Featherless AI with MarketingClaw
...
OpenAI-compatible API. MarketingClaw installs Featherless as an official external
...
| Package         | `@marketingclaw/featherless-provider`    |
...
```bash
marketingclaw onboard --auth-choice featherless-api-key
```
...
```bash
marketingclaw models list --provider featherless
```

## Default model

The plugin uses `Qwen/Qwen3-32B` as the setup default because Featherless
documents native tool calling for the Qwen 3 family. MarketingClaw configures its
...
modes and MarketingClaw does not embed account-specific plan or request-pricing
rates.
...
MarketingClaw deliberately does not copy Featherless's full public model index into
...
- Managed Gateway cannot see the key: put it in `~/.marketingclaw/.env` or another
