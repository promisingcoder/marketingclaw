...
summary: "Fleet supervision plan for Codex app-server sessions controlled by MarketingClaw."
read_when:
  - Designing Codex fleet supervision
  - Building MarketingClaw tools that read, steer, or spawn Codex sessions
...
Claw Supervisor lets one always-on MarketingClaw instance monitor and drive a fleet of Codex sessions without changing the normal Codex user experience. A user can SSH into a host, start Codex, work in the TUI, and still have the supervisor read the session, steer it, interrupt it, spawn related sessions, and accept handoffs. Codex sessions can also call back into MarketingClaw through MCP.

## Product Model

Codex remains the primary work surface. MarketingClaw supervises Codex rather than hiding Codex inside an opaque MarketingClaw subagent.

The MarketingClaw plugin is named `codex-supervisor`. `crabfleet` remains the deployment
...
- Supervisor Claw: an always-on MarketingClaw agent with tools for fleet state, transcript reads, steering, interruption, spawning, and handoff.

MarketingClaw may use its existing subagent machinery internally, but the external contract is an attachable Codex session with a Codex thread id.
  ...
  -> codex --remote unix://... or ws://...
      -> local codex app-server daemon
          <-> host sidecar / supervisor connector
              <-> MarketingClaw fleet supervisor
...
- MarketingClaw tools for Claw-to-Codex control.
...
```text
Control UI or marketingclaw codex sessions
...
Every supervised Codex gets an MCP server named `marketingclaw-codex-supervisor`.
...
- Supervisor state lives in MarketingClaw Gateway storage.
...
- Transcript reads redact secrets at MarketingClaw boundary.
...
Phase 2: MarketingClaw integration
...
- Durable state owner: MarketingClaw Gateway DB, Cloudflare Durable Object, or VPS database.
