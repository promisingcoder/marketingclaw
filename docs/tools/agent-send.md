...
`marketingclaw agent` runs a single agent turn from the command line without an
...
<Steps>
  <Step title="Run a simple agent turn">
    ```bash
    marketingclaw agent --agent main --message "What is the weather today?"
    ```
  ...
  <Step title="Target a specific agent or session">
    ...
    marketingclaw agent --to +15555550123 --message "Status update"

    # Reuse an existing session
    marketingclaw agent --session-id abc123 --message "Continue the task"
  ...
  <Step title="Deliver the reply to a channel">
    ...
    marketingclaw agent --to +15555550123 --message "Report ready" --deliver

    # Deliver to Slack
    marketingclaw agent --agent ops --message "Generate report" \
...
marketingclaw agent --agent ops --message "Alert" --deliver --reply-channel telegram --reply-to "@admin"
```
...
<CardGroup cols={2}>
  <Card title="Agent CLI reference" href="/cli/agent" icon="terminal">
    Full `marketingclaw agent` flag and option reference.
  </Card>
