...
MarketingClaw supports MiniMax as a `web_search` provider through the MiniMax
...
MarketingClaw also accepts `MINIMAX_CODING_API_KEY`, `MINIMAX_OAUTH_TOKEN`, and
...
If `plugins.entries.minimax.config.webSearch.region` is unset, MarketingClaw resolves
...
| `count`   | integer | 1-10, default 5 | Number of results to return. MarketingClaw trims the returned list to this size. |
