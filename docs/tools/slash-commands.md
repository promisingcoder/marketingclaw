...
the MarketingClaw command handler, and `/status` plus `/unfocus` stay local whenever
...
<ParamField path="commands.config" type="boolean" default="false">
  Enables `/config` (reads/writes `marketingclaw.json`). Owner-only.
</ParamField>

<ParamField path="commands.mcp" type="boolean" default="false">
  Enables `/mcp` (reads/writes MarketingClaw-managed MCP config under `mcp.servers`). Owner-only.
</ParamField>
...
<AccordionGroup>
  ...
  <Accordion title="Owner-only writes and admin">
    ...
    | --- | --- | --- |
    | `/config show\|get\|set\|unset` | `commands.config: true` | Read or write `marketingclaw.json`. Owner-only |
    | `/mcp show\|get\|set\|unset` | `commands.mcp: true` | Read or write MarketingClaw-managed MCP server config. Owner-only |
    ...
    | `/restart` | `commands.restart: true` (default) | Restart MarketingClaw |
...
/config set messages.responsePrefix="[marketingclaw]"
...
`/mcp` stores config in MarketingClaw config, not embedded-agent project settings.
...
<AccordionGroup>
  ...
  <Accordion title="Slack specifics">
    `channels.slack.slashCommand` supports a single `/marketingclaw`-style command.
...
- **Execution vs runtime:** `/status` reports `Execution` for the effective sandbox path and `Runtime` for who is running the session: `MarketingClaw Default`, `OpenAI Codex`, a CLI backend, or an ACP backend.
