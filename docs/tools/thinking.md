...
- Provider notes:
  ...
  - OpenAI GPT models map `/think` through model-specific Responses API effort support. `/think off` sends `reasoning.effort: "none"` only when the target model supports it; otherwise MarketingClaw omits the disabled reasoning payload instead of sending an unsupported value.
  ...
  - Moonshot Kimi K2.7 Code (`moonshot/kimi-k2.7-code`) always thinks. Its profile exposes only `on`, and MarketingClaw omits the outbound `thinking` field as required by Moonshot. Other `moonshot/*` models map `/think off` to `thinking: { type: "disabled" }` and any non-`off` level to `thinking: { type: "enabled" }`. When thinking is enabled, Moonshot only accepts `tool_choice` `auto|none`; MarketingClaw normalizes incompatible values to `auto`.
...
- **Embedded MarketingClaw**: the resolved level is passed to the in-process MarketingClaw agent runtime.
...
- MarketingClaw resolves fast mode in this order:
...
- For Codex-backed `openai/*` / `openai-codex/*` models, fast mode sends the same `service_tier=priority` flag on Codex Responses. Native Codex app-server turns receive the tier only on `turn/start` or thread start/resume, so `auto` cannot retier one already-running app-server turn; it applies to the next model turn MarketingClaw starts.
...
- Explicit Anthropic `serviceTier` / `service_tier` model params override the fast-mode default when both are set. MarketingClaw still skips Anthropic service-tier injection for non-Anthropic proxy base URLs.
...
- Provider plugins that proxy Claude models should reuse `resolveClaudeThinkingProfile(modelId)` from `marketingclaw/plugin-sdk/provider-model-shared` so direct Anthropic and proxy catalogs stay aligned.
