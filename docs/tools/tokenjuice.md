...
title: "Tokenjuice"
read_when:
  - You want shorter `exec` or `bash` tool results in MarketingClaw
...
Today this applies to MarketingClaw embedded runs and MarketingClaw dynamic tools in the Codex
app-server harness. Tokenjuice hooks MarketingClaw's tool-result middleware and
...
```bash
marketingclaw plugins install clawhub:@marketingclaw/tokenjuice
```
...
```bash
marketingclaw config set plugins.entries.tokenjuice.enabled false
```

Or:

```bash
marketingclaw plugins disable tokenjuice
```
