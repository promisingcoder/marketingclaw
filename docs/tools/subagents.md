...
<AccordionGroup>
  <Accordion title="Non-blocking, push-based completion">
    ...
    - On completion, MarketingClaw best-effort closes tracked browser tabs/processes opened by that sub-agent session before the announce cleanup flow continues.

  </Accordion>
  <Accordion title="Completion delivery">
    - MarketingClaw hands completions back to the requester session through an `agent` turn with a stable idempotency key.
    - If the requester run is still active, MarketingClaw first tries to wake/steer that run instead of starting a second visible reply path.
    - If an active requester cannot be woken, MarketingClaw falls back to a requester-agent handoff with the same completion context instead of dropping the announce.
    ...
    - Delivery keeps the resolved requester route: thread-bound or conversation-bound completion routes win when available. If the completion origin only provides a channel, MarketingClaw fills the missing target/account from the requester session's resolved route (`lastChannel` / `lastTo` / `lastAccountId`) so direct delivery still works.
  ...
  <Accordion title="Modes and ACP runtime">
    ...
    - MarketingClaw hides `runtime: "acp"` until ACP is enabled, the requester is not sandboxed, and a backend plugin such as `acpx` is loaded. `runtime: "acp"` expects an external ACP harness id, or an `agents.list[]` entry with `runtime.type="acp"`; use the default sub-agent runtime for normal MarketingClaw config agents from `agents_list`.
...
- **Run timeout:** MarketingClaw uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise it falls back to `0` (no timeout). `sessions_spawn` does not accept per-call timeout overrides.
...
When active children exist, MarketingClaw injects a compact runtime-generated
...
<Steps>
  ...
  <Step title="Bind">
    MarketingClaw creates or binds a thread to that session target in the active channel.
  </Step>
...
model and embedded runtime metadata so callers can distinguish MarketingClaw, Codex
...
and `agents_list` omits it. Run `marketingclaw doctor --fix` to clean stale
...
- If a nested requester subagent session is gone, MarketingClaw falls back to that session's requester when available.
...
target agent first. After that, MarketingClaw applies the sub-agent restriction
layer.
...
MarketingClaw does not treat `endedAt` absence as permanent proof that a
...
rapid re-wedge window, MarketingClaw persists a recovery tombstone on that
session and stops auto-resuming it on later restarts. Run
`marketingclaw tasks maintenance --apply` to reconcile the task record, or
`marketingclaw doctor --fix` to clear stale aborted recovery flags on
tombstoned sessions.
