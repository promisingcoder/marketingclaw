...
read_when:
  ...
  - Debugging why marketingclaw is interfering with your own Chrome
  - Implementing browser settings + lifecycle in the macOS app
title: "Browser (MarketingClaw-managed)"
---

MarketingClaw can run a **dedicated Chrome/Brave/Edge/Chromium profile** that the agent controls. It runs through a small local control service inside the Gateway (loopback only) and is isolated from your personal browser.

- Think of it as a **separate, agent-only browser**. The `marketingclaw` profile never touches your personal browser profile.
...
- A separate browser profile named **marketingclaw** (orange accent by default).
...
- A bundled `browser-automation` skill that teaches agents the snapshot,
  ...
  plugin is enabled.
- Optional multi-profile support (`marketingclaw`, `work`, `remote`, ...).
...
marketingclaw browser --browser-profile marketingclaw doctor --deep
marketingclaw browser --browser-profile marketingclaw status
...
If `marketingclaw browser` is missing entirely, or the agent says the browser tool
...
Defaults need both `plugins.entries.browser.enabled` **and** `browser.enabled=true`. Disabling only the plugin removes the `marketingclaw browser` CLI, `browser.request` gateway method, agent tool, and control service as one unit; your `browser.*` config stays intact for a replacement.
...
If `marketingclaw browser` is unknown after an upgrade, `browser.request` is missing, or the agent reports the browser tool as unavailable, the usual cause is a `plugins.allow` list that omits `browser` and no root `browser` config block exists. Add it:
...
## Profiles: `marketingclaw`, `user`, `chrome`

- `marketingclaw`: managed, isolated browser (no extension required).
- `user`: built-in Chrome DevTools MCP attach profile for your **real
  ...
  prompt the first time MarketingClaw attaches, so someone must be at the computer.
- `chrome`: built-in [Chrome extension](/tools/chrome-extension) profile for
  ...
  desk because it drives tabs through the MarketingClaw browser extension instead of
  the remote-debugging port, so there is no "Allow remote debugging?" prompt.

For agent browser tool calls:

- Default: use the isolated `marketingclaw` browser.
...
Set `browser.defaultProfile: "marketingclaw"` if you want managed mode by default.

## Configuration

Browser settings live in `~/.marketingclaw/marketingclaw.json`.
...
but MarketingClaw does not automatically attach them to channel replies. To share a
...
understanding model is configured, MarketingClaw keeps the normal image result so the
...
- Control service binds to loopback on a port derived from `gateway.port` (default `18791` = gateway + 2). `MARKETINGCLAW_GATEWAY_PORT` takes priority over `gateway.port`; either shifts the derived ports in the same family.
- Local `marketingclaw` profiles auto-assign `cdpPort`/`cdpUrl` from a range starting 9 ports above the control port (default `18800`-`18899`); set those only for
...
- Repeated managed Chrome launch/readiness failures are circuit-broken per
  profile. After several consecutive failures, MarketingClaw pauses new launch
...
- Gateway/provider `HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY`, and `NO_PROXY` environment variables do not automatically proxy the MarketingClaw-managed browser. Managed Chrome launches direct by default so provider proxy settings do not weaken browser SSRF checks.
- MarketingClaw-managed local CDP readiness probes and DevTools WebSocket connections bypass the managed network proxy for the exact launched loopback endpoint, so `marketingclaw browser start` still works when an operator proxy blocks loopback egress.
...
- `POST /start?headless=true` and `marketingclaw browser start --headless` request a
  ...
  remote CDP profiles reject the override because MarketingClaw does not launch those
  browser processes.
- On Linux hosts without `DISPLAY` or `WAYLAND_DISPLAY`, local managed profiles
  default to headless automatically when neither the environment nor profile/global
  config explicitly chooses headed mode. `marketingclaw browser status --json`
  reports `headlessSource` as `env`, `profile`, `config`,
  `request`, `linux-display-fallback`, or `default`.
- `MARKETINGCLAW_BROWSER_HEADLESS=1` forces local managed launches headless for the
  current process. `MARKETINGCLAW_BROWSER_HEADLESS=0` forces headed mode for ordinary
...
- Default profile is `marketingclaw` (managed standalone). Use `defaultProfile: "user"` to opt into the signed-in user browser.
...
- `driver: "extension"` drives your signed-in Chrome through the [MarketingClaw Chrome extension](/tools/chrome-extension). The relay owns its loopback endpoint, so these profiles do not accept `cdpUrl`. This is the only signed-in-browser mode that works with nobody at the computer.
...
MarketingClaw uses it automatically. Set `browser.executablePath` to override
...
Per-profile `executablePath` only affects local managed profiles that MarketingClaw
...
- **Remote CDP:** set `browser.profiles.<name>.cdpUrl` (or `browser.cdpUrl`) to
  attach to a remote Chromium-based browser. In this case, MarketingClaw will not launch a local browser.
- For externally managed CDP services on loopback (for example Browserless in
  ...
  without `attachOnly` is treated as a local MarketingClaw-managed browser profile.
- `headless` only affects local managed profiles that MarketingClaw launches. It does not restart or change existing-session or remote CDP browsers.
...
- local managed profiles: `marketingclaw browser stop` stops the browser process that
  MarketingClaw launched
- attach-only and remote CDP profiles: `marketingclaw browser stop` closes the active
  ...
  though no browser process was launched by MarketingClaw
...
MarketingClaw preserves the auth when calling `/json/*` endpoints and when connecting
to the CDP WebSocket. Prefer environment variables or secrets managers for
tokens instead of committing them to config files.

## Node browser proxy (zero-config default)

If you run a **node host** on the machine that has your browser, MarketingClaw can
...
- If you set `nodeHost.browserProxy.allowProfiles`, MarketingClaw treats it as a least-privilege boundary limiting which profile names the proxy will target.
...
CDP connection URLs over HTTPS and WebSocket. MarketingClaw can use either form, but
...
- If Browserless gives you an HTTPS base URL, you can either convert it to
  `wss://` for a direct CDP connection or keep the HTTPS URL and let MarketingClaw
  discover `/json/version`.

### Browserless Docker on the same host

When Browserless is self-hosted in Docker and MarketingClaw runs on the host, treat
...
MarketingClaw process. Browserless must also advertise a matching reachable endpoint;
set Browserless `EXTERNAL` to that same public-to-MarketingClaw WebSocket base, such
...
an address MarketingClaw cannot reach, CDP HTTP can look healthy while the WebSocket
attach still fails.

Do not leave `attachOnly` unset for a loopback Browserless profile. Without
`attachOnly`, MarketingClaw treats the loopback port as a local managed browser
profile and may report that the port is in use but not owned by MarketingClaw.

## Direct WebSocket CDP providers

Some hosted browser services expose a **direct WebSocket** endpoint rather than
the standard HTTP-based CDP discovery (`/json/version`). MarketingClaw accepts three
CDP URL shapes and picks the right connection strategy automatically:

- **HTTP(S) discovery** - `http://host[:port]` or `https://host[:port]`.
  MarketingClaw calls `/json/version` to discover the WebSocket debugger URL, then
...
- **Direct WebSocket endpoints** - `ws://host[:port]/devtools/<kind>/<id>` or
  ...
  path. MarketingClaw connects directly via a WebSocket handshake and skips
...
- **Bare WebSocket roots** - `ws://host[:port]` or `wss://host[:port]` with no
  ...
  [Browserbase](https://www.browserbase.com)). MarketingClaw tries HTTP
  `/json/version` discovery first (normalising the scheme to `http`/`https`);
  if discovery returns a `webSocketDebuggerUrl` it is used, otherwise MarketingClaw
  ...
  accepts it, MarketingClaw falls back to that root as well. This lets a bare `ws://`
...
`marketingclaw browser doctor` uses the same discovery-first, WebSocket-fallback
...
- The standalone loopback browser HTTP API uses **shared-secret auth only**:
  gateway token bearer auth, `x-marketingclaw-password`, or HTTP Basic auth with the
...
- If browser control is enabled and no shared-secret auth is configured, MarketingClaw
...
- Configure `gateway.auth.token`, `gateway.auth.password`, `MARKETINGCLAW_GATEWAY_TOKEN`, or
  `MARKETINGCLAW_GATEWAY_PASSWORD` explicitly if you want a stable secret you control
...
MarketingClaw supports multiple named profiles (routing configs). Profiles can be:

- **marketingclaw-managed**: a dedicated Chromium-based browser instance with its own user data directory + CDP port
- **remote**: an explicit CDP URL (Chromium-based browser running elsewhere)
- **existing session**: your existing Chrome profile via Chrome DevTools MCP auto-connect

Defaults:

- The `marketingclaw` profile is auto-created if missing.
...
MarketingClaw can also attach to a running Chromium-based browser profile through the
...
3. Keep the browser running and approve the connection prompt when MarketingClaw attaches.
...
```bash
marketingclaw browser --browser-profile user start
marketingclaw browser --browser-profile user status
marketingclaw browser --browser-profile user tabs
marketingclaw browser --browser-profile user snapshot --format ai
```
...
- `marketingclaw doctor` migrates old extension-based browser config and checks that
...
- This path is higher-risk than the isolated `marketingclaw` profile because it can
  act inside your signed-in browser session.
- MarketingClaw does not launch the browser for this driver; it only attaches.
- MarketingClaw uses the official Chrome DevTools MCP `--autoConnect` flow here. If
...
When `cdpUrl` is set on an existing-session profile, MarketingClaw skips
...
Compared to the managed `marketingclaw` profile, existing-session drivers are more constrained:
...
- **Dialog visibility** - Managed browser action responses include `blockedByDialog` and `browserState.dialogs.pending` when an action opens a modal dialog; snapshots also include pending dialog state. Respond with `browser dialog --accept/--dismiss --dialog-id <id>` while a dialog is pending. Dialogs handled outside MarketingClaw appear under `browserState.dialogs.recent`.
...
When launching locally, MarketingClaw picks the first available:
...
control API** plus a matching `marketingclaw browser` CLI (snapshots, refs, wait
...
- **CDP startup or readiness failure** means MarketingClaw cannot confirm that the browser control plane is healthy.
...
- CDP startup or readiness failure:
  - `Chrome CDP websocket for profile "marketingclaw" is not reachable after start`
  - `Remote CDP for profile "<name>" is not reachable at <cdpUrl>`
  - `Port <port> is in use for profile "<name>" but not by marketingclaw` when a
...
- For the local loopback `marketingclaw` managed profile, CDP health checks intentionally skip browser SSRF reachability enforcement for MarketingClaw's own local control plane.
...
- `browser` accepts:
  - `profile` to choose a named browser profile (marketingclaw, chrome, or remote CDP).
