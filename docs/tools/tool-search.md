---
summary: "Tool Search: compact large MarketingClaw tool catalogs behind search, describe, and call"
title: "Tool Search"
read_when:
  - You want MarketingClaw agents to use a large tool catalog without adding every tool schema to the prompt
  - You want MarketingClaw tools, MCP tools, and client tools exposed through one compact runtime surface
  - You are implementing or debugging tool discovery for MarketingClaw runs
---

Tool Search is an experimental MarketingClaw agent runtime feature. It gives agents one
...
This page documents MarketingClaw Tool Search. It is not the Codex-native tool
...
When enabled for MarketingClaw runs, the model receives one `tool_search_code` tool
by default. That tool runs a short JavaScript body in an isolated Node
subprocess with an `marketingclaw.tools` bridge:

```js
const hits = await marketingclaw.tools.search("create a GitHub issue");
const tool = await marketingclaw.tools.describe(hits[0].id);
return await marketingclaw.tools.call(tool.id, {
  title: "Crash on startup",
  body: "Steps to reproduce...",
});
```

The catalog can include MarketingClaw tools, plugin tools, MCP tools, and
...
needs the exact schema, and calls that tool through MarketingClaw.

Codex harness runs do not receive these experimental MarketingClaw Tool Search
controls. MarketingClaw passes product capabilities to Codex as dynamic tools, and
Codex owns the stable native code mode, native tool search, deferred dynamic
tools, and nested tool calls.

## How a turn runs

At planning time the MarketingClaw embedded runner builds the effective catalog for the
run:

1. Resolve the active tool policy for the agent, profile, sandbox, and session.
2. List eligible MarketingClaw and plugin tools.
...
6. Expose the MarketingClaw code bridge, the structured fallback tools, or the
   compact directory surface to the model.

At execution time every real tool call returns to MarketingClaw. The isolated Node
runtime does not hold plugin implementations, MCP client objects, or secrets.
`marketingclaw.tools.call(...)` crosses the bridge back into the Gateway, where the
...
- `directory`: exposes `tool_search`, `tool_describe`, and `tool_call` plus a
  ...
  providers that should see tool names without every full schema. MarketingClaw can
  also expose a small bounded set of likely or required tool schemas directly
  for the current turn.

All modes use the same policy-filtered catalog and normal MarketingClaw execution
...
for the current run while MarketingClaw tools, plugin tools, and MCP tools can be
...
All modes are experimental. Prefer direct tool exposure for small MarketingClaw tool
...
catalog includes eligible MarketingClaw, MCP, and client tools after normal policy
filtering.
...
`marketingclaw.tools.search(query, options?)`

Searches the effective catalog for the current run. Results are compact and safe
to put back into prompt context.

```js
const hits = await marketingclaw.tools.search("calendar event", { limit: 5 });
```

`marketingclaw.tools.describe(id)`

Loads full metadata for one search result, including the exact input schema.

```js
const calendarCreate = await marketingclaw.tools.describe("mcp:calendar:create_event");
```

`marketingclaw.tools.call(id, args)`

Calls a selected tool through MarketingClaw.

```js
await marketingclaw.tools.call(calendarCreate.id, {
  summary: "Planning",
...
the model requests an exact hidden directory tool name directly, MarketingClaw
hydrates it from the authorized catalog before normal execution.
Directory-mode client tool names must not collide with MarketingClaw, plugin, or MCP
...
network grants, and no child-process or worker grants. MarketingClaw enforces a
...
- `marketingclaw.tools.search`
- `marketingclaw.tools.describe`
- `marketingclaw.tools.call`

Normal MarketingClaw behavior still applies to final calls:
...
Enable Tool Search for MarketingClaw runs with the default code bridge:

```bash
marketingclaw config set tools.toolSearch true
```
...
Use the structured fallback tools instead for MarketingClaw runs:
...
Use the compact directory surface instead for MarketingClaw runs:
...
- final tool calls executed through MarketingClaw
...
- whether the result came from MarketingClaw, MCP, or a client tool

## E2E validation

The QA Lab gateway scenario proves both paths with the MarketingClaw runtime:

```bash
pnpm marketingclaw qa suite --provider-mode mock-openai --scenario tool-search-gateway-e2e
```
