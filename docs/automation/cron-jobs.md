...
read_when:
  ...
  - Wiring external triggers (webhooks, Gmail) into MarketingClaw
...
<Steps>
  <Step title="Add a one-shot reminder">
    ```bash
    marketingclaw cron create "2027-02-01T16:00:00Z" \
      --name "Reminder" \
      --session main \
  ...
  <Step title="Check your jobs">
    ...
    marketingclaw cron get <job-id>
    marketingclaw cron show <job-id>
    ```
  </Step>
  <Step title="See run history">
    ```bash
    marketingclaw cron runs --id <job-id>
    ```
  </Step>
</Steps>
...
- Job definitions, runtime state, and run history persist in MarketingClaw's shared SQLite state database, so restarts do not lose schedules.
...
- If you drive `marketingclaw agent` from system cron or another external scheduler, wrap it with a hard-kill escalation even though the CLI already handles `SIGTERM`/`SIGINT`. Gateway-backed runs ask the Gateway to abort accepted runs; local and embedded fallback runs get the same abort signal. For GNU `timeout`, prefer `timeout -k 60 600 marketingclaw agent ...` over plain `timeout 600 ...` — the `-k` value is the backstop if the process cannot drain in time. For systemd units, use a `SIGTERM` stop signal with a grace window (`TimeoutStopSec`) before the final kill. Reusing a `--run-id` while the original Gateway run is still active reports the duplicate as in-flight instead of starting a second run.
...
<AccordionGroup>
  <Accordion title="Isolated run hardening">
    ...
    - Isolated runs guard against stale acknowledgement replies: if the first result is only an interim status update (`on it`, `pulling everything together`, and similar hints) and no descendant subagent is still responsible for the final answer, MarketingClaw re-prompts once for the actual result before delivery.
...
<AccordionGroup>
  ...
  <Accordion title="What 'fresh session' means for isolated jobs">
    A new transcript/session id per run. MarketingClaw carries safe preferences (thinking/fast/verbose settings, labels, explicit user-selected model/auth overrides), but does not inherit ambient conversation context from an older cron row: channel/group routing, send or queue policy, elevation, origin, or ACP runtime binding. Use `current` or `session:<id>` when a recurring job should deliberately build on the same conversation context.
  </Accordion>
  <Accordion title="Subagent and Discord delivery">
    When isolated cron runs orchestrate subagents, delivery prefers the final descendant output over stale parent interim text. If descendants are still running, MarketingClaw suppresses that partial parent update instead of announcing it.

    For text-only Discord announce targets, MarketingClaw sends the canonical final assistant text once instead of replaying both streamed/intermediate text and the final answer. Media and structured Discord payloads are still delivered separately so attachments and components are not dropped.
...
Use `--announce --channel telegram --to "-1001234567890"` for channel delivery. For Telegram forum topics, use `-1001234567890:topic:123`; MarketingClaw also accepts the Telegram-owned `-1001234567890:123` shorthand. Direct RPC/config callers may pass `delivery.threadId` as a string or number. Slack/Discord/Mattermost targets use explicit prefixes (`channel:<id>`, `user:<id>`). Matrix room IDs are case-sensitive; use the exact room ID or `room:!room:server` form from Matrix.
...
For isolated jobs, chat delivery is shared: if a chat route is available, the agent can use the `message` tool even with `--no-deliver`. If the agent sends to the configured/current target, MarketingClaw skips the fallback announce. Otherwise `announce`, `webhook`, and `none` only control what the runner does with the final reply after the agent turn.

When an agent creates an isolated reminder from an active chat, MarketingClaw stores the preserved live delivery target for the fallback announce route. Internal session keys may be lowercase; provider delivery targets are not reconstructed from those keys when current chat context is available.
...
- `marketingclaw cron edit` exposes per-job alert tuning: `--failure-alert`/`--no-failure-alert`, `--failure-alert-after <n>`, `--failure-alert-channel`, `--failure-alert-to`, `--failure-alert-cooldown`, `--failure-alert-include-skipped`/`--failure-alert-exclude-skipped`, `--failure-alert-mode`, and `--failure-alert-account-id`.
...
<Tabs>
  ...
  <Tab title="Webhook output">
    ```bash
    marketingclaw cron create "0 18 * * 1-5" \
      "Summarize today's deploys as JSON." \
      --name "Deploy digest" \
      --webhook "https://example.invalid/marketingclaw/cron"
    ```
  </Tab>
...
marketingclaw cron get <jobId>

# Show one job, including resolved delivery route
marketingclaw cron show <jobId>

# Enable/disable without deleting
marketingclaw cron enable <jobId>
marketingclaw cron disable <jobId>

# Edit a job
marketingclaw cron edit <jobId> --message "Updated prompt" --model "opus"

# Force run a job now
marketingclaw cron run <jobId>

# Force run a job now and wait for its terminal status
marketingclaw cron run <jobId> --wait --wait-timeout 10m --poll-interval 2s

# Run only if due
marketingclaw cron run <jobId> --due

# View run history
marketingclaw cron runs --id <jobId> --limit 50

# View one exact run
marketingclaw cron runs --id <jobId> --run-id <runId>

# Delete a job
marketingclaw cron remove <jobId>

# Agent selection (multi-agent setups)
marketingclaw cron create "0 6 * * *" "Check ops queue" --name "Ops sweep" --session isolated --agent ops
marketingclaw cron edit <jobId> --clear-agent
```

`marketingclaw cron run <jobId>` returns after enqueueing the manual run. Use `--wait` for shutdown hooks, maintenance scripts, or other automation that must block until the queued run finishes; it polls the returned `runId` (default timeout `10m`, poll interval `2s`) and exits `0` for status `ok`, non-zero for `error`, `skipped`, or a wait timeout.
...
- `marketingclaw cron edit <job-id> --clear-model` clears that override from the CLI (same effect as the `model: null` patch) and cannot combine with `--model`.
- Configured fallback chains still apply because cron `--model` is a job primary, not a session `/model` override.
- `marketingclaw cron add|edit --fallbacks ...` sets payload `fallbacks`, replacing configured fallbacks for that job; `--fallbacks ""` disables fallback and makes the run strict. `marketingclaw cron edit <job-id> --clear-fallbacks` clears the per-job override.
...
Wire Gmail inbox triggers to MarketingClaw via Google PubSub.

<Note>
**Prerequisites:** `gcloud` CLI, `gog` (gogcli), MarketingClaw hooks enabled, Tailscale for the public HTTPS endpoint.
</Note>

### Wizard setup (recommended)

```bash
marketingclaw webhooks gmail setup --account marketingclaw@gmail.com
```
...
When `hooks.enabled=true` and `hooks.gmail.account` is set, the Gateway starts `gog gmail watch serve` on boot and auto-renews the watch. Set `MARKETINGCLAW_SKIP_GMAIL_WATCHER=1` to opt out.
...
```json5
{
  cron: {
    enabled: true,
    store: "~/.marketingclaw/cron/jobs.json",
    maxConcurrentRuns: 8,
    triggers: {
      enabled: false,
      minIntervalMs: 30000,
...
Disable cron: `cron.enabled: false` or `MARKETINGCLAW_SKIP_CRON=1`.
...
<AccordionGroup>
  ...
  <Accordion title="Legacy store migration">
    On upgrade, run `marketingclaw doctor --fix` to import legacy `~/.marketingclaw/cron/jobs.json`, `jobs-state.json`, and `runs/*.jsonl` files into SQLite and rename them with a `.migrated` suffix. Malformed job rows are skipped from runtime and copied to `jobs-quarantine.json` for later repair or review.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Cron not firing">
    - Check `cron.enabled` and the `MARKETINGCLAW_SKIP_CRON` env var.
    - Confirm the Gateway is running continuously.
    - For `cron` schedules, verify timezone (`--tz`) vs the host timezone.
    - `reason: not-due` in run output means the manual run was checked with `marketingclaw cron run <jobId> --due` and the job was not due yet.
  ...
  <Accordion title="Cron fired but no delivery">
    ...
    - If the isolated run returns only the silent token (`NO_REPLY` / `no_reply`), MarketingClaw suppresses direct outbound delivery and the fallback queued-summary path, so nothing is posted back to chat.
  ...
  <Accordion title="Cron or heartbeat appears to prevent /new-style rollover">
    ...
    - For legacy rows created before those fields existed, MarketingClaw can recover `sessionStartedAt` from the transcript JSONL session header when the file is still available. Legacy idle rows without `lastInteractionAt` use that recovered start time as their idle baseline.
