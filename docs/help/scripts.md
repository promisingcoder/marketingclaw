...
- Prefer CLI surfaces when they exist (example: `marketingclaw models status --check`).
...
- `scripts/claude-auth-status.sh [full|json|simple]` - checks Claude Code + MarketingClaw auth status.
- `scripts/auth-monitor.sh` - polls status and sends a notification (via MarketingClaw send, and/or ntfy.sh) when the token nears expiry. Env: `WARN_HOURS` (default `2`), `NOTIFY_PHONE`, `NOTIFY_NTFY`. Run on a schedule via the bundled `scripts/systemd/marketingclaw-auth-monitor.{service,timer}` (every 30 minutes).
...
- `MARKETINGCLAW_GH_READ_APP_ID`
- `MARKETINGCLAW_GH_READ_PRIVATE_KEY_FILE`

Optional env:

- `MARKETINGCLAW_GH_READ_INSTALLATION_ID` when you want to skip repo-based installation lookup
- `MARKETINGCLAW_GH_READ_PERMISSIONS` as a comma-separated override for the read permission subset to request
...
- `scripts/gh-read run list -R promisingcoder/marketingclaw`
- `scripts/gh-read api repos/promisingcoder/marketingclaw/pulls/123`
