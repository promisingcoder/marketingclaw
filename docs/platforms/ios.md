...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `marketingclaw.internal.`), **or**
...
```bash
marketingclaw gateway --port 18789 --tailscale serve
```
...
Gateway has not been configured yet, run `marketingclaw onboard` first so setup-code
...
If the app retries pairing with changed auth details (role/scopes/public key), the previous pending request is superseded and a new `requestId` is created. Run `marketingclaw devices list` again before approval.
...
Apple's Watch app, install MarketingClaw from **Watch app -> My Watch -> Available
Apps**, then open MarketingClaw once on both devices.
...
MarketingClaw is active, even when the paired iPhone is unavailable.
...
- MarketingClaw is active on the watch. Apple does not allow ordinary watchOS apps to
...
3. Open MarketingClaw on the watch before the short-lived setup code expires.
4. Verify the separate Apple Watch row with `marketingclaw nodes status`.
...
- `MARKETINGCLAW_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway (`gateway.push.apns.relay.baseUrl` is the config-first path).
- The App Store release build's push mode hardcodes the hosted relay host and never reads a relay-URL override — the `MARKETINGCLAW_PUSH_RELAY_BASE_URL` build-time env var only affects local/sandbox iOS build modes.

## Authentication and trust flow

The relay exists to enforce two constraints direct APNs-on-gateway cannot provide for official iOS builds:

- Only genuine MarketingClaw iOS builds distributed through Apple can use the hosted relay.
...
Why this design was created: to keep production APNs credentials out of user gateways, avoid storing raw official-build APNs tokens on the gateway, allow hosted relay usage only for official MarketingClaw iOS builds, and prevent one gateway from sending wake pushes to iOS devices owned by a different gateway.
...
```bash
export MARKETINGCLAW_APNS_TEAM_ID="TEAMID"
export MARKETINGCLAW_APNS_KEY_ID="KEYID"
export MARKETINGCLAW_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
Recommended gateway-host storage, consistent with other provider credentials under `~/.marketingclaw/credentials/`:

```bash
mkdir -p ~/.marketingclaw/credentials/apns
chmod 700 ~/.marketingclaw/credentials/apns
mv /path/to/AuthKey_KEYID.p8 ~/.marketingclaw/credentials/apns/AuthKey_KEYID.p8
chmod 600 ~/.marketingclaw/credentials/apns/AuthKey_KEYID.p8
export MARKETINGCLAW_APNS_PRIVATE_KEY_PATH="$HOME/.marketingclaw/credentials/apns/AuthKey_KEYID.p8"
```
...
The iOS app browses `_marketclaw-gw._tcp` on `local.` and, when configured, the same wide-area DNS-SD discovery domain. Same-LAN gateways appear automatically from `local.`; cross-network discovery can use the configured wide-area domain without changing the beacon type.

### Tailnet (cross-network)

If mDNS is blocked, use a unicast DNS-SD zone (choose a domain; example: `marketingclaw.internal.`) and Tailscale split DNS. See [Bonjour](/gateway/bonjour) for the CoreDNS example.
...
```bash
marketingclaw nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__marketingclaw__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__marketingclaw__/canvas/` and `/__marketingclaw__/a2ui/`, from the Gateway HTTP server (same port as `gateway.port`, default `18789`).
...
The iOS app is a mobile node surface, not a Codex Computer Use backend. Codex Computer Use and `cua-driver mcp` control a local macOS desktop through MCP tools; the iOS app exposes iPhone capabilities through MarketingClaw node commands such as `canvas.*`, `camera.*`, `screen.*`, `location.*`, and `talk.*`.

Agents can still operate the iOS app through MarketingClaw by invoking node commands, but those calls go through the gateway node protocol and follow iOS foreground/background limits. Use [Codex Computer Use](/plugins/codex-computer-use) for local desktop control and this page for iOS node capabilities.

### Canvas eval / snapshot

```bash
marketingclaw nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__marketingclaw; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
marketingclaw nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `marketingclaw devices list` and approve manually.
- Watch shows no iPhone state: confirm the iPhone reports `watchPaired: true`
  ...
  from **My Watch -> Available Apps**. After either change, open MarketingClaw on the
