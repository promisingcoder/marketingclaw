...
Use the CLI flow in [Gateway health](/gateway/health) (`marketingclaw status`,
`marketingclaw status --deep`, `marketingclaw health --json`) and tail
`/tmp/marketingclaw/marketingclaw-*.log`, filtering for `web-heartbeat` / `web-reconnect`.
