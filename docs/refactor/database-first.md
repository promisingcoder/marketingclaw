...
read_when:
  - Moving MarketingClaw runtime data, cache, transcripts, task state, or scratch files into SQLite
...
- Global database: `~/.marketingclaw/state/marketingclaw.sqlite`
...
- Configuration stays file-backed: `marketingclaw.json` remains outside the
  database. Runtime auth profiles move to SQLite; external provider or CLI
  credential files remain owner-managed outside MarketingClaw's database.
...
- Raw stream logging uses `MARKETINGCLAW_RAW_STREAM=1` plus SQLite diagnostics rows.
  The old pi-mono `PI_RAW_STREAM`, `PI_RAW_STREAM_PATH`, and
  `raw-openai-completions.jsonl` file logger contract is not part of MarketingClaw
  runtime or tests.
...
- One global SQLite database owns control-plane state:
  `state/marketingclaw.sqlite`.
- One per-agent SQLite database owns data-plane state:
  `agents/<agentId>/agent/marketingclaw-agent.sqlite`.
- Config remains file-backed. `marketingclaw.json` is not part of this database
  refactor.
...
- Task registry: `clean`. Task and Task Flow runtime rows live in
  `state/marketingclaw.sqlite`; unshipped sidecar SQLite importers are deleted.
...
- [x] Keep Kysely generated types aligned after any schema change.
      Files: `src/state/marketingclaw-state-schema.sql`,
      `src/state/marketingclaw-agent-schema.sql`,
      `src/state/*generated*`.
...
- `marketingclaw doctor --fix` owns the legacy file-to-database migration step.
  Runtime startup and `marketingclaw migrate` should not carry legacy MarketingClaw
...
- The old runtime-owned JSONL transcript streaming helper was deleted. Doctor
  ...
  SQLite rows.
- Codex app-server bindings use the MarketingClaw `sessionId` as the canonical
...
former stores now write to `state/marketingclaw.sqlite` or the owning
`marketingclaw-agent.sqlite` database.
...
- `src/state/marketingclaw-state-db.ts` opens `marketingclaw.sqlite`, sets WAL,
  `synchronous=NORMAL`, `busy_timeout=30000`, `foreign_keys=ON`, and applies
  the generated schema module derived from
  `src/state/marketingclaw-state-schema.sql`.
...
- Plugin migrations are data migrations over plugin-owned namespaces, not host
  ...
  `marketingclaw-state-schema.sql` unless the host itself is taking ownership of a
  new cross-plugin contract.
- `src/state/marketingclaw-agent-db.ts` opens
  `agents/<agentId>/agent/marketingclaw-agent.sqlite`, registers the database in the
...
- Per-agent external conversation identity is relational too:
  ...
  `session_conversations` links one MarketingClaw session to one or more external
  ...
  MarketingClaw session can represent multiple external DM peers without demoting
...
- Secret target metadata now talks about stores instead of pretending every
  credential target is a config file. `marketingclaw.json` remains the config store;
...
- Workspace bootstrap completion markers now live in typed shared
  ...
  `.marketingclaw/workspace-state.json`; runtime no longer reads or rewrites the
  legacy workspace marker, and helper APIs no longer pass around a fake
  `.marketingclaw/setup-state` path just to derive storage identity.
- Exec approvals now live in the typed shared SQLite `exec_approvals_config`
  singleton row. Doctor imports legacy `~/.marketingclaw/exec-approvals.json`;
  ...
  `state/marketingclaw.sqlite` table row; it keeps only the Unix prompt socket on disk
...
- GitHub Copilot compaction no longer writes `marketingclaw-compaction-*.json`
  workspace sidecars. The harness calls the SDK history compaction RPC for the
  tracked SDK session, and MarketingClaw keeps durable session/transcript state in
  SQLite instead of compatibility marker files.
- The shared Swift runtime (`MarketingClawKit`) uses the same
  `state/marketingclaw.sqlite` rows for device identity and device auth. macOS app
...
- Android device identity uses the same TypeScript-compatible key material
  stored in typed `state/marketingclaw.sqlite#table/device_identities` rows. It never
  reads or writes `marketingclaw/identity/device.json`; a leftover legacy file blocks
  startup until doctor imports it into SQLite.
- Android cached device auth tokens also use typed
  `state/marketingclaw.sqlite#table/device_auth_tokens` rows and share the same
...
- Non-doctor commands do not auto-run legacy config repair. For example,
  `marketingclaw update --channel` now fails on invalid legacy config and asks the
...
- Plugin state now uses the shared `state/marketingclaw.sqlite` database. The old
...
- Task and Task Flow runtime tables now live in the shared
  `state/marketingclaw.sqlite` database instead of `tasks/runs.sqlite` and
...
- Session target resolution now exposes per-agent database targets, not legacy
  ...
  `marketingclaw sessions` enumerate `agent_databases` plus configured agents.
...
- Cache trace, Anthropic payload, raw stream, and diagnostics timeline records
  ...
  `diagnostics.cacheTrace.filePath`, `MARKETINGCLAW_CACHE_TRACE_FILE`,
  `MARKETINGCLAW_ANTHROPIC_PAYLOAD_LOG_FILE`, and
  `MARKETINGCLAW_DIAGNOSTICS_TIMELINE_PATH` JSONL override paths are removed, and
...
- Memory Wiki activity logs now use SQLite plugin state instead of
  `.marketingclaw-wiki/log.jsonl`. The Memory Wiki migration provider imports old
  JSONL logs; wiki markdown and user vault content stay file-backed as
  workspace content.
- Memory Wiki no longer creates `.marketingclaw-wiki/state.json` or the unused
  `.marketingclaw-wiki/locks` directory. The migration provider removes those retired
...
- The macOS companion no longer writes app-local `logs/config-audit.jsonl` or
  `logs/config-health.json` sidecars while editing `marketingclaw.json`. The config
...
- Codex app-server conversation bindings now key SQLite plugin state by
  MarketingClaw session key or explicit `{agentId, sessionId}` scope. They must not
...
- Zalo hosted outbound media now uses shared SQLite `plugin_blob_entries`
  instead of `marketingclaw-zalo-outbound-media` JSON/bin temp sidecars.
...
- ACPX generated wrapper scripts and the isolated Codex home are temporary
  materialization under the MarketingClaw temp root, not durable MarketingClaw state. The
...
- Gateway media attachments now use the shared `media_blobs` SQLite table as
  ...
  `$MARKETINGCLAW_STATE_DIR/media` or config-dir `media` roots; those directories are
  doctor import sources only.
- Shell completion no longer writes `$MARKETINGCLAW_STATE_DIR/completions/*` cache
...
- Subagent inline attachments no longer materialize under workspace
  `.marketingclaw/attachments/*`. The spawn path prepares SQLite VFS seed entries,
...
- CLI image hydration no longer maintains stable `marketingclaw-cli-images` cache
...
- `marketingclaw secrets apply` scrubs the config file, env file, and SQLite
...
- User-facing auth docs now describe
  `state/marketingclaw.sqlite#table/auth_profile_stores/<agentDir>` instead of
...
- Memory Wiki imported-source sync ledgers now store one SQLite plugin-state row
  per vault/source key instead of rewriting `.marketingclaw-wiki/source-sync.json`;
...
- Memory Wiki ChatGPT import-run records now store one SQLite plugin-state row
  per vault/run id instead of writing `.marketingclaw-wiki/import-runs/*.json`.
...
- Memory Wiki compiled digests now store SQLite plugin blob rows instead of
  writing `.marketingclaw-wiki/cache/agent-digest.json` and
  `.marketingclaw-wiki/cache/claims.jsonl`. The migration provider imports old cache
...
- ACPX wrapper scripts and the isolated Codex home are generated in the
  MarketingClaw temp root. They are recreated as needed and are not backup or
  migration inputs.
...
- Backup stages the state directory before archiving, copies non-database files,
  ...
  completed backup runs in SQLite with the archive manifest. `marketingclaw backup
create` validates the written archive by default; `--no-verify` is the
  explicit fast path.
- `marketingclaw backup restore` validates the archive before extraction, reuses the
...
- `marketingclaw reset --scope config+creds+sessions` removes per-agent
  `marketingclaw-agent.sqlite` databases plus WAL/SHM sidecars, not only legacy
...
- Generated model catalog config is stored in typed global SQLite
  ...
  `ensureMarketingClawModelCatalog`; there is no `models.json` compatibility API in
...
- Memory-core runtime imports SQLite transcript indexing helpers from
  `marketingclaw/plugin-sdk/memory-core-host-engine-session-transcripts`, not the
...
- QMD's own `index.sqlite` is now a temp runtime materialization backed by the
  ...
  `~/.marketingclaw/agents/<agentId>/qmd` sidecar.
- The optional `memory-lancedb` plugin no longer creates
  `~/.marketingclaw/memory/lancedb` as an implicit MarketingClaw-managed store. It is an
...
`marketingclaw doctor --fix` invokes the state migration implementation after
ordinary config preflight and creates a verified backup before import. Runtime
startup and `marketingclaw migrate` must not import legacy MarketingClaw state files.
...
- Add a single doctor-owned state migration service for file-to-database import.
  Done: `marketingclaw doctor --fix` uses the legacy-state migration implementation.
...
Keep shared coordination state in `state/marketingclaw.sqlite`:
...
```text
~/.marketingclaw/state/marketingclaw.sqlite
~/.marketingclaw/agents/<agentId>/agent/marketingclaw-agent.sqlite
```
...
- Verify by opening every DB snapshot and running `PRAGMA integrity_check`.
  `marketingclaw backup create` does this archive verification by default;
...
5. Verify the archive by opening every included SQLite snapshot and running
   ...
   `marketingclaw backup create` does this by default; `--no-verify` is only for
...
- Use `{ agentId, sessionKey }` row operations.
  ...
  and the `marketingclaw sessions` listing command now read per-agent rows directly
...
8. Backup integration.
   ...
   - Add restore from verified archive snapshots. Done: `marketingclaw backup
...
- Remove tests that assert runtime creation of `sessions.json` or transcript
  ...
  MarketingClaw test-state helper no longer exposes a legacy `auth-profiles.json`
...
- ad-hoc `marketingclaw-state.sqlite` runtime sidecars
- `tasks/runs.sqlite`
...
- Memory Wiki `.marketingclaw-wiki/log.jsonl`
- Memory Wiki `.marketingclaw-wiki/state.json`
- Memory Wiki `.marketingclaw-wiki/locks/`
- Memory Wiki `.marketingclaw-wiki/source-sync.json`
- Memory Wiki `.marketingclaw-wiki/import-runs/*.json`
- Memory Wiki `.marketingclaw-wiki/cache/agent-digest.json`
- Memory Wiki `.marketingclaw-wiki/cache/claims.jsonl`
- ClawHub `.clawhub/lock.json`
- ClawHub `.clawhub/origin.json`
- Browser profile decoration `.marketingclaw-profile-decorated`
