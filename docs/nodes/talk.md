...
## Config (`~/.marketingclaw/marketingclaw.json`)
...
| `consultThinkingLevel`                   | unset                                      | Thinking level override for the agent run behind realtime `marketingclaw_agent_consult` calls.                                                                                                                                                                             |
| `consultFastMode`                        | unset                                      | Fast-mode override for realtime `marketingclaw_agent_consult` calls.                                                                                                                                                                                                       |
...
| `realtime.consultRouting`                | -                                          | `provider-direct` preserves the provider's direct reply when it skips `marketingclaw_agent_consult`; `force-agent-consult` routes finalized user transcripts through MarketingClaw instead.                                                                                |
| `realtime.instructions`                  | -                                          | Appends provider-facing system instructions to MarketingClaw's built-in realtime prompt (voice style/tone); the default `marketingclaw_agent_consult` guidance stays.                                                                                                      |
...
- macOS local MLX playback uses the bundled `marketingclaw-mlx-tts` helper when present, or an executable on `PATH`. Set `MARKETINGCLAW_MLX_TTS_BIN` to point at a custom helper binary during development.
