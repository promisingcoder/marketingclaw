...
`marketingclaw doctor` is the repair and migration tool for MarketingClaw. It fixes stale config/state, checks health, and provides actionable repair steps.
...
<Tabs>
  ...
  <Tab title="--lint">
    ```bash
    marketingclaw doctor --lint
    marketingclaw doctor --lint --json
    ```
  ...
  <Tab title="--fix --force">
    ```bash
    marketingclaw doctor --fix --force
    ```
...
```bash
cat ~/.marketingclaw/marketingclaw.json
```

## Read-only lint mode

`marketingclaw doctor --lint` is the automation-friendly sibling of
`marketingclaw doctor --fix`. They share the same Doctor rule registry, but they do
not select or act on rules in the same way:

| Mode                          | Prompts   | Writes config/state     | Output                 | Use it for                      |
| ----------------------------- | --------- | ----------------------- | ---------------------- | ------------------------------- |
| `marketingclaw doctor`        | yes       | no                      | friendly health report | a human checking status         |
| `marketingclaw doctor --fix`  | sometimes | yes, with repair policy | friendly repair log    | applying approved repairs       |
| `marketingclaw doctor --lint` | no        | no                      | structured findings    | CI, preflight, and review gates |
...
```bash
marketingclaw doctor --lint
marketingclaw doctor --lint --severity-min warning
marketingclaw doctor --lint --json
marketingclaw doctor --lint --all
marketingclaw doctor --lint --only core/doctor/gateway-config --json
```
...
- `--json`, `--severity-min`, `--all`, `--only`, and `--skip` require `--lint`; plain `marketingclaw doctor` and `--fix` runs reject them.
...
<AccordionGroup>
  ...
  <Accordion title="Gateway, services, and supervisors">
    ...
    - Channel-specific permission checks live under `marketingclaw channels capabilities`; for example, Discord voice channel permissions are audited with `marketingclaw channels capabilities --channel discord --target channel:<channel-id>`.
...
The Control UI Dreams scene includes **Backfill**, **Reset**, and **Clear Grounded** actions for the grounded dreaming workflow. These use gateway doctor-style RPC methods but are **not** part of `marketingclaw doctor` CLI repair/migration.
...
<AccordionGroup>
  ...
  <Accordion title="2. Legacy config key migrations">
    When the config contains a deprecated key with an active migration, other commands refuse to run and ask you to run `marketingclaw doctor`. Doctor explains which legacy keys were found, shows the migration it applied, and rewrites `~/.marketingclaw/marketingclaw.json` with the updated schema. Gateway startup refuses legacy config formats and asks you to run `marketingclaw doctor --fix`; it does not rewrite `marketingclaw.json` on startup. Cron job store migrations are also handled by `marketingclaw doctor --fix`.
    ...
    | `agents.defaults/list[].systemPromptOverride`                                                    | removed (MarketingClaw owns the generated system prompt)                        |
    ...
    <Note>
      ...
      the Voice Call plugin itself on every config load, not by `marketingclaw
      doctor`. The plugin also logs a startup warning pointing at `marketingclaw
      doctor --fix`, but doctor does not currently rewrite
      `marketingclaw.json` for these keys; the plugin's own normalization is what
  ...
  <Accordion title="2b. OpenCode provider overrides">
    If you have added `models.providers.opencode`, `opencode-zen`, or `opencode-go` manually, it overrides the built-in OpenCode catalog from `marketingclaw/plugin-sdk/llm`. That can force models onto the wrong API or zero out costs. Doctor warns so you can remove the override and restore per-model API routing + costs.
  </Accordion>
  ...
  <Accordion title="2f. Codex route repair">
    Doctor checks for legacy `openai-codex/*` model refs. Native Codex harness routing uses canonical `openai/*` model refs; OpenAI agent turns go through the Codex app-server harness instead of the MarketingClaw OpenAI provider path.
  ...
  <Accordion title="2g. Session route cleanup">
    ...
    `marketingclaw doctor --fix` can clear auto-created stale state such as `modelOverrideSource: "auto"` model pins, runtime model metadata, pinned harness ids, CLI session bindings, and auto auth-profile overrides when their owning route is no longer configured. Explicit user or legacy session model choices are reported for manual review and left untouched; switch them with `/model ...`, `/new`, or reset the session when that route is no longer intended.

  </Accordion>
  <Accordion title="3. Legacy state migrations (disk layout)">
    Doctor can migrate older on-disk layouts into the current structure:

    - Sessions store + transcripts: from `~/.marketingclaw/sessions/` to `~/.marketingclaw/agents/<agentId>/sessions/`
    - Agent dir: from `~/.marketingclaw/agent/` to `~/.marketingclaw/agents/<agentId>/agent/`
    - WhatsApp auth state (Baileys): from legacy `~/.marketingclaw/credentials/*.json` (except `oauth.json`) to `~/.marketingclaw/credentials/whatsapp/<accountId>/...` (default account id: `default`)

    These migrations are best-effort and idempotent; doctor emits warnings when it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates the legacy sessions + agent dir on startup so history/auth/models land in the per-agent path without a manual doctor run. WhatsApp auth is intentionally only migrated via `marketingclaw doctor`. Talk provider/provider-map normalization compares by structural equality, so key-order-only diffs no longer trigger repeat no-op `doctor --fix` changes.
  ...
  <Accordion title="3b. Legacy cron store migrations">
    Doctor also checks the cron job store (`~/.marketingclaw/cron/jobs.json` by default, or `cron.store` when overridden) for old job shapes that the scheduler still accepts for compatibility.
    ...
    On Linux, doctor also warns when the user's crontab still invokes legacy `~/.marketingclaw/bin/ensure-whatsapp.sh`. That host-local script is not maintained by current MarketingClaw and can write false `Gateway inactive` messages to `~/.marketingclaw/logs/whatsapp-health.log` when cron cannot reach the systemd user bus. Remove the stale crontab entry with `crontab -e`; use `marketingclaw channels status --probe`, `marketingclaw doctor`, and `marketingclaw gateway status` for current health checks.

  </Accordion>
  <Accordion title="3c. Session lock cleanup">
    Doctor scans every agent session directory for stale write-lock files left behind when a session exited abnormally. For each lock file found it reports: the path, PID, whether the PID is still alive, lock age, and whether it is considered stale (dead PID, malformed owner metadata, older than 30 minutes, or a live PID proven to belong to a non-MarketingClaw process). In `--fix` / `--repair` mode it removes locks with dead, orphaned, recycled, malformed-old, or non-MarketingClaw owners automatically. Old locks still owned by a live MarketingClaw process are reported but left in place so doctor does not cut off an active transcript writer.
  </Accordion>
  <Accordion title="3d. Session transcript branch repair">
    Doctor scans agent session JSONL files for the duplicated branch shape created by the 2026.4.24 prompt transcript rewrite bug: an abandoned user turn with MarketingClaw internal runtime context plus an active sibling containing the same visible user prompt. In `--fix` / `--repair` mode, doctor backs up each affected file next to the original and rewrites the transcript to the active branch so gateway history and memory readers no longer see duplicate turns.
  </Accordion>
  <Accordion title="4. State integrity checks (session persistence, routing, and safety)">
    ...
    - **Multiple state dirs**: warns when multiple `~/.marketingclaw` folders exist across home directories, or when `MARKETINGCLAW_STATE_DIR` points elsewhere (history can split between installs).
    - **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run it on the remote host (the state lives there).
    - **Config file permissions**: warns if `~/.marketingclaw/marketingclaw.json` is group/world readable and offers to tighten to `600`.
  ...
  <Accordion title="5. Model auth health (OAuth expiry)">
    ...
    When an OAuth refresh fails permanently (for example `refresh_token_reused`, `invalid_grant`, or a provider telling you to sign in again), doctor reports that re-auth is required and prints the exact `marketingclaw models auth login --provider ...` command to run.
    ...
    Legacy Codex OAuth profiles whose tokens live in macOS Keychain (older onboarding before the file-based sidecar layout) are repaired only by doctor. Run `marketingclaw doctor --fix` once from an interactive terminal to migrate Keychain-backed legacy tokens inline into `auth-profiles.json`; after that, embedded turns (Telegram, cron, sub-agent dispatch) resolve them as canonical OpenAI OAuth profiles.
  ...
  <Accordion title="7b. Plugin install cleanup">
    Doctor removes legacy MarketingClaw-generated plugin dependency staging state in `marketingclaw doctor --fix` / `marketingclaw doctor --repair` mode: stale generated dependency roots, old install-stage directories, package-local debris from earlier bundled-plugin dependency repair code, and orphaned or recovered managed npm copies of bundled `@marketingclaw/*` plugins that can shadow the current bundled manifest. Doctor also relinks the host `marketingclaw` package into managed npm plugins that declare `peerDependencies.marketingclaw`, so package-local runtime imports such as `marketingclaw/plugin-sdk/*` keep resolving after updates or npm repairs.

    Doctor can also reinstall missing downloadable plugins when config references them but the local plugin registry cannot find them (material `plugins.entries`, configured channel/provider/search settings, configured agent runtimes). During package updates, doctor avoids reinstalling plugin packages while the core package is being swapped; run `marketingclaw doctor --fix` again after the update if a configured plugin still needs recovery. Outside the container image startup exception below, gateway startup and config reload do not run package repair; plugin installs remain explicit doctor/install/update work.

    Containerized gateway startup has a narrow upgrade exception: when `marketingclaw gateway run` starts on a new MarketingClaw version, it runs safe state migrations and the existing post-core plugin convergence before readiness, then records a per-version checkpoint. This startup pass can clean stale bundled-plugin records, repair local plugin links, reinstall configured plugin packages when the convergence path requires it, and check active plugin payloads. If startup cannot repair safely, run the same image once with `marketingclaw doctor --fix` against the same mounted state/config before restarting the container normally.

  </Accordion>
  <Accordion title="8. Gateway service migrations and cleanup hints">
    Doctor detects legacy gateway services (launchd/systemd/schtasks) and offers to remove them and install the MarketingClaw service using the current gateway port. It can also scan for extra gateway-like services and print cleanup hints. Profile-named MarketingClaw gateway services are considered first-class and are not flagged as "extra."

    On Linux, if the user-level gateway service is missing but a system-level MarketingClaw gateway service exists, doctor does not install a second user-level service automatically. Inspect with `marketingclaw gateway status --deep` or `marketingclaw doctor --deep`, then remove the duplicate or set `MARKETINGCLAW_SERVICE_REPAIR_POLICY=external` when a system supervisor owns the gateway lifecycle.

  </Accordion>
  <Accordion title="8b. Startup Matrix migration">
    When a Matrix channel account has a pending or actionable legacy state migration, doctor (in `--fix` / `--repair` mode) creates a pre-migration snapshot and then runs the best-effort migration steps: legacy Matrix state migration and legacy encrypted-state preparation. Both steps are non-fatal; errors are logged and startup continues. In read-only mode (`marketingclaw doctor` without `--fix`) this check is skipped entirely.
  </Accordion>
  <Accordion title="8c. Device pairing and auth drift">
    ...
    - inspect pending requests with `marketingclaw devices list`
    - approve the exact request with `marketingclaw devices approve <requestId>`
    - rotate a fresh token with `marketingclaw devices rotate --device <deviceId> --role <role>`
    - remove and re-approve a stale record with `marketingclaw devices remove <deviceId>`
  ...
  <Accordion title="11c. Shell completion">
    ...
    - If the shell profile uses a slow dynamic completion pattern (`source <(marketingclaw completion ...)`), doctor upgrades it to the faster cached file variant.
  ...
  <Accordion title="12. Gateway auth checks (local token)">
    ...
    - `marketingclaw doctor --generate-gateway-token` forces generation only when no token SecretRef is configured.
  ...
  <Accordion title="12b. Read-only SecretRef-aware repairs">
    ...
    - `marketingclaw doctor --fix` uses the same read-only SecretRef summary model as status-family commands for targeted config repairs.
  ...
  <Accordion title="13b. Memory search readiness">
    ...
    Use `marketingclaw memory status --deep` to verify embedding readiness at runtime.
  ...
  <Accordion title="15. Supervisor config audit + repair">
    ...
    - `marketingclaw doctor` prompts before rewriting supervisor config.
    - `marketingclaw doctor --yes` accepts the default repair prompts.
    - `marketingclaw doctor --fix` applies recommended fixes without prompts (`--repair` is an alias).
    - `marketingclaw doctor --fix --force` overwrites custom supervisor configs.
    - `MARKETINGCLAW_SERVICE_REPAIR_POLICY=external` keeps doctor read-only for gateway service lifecycle. It still reports service health and runs non-service repairs, but skips service install/start/restart/bootstrap, supervisor config rewrites, and legacy service cleanup because an external supervisor owns that lifecycle.
    ...
    - Doctor service repairs refuse to rewrite, stop, or restart a gateway service from an older MarketingClaw binary when the config was last written by a newer version. See [Gateway troubleshooting](/gateway/troubleshooting#split-brain-installs-and-newer-config-guard).
    - You can always force a full rewrite via `marketingclaw gateway install --force`.
