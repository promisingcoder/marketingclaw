---
summary: "Use OpenShell as a managed sandbox backend for MarketingClaw agents"
title: OpenShell
read_when:
...
locally, MarketingClaw delegates sandbox lifecycle to the `openshell` CLI, which
...
- OpenShell plugin installed (`marketingclaw plugins install @marketingclaw/openshell-sandbox`)
- `openshell` CLI on `PATH` (or a custom path via
  `plugins.entries.openshell.config.command`)
- An OpenShell account with sandbox access
- MarketingClaw Gateway running on the host
...
Restart the Gateway. On the next agent turn MarketingClaw creates an OpenShell
...
- Before `exec`, MarketingClaw syncs the local workspace into the sandbox.
- After `exec`, MarketingClaw syncs the remote workspace back to local.
- File tools go through the sandbox bridge, but local stays source of truth
  between turns.

Best for development workflows: local edits outside MarketingClaw show up on the
...
- On first sandbox creation, MarketingClaw seeds the remote workspace from local
  once.
- After that, `exec`, `read`, `write`, `edit`, and `apply_patch` operate
  directly on the remote workspace. MarketingClaw does **not** sync remote changes
  back to local.
...
<Warning>
Editing files on the host outside MarketingClaw after the initial seed is invisible to the remote sandbox. Run `marketingclaw sandbox recreate` to re-seed.
</Warning>
...
| `from`                    | `string`                 | `"marketingclaw"` | Sandbox source for first-time create                                                   |
...
1. MarketingClaw runs `sandbox get` for the sandbox name (with any configured
...
2. MarketingClaw runs `sandbox ssh-config` for the sandbox name to fetch SSH
   connection details.
...
- [Sandbox CLI](/cli/sandbox) - `marketingclaw sandbox` commands
