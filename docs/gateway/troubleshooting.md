...
- `marketingclaw gateway status` shows `Runtime: running`, `Connectivity probe: ok`, and a `Capability: ...` line.
- `marketingclaw doctor` reports no blocking config/service issues.
- `marketingclaw channels status --probe` shows live per-account transport status and, where supported, `works` or `audit ok`.
...
marketingclaw update status --json
...
- `Update restart` in `marketingclaw status` / `marketingclaw status --all`. Pending or failed handoffs include the next command to run.
- `plugin load failed: dependency tree corrupted; run marketingclaw doctor --fix` under Channels: the channel config still exists, but plugin registration failed before the channel could load.
- Provider 401s after re-auth: `marketingclaw doctor --fix` checks for stale per-agent OAuth auth shadows and removes old copies so all agents resolve the current shared profile.

## Split brain installs and newer config guard

Use when a gateway service unexpectedly stops after an update, or logs show one `marketingclaw` binary is older than the version that last wrote `marketingclaw.json`.

MarketingClaw stamps config writes with `meta.lastTouchedVersion`. Read-only commands can inspect a config written by a newer MarketingClaw, but process and service mutations refuse to run from an older binary. Blocked actions: gateway service start/stop/restart/uninstall, forced service reinstall, service-mode gateway startup, and `gateway --force` port cleanup.

```bash
which marketingclaw
marketingclaw --version
marketingclaw gateway status --deep
marketingclaw config get meta.lastTouchedVersion
```

<Steps>
  <Step title="Fix PATH">
    Fix `PATH` so `marketingclaw` resolves to the newer install, then rerun the action.
  </Step>
  ...
  <Step title="Remove stale wrappers">
    Remove stale system package or old wrapper entries that still point at an old `marketingclaw` binary.
  </Step>
</Steps>

<Warning>
For intentional downgrade or emergency recovery only, set `MARKETINGCLAW_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1` for the single command. Leave it unset for normal operation.
</Warning>
...
```bash
marketingclaw --version
which -a marketingclaw
marketingclaw gateway status --deep
marketingclaw doctor --deep
marketingclaw logs --follow
```
...
- `Established clients:` in `marketingclaw gateway status --deep` or `Gateway clients` in `marketingclaw doctor --deep`: active TCP clients connected to the Gateway port, with PIDs and command lines when the OS allows it.
- A client process whose command line points at the newer MarketingClaw install or wrapper you rolled back from.

Fix:

1. Stop or restart the stale MarketingClaw client process shown by `gateway status --deep`.
2. Restart apps or wrappers that embed MarketingClaw: local dashboards, editors, app-server helpers, or long-running `marketingclaw logs --follow` shells.
3. Re-run `marketingclaw gateway status --deep` or `marketingclaw doctor --deep` and confirm the stale client PID is gone.
...
Every skill root is a containment boundary. A symlink under `~/.agents/skills`, `<workspace>/.agents/skills`, `<workspace>/skills`, or `~/.marketingclaw/skills` is skipped when its real target resolves outside that root, unless the target is explicitly trusted.
...
marketingclaw config get skills.load
```
...
marketingclaw config get agents.defaults.models
```
...
Do not assume this is always an MarketingClaw configuration issue. The response can come from an upstream security layer such as a CDN, WAF, bot-management rule, or reverse proxy in front of an OpenAI-compatible endpoint.
...
Fix the provider-side filtering first when evidence points to a WAF/CDN block. Prefer a narrowly scoped allow or skip rule for the API path MarketingClaw uses, and avoid disabling protection for the whole site.
...
- MarketingClaw model runs fail only on normal agent turns.
...
marketingclaw infer model run --model <provider/model> --prompt "hi" --json
marketingclaw logs --follow
```

Look for:

- Direct tiny calls succeed, but MarketingClaw runs fail only on larger prompts.
...
<AccordionGroup>
  <Accordion title="Common signatures">
    ...
    - `incomplete turn detected ... stopReason=stop payloads=0`: the backend completed the Chat Completions request but returned no user-visible assistant text for that turn. MarketingClaw retries replay-safe empty OpenAI-compatible turns once; persistent failures usually mean the backend is emitting empty/non-text content or suppressing final-answer text.
    - Direct tiny requests succeed, but MarketingClaw agent runs fail with backend/model crashes (for example Gemma on some `inferrs` builds): MarketingClaw transport is likely already correct; the backend is failing on the larger agent-runtime prompt shape.
  ...
  <Accordion title="Fix options">
    ...
    3. Set `compat.supportsTools: false` for models/backends that cannot handle MarketingClaw's tool schema surface reliably.
    ...
    5. If tiny direct requests keep passing while MarketingClaw agent turns still crash inside the backend, treat it as an upstream server/model limitation and file a repro there with the accepted payload shape.
  </Accordion>
</AccordionGroup>
...
marketingclaw status
marketingclaw channels status --probe
marketingclaw pairing list --channel <channel> [--account <id>]
marketingclaw config get channels
marketingclaw logs --follow
```
...
If `curl` returns MarketingClaw HTML, the Gateway is working and the remaining issue is likely browser cache, an old deep link, or stale tab state. Open `http://127.0.0.1:18789` directly and navigate from the dashboard. If restart does not leave the service running, run `marketingclaw gateway start` and recheck `marketingclaw gateway status`.
...
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.                                                                                                                                                 | Paste/set token in the client and retry. For dashboard paths: `marketingclaw config get gateway.auth.token` then paste into Control UI settings.                                                                                                                                         |
...
| `PAIRING_REQUIRED`           | Device identity needs approval. Check `error.details.reason` for `not-paired`, `scope-upgrade`, `role-upgrade`, or `metadata-upgrade`, and use `requestId` / `remediationHint` when present. | Approve pending request: `marketingclaw devices list` then `marketingclaw devices approve <requestId>`. Scope/role upgrades use the same flow after you review the requested access.                                                                                                     |
...
If `marketingclaw devices rotate` / `revoke` / `remove` is denied unexpectedly:

- Paired-device token sessions can manage only **their own** device unless the caller also has `operator.admin`.
- `marketingclaw devices rotate --scope ...` can only request operator scopes that the caller session already holds.
...
marketingclaw logs --follow
marketingclaw doctor
marketingclaw gateway status --deep   # also scan system-level services
```
...
<AccordionGroup>
  <Accordion title="Common signatures">
    - `Gateway start blocked: set gateway.mode=local` or `existing config is missing gateway.mode` → local gateway mode is not enabled, or the config file was clobbered and lost `gateway.mode`. Fix: set `gateway.mode="local"` in your config, or re-run `marketingclaw onboard --mode local` / `marketingclaw setup` to restamp the expected local-mode config. If you are running MarketingClaw via Podman, the default config path is `~/.marketingclaw/marketingclaw.json`.
    ...
    - `System-level MarketingClaw gateway service detected` from doctor → a systemd system unit exists while the user-level service is missing. Remove or disable the duplicate before allowing doctor to install a user service, or set `MARKETINGCLAW_SERVICE_REPAIR_POLICY=external` if the system unit is the intended supervisor.
    - `Gateway service port does not match current gateway config` → the installed supervisor still pins the old `--port`. Run `marketingclaw doctor --fix` or `marketingclaw gateway install --force`, then restart the gateway service.
...
Use when channels (Telegram, WhatsApp, etc.) on a macOS host go quiet for minutes to hours at a time, and the gateway appears to come back the moment you open the Control UI, SSH in, or otherwise interact with the host. There is usually no obvious symptom in `marketingclaw status` because by the time you look the gateway is alive again.

```bash
ls ~/.marketingclaw/logs/stability/ | tail -5
marketingclaw gateway stability --bundle latest
pmset -g log | grep -iE "sleep|wake|maintenance" | tail -50
launchctl print gui/$UID/ai.marketingclaw.gateway | grep -E "state|last exit|runs"
```

Look for:

- One or more `*-uncaught_exception.json` bundles in `~/.marketingclaw/logs/stability/` with `error.code` set to a transient network code such as `ENETDOWN`, `ENETUNREACH`, `EHOSTUNREACH`, or `ECONNREFUSED`.
...
- `runs` count incrementing across the day with no corresponding `received SIG*; shutting down` line in `~/Library/Logs/marketingclaw/gateway.log`: clean shutdowns log a signal; transient crashes do not.
...
3. **Add a liveness watchdog** so a future crash burst that gets parked by launchd is caught quickly:
   ...
   state=$(launchctl print gui/$UID/ai.marketingclaw.gateway 2>/dev/null | awk -F'= ' '/state =/ {print $2; exit}')
   if [ "$state" != "running" ]; then
     launchctl kickstart -k gui/$UID/ai.marketingclaw.gateway
   fi
   ```
...
marketingclaw logs --follow
marketingclaw gateway stability --bundle latest
marketingclaw gateway diagnostics export
```
...
- `critical memory pressure bundle written` appears shortly before restart → MarketingClaw captured a pre-OOM stability bundle. Inspect it with `marketingclaw gateway stability --bundle latest`.
- `memory pressure: level=critical ... memoryPressureSnapshot=disabled` appears in gateway logs → MarketingClaw detected critical memory pressure, but the pre-OOM stability snapshot is off.
...
- A timestamped `marketingclaw.json.rejected.*` file beside the active config.
- A timestamped `marketingclaw.json.clobbered.*` file if `doctor --fix` repaired a broken direct edit.
- MarketingClaw keeps the latest 32 `.clobbered.*` files for each config path and rotates older ones.

<AccordionGroup>
  <Accordion title="What happened">
    - The config did not validate during startup, hot reload, or an MarketingClaw-owned write.
    - Gateway startup fails closed instead of rewriting `marketingclaw.json`.
    - Hot reload skips invalid external edits and keeps the current runtime config active.
    - MarketingClaw-owned writes reject invalid/destructive payloads before commit and save `.rejected.*`.
    - `marketingclaw doctor --fix` owns repair. It can remove non-JSON prefixes or restore the last-known-good copy while preserving the rejected payload as `.clobbered.*`.
    - When many repairs happen for one config path, MarketingClaw rotates older `.clobbered.*` files so the newest repaired payload is still available.
  ...
  <Accordion title="Common signatures">
    ...
    - `.rejected.*` exists → an MarketingClaw-owned config write failed schema or clobber checks before commit.
    ...
    - `missing-meta-vs-last-good`, `gateway-mode-missing-vs-last-good`, or `size-drop-vs-last-good:*` → an MarketingClaw-owned write was rejected because it lost fields or size compared with the last-known-good backup.
    - `Config last-known-good promotion skipped` → the candidate contained redacted secret placeholders such as `***`.

  </Accordion>
  <Accordion title="Fix options">
    1. Run `marketingclaw doctor --fix` to let doctor repair prefixed/clobbered config or restore last-known-good.
    2. Copy only the intended keys from `.clobbered.*` or `.rejected.*`, then apply them with `marketingclaw config set` or `config.patch`.
    3. Run `marketingclaw config validate` before restarting.
...
Use when `marketingclaw gateway probe` reaches something, but still prints a warning block.

```bash
marketingclaw gateway probe
marketingclaw gateway probe --json
marketingclaw gateway probe --ssh user@gateway-host
```
...
- `multiple reachable gateway identities detected` → distinct gateways answered, or MarketingClaw could not prove reachable targets are the same gateway. An SSH tunnel, proxy URL, or configured remote URL to the same gateway is treated as one gateway with multiple transports, even when transport ports differ.
...
```bash
marketingclaw cron status
marketingclaw cron list
marketingclaw cron runs --id <jobId> --limit 20
marketingclaw system heartbeat last
marketingclaw logs --follow
```
...
<AccordionGroup>
  <Accordion title="Common signatures">
    ...
    - `heartbeat skipped` with `reason=empty-heartbeat-file` → `HEARTBEAT.md` exists but only contains blank, comment, header, fence, or empty-checklist scaffolding, so MarketingClaw skips the model call.
...
marketingclaw nodes describe --node <idOrNameOrIp>
marketingclaw approvals get --node <idOrNameOrIp>
marketingclaw logs --follow
marketingclaw status
```
...
marketingclaw browser start --browser-profile marketingclaw
...
<AccordionGroup>
  ...
  <Accordion title="Element / screenshot / upload signatures">
    ...
    - Stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `marketingclaw browser stop --browser-profile <name>` to close the active control session and release Playwright/CDP emulation state without restarting the whole gateway.
...
- [Browser (MarketingClaw-managed)](/tools/browser)
...
<AccordionGroup>
  <Accordion title="1. Auth and URL override behavior changed">
    ```bash
    marketingclaw gateway status
    marketingclaw config get gateway.mode
    marketingclaw config get gateway.remote.url
    marketingclaw config get gateway.auth.mode
    ```
  ...
  <Accordion title="2. Bind and auth guardrails are stricter">
    ```bash
    marketingclaw config get gateway.bind
    marketingclaw config get gateway.auth.mode
