...
read_when:
  - Setting up MarketingClaw for the first time
  - Looking for common configuration patterns
  - Navigating to specific config sections
title: "Configuration"
---

MarketingClaw reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.marketingclaw/marketingclaw.json`. If the file is missing, MarketingClaw uses safe defaults.

The active config path must be a regular file. MarketingClaw-owned writes replace it atomically (rename onto the path), so a symlinked `marketingclaw.json` gets its target replaced rather than written through - avoid symlinked config layouts. If you keep config outside the default state directory, point `MARKETINGCLAW_CONFIG_PATH` directly at the real file.
...
<Tip>
**New to configuration?** Start with `marketingclaw onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    marketingclaw onboard       # full onboarding flow
    marketingclaw configure     # config wizard
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.marketingclaw/marketingclaw.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
MarketingClaw only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

`marketingclaw config schema` prints the canonical JSON Schema used by Control UI
...
- Only diagnostic commands work (`marketingclaw doctor`, `marketingclaw logs`, `marketingclaw health`, `marketingclaw status`)
- Run `marketingclaw doctor` to see exact issues
- Run `marketingclaw doctor --fix` (`--repair` is the same flag; `--yes` skips prompts) to apply repairs

The Gateway keeps a trusted last-known-good copy after each successful startup,
but startup and hot reload do not restore it automatically - only `marketingclaw doctor --fix`
does. If `marketingclaw.json` fails validation (including plugin-local validation), Gateway
...
<AccordionGroup>
  ...
  <Accordion title="Choose and configure models">
    ...
    - Use `marketingclaw config set agents.defaults.models '<json>' --strict-json --merge` to add allowlist entries without removing existing models. Plain replacements that would remove entries are rejected unless you pass `--replace`.
  ...
  <Accordion title="Tune gateway WebSocket handshake timeout">
    ...
    - `MARKETINGCLAW_HANDSHAKE_TIMEOUT_MS` still takes precedence for one-off service or shell overrides.
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push for public App Store builds uses the hosted MarketingClaw relay: `https://ios-push-relay.marketingclaw.ai`.
    ...
    ```bash
    marketingclaw config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `MARKETINGCLAW_APNS_RELAY_BASE_URL` and `MARKETINGCLAW_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    ...
    - `MARKETINGCLAW_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
  ...
  <Accordion title="Set up webhooks (hooks)">
    ...
    - Use a dedicated `hooks.token`; do not reuse active Gateway auth secrets (`gateway.auth.token` / `MARKETINGCLAW_GATEWAY_TOKEN` or `gateway.auth.password` / `MARKETINGCLAW_GATEWAY_PASSWORD`).
    - Hook auth is header-only (`Authorization: Bearer ...` or `x-marketingclaw-token`); query-string tokens are rejected.
  ...
  <Accordion title="Split config into multiple files ($include)">
    ...
    - **MarketingClaw-owned writes**: when a write changes only one top-level section
      backed by a single-file include such as `plugins: { $include: "./plugins.json5" }`,
      MarketingClaw updates that included file and leaves `marketingclaw.json` intact
    - **Unsupported write-through**: root includes, include arrays, and includes
      with sibling overrides fail closed for MarketingClaw-owned writes instead of
      flattening the config
    - **Confinement**: `$include` paths must resolve under the directory holding
      `marketingclaw.json`. To share a tree across machines or users, set
      `MARKETINGCLAW_INCLUDE_ROOTS` to a path-list (`:` on POSIX, `;` on Windows) of
...
The Gateway watches `~/.marketingclaw/marketingclaw.json` and applies changes automatically - no manual restart needed for most settings.

Direct file edits are treated as untrusted until they validate. The watcher waits
for editor temp-write/rename churn to settle, reads the final file, and rejects
invalid external edits without rewriting `marketingclaw.json`. MarketingClaw-owned config
...
config`, inspect the config, run `marketingclaw config validate`, then run `marketingclaw
...
When you edit a source file that is referenced through `$include`, MarketingClaw plans
...
```bash
marketingclaw gateway call config.get --params '{}'  # capture payload.hash
marketingclaw gateway call config.patch --params '{
...
MarketingClaw reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.marketingclaw/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, MarketingClaw runs your login shell and imports only the missing keys:
...
Env var equivalent: `MARKETINGCLAW_LOAD_SHELL_ENV=1`. Default `timeoutMs`: `15000`.
</Accordion>
