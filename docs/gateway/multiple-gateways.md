---
summary: "Run multiple MarketingClaw Gateways on one host (isolation, ports, and profiles)"
read_when:
...
marketingclaw --profile rescue onboard
marketingclaw --profile rescue gateway install --port 19789
```
...
During `marketingclaw --profile rescue onboard`:
...
- Profile/config file
- State directory
- Workspace (default: `~/.marketingclaw/workspace-rescue`)
...
# extra gateway
marketingclaw --profile ops setup
marketingclaw --profile ops gateway --port 19789
```

Named profiles on both sides also work:

```bash
marketingclaw --profile main setup
marketingclaw --profile main gateway --port 18789
...
marketingclaw --profile ops gateway install --port 19789
```
...
| Setting                      | Purpose                              |
| ---------------------------- | ------------------------------------ |
| `MARKETINGCLAW_CONFIG_PATH`  | Per-instance config file             |
| `MARKETINGCLAW_STATE_DIR`    | Per-instance sessions, creds, caches |
...
Base port = `gateway.port` (or `MARKETINGCLAW_GATEWAY_PORT` / `--port`).
...
```bash
MARKETINGCLAW_CONFIG_PATH=~/.marketingclaw/main.json \
MARKETINGCLAW_STATE_DIR=~/.marketingclaw \
marketingclaw gateway --port 18789

MARKETINGCLAW_CONFIG_PATH=~/.marketingclaw/rescue.json \
MARKETINGCLAW_STATE_DIR=~/.marketingclaw-rescue \
marketingclaw gateway --port 19789
```
...
marketingclaw --profile rescue gateway status --deep
marketingclaw --profile rescue gateway probe
marketingclaw status
marketingclaw --profile rescue status
marketingclaw --profile rescue browser status
```

- `gateway status --deep` catches stale launchd/systemd/schtasks services from older installs.
- `gateway probe` warning text such as `multiple reachable gateway identities detected` is expected only when you intentionally run more than one isolated gateway, or when MarketingClaw cannot prove reachable probe targets are the same gateway. An SSH tunnel, proxy URL, or configured remote URL to the same gateway is one gateway with multiple transports, even when transport ports differ.
