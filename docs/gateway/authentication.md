...
MarketingClaw supports OAuth and API keys for model providers. For an always-on gateway host, an API key is the most predictable option; subscription/OAuth flows work too when they match your provider account model.
...
2. Put it on the **gateway host** (the machine running `marketingclaw gateway`):
...
3. If the gateway runs under systemd/launchd, put the key in `~/.marketingclaw/.env` so the daemon can read it:

```bash
cat >> ~/.marketingclaw/.env <<'EOF'
<PROVIDER>_API_KEY=...
EOF
```
...
`marketingclaw onboard` can also store API keys for daemon use if you don't want to manage env vars yourself. See [Environment variables](/help/environment) for the full env-loading precedence (`env.shellEnv`, `~/.marketingclaw/.env`, systemd/launchd).
...
This is two steps: log Claude Code into Anthropic on the host, then tell MarketingClaw to route Anthropic model selection through the local `claude-cli` backend and store the matching MarketingClaw auth profile.
...
```bash
marketingclaw models auth paste-token --provider openrouter
```

MarketingClaw reads auth profiles from each agent's `marketingclaw-agent.sqlite`. Endpoint details (`baseUrl`, `api`, model ids, headers, timeouts) belong under `models.providers.<id>` in `marketingclaw.json` or `models.json`, not in auth profiles.

If an older install still has `auth-profiles.json`, `auth-state.json`, or a flat shape like `{ "openrouter": { "apiKey": "..." } }`, run `marketingclaw doctor --fix` to import it into SQLite; doctor keeps timestamped backups beside the original JSON files.

External auth routes such as Bedrock `auth: "aws-sdk"` aren't credentials. For a named Bedrock route, set `auth.profiles.<id>.mode: "aws-sdk"` in `marketingclaw.json` — don't write `type: "aws-sdk"` into the auth profile store. `marketingclaw doctor --fix` migrates legacy AWS SDK markers from the credential store into config metadata.
...
- If auth exists but MarketingClaw can't resolve a probeable model for that provider, probe reports `status: no_model`.
...
1. `MARKETINGCLAW_LIVE_<PROVIDER>_KEY` (single override, pins one key)
...
```bash
marketingclaw models auth login --provider anthropic --force
```
...
```bash
marketingclaw models auth order get --provider anthropic
marketingclaw models auth order set --provider anthropic anthropic:default
marketingclaw models auth order clear --provider anthropic
```

Use `--agent <id>` to target a specific agent; omit it to use the configured default agent. `marketingclaw models status --probe` shows omitted stored profiles as `excluded_by_auth_order` rather than silently skipping them.
...
Run `marketingclaw models status` to see which profile is expiring. If an Anthropic token profile is missing or expired, refresh it via setup-token or migrate to an Anthropic API key.
