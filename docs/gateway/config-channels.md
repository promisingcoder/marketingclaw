...
Each channel starts automatically when its config section exists (unless `enabled: false`). Telegram and iMessage ship inside the core `marketingclaw` package. Other official channels (Discord, Slack, WhatsApp, Matrix, Microsoft Teams, IRC, Google Chat, Signal, Mattermost, and more) install as separate plugins with `marketingclaw plugins install <spec>`; see [Channels](/channels) for the full list and install specs.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // authDir: "~/.marketingclaw/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- Legacy single-account Baileys auth dir is migrated by `marketingclaw doctor` into `whatsapp/default`.
...
- `apiRoot` is the Telegram Bot API root only. Use `https://api.telegram.org` or your self-hosted/proxy root, not `https://api.telegram.org/bot<TOKEN>`; `marketingclaw doctor --fix` removes an accidental trailing `/bot<TOKEN>` suffix.
- For a self-hosted Bot API server in `--local` mode, `trustedLocalFileRoots` lists host paths MarketingClaw may read. Mount the server data volume on the MarketingClaw host and configure either its data root or per-token directory; container paths under `/var/lib/telegram-bot-api` are mapped into those roots. Other absolute paths remain rejected.
- Optional `channels.telegram.defaultAccount` overrides default account selection when it matches a configured account id.
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `marketingclaw doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["marketingclaw-dm"] },
      guilds: {
        "123456789012345678": {
          slug: "friends-of-marketingclaw",
          requireMention: false,
...
- `channels.discord.voice.reconnectGraceMs` controls how long a disconnected voice session may take to enter reconnect signalling before MarketingClaw destroys it (default `15000`).
- Discord voice playback is not interrupted by another user's speaking-start event. To avoid feedback loops, MarketingClaw ignores new voice capture while TTS is playing.
- MarketingClaw additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
- `channels.discord.streaming` is the canonical stream mode key. Discord defaults to `streaming.mode: "progress"` so tool/work progress appears in one edited preview message; set `streaming.mode: "off"` to disable it. Legacy `streamMode` and boolean `streaming` values remain runtime aliases; run `marketingclaw doctor --fix` to rewrite persisted config.
...
- `channels.slack.streaming.mode` is the canonical Slack stream mode key (default `"partial"`). `channels.slack.streaming.nativeTransport` controls Slack's native streaming transport (default `true`). Legacy `streamMode`, boolean `streaming`, `chunkMode`, `blockStreaming`, `blockStreamingCoalesce`, and `nativeStreaming` values remain runtime aliases; run `marketingclaw doctor --fix` to rewrite persisted config to `streaming.{mode,chunkMode,block.enabled,block.coalesce,nativeTransport}`.
...
Check [npmjs.com/package/@marketingclaw/mattermost](https://www.npmjs.com/package/@marketingclaw/mattermost) for the current dist-tags before pinning a version.
...
- `commands.callbackUrl` must resolve to the MarketingClaw gateway endpoint and be reachable from the Mattermost server.
- Native slash callbacks are authenticated with the per-command tokens returned
  ...
  commands are activated, MarketingClaw rejects callbacks with
...
- `channels.imessage.actions.*`: enable private API actions that are also gated by `imsg status` / `marketingclaw channels status --probe`.
...
- If you add a non-default account via `marketingclaw channels add` (or channel onboarding) while still on a single-account top-level channel config, MarketingClaw promotes account-scoped top-level single-account values into the channel account map first so the original account keeps working. Most channels move them into `channels.<channel>.accounts.default`; Matrix can preserve an existing matching named/default target instead.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `marketingclaw doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into the promoted account chosen for that channel. Most channels use `accounts.default`; Matrix can preserve an existing matching named/default target instead.
...
If the message tool is unavailable under the active tool policy, MarketingClaw falls back to automatic visible replies instead of silently suppressing the response. `marketingclaw doctor` warns about this mismatch.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@marketingclaw", "marketingclaw"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@marketingclaw"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `marketingclaw.json`). For gateway `chat.send` clients, persistent `/config set|unset` writes also require `operator.admin`; read-only `/config show` stays available to normal write-scoped operator clients.
- `mcp: true` enables `/mcp` for MarketingClaw-managed MCP server config under `mcp.servers`.
