...
read_when:
  - Setting up Matrix in MarketingClaw
  - Configuring Matrix E2EE and verification
title: "Matrix"
---

Matrix is a downloadable channel plugin (`@marketingclaw/matrix`) built on the official `matrix-js-sdk`. It supports DMs, rooms, threads, media, reactions, polls, location, and E2EE.

## Install

```bash
marketingclaw plugins install @marketingclaw/matrix
```

Bare plugin specs try ClawHub first, then npm fallback. Force a source with `marketingclaw plugins install clawhub:@marketingclaw/matrix` or `npm:@marketingclaw/matrix`. From a local checkout: `marketingclaw plugins install ./path/to/local/matrix-plugin`.
...
marketingclaw configure --section channels
```

The wizard asks for homeserver URL, auth method (token or password), user ID (password auth only), optional device name, whether to enable E2EE, and room access/auto-join. If matching `MATRIX_*` env vars already exist and the account has no saved auth, the wizard offers an env-var shortcut. Resolve room names before saving an allowlist with `marketingclaw channels resolve --channel matrix "Project Room"`. Enabling E2EE in the wizard runs the same bootstrap as [`marketingclaw matrix encryption setup`](#encryption-and-verification).
...
```json5
{
  channels: {
    matrix: {
      ...
      deviceName: "MarketingClaw Gateway",
    },
  },
}
```
...
Matrix caches credentials under `~/.marketingclaw/credentials/matrix/`: `credentials.json` for the default account, `credentials-<account>.json` for named accounts. When cached credentials exist, MarketingClaw treats Matrix as configured even without an `accessToken` in the config file - this covers setup, `marketingclaw doctor`, and channel-status probes.
...
Matrix reply streaming is opt-in. `streaming` controls how MarketingClaw delivers the in-flight assistant reply; `blockStreaming` controls whether each completed block is kept as its own Matrix message.
...
- If a preview grows past Matrix's per-event size limit, MarketingClaw stops preview streaming and falls back to final-only delivery.
- Media replies always send attachments normally; if a stale preview cannot be reused safely, MarketingClaw redacts it before sending the final media reply.
...
- In encrypted rooms, MarketingClaw decrypts the attachment through the existing Matrix media path before transcription.
...
Matrix native approval prompts are normal `m.room.message` events with MarketingClaw-specific content under the `com.marketingclaw.approval` key. Stock clients still render the text body; MarketingClaw-aware clients can read the structured approval id, kind, state, decisions, and exec/plugin details.

When a prompt is too long for one Matrix event, MarketingClaw chunks the visible text and attaches `com.marketingclaw.approval` to the first chunk only. Allow/deny reactions bind to that first event, so long prompts keep the same approval target as single-event prompts.
...
By default, Matrix messages from other configured MarketingClaw Matrix accounts are ignored. Use `allowBots` to intentionally allow inter-agent traffic:
...
- MarketingClaw still ignores messages from the same Matrix user ID to avoid self-reply loops.
- Matrix has no native bot flag; MarketingClaw treats "bot-authored" as "sent by another configured Matrix account on this MarketingClaw gateway".
...
All `marketingclaw matrix` commands accept `--verbose` (full diagnostics), `--json` (machine-readable output), and `--account <id>` (multi-account setups). Output is concise by default.

### Enable encryption

```bash
marketingclaw matrix encryption setup
```
...
```bash
marketingclaw matrix account add \
  --homeserver https://matrix.example.org \
...
marketingclaw matrix verify status --include-recovery-key --json
```
...
The literal-key form `marketingclaw matrix verify device "<recovery-key>"` also works, but the key ends up in shell history.
...
If the homeserver requires UIA to upload cross-signing keys, MarketingClaw tries no-auth first, then `m.login.dummy`, then `m.login.password` (requires `channels.matrix.password`).
...
```bash
marketingclaw matrix verify list
```

Lists pending verification requests for the selected account.

```bash
marketingclaw matrix verify request --own-user
marketingclaw matrix verify request --user-id @ops:example.org --device-id ABCDEF
```
...
| Command                                         | Purpose                                                             |
| ----------------------------------------------- | ------------------------------------------------------------------- |
| `marketingclaw matrix verify accept <id>`       | Accept an inbound request                                           |
| `marketingclaw matrix verify start <id>`        | Start the SAS flow                                                  |
| `marketingclaw matrix verify sas <id>`          | Print the SAS emoji or decimals                                     |
| `marketingclaw matrix verify confirm-sas <id>`  | Confirm that the SAS matches what the other client shows            |
| `marketingclaw matrix verify mismatch-sas <id>` | Reject the SAS when the emoji or decimals do not match              |
| `marketingclaw matrix verify cancel <id>`       | Cancel; takes optional `--reason <text>` and `--code <matrix-code>` |
...
<AccordionGroup>
  <Accordion title="Startup behavior">
    ...
    Startup also runs a conservative crypto bootstrap pass reusing the current secret storage and cross-signing identity. If bootstrap state is broken, MarketingClaw attempts a guarded repair even without `channels.matrix.password`; if the homeserver requires password UIA, startup logs a warning and stays non-fatal. Already-owner-signed devices are preserved.
  ...
  <Accordion title="Verification notices">
    ...
    Incoming requests from another Matrix client are tracked and auto-accepted. For self-verification, MarketingClaw starts the SAS flow automatically and confirms its own side once emoji verification is available - you still need to compare and confirm "They match" in your Matrix client.
  ...
  <Accordion title="Deleted or invalid Matrix device">
    If `verify status` says the current device is no longer listed on the homeserver, create a new MarketingClaw Matrix device. For password login:
...
marketingclaw matrix account add \
  ...
  --password '<password>' \
  --device-name MarketingClaw-Gateway
```

    For token auth, create a fresh access token in your Matrix client or admin UI, then update MarketingClaw:
...
```
  ...
  <Accordion title="Device hygiene">
    Old MarketingClaw-managed devices can accumulate. List and prune:

```bash
marketingclaw matrix devices list
marketingclaw matrix devices prune-stale
```
  ...
  <Accordion title="Crypto store">
    ...
    Encrypted runtime state lives under `~/.marketingclaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/` and includes the sync store, crypto store, recovery key, IDB snapshot, thread bindings, and startup verification state. When the token changes but the account identity stays the same, MarketingClaw reuses the best existing root so prior state remains visible.

    A single older token-hash root can be a normal token-rotation continuity path. If MarketingClaw logs `matrix: multiple populated token-hash storage roots detected`, inspect the account directory and archive stale sibling roots only after confirming the selected active root is healthy. Prefer moving stale roots into an `_archive/` directory over deleting them immediately.
...
```bash
marketingclaw matrix profile set --name "MarketingClaw Assistant"
marketingclaw matrix profile set --avatar-url https://cdn.example.org/avatar.png
```
...
`dm.sessionScope` decides how Matrix DM rooms map to MarketingClaw sessions:
...
- DM user-target reuse only kicks in when current session metadata proves the same DM peer on the same Matrix account; otherwise MarketingClaw falls back to normal user-scoped routing.
...
When MarketingClaw detects a Matrix DM room colliding with another DM room on the same shared session, it posts a one-time `m.notice` pointing to the `/focus` escape hatch and suggesting a `dm.sessionScope` change. The notice only appears when thread bindings are enabled.
...
`--bind here` does not create a child Matrix thread. `threadBindings.spawnSessions` gates `/acp spawn --thread auto|here`, where MarketingClaw needs to create or bind a child thread.
...
- Room history is pending-only: MarketingClaw buffers room messages that did not trigger a reply yet, then snapshots that window when a mention or other trigger arrives.
...
```bash
marketingclaw pairing list matrix
marketingclaw pairing approve matrix <CODE>
```

If an unapproved Matrix user keeps messaging before approval, MarketingClaw reuses the same pending pairing code and may send a reminder reply after a short cooldown instead of minting a new code.

See [Pairing](/channels/pairing) for the shared DM pairing flow and storage layout.

## Direct room repair

If direct-message state drifts, MarketingClaw can end up with stale `m.direct` mappings pointing at old solo rooms instead of the live DM. Inspect the current mapping for a peer:

```bash
marketingclaw matrix direct inspect --user-id @alice:example.org
```

Repair it:

```bash
marketingclaw matrix direct repair --user-id @alice:example.org
```
...
- If you have multiple accounts and one is literally named `default`, MarketingClaw uses it implicitly even when `defaultAccount` is unset.
...
- When MarketingClaw promotes a single-account config to multi-account during repair or setup, it preserves the existing named account if one exists or `defaultAccount` already points at one. Only Matrix auth/bootstrap keys move into the promoted account; shared delivery-policy keys stay at the top level.

See [Configuration reference](/gateway/config-channels#multi-account-all-channels) for the shared multi-account pattern.

## Private/LAN homeservers

By default, MarketingClaw blocks private/internal Matrix homeservers for SSRF protection unless you opt in per account.
...
Named accounts can override the top-level default with `channels.matrix.accounts.<id>.proxy`. MarketingClaw uses the same proxy setting for runtime Matrix traffic and account status probes.

## Target resolution

Matrix accepts these target forms anywhere MarketingClaw asks for a room or user target:
...
Matrix room IDs are case-sensitive. Use the exact room ID casing from Matrix when configuring explicit delivery targets, cron jobs, bindings, or allowlists. MarketingClaw keeps internal session keys canonical for storage, so those lowercase keys are not a reliable source for Matrix delivery IDs.
