...
Google Chat runs as the official `@marketingclaw/googlechat` plugin: DMs and spaces through Google Chat API webhooks (HTTP endpoint only, no Pub/Sub).

## Install

```bash
marketingclaw plugins install @marketingclaw/googlechat
```

Local checkout (when running from a git repo):

```bash
marketingclaw plugins install ./path/to/local/googlechat-plugin
```
...
2. Create a **Service Account**:
   ...
   - Name it whatever you want (e.g., `marketingclaw-chat`).
...
4. Store the downloaded JSON file on your gateway host (e.g., `~/.marketingclaw/googlechat-service-account.json`).
...
7. Configure MarketingClaw with the service account and the webhook audience (must match the Chat app config):
   - Env: `GOOGLE_CHAT_SERVICE_ACCOUNT_FILE=/path/to/service-account.json` (default account only), or
   - Config: see [Config highlights](#config-highlights). `marketingclaw channels add --channel googlechat` also accepts `--audience-type`, `--audience`, `--webhook-path`, and `--webhook-url`.
...
Google Chat webhooks require a public HTTPS endpoint. For security, expose **only the `/googlechat` path** to the internet and keep the MarketingClaw dashboard and other endpoints private.
...
Requests to `your-domain.com/` are ignored or 404, while `your-domain.com/googlechat` routes to MarketingClaw.
...
2. MarketingClaw authenticates every request before dispatch:
...
5. DM access is pairing by default. Unknown senders receive a pairing code; approve with:
   - `marketingclaw pairing approve googlechat <code>`
...
1. **Channel not configured**: the `channels.googlechat` section is missing. Verify with:
   ...
   ```bash
   marketingclaw config get channels.googlechat
   ```
...
2. **Plugin not enabled**: check plugin status:
   ...
   ```bash
   marketingclaw plugins list | grep googlechat
   ```
...
- `marketingclaw channels status --probe` surfaces auth errors and missing audience config (`audience` and `audienceType` are both required).
...
- `marketingclaw logs --follow` while sending a test message shows whether requests reach the gateway.
