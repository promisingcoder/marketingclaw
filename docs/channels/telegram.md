...
<Steps>
  <Step title="Create the bot token in BotFather">
    Both flows end with a token you paste into MarketingClaw — pick one:
...
```
    ...
    Telegram does **not** use `marketingclaw channels login telegram`; set the token in config/env, then start the gateway.
...
```
  ...
  <Step title="Add the bot to a group">
    ...
    Get the group chat ID from `marketingclaw logs --follow`, a forwarded-ID bot, or Bot API `getUpdates`. After the group is allowed, `/whoami@<bot_username>` confirms the user and group IDs.
...
<Note>
Token resolution is account-aware: `tokenFile` beats `botToken` beats env, and config always wins over `TELEGRAM_BOT_TOKEN` (which only resolves for the default account). After a successful startup, MarketingClaw caches the bot identity for up to 24 hours so restarts skip an extra `getMe` call; changing or removing the token clears that cache.
</Note>
...
In groups and forum topics, an explicit mention of the configured bot handle (for example `@my_bot`) addresses the selected MarketingClaw agent, even when the agent persona name differs from the Telegram username. Group silence policy still applies to unrelated traffic, but the bot handle itself is never "someone else."
...
<Tabs>
  <Tab title="DM policy">
    ...
    Setup asks for numeric user IDs only. If your config has `@username` allowlist entries from an older setup, run `marketingclaw doctor --fix` to resolve them to numeric IDs (best-effort; requires a Telegram bot token).
    If you previously relied on pairing-store allowlist files, `marketingclaw doctor --fix` can recover entries into `channels.telegram.allowFrom` for allowlist flows (for example when `dmPolicy: "allowlist"` has no explicit IDs yet).
    ...
    Safer (no third-party bot): DM your bot, run `marketingclaw logs --follow`, read `from.id`.
...
```
    ...
    Group history context is always on and bounded by `historyLimit`. Set `channels.telegram.historyLimit: 0` to disable the group history window. `marketingclaw doctor --fix` removes the retired `includeGroupHistoryContext` key.

    Getting the group chat ID: forward a group message to `@userinfobot` / `@getidsbot`, read `chat.id` from `marketingclaw logs --follow`, inspect Bot API `getUpdates`, or (once the group is allowed) run `/whoami@<bot_username>`.
...
- DM messages can carry `message_thread_id`; MarketingClaw preserves it for replies. DM topic sessions split only when Telegram `getMe` reports `has_topics_enabled: true` for the bot; otherwise DMs stay on the flat session.
...
- Each gateway process guards long polling so only one active poller can use a bot token at a time. Persistent `getUpdates` 409 conflicts point to another MarketingClaw gateway, script, or external poller using the same token.
...
<Note>
  `channels.telegram.dm.threadReplies` and `channels.telegram.direct.<chatId>.threadReplies` were removed. Run `marketingclaw doctor --fix` after upgrading if your config still has those keys. DM topic routing now follows Telegram `getMe.has_topics_enabled` (controlled by BotFather threaded mode): topics-enabled bots use thread-scoped DM sessions when Telegram sends `message_thread_id`; other DMs stay on the flat session.
</Note>
...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    MarketingClaw streams partial replies in real time in direct chats, groups, and topics: send a preview message, then `editMessageText` repeatedly, finalizing in place.
    ...
    - legacy `channels.telegram.streamMode`, boolean `streaming` values, and retired native draft preview keys are detected; run `marketingclaw doctor --fix` to migrate them
    ...
    <Note>
      Selected quote replies are the exception. When `replyToMode` is `first`, `all`, or `batched` and the inbound message has selected quote text, MarketingClaw sends the final answer through Telegram's native quote-reply path instead of editing the answer preview, so `streaming.preview.toolProgress` cannot show status lines that turn. Current-message replies without selected quote text still stream. Set `replyToMode: "off"` when tool-progress visibility matters more than native quote replies, or `streaming.preview.toolProgress: false` to accept that trade-off.
    </Note>

    For text-only replies: short previews get the final edit in place; long finals that split into multiple messages reuse the preview as the first chunk, then send only the remainder; progress-mode finals clear the status draft and use normal final delivery; if the final edit fails before completion is confirmed, MarketingClaw falls back to normal final delivery and cleans up the stale preview. For complex replies (media payloads), MarketingClaw always falls back to normal final delivery and cleans up the preview.

    Preview streaming and block streaming are mutually exclusive — when block streaming is explicitly enabled, MarketingClaw skips the preview stream to avoid double-streaming.
...
```
    ...
    When enabled: the agent is told rich messages are available for this bot/account; Markdown text renders through MarketingClaw's Markdown IR as Telegram rich HTML; explicit rich HTML payloads preserve supported Bot API 10.1 tags (headings, tables, details, rich media, formulas); media captions still use Telegram HTML captions (rich messages do not replace captions, and captions cap at 1024 characters).
...
```
    ...
    - `deleteWebhook`, `deleteMyCommands`, or `setMyCommands` failing with `404: Not Found` while direct Bot API curl commands work usually means `channels.telegram.apiRoot` was set to the full `/bot<TOKEN>` endpoint. `apiRoot` must be the Bot API root only; `marketingclaw doctor --fix` removes an accidental trailing `/bot<TOKEN>`.
    - `getMe returned 401` means Telegram rejected the configured bot token. Update `botToken`, `tokenFile`, or `TELEGRAM_BOT_TOKEN` (default account) with the current BotFather token; MarketingClaw stops before polling so this is not reported as a webhook cleanup failure.
...
```
  ...
  <Accordion title="Reply threading tags">
    ...
    When reply threading is enabled and the original text/caption is available, MarketingClaw adds a native quote excerpt automatically. Telegram caps native quote text at 1024 UTF-16 code units; longer messages are quoted from the start and fall back to a plain reply if Telegram rejects the quote.
  ...
  <Accordion title="Forum topics and thread behavior">
    ...
    **Thread-bound ACP spawn from chat**: `/acp spawn <agent> --thread here|auto` binds the current topic to a new ACP session; follow-ups route there directly, and MarketingClaw pins the spawn confirmation in-topic. Requires `channels.telegram.threadBindings.spawnSessions` (default: `true`).
    ...
    The retired `dm.threadReplies` and `direct.*.threadReplies` overrides are gone; BotFather threaded mode is the single source of truth. Run `marketingclaw doctor --fix` to remove stale config keys.
...
```
    ...
    Sticker context fields: `Sticker.emoji`, `Sticker.setName`, `Sticker.fileId`, `Sticker.fileUniqueId`, `Sticker.cachedDescription`. Descriptions are cached in MarketingClaw SQLite plugin state to reduce repeated vision calls.
...
```
  ...
  <Accordion title="Reaction notifications">
    Telegram reactions arrive as `message_reaction` updates, separate from message payloads. When enabled, MarketingClaw enqueues system events like `Telegram reaction added: 👍 by Alice (@alice) on msg 42`.
  ...
  <Accordion title="Ack reactions">
    `ackReaction` sends an acknowledgement emoji while MarketingClaw processes an inbound message. `messages.ackReactionScope` decides *when* it is sent.
...
```
  ...
  <Accordion title="Long polling vs webhook">
    ...
    In long-polling mode, MarketingClaw persists its restart watermark only after an update dispatches successfully; a failed handler leaves that update retryable in the same process instead of marking it completed.
    ...
    Webhook mode validates request guards, the Telegram secret token, and the JSON body before returning `200`. MarketingClaw then processes the update asynchronously through the same per-chat/per-topic bot lanes used by long polling, so slow agent turns do not hold Telegram's delivery ACK.
...
```bash
marketingclaw message send --channel telegram --target 123456789 --message "hi"
marketingclaw message send --channel telegram --target @name --message "hi"
marketingclaw message send --channel telegram --target -1001234567890:topic:42 --message "hi topic"
```

    Polls use `marketingclaw message poll` and support forum topics:

```bash
marketingclaw message poll --channel telegram --target 123456789 \
  --poll-question "Ship it?" --poll-option "Yes" --poll-option "No"
marketingclaw message poll --channel telegram --target -1001234567890:topic:42 \
...
```
  ...
  <Accordion title="Exec approvals in Telegram">
    ...
    Channel delivery shows the command text in the chat; only enable `channel` or `both` in trusted groups/topics. When the prompt lands in a forum topic, MarketingClaw preserves the topic for the approval prompt and follow-up. Exec approvals expire after 30 minutes by default.
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `marketingclaw channels status` warns when config expects unmentioned group messages.
    - `marketingclaw channels status --probe` checks explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - Review `marketingclaw logs --follow` for skip reasons.
  ...
  <Accordion title="Polling or network instability">
    ...
    - During polling startup, MarketingClaw reuses the successful startup `getMe` probe for grammY so the runner does not need a second `getMe` before the first `getUpdates`.
    - If `deleteWebhook` fails with a transient network error during polling startup, MarketingClaw continues into long polling instead of making another pre-poll control-plane call. A still-active webhook then surfaces as a `getUpdates` conflict; MarketingClaw rebuilds the transport and retries webhook cleanup.
    ...
    - `Polling stall detected` in logs means MarketingClaw restarts polling and rebuilds the transport after 120 seconds without completed long-poll liveness by default.
    - `marketingclaw channels status --probe` and `marketingclaw doctor` warn when a running polling account has not completed `getUpdates` after startup grace, a running webhook account has not completed `setWebhook` after startup grace, or the last successful polling transport activity is stale.
    ...
    - If `MARKETINGCLAW_PROXY_URL` is set for a service environment and no standard proxy env is present, Telegram uses that URL for Bot API transport too.
...
```
    ...
    - Node 22+ defaults to `autoSelectFamily=true` (except WSL2). Telegram DNS result order honors `MARKETINGCLAW_TELEGRAM_DNS_RESULT_ORDER`, then `channels.telegram.network.dnsResultOrder`, then the process default (for example `NODE_OPTIONS=--dns-result-order=ipv4first`), falling back to `ipv4first` on Node 22+ if none applies.
...
```
    ...
    - Temporary environment overrides: `MARKETINGCLAW_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY=1`, `MARKETINGCLAW_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY=1`, `MARKETINGCLAW_TELEGRAM_DNS_RESULT_ORDER=ipv4first`.
...
<Note>
Multi-account precedence: with two or more account IDs configured, set `channels.telegram.defaultAccount` (or include `channels.telegram.accounts.default`) to make default routing explicit. Otherwise MarketingClaw falls back to the first normalized account ID and `marketingclaw doctor` warns. Named accounts inherit `channels.telegram.allowFrom` / `groupAllowFrom`, but not `accounts.default.*` values.
</Note>
