...
MarketingClaw routes replies **back to the channel where a message came from**. The
...
MarketingClaw infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, MarketingClaw still records inbound session metadata, but it
...
when a guarded path must not create a new MarketingClaw session. In that mode,
MarketingClaw may update metadata and `lastRoute` for an existing session, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when MarketingClaw would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.marketingclaw/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.marketingclaw`):

- `~/.marketingclaw/agents/<agentId>/sessions/sessions.json`
