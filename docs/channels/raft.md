...
read_when:
  - You want to connect MarketingClaw to a Raft workspace
...
Raft connects an MarketingClaw agent to a Raft External Agent through the local
...
```bash
marketingclaw plugins install @marketingclaw/raft
...
- The Raft CLI installed on the same host as the MarketingClaw Gateway, on the
  service's `PATH`.
...
```bash
marketingclaw channels add --channel raft
```
...
7. Starts one serialized MarketingClaw agent turn per accepted wake.

The bridge owns Raft delivery retries and reconnects. The MarketingClaw turn
...
```bash
raft --profile marketingclaw message check
raft --profile marketingclaw message send
```

<Note>
Raft is not a push-message transport. MarketingClaw does not automatically send the model's final text back through the bridge, so the agent must use the Raft CLI after processing a wake.
</Note>

## Verify

Check that MarketingClaw can find the CLI and has a configured profile:

```bash
marketingclaw channels status --probe
marketingclaw plugins inspect raft --runtime --json
```
