---
summary: "CLI reference for `marketingclaw configure` (interactive configuration prompts)"
read_when:
...
Use `marketingclaw onboard` or `marketingclaw setup` for the full guided first-run journey, `marketingclaw setup --baseline` for the baseline config/workspace only, and `marketingclaw channels add` when you only need channel account setup.

<Tip>
`marketingclaw config` with no subcommand opens the same wizard. Use `marketingclaw config get|set|unset` for non-interactive edits.
</Tip>
...
marketingclaw configure --section model --section channels
marketingclaw configure --section gateway --section daemon
```
...
<Note>
`marketingclaw configure` requires an interactive terminal (both stdin and stdout must be TTYs). Without one it prints the equivalent non-interactive `marketingclaw config get|set|patch|validate` commands and exits with an error instead of partially running.
</Note>
...
Re-running provider auth from configure preserves an existing `agents.defaults.model.primary`, even when the provider's auth step returns a config patch with its own recommended default model. Adding or reauthing a provider makes its models available without taking over your current primary model. Use `marketingclaw models auth login --provider <id> --set-default` or `marketingclaw models set <model>` to intentionally change the default model.
</Note>
...
`marketingclaw configure --section web` picks a web-search provider and configures its credentials. Some providers show provider-specific follow-ups:
