---
summary: "CLI reference for `marketingclaw hooks` (agent hooks)"
read_when:
...
Manage agent hooks (event-driven automations for commands like `/new`, `/reset`, and gateway startup). Bare `marketingclaw hooks` is equivalent to `marketingclaw hooks list`.
...
```bash
marketingclaw hooks list [--eligible] [--json] [-v|--verbose]
```
...
```bash
marketingclaw hooks info <name> [--json]
```
...
```bash
marketingclaw hooks check [--json]
```
...
```bash
marketingclaw hooks enable <name>
```
...
```bash
marketingclaw hooks disable <name>
```
...
```bash
marketingclaw plugins install <package>        # npm by default
marketingclaw plugins install npm:<package>    # npm only
marketingclaw plugins install <package> --pin  # pin resolved version
marketingclaw plugins install <path>           # local directory or archive
marketingclaw plugins install -l <path>        # link a local directory instead of copying

marketingclaw plugins update <id>
marketingclaw plugins update --all
marketingclaw plugins update --dry-run
```

Hook packs install through the unified plugins installer/updater; `marketingclaw hooks install` / `marketingclaw hooks update` still work as deprecated aliases that print a warning and forward to the `plugins` commands.
...
- Bare specs and `@latest` stay on the stable track; if npm resolves to a prerelease, MarketingClaw stops and asks you to opt in explicitly (`@beta`, `@rc`, or an exact prerelease version).
...
- Install copies the pack into `~/.marketingclaw/hooks/<id>`, enables its hooks under `hooks.internal.entries.*`, and records the install under `hooks.internal.installs`.
- If a stored integrity hash no longer matches the fetched artifact, MarketingClaw warns and prompts before continuing; pass global `--yes` to bypass the prompt (for example in CI).
...
| command-logger        | `command`                                         | Logs command events to `~/.marketingclaw/logs/commands.log`                                        |
...
Enable any bundled hook with `marketingclaw hooks enable <hook-name>`. Full details, config keys, and defaults: [Bundled hooks](/automation/hooks#bundled-hooks).

### command-logger log file

```bash
tail -n 20 ~/.marketingclaw/logs/commands.log        # recent commands
cat ~/.marketingclaw/logs/commands.log | jq .          # pretty-print
grep '"action":"new"' ~/.marketingclaw/logs/commands.log | jq .   # filter by action
```
