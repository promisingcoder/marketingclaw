---
summary: "CLI reference for `marketingclaw update` (safe-ish source update + gateway auto-restart)"
read_when:
  - You want to update a source checkout safely
  - You are debugging `marketingclaw update` output or options
...
Update MarketingClaw and switch between stable/extended-stable/beta/dev channels.
...
marketingclaw update repair
marketingclaw update wizard
marketingclaw update --channel extended-stable
marketingclaw update --channel beta
marketingclaw update --channel dev
marketingclaw update --tag beta
marketingclaw update --tag main
marketingclaw update --dry-run
marketingclaw update --no-restart
marketingclaw update --yes
marketingclaw update --acknowledge-clawhub-risk
marketingclaw update --json
marketingclaw --update
```

`marketingclaw --update` rewrites to `marketingclaw update` (useful for shells and
launcher scripts).
...
| `--tag <dist-tag\|version\|spec>`                | Override the package target for this update only. It cannot be combined with an effective `extended-stable` channel, whose verified exact target is mandatory. For other package installs, `main` maps to `github:marketingclaw/marketingclaw#main`; GitHub/git source specs are packed into a temporary tarball before the staged global npm install. |
...
| `--acknowledge-clawhub-risk`                     | Allow post-update plugin sync to continue past community ClawHub trust warnings without an interactive prompt. Without it, risky community releases are skipped and left unchanged when MarketingClaw cannot prompt. Official ClawHub packages and bundled plugin sources bypass this prompt.                                                          |

There is no `--verbose` flag. Use `--dry-run` to preview planned actions,
`--json` for machine-readable results, and `marketingclaw update status --json`
...
<Note>
In Nix mode (`MARKETINGCLAW_NIX_MODE=1`), mutating `marketingclaw update` runs are disabled. Update the Nix source or flake input for this install instead; for nix-marketingclaw, use the agent-first [Quick Start](https://github.com/openclaw/nix-openclaw#quick-start). `marketingclaw update status` and `marketingclaw update --dry-run` remain read-only.
</Note>
...
marketingclaw update status --timeout 10
```
...
`marketingclaw update` installed the new core package but post-core plugin sync,
managed npm plugin metadata, registry refresh, or doctor repair did not
converge.

```bash
marketingclaw update repair
marketingclaw update repair --channel beta
marketingclaw update repair --acknowledge-clawhub-risk
marketingclaw update repair --json
```
...
| `--acknowledge-clawhub-risk`                     | Same behavior as on `marketingclaw update`.                                                                                                                                                                                                                         |
| `--no-restart`                                   | Accepted for parity; repair never restarts the Gateway.                                                                                                                                                                                                             |

`update repair` runs `marketingclaw doctor --fix`, reloads the repaired config and
...
- `dev` -> ensures a git checkout (default `~/marketingclaw`, or
  `$MARKETINGCLAW_HOME/marketingclaw` when `MARKETINGCLAW_HOME` is set; override with
  `MARKETINGCLAW_GIT_DIR`), updates it, and installs the global CLI from that
  checkout.
...
detached helper and exits, and that helper runs `marketingclaw update --yes --json`
...
healthy. If the plist is installed but launchd is not supervising it, MarketingClaw
...
`Gateway: restart failed: ...` with a manual `marketingclaw gateway restart` hint.
...
- `ok: true`, `result.status: "skipped"`,
  ...
  `marketingclaw update --yes --json` outside the live service process.
- `ok: false`, `result.reason: "managed-service-handoff-unavailable"`, and
  `handoff.status: "unavailable"`: MarketingClaw could not find a supervising
  service boundary and durable service identity for a safe handoff (for
  example, systemd handoff requires the `MARKETINGCLAW_SYSTEMD_UNIT` unit identity,
...
`marketingclaw status` and `marketingclaw status --all` show an `Update restart` row
...
<Steps>
  ...
  <Step title="Preflight build (dev only)">
    Runs the TypeScript build in a temp worktree. If the tip fails, walks back up to 10 commits to find the newest buildable commit. Set `MARKETINGCLAW_UPDATE_PREFLIGHT_LINT=1` to also run lint during this preflight; lint runs in constrained serial mode because user update hosts are often smaller than CI runners.
  </Step>
  <Step title="Rebase">
  ...
  <Step title="Run doctor">
    `marketingclaw doctor` runs as the final safe-update check.
  </Step>
...
beta release, MarketingClaw falls back to the recorded default/latest spec and
reports a warning. For npm plugins, MarketingClaw also falls back when the beta
...
<Warning>
If an exact pinned npm plugin update resolves to an artifact whose integrity differs from the stored install record, `marketingclaw update` aborts that plugin artifact update instead of installing it. Reinstall or update the plugin explicitly only after verifying you trust the new artifact.
</Warning>

<Note>
Post-update plugin sync failures that are scoped to a managed plugin and that the sync path can route around (for example an unreachable npm registry for a non-essential plugin) are reported as warnings after the core update succeeds. The JSON result keeps top-level update `status: "ok"` and reports `postUpdate.plugins.status: "warning"` with `marketingclaw update repair` and `marketingclaw plugins inspect <id> --runtime --json` guidance. Unexpected updater or sync exceptions still fail the update result. Fix the plugin install or update error, then rerun `marketingclaw update repair`.

After the per-plugin sync step, `marketingclaw update` runs a mandatory **post-core convergence** pass before the gateway restarts: it repairs missing configured plugin payloads, validates each _active_ tracked install record on disk, and statically verifies its `package.json` is parseable (and any explicitly declared `main` exists). Failures from this pass, and an invalid config snapshot, return `postUpdate.plugins.status: "error"` and flip the top-level update `status` to `"error"`, so `marketingclaw update` exits non-zero and the gateway is _not_ restarted with an unverified plugin set. The error includes structured `postUpdate.plugins.warnings[].guidance` lines pointing at `marketingclaw update repair` and `marketingclaw plugins inspect <id> --runtime --json`. Disabled plugin entries and records that are not trusted-source-linked official sync targets are skipped here (mirroring the `skipDisabledPlugins` policy used by the missing-payload check), so a stale disabled plugin record cannot block an otherwise valid update.
...
version. For default/`latest` intent, MarketingClaw does not query plugin
...
For package-manager installs, `marketingclaw update` resolves the target package
version before invoking the package manager. npm global installs use a staged
install: MarketingClaw installs the new package into a temporary npm prefix,
...
plugin records aligned with the installed MarketingClaw build, while leaving full
plugin-command completion rebuilds to explicit
`marketingclaw completion --write-state` runs.

## Related

- `marketingclaw doctor` (offers to run update first on git checkouts)
