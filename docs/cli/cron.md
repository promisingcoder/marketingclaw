---
summary: "CLI reference for `marketingclaw cron` (schedule and run background jobs)"
read_when:
...
<Tip>
Run `marketingclaw cron --help` for the full command surface. See [Cron jobs](/automation/cron-jobs) for the conceptual guide.
</Tip>
...
`marketingclaw cron create` is an alias for `marketingclaw cron add`. For new jobs, put the schedule first and the prompt second:
...
Use `--command` for deterministic shell-style jobs that run inside MarketingClaw cron without starting an isolated agent/model run:
...
`marketingclaw cron list` and `marketingclaw cron show <job-id>` preview the resolved delivery route. For `channel: "last"`, the preview shows whether the route resolved from the main or current session, or will fail closed.
...
Skipped runs are tracked separately from execution errors. They do not affect retry backoff, but `marketingclaw cron edit <job-id> --failure-alert-include-skipped` can opt failure alerts into repeated skipped-run notifications.
...
Cron jobs, pending runtime state, and run history live in the shared SQLite state database. Legacy `jobs.json`, `<name>-state.json`, and `runs/*.jsonl` files are imported once and renamed with a `.migrated` suffix. After import, edit schedules with `marketingclaw cron add|edit|remove` instead of editing JSON files.

### Manual runs

`marketingclaw cron run <job-id>` force-runs by default and returns as soon as the manual run is queued. Successful responses include `{ ok: true, enqueued: true, runId }`. Use the returned `runId` to inspect the later result:

```bash
marketingclaw cron run <job-id>
marketingclaw cron runs --id <job-id> --run-id <run-id>
```

Add `--wait` when a script should block until that exact queued run records a terminal status:

```bash
marketingclaw cron run <job-id> --wait --wait-timeout 10m --poll-interval 2s
```
...
- When a job has `--model` but no fallback list is configured, MarketingClaw passes an explicit empty fallback override so the agent primary is not appended as a hidden retry target.
- Local-provider preflight checks walk configured fallbacks before marking a cron run `skipped`.

`marketingclaw doctor` reports jobs that already have `payload.model` set, including provider namespace counts and mismatches against `agents.defaults.model`. Use that check when auth, provider, or billing behavior looks different between live chat and scheduled jobs.
...
```bash
marketingclaw cron edit <job-id> --announce --channel telegram --to "123456789"
```

Disable delivery for an isolated job:

```bash
marketingclaw cron edit <job-id> --no-deliver
```

Enable lightweight bootstrap context for an isolated job:

```bash
marketingclaw cron edit <job-id> --light-context
```

Announce to a specific channel:

```bash
marketingclaw cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
```

Announce to a Telegram forum topic:

```bash
marketingclaw cron edit <job-id> --announce --channel telegram --to "-1001234567890" --thread-id 42
```
...
marketingclaw cron list --agent ops
marketingclaw cron get <job-id>
marketingclaw cron show <job-id>
marketingclaw cron run <job-id>
marketingclaw cron run <job-id> --due
marketingclaw cron run <job-id> --wait --wait-timeout 10m
marketingclaw cron run <job-id> --wait --wait-timeout 10m --poll-interval 2s
marketingclaw cron runs --id <job-id> --limit 50
marketingclaw cron runs --id <job-id> --run-id <run-id>
```

`marketingclaw cron list` shows all matching jobs by default. Pass `--agent <id>` to show only jobs whose effective normalized agent id matches; jobs without a stored agent id count as the configured default agent.

`marketingclaw cron get <job-id>` returns the stored job JSON directly. Use `cron show <job-id>` when you want the human-readable view with delivery-route preview.
...
```bash
marketingclaw cron edit <job-id> --agent ops
marketingclaw cron edit <job-id> --clear-agent
marketingclaw cron edit <job-id> --session current
marketingclaw cron edit <job-id> --session "session:daily-brief"
```

`marketingclaw cron add` warns when `--agent` is omitted on agent-turn jobs and falls back to the default agent (`main`). Pass `--agent <id>` at create time to pin a specific agent.

Delivery tweaks:

```bash
marketingclaw cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
marketingclaw cron edit <job-id> --webhook "https://example.invalid/marketingclaw/cron"
marketingclaw cron edit <job-id> --best-effort-deliver
marketingclaw cron edit <job-id> --no-best-effort-deliver
marketingclaw cron edit <job-id> --no-deliver
```
