---
summary: "CLI reference for `marketingclaw doctor` (health checks + guided repairs)"
read_when:
...
| ------- | ----------------------------- | --------------------------------------------------------------------------- |
| Inspect | `marketingclaw doctor`        | Human-oriented checks and guided prompts.                                   |
| Repair  | `marketingclaw doctor --fix`  | Applies supported repairs, prompting unless non-interactive repair is safe. |
| Lint    | `marketingclaw doctor --lint` | Read-only structured findings for CI, preflight, and review gates.          |
...
marketingclaw doctor --lint --allow-exec
marketingclaw doctor --deep
marketingclaw doctor --fix
marketingclaw doctor --fix --non-interactive
marketingclaw doctor --generate-gateway-token
marketingclaw doctor --post-upgrade
marketingclaw doctor --post-upgrade --json
```

For channel-specific permissions, use the channel probes instead of `doctor`:

```bash
marketingclaw channels capabilities --channel discord --target channel:<channel-id>
...
`marketingclaw doctor --lint` is read-only: no prompts, no repair, no config/state rewrites.
...
doctor --lint: ran 6 check(s), 1 finding(s)
  [warning] core/doctor/gateway-config gateway.mode - gateway.mode is unset; gateway start will be blocked.
    fix: Run `marketingclaw configure` and set Gateway mode (local/remote), or `marketingclaw config set gateway.mode local`.
```
  ...
  "findings": [
    {
      ...
      "path": "gateway.mode",
      "fixHint": "Run `marketingclaw configure` and set Gateway mode (local/remote), or `marketingclaw config set gateway.mode local`."
    }
  ]
}
```
...
`--severity-min` controls both which findings print and the exit threshold: `marketingclaw doctor --lint --severity-min error` can print nothing and exit `0` even when lower-severity `info`/`warning` findings exist.
...
`marketingclaw doctor --fix` against the same mounted state/config before restarting
the container normally.

## Notes

- In Nix mode (`MARKETINGCLAW_NIX_MODE=1`), read-only doctor checks still work, but `doctor --fix`, `doctor --repair`, `doctor --yes`, and `doctor --generate-gateway-token` are disabled because `marketingclaw.json` is immutable. Edit the Nix source for this install instead; for nix-marketingclaw, use the agent-first [Quick Start](https://github.com/openclaw/nix-openclaw#quick-start).
...
- Any config write (including a `--fix` repair) rotates a backup to `~/.marketingclaw/marketingclaw.json.bak` (with a numbered `.bak.1`..`.bak.4` ring). `--fix` also drops unknown config keys reported by schema validation, listing each removal; it skips this while an update is in progress so partially written upgrade state is not stripped before its migration finishes.
- Set `MARKETINGCLAW_SERVICE_REPAIR_POLICY=external` when another supervisor owns the gateway lifecycle. Doctor still reports gateway/service health and applies non-service repairs, but skips service install/start/restart/bootstrap and legacy service cleanup.
- On Linux, doctor ignores inactive extra gateway-like systemd units and does not rewrite command/entrypoint metadata for a running systemd gateway service during repair. Stop the service first, or use `marketingclaw gateway install --force` to replace the active launcher.
- `doctor --fix --non-interactive` reports missing or stale gateway service definitions but does not install or rewrite them outside update repair mode. Run `marketingclaw gateway install` for a missing service, or `marketingclaw gateway install --force` to replace the launcher.
...
- Doctor scans `~/.marketingclaw/cron/jobs.json` (or `cron.store`) for legacy cron job shapes and rewrites them before importing canonical rows into SQLite.
...
- On Linux, doctor warns when the user's crontab still runs the unmaintained legacy `~/.marketingclaw/bin/ensure-whatsapp.sh`, which can misreport `Gateway inactive` when cron lacks the systemd user-bus environment.
- When WhatsApp is enabled, doctor checks for a degraded Gateway event loop with local `marketingclaw-tui` clients still running. `doctor --fix` stops only verified local TUI clients so WhatsApp replies are not queued behind stale TUI refresh loops.
...
- Doctor cleans legacy plugin dependency staging state from older MarketingClaw versions and relinks the host `marketingclaw` package for managed npm plugins that declare it as a peer dependency. It also repairs missing downloadable plugins referenced by config (`plugins.entries`, configured channels, configured provider/search settings, configured agent runtimes). During package updates, doctor skips package-manager plugin repair until the package swap completes; rerun `marketingclaw doctor --fix` afterward if a configured plugin still needs recovery. If a download fails, doctor reports the install error and preserves the configured plugin entry for the next repair attempt.
...
- Doctor includes a memory-search readiness check and can recommend `marketingclaw configure --section model` when embedding credentials are missing.
...
- Doctor reports an info note when Codex-mode agents are configured and personal Codex CLI assets exist in the operator's Codex home. Local Codex app-server launches use isolated per-agent homes; install the Codex plugin first if needed, then use `marketingclaw migrate plan codex` to inventory assets that should be promoted deliberately.
...
- If sandbox mode is enabled but Docker is unavailable, doctor reports a high-signal warning with remediation (`install Docker` or `marketingclaw config set agents.defaults.sandbox.mode off`).
- If legacy sandbox registry files or shard directories are present (`~/.marketingclaw/sandbox/containers.json`, `~/.marketingclaw/sandbox/browsers.json`, `~/.marketingclaw/sandbox/containers/`, or `~/.marketingclaw/sandbox/browsers/`), doctor reports them; `--fix` migrates valid entries into SQLite and quarantines invalid legacy files.
...
If you previously ran `launchctl setenv MARKETINGCLAW_GATEWAY_TOKEN ...` (or `...PASSWORD`), that value overrides your config file and can cause persistent "unauthorized" errors.

```bash
launchctl getenv MARKETINGCLAW_GATEWAY_TOKEN
launchctl getenv MARKETINGCLAW_GATEWAY_PASSWORD

launchctl unsetenv MARKETINGCLAW_GATEWAY_TOKEN
launchctl unsetenv MARKETINGCLAW_GATEWAY_PASSWORD
```
