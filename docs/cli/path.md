---
summary: "CLI reference for `marketingclaw path` (inspect and edit workspace files via the `oc://` addressing scheme)"
read_when:
...
```bash
marketingclaw plugins enable oc-path
```
...
MarketingClaw state is spread across human-edited markdown, commented JSONC
...
`marketingclaw path` gives those callers a stable address instead of a one-off
...
Skip `marketingclaw path` for ordinary whole-file edits, rich config migrations, or
...
```bash
marketingclaw path resolve 'oc://config.jsonc/plugins/github/enabled'
```

Preview a write without touching disk:

```bash
marketingclaw path set 'oc://config.jsonc/plugins/github/enabled' 'true' --dry-run
```

Find matching records in an append-only JSONL log:

```bash
marketingclaw path find 'oc://session.jsonl/[event=tool_call]/name'
```
...
```bash
marketingclaw path resolve 'oc://AGENTS.md/runtime-safety/marketingclaw-gateway'
```
...
```bash
marketingclaw path validate 'oc://AGENTS.md/tools/$last/risk'
```
...
# Read a leaf
marketingclaw path resolve 'oc://gateway.jsonc/version'

# Wildcard search
marketingclaw path find 'oc://session.jsonl/*/event' --file ./logs/session.jsonl

# Dry-run a write
marketingclaw path set 'oc://gateway.jsonc/version' '2.0' --dry-run

# Dry-run a write as a unified diff
marketingclaw path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff

# Apply the write
marketingclaw path set 'oc://gateway.jsonc/version' '2.0'

# Byte-fidelity round-trip (diagnostic)
marketingclaw path emit ./AGENTS.md
```
...
marketingclaw path resolve 'oc://config.jsonc/agents.defaults.models/"anthropic/claude-opus-4-7"/alias'

# Deep JSON/JSONC paths can use slash segments; they normalize to dotted subsegments
marketingclaw path set 'oc://marketingclaw.json/agents/list/0/tools/exec/security' 'allowlist' --dry-run

# Replace a JSONC leaf with a parsed object
marketingclaw path set 'oc://marketingclaw.json/gateway/auth/token' '{"source":"file","provider":"secrets","id":"/test"}' --value-json --dry-run

# Predicate search over JSONC children
marketingclaw path find 'oc://config.jsonc/plugins/[enabled=true]/id'

# Insert into a JSONC array
marketingclaw path set 'oc://config.jsonc/items/+1' '{"id":"new","enabled":true}' --dry-run

# Insert a JSONC object key
marketingclaw path set 'oc://config.jsonc/plugins/+github' '{"enabled":true}' --dry-run

# Append a JSONL event
marketingclaw path set 'oc://session.jsonl/+' '{"event":"checkpoint","ok":true}' --file ./logs/session.jsonl

# Resolve the last JSONL value line
marketingclaw path resolve 'oc://session.jsonl/$last/event' --file ./logs/session.jsonl

# Resolve a YAML workflow step
marketingclaw path resolve 'oc://workflow.yaml/steps/0/id'

# Update a YAML scalar
marketingclaw path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --dry-run

# Address markdown frontmatter
marketingclaw path resolve 'oc://AGENTS.md/[frontmatter]/name'

# Insert markdown frontmatter
marketingclaw path set 'oc://AGENTS.md/[frontmatter]/+description' 'Agent instructions' --dry-run

# Find markdown item fields
marketingclaw path find 'oc://SKILL.md/Tools/*/send_email'

# Validate a session-scoped path
marketingclaw path validate 'oc://AGENTS.md/Tools/$last/risk?session=cron-daily'
```
...
```bash
$ marketingclaw path resolve 'oc://x.md/[frontmatter]/tier' --file frontmatter.md --human
leaf @ L4: "core" (string)

$ marketingclaw path resolve 'oc://x.md/tools/gh/gh' --file frontmatter.md --human
leaf @ L9: "GitHub CLI" (string)

$ marketingclaw path find 'oc://x.md/tools/*' --file frontmatter.md --human
...
```bash
$ marketingclaw path resolve 'oc://config.jsonc/plugins/github/enabled' --file config.jsonc --human
leaf @ L4: "true" (boolean)

$ marketingclaw path set 'oc://config.jsonc/plugins/slack/enabled' 'true' --file config.jsonc --dry-run
...
```bash
$ marketingclaw path find 'oc://session.jsonl/[event=action]/userId' --file session.jsonl --human
1 match for oc://session.jsonl/[event=action]/userId:
  oc://session.jsonl/L2/userId  →  leaf @ L2: "u1" (string)

$ marketingclaw path resolve 'oc://session.jsonl/L2/ts' --file session.jsonl --human
leaf @ L2: "2" (number)
```
...
steps:
  ...
  - id: classify
    command: marketingclaw.invoke
```

```bash
$ marketingclaw path resolve 'oc://workflow.yaml/steps/0/id' --file workflow.yaml --human
leaf @ L3: "fetch" (string)

$ marketingclaw path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --file workflow.yaml --dry-run
...
```bash
marketingclaw path resolve 'oc://AGENTS.md/tools/gh/risk' --human
marketingclaw path resolve 'oc://gateway.jsonc/server/port' --json
```
...
```bash
marketingclaw path find 'oc://AGENTS.md/tools/**/risk'
marketingclaw path find 'oc://session.jsonl/[event=action]/userId'
marketingclaw path find 'oc://config.jsonc/plugins/{github,slack}/enabled'
```
...
marketingclaw path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff
marketingclaw path set 'oc://gateway.jsonc/version' '2.0'
marketingclaw path set 'oc://AGENTS.md/Tools/+gh/risk' 'low'
```
...
```bash
$ marketingclaw path validate 'oc://AGENTS.md/tools/gh' --human
...
```bash
marketingclaw path emit ./AGENTS.md
marketingclaw path emit ./gateway.jsonc --json
```
...
`marketingclaw path` is TTY-aware: human-readable output on a terminal, JSON when
...
- `set` writes bytes through the substrate's emit path, which applies the
  ...
  `__MARKETINGCLAW_REDACTED__` (verbatim or as a substring) is refused at write
  time.
