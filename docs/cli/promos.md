---
summary: "CLI reference for `marketingclaw promos` (list and claim promotional model offers)"
read_when:
...
```bash
marketingclaw promos list
marketingclaw promos claim <slug>
marketingclaw promos claim <slug> --api-key <key> --set-default
```
...
2. Validates the promotion's provider, auth choice, and declared plugin packages
   against your installed MarketingClaw version. Unknown ids or package mismatches are
...
3. Reuses your existing provider credentials when you have them. Otherwise it
   ...
   prompts, matching the `marketingclaw onboard` non-interactive flags; to keep the
...
`marketingclaw models set <model>`.

## Passive discovery in `models list`

`marketingclaw models list` also surfaces promotions without you asking ClawHub
directly:
...
- The first time a new offer is seen, a one-time notice points at
  `marketingclaw promos list`. Offers you have already listed or claimed are never
  announced again.
