---
summary: "CLI reference for `marketingclaw nodes` (status, pairing, invoke, camera/canvas/screen/location/notify)"
read_when:
...
marketingclaw nodes status --last-connected 24h
marketingclaw nodes list
...
marketingclaw nodes rename --node <id|name|ip> --name <displayName>
```

These commands drive the gateway-owned `node.pair.*` store, separate from device pairing (`marketingclaw devices approve`) that gates the node's WS `connect` handshake. See [Nodes](/nodes) for how the two relate.
...
```bash
marketingclaw nodes notify --node <id> --title "Build" --body "Done" --priority timeSensitive
marketingclaw nodes push --node <id> --title "MarketingClaw" --environment sandbox
marketingclaw nodes location get --node <id> --accuracy precise
marketingclaw nodes screen record --node <id> --duration 10s --fps 10 --out ./clip.mp4
```

- `notify` sends a local notification on a node that declares `system.notify`, including macOS, iOS, Android, and direct watchOS nodes. Direct watchOS delivery requires MarketingClaw to be active. Requires `--title` or `--body`. Options: `--sound <name>`, `--priority <passive|active|timeSensitive>`, `--delivery <system|overlay|auto>` (default `system`), `--invoke-timeout <ms>` (default `15000`).
- `push` sends an APNs test push to an iOS node. Options: `--title <text>` (default `MarketingClaw`), `--body <text>`, `--environment <sandbox|production>` to override the detected APNs environment.
...
Camera and Canvas commands have their own docs: [Camera nodes](/nodes/camera), [Canvas](/platforms/mac/canvas). Canvas is implemented by the bundled experimental Canvas plugin; core keeps `marketingclaw nodes canvas` as a compatibility mount point.
