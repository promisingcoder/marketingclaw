---
summary: "CLI reference for `marketingclaw logs` (tail gateway logs via RPC)"
read_when:
...
marketingclaw logs --follow --interval 2000
marketingclaw logs --limit 500 --max-bytes 500000
marketingclaw logs --json
marketingclaw logs --plain
marketingclaw logs --no-color
marketingclaw logs --utc
marketingclaw logs --follow --local-time
marketingclaw logs --url ws://127.0.0.1:18789 --token "$MARKETINGCLAW_GATEWAY_TOKEN"
```

## Fallback and recovery behavior

- If the implicit local loopback Gateway asks for pairing, closes during connect, or times out before `logs.tail` answers, `marketingclaw logs` falls back to the configured Gateway file log automatically. Explicit `--url` targets never use this fallback.
