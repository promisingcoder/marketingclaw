---
summary: "CLI reference for `marketingclaw webhooks` (Gmail Pub/Sub setup and runner)"
read_when:
  - You want to wire Gmail Pub/Sub events into MarketingClaw
  - You need the full flag list and default values
title: "Webhooks"
---

# `marketingclaw webhooks`
...
```bash
marketingclaw webhooks gmail setup --account <email> [...]
marketingclaw webhooks gmail run   [--account <email>] [...]
```

| Subcommand    | Description                                                                                |
| ------------- | ------------------------------------------------------------------------------------------ |
| `gmail setup` | One-time wizard: Gmail watch, Pub/Sub topic/subscription, and MarketingClaw hook delivery. |
| `gmail run`   | Run `gog watch serve` plus the watch auto-renew loop in the foreground.                    |

<Note>
The Gateway also auto-starts `gog gmail watch serve` on boot once `hooks.enabled=true` and `hooks.gmail.account` is set (set by `gmail setup`). `gmail run` is the same logic in the foreground, useful for debugging or when the Gateway watcher is disabled. See [Gmail Pub/Sub integration](/automation/cron-jobs#gmail-pubsub-integration) for the auto-start details and `MARKETINGCLAW_SKIP_GMAIL_WATCHER` opt-out.
</Note>

## `webhooks gmail setup`

```bash
marketingclaw webhooks gmail setup --account you@example.com
marketingclaw webhooks gmail setup --account you@example.com --project my-gcp-project --json
marketingclaw webhooks gmail setup --account you@example.com --hook-url https://gateway.example.com/hooks/gmail
```

Installs `gcloud` and `gog` if missing, authenticates `gcloud`, creates the Pub/Sub topic and subscription, starts the Gmail watch, and writes `hooks.gmail` config with `hooks.enabled=true`. Prints `Next: marketingclaw webhooks gmail run`.
...
### MarketingClaw delivery options

| Flag                   | Default                                      | Description                                |
| ---------------------- | -------------------------------------------- | ------------------------------------------ |
| `--hook-url <url>`     | Built from `hooks.path` and the Gateway port | MarketingClaw webhook URL.                 |
| `--hook-token <token>` | `hooks.token`, or a generated token          | MarketingClaw webhook token.               |
...
```bash
marketingclaw webhooks gmail run --account you@example.com
```
...
`run` accepts the same Pub/Sub, MarketingClaw delivery, `gog watch serve`, and Tailscale flags as `setup`, except:
...
| MarketingClaw delivery | `--hook-url`, `--hook-token`, `--push-token`                                     |
