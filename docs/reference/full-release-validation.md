...
`release_package_spec` values derive `codex_plugin_spec=npm:@marketingclaw/codex@<version>`;
...
`MARKETINGCLAW_EXTENSIONS=diagnostics-otel,codex` before anything else dispatches. A
...
| Product performance     | **Job:** `Run product performance evidence`<br />**Child workflow:** `MarketingClaw Performance`<br />**Proves:** release-profile performance run (`profile=release`, `repeat=3`, `fail_on_regression=true`) against the target SHA. Required (blocking) only for `rerun_group=all` or `rerun_group=performance`; not required for narrower rerun groups.<br />**Rerun:** `rerun_group=performance`.                                                              |
...
`MarketingClaw Release Checks` is the largest child workflow. It resolves the target
...
| QA runtime parity        | **Job:** `Run QA Lab runtime parity lane`<br />**Backing workflow:** direct job<br />**Tests:** an `marketingclaw`/`codex` runtime-pair agentic parity lane (`pnpm marketingclaw qa suite --runtime-pair marketingclaw,codex`), including a standard tier and, with `run_release_soak=true`, a soak tier. Advisory: individual failures do not block the release-check verifier.<br />**Rerun:** `rerun_group=qa-parity` or `rerun_group=qa`.                                                                                                                                     |
| QA runtime tool coverage | **Job:** `Enforce QA Lab runtime tool coverage`<br />**Backing workflow:** direct job<br />**Tests:** dynamic tool drift between `marketingclaw` and `codex` in the standard runtime-parity tier (`pnpm marketingclaw qa coverage --tools`), using the QA runtime parity lane's output. Blocking: this job is not advisory-overridable.<br />**Rerun:** `rerun_group=qa-parity` or `rerun_group=qa`.                                                                                                                                                                              |
...
| `release-checks`    | All MarketingClaw Release Checks stages.                                                        |
...
coverage check (dynamic tool drift between `marketingclaw` and `codex` in the
...
WhatsApp, or Slack, the matching `MARKETINGCLAW_RELEASE_QA_*_LIVE_CI_ENABLED` repo
...
- `release-package-under-test` from `MarketingClaw Release Checks`
...
- `.github/workflows/marketingclaw-release-checks.yml`
- `.github/workflows/marketingclaw-live-and-e2e-checks-reusable.yml`
- `.github/workflows/plugin-prerelease.yml`
- `.github/workflows/install-smoke.yml`
- `.github/workflows/marketingclaw-cross-os-release-checks-reusable.yml`
- `.github/workflows/package-acceptance.yml`
- `.github/workflows/marketingclaw-performance.yml`
