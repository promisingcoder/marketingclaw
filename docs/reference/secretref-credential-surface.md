...
- In scope: strictly user-supplied credentials that MarketingClaw does not mint or rotate.
...
### `marketingclaw.json` targets (`secrets configure` + `secrets apply` + `secrets audit`)
...
- In `marketingclaw.json`, SecretRefs must use structured objects such as `{"source":"env","provider":"default","id":"DISCORD_BOT_TOKEN"}`. Legacy `secretref-env:<ENV_VAR>` marker strings are rejected on SecretRef credential paths; run `marketingclaw doctor --fix` to migrate valid markers.
