...
read_when:
  - Planning a broad MarketingClaw application modernization pass
...
Use this section to update the frontend-focused `SKILL.md` supplied with the modernization task. If adopting this guidance as a repo-local MarketingClaw skill, create `.agents/skills/marketingclaw-frontend/SKILL.md` first, keep the frontmatter that belongs in that target skill, then add or replace the body guidance with the following content.
