...
| Local             | `local`             | `@marketingclaw/llama-cpp-provider` |
| Mistral           | `mistral`           |                                     |
...
MarketingClaw indexes `MEMORY.md` and `memory/*.md` into chunks (400 tokens with
...
- **Reindex on demand:** `marketingclaw memory index --force`
...
**Memory search disabled?** Check `marketingclaw memory status`. If no provider is
...
**Stale results?** Run `marketingclaw memory index --force` to rebuild. The watcher
may miss changes in rare edge cases.

**sqlite-vec not loading?** MarketingClaw falls back to in-process cosine
similarity automatically. `marketingclaw memory status --deep` reports the local
