...
alongside MarketingClaw. It combines BM25, vector search, and reranking in a single
...
- **Automatic fallback** - if QMD is unavailable, MarketingClaw falls back to the
...
MarketingClaw creates a self-contained QMD home under
`~/.marketingclaw/agents/<agentId>/qmd/` and manages the sidecar lifecycle
...
- MarketingClaw creates collections from your workspace memory files and any
...
- Searches use the configured `searchMode` (default: `search`; also supports
  `vsearch` and `query`). `search` is BM25-only, so MarketingClaw skips semantic
  vector readiness probes and embedding maintenance in that mode. If a mode
  fails, MarketingClaw retries with `qmd query`.
- When `searchMode` is `query`, set `memory.qmd.rerank` to `false` to use
  ...
  MarketingClaw passes `--no-rerank` to the direct QMD CLI path and
  `rerank: false` to QMD's MCP query tool.
- With QMD releases that advertise multi-collection filters, MarketingClaw groups
  same-source collections into one QMD search invocation. Older QMD releases
  keep the compatible per-collection fallback.
- If QMD fails entirely, MarketingClaw falls back to the builtin SQLite engine.
  ...
  `marketingclaw memory status` and one-shot CLI probes still recheck QMD
  directly.
...
MarketingClaw keeps the QMD search path compatible with both current and older QMD
installs.

On startup, MarketingClaw checks the installed QMD help text once per manager. If
the binary advertises support for multiple collection filters, MarketingClaw
...
Older QMD builds only accept one collection filter. When MarketingClaw detects one
...
process, so you can tune QMD globally without adding new MarketingClaw config:
...
collection under `~/.marketingclaw/agents/<id>/qmd/sessions/`. Setting only
...
MarketingClaw logs a warning with the derived channel and chat type so empty
...
**QMD not found?** Ensure the binary is on the gateway's `PATH`. If MarketingClaw
runs as a service, create a symlink:
`sudo ln -s ~/.bun/bin/qmd /usr/local/bin/qmd`.

If `qmd --version` works in your shell but MarketingClaw still reports
...
with `marketingclaw memory status --deep`.

**First search very slow?** QMD downloads GGUF models on first use. Pre-warm
with `qmd query "test"` using the same XDG dirs MarketingClaw uses.

**Many QMD subprocesses during search?** Update QMD if possible. MarketingClaw
...
`memory.qmd.searchMode = "search"`. MarketingClaw treats that mode as
...
for the next startup reconciliation. MarketingClaw recreates stale managed
...
QMD traversal follows the underlying QMD scanner rather than MarketingClaw's
