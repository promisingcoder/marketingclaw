...
read_when:
  ...
  - You need to restore or clean up an MarketingClaw-managed worktree
title: "Managed worktrees"
---

Managed worktrees give an agent task its own git branch and checkout without placing temporary directories inside the source repository. MarketingClaw creates them under its state directory, records them in the shared state database, and snapshots their tracked and non-ignored untracked contents before removal.
...
```text
<marketingclaw-state-dir>/worktrees/<repo-fingerprint>/<name>
```

The repository fingerprint is the first 16 hexadecimal characters of a SHA-256 hash over the canonical git common directory and origin URL. A supplied name must match `[a-z0-9][a-z0-9-]{0,63}`. Without a name, MarketingClaw generates `wt-` followed by eight random hexadecimal characters.

MarketingClaw creates branch `marketingclaw/<name>` at the requested base ref. Without a base ref, it fetches `origin`, uses the remote default branch when available, and falls back to local `HEAD` when the repository is offline or has no usable remote.
...
Only files reported by git as both ignored and untracked are eligible. Tracked files are already present through git and are never copied by this step. MarketingClaw does not overwrite destination files or follow symlinked directories, and it preserves copied file modes.

## Run repository setup

If `.marketingclaw/worktree-setup.sh` exists in the source repository and is executable, MarketingClaw runs it with the new worktree as its current directory. The script receives:

```text
MARKETINGCLAW_SOURCE_TREE_PATH=<source checkout>
MARKETINGCLAW_WORKTREE_PATH=<managed worktree>
```

A nonzero exit aborts creation and removes the new worktree and branch. This is a repository-local contract; there is no MarketingClaw config key for it.
...
- A live MarketingClaw process lock and any foreign or unrecognized git worktree lock protect a worktree from garbage collection.

Restore recreates `marketingclaw/<name>` at the original pre-snapshot commit, then rebuilds the snapshot differences as unstaged modifications and untracked files. This keeps the synthetic snapshot commit out of branch history. The snapshot ref remains recorded as provenance.

## CLI

```bash
marketingclaw worktrees list [--json]
marketingclaw worktrees create <repo-root> [--name <name>] [--base-ref <ref>] [--json]
marketingclaw worktrees remove <id> [--force] [--json]
marketingclaw worktrees restore <id> [--json]
marketingclaw worktrees gc [--json]
```
