...
read_when:
  - You want MarketingClaw to remember natural follow-ups
  - You want to understand how inferred check-ins differ from reminders
  - You want to review or dismiss follow-up commitments
---

Commitments are short-lived follow-up memories. When enabled, MarketingClaw can
notice that a conversation created a future check-in opportunity and remember
to bring it back later.

Examples:

- You mention an interview tomorrow. MarketingClaw may check in afterward.
- You say you are exhausted. MarketingClaw may ask later whether you slept.
- The agent says it will follow up after something changes. MarketingClaw may track
  that open loop.

Commitments are not durable facts like `MEMORY.md`, and they are not exact
reminders. They sit between memory and automation: MarketingClaw remembers a
...
```bash
marketingclaw config set commitments.enabled true
marketingclaw config set commitments.maxPerDay 3
```

Equivalent `marketingclaw.json`:
...
After an agent reply, MarketingClaw may run a hidden background extraction pass in a
...
When it finds a high-confidence candidate, MarketingClaw stores a commitment with:
...
metadata, and due-commitment heartbeat turns run without MarketingClaw tools.

MarketingClaw never delivers an inferred commitment immediately after writing it.
...
marketingclaw commitments dismiss cm_abc123
```

See [`marketingclaw commitments`](/cli/commitments) for the full command reference.
...
Stored commitments are local MarketingClaw state. They are operational memory, not
long-term memory. Disable the feature with:

```bash
marketingclaw config set commitments.enabled false
```
...
- Check `marketingclaw commitments --all` for pending, dismissed, snoozed, or expired
  records.
...
- [`marketingclaw commitments`](/cli/commitments)
