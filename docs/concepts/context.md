...
read_when:
  - You want to understand what "context" means in MarketingClaw
  - You are debugging why the model "knows" something (or forgot it)
  - You want to reduce context overhead (/context, /status, /compact)
title: "Context"
---

"Context" is **everything MarketingClaw sends to the model for a run**. It is bounded by the model's **context window** (token limit).

Beginner mental model:

- **System prompt** (MarketingClaw-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How MarketingClaw builds the system prompt

The system prompt is **MarketingClaw-owned** and rebuilt each run. It includes:
...
By default, MarketingClaw injects a fixed set of workspace files (if present):
...
Large files are truncated per-file using `agents.defaults.bootstrapMaxChars` (default `20000` chars). MarketingClaw also enforces a total bootstrap injection cap across files with `agents.defaults.bootstrapTotalMaxChars` (default `60000` chars). `/context` shows **raw vs injected** sizes and whether truncation happened.
...
By default, MarketingClaw uses the built-in `legacy` context engine for assembly and
compaction. If you install a plugin that provides `kind: "context-engine"` and
select it with `plugins.slots.contextEngine`, MarketingClaw delegates context
