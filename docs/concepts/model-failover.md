---
summary: "How MarketingClaw rotates auth profiles and falls back across models"
read_when:
...
MarketingClaw handles failures in two stages:
...
- **Auto fallback override**: a runtime fallback writes `providerOverride`, `modelOverride`, `modelOverrideSource: "auto"`, and the selected origin model before retrying. This override keeps walking the configured fallback chain without probing the primary on every message, but MarketingClaw probes the configured origin every 5 minutes (not configurable) and clears the override once it recovers. `/new`, `/reset`, and `sessions.reset` also clear auto-sourced overrides. Heartbeat runs without an explicit `heartbeat.model` clear direct auto overrides when their origin no longer matches the current configured default.
- **User session override**: `/model`, the model picker, `session_status(model=...)`, and `sessions.patch` write `modelOverrideSource: "user"`. This is an exact session selection. If the selected provider/model fails before producing a reply, MarketingClaw reports the failure instead of answering from an unrelated configured fallback.
- **Legacy session override**: older session entries may have `modelOverride` without `modelOverrideSource`. MarketingClaw treats those as user overrides so an explicit old selection is not silently converted into fallback behavior.
...
When a session moves onto an auto-selected fallback, MarketingClaw sends a status notice in the same reply surface:

```text
↪️ Model Fallback: <fallback> (selected <primary>; <reason>)
```

When a later probe succeeds and the session returns to the selected primary, MarketingClaw sends:
...
MarketingClaw uses **auth profiles** for both API keys and OAuth tokens.

- Secrets and runtime auth-routing state live in `~/.marketingclaw/agents/<agentId>/agent/marketingclaw-agent.sqlite`.
- Config `auth.profiles` / `auth.order` are **metadata + routing only** (no secrets).
- Legacy import-only OAuth file: `~/.marketingclaw/credentials/oauth.json` (imported into the per-agent auth store on first use).
- Legacy `auth-profiles.json`, `auth-state.json`, and per-agent `auth.json` files are imported by `marketingclaw doctor --fix`.
...
- `type: "token"` → static bearer-style token, optionally expiring; MarketingClaw does not refresh it (used for `aws-sdk` and other credential-chain auth modes)
...
Profiles live in the per-agent `marketingclaw-agent.sqlite` auth profile store.

## Rotation order

When a provider has multiple profiles, MarketingClaw chooses an order like this:
...
If no explicit order is configured, MarketingClaw uses a round-robin order:
...
MarketingClaw **pins the chosen auth profile per session** to keep provider caches warm. It does **not** rotate on every request. The pinned profile is reused until:
...
<Note>
Auto-pinned profiles (selected by the session router) are treated as a **preference**: they are tried first, but MarketingClaw may rotate to another profile on rate limits/timeouts. When the original profile becomes available again, new runs can prefer it again without changing the selected model or runtime. User-pinned profiles stay locked to that profile; if it fails and model fallbacks are configured, MarketingClaw moves to the next model instead of switching profiles.
</Note>
...
When a profile fails due to auth/rate-limit errors (or a timeout that looks like rate limiting), MarketingClaw marks it in cooldown and moves to the next profile.
...
<AccordionGroup>
  ...
  <Accordion title="SDK retry-after caps">
    Some provider SDKs may otherwise sleep for a long `Retry-After` window before returning control to MarketingClaw. For Stainless-based SDKs such as Anthropic and OpenAI, MarketingClaw caps SDK-internal `retry-after-ms` / `retry-after` waits at 60 seconds by default and surfaces longer retryable responses immediately so this failover path can run. Tune or disable the cap with `MARKETINGCLAW_SDK_RETRY_MAX_WAIT_SECONDS`; see [Retry behavior](/concepts/retry).
  </Accordion>
  <Accordion title="Model-scoped cooldowns">
    Rate-limit cooldowns can also be model-scoped:

    - MarketingClaw records `cooldownModel` for rate-limit failures when the failing model id is known.
...
Billing/credit failures (for example "insufficient credits" / "credit balance too low") are treated as failover-worthy, but they're usually not transient. Instead of a short cooldown, MarketingClaw marks the profile as **disabled** (with a longer backoff) and rotates to the next profile/provider.

<Note>
Not every billing-shaped response is `402`, and not every HTTP `402` lands here. MarketingClaw keeps explicit billing text in the billing lane even when a provider returns `401` or `403` instead, but provider-specific matchers stay scoped to the provider that owns them (for example OpenRouter `403 Key limit exceeded`).
...
Overloaded and rate-limit errors are handled more aggressively than billing cooldowns: by default, MarketingClaw allows one same-provider auth-profile retry, then switches to the next configured model fallback without waiting.

## Model fallback

If all profiles for a provider fail, MarketingClaw moves to the next model in `agents.defaults.model.fallbacks`. This applies to auth failures, rate limits, and timeouts that exhausted profile rotation (other errors do not advance fallback). Provider errors that do not expose enough detail are still labeled precisely in fallback state: `empty_response` means the provider returned no usable message or status, `no_error_details` means the provider explicitly returned `Unknown error (no error details in response)`, and `unclassified` means MarketingClaw preserved the raw preview but no classifier matched it yet.
...
When a run starts from the configured default primary, a cron job primary, an agent primary with explicit fallbacks, or an auto-selected fallback override, MarketingClaw can walk the matching configured fallback chain. Agent primaries without explicit fallbacks and explicit user selections (for example `/model ollama/qwen3.5:27b`, the model picker, `sessions.patch`, or one-off CLI provider/model overrides) are strict: if that provider/model is unreachable or fails before producing a reply, MarketingClaw reports the failure instead of answering from an unrelated fallback.

### Candidate chain rules

MarketingClaw builds the candidate list from the currently requested `provider/model` plus configured fallbacks.
...
<AccordionGroup>
  <Accordion title="Rules">
    ...
    - If the current run is already on a configured fallback in the same provider family, MarketingClaw keeps using the full configured chain.
...
When every auth profile for a provider is already in cooldown, MarketingClaw does not automatically skip that provider forever. It makes a per-candidate decision:
...
- Auto fallback overrides remain selected on subsequent turns so MarketingClaw does not probe a known-bad primary on every message. MarketingClaw periodically probes the configured origin again and clears the auto override when it recovers; `/new`, `/reset`, and `sessions.reset` clear auto-sourced overrides immediately.
...
- If a live-switch error points at a later candidate in the active fallback chain, MarketingClaw jumps directly to that selected model instead of walking unrelated candidates first.
...
When every candidate fails, MarketingClaw throws `FallbackSummaryError`. The outer reply runner can use that to build a more specific message such as "all models are temporarily rate-limited" and include the soonest cooldown expiry when one is known.
...
- if the remaining block is a matching model-scoped rate limit, MarketingClaw reports the last matching expiry that still blocks that model
