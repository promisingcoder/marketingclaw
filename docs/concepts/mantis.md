...
title: "Mantis"
read_when:
  - Building or running live visual QA for MarketingClaw bugs
...
Mantis publishes visual CI evidence and a PR comment for MarketingClaw behavior.
...
- MarketingClaw (`extensions/qa-lab/src/mantis/*`): scenario runtime, `pnpm marketingclaw qa mantis <command>` CLI, evidence schema.
- QA Lab (`extensions/qa-lab/src/live-transports/*`): live transport harness, driver/SUT bots, report/evidence writers.
- Crabbox (`marketingclaw/crabbox`): warmed Linux machines, leases, VNC, `crabbox media preview`.
...
All commands are `pnpm marketingclaw qa mantis <command>`, defined in
`extensions/qa-lab/src/mantis/cli.ts`. Requires `MARKETINGCLAW_ENABLE_PRIVATE_QA_CLI=1`
at build/run time (bundled workflows set `MARKETINGCLAW_BUILD_PRIVATE_QA=1` and
`MARKETINGCLAW_ENABLE_PRIVATE_QA_CLI=1` before building).
...
| `telegram-desktop-builder`      | Lease/reuse a Crabbox desktop, install Telegram Desktop, optionally configure an MarketingClaw gateway.                                                   |
...
```bash
pnpm marketingclaw qa mantis discord-smoke \
...
Token resolution order: `--token-file` value, then `MARKETINGCLAW_QA_DISCORD_MANTIS_BOT_TOKEN`
(override with `--token-env`), then a file named by `MARKETINGCLAW_QA_DISCORD_MANTIS_BOT_TOKEN_FILE`
(override with `--token-file-env`). Guild/channel ids come from
`MARKETINGCLAW_QA_DISCORD_GUILD_ID` / `MARKETINGCLAW_QA_DISCORD_CHANNEL_ID` (override with
`--guild-id` / `--channel-id`) and must be 17-20 digit Discord snowflakes. Set
`MARKETINGCLAW_QA_REDACT_PUBLIC_METADATA=1` to replace bot/guild/channel/message ids
and names with `<redacted>` in the published summary and report.

### `run`

```bash
pnpm marketingclaw qa mantis run \
  --transport discord \
...
`pnpm marketingclaw qa discord --scenario <id> --model openai/gpt-5.4 --alt-model openai/gpt-5.4 --allow-failures`
...
```bash
pnpm marketingclaw qa mantis desktop-browser-smoke \
  --output-dir .artifacts/qa-e2e/mantis/desktop-browser
```

Leases or reuses a Crabbox desktop, launches a browser inside the VNC session
pointed at `--browser-url` (default `https://marketingclaw.ai`) or a rendered
...
- `--browser-profile-archive-env <name>` restores a base64 `.tgz` Chrome profile archive from that env var before launch (default `MARKETINGCLAW_MANTIS_BROWSER_PROFILE_TGZ_B64`); used for logged-in witnesses like Discord Web.
- `--video-duration <seconds>` controls MP4 capture length (default 10s).
- `--keep-lease` (or `MARKETINGCLAW_MANTIS_KEEP_VM=1`) keeps a lease this run created open for VNC inspection; failed runs that created a lease also keep it by default.
...
`MARKETINGCLAW_QA_DISCORD_CAPTURE_UI_METADATA=1` is set, the scenario also writes a
Discord Web URL artifact, and `MARKETINGCLAW_QA_DISCORD_KEEP_THREADS=1` leaves the
...
`pnpm marketingclaw qa slack` inside it, opens Slack Web in the VNC browser,
...
With `--gateway-setup`, the command creates a persistent disposable MarketingClaw
home at `$HOME/.marketingclaw-mantis/slack-marketingclaw` in the VM, patches Slack
Socket Mode config for the target channel, starts
`marketingclaw gateway run --dev --allow-unconfigured --port 38973`, and leaves
...
- `MARKETINGCLAW_LIVE_OPENAI_KEY` for the remote model lane (if only `OPENAI_API_KEY`
  is set locally, Mantis copies it to `MARKETINGCLAW_LIVE_OPENAI_KEY` before
  invoking Crabbox)
...
bot token into the VM as `MARKETINGCLAW_MANTIS_SLACK_*` env vars, so GitHub
...
`MARKETINGCLAW_MANTIS_SLACK_BROWSER_PROFILE_DIR` controls the persistent Chrome
profile inside the VM (default `$HOME/.config/marketingclaw-mantis/slack-chrome-profile`);
...
optionally restores a user-session archive, configures MarketingClaw with the
leased Telegram SUT bot token, starts
`marketingclaw gateway run --dev --allow-unconfigured --port 38974`, posts a
driver-bot readiness message to the leased private group, then captures a
screenshot and MP4. A bot token only configures MarketingClaw; it never logs
...
`MARKETINGCLAW_QA_REDACT_PUBLIC_METADATA=1` for public artifact uploads; it is
...
- `MANTIS_ARTIFACT_R2_BUCKET` (workflows set `marketingclaw-crabbox-artifacts`)
- `MANTIS_ARTIFACT_R2_ENDPOINT`
- `MANTIS_ARTIFACT_R2_REGION` (workflows set `auto`)
- `MANTIS_ARTIFACT_R2_PUBLIC_BASE_URL` (workflows set `https://artifacts.marketingclaw.ai`)
...
| `Mantis Telegram Live`            | PR comment or manual dispatch                                                              | Runs the bot-API Telegram live QA lane (`marketingclaw qa telegram`), writes `mantis-evidence.json` from the QA summary, renders redacted evidence HTML through a Crabbox desktop browser, generates a motion GIF, posts PR evidence. Telegram Web login is not required for this lane.                          |
...
| `Mantis Web UI Chat Proof`        | PR comment or manual dispatch                                                              | Runs the focused MarketingClaw Control UI chat Playwright proof against the candidate, verifies the browser sends through the mocked Gateway, captures screenshot/video artifacts, and posts PR evidence. This lane is web chat proof only, not WinUI/native-app or arbitrary visual proof.                      |
...
```text
@marketingclaw-mantis discord status reactions
@marketingclaw-mantis discord status reactions baseline=origin/main candidate=HEAD
...
@marketingclaw-mantis web ui chat
@marketingclaw-mantis web-ui-chat candidate=HEAD
```
...
`MARKETINGCLAW_MANTIS_CRABBOX_PROVIDER` / `MARKETINGCLAW_MANTIS_CRABBOX_CLASS`. GitHub
...
noVNC, Node 22+ and pnpm, an MarketingClaw checkout, and outbound access to the
target transport, GitHub, model providers, and the credential broker.

Secret names used across the Mantis workflows:

- `MARKETINGCLAW_QA_DISCORD_MANTIS_BOT_TOKEN`
...
- `MARKETINGCLAW_QA_REDACT_PUBLIC_METADATA=1` for public artifact uploads
- `MARKETINGCLAW_QA_CONVEX_SITE_URL`, `MARKETINGCLAW_QA_CONVEX_SECRET_CI`
- `CRABBOX_COORDINATOR` / `CRABBOX_COORDINATOR_TOKEN` (workflows also accept
  `MARKETINGCLAW_QA_MANTIS_CRABBOX_COORDINATOR` / `_TOKEN` as a fallback and map
...
ref policy, candidate ref policy, MarketingClaw config patch, setup/stimulus steps,
