...
The private QA stack exercises MarketingClaw in a realistic, channel-shaped way that
a unit test cannot.
...
Every QA flow runs under `pnpm marketingclaw qa <subcommand>`. Many have `pnpm qa:*`
...
| `qa parity-report`                                  | Compare two `qa-suite-summary.json` files for a model-axis parity gate, or use `--runtime-axis --token-efficiency` to write Codex-vs-MarketingClaw runtime parity and token-efficiency reports.                                                                     |
...
command also needs an MarketingClaw root profile, put the root profile before the
QA command:

```bash
pnpm marketingclaw --profile work qa run --qa-profile smoke-ci
```
...
```bash
pnpm marketingclaw qa docker-build-image
pnpm qa:lab:build
pnpm qa:lab:up:fast
pnpm qa:lab:watch
```
...
`marketingclaw.run`, `marketingclaw.harness.run`, a latest GenAI semantic-convention
model-call span, `marketingclaw.context.assembled`, and `marketingclaw.message.delivery`
...
IDs and `marketingclaw.content.*` attributes must stay out of the trace. The scenario
...
```bash
MARKETINGCLAW_QA_MATRIX_NO_REPLY_WINDOW_MS=3000 \
  pnpm marketingclaw qa matrix --provider-mode mock-openai --profile fast --fail-fast
```
...
```bash
MARKETINGCLAW_LIVE_OPENAI_KEY="${OPENAI_API_KEY}" \
MARKETINGCLAW_QA_MATRIX_NO_REPLY_WINDOW_MS=3000 \
  pnpm marketingclaw qa matrix --provider-mode live-frontier --profile fast --fail-fast
```
...
E2EE CLI profile also drives `marketingclaw matrix encryption setup` and
...
credentials, `--fast`, and `MARKETINGCLAW_QA_MATRIX_NO_REPLY_WINDOW_MS=3000`.
...
```bash
pnpm marketingclaw qa discord
pnpm marketingclaw qa slack
pnpm marketingclaw qa telegram
pnpm marketingclaw qa whatsapp
```
...
```bash
pnpm marketingclaw qa mantis slack-desktop-smoke \
  --gateway-setup \
...
MarketingClaw Slack gateway running inside the VM on port `38973`; without it, the
...
```bash
pnpm marketingclaw qa mantis visual-task \
...
`visual-driver`, captures `visual-task.png`, runs `marketingclaw infer image
...
The doctor checks Convex broker env (`MARKETINGCLAW_QA_CONVEX_SITE_URL`,
`MARKETINGCLAW_QA_CONVEX_ENDPOINT_PREFIX`), validates endpoint settings, reports
only set/missing status for `MARKETINGCLAW_QA_CONVEX_SECRET_CI` and
`MARKETINGCLAW_QA_CONVEX_SECRET_MAINTAINER`, and verifies admin/list reachability
...
`marketingclaw/plugin-sdk/qa-live-transport-scenarios`.
...
```bash
pnpm marketingclaw qa suite --runner multipass --scenario channel-chat-baseline
```

This boots a fresh Multipass guest, installs dependencies, builds MarketingClaw
...
- `MARKETINGCLAW_QA_TELEGRAM_GROUP_ID` - numeric chat id (string).
...
of the `/usage` footer on tool-only replies. Use `pnpm marketingclaw qa telegram
...
```bash
MARKETINGCLAW_QA_CREDENTIAL_SOURCE=convex \
pnpm test:docker:npm-telegram-live
```

When `MARKETINGCLAW_QA_CREDENTIAL_SOURCE=convex` is set, the package live wrapper
...
`MARKETINGCLAW_NPM_TELEGRAM_RTT_SAMPLES`, `MARKETINGCLAW_NPM_TELEGRAM_RTT_TIMEOUT_MS`,
or `MARKETINGCLAW_NPM_TELEGRAM_RTT_MAX_FAILURES` to tune RTT measurement without
...
controlled by the harness and a SUT bot started by the child MarketingClaw gateway
...
- `MARKETINGCLAW_QA_DISCORD_GUILD_ID`
- `MARKETINGCLAW_QA_DISCORD_CHANNEL_ID`
- `MARKETINGCLAW_QA_DISCORD_DRIVER_BOT_TOKEN`
- `MARKETINGCLAW_QA_DISCORD_SUT_BOT_TOKEN`
- `MARKETINGCLAW_QA_DISCORD_SUT_APPLICATION_ID` - must match the SUT bot user id
  returned by Discord (the lane fails fast otherwise).

Optional:

- `MARKETINGCLAW_QA_DISCORD_CAPTURE_CONTENT=1` keeps message bodies in
  observed-message artifacts.
- `MARKETINGCLAW_QA_DISCORD_VOICE_CHANNEL_ID` selects the voice/stage channel for
...
- `discord-qa-observed-messages.json` - bodies redacted unless
  `MARKETINGCLAW_QA_DISCORD_CAPTURE_CONTENT=1`.
...
- `MARKETINGCLAW_QA_SLACK_CHANNEL_ID`
- `MARKETINGCLAW_QA_SLACK_DRIVER_BOT_TOKEN`
- `MARKETINGCLAW_QA_SLACK_SUT_BOT_TOKEN`
- `MARKETINGCLAW_QA_SLACK_SUT_APP_TOKEN`

Optional:

- `MARKETINGCLAW_QA_SLACK_CAPTURE_CONTENT=1` keeps message bodies in
  observed-message artifacts.
- `MARKETINGCLAW_QA_SLACK_APPROVAL_CHECKPOINT_DIR` enables visual approval
...
- `MARKETINGCLAW_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS` overrides the checkpoint
...
- `slack-codex-approval-exec-native` - opt-in Codex Guardian command approval
  ...
  `marketingclaw-codex-app-server`, resolves it, and verifies the Codex turn
...
- `slack-qa-observed-messages.json` - bodies redacted unless
  `MARKETINGCLAW_QA_SLACK_CAPTURE_CONTENT=1`.
- `approval-checkpoints/` - only when Mantis sets
  `MARKETINGCLAW_QA_SLACK_APPROVAL_CHECKPOINT_DIR`; contains checkpoint JSON,
...
{
  "display_information": {
    "name": "MarketingClaw QA Driver",
    "description": "Test driver bot for MarketingClaw QA Slack live lane"
  },
  "features": {
    "bot_user": {
      "display_name": "MarketingClaw QA Driver",
      "always_online": true
    }
  },
  "oauth_config": {
...
{
  "display_information": {
    "name": "MarketingClaw QA SUT",
    "description": "MarketingClaw QA SUT connector for MarketingClaw"
  },
  "features": {
    "bot_user": {
      "display_name": "MarketingClaw QA SUT",
      "always_online": true
    },
    "app_home": {
...
In the QA workspace, create a channel (e.g. `#marketingclaw-qa`) and invite both
bots from inside the channel:

```text
/invite @MarketingClaw QA Driver
/invite @MarketingClaw QA SUT
```
...
`MARKETINGCLAW_QA_SLACK_*` variables and pass `--credential-source env`), or seed
...
With `MARKETINGCLAW_QA_CONVEX_SITE_URL` and `MARKETINGCLAW_QA_CONVEX_SECRET_MAINTAINER`
exported in your shell, register and verify:

```bash
pnpm marketingclaw qa credentials add \
  --kind slack \
  --payload-file slack-creds.json \
  --note "QA Slack pool seed"

pnpm marketingclaw qa credentials list --kind slack --status all --json
```
...
the harness and a SUT account started by the child MarketingClaw gateway through
the bundled WhatsApp plugin.

Required env when `--credential-source env`:

- `MARKETINGCLAW_QA_WHATSAPP_DRIVER_PHONE_E164`
- `MARKETINGCLAW_QA_WHATSAPP_SUT_PHONE_E164`
- `MARKETINGCLAW_QA_WHATSAPP_DRIVER_AUTH_ARCHIVE_BASE64`
- `MARKETINGCLAW_QA_WHATSAPP_SUT_AUTH_ARCHIVE_BASE64`

Optional:

- `MARKETINGCLAW_QA_WHATSAPP_GROUP_JID` enables group scenarios such as
...
- `MARKETINGCLAW_QA_WHATSAPP_CAPTURE_CONTENT=1` keeps message bodies in
  observed-message artifacts.
...
`@marketingclaw/whatsapp/api.js` package surface instead of reaching into private
...
- `whatsapp-qa-observed-messages.json` - bodies redacted unless
  `MARKETINGCLAW_QA_WHATSAPP_CAPTURE_CONTENT=1`.
...
`--credential-source convex` (or set `MARKETINGCLAW_QA_CREDENTIAL_SOURCE=convex`);
...
- `mock-openai` is the scenario-aware MarketingClaw mock. It remains the default
...
runs MarketingClaw's normal channel plugins against them. `live` is reserved for
...
- the `marketingclaw qa` command root
...
- how `marketingclaw qa <runner>` is mounted beneath the shared `qa` root
...
3. Keep transport-specific mechanics inside the runner plugin or channel
   harness.
4. Mount the runner as `marketingclaw qa <runner>` instead of registering a
   competing root command. Runner plugins should declare `qaRunners` in
   `marketingclaw.plugin.json` and export a matching `qaRunnerCliRegistrations`
...
or wiring a new transport - run `pnpm marketingclaw qa coverage` (add `--json`
for machine-readable output). When choosing focused proof for a touched
behavior or file path, run `pnpm marketingclaw qa coverage --match <query>`. The
...
```bash
pnpm marketingclaw qa character-eval \
