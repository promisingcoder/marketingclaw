---
summary: "How MarketingClaw resolves provider/model refs, config keys, and the `/model` chat command"
read_when:
...
<CardGroup cols={2}>
  ...
  <Card title="Models CLI reference" href="/cli/models">
    Full `marketingclaw models` command and flag reference.
  </Card>
...
<Steps>
  ...
  <Step title="Auth failover">
    Auth-profile rotation happens inside a provider before MarketingClaw moves to the next fallback model.
  </Step>
</Steps>

Related model-config surfaces:

- `agents.defaults.models` is the allowlist/catalog of models MarketingClaw can use, plus aliases. Use `provider/*` entries to allow every discovered model from a provider without listing each one.
...
| Auto fallback                                                           | Temporary recovery state, stored as `modelOverrideSource: "auto"`. MarketingClaw periodically reprobes the original primary, clears the auto selection on recovery, and announces fallback/recovery transitions once per state change.                         |
...
Add it with: marketingclaw config set agents.defaults.models '{"provider/model":{}}' --strict-json --merge
```
...
For local/GGUF models, the allowlist needs the full provider-prefixed ref, for example `ollama/gemma4:26b` or `lmstudio/Gemma4-26b-a4-it-gguf` — check `marketingclaw models list --provider <provider>` for the exact string. Bare filenames or display names are not enough once the allowlist is active.
...
`marketingclaw config set` refuses plain-object assignments to `agents.defaults.models`, `models.providers`, or `models.providers.<id>.models` when they would drop existing entries; use `--replace` only when the new value should become the complete target value. Interactive provider setup and `marketingclaw configure --section model` already merge provider-scoped selections into the allowlist, so adding a provider does not drop unrelated entries; configure preserves an existing `agents.defaults.model.primary`. Explicit commands like `marketingclaw models auth login --provider <id> --set-default` and `marketingclaw models set <model>` still replace the primary.
</Accordion>
...
marketingclaw models set <provider/model>
marketingclaw models set-image <provider/model>
marketingclaw models scan
marketingclaw models aliases list|add|remove
marketingclaw models fallbacks list|add|remove|clear
marketingclaw models image-fallbacks list|add|remove|clear
marketingclaw models auth list|add|login|paste-api-key|paste-token|setup-token|order
```

`marketingclaw models` with no subcommand is a shortcut for `models status`, which also surfaces OAuth expiry for auth-store profiles (warns within 24h by default). Full flags, JSON shapes, and auth-profile subcommands: [Models CLI reference](/cli/models).

<AccordionGroup>
  <Accordion title="Scanning (OpenRouter free models)">
    `marketingclaw models scan` inspects OpenRouter's public free-model catalog and can probe candidates for tool and image support live. The catalog itself is public, so metadata-only scans (`--no-probe`) need no key; live probing and `--set-default`/`--set-image` require an OpenRouter API key (auth profile or `OPENROUTER_API_KEY`) and fail closed to metadata-only output without one.
...
Marker persistence is source-authoritative: MarketingClaw writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values, whenever it regenerates `models.json` — including command-driven paths like `marketingclaw agent`.

## Related

- [Agent runtimes](/concepts/agent-runtimes) — MarketingClaw, Codex, and other agent loop runtimes
