...
An owner is a ClawHub publisher handle, such as `@alice` or `@marketingclaw`.
...
`@openclaw/dronzer` can only be published as `@marketingclaw`. To publish as
...
```text
Package scope "@marketingclaw" must match selected owner "@vintageayu".
Publish as "@marketingclaw" or rename this package to "@vintageayu/dronzer".
```
...
```sh
clawhub package transfer @opik/opik-marketingclaw --to opik
```
...
`@openclaw/dronzer` claims the `@marketingclaw` namespace, so only publishers with
access to `@marketingclaw` can publish or transfer into it.
