---
summary: "Find and publish community-maintained MarketingClaw plugins"
read_when:
  - You want to find third-party MarketingClaw plugins
  - You want to publish or list your own plugin on ClawHub
title: "Community plugins"
doc-schema-version: 1
---

Community plugins are third-party packages that extend MarketingClaw with
...
```bash
marketingclaw plugins install npm:<package-name>
```

Use [Manage plugins](/plugins/manage-plugins) for common install, update,
inspect, and uninstall examples. Use [`marketingclaw plugins`](/cli/plugins) for
the full command reference and source-selection rules.

## Publish plugins

Publish public community plugins on ClawHub so MarketingClaw users can discover
...
| Requirement          | Why                                                      |
| -------------------- | -------------------------------------------------------- |
| Published on ClawHub | Users need `marketingclaw plugins install` hints to work |
