---
summary: "Run MarketingClaw embedded agent turns through the external GitHub Copilot SDK harness"
...
read_when:
  ...
  - You are wiring an agent to subscription Copilot (github / marketingclaw / copilot) and want it to run through the Copilot CLI
---

The external `@marketingclaw/copilot` plugin runs embedded subscription Copilot
agent turns through the GitHub Copilot CLI (`@github/copilot-sdk`) instead of
MarketingClaw's built-in PI harness. The Copilot CLI session owns the low-level
agent loop: native tool execution, native compaction (`infiniteSessions`), and
CLI-managed thread state under `copilotHome`. MarketingClaw still owns chat
...
[Side questions (`/btw`)](#side-questions-btw)), and `marketingclaw doctor`.
...
- MarketingClaw with the `@marketingclaw/copilot` plugin installed.
- If your config uses `plugins.allow`, include `copilot` (the manifest id the
  ...
  `@marketingclaw/copilot` will not match and leaves the plugin blocked, even with
...
- A writable `copilotHome` directory. Defaults to `<agentDir>/copilot` when
  MarketingClaw provides an agent directory, otherwise
  `~/.marketingclaw/agents/<agentId>/copilot`.

`marketingclaw doctor` runs the plugin's [doctor contract](#doctor) for
session-state ownership and future config migrations. It does not probe the
Copilot CLI environment.

## Install

The Copilot runtime ships as an external plugin so the core `marketingclaw`
...
```bash
marketingclaw plugins install @marketingclaw/copilot
```
...
[Quickstart](#quickstart). Without that opt-in, MarketingClaw uses its built-in
GitHub Copilot provider and never installs this plugin.

The runtime resolves the SDK in this order:

1. `import("@github/copilot-sdk")` from the installed `@marketingclaw/copilot`
   package.
2. The fallback dir `~/.marketingclaw/npm-runtime/copilot/` (legacy on-demand
   install target).
...
through MarketingClaw's guarded fetch path so DNS pinning and SSRF policy stay
owned by MarketingClaw. Use the native MarketingClaw runtime for local Ollama, LM
Studio, or LAN model servers.

## BYOK

Copilot BYOK uses the SDK's session-level custom provider contract. MarketingClaw
...
4. **Env-var fallback**, checked in this order (first non-empty value wins,
   ...
   1. `MARKETINGCLAW_GITHUB_TOKEN` — harness-specific override; lets you pin a
      token for the MarketingClaw harness without disturbing system-wide `gh` /
      Copilot CLI config.
...
MarketingClaw's `models.json` / `auth-profiles.json`), or
`~/.marketingclaw/agents/<agentId>/copilot` when no agent directory is supplied.
Override with `copilotHome: <path>` on the attempt input for a custom
location (for example, a shared mount for migration).

Live harness tests use `MARKETINGCLAW_COPILOT_AGENT_LIVE_TOKEN` for a direct
...
| `reasoningEffort`        | `"low" \| "medium" \| "high" \| "xhigh"`. Maps from MarketingClaw's `ThinkLevel` / `ReasoningLevel` resolution in `auto-reply/thinking.ts`.                                                                                                                                                     |
| `infiniteSessionConfig`  | Optional override for the SDK `infiniteSessions` block driven by `harness.compact`. Safe to leave as-is.                                                                                                                                                                                        |
| `hooksConfig`            | Optional native Copilot SDK `SessionHooks` config for tool/MCP, user-prompt, session, and error callbacks. Separate from MarketingClaw's portable lifecycle hooks.                                                                                                                              |
...
MarketingClaw plugin hooks need no Copilot-specific attempt configuration. The
...
`before_compaction` and `after_compaction`. Bridged MarketingClaw tools run
`before_tool_call` and report `after_tool_call`; `hooksConfig` remains for
native SDK-only callbacks with no portable equivalent.

Nothing else in MarketingClaw needs to know about these fields. Other plugins,
...
The MarketingClaw-side transcript mirror (below) keeps receiving post-compaction
...
MarketingClaw audit transcript via
...
so MarketingClaw's `/btw` dispatcher (`src/agents/btw.ts`) falls through to the
...
- `ask_user` uses the same MarketingClaw prompt-and-reply path as the Codex
  harness: when the Copilot SDK asks for user input, MarketingClaw posts a
...
Permission enforcement for bridged MarketingClaw tools happens **inside the tool
...
`createMarketingClawCodingTools` to every coding tool: loop detection, trusted
...
The SDK Tool returned by `convertMarketingClawToolToSdkTool` is marked with:
...
- `skipPermission: true` — tells the SDK not to fire
  ...
  wrapped `execute()` already performs the richer MarketingClaw policy check; an
  SDK-level prompt would either short-circuit MarketingClaw's enforcement
  (allow-all) or block every tool call (reject-all) — neither matches PI
  parity.

The in-tree Codex harness uses the same split: bridged MarketingClaw tools are
...
`createMarketingClawCodingTools`: identity (`senderIsOwner`, `memberRoleIds`,
...
`marketingclaw/plugin-sdk/agent-harness-tool-runtime` for PI parity. When
tool-search is enabled, the SDK sees compact control tools plus a hidden
catalog executor instead of every MarketingClaw tool schema. When code mode is
...
the SDK request allows them, and cancels a pending request when the MarketingClaw
attempt is aborted.
