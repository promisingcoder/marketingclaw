...
read_when:
  ...
  - You are changing plugin behavior across MarketingClaw and Codex harness turns
---
...
continuation, and native compaction. MarketingClaw owns channel routing, session
files, visible message delivery, MarketingClaw dynamic tools, approvals, media
...
MarketingClaw compatibility route keeps the normal MarketingClaw system prompt even when
it uses Codex-flavored OpenAI auth or transport.

MarketingClaw starts and resumes native Codex threads with Codex's built-in
personality disabled (`personality: "none"`) so workspace personality files
and MarketingClaw agent identity stay authoritative. Native Codex keeps Codex-owned
base/model instructions and project-doc loading otherwise. Lightweight
MarketingClaw runs (for example cron) still suppress project-doc loading.

MarketingClaw developer instructions cover MarketingClaw runtime concerns: source-channel
delivery, MarketingClaw dynamic tools, ACP delegation, adapter context, and the
...
When an MarketingClaw session is attached to an existing Codex thread, the next
...
contract: the agent replies normally and MarketingClaw posts the final text to the
...
Codex heartbeat turns get `heartbeat_respond` in the searchable MarketingClaw tool
...
| Layer                                 | Owner                         | Purpose                                                                |
| ------------------------------------- | ----------------------------- | ---------------------------------------------------------------------- |
| MarketingClaw plugin hooks            | MarketingClaw                 | Product/plugin compatibility across MarketingClaw and Codex harnesses. |
| Codex app-server extension middleware | MarketingClaw bundled plugins | Per-turn adapter behavior around MarketingClaw dynamic tools.          |
| Codex native hooks                    | Codex                         | Low-level Codex lifecycle and native tool policy from Codex config.    |

MarketingClaw does not use project or global Codex `hooks.json` files to route
plugin behavior. For the native tool and permission bridge, MarketingClaw injects
...
so Codex's app-server reviewer and MarketingClaw's approval bridge handle real
...
controls; they are not exposed as MarketingClaw plugin hooks in the v1 contract.

For MarketingClaw dynamic tools, MarketingClaw executes the tool after Codex asks for
the call, so plugin and middleware behavior runs in the harness adapter. For
Codex-native tools, Codex owns the canonical tool record; MarketingClaw can mirror
...
matching app-server approval. If an MarketingClaw `before_tool_call` hook returns
`requireApproval` while the native payload sets `marketingclaw_approval_mode:
...
request for the same tool use, MarketingClaw opens the plugin approval prompt and
maps the decision back to Codex. Codex `PermissionRequest` events are a
separate approval path and can still route through MarketingClaw approvals when
...
notifications and MarketingClaw adapter state, not native Codex hook commands.
...
debugging. They do not invoke MarketingClaw plugin hooks.
...
| MarketingClaw channel routing and delivery    | Supported                                                                        | Telegram, Discord, Slack, WhatsApp, iMessage, and other channels stay outside the model runtime.                                                                                                                                                                                                                                                                                                                                                                                                   |
| MarketingClaw dynamic tools                   | Supported                                                                        | Codex asks MarketingClaw to execute these tools, so MarketingClaw stays in the execution path.                                                                                                                                                                                                                                                                                                                                                                                                     |
| Prompt and context plugins                    | Supported                                                                        | MarketingClaw projects MarketingClaw-specific prompt/context into the Codex turn while leaving Codex-owned base, model, and configured project-doc prompts in the native Codex lane. MarketingClaw disables Codex's built-in personality for native threads so agent workspace personality files remain authoritative. Native Codex developer instructions accept only command guidance explicitly scoped to `codex_app_server`; legacy global command hints remain for non-Codex prompt surfaces. |
...
| Dynamic tool hooks                            | Supported                                                                        | `before_tool_call`, `after_tool_call`, and tool-result middleware run around MarketingClaw-owned dynamic tools.                                                                                                                                                                                                                                                                                                                                                                                    |
...
| Native permission policy                      | Supported through Codex app-server approvals and compatibility native hook relay | Codex app-server approval requests route through MarketingClaw after Codex review. The `PermissionRequest` native hook relay is opt-in for native approval modes because Codex emits it before guardian review.                                                                                                                                                                                                                                                                                    |
| App-server trajectory capture                 | Supported                                                                        | MarketingClaw records the request it sent to app-server and the app-server notifications it receives.                                                                                                                                                                                                                                                                                                                                                                                              |

Not supported in Codex runtime v1:

| Surface                                             | V1 boundary                                                                                                                                          | Future path                                                                               |
| --------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| Native tool argument mutation                       | Codex native pre-tool hooks can block, but MarketingClaw does not rewrite Codex-native tool arguments.                                               | Requires Codex hook/schema support for replacement tool input.                            |
| Editable Codex-native transcript history            | Codex owns canonical native thread history. MarketingClaw owns a mirror and can project future context, but should not mutate unsupported internals. | Add explicit Codex app-server APIs if native thread surgery is needed.                    |
| `tool_result_persist` for Codex-native tool records | That hook transforms MarketingClaw-owned transcript writes, not Codex-native tool records.                                                           | Could mirror transformed records, but canonical rewrite needs Codex support.              |
| Rich native compaction metadata                     | MarketingClaw can request native compaction, but does not receive a stable kept/dropped list, token delta, completion summary, or summary payload.   | Needs richer Codex compaction events.                                                     |
| Compaction intervention                             | MarketingClaw does not let plugins or context engines veto, rewrite, or replace native Codex compaction.                                             | Add Codex pre/post compaction hooks if plugins need to veto or rewrite native compaction. |
| Byte-for-byte model API request capture             | MarketingClaw can capture app-server requests and notifications, but Codex core builds the final OpenAI API request internally.                      | Needs a Codex model-request tracing event or debug API.                                   |

## Native permissions and MCP elicitations

For `PermissionRequest`, MarketingClaw only returns explicit allow or deny
...
request, MarketingClaw remembers that exact provider/session/tool input/cwd
...
Codex MCP tool approval elicitations route through MarketingClaw's plugin approval
...
default `messages.queue.mode: "steer"`, MarketingClaw batches steer-mode chat
...
that case, MarketingClaw waits for the active run to finish before starting the
...
harness, MarketingClaw also calls Codex app-server `feedback/upload` for relevant
...
MarketingClaw session ids, Codex thread ids, and local `codex resume <thread-id>`
commands for the threads that were sent.

If you deny or ignore the approval, MarketingClaw does not print those Codex ids
...
belongs to Codex app-server. MarketingClaw does not run preflight compaction for
Codex turns, replace Codex compaction with context-engine compaction, or fall
back to MarketingClaw or public OpenAI summarization when native compaction cannot
be started. MarketingClaw keeps a transcript mirror for channel history, search,
...
MarketingClaw keeps the request and shared-client lease open until Codex emits the
...
timeout, MarketingClaw requests a native turn interrupt. The lease and per-thread
...
period, MarketingClaw retires the connection before releasing the fence. Remote
...
compaction is Codex's job; MarketingClaw only starts native compaction for manually
requested triggers.

When a context engine requests Codex thread-bootstrap projection, MarketingClaw
...
Codex reasoning or plan records when the app-server emits them. MarketingClaw
...
not rewrite Codex-native tool result records. It only applies when MarketingClaw
writes an MarketingClaw-owned session transcript tool result.

## Media and delivery

MarketingClaw continues to own media delivery and media provider selection. Image,
...
through the normal MarketingClaw delivery path; media generation does not require
the legacy runtime. When Codex emits a native image-generation item with a
`savedPath`, MarketingClaw forwards that exact file through the normal reply-media
