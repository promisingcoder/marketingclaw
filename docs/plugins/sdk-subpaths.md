...
`marketingclaw/plugin-sdk/`. This page catalogs the commonly used subpaths grouped by
...
| `plugin-sdk/config-schema`     | Deprecated. Root `marketingclaw.json` Zod schema (`MarketingClawSchema`); define plugin-local schemas instead and validate with `plugin-sdk/json-schema-runtime`       |
...
MarketingClaw's Vitest-backed test-helper subpaths are repo-local only and are no
...
<AccordionGroup>
  <Accordion title="Channel subpaths">
    ...
    | `plugin-sdk/bundled-channel-config-schema` | Bundled MarketingClaw channel config schemas for maintained bundled plugins only |
    ...
    | `plugin-sdk/discord` | Deprecated Discord compatibility facade for published `@marketingclaw/discord@2026.3.13` and tracked owner compatibility; new plugins should use generic channel SDK subpaths |
...
and pairing-path families.
  ...
  <Accordion title="Provider subpaths">
    ...
    | `plugin-sdk/provider-auth` | `createProviderApiKeyAuthMethod`, `ensureApiKeyFromOptionEnvOrPrompt`, `upsertAuthProfile`, `upsertApiKeyProfile`, `writeOAuthCredentials`, OpenAI Codex auth-import helpers, deprecated `resolveMarketingClawAgentDir` compatibility export |
...
MarketingClaw displays that summary text in status output; use `error` only when the
usage endpoint failed or returned no usable usage data.
  ...
  <Accordion title="Runtime and storage subpaths">
    ...
    | `plugin-sdk/config-contracts` | Focused type-only config surface for plugin config shapes such as `MarketingClawConfig` and channel/provider config types |
    ...
    | `plugin-sdk/agent-harness` | Experimental trusted-plugin surface for low-level agent harnesses: harness types, active-run steer/abort helpers, MarketingClaw tool bridge helpers, runtime-plan tool policy helpers, terminal outcome classification, tool progress formatting/detail helpers, and attempt result utilities |
    ...
    | `plugin-sdk/agent-runtime` | Deprecated broad barrel for agent dir/identity/workspace helpers, including `resolveAgentDir`, `resolveDefaultAgentDir`, and the deprecated `resolveMarketingClawAgentDir` compatibility export; prefer focused agent/runtime subpaths |
  ...
  <Accordion title="Capability and testing subpaths">
    ...
    | `plugin-sdk/testing` | Repo-local deprecated compatibility barrel for legacy MarketingClaw tests. New repo tests should import focused local test subpaths such as `plugin-sdk/agent-runtime-test-contracts`, `plugin-sdk/plugin-test-runtime`, `plugin-sdk/channel-test-helpers`, `plugin-sdk/test-env`, or `plugin-sdk/test-fixtures` instead |
  ...
  <Accordion title="Reserved bundled-helper subpaths">
    ...
    | `plugin-sdk/codex-native-task-runtime` | Bundled Codex plugin helper for mirroring Codex app-server native subagents into MarketingClaw task state (repo-local only, not a package export) |
