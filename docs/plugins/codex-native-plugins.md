---
summary: "Configure migrated native Codex plugins for Codex-mode MarketingClaw agents"
title: "Native Codex plugins"
read_when:
  - You want Codex-mode MarketingClaw agents to use native Codex plugins
  - You are migrating source-installed openai-curated Codex plugins
  - You are troubleshooting codexPlugins, app inventory, destructive actions, or plugin app diagnostics
---

Native Codex plugin support lets a Codex-mode MarketingClaw agent use Codex
app-server's own app and plugin capabilities inside the same Codex thread that
handles the MarketingClaw turn. Plugin calls stay in the native Codex transcript;
Codex app-server owns app-backed MCP execution. MarketingClaw does not translate
Codex plugins into synthetic `codex_plugin_*` MarketingClaw dynamic tools.
...
`codexPlugins` has no effect on MarketingClaw-provider runs, ACP conversation
...
```bash
marketingclaw migrate codex --dry-run --verify-plugin-apps
```
...
`enable`/`disable` write only to `~/.marketingclaw/marketingclaw.json`; they never edit
...
`auth_required`, reauthorize the app in Codex before enabling it in MarketingClaw.
...
| Enabled    | MarketingClaw config allows the plugin for Codex harness turns.                                                                  |
...
- During planning, MarketingClaw reads source Codex `plugin/read` details and
...
MarketingClaw reads Codex app inventory through app-server `app/list`, caches it
...
drops it, and MarketingClaw rebuilds it from the next `app/list` read.
...
A plugin app is exposed only when MarketingClaw can map it back to the migrated
...
MarketingClaw injects a restrictive `config.apps` patch for the Codex thread:
...
get `open_world_enabled: true`. MarketingClaw does not expose a separate
...
- Per-plugin `allow_destructive_actions` overrides the global policy for
  that plugin.
- `false`: MarketingClaw returns a deterministic decline.
- `true`: MarketingClaw auto-accepts only safe schemas it can map to an approval
  response, such as a boolean approve field.
- `"auto"`: MarketingClaw exposes destructive plugin actions to Codex, then
  turns ownership-proven MCP approval elicitations into MarketingClaw plugin
  approvals before returning the Codex approval response.
- `"ask"`: MarketingClaw uses the same Codex write/destructive gating as
  ...
  admitted app using `"ask"`, MarketingClaw selects Codex's human approvals
  reviewer for that app so Codex sends its approval elicitations to
  MarketingClaw; other apps and non-app thread approvals keep their configured
  reviewer and policy.
...
| Code                                              | Meaning                                                                                                                              | Fix                                                                                                                        |
| ------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------- |
| `auth_required`                                   | Migration installed the plugin, but one of its apps still needs authentication. The entry is written disabled until you reauthorize. | Reauthorize the app in Codex, then enable the plugin in MarketingClaw.                                                     |
...
| `app_inventory_missing`, `app_inventory_stale`    | App readiness came from an empty or stale cache.                                                                                     | MarketingClaw schedules an async refresh automatically; plugin apps stay excluded until ownership and readiness are known. |
...
Codex thread bindings keep the app config they started with until MarketingClaw
