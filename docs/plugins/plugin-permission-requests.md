...
| Codex native permission requests | Codex asks before native shell, file, MCP, or app-server actions.        | Codex app-server or native hook approval handling, routed through plugin approvals when MarketingClaw owns the prompt. |
| MCP approval elicitations        | A Codex MCP server requests approval for a tool call.                    | MCP approval responses bridged through MarketingClaw plugin approvals.                                                 |
...
runs after the model selects a tool and before MarketingClaw executes it:
...
MarketingClaw creates a pending approval with a `plugin:` ID, delivers it to the
...
MarketingClaw treats `allow-once` and `allow-always` as approval decisions for the
...
If the hook also returns `params`, MarketingClaw applies those parameter changes only
...
- Codex app-server approval requests route through MarketingClaw after Codex review.
