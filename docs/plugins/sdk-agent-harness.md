...
An **agent harness** is the low level executor for one prepared MarketingClaw agent
...
runtime and the normal MarketingClaw provider transport is the wrong abstraction:
...
- a model runtime that needs its own resume id in addition to the MarketingClaw
  session transcript
...
Before a harness is selected, MarketingClaw has already resolved:

- provider and model
- runtime auth state
- thinking level and context budget
- the MarketingClaw transcript/session file
...
The prepared attempt also includes `params.runtimePlan`, an MarketingClaw-owned
policy bundle for runtime decisions that must stay shared across MarketingClaw and
native harnesses:
...
Harnesses may use the plan for decisions that need to match MarketingClaw behavior,
but treat it as host-owned attempt state: do not mutate it or use it to switch
providers/models inside a turn.

## Register a harness

**Import:** `marketingclaw/plugin-sdk/agent-harness`

```typescript
import type { AgentHarness } from "marketingclaw/plugin-sdk/agent-harness";
...
MarketingClaw chooses a harness after provider/model resolution:
...
3. `auto` asks registered harnesses if they support the resolved
   provider/model.
4. If no registered harness matches, MarketingClaw uses its embedded runtime.
...
provider/model. Once a plugin harness has claimed a run, MarketingClaw does not
...
`agents.list[].agentRuntime`, and `MARKETINGCLAW_AGENT_RUNTIME`. `/status` shows the
...
MarketingClaw. The harness then claims that provider in `supports(...)`.
...
- app-server request: MarketingClaw sends the bare model id to Codex and lets the
...
MarketingClaw requires Codex app-server `0.142.0` or newer. The Codex plugin checks
the app-server initialize handshake and blocks older or unversioned servers,
so MarketingClaw only runs against the protocol surface it has tested.
...
seam is for async tool-result transforms that must run before MarketingClaw or
...
`marketingclaw/plugin-sdk/agent-harness-runtime` when a completed turn produced no
visible assistant text. The helper returns `empty`, `reasoning-only`, or
`planning-only` so MarketingClaw's fallback policy can decide whether to retry on a
different model. `planning-only` requires the harness's explicit `planText`
field; MarketingClaw does not infer it from assistant prose. The helper
...
`marketingclaw/plugin-sdk/agent-harness-runtime` after they finalize an attempt. It
dispatches the portable `agent_end` hook and MarketingClaw's research capture
...
user-input helpers from `marketingclaw/plugin-sdk/agent-harness-runtime` to format
the prompt, deliver it through MarketingClaw's blocking reply path, and normalize
...
`marketingclaw/plugin-sdk/agent-harness-tool-runtime`. It owns
...
The bundled `codex` harness is the native Codex mode for embedded MarketingClaw
...
`marketingclaw doctor --fix`, and legacy `codex/*` model refs remain compatibility
aliases for the native harness.

When this mode runs, Codex owns the native thread id, resume behavior,
compaction, and app-server execution. MarketingClaw still owns the chat channel,
...
By default, MarketingClaw uses `auto` provider/model runtime policy: registered
...
token. Keep that binding explicitly associated with the MarketingClaw session, and
keep mirroring user-visible assistant/tool output into the MarketingClaw
transcript.

The MarketingClaw transcript remains the compatibility layer for:

- channel-visible session history
- transcript search and indexing
- switching back to the built-in MarketingClaw harness on a later turn
- generic `/new`, `/reset`, and session deletion behavior

If your harness stores a sidecar binding, implement `reset(...)` so MarketingClaw
can clear it when the owning MarketingClaw session is reset.

## Tool and media results

Core constructs the MarketingClaw tool list and passes it into the prepared
...
outputs on the same delivery path as MarketingClaw-backed runs.
