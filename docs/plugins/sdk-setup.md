...
read_when:
  ...
  - You are defining plugin config schemas or package.json marketingclaw metadata
---

Reference for plugin packaging (`package.json` metadata), manifests (`marketingclaw.plugin.json`), setup entries, and config schemas.
...
Your `package.json` needs an `marketingclaw` field that tells the plugin system what your plugin provides:

<Tabs>
  <Tab title="Channel plugin">
    ```json
    {
      "name": "@myorg/marketingclaw-my-channel",
      "version": "1.0.0",
      "type": "module",
      "marketingclaw": {
        "extensions": ["./index.ts"],
  ...
  <Tab title="Provider plugin / ClawHub baseline">
    ```json marketingclaw-clawhub-package.json
    {
...
<ParamField path="runtimeExtensions" type="string[]">
  Built JavaScript peers for `extensions`, preferred when MarketingClaw loads an installed npm package. See [SDK entry points](/plugins/sdk-entrypoints) for the source/built resolution order.
</ParamField>
...
<Note>
Provider ids (`providers: string[]`) are manifest metadata, not package metadata. Declare them in `marketingclaw.plugin.json`, not here — see [Plugin manifest](/plugins/manifest).
</Note>

### `marketingclaw.channel`

`marketingclaw.channel` is cheap package metadata for channel discovery and setup surfaces before runtime loads.
...
### `marketingclaw.install`

`marketingclaw.install` is package metadata, not manifest metadata.
...
| `minHostVersion`             | `string`                            | Minimum supported MarketingClaw version, `>=x.y.z` or `>=x.y.z-prerelease`.       |
...
<AccordionGroup>
  <Accordion title="Onboarding behavior">
    Interactive onboarding uses `marketingclaw.install` for install-on-demand surfaces: if your plugin exposes provider auth choices or channel setup/catalog metadata before runtime loads, onboarding can prompt for ClawHub, npm, or local install, install or enable the plugin, then continue the selected flow. ClawHub choices use `clawhubSpec` and are preferred when present; npm choices require trusted catalog metadata with a registry `npmSpec` (exact versions and `expectedIntegrity` are optional pins, enforced on install/update when set). Keep "what to show" in `marketingclaw.plugin.json` and "how to install it" in `package.json`.
  </Accordion>
...
When enabled, MarketingClaw loads only `setupEntry` during the pre-listen startup phase, even for already-configured channels. The full entry loads after the gateway starts listening.
...
Every native plugin must ship an `marketingclaw.plugin.json` in the package root. MarketingClaw uses this to validate config without executing plugin code.

```json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds My Plugin capabilities to MarketingClaw",
  "configSchema": {
    "type": "object",
...
`setup-entry.ts` is a lightweight alternative to `index.ts` that MarketingClaw loads when it only needs setup surfaces (onboarding, config repair, disabled channel inspection):
...
If you already author the contract as JSON Schema or TypeBox, use the direct helper so MarketingClaw can skip Zod-to-JSON-Schema conversion on metadata paths:

```typescript
import { Type } from "typebox";
import { buildJsonChannelConfigSchema } from "marketingclaw/plugin-sdk/channel-config-schema";
...
For third-party plugins, the cold-path contract is still the plugin manifest: mirror the generated JSON Schema into `marketingclaw.plugin.json#channelConfigs` so config schema, setup, and UI surfaces can inspect `channels.<id>` without loading runtime code.

## Setup wizards

Channel plugins can provide interactive setup wizards for `marketingclaw onboard`. The wizard is a `ChannelSetupWizard` object on the `ChannelPlugin`:

```typescript
import type { ChannelSetupWizard } from "marketingclaw/plugin-sdk/channel-setup";
...
<AccordionGroup>
  <Accordion title="Shared allowFrom prompts">
    For DM allowlist prompts that only need the standard `note -> prompt -> parse -> merge -> patch` flow, prefer the shared setup helpers from `marketingclaw/plugin-sdk/setup`: `createPromptParsedAllowFromForAccount(...)`, `createTopLevelChannelParsedAllowFromPrompt(...)`, and `createNestedChannelParsedAllowFromPrompt(...)`.
  </Accordion>
  <Accordion title="Standard channel setup status">
    For channel setup status blocks that only vary by labels, scores, and optional extra lines, prefer `createStandardChannelSetupStatus(...)` from `marketingclaw/plugin-sdk/setup` instead of hand-rolling the same `status` object in each plugin.
  </Accordion>
  <Accordion title="Optional channel setup surface">
    For optional setup surfaces that should only appear in certain contexts, use `createOptionalChannelSetupSurface` from `marketingclaw/plugin-sdk/channel-setup`:

    ```typescript
    import { createOptionalChannelSetupSurface } from "marketingclaw/plugin-sdk/channel-setup";

    const setupSurface = createOptionalChannelSetupSurface({
      channel: "my-channel",
      label: "My Channel",
      npmSpec: "@myorg/marketingclaw-my-channel",
      docsPath: "/channels/my-channel",
    });
...
<Tabs>
  <Tab title="npm">
    ```bash
    marketingclaw plugins install @myorg/marketingclaw-my-plugin
    ```

    Bare package specs install from npm during the launch cutover, unless the name matches a bundled or official plugin id, in which case MarketingClaw uses that local/official copy instead. Use `clawhub:`, `npm:`, `git:`, or `npm-pack:` for deterministic source selection — see [Manage plugins](/plugins/manage-plugins).

  </Tab>
  <Tab title="ClawHub only">
    ```bash
    marketingclaw plugins install clawhub:@myorg/marketingclaw-my-plugin
    ```
  </Tab>
  <Tab title="npm package spec">
    ...
    ```bash
    marketingclaw plugins install npm:@myorg/marketingclaw-my-plugin
    ```
...
<Info>
For npm-sourced installs, `marketingclaw plugins install` installs the package into a per-plugin project under `~/.marketingclaw/npm/projects` with lifecycle scripts disabled (`--ignore-scripts`). Keep plugin dependency trees pure JS/TS and avoid packages that require `postinstall` builds.
</Info>
...
Bundled package metadata is explicit, not inferred from built JavaScript at gateway startup. Runtime dependencies belong in the plugin package that owns them; packaged MarketingClaw startup never repairs or mirrors plugin dependencies.
