...
The Policy plugin contributes doctor health checks for policy-managed MarketingClaw
...
posture, ingress/channel access posture, data-handling posture, and MarketingClaw config secret
provider/auth profile posture.

Policy stores authored requirements in `policy.jsonc`, observes existing
MarketingClaw settings and workspace declarations as evidence, and reports drift
through `marketingclaw policy check` and `marketingclaw doctor --lint`. A clean policy
check emits policy, evidence, findings, and attestation hashes that operators
can record for audit.

`marketingclaw policy compare --baseline <file>` compares one policy file to another
