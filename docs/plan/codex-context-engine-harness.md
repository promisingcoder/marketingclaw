...
summary: "Specification for making the bundled Codex app-server harness honor MarketingClaw context-engine plugins"
read_when:
  ...
  - You are comparing embedded MarketingClaw and Codex app-server context behavior
---
...
Make the bundled Codex app-server harness honor the same MarketingClaw context-engine
lifecycle contract that embedded MarketingClaw turns already honor.
...
MarketingClaw-level compaction policy as far as the Codex app-server boundary allows.
...
as built-in MarketingClaw attempts:

- `extensions/codex/src/app-server/run-attempt.ts`

That means the required hook point is in MarketingClaw-controlled code. The external
boundary is the Codex app-server protocol itself: MarketingClaw can control what it
...
Built-in MarketingClaw attempts call the context-engine lifecycle directly:
...
For Codex harness turns, MarketingClaw should preserve this lifecycle:

1. Read the mirrored MarketingClaw session transcript.
...
8. Mirror the Codex result back into the MarketingClaw transcript.
...
11. Preserve Codex native compaction signals and MarketingClaw compaction hooks.

## Design constraints

### Codex app-server remains canonical for native thread state

Codex owns its native thread and any internal extended history. MarketingClaw should
not try to mutate the app-server's internal history except through supported
protocol calls.

MarketingClaw's transcript mirror remains the source for MarketingClaw features:
...
The context-engine interface returns MarketingClaw `AgentMessage[]`, not a Codex
...
- `runtime: "marketingclaw"` selects the built-in MarketingClaw harness
- `runtime: "codex"` selects the registered Codex harness
- `runtime: "auto"` lets plugin harnesses claim supported providers
- unmatched `auto` runs use the built-in MarketingClaw harness
...
- Label it clearly as MarketingClaw assembled context.
...
inside MarketingClaw and preserves context-engine semantics.
...
we need strict MarketingClaw parity, run context-engine assembly before hook
...
MarketingClaw transcript mirror.
...
1. MarketingClaw context-engine `compact()`
2. Codex app-server native `thread/compact/start`

Do not silently conflate them.

#### `/compact` and explicit MarketingClaw compaction

When the selected context engine has `info.ownsCompaction === true`, explicit
MarketingClaw compaction should prefer the context engine's `compact()` result for
the MarketingClaw transcript mirror and plugin state.
...
the MarketingClaw session file. Preserve that behavior.

Also ensure context-engine state cleanup continues to happen through existing
MarketingClaw session lifecycle paths. Do not add Codex-specific cleanup unless the
context-engine lifecycle currently misses reset/delete events for all harnesses.

### 10. Error handling

Follow built-in MarketingClaw semantics:
...
- If Codex native compaction fails after context-engine compaction succeeds,
  do not fail the whole MarketingClaw compaction when the context engine is primary.
...
Avoid requiring lossless-claw in MarketingClaw core tests. Use a small in-repo fake
...
- Context-engine-owned compaction remains primary for MarketingClaw/plugin state.
