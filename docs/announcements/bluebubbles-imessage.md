---
summary: "BlueBubbles support was removed from MarketingClaw. Use the bundled iMessage plugin with imsg for new and migrated iMessage setups."
read_when:
  - You used the old BlueBubbles channel and need to move to iMessage
  - You are choosing the supported MarketingClaw iMessage setup
...
MarketingClaw no longer ships the BlueBubbles channel. iMessage support runs through the bundled `imessage` plugin: the Gateway spawns [`imsg`](https://github.com/steipete/imsg) as a child process, locally or through an SSH wrapper, and talks JSON-RPC over stdin/stdout. No server, no webhook, no port.
...
- MarketingClaw reads and watches Messages through `imsg` on the Mac where Messages.app is signed in.
...
2. Grant Full Disk Access and Automation permissions to the process context that runs `imsg` and MarketingClaw.
