---
summary: "Updating MarketingClaw safely (global install or source), plus rollback strategy"
read_when:
  - Updating MarketingClaw
...
## Recommended: `marketingclaw update`

Detects your install type (npm or git), fetches the latest version, runs `marketingclaw doctor`, and restarts the gateway.
...
marketingclaw update --dry-run   # preview without applying
```

`marketingclaw update` has no `--verbose` flag (the installer does). For diagnostics use
`--dry-run` to preview planned actions, `--json` for structured results, or
`marketingclaw update status --json` to inspect channel and availability state.
...
foreground-only. MarketingClaw reads the public npm `extended-stable` selector,
...
Catalog installs created by current MarketingClaw versions retain that default
intent. Older records that contain only an exact version remain pinned because
MarketingClaw cannot safely distinguish an old automatic pin from a user pin; run
`marketingclaw plugins update @openclaw/name` once on the extended-stable channel
...
credentials, and workspace in `~/.marketingclaw`; it only changes which MarketingClaw
...
```bash
marketingclaw update --channel dev --dry-run
marketingclaw update --channel stable --dry-run
```
...
converting it. If the gateway is already installed, `marketingclaw update` refreshes
the service metadata and restarts it unless you pass `--no-restart`.

For package installs with a managed Gateway service, `marketingclaw update` targets
the package root used by that service. If the shell `marketingclaw` command comes
...
If `marketingclaw update` fails after the npm package install phase, re-run the
...
```bash
curl -fsSL https://marketingclaw.ai/install.sh | bash -s -- --install-method npm
```

Pin the recovery to a specific version or dist-tag with `--version`:

```bash
curl -fsSL https://marketingclaw.ai/install.sh | bash -s -- --install-method npm --version <version-or-dist-tag>
```
...
Prefer `marketingclaw update` for supervised installs: it can coordinate the package
...
For a root-owned Linux system-global install, if `marketingclaw update` fails with
...
marketingclaw gateway status --deep --json
marketingclaw doctor --lint --json
```

When `marketingclaw update` manages a global npm install, it installs the target
...
command fails, MarketingClaw retries once with `--omit=optional`, which helps hosts
where native optional dependencies cannot compile.

MarketingClaw-managed npm update and plugin-update commands also clear npm's
...
explicit MarketingClaw update means "install the selected release now."
...
<AccordionGroup>
  <Accordion title="Read-only package tree">
    MarketingClaw treats packaged global installs as read-only at runtime, even when the global package directory is writable by the current user. Plugin package installs live in MarketingClaw-owned npm/git roots under the user config directory, and Gateway startup does not mutate the MarketingClaw package tree.

    Some Linux npm setups install global packages under root-owned directories such as `/usr/lib/node_modules/marketingclaw`. MarketingClaw supports that layout because plugin install/update commands write outside that global package directory.

  </Accordion>
  <Accordion title="Hardened systemd units">
    Give MarketingClaw write access to its config/state roots so explicit plugin installs, plugin updates, and doctor cleanup can persist their changes:

    ```ini
    ReadWritePaths=/var/lib/marketingclaw /home/marketingclaw/.marketingclaw /tmp
    ```

  </Accordion>
  <Accordion title="Disk-space preflight">
    Before package updates and explicit plugin installs, MarketingClaw tries a best-effort disk-space check for the target volume. Low space produces a warning with the checked path, but does not block the update because filesystem quotas, snapshots, and network volumes can change after the check. The actual package-manager install and post-install verification remain authoritative.
  </Accordion>
</AccordionGroup>

## Auto-updater

Off by default. Enable it in `~/.marketingclaw/marketingclaw.json`:
...
| `dev`             | No automatic apply. Use `marketingclaw update` manually.                                                                                     |
...
For downgrade or incident recovery, set `MARKETINGCLAW_NO_AUTO_UPDATE=1` in the gateway environment to block automatic applies even when `update.auto.enabled` is configured. Startup update hints can still run unless `update.checkOnStart` is also disabled.
...
exits, and lets the normal `marketingclaw update --yes --json` CLI path stop the
...
```bash
npm i -g marketingclaw@<version>
marketingclaw doctor
marketingclaw gateway restart
```

<Tip>
`npm view marketingclaw version` shows the current published version.
</Tip>
...
- Run `marketingclaw doctor` again and read the output carefully.
- For `marketingclaw update --channel dev` on source checkouts, the updater auto-bootstraps `pnpm` when needed. If you see a pnpm/corepack bootstrap error, install `pnpm` manually (or re-enable `corepack`) and rerun the update.
