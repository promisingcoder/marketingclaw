---
summary: "Host MarketingClaw on a DigitalOcean Droplet"
read_when:
  - Setting up MarketingClaw on DigitalOcean
  - Looking for a simple paid VPS for MarketingClaw
title: "DigitalOcean"
---

Run a persistent MarketingClaw Gateway on a DigitalOcean Droplet (~$6/month for the 1 GB Basic plan).
...
<Steps>
  ...
  <Step title="Connect and install">
    ...
    # Create the non-root user that will own MarketingClaw state and services.
    adduser marketingclaw
    usermod -aG sudo marketingclaw
    loginctl enable-linger marketingclaw

    su - marketingclaw
    marketingclaw --version
    ```

    Use the root shell only for system bootstrap. Run MarketingClaw commands as the non-root `marketingclaw` user so state lives under `/home/marketingclaw/.marketingclaw/` and the Gateway installs as that user's systemd `--user` service.
  ...
  <Step title="Access the Control UI">
    ...
    **Option C: Tailnet bind (no Serve)**
    ...
    ```bash
    marketingclaw config set gateway.bind tailnet
    marketingclaw gateway restart
    ```
...
- `~/.marketingclaw/` -- `marketingclaw.json`, channel/provider credentials, per-agent `auth-profiles.json`, and session data.
...
DigitalOcean snapshots back up the whole Droplet; `marketingclaw backup create` is portable across hosts.
