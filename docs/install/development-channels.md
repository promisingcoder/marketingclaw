...
MarketingClaw ships four update channels:
...
| `extended-stable` | resolves the public npm `extended-stable` selector, verifies the exact selected package, and installs that exact version. Fails closed with no fallback to `latest`, `beta`, or `dev`. | unsupported: MarketingClaw leaves the checkout unchanged and asks you to use a package installation                                                                |
...
For `dev` git installs, the default checkout is `~/marketingclaw` (or
...
marketingclaw update --tag 2026.4.1-beta.1
...
marketingclaw update --tag openclaw@2026.4.1-beta.1
...
- `--tag main` maps to the npm-compatible spec `github:marketingclaw/marketingclaw#main`
  ...
  `curl -fsSL https://marketingclaw.ai/install.sh | bash -s -- --install-method git --version main`.
...
- Downgrade protection: if the target version is older than the current
  version, MarketingClaw prompts for confirmation (skip with `--yes`).
...
Preview what `marketingclaw update` would do without making changes:

```bash
marketingclaw update --dry-run
marketingclaw update --channel beta --dry-run
marketingclaw update --tag 2026.4.1-beta.1 --dry-run
marketingclaw update --dry-run --json
```
...
Switching channels with `marketingclaw update` also syncs plugin sources:
