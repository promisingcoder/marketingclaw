---
summary: "Move from Hermes to MarketingClaw with a previewed, reversible import"
read_when:
  - You are coming from Hermes and want to keep your model config, prompts, memory, and skills
  - You want to know what MarketingClaw imports automatically and what stays archive-only
  - You need a clean, scripted migration path (CI, fresh laptop, automation)
title: "Migrating from Hermes"
---

The bundled Hermes migration provider detects state at `~/.hermes`, previews every change before applying, redacts secrets in plans and reports, and writes a verified MarketingClaw backup before it touches anything.

<Note>
Imports require a fresh MarketingClaw setup. If you already have local MarketingClaw state, reset config, credentials, sessions, and the workspace first, or use `marketingclaw migrate apply hermes` directly with `--overwrite` after reviewing the plan.
</Note>
...
<Tabs>
  ...
  <Tab title="CLI">
    ...
    ```bash
    marketingclaw migrate hermes --dry-run    # preview only
    marketingclaw migrate apply hermes --yes  # apply with confirmation skipped
    ```
...
<AccordionGroup>
  ...
  <Accordion title="Workspace files">
    - `SOUL.md` and `AGENTS.md` are copied into the MarketingClaw agent workspace.
    - `memories/MEMORY.md` and `memories/USER.md` are **appended** to the matching MarketingClaw memory files instead of overwriting them.

  </Accordion>
  <Accordion title="Memory configuration">
    Memory config defaults for MarketingClaw file memory. External memory providers such as Honcho are recorded as archive or manual-review items so you can move them deliberately.
  </Accordion>
...
MarketingClaw refuses to execute or trust this state automatically because formats and trust assumptions can drift between systems. Move what you need by hand after reviewing the archive.
...
<Steps>
  ...
  <Step title="Apply with backup">
    ...
    MarketingClaw creates and verifies a backup before applying. This non-interactive example imports non-secret state only. Run without `--yes` to answer the credential prompt interactively, or add `--include-secrets` to include supported credentials in an unattended run.
...
Interactive `marketingclaw migrate` asks whether to import detected auth credentials, with yes selected by default.
...
```bash
marketingclaw migrate hermes --dry-run --json
marketingclaw migrate apply hermes --json --yes
```
...
<AccordionGroup>
  ...
  <Accordion title="Onboarding refuses to import on an existing setup">
    Onboarding imports require a fresh setup. Either reset state and re-onboard, or use `marketingclaw migrate apply hermes` directly, which supports `--overwrite` and explicit backup control.
  </Accordion>
  <Accordion title="API keys did not import">
    Interactive `marketingclaw migrate` imports API keys only when you accept the credential prompt. Non-interactive `--yes` runs need `--include-secrets`; onboarding imports need `--import-secrets`. Only the [supported `.env` keys](/cli/migrate#supported-env-keys) are recognized — other `.env` variables are ignored.
  </Accordion>
</AccordionGroup>
...
- [Migrating](/install/migrating): move an MarketingClaw install between machines.
