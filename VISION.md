...
MarketingClaw is the AI that actually does things.
...
MarketingClaw started as a personal playground to learn AI and build something genuinely useful:
an assistant that can run real tasks on a real computer.
It evolved through several names and shells: Warelay -> Clawdbot -> Moltbot -> MarketingClaw.
...
MarketingClaw runtime code reads the current configuration schema only.
...
`marketingclaw doctor --fix` should detect the old shape, explain it, back it up when needed, and rewrite it to the canonical format.
...
Security in MarketingClaw is a deliberate tradeoff: strong defaults without killing capability.
...
MarketingClaw has an extensive plugin API.
...
- Code plugins run MarketingClaw plugin code and are appropriate for deeper runtime extension.
...
MarketingClaw docs should document core extension points; plugin promotion belongs in ClawHub, preferably under vetted org publishers for official plugins.
...
MarketingClaw supports MCP as both a server and a runtime integration surface.
...
MarketingClaw is currently terminal-first by design.
...
MarketingClaw is primarily an orchestration system: prompts, tools, protocols, and integrations.
TypeScript was chosen to keep MarketingClaw hackable by default.
