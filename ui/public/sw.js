// MarketingClaw Control – Service Worker
// Handles offline caching and push notifications.

const CACHE_PREFIX = "marketingclaw-control-";
...
const CACHE_VERSION =
  (EMBEDDED_CACHE_VERSION !== "__MARKETINGCLAW_CONTROL_UI_BUILD_ID__"
...
self.addEventListener("push", (event) => {
  ...
  try {
    ...
    data = { title: "MarketingClaw", body: event.data.text() };
  }

  const title = data.title || "MarketingClaw";
  const options = {
    body: data.body || "",
    icon: "./apple-touch-icon.png",
    badge: "./favicon-32.png",
    tag: data.tag || "marketingclaw-notification",
    data: { url: data.url || "./" },
  };
