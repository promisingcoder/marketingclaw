...
function createAssistantCanvasBlock(params: {
  ...
  const url = params.url ?? `/__marketingclaw__/canvas/documents/${viewId}/index.html`;
...
function clearDeleteConfirmSkip() {
  localStorageValues.delete("marketingclaw:skipDeleteConfirm");
}
...
describe("grouped chat rendering", () => {
  ...
  it("renders allowed transcript and content image variants", async () => {
    ...
    const fetchMock = vi.fn(async (url: string, init?: RequestInit) => {
      ...
      expect(mediaUrl.pathname).toBe("/marketingclaw/__marketingclaw__/assistant-media");
      expect([...mediaUrl.searchParams.keys()].toSorted()).toEqual(["meta", "source"]);
      expect(mediaUrl.searchParams.get("meta")).toBe("1");
      expect(mediaUrl.searchParams.get("source")).toMatch(/^\/tmp\/marketingclaw\/.+\.(png|jpg)$/u);
    ...
    const renderUserMedia = (message: unknown) => {
      ...
      const renderMessage = () =>
        renderGroupedMessage(container, message, "user", {
          ...
          localMediaPreviewRoots: ["/tmp/marketingclaw"],
    ...
    expect(
      ...
      "/marketingclaw/__marketingclaw__/assistant-media?source=%2Ftmp%2Fmarketingclaw%2Fuser-upload.png&mediaTicket=ticket-user",
    );
    ...
    expect(
      ...
      "/marketingclaw/__marketingclaw__/assistant-media?source=%2Ftmp%2Fmarketingclaw%2Ffirst.png&mediaTicket=ticket-user",
      "/marketingclaw/__marketingclaw__/assistant-media?source=%2Ftmp%2Fmarketingclaw%2Fsecond.jpg&mediaTicket=ticket-user",
    ]);
    ...
    expect(pairingQrImage?.getAttribute("alt")).toBe("MarketingClaw pairing QR code");
    ...
    container = renderUserMedia({
      ...
      role: "user",
      content: "",
      MediaPath: "/__marketingclaw__/media/user-upload.pdf",
    ...
    expect(documentLink?.getAttribute("href")).toBe("/__marketingclaw__/media/user-upload.pdf");
    vi.unstubAllGlobals();
  });
  ...
  it("renders canonical inbound transcript images through the authenticated media route", async () => {
    ...
    expect(
      ...
      "/marketingclaw/__marketingclaw__/assistant-media?source=media%3A%2F%2Finbound%2Ftelegram-photo.png&mediaTicket=ticket-inbound",
    );
    vi.unstubAllGlobals();
  });
  ...
  it("fetches managed chat images with auth and renders blob previews", async () => {
    ...
    const fetchMock = vi.fn(async (_url: string, init?: RequestInit) => {
      const headers = init?.headers as Headers;
      expect(headers.get("Authorization")).toBe("Bearer session-token");
      expect(headers.get("x-marketingclaw-requester-session-key")).toBe("agent:main:main");
      return {
        ok: true,
  ...
  it("renders verified local assistant attachments through the Control UI media route", async () => {
    ...
    const renderMessage = () =>
      renderAssistantMessage(
        container,
        {
          ...
          role: "assistant",
          content:
            "Local image\nMEDIA:/tmp/marketingclaw/test image.png\nMEDIA:/tmp/marketingclaw/test-doc.pdf",
          timestamp: Date.now(),
        },
        {
          showToolCalls: false,
    ...
    const [, fetchInit] = requireFetchCallForUrl(
      fetchMock,
      "/marketingclaw/__marketingclaw__/assistant-media?source=%2Ftmp%2Fmarketingclaw%2Ftest+image.png&meta=1",
    );
    expectSameOriginGet(fetchInit);
    ...
    expect(image?.getAttribute("src")).toBe(
      "/marketingclaw/__marketingclaw__/assistant-media?source=%2Ftmp%2Fmarketingclaw%2Ftest+image.png&mediaTicket=ticket-local",
    );
    expect(docLink?.getAttribute("href")).toBe(
      "/marketingclaw/__marketingclaw__/assistant-media?source=%2Ftmp%2Fmarketingclaw%2Ftest-doc.pdf&mediaTicket=ticket-local",
    );
  ...
  it("refreshes cached local assistant media tickets before they expire without another render", async () => {
    ...
    expect(
      ...
      "/marketingclaw/__marketingclaw__/assistant-media?source=%2Ftmp%2Fmarketingclaw%2Ftest+image.png&mediaTicket=ticket-old",
    );
    ...
    expect(
      ...
      "/marketingclaw/__marketingclaw__/assistant-media?source=%2Ftmp%2Fmarketingclaw%2Ftest+image.png&mediaTicket=ticket-new",
    );
    vi.useRealTimers();
    vi.unstubAllGlobals();
  });
  ...
  it("rechecks local assistant attachment availability when the auth token changes", async () => {
    ...
    expect(image.getAttribute("src")).toBe(
      "/marketingclaw/__marketingclaw__/assistant-media?source=%2Ftmp%2Fmarketingclaw%2Ftest+image.png&mediaTicket=ticket-fresh",
    );
  ...
  it("preserves same-origin assistant attachments without local preview rewriting", () => {
    ...
    renderAssistantMessage(
      container,
      {
        ...
        role: "assistant",
        content:
          "Inline\nMEDIA:/media/inbound/test-image.png\nMEDIA:/__marketingclaw__/media/test-doc.pdf",
        timestamp: Date.now(),
      },
      {
        showToolCalls: false,
    ...
    expect(docLink?.getAttribute("href")).toBe("/__marketingclaw__/media/test-doc.pdf");
    expect(container.querySelector(".chat-assistant-attachment-badge")).toBeNull();
  ...
  it("allows platform-specific local assistant attachments inside preview roots", async () => {
    ...
    const cases = [
      renderCase({
        roots: ["C:\\tmp\\marketingclaw"],
        message: {
          id: "assistant-windows-file-url",
          role: "assistant",
          content: "Windows image\nMEDIA:file:///C:/tmp/marketingclaw/test%20image.png",
          timestamp: Date.now(),
        },
        expectedUrl:
          "/marketingclaw/__marketingclaw__/assistant-media?source=%2FC%3A%2Ftmp%2Fmarketingclaw%2Ftest%2520image.png&meta=1",
      }),
      renderCase({
        ...
        expectedUrl:
          "/marketingclaw/__marketingclaw__/assistant-media?source=C%3A%5CUsers%5CTest%5CPictures%5Ctest+image.png&meta=1",
      }),
      renderCase({
        ...
        expectedUrl:
          "/marketingclaw/__marketingclaw__/assistant-media?source=%7E%2FPictures%2Ftest+image.png&meta=1",
      }),
    ];
  ...
  it("revalidates cached unavailable local assistant attachments after retry window", async () => {
    ...
    expect(
      ...
      "/marketingclaw/__marketingclaw__/assistant-media?source=%2Ftmp%2Fmarketingclaw%2Ftest+image.png&mediaTicket=ticket-retry",
    );
    expect(container.querySelector(".chat-assistant-attachment-badge")).toBeNull();
  ...
  it("routes inline canvas blocks through the scoped canvas host when available", () => {
    ...
    renderAssistantMessage(
        ...
        content: [
          { type: "text", text: "Rendered inline." },
          {
            ...
            preview: {
              ...
              url: "/__marketingclaw__/canvas/documents/cv_inline_scoped/index.html",
              preferredHeight: 320,
            },
          },
        ],
      ...
      {
        canvasPluginSurfaceUrl: "http://127.0.0.1:19003/__marketingclaw__/cap/cap_123",
      },
    );

    const iframe = container.querySelector(".chat-tool-card__preview-frame");
    expect(iframe?.getAttribute("src")).toBe(
      "http://127.0.0.1:19003/__marketingclaw__/cap/cap_123/__marketingclaw__/canvas/documents/cv_inline_scoped/index.html",
    );
  });
  ...
  it("renders server-history canvas blocks for the live toolResult sequence after history reload", () => {
        ...
        content: [
          ...
          { type: "text", text: "This item is ready." },
          {
            ...
            preview: {
              ...
              url: "/__marketingclaw__/canvas/documents/cv_canvas_live_history/index.html",
              preferredHeight: 420,
            },
            rawText: JSON.stringify({
              kind: "canvas",
    ...
    expect(iframe.getAttribute("src")).toBe(
      "/__marketingclaw__/canvas/documents/cv_canvas_live_history/index.html",
    );
  ...
  it("keeps lifted assistant canvas previews beside flat tool rows", () => {
    ...
    const iframe = expectElement(container, ".chat-tool-card__preview-frame", HTMLIFrameElement);
    expect(iframe.getAttribute("src")).toBe(
      "/__marketingclaw__/canvas/documents/cv_inline_tool_canvas/index.html",
    );
  ...
  it("renders hidden assistant_message canvas results with the configured sandbox", () => {
    ...
    expect(iframe.getAttribute("src")).toBe(
      "/__marketingclaw__/canvas/documents/cv_inline_default/index.html",
    );
  ...
  it("renders assistant_message canvas results in the assistant bubble even when tool rows are visible", () => {
        ...
        createMessageGroup(
          {
            ...
            content: JSON.stringify({
              ...
              view: {
                ...
                url: "/__marketingclaw__/canvas/documents/cv_inline_visible/index.html",
                title: "Inline demo",
    ...
    const iframe = expectElement(bubble, ".chat-tool-card__preview-frame", HTMLIFrameElement);
    expect(iframe.getAttribute("src")).toBe(
      "/__marketingclaw__/canvas/documents/cv_inline_visible/index.html",
    );
  ...
  it("adds a full-message request when opening a truncated assistant message", () => {
    ...
    renderAssistantMessage(
      container,
      {
        ...
        __marketingclaw: { id: "msg-truncated-1", seq: 1 },
      },
      {
        sessionKey: "global",
        agentId: "work",
  ...
  it("does not add a full-message request for non-truncated assistant messages", () => {
    ...
    renderAssistantMessage(
      container,
      {
        ...
        __marketingclaw: { id: "msg-visible-1", seq: 1 },
      },
      {
        sessionKey: "global",
        agentId: "work",
  ...
  it("does not add a full-message request for mirrored message-tool replies", () => {
    ...
    renderAssistantMessage(
      container,
      {
        ...
        marketingclawMessageToolMirror: { toolName: "message", toolCallId: "call-1" },
        __marketingclaw: { id: "msg-tool-result", seq: 2, truncated: true },
      },
      {
        sessionKey: "global",
        agentId: "work",
