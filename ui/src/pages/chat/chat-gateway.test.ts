...
describe("handleChatEvent", () => {
  ...
  it("keeps repeated assistant final text within the same turn", () => {
    ...
    const secondAssistant = {
      ...
      content: [
        { type: "text", text: "OK" },
        { type: "canvas", url: "/__marketingclaw__/canvas/documents/repeat/index.html" },
      ],
      timestamp: 3,
    };
    const state = createState({
...
describe("loadChatHistory filtering", () => {
    ...
    const messages = [
      { role: "user", content: [{ type: "text", text: "hello" }] },
      {
        ...
        content: [
          {
            type: "text",
            text: "[marketingclaw] missing tool result in session history; inserted synthetic error result for transcript repair.",
          },
        ],
      },
      {
  ...
  it("keeps image-only user messages that carry transcript media paths", async () => {
    const messages = [
      { role: "user", content: "", MediaPath: "/tmp/marketingclaw/user-upload.png" },
      {
        role: "user",
        content: "",
