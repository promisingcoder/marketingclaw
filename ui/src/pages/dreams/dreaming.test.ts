...
describe("dreaming controller", () => {
  ...
  it("reads dreaming enabled state from the selected memory slot plugin", () => {
    expect(
      ...
      pluginId: "memos-local-marketingclaw-plugin",
      enabled: true,
    });
  });
  ...
  it("repairs dreaming artifacts and reloads only dreaming status", async () => {
    ...
    request.mockImplementation(async (method: string) => {
      if (method === "doctor.memory.repairDreamingArtifacts") {
        return {
          ...
          changed: true,
          archiveDir: "/tmp/marketingclaw/.marketingclaw-repair/dreaming/2026-04-11T22-10-00-000Z",
    ...
    expect(state.dreamDiaryActionMessage).toEqual({
      kind: "success",
      text: "Dream cache repair complete: archived session corpus, archived ingestion state. Archive: /tmp/marketingclaw/.marketingclaw-repair/dreaming/2026-04-11T22-10-00-000Z",
    });
    expect(state.dreamDiaryActionArchivePath).toBe(
      "/tmp/marketingclaw/.marketingclaw-repair/dreaming/2026-04-11T22-10-00-000Z",
    );
    expect(state.dreamDiaryActionLoading).toBe(false);
  });
