...
import type { MarketingClawModalDialog } from "./modal-dialog.ts";
...
function expectShadowElement(modal: MarketingClawModalDialog, id: string): HTMLElement {
...
describe("marketingclaw-modal-dialog", () => {
  beforeEach(() => {
  ...
  it("opens a labelled modal dialog with an optional description", async () => {
    ...
    expect(labelId).toBe("marketingclaw-modal-dialog-label");
    expect(descriptionId).toBe("marketingclaw-modal-dialog-description");
    expect(dialog.getRootNode()).toBe(modal.shadowRoot);
    expect(dialog.ownerDocument.querySelector(`#${labelId}`)).toBeNull();
    expect(expectShadowElement(modal, "marketingclaw-modal-dialog-label").textContent).toBe(
      "Confirm action",
    );
    expect(expectShadowElement(modal, "marketingclaw-modal-dialog-description").textContent).toBe(
