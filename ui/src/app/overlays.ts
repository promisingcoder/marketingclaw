  ...
  const guidance =
    {
      ...
      "not-git-install":
        "Not a git checkout. Run `marketingclaw update` from the CLI for a global reinstall.",
      "not-marketingclaw-root":
        "Run the update from an MarketingClaw checkout or use the CLI global reinstall path.",
      ...
      "doctor-failed":
        "Doctor repair failed. Run `marketingclaw doctor --non-interactive` and retry.",
...
function resolvePendingUpdateHandoffTimeoutBanner(): ApplicationStatusBanner {
  return {
    tone: "danger",
    text: "Update handoff started, but completion was not reported after reconnect. Run `marketingclaw update status` for the final result.",
  };
}
