...
export async function startControlUiE2eServer(): Promise<ControlUiE2eServer> {
  ...
  const server = await createServer({
    ...
    configFile: false,
    define: {
      MARKETINGCLAW_CONTROL_UI_BUILD_ID: JSON.stringify("e2e"),
    },
    logLevel: "error",
    optimizeDeps: {
      include: [
...
function normalizeScenario(
  ...
  return {
    ...
    assistantName: scenario.assistantName?.trim() || "MarketingClaw",
...
function installControlUiMockGateway(input: {
  ...
  type WindowWithGateway = Window & {
    marketingclawControlUiE2eGateway?: ExposedGateway;
  };
  ...
  (window as WindowWithGateway).marketingclawControlUiE2eGateway = exposed;
  ...
  const emitGatewayEvent = async (event: string, payload?: unknown) => {
    await page.evaluate(
      ({ eventName, eventPayload }) => {
        const gateway = (
          window as Window & {
            marketingclawControlUiE2eGateway?: {
              emit: (event: string, payload?: unknown) => void;
            };
          }
        ).marketingclawControlUiE2eGateway;
